...
static void
do_scroll(int sx, int sy, Bool do_send_message, Bool is_message_recieved)
{
	static int psx		 = 0;
	static int psy		 = 0;
	...
	if ((do_send_message || messages_sent < MAX_UNPROCESSED_MESSAGES) &&
		...
		if (monitor_to_track != NULL)
			snprintf(screen, sizeof(screen), "screen %s",
			    monitor_to_track);
		snprintf(command, sizeof(command), "Scroll %s %dp %dp", screen,
		    psx, psy);
...
static int
_pred_weed_window_events(Display *display, XEvent *current_event, XPointer arg)
{
...
static void
discard_events(long event_type, Window w, XEvent *last_ev)
{
...
static rectangle
CalcGeom(PagerWindow *t, bool is_icon)
{
...
initialize_viz_pager(void)
{
	...
	if (Pdefault) {
		...
		attr.border_pixel      = 0;
		attr.colormap	       = Pcmap;
		Scr.Pager_w  = XCreateWindow(dpy, Scr.Root, -10, -10, 10, 10, 0,
	...
	Scr.MiniIconGC	= fvwmlib_XCreateGC(dpy, Scr.Pager_w, GCPlaneMask,
	     &xgcv);
...
void
draw_desk_background(int i, int page_w, int page_h)
{
	if (Desks[i].colorset > -1) {
		XSetWindowBorder(dpy, Desks[i].title_w,
		    Colorset[Desks[i].colorset].fg);
		XSetWindowBorder(dpy, Desks[i].w,
		    Colorset[Desks[i].colorset].fg);
		XSetForeground(dpy, Desks[i].NormalGC,
		    Colorset[Desks[i].colorset].fg);
		XSetForeground(dpy, Desks[i].DashedGC,
		    Colorset[Desks[i].colorset].fg);
		if (uselabel) {
			if (CSET_IS_TRANSPARENT(Desks[i].colorset)) {
				SetWindowBackground(dpy, Desks[i].title_w,
				    desk_w, label_h,
		...
		if (label_h != 0 && uselabel && !LabelsBelow &&
			...
			SetWindowBackgroundWithOffset(dpy, Desks[i].w, 0,
			    -label_h, desk_w, desk_h + label_h,
			    &Colorset[Desks[i].colorset], Pdepth, Scr.NormalGC,
			    True);
		} else {
			if (CSET_IS_TRANSPARENT(Desks[i].colorset)) {
				SetWindowBackground(dpy, Desks[i].w, desk_w,
				    desk_h, &Colorset[Desks[i].colorset],
	...
	if (Desks[i].highcolorset > -1) {
		XSetForeground(dpy, Desks[i].HiliteGC,
		    Colorset[Desks[i].highcolorset].bg);
		XSetForeground(dpy, Desks[i].rvGC,
		    Colorset[Desks[i].highcolorset].fg);
		if (HilightDesks) {
			SetWindowBackground(dpy, Desks[i].CPagerWin, page_w,
			    page_h, &Colorset[Desks[i].highcolorset], Pdepth,
			    Scr.NormalGC, True);
		}
	}
	if (uselabel) {
...
void
initialize_balloon_window(void)
{
	XGCValues	     xgcv;
	...
	Scr.balloon_w  = XCreateWindow(dpy, Scr.Root, 0, 0,
	     /* coords set later */ 1, 1, BalloonBorderWidth, Pdepth,
...
pagerrec_to_fvwm(rectangle *rec, bool is_icon)
{
	...
	if (monitor_to_track != NULL) {
		offset_x = (m_width - mon->w) *
			((mon->virtual_scr.VxPages * rec->x) / scale_w) +
		    mon->x;
		offset_y = (m_height - mon->h) *
			((mon->virtual_scr.VyPages * rec->y) / scale_h) +
		    mon->y;
		m_width	 = mon->w;
		m_height = mon->h;
	}
...
set_vp_size_and_loc(void)
{
	...
	vp.x	 = (mon->virtual_scr.Vx * desk_w) / mon->virtual_scr.VWidth;
	vp.y	 = (mon->virtual_scr.Vy * desk_h) / mon->virtual_scr.VHeight;
	vp.width = ((mon->virtual_scr.Vx +
			mon->virtual_scr.VWidth / mon->virtual_scr.VxPages) *
		       desk_w) /
		mon->virtual_scr.VWidth -
	    vp.x;
	vp.height = ((mon->virtual_scr.Vy +
			 mon->virtual_scr.VHeight / mon->virtual_scr.VyPages) *
			desk_h) /
...
initialize_pager(void)
{
	...
	int		  VxPages = mon->virtual_scr.VxPages,
	    VyPages		  = mon->virtual_scr.VyPages;
	...
	*/
	...
	if (Pdepth < 2) {
		...
		Scr.light_gray_pixmap  = XCreatePixmapFromBitmapData(dpy,
		     Scr.Pager_w, l_g_bits, l_g_width, l_g_height, fore_pix,
		     back_pix, Pdepth);
		Scr.sticky_gray_pixmap = XCreatePixmapFromBitmapData(dpy,
		    Scr.Pager_w, s_g_bits, s_g_width, s_g_height, fore_pix,
		    back_pix, Pdepth);
	}
	...
	if (is_transient) {
		...
		FScreenGetScrRect(&fscr, FSCREEN_XYPOS, &screen_g.x,
		...
		if (pwindow.width + pwindow.x > screen_g.x + screen_g.width) {
			pwindow.x = screen_g.x + screen_g.width -
			    mon->w; // mon->virtual_scr.MyDisplayWidth;
			xneg = 1;
		}
		if (pwindow.height + pwindow.y > screen_g.y + screen_g.height) {
			pwindow.y = screen_g.y + screen_g.height -
			    mon->h; // mon->virtual_scr.MyDisplayHeight;
			yneg = 1;
		}
	}
	if (xneg) {
		sizehints.win_gravity = NorthEastGravity;
		pwindow.x = monitor_get_all_widths() - pwindow.width +
		    pwindow.x;
	}
	if (yneg) {
		pwindow.y = monitor_get_all_heights() - pwindow.height +
		    pwindow.y;
	...
	Scr.Pager_w = XCreateWindow(dpy, Scr.Root, pwindow.x, pwindow.y,
	    pwindow.width, pwindow.height, 0, Pdepth, InputOutput, Pvisual,
	...
	if ((desk1 == desk2) && (Desks[0].label != NULL)) {
		if (FlocaleTextListToTextProperty(dpy, &Desks[0].label, 1,
			XStdICCTextStyle, &name) == 0) {
			fprintf(stderr,
			    "%s: fatal error: cannot allocate desk name",
			    MyName);
			exit(0);
		}
	} else {
		if (FlocaleTextListToTextProperty(dpy, &Desks[0].label, 1,
			XStdICCTextStyle, &name) == 0) {
			fprintf(stderr,
			    "%s: fatal error: cannot allocate pager name",
			    MyName);
			exit(0);
		}
	}
	...
	icon_win    = XCreateWindow(dpy, Scr.Root, pwindow.x, pwindow.y,
	       icon.width, icon.height, 0, Pdepth, InputOutput, Pvisual, valuemask,
	       &attributes);
	...
	if (icon.x != -10000) {
		if (icon_xneg)
			icon.x = mon->virtual_scr.MyDisplayWidth + icon.x -
			    icon.width;
		if (icon.y != -10000) {
			if (icon_yneg)
				icon.y = mon->virtual_scr.MyDisplayHeight +
				    icon.y - icon.height;
		} else {
			icon.y = 0;
		}
	...
	if (windowcolorset >= 0) {
		...
		Scr.whGC = fvwmlib_XCreateGC(dpy, Scr.Pager_w, GCForeground,
		    &gcv);
		gcv.foreground = Colorset[windowcolorset].shadow;
		Scr.wsGC = fvwmlib_XCreateGC(dpy, Scr.Pager_w, GCForeground,
		    &gcv);
	}
	if (activecolorset >= 0) {
		gcv.foreground = Colorset[activecolorset].hilite;
		Scr.ahGC = fvwmlib_XCreateGC(dpy, Scr.Pager_w, GCForeground,
		    &gcv);
		gcv.foreground = Colorset[activecolorset].shadow;
		Scr.asGC = fvwmlib_XCreateGC(dpy, Scr.Pager_w, GCForeground,
		    &gcv);
	}
	...
	for (i = 0; i < ndesks; i++) {
		...
		gcv.foreground = (Desks[i].colorset < 0) ?
		    fore_pix :
		    Colorset[Desks[i].colorset].fg;
		if (uselabel && Ffont && Ffont->font) {
			gcv.font	  = Ffont->font->fid;
			Desks[i].NormalGC = fvwmlib_XCreateGC(dpy, Scr.Pager_w,
		...
		if (Pdepth < 2)
			...
			gcv.foreground = (Desks[i].highcolorset < 0) ?
			    hi_pix :
			    Colorset[Desks[i].highcolorset].bg;
		Desks[i].HiliteGC = fvwmlib_XCreateGC(dpy, Scr.Pager_w,
		    GCForeground, &gcv);
		...
		if (uselabel && Ffont && Ffont->font) {
			Desks[i].rvGC = fvwmlib_XCreateGC(dpy, Scr.Pager_w,
			    GCForeground | GCFont, &gcv);
		} else {
			Desks[i].rvGC = fvwmlib_XCreateGC(dpy, Scr.Pager_w,
			    GCForeground, &gcv);
		...
		gcv.foreground	  = (Desks[i].colorset < 0) ?
		       ...
		       Colorset[Desks[i].colorset].fg;
		gcv.line_width	  = 1;
		gcv.line_style	  = (use_dashed_separators) ? LineOnOffDash :
							      LineSolid;
		Desks[i].DashedGC = fvwmlib_XCreateGC(dpy, Scr.Pager_w,
		    GCForeground | GCLineStyle | GCLineWidth, &gcv);
		if (use_dashed_separators) {
			/* Although this should already be the default for a
			 * freshly created GC, some X servers do not draw
			 * properly dashed lines if the dash style is not set
			 * explicitly. */
			dash_list[0] = 4;
			dash_list[1] = 4;
		...
		if (Desks[i].colorset >= 0 &&
			...
			attributes.background_pixel = (Desks[i].colorset < 0) ?
			    (Desks[i].Dcolor ? GetColor(Desks[i].Dcolor) :
					       back_pix) :
			    Colorset[Desks[i].colorset].bg;
		}

		attributes.border_pixel = (Desks[i].colorset < 0) ?
		    fore_pix :
		    Colorset[Desks[i].colorset].fg;
		attributes.event_mask	= (ExposureMask | ButtonReleaseMask);
		Desks[i].title_w = XCreateWindow(dpy, Scr.Pager_w, vp.x - 1,
		    vp.y - 1, vp.width, vp.height, 1, CopyFromParent,
		...
		Desks[i].w = XCreateWindow(dpy, Desks[i].title_w, vp.x - 1,
		    LabelsBelow ? -1 : label_h - 1, vp.width, desk_h, 1,
		...
		if (HilightDesks) {
			...
			if (Desks[i].highcolorset > -1 &&
				...
				attributes.background_pixel =
				    (Desks[i].highcolorset < 0) ?
				    hi_pix :
				    Colorset[Desks[i].highcolorset].bg;
			}

			Desks[i].CPagerWin = XCreateWindow(dpy, Desks[i].w,
...
UpdateWindowShape(void)
{
	if (FHaveShapeExtension) {
		...
		shape_count = ndesks +
		    ((m->virtual_scr.CurrentDesk < desk1 ||
			 m->virtual_scr.CurrentDesk > desk2) ?
			    0 :
			    1);
		...
		FShapeCombineRectangles(dpy, Scr.Pager_w, FShapeBounding, 0, 0,
		    shape, shape_count, FShapeSet, 0);
		free(shape);
	}
}
...
DispatchEvent(XEvent *Event)
{
	...
	case ConfigureNotify:
		...
		discard_events(ConfigureNotify, Event->xconfigure.window,
		    Event);
		fev_sanitise_configure_notify(&Event->xconfigure);
		if (w != icon_win) {
	...
	case KeyPress:
		if (is_transient) {
			XLookupString(&(Event->xkey), &keychar, 1, &keysym,
			    NULL);
			switch (keysym) {
			case XK_Up:
				dy	     = -100;
		...
		if (Event->xbutton.button == 3) {
			for (i = 0; i < ndesks; i++) {
				if (Event->xany.window == Desks[i].w) {
					if (FQueryPointer(dpy, Desks[i].w,
			...
			if (Event->xany.window == icon_win) {
				if (FQueryPointer(dpy, icon_win, &JunkRoot,
			...
			for (i = 0; i < ndesks; i++) {
				if (Event->xany.window == Desks[i].w) {
					...
					Scroll(x, y,
					    ...
					    False);
					if (mon->virtual_scr.CurrentDesk !=
					    i + desk1) {
						Wait = 0;
						SwitchToDesk(i);
					}
...
ReConfigure(void)
{
	...
	if (!XGetGeometry(dpy, Scr.Pager_w, &root, &vp.x, &vp.y,
		...
		for (j = 0; j < Columns; j++) {
			...
			if (i < ndesks) {
				...
				XMoveResizeWindow(dpy, Desks[i].w, -1,
				    (LabelsBelow) ? -1 : label_h - 1, desk_w,
				    desk_h);
				if (HilightDesks) {
					if (i ==
					    mon->virtual_scr.CurrentDesk -
						...
						XMoveResizeWindow(dpy,
						    Desks[i].CPagerWin, vp.x,
						    vp.y, vp.width, vp.height);
					else
						XMoveResizeWindow(dpy,
						    Desks[i].CPagerWin, -32768,
						    -32768, vp.width,
						    vp.height);
				}
				draw_desk_background(i, vp.width, vp.height);
...
static void
update_pr_transparent_subwindows(int i)
{
	int	     cset;
	rectangle    vp;
	PagerWindow *t;
	...
	if (CSET_IS_TRANSPARENT_PR(Desks[i].highcolorset) && HilightDesks) {
		SetWindowBackground(dpy, Desks[i].CPagerWin, vp.width,
		    vp.height, &Colorset[Desks[i].highcolorset], Pdepth,
		    Scr.NormalGC, True);
	}
	...
	for (t = Start; t != NULL; t = t->next) {
		...
		if (t->PagerView != None) {
			SetWindowBackground(dpy, t->PagerView,
			    t->pager_view_width, t->pager_view_height,
			    &Colorset[cset], Pdepth, Scr.NormalGC, True);
		}
		if (t->IconView) {
			SetWindowBackground(dpy, t->IconView,
			    t->icon_view_width, t->icon_view_height,
			    &Colorset[cset], Pdepth, Scr.NormalGC, True);
		}
	}
}
...
void
update_pr_transparent_windows(void)
{
	int	     i, j, k, cset;
	rectangle    vp;
	PagerWindow *t;
...
MovePage(Bool is_new_desk)
{
	...
	for (i = 0; i < ndesks; i++) {
		if (HilightDesks) {
			if (i == mon->virtual_scr.CurrentDesk - desk1) {
				XMoveResizeWindow(dpy, Desks[i].CPagerWin, vp.x,
				    vp.y, vp.width, vp.height);
				XLowerWindow(dpy, Desks[i].CPagerWin);
				if (CSET_IS_TRANSPARENT(
					Desks[i].highcolorset)) {
					SetWindowBackground(dpy,
					    Desks[i].CPagerWin, vp.width,
					    vp.height,
				...
				XMoveWindow(dpy, Desks[i].CPagerWin, -32768,
				    -32768);
			}
		}
	}
	...
	if (mon->virtual_scr.CurrentDesk != icon_desk_shown) {
		...
		if ((mon->virtual_scr.CurrentDesk >= desk1) &&
		    (mon->virtual_scr.CurrentDesk <= desk2))
			sptr =
			    Desks[mon->virtual_scr.CurrentDesk - desk1].label;
		else {
			snprintf(str, sizeof(str), "GotoDesk %s %d", mon->name,
			    mon->virtual_scr.CurrentDesk);
			sptr = &str[0];
		}

		if (FlocaleTextListToTextProperty(dpy, &sptr, 1,
			XStdICCTextStyle, &name) == 0) {
			return;
		}
...
void
ReConfigureIcons(Bool do_reconfigure_desk_only)
{
	PagerWindow	 *t;
	rectangle	  rec;
	struct fpmonitor *mon = fpmonitor_this();
	...
	for (t = Start; t != NULL; t = t->next) {
		if (do_reconfigure_desk_only &&
		    t->desk != mon->virtual_scr.CurrentDesk)
			continue;
		rec		    = CalcGeom(t, true);
		t->icon_view_x	    = rec.x;
		t->icon_view_y	    = rec.y;
		...
		if ((mon->virtual_scr.CurrentDesk != t->desk) ||
		    (HideSmallWindows &&
			(rec.width == MinSize || rec.height == MinSize))) {
			rec.x = -32768;
			rec.y = -32768;
		}
		XMoveResizeWindow(dpy, t->IconView, rec.x, rec.y, rec.width,
		    rec.height);
	}
}
 ...
 */
void
DrawGrid(int desk, int erase, Window ew, XRectangle *r)
{
	...
	if (!ew || ew == Desks[desk].w) {
		...
		while (x < mon->virtual_scr.VWidth) {
			...
			if (!use_no_separators) {
				XDrawLine(dpy, Desks[desk].w,
				    Desks[desk].DashedGC, x1, y1, x1, y2);
			}
			x += mon->virtual_scr.MyDisplayWidth;
		}
		...
		while (y < mon->virtual_scr.VHeight) {
			...
			if (!use_no_separators) {
				XDrawLine(dpy, Desks[desk].w,
				    Desks[desk].DashedGC, x1, y1, x2, y1);
			}
			y += mon->virtual_scr.MyDisplayHeight;
		}
	}
	...
	if (((mon->virtual_scr.CurrentDesk - desk1) == desk) && !ShapeLabels) {
		if (uselabel) {
			XFillRectangle(dpy, Desks[desk].title_w,
			    Desks[desk].HiliteGC, bound.x, bound.y, bound.width,
			    bound.height);
		}
	} else {
		if (uselabel && erase) {
			XClearArea(dpy, Desks[desk].title_w, bound.x, bound.y,
			    bound.width, bound.height, False);
		}
	}
	...
	if ((w <= desk_w) && (uselabel)) {
		...
		FwinString->y = (LabelsBelow ? desk_h + Ffont->ascent + 1 :
					       Ffont->ascent + 1);
		if (region) {
...
DrawIconGrid(int erase)
{
	...
	if (!use_no_separators) {
		...
		for (i = 1; i < rec.y; i++)
			XDrawLine(dpy, icon_win, Desks[tmp].DashedGC, 0,
			    i * rec.height, icon.width, i * rec.height);
	}

	rec.x = (mon->virtual_scr.Vx * rec.width) /
	    mon->virtual_scr.MyDisplayWidth;
	rec.y = (mon->virtual_scr.Vy * rec.height) /
	    mon->virtual_scr.MyDisplayHeight;
	if (HilightDesks) {
		if (HilightPixmap) {
			XCopyArea(dpy, HilightPixmap->picture, icon_win,
			    Scr.NormalGC, 0, 0, rec.width, rec.height, rec.x,
			    rec.y);
		} else {
...
SwitchToDesk(int Desk)
{
	...
	snprintf(command, sizeof(command), "GotoDesk %s 0 %d",
	    monitor_to_track ? m->name : "", Desk + desk1);
	SendText(fd, command, 0);
}
...
SwitchToDeskAndPage(int Desk, XEvent *Event)
{
	...
	if (mon->virtual_scr.CurrentDesk != (Desk + desk1)) {
		...
		if (desk_w == 0)
			...
			vx = Event->xbutton.x * mon->virtual_scr.VWidth /
			    (desk_w * mon->virtual_scr.MyDisplayWidth);
		if (desk_h == 0)
			vy = 0;
		else
			vy = Event->xbutton.y * mon->virtual_scr.VHeight /
			    (desk_h * mon->virtual_scr.MyDisplayHeight);
		mon->virtual_scr.Vx = vx * mon->virtual_scr.MyDisplayWidth;
		mon->virtual_scr.Vy = vy * mon->virtual_scr.MyDisplayHeight;
		snprintf(command, sizeof(command),
		    "GotoDeskAndPage %s %d %d %d",
		...
		if (desk_w == 0)
			...
			x = Event->xbutton.x * mon->virtual_scr.VWidth /
			    (desk_w * mon->virtual_scr.MyDisplayWidth);
		if (desk_h == 0)
			y = 0;
		else
			y = Event->xbutton.y * mon->virtual_scr.VHeight /
			    (desk_h * mon->virtual_scr.MyDisplayHeight);

		/* Fix for buggy XFree86 servers that report button release
		 * events incorrectly when moving fast. Not perfect, but should
		 * at least prevent that we get a random page. */
		if (x < 0)
			x = 0;
		if (y < 0)
			y = 0;
		if (x * mon->virtual_scr.MyDisplayWidth >
		    mon->virtual_scr.VxMax)
			x = mon->virtual_scr.VxMax /
			    mon->virtual_scr.MyDisplayWidth;
		if (y * mon->virtual_scr.MyDisplayHeight >
		    mon->virtual_scr.VyMax)
			y = mon->virtual_scr.VyMax /
			    mon->virtual_scr.MyDisplayHeight;
		snprintf(command, sizeof(command), "GotoPage %s %d %d",
		    monitor_to_track ? mon->name : "", x, y);
		SendText(fd, command, 0);
	}
	Wait = 1;
}
...
AddNewWindow(PagerWindow *t)
{
	...
	if ((i >= 0) && (i < ndesks)) {
		...
		t->PagerView = XCreateWindow(dpy, Desks[i].w, rec.x, rec.y,
		    rec.width, rec.height, 0, CopyFromParent, InputOutput,
		    CopyFromParent, valuemask, &attributes);
		if (windowcolorset > -1) {
			SetWindowBackground(dpy, t->PagerView, rec.width,
			    rec.height, &Colorset[windowcolorset], Pdepth,
			    Scr.NormalGC, True);
		}
		if (!UseSkipList || !DO_SKIP_WINDOW_LIST(t)) {
			if (IS_ICONIFIED(t)) {
				XMoveResizeWindow(dpy, t->PagerView, -32768,
				    -32768, 1, 1);
			}
	...
	t->IconView = XCreateWindow(dpy, icon_win, rec.x, rec.y, rec.width,
	    rec.height, 0, CopyFromParent, InputOutput, CopyFromParent,
	    valuemask, &attributes);
	if (windowcolorset > -1) {
		SetWindowBackground(dpy, t->IconView, rec.width, rec.height,
	...
	if (!UseSkipList || !DO_SKIP_WINDOW_LIST(t)) {
		if (IS_ICONIFIED(t)) {
			XMoveResizeWindow(dpy, t->IconView, -32768, -32768, 1,
			    1);
		}
...
ChangeDeskForWindow(PagerWindow *t, long newdesk)
{
	...
	if (mon->virtual_scr.CurrentDesk != t->desk)
		XMoveResizeWindow(dpy, t->IconView, -32768, -32768, rec.width,
		    rec.height);
	else {
		int cset;
...
void
MoveResizePagerView(PagerWindow *t, Bool do_force_redraw)
{
	rectangle	  rec;
	Bool		  size_changed;
	...
	rec		     = CalcGeom(t, false);
	position_changed     = (t->pager_view_x != rec.x ||
		t->pager_view_y != rec.y);
	size_changed	     = (t->pager_view_width != rec.width ||
	    t->pager_view_height != rec.height);
	t->pager_view_x	     = rec.x;
	...
	if (t->PagerView != None) {
		if (size_changed || position_changed || do_force_redraw) {
			...
			XMoveResizeWindow(dpy, t->PagerView, rec.x, rec.y,
			    rec.width, rec.height);
			cset = (t != FocusWin) ? windowcolorset :
						 activecolorset;
			if (cset > -1 &&
			    (size_changed || CSET_IS_TRANSPARENT(cset))) {
				SetWindowBackground(dpy, t->PagerView,
				    t->pager_view_width, t->pager_view_height,
				    &Colorset[cset], Pdepth, Scr.NormalGC,
				    True);
			}
		}
...
void
MoveStickyWindow(Bool is_new_page, Bool is_new_desk)
{
	PagerWindow	 *t;
	struct fpmonitor *mon = fpmonitor_this();

	for (t = Start; t != NULL; t = t->next) {
		if (is_new_desk && t->desk != mon->virtual_scr.CurrentDesk &&
...
Hilight(PagerWindow *t, int on)
{
	...
	if (monitor_to_track != NULL &&
	    strcmp(t->m->name, monitor_to_track) != 0)
		return;
...
void
Scroll(int x, int y, int Desk, Bool do_scroll_icon)
{
	...
	/* center around mouse */
	adjx = (window_w /
	    (1 + mon->virtual_scr.VxMax / mon->virtual_scr.MyDisplayWidth));
	adjy = (window_h /
	    (1 + mon->virtual_scr.VyMax / mon->virtual_scr.MyDisplayHeight));
	x -= adjx / 2;
	y -= adjy / 2;
...
MoveWindow(XEvent *Event)
{
	...
	XTranslateCoordinates(dpy, Event->xany.window, t->PagerView,
	    Event->xbutton.x, Event->xbutton.y, &rec.x, &rec.y, &dumwin);
	xi = rec.x;
	yi = rec.y;
	while (!finished) {
		FMaskEvent(dpy,
			...
			if ((IS_ICONIFIED(t) &&
				...
				if (t->desk != mon->virtual_scr.CurrentDesk)
					ChangeDeskForWindow(t,
					    mon->virtual_scr.CurrentDesk);
			} else if (NewDesk + desk1 !=
			    mon->virtual_scr.CurrentDesk) {
				snprintf(command, sizeof(command),
				    "Silent MoveToDesk 0 %d", NewDesk + desk1);
		...
		if (NewDesk >= 0 && NewDesk < ndesks) {
			...
			if (moved) {
				...
				snprintf(buf, sizeof(buf),
				    "Silent Move +%dp +%dp ewmhiwa", rec.x,
				    rec.y);
				SendText(fd, buf, t->w);
				XSync(dpy, 0);
			} else {
...
static void
draw_window_border(PagerWindow *t, Window w, int width, int height)
{
	if (w != None) {
		if (!WindowBorders3d || windowcolorset < 0 ||
		    activecolorset < 0) {
			XSetForeground(dpy, Scr.NormalGC, Scr.black);
			RelieveRectangle(dpy, w, 0, 0, width - 1, height - 1,
...
void
BorderWindow(PagerWindow *t)
{
	draw_window_border(t, t->PagerView, t->pager_view_width,
	    t->pager_view_height);
}

void
BorderIconWindow(PagerWindow *t)
{
	draw_window_border(t, t->IconView, t->icon_view_width,
	    t->icon_view_height);
}

/* draw the window label with simple greedy wrapping */
static void
label_window_wrap(PagerWindow *t)
{
	char *cur, *next, *end;
	...
	while (*cur) {
		while (*next) {
			...
			if (width > t->pager_view_width - cur_width -
		...
		if (cur == next) {
			...
			while (*next) {
				...
				len   = FlocaleStringNumberOfBytes(FwindowFont,
				      next);
				width = FlocaleTextWidth(FwindowFont, next,
				    len);
...
static void
do_label_window(PagerWindow *t, Window w)
{
	int cs;
...
static void
do_picture_window(PagerWindow *t, Window w, int width, int height)
{
	int		     iconX;
	int		     iconY;
	int		     src_x = 0;
	...
	if (MiniIcons) {
		if (t->mini_icon.picture && w != None) {
			...
			PGraphicsRenderPicture(dpy, w, &t->mini_icon, &fra, w,
			    ...
			    t->mini_icon.height - src_y, iconX, iconY, dest_w,
			    dest_h, False);
		}
	}
}
...
{
	do_picture_window(t, t->PagerView, t->pager_view_width,
	    t->pager_view_height);
}
...
{
	do_picture_window(t, t->IconView, t->icon_view_width,
	    t->icon_view_height);
}
...
IconMoveWindow(XEvent *Event, PagerWindow *t)
{
	...
	if (moved &&
	    (abs(x - xi) < MoveThreshold && abs(y - yi) < MoveThreshold))
		moved = 0;
	...
	if (KeepMoving) {
		...
		if (moved) {
			char buf[64];
			snprintf(buf, sizeof(buf),
			    "Silent Move +%dp +%dp ewmhiwa", rec.x, rec.y);
			SendText(fd, buf, t->w);
			XSync(dpy, 0);
		} else {
			MoveResizePagerView(t, True);
...
setup_balloon_window(int i)
{
	...
	if (Desks[i].ballooncolorset < 0) {
		...
		xswa.background_pixel = (!BalloonBack) ? back_pix :
...
void
setup_balloon_gc(int i, PagerWindow *t)
{
	XGCValues     xgcv;
	unsigned long valuemask;
	...
	if (Desks[i].ballooncolorset < 0) {
		xgcv.foreground = (!BalloonFore) ? fore_pix :
...
/* Just maps window ... draw stuff in it later after Expose event
   -- ric@giccs.georgetown.edu */
void
MapBalloonWindow(PagerWindow *t, Bool is_icon_view)
{
	...
	new_g.width  = 4;
	if (Scr.balloon_label) {
		new_g.width += FlocaleTextWidth(Desks[i].balloon.Ffont,
		    Scr.balloon_label, strlen(Scr.balloon_label));
	}
	...
	XTranslateCoordinates(dpy, view, Scr.Root, x, y, &new_g.x, &new_g.y,
	    &dummy);
	...
	if (new_g.y < 2) {
		...
		XTranslateCoordinates(dpy, view, Scr.Root, x, y, &new_g.x,
		    &new_g.y, &dummy);
	}
	...
	XMoveResizeWindow(dpy, Scr.balloon_w, new_g.x, new_g.y, new_g.width,
	    new_g.height);
	...
	if (Desks[i].ballooncolorset > -1) {
		SetWindowBackground(dpy, Scr.balloon_w, new_g.width,
		    new_g.height, &Colorset[Desks[i].ballooncolorset], Pdepth,
		    Scr.NormalGC, True);
		XSetWindowBorder(dpy, Scr.balloon_w,
...
static void
set_window_colorset_background(PagerWindow *t, colorset_t *csetp)
{
	if (t->PagerView != None) {
		SetWindowBackground(dpy, t->PagerView, t->pager_view_width,
		    t->pager_view_height, csetp, Pdepth, Scr.NormalGC, True);
	}
	SetWindowBackground(dpy, t->IconView, t->icon_view_width,
	    t->icon_view_height, csetp, Pdepth, Scr.NormalGC, True);
...
change_colorset(int colorset)
{
	...
	for (i = 0; i < ndesks; i++) {
		if (Desks[i].highcolorset == colorset) {
			...
			if (HilightDesks) {
				if (uselabel &&
				    (i == mon->virtual_scr.CurrentDesk)) {
					SetWindowBackground(dpy,
					    Desks[i].title_w, desk_w, desk_h,
					    &Colorset[colorset], Pdepth,
					    Scr.NormalGC, True);
				}
				SetWindowBackground(dpy, Desks[i].CPagerWin, 0,
				    0, &Colorset[colorset], Pdepth,
				    Scr.NormalGC, True);
				XLowerWindow(dpy, Desks[i].CPagerWin);
			}
		}
		...
		if (Desks[i].colorset == colorset) {
			...
			if (uselabel) {
				SetWindowBackground(dpy, Desks[i].title_w,
				    desk_w, desk_h + label_h,
				    &Colorset[colorset], Pdepth, Scr.NormalGC,
				    True);
			}
			if (label_h != 0 && uselabel && !LabelsBelow &&
			    !CSET_IS_TRANSPARENT_PR(Desks[i].colorset)) {
				SetWindowBackgroundWithOffset(dpy, Desks[i].w,
				    0, -label_h, desk_w, desk_h + label_h,
				    &Colorset[colorset], Pdepth, Scr.NormalGC,
				    True);
			} else {
				SetWindowBackground(dpy, Desks[i].w, desk_w,
	...
	if (ShowBalloons &&
	    Desks[Scr.balloon_desk].ballooncolorset == colorset) {
