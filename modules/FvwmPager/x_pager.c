...
int	  Wait = 0;
int
	    FvwmErrorHandler(Display *, XErrorEvent *);
...
static void
CalcGeom(PagerWindow *t, int win_w, int win_h, int *x_ret, int *y_ret,
    int *w_ret, int *h_ret)
{
	...
	if (monitor_to_track == NULL) {
		...
		ywin = page * m->h
		       + min(m->h, max(0, ywin % m->virtual_scr.MyDisplayHeight
					      - m->y));
	}
...
initialize_viz_pager(void)
{
	...
	Scr.MiniIconGC =
	    fvwmlib_XCreateGC(dpy, Scr.Pager_w, GCPlaneMask, &xgcv);
...
draw_desk_background(int i, int page_w, int page_h)
{
	...
	if (Desks[i].highcolorset > -1) {
		XSetForeground(
		    dpy, Desks[i].HiliteGC, Colorset[Desks[i].highcolorset].bg);
		XSetForeground(
	...
	*/
	...
	if (Pdepth < 2) {
		...
		Scr.light_gray_pixmap =
		    XCreatePixmapFromBitmapData(dpy, Scr.Pager_w, l_g_bits,
			l_g_width, l_g_height, fore_pix, back_pix, Pdepth);
		Scr.sticky_gray_pixmap =
		    XCreatePixmapFromBitmapData(dpy, Scr.Pager_w, s_g_bits,
			s_g_width, s_g_height, fore_pix, back_pix, Pdepth);
	}

	n = mon->virtual_scr.VxMax
	    / (mon->x + mon->w); // mon->virtual_scr.MyDisplayWidth;
	m = mon->virtual_scr.VyMax
	    / (mon->y + mon->h); // mon->virtual_scr.MyDisplayHeight;
	...
	if (window_w > 0) {
		window_w =
		    (window_w - sizehints.base_width) / sizehints.width_inc;
		window_w =
		    window_w * sizehints.width_inc + sizehints.base_width;
	} else {
		window_w = Columns * (vWidth / Scr.VScale + n) + Columns - 1;
	}
	if (window_h > 0) {
		window_h =
		    (window_h - sizehints.base_height) / sizehints.height_inc;
		window_h =
		    window_h * sizehints.height_inc + sizehints.base_height;
	} else {
	...
	if (is_transient) {
		...
		if (window_w + window_x > screen_g.x + screen_g.width) {
			window_x = screen_g.x + screen_g.width
	...
	Scr.Pager_w = XCreateWindow(dpy, Scr.Root, window_x, window_y, window_w,
	    window_h, 0, Pdepth, InputOutput, Pvisual, valuemask, &attributes);
	...
	if ((desk1 == desk2) && (Desks[0].label != NULL)) {
		if (FlocaleTextListToTextProperty(
			dpy, &Desks[0].label, 1, XStdICCTextStyle, &name)
		    == 0) {
			fprintf(stderr,
	...
	icon_win = XCreateWindow(dpy, Scr.Root, window_x, window_y, icon_w,
	    icon_h, 0, Pdepth, InputOutput, Pvisual, valuemask, &attributes);
	...
	if (icon_x != -10000) {
		if (icon_xneg)
			icon_x =
			    mon->virtual_scr.MyDisplayWidth + icon_x - icon_w;
		if (icon_y != -10000) {
			if (icon_yneg)
				icon_y = mon->virtual_scr.MyDisplayHeight
					 + icon_y - icon_h;
		} else {
			icon_y = 0;
		}
	...
	for (i = 0; i < ndesks; i++) {
		...
		if (uselabel && Ffont && Ffont->font) {
			...
			Desks[i].NormalGC = fvwmlib_XCreateGC(
		...
		if (uselabel && Ffont && Ffont->font) {
			Desks[i].rvGC = fvwmlib_XCreateGC(
			    dpy, Scr.Pager_w, GCForeground | GCFont, &gcv);
		} else {
			Desks[i].rvGC = fvwmlib_XCreateGC(
			    dpy, Scr.Pager_w, GCForeground, &gcv);
		...
		gcv.line_width = 1;
		gcv.line_style =
		    (use_dashed_separators) ? LineOnOffDash : LineSolid;
		...
		Desks[i].title_w = XCreateWindow(dpy, Scr.Pager_w, x - 1, y - 1,
		    w, h, 1, CopyFromParent, InputOutput, CopyFromParent,
		...
		Desks[i].w = XCreateWindow(dpy, Desks[i].title_w, x - 1,
		    LabelsBelow ? -1 : label_h - 1, w, desk_h, 1,
		...
		if (HilightDesks) {
			...
			h = desk_h / (m + 1);
			Desks[i].CPagerWin =
			    XCreateWindow(dpy, Desks[i].w, -32768, -32768, w, h,
...
DispatchEvent(XEvent *Event)
{
	...
	case ConfigureNotify:
		...
		discard_events(
		    ConfigureNotify, Event->xconfigure.window, Event);
		fev_sanitise_configure_notify(&Event->xconfigure);
		if (w != icon_win) {
	...
	case KeyPress:
		if (is_transient) {
			XLookupString(
			    &(Event->xkey), &keychar, 1, &keysym, NULL);
			switch (keysym) {
			case XK_Up:
				dy	     = -100;
				do_move_page = True;
...
ReConfigure(void)
{
	...
	if (!XGetGeometry(dpy, Scr.Pager_w, &root, &x, &y,
		(unsigned *)&window_w, (unsigned *)&window_h, &border_width,
		&depth)) {
		return;
	}
...
update_pr_transparent_subwindows(int i)
{
	...
	if (CSET_IS_TRANSPARENT_PR(Desks[i].highcolorset) && HilightDesks) {
		SetWindowBackground(dpy, Desks[i].CPagerWin, w, h,
		    &Colorset[Desks[i].highcolorset], Pdepth, Scr.NormalGC,
		    True);
	}
...
MovePage(Bool is_new_desk)
{
	...
	for (i = 0; i < ndesks; i++) {
		if (HilightDesks) {
			if (i == mon->virtual_scr.CurrentDesk - desk1) {
				...
				XMoveWindow(
				    dpy, Desks[i].CPagerWin, -32768, -32768);
			}
		}
	}
	DrawIconGrid(1);
	...
	if (mon->virtual_scr.CurrentDesk != icon_desk_shown) {
		...
		if ((mon->virtual_scr.CurrentDesk >= desk1)
			...
			sprintf(str, "GotoDesk %s %d", mon->name,
			    mon->virtual_scr.CurrentDesk);
			sptr = &str[0];
		}

		if (FlocaleTextListToTextProperty(
			dpy, &sptr, 1, XStdICCTextStyle, &name)
		    == 0) {
			fprintf(
...
ReConfigureIcons(Bool do_reconfigure_desk_only)
{
	...
	for (t = Start; t != NULL; t = t->next) {
		...
		if (mon->virtual_scr.CurrentDesk == t->desk)
			...
			XMoveResizeWindow(
			    dpy, t->IconView, -32768, -32768, w, h);
	}
}
...
DrawIconGrid(int erase)
{
		...
		if ((tmp < 0) || (tmp >= ndesks)) {
			if (PixmapBack)
				XSetWindowBackgroundPixmap(
				    dpy, icon_win, PixmapBack->picture);
			else
			...
			if (Desks[tmp].bgPixmap)
				...
				XSetWindowBackground(
				    dpy, icon_win, GetColor(Desks[tmp].Dcolor));
			else if (PixmapBack)
				XSetWindowBackgroundPixmap(
				    dpy, icon_win, PixmapBack->picture);
			else
	...
	while (x < mon->virtual_scr.VWidth) {
		...
		if (!use_no_separators)
			for (i = 0; i < ndesks; i++)
				XDrawLine(dpy, icon_win, Desks[i].DashedGC, x1,
				    y1, x1, y2);
		x += mon->virtual_scr.MyDisplayWidth;
	}
		...
		if (HilightPixmap) {
			for (i = 0; i < ndesks; i++)
				XCopyArea(dpy, HilightPixmap->picture, icon_win,
				    Scr.NormalGC, 0, 0, w, h, x, y);
		} else {
			for (i = 0; i < ndesks; i++)
				XFillRectangle(dpy, icon_win, Desks[i].HiliteGC,
				    x, y, w, h);
		}
	}
}
...
IconSwitchPage(XEvent *Event)
{
	...
	sprintf(command, "GotoPage %s %d %d", monitor_to_track ? mon->name : "",
	    Event->xbutton.x * mon->virtual_scr.VWidth
		/ (icon_w * mon->virtual_scr.MyDisplayWidth),
	    Event->xbutton.y * mon->virtual_scr.VHeight
		/ (icon_h * mon->virtual_scr.MyDisplayHeight));
	fprintf(stderr, "%s: cmd: %s\n", __func__, command);
	SendText(fd, command, 0);
	Wait = 1;
}
...
AddNewWindow(PagerWindow *t)
{
	...
	if ((i >= 0) && (i < ndesks)) {
		...
		if (windowcolorset > -1) {
			SetWindowBackground(dpy, t->PagerView, w, h,
			    &Colorset[windowcolorset], Pdepth, Scr.NormalGC,
			    True);
		}
		if (!UseSkipList || !DO_SKIP_WINDOW_LIST(t)) {
			if (IS_ICONIFIED(t)) {
				XMoveResizeWindow(
				    dpy, t->PagerView, -32768, -32768, 1, 1);
			}
	...
	t->IconView =
	    XCreateWindow(dpy, icon_win, x, y, w, h, 0, CopyFromParent,
	...
	if (!UseSkipList || !DO_SKIP_WINDOW_LIST(t)) {
		if (IS_ICONIFIED(t)) {
			XMoveResizeWindow(
			    dpy, t->IconView, -32768, -32768, 1, 1);
		}
...
MoveResizePagerView(PagerWindow *t, Bool do_force_redraw)
{
	...
	if (t->PagerView != None) {
		if (size_changed || position_changed || do_force_redraw) {
			...
			cset =
			    (t != FocusWin) ? windowcolorset : activecolorset;
			if (cset > -1
...
void
Scroll(int window_w, int window_h, int x, int y, int Desk, Bool do_scroll_icon)
{
...
MoveWindow(XEvent *Event)
{
	...
	wx = (mon->virtual_scr.Vx + t->x) * (desk_w - n)
		 / mon->virtual_scr.VWidth
	     + n1;
	wy = (mon->virtual_scr.Vy + t->y) * (desk_h - m)
	...
	if (KeepMoving) {
		...
		n1 = x2 * mon->virtual_scr.VWidth
		     / (desk_w * mon->virtual_scr.MyDisplayWidth);
		m1 = y2 * mon->virtual_scr.VHeight
		     / (desk_h * mon->virtual_scr.MyDisplayHeight);
		x = (x2 - n1) * mon->virtual_scr.VWidth / (desk_w - n)
		    - mon->virtual_scr.Vx;
		y = (y2 - m1) * mon->virtual_scr.VHeight / (desk_h - m)
		    - mon->virtual_scr.Vy;
		/* force onto desk */
		...
		if (NewDesk >= 0 && NewDesk < ndesks) {
			XReparentWindow(
			    dpy, t->PagerView, Desks[NewDesk].w, x2, y2);
			t->desk = NewDesk;
			XClearArea(dpy, t->PagerView, 0, 0, 0, 0, True);
			if (moved) {
				char buf[64];
		...
		if (do_switch_desk_later) {
			sprintf(
			    command, "Silent MoveToDesk 0 %d", NewDesk + desk1);
			SendText(fd, command, t->w);
			t->desk = NewDesk + desk1;
		}
		if (mon->virtual_scr.CurrentDesk == t->desk) {
...
draw_window_border(PagerWindow *t, Window w, int width, int height)
{
	if (w != None) {
		if (!WindowBorders3d || windowcolorset < 0
			...
			RelieveRectangle(dpy, w, 0, 0, width - 1,
			    t->pager_view_height - 1, Scr.NormalGC,
			    Scr.NormalGC, WindowBorderWidth);
		} else if (t == FocusWin) {
		...
		if (cur == next) {
			...
			while (*next) {
				...
				len = FlocaleStringNumberOfBytes(
				    FwindowFont, next);
				width =
				    FlocaleTextWidth(FwindowFont, next, len);
...
do_label_window(PagerWindow *t, Window w)
{
	...
	if (w != None) {
		if (FftSupport && FwindowFont != NULL
		    && FwindowFont->fftf.fftfont != NULL)
			XClearWindow(dpy, w);
		FwinString->win		       = w;
...
IconMoveWindow(XEvent *Event, PagerWindow *t)
{
	...
	if (KeepMoving) {
		...
		m1 = y * mon->virtual_scr.VHeight
		     / (icon_h * mon->virtual_scr.MyDisplayHeight);
		x = (x - n1) * mon->virtual_scr.VWidth / (icon_w - n)
		    - mon->virtual_scr.Vx;
		y = (y - m1) * mon->virtual_scr.VHeight / (icon_h - m)
		    - mon->virtual_scr.Vy;
		/* force onto desk */
...
MapBalloonWindow(PagerWindow *t, Bool is_icon_view)
{
	...
	else if (new_g.y + new_g.height > mon->virtual_scr.MyDisplayHeight
	...
	/* too close to right */
	else if (new_g.x + new_g.width > mon->virtual_scr.MyDisplayWidth
					     - (2 * BalloonBorderWidth) - 2) {
	...
	XMoveResizeWindow(
	    dpy, Scr.balloon_w, new_g.x, new_g.y, new_g.width, new_g.height);
	...
	if (Desks[i].ballooncolorset > -1) {
		SetWindowBackground(dpy, Scr.balloon_w, new_g.width,
		    ...
		    Scr.NormalGC, True);
		XSetWindowBorder(
		    dpy, Scr.balloon_w, Colorset[Desks[i].ballooncolorset].fg);
	}
...
change_colorset(int colorset)
{
	...
	for (i = 0; i < ndesks; i++) {
		if (Desks[i].highcolorset == colorset) {
			XSetForeground(
			    dpy, Desks[i].HiliteGC, Colorset[colorset].bg);
			XSetForeground(
			    dpy, Desks[i].rvGC, Colorset[colorset].fg);
			if (HilightDesks) {
				if (uselabel
		...
		if (Desks[i].colorset == colorset) {
			XSetWindowBorder(
			    dpy, Desks[i].title_w, Colorset[colorset].fg);
			XSetWindowBorder(
			    dpy, Desks[i].w, Colorset[colorset].fg);
			XSetForeground(
			    dpy, Desks[i].NormalGC, Colorset[colorset].fg);
			XSetForeground(
			    dpy, Desks[i].DashedGC, Colorset[colorset].fg);
			if (uselabel) {
				SetWindowBackground(dpy, Desks[i].title_w,
				    desk_w, desk_h + label_h,
