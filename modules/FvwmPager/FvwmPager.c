...
main(int argc, char **argv)
{
	...
	fvwmSetSignalMask(sigmask(SIGPIPE) | sigmask(SIGTERM) |
	    sigmask(SIGQUIT) | sigmask(SIGINT) | sigmask(SIGHUP));
#endif
	...
	SetMessageMask(fd,
	    M_VISIBLE_NAME | M_ADD_WINDOW | M_CONFIGURE_WINDOW |
		M_DESTROY_WINDOW | M_FOCUS_CHANGE | M_NEW_PAGE | M_NEW_DESK |
		M_RAISE_WINDOW | M_LOWER_WINDOW | M_ICONIFY | M_ICON_LOCATION |
		M_DEICONIFY | M_RES_NAME | M_RES_CLASS | M_CONFIG_INFO |
		M_END_CONFIG_INFO | M_MINI_ICON | M_END_WINDOWLIST | M_RESTACK);
	SetMessageMask(fd,
	    MX_VISIBLE_ICON_NAME | MX_PROPERTY_CHANGE | MX_MONITOR_FOCUS);

	ParseOptions();
	if (is_transient) {
		if (FQueryPointer(dpy, Scr.Root, &JunkRoot, &JunkChild,
			&pwindow.x, &pwindow.y, &JunkX, &JunkY,
			&JunkMask) == False) {
	...
	while (!isTerminated) {
		...
		if (error_occured) {
			...
			if (XGetGeometry(dpy, Scr.Pager_w, &root, &x, &y,
				(unsigned *)&pwindow.width,
				(unsigned *)&pwindow.height, &border_width,
				&depth) == 0) {
				/* does not return */
				ExitPager();
			}
...
list_new_page(unsigned long *body)
{
	...
	if (m->virtual_scr.VxPages != body[5] ||
		...
		m->virtual_scr.VyPages = body[6];
		m->virtual_scr.VWidth  = m->virtual_scr.VxPages *
		    m->virtual_scr.MyDisplayWidth;
		m->virtual_scr.VHeight = m->virtual_scr.VyPages *
		    m->virtual_scr.MyDisplayHeight;
		m->virtual_scr.VxMax = m->virtual_scr.VWidth -
		    m->virtual_scr.MyDisplayWidth;
		m->virtual_scr.VyMax = m->virtual_scr.VHeight -
		    m->virtual_scr.MyDisplayHeight;
		ReConfigure();
	}
	MovePage(False);
...
list_new_desk(unsigned long *body)
{
	...
	if (fAlwaysCurrentDesk && oldDesk != m->virtual_scr.CurrentDesk) {
		...
		if (change_cs > -1) {
			...
			XSetWindowBackground(dpy, Desks[0].w,
		...
		if (i >= 0 && i < ndesks && Desks[i].label != NULL) {
			...
			snprintf(line, sizeof(line), "Desk %d",
	...
	if (StrEquals(token, "Colorset")) {
		...
		TAILQ_FOREACH (m, &fp_monitor_q, entry) {
			...
			m->virtual_scr.VHeight = m->virtual_scr.VyMax +
			    m->virtual_scr.MyDisplayHeight;
			m->virtual_scr.VxPages = m->virtual_scr.VWidth /
			    m->virtual_scr.MyDisplayWidth;
			m->virtual_scr.VyPages = m->virtual_scr.VHeight /
			    m->virtual_scr.MyDisplayHeight;
		}
	}
}
...
ParseOptions(void)
{
	...
	for (GetConfigLine(fd, &tline); tline != NULL;
		...
		if (StrEquals(resource, "Monitor")) {
			...
			yneg	       = 0;
			usposition     = 0;
			flags = FScreenParseGeometry(arg1, &g_x, &g_y, &width,
			    &height);
			if (flags & WidthValue) {
				pwindow.width = width;
			}
			...
			if (fAlwaysCurrentDesk) {
				...
				Desks[dNr].bgPixmap = PCacheFvwmPicture(dpy,
				    Scr.Pager_w, ImagePath, arg2, fpa);
			}
