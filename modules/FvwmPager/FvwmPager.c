...
static void
extract_monitor_config(struct fpmonitor *, char *);
static void
SetDeskLabel(struct fpmonitor *, int desk, const char *label);
static RETSIGTYPE
TerminateHandler(int);
void
ExitPager(void);
void
list_monitor_focus(unsigned long *);
...
main(int argc, char **argv)
{
	...
	fvwmSetSignalMask(sigmask(SIGPIPE) | sigmask(SIGTERM) | sigmask(SIGQUIT)
	...
	SetMessageMask(fd, M_VISIBLE_NAME | M_ADD_WINDOW | M_CONFIGURE_WINDOW
			       ...
			       | M_END_CONFIG_INFO | M_MINI_ICON
			       | M_END_WINDOWLIST | M_RESTACK);
	SetMessageMask(
	    fd, MX_VISIBLE_ICON_NAME | MX_PROPERTY_CHANGE | MX_MONITOR_FOCUS);
	...
	if (is_transient) {
		...
		XSync(dpy, 0);
		for (i = 0;
		     i < 50 && !(is_pointer_grabbed && is_keyboard_grabbed);
		     i++) {
	...
	if (isTerminated) {
		fvwm_debug(
		    __func__, "%s: Received signal: exiting...\n", MyName);
	}
#endif
 ...
 */
void
process_message(FvwmPacket *packet)
{
 ...
 */
void
handle_config_win_package(PagerWindow *t, ConfigWinPacket *cfgpacket)
{
...
void
list_monitor_focus(unsigned long *body)
{
...
list_new_page(unsigned long *body)
{
	...
	if (m->virtual_scr.VxPages != body[5]
	    || m->virtual_scr.VyPages != body[6]) {
		m->virtual_scr.VxPages = body[5];
		m->virtual_scr.VyPages = body[6];
		m->virtual_scr.VWidth =
		    m->virtual_scr.VxPages * m->virtual_scr.MyDisplayWidth;
		m->virtual_scr.VHeight =
		    m->virtual_scr.VyPages * m->virtual_scr.MyDisplayHeight;
		m->virtual_scr.VxMax =
...
list_new_desk(unsigned long *body)
{
	...
	/* If the monitor for which the event was sent, does not match the
	 * monitor itself, then don't change the FvwmPager's desk.  Only do this
	 * if we're tracking a specific monitor though.
	 */
	...
	if (fAlwaysCurrentDesk && oldDesk != m->virtual_scr.CurrentDesk) {
		...
		for (t = Start; t != NULL; t = t->next) {
			if (t->desk == oldDesk
			    || t->desk == m->virtual_scr.CurrentDesk)
		...
		if (change_cs < 0
			...
			XSetWindowBackground(
			    dpy, Desks[0].title_w, GetColor(Desks[0].Dcolor));
...
list_lower(unsigned long *body)
{
	...
	if (t != NULL) {
		...
		if (HilightDesks && (t->desk - desk1 >= 0)
		    && (t->desk - desk1 < ndesks))
...
void
list_deiconify(unsigned long *body, unsigned long length)
{
	PagerWindow *t;
	Window	     target_w;
...
void
list_window_name(unsigned long *body, unsigned long type)
{
	PagerWindow *	  t;
	Window		  target_w;
...
void
extract_monitor_config(struct fpmonitor *m, char *tline)
{
	int output, mdw, mdh, vx, vy, vxmax, vymax, iscur;
	int x, y, w, h;

	sscanf(tline, "%d %d %d %d %d %d %d %d %d %d %d %d", &output, &iscur,
	    &mdw, &mdh, &vx, &vy, &vxmax, &vymax, &x, &y, &w, &h);
	m->x			       = x;
	m->y			       = y;
	m->w			       = w;
	m->h			       = h;
...
void
list_config_info(unsigned long *body)
{
	...
	if (StrEquals(token, "Colorset")) {
		...
		{
			m->virtual_scr.VxMax =
			    dx * m->virtual_scr.MyDisplayWidth
			    - m->virtual_scr.MyDisplayWidth;
			m->virtual_scr.VyMax =
			    dy * m->virtual_scr.MyDisplayHeight
...
void
list_property_change(unsigned long *body)
{
...
static void
ParseColorset(char *arg1, char *arg2, void *offset_deskinfo, void *offset_item,
    int *colorset_global)
{
	...
	if (fAlwaysCurrentDesk) {
		...
		if (desk == mon->virtual_scr.CurrentDesk || all_desks) {
			*(int *)(((char *)&Desks[0]) + colorset_offset) =
			    colorset;
		}
	} else if (all_desks) {
		for (i = 0; i < ndesks; i++) {
			*(int *)(((char *)&Desks[i]) + colorset_offset) =
			    colorset;
		}
	} else if ((desk >= desk1) && (desk <= desk2)) {
		*(int *)(((char *)&Desks[desk - desk1]) + colorset_offset) =
		    colorset;
	}
...
static void
SetDeskLabel(struct fpmonitor *m, int desk, const char *label)
{
	PagerStringList *item;
...
ParseOptions(void)
{
	...
	for (GetConfigLine(fd, &tline); tline != NULL;
		...
		if (StrEquals(token, "Colorset")) {
			...
			if (!MoveThresholdSetForModule) {
				int val;
				if (GetIntegerArguments(next, NULL, &val, 1)
				    > 0) {
					if (val >= 0)
						MoveThreshold = val;
					else
		...
		if (StrEquals(resource, "Monitor")) {
			...
			if (fAlwaysCurrentDesk) {
				...
				if (item->next != NULL) {
					if (item->next->bgPixmap != NULL) {
						PDestroyFvwmPicture(
						    dpy, item->next->bgPixmap);
						item->next->bgPixmap = NULL;
					}
					item->next->bgPixmap =
					    PCacheFvwmPicture(dpy, Scr.Pager_w,
						ImagePath, arg2, fpa);
				} else {
					/* new Dcolor and desktop */
					item = NewPagerStringItem(item, desk);
					item->bgPixmap = PCacheFvwmPicture(dpy,
			...
			ShapeLabels = 1;
			   ...
			   && StrEquals(resource, "NoShapeLabels")) {
			ShapeLabels = 0;
		...
		/* ... and get Balloon config options ...
		   -- ric@giccs.georgetown.edu */
			...
			if (BalloonYOffset == 0) {
				fvwm_debug(__func__,
				    "%s: Warning:"
				    " you're not allowed BalloonYOffset 0; "
				    "defaulting to +3\n",
				    MyName);
				/* we don't allow yoffset of 0 because it allows
				 * direct transit from pager window to balloon
				 * window, setting up a LeaveNotify/EnterNotify
				 * event loop */
				BalloonYOffset = 3;
			}
	...
	TAILQ_FOREACH(m, &fp_monitor_q, entry)
	{
		m->virtual_scr.VxMax = dx * m->virtual_scr.MyDisplayWidth
				       - m->virtual_scr.MyDisplayWidth;
		...
		if (m->virtual_scr.VyMax < 0)
			m->virtual_scr.VyMax = 0;
		m->virtual_scr.VWidth =
		    m->virtual_scr.VxMax + m->virtual_scr.MyDisplayWidth;
		m->virtual_scr.VHeight =
		    m->virtual_scr.VyMax + m->virtual_scr.MyDisplayHeight;
		m->virtual_scr.VxPages =
		    m->virtual_scr.VWidth / m->virtual_scr.MyDisplayWidth;
		m->virtual_scr.VyPages =
		    m->virtual_scr.VHeight / m->virtual_scr.MyDisplayHeight;
		...
		fvwm_debug(__func__,
		    ...
		    "VxPages: %d, VyPages: %d\n",
		    m->name, m->virtual_scr.VxMax, m->virtual_scr.VyMax,
