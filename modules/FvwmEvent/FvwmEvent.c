...
static event_entry message_event_table[] = { EVENT_ENTRY("new_page", -1),
	...
	EVENT_ENTRY("old_add_window", 0), EVENT_ENTRY("raise_window", 0),
	EVENT_ENTRY("lower_window", 0), EVENT_ENTRY("old_configure_window", 0),
	EVENT_ENTRY("focus_change", 0), EVENT_ENTRY("destroy_window", 0),
	EVENT_ENTRY("iconify", 0), EVENT_ENTRY("deiconify", 0),
	EVENT_ENTRY("window_name", 0), EVENT_ENTRY("icon_name", 0),
	EVENT_ENTRY("res_class", 0), EVENT_ENTRY("res_name", 0),
	EVENT_ENTRY("end_windowlist", -1), EVENT_ENTRY("icon_location", 0),
	EVENT_ENTRY("map", 0), EVENT_ENTRY("error", -1),
	EVENT_ENTRY("config_info", -1), EVENT_ENTRY("end_config_info", -1),
	EVENT_ENTRY("icon_file", 0), EVENT_ENTRY("default_icon", -1),
	EVENT_ENTRY("string", -1), EVENT_ENTRY("mini_icon", 0),
	EVENT_ENTRY("windowshade", 0), EVENT_ENTRY("dewindowshade", 0),
	EVENT_ENTRY("visible_name", 0), EVENT_ENTRY("sendconfig", -1),
	EVENT_ENTRY("restack", -1), EVENT_ENTRY("add_window", 0),
	EVENT_ENTRY("configure_window", 0), EVENT_ENTRY(NULL, 0) };
static event_entry extended_message_event_table[] = {
	...
	EVENT_ENTRY("monitor_focus", 0 | ARG_NO_WINID), EVENT_ENTRY("echo", 0),
		...
		if (audio_compat) {
			...
			if (PassID && action_arg != -1) {
				if (action_arg & ARG_NO_WINID) {
					...
					snprintf(buf, len, "%s %s %ld",
					    cmd_line, action, body[action_arg]);
				} else {
					snprintf(buf, len, "%s %s 0x%lx",
					    cmd_line, action, body[action_arg]);
				}
			} else {
				snprintf(buf, len, "%s %s", cmd_line, action);
