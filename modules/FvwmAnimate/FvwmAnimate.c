...
struct ASAnimate Animate = { NULL, NULL, ANIM_ITERATIONS, ANIM_DELAY,
	ANIM_TWIST, ANIM_WIDTH, AnimateResizeTwist, ANIM_TIME };
	...
	for (ant_ctr = 0; ant_ctr <= ants; ant_ctr++) {
		...
		for (i = 0; i < Animate.iterations; i++) {
			for (j = 0; j < 4; j++) { /* all 4 line segs */
				BEG.seg[j].x2 = BEG.seg[j].x1 +
				    INC.seg[j].x2; /* calc end points */
				BEG.seg[j].y2 = BEG.seg[j].y1 +
...
main(int argc, char **argv)
{
	...
	SetMessageMask(Channel,
	    M_ICONIFY | M_DEICONIFY | M_STRING | M_SENDCONFIG |
		...
		if ((e = FindToken(p, table, char *))) { /* config option ? */
			...
			case Resize_arg: /* -or - Resize */
				for (i = 0; i < NUM_EFFECTS; i++) {
					if (strcasecmp(q, effects[i].name) ==
						0 ||
					    (effects[i].alias &&
						strcasecmp(q,
						    effects[i].alias) == 0)) {
				...
				/* Logically, you would only reset these when
				   you got a command of None, or Random, but it
	...
	if (Animate.pixmap) { /* if pixmap called for */
		...
		if (XParseColor(dpy, Pcmap, Animate.color, &xcol)) {
			if (PictureAllocColor(dpy, Pcmap, &xcol, True)) {
				...
				PictureFreeColors(dpy, Pcmap, &xcol.pixel, 1, 0,
				    True);
...
DefineMe(void)
{
	...
	CMD10(
	    "AddToMenu MenuColor%s \"&B. Color DodgerBlue\" %sColor DodgerBlue");
	CMD10(
	    "AddToMenu MenuColor%s \"&C. Color SteelBlue\" %sColor SteelBlue");
	CMD10(
	    "AddToMenu MenuColor%s \"&D. Color Chartreuse\" %sColor chartreuse");
...
SaveConfig(void)
{
	...
	*/
	snprintf(filename, sizeof(filename), "%s/.%s", getenv("FVWM_USERDIR"),
	    module->name);
...
#define CMD1V(TEXT, VAR)                                     \
	...
	CMD1V("*Form%sChoice FRAME3D FRAME3D %s \"Frame3d\"",
	    effects[4].button);
