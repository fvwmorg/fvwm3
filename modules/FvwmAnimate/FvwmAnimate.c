...
StopCmd(void);
static void
AnimateResizeZoom(int, int, int, int, int, int, int, int);
static void
AnimateResizeLines(int, int, int, int, int, int, int, int);
static void
AnimateResizeFlip(int, int, int, int, int, int, int, int);
static void
AnimateResizeTurn(int, int, int, int, int, int, int, int);
static void
AnimateResizeRandom(int, int, int, int, int, int, int, int);
static void
AnimateResizeZoom3D(int, int, int, int, int, int, int, int);
static void
AnimateResizeNone(int, int, int, int, int, int, int, int);
static void
AnimateResizeTwist(int, int, int, int, int, int, int, int);
static void
DefineForm(void);
...
struct ASAnimate Animate = {NULL, NULL, ANIM_ITERATIONS, ANIM_DELAY, ANIM_TWIST,
    ANIM_WIDTH, AnimateResizeTwist, ANIM_TIME};
...
struct AnimateEffects effects[] = {{"None", 0, AnimateResizeNone, NULL},
...
static Bool
is_animation_visible(int x, int y, int w, int h, int fx, int fy, int fw, int fh)
{
	Bool is_start_visible = True;
	Bool is_end_visible   = True;

	if (x >= Scr.MyDisplayWidth || x + w < 0 || y >= Scr.MyDisplayWidth
	    || y + h < 0) {
		is_start_visible = False;
	}
	if (fx >= Scr.MyDisplayWidth || fx + fw < 0 || fy >= Scr.MyDisplayWidth
	    || fy + fh < 0) {
...
AnimateResizeZoom(int x, int y, int w, int h, int fx, int fy, int fw, int fh)
{
	...
	for (i = 0; i < Animate.iterations; i++) {
		XDrawRectangle(
		    dpy, Scr.root, gc, (int)cx, (int)cy, (int)cw, (int)ch);
		XFlush(dpy);
		usleep(Animate.delay * 1000);
		XDrawRectangle(
		    dpy, Scr.root, gc, (int)cx, (int)cy, (int)cw, (int)ch);
...
AnimateResizeZoom3D(int x, int y, int w, int h, int fx, int fy, int fw, int fh)
{
	...
	{
		for (i = 0; i < Animate.iterations; i++) {
			XDrawRectangle(dpy, Scr.root, gc, (int)cx, (int)cy,
			    (int)cw, (int)ch);
			XDrawRectangle(dpy, Scr.root, gc, (int)fx, (int)fy,
			    (int)fw, (int)fh);
			...
			XDrawLine(dpy, Scr.root, gc, (int)cx,
			    ((int)cy + (int)ch), fx, (fy + fh));
			XFlush(dpy);
			usleep(Animate.delay * 1000);
			XDrawRectangle(dpy, Scr.root, gc, (int)cx, (int)cy,
			    (int)cw, (int)ch);
...
AnimateResizeRandom(int x, int y, int w, int h, int fx, int fy, int fw, int fh)
{
	...
	effects[(rand() + (x * y + w * h + fx)) % (NUM_EFFECTS - 2) + 2]
	    .function(x, y, w, h, fx, fy, fw, fh);
}
 ...
 */
static void
AnimateResizeLines(int x, int y, int w, int h, int fx, int fy, int fw, int fh)
{
	int i, j;
	int ants = 1, ant_ctr;
	typedef struct
...
#define INC ends[1]
	...
	for (ant_ctr = 0; ant_ctr <= ants; ant_ctr++) {
		...
		/* The lines look a little better if they start and end a little
		   in from the edges.  Allowing tuning might be overkill. */
		for (i = 0; i < 2; i++) {	  /* for begin and endpoints */
			...
			/* Now width and height have been used, change LL to
			 * endpoints. */
		...
		for (i = 0; i < 4; i++) { /* for each of 4 line segs */
			INC.seg[i].x2 = (INC.seg[i].x1 - BEG.seg[i].x1)
					/ Animate.iterations;
			INC.seg[i].y2 = (INC.seg[i].y1 - BEG.seg[i].y1)
					/ Animate.iterations;
		}
		for (i = 0; i < Animate.iterations; i++) {
			for (j = 0; j < 4; j++) { /* all 4 line segs */
				BEG.seg[j].x2 =
				    BEG.seg[j].x1
				    + INC.seg[j].x2; /* calc end points */
				BEG.seg[j].y2 =
				    BEG.seg[j].y1
				    + INC.seg[j].y2; /* calc end points */
			}
			myaprintf((stderr,
			    "Lines %dx%d-%dx%d, %dx%d-%dx%d, %dx%d-%dx%d, "
			    "%dx%d-%dx%d,"
			    "ant_ctr %d\n",
			    BEG.UR.x1, BEG.UR.y1, BEG.UR.x2, BEG.UR.y2,
			    BEG.UL.x1, BEG.UL.y1, BEG.UL.x2, BEG.UL.y2,
			    BEG.LR.x1, BEG.LR.y1, BEG.LR.x2, BEG.LR.y2,
			    BEG.LL.x1, BEG.LL.y1, BEG.LL.x2, BEG.LL.y2,
			    ant_ctr));
			if (ants == 0) {
				MyXGrabServer(dpy);
				XInstallColormap(dpy, Pcmap);
			...
			for (j = 0; j < 4; j++) { /* all 4 lines segs */
				BEG.seg[j].x1 +=
				    INC.seg[j].x2; /* calc new starting point */
				BEG.seg[j].y1 +=
				    INC.seg[j].y2; /* calc new starting point */
			}			   /* end 4 lines segs */
		}				   /* end iterations */
	}					   /* end for ants */
	if (ants == 1) {			   /* if draw then erase */
 ...
 */
static void
AnimateResizeNone(int x, int y, int w, int h, int fx, int fy, int fw, int fh)
{
	(void)x;
	(void)y;
	(void)w;
...
main(int argc, char **argv)
{
	...
	if (module == NULL) {
		fprintf(stderr, "FvwmAnimate Version " VERSION
...
#ifdef SA_INTERRUPT
		sigact.sa_flags =
		    SA_INTERRUPT; /* to interrupt ReadFvwmPacket() */
#else
		sigact.sa_flags = 0;
#endif
	...
	fvwmSetSignalMask(
	    sigmask(SIGTERM) | sigmask(SIGINT) | sigmask(SIGPIPE));
#endif
	...
	SetMessageMask(
	    Channel, M_ICONIFY | M_DEICONIFY | M_STRING | M_SENDCONFIG
	...
	SetSyncMask(Channel,
	    M_ICONIFY | M_DEICONIFY | M_STRING); /* lock on send mask */
	SetNoGrabMask(Channel,
	    M_ICONIFY | M_DEICONIFY | M_STRING); /* ignore during recapture */
	Loop();					 /* start running */
	return 0;
}
...
Loop(void)
{
	...
	while (!isTerminated) {
		...
		case M_ICONIFY:
			...
			/* In Afterstep, this logic waited for
			   M_CONFIGURE_WINDOW before animating.  To this time, I
			   don't know why. (One is sent right after the other.)
			  */
			if (packet->size < 15 /* if not enough info */
			    || (int)packet->body[3]
				   == -10000 /* or a transient window */
			    || (int)packet->body[5]
				   == 0) { /* or a "noicon" icon */
				break;	   /* don't animate it */
			}
		...
		case M_STRING: {
			...
			if (strcasecmp(token, "animate") == 0) {
				/* This message lets anything create an
				   animation. Eg:
				...
				int matched;
				matched = sscanf(line,
				    "%5d %5d %5d %5d %5d %5d %5d %5d", &locs[0],
				    &locs[1], &locs[2], &locs[3], &locs[4],
				    &locs[5], &locs[6], &locs[7]);
				if (matched == 8 && play_state == True) {
					Animate.resize(locs[0], locs[1],
					    locs[2], locs[3], locs[4], locs[5],
					    locs[6], locs[7]);
					myaprintf((stderr,
					    "animate %d+%d+%dx%d %d+%d+%dx%d\n",
					    locs[0], locs[1], locs[2], locs[3],
					    locs[4], locs[5], locs[6],
					    locs[7]));
				}
				free(token);
				break;
		...
		if (packet->type != M_CONFIG_INFO)
			SendUnlockNotification(
			    Channel); /* fvwm can continue now! */
		if ((Animate.resize
			    == AnimateResizeNone /* If no animation desired */
			&& animate_none >= 1)	 /* and 1 animation(s) */
		    || stop_recvd) {		 /* or stop cmd */
			/* This still isn't perfect, if the user turns off
			   animation, they would expect the menu to change on
			   the spot. On the otherhand, the menu shouldn't change
			   if the module is still running. This logic is
			   dependent on fvwm sending iconify messages to make
			   this module exit.  Currently it is sending messages
...
ParseConfigLine(char *buf)
{
	...
	/* Search for MyName (normally *FvwmAnimate) */
	else if (strncasecmp(buf, mname, module->namelen + 1)
		 == 0) { /* If its for me */
		...
		if ((e = FindToken(p, table, char *))) { /* config option ? */
			...
			case Custom_arg:       /* Custom command */
				if (running) { /* if not a stored command */
					custom_recvd =
					    True; /* remember someone asked */
				}
				break;
			case Color_arg: /* Color */
				if (Animate.color) {
					free(Animate.color); /* release storage
								holding color
								name */
			...
			case Pixmap_arg:
				if (Animate.pixmap) {
					free(Animate.pixmap); /* release storage
								 holding pixmap
								 name */
					Animate.pixmap = 0; /* show its gone */
				}
				if (strcasecmp(q, "None")
				    != 0) { /* If not pixmap "none"  */
					Animate.pixmap =
					    fxstrdup(q); /* make copy of name */
				}
				if (pixmap) {
			...
			case Resize_arg: /* -or - Resize */
				...
				/* Logically, you would only reset these when
				   you got a command
				   of None, or Random, but it doesn't really
				   matter. */
				animate_none = 0;
				curtime	     = time(NULL);
...
CreateDrawGC(void)
{
	...
	if (Animate.pixmap) { /* if pixmap called for */
		...
		if (!picture)
			...
			pixmap = XCreatePixmap(dpy, RootWindow(dpy, Scr.screen),
			    picture->width, picture->height, Pdepth);
			PGraphicsCopyPixmaps(dpy, picture->picture, None, None,
			    picture->depth, pixmap, None, 0, 0, picture->width,
			    picture->height, 0, 0);
	...
	gc = fvwmlib_XCreateGC(dpy, Scr.root,
	    GCFunction | GCForeground | GCLineWidth | GCSubwindowMode
		| (pixmap ? GCFillStyle | GCTile : 0),
	    &gcv);
}
...
DefineMe(void)
{
	...
	CMD10(
	    "AddToMenu MenuIterations%s \"&5. Iterations 16\" %sIterations 16");
	CMD10(
	    "AddToMenu MenuIterations%s \"&6. Iterations 32\" %sIterations 32");
	...
	CMD1X(
	    "AddToMenu MenuTwist%s \"Twists (Twist, Turn, Flip only)\" Title");
	...
	CMD10("AddToMenu MenuColor%s \"&B. Color DodgerBlue\" %sColor "
	      "DodgerBlue");
	CMD10(
	    "AddToMenu MenuColor%s \"&C. Color SteelBlue\" %sColor SteelBlue");
	CMD10("AddToMenu MenuColor%s \"&D. Color Chartreuse\" %sColor "
	      "chartreuse");
	CMD10("AddToMenu MenuColor%s \"&E. Color Wheat\" %sColor wheat");
	CMD10(
	    "AddToMenu MenuColor%s \"&F. Color Turquoise\" %sColor turquoise");
	...
	CMD10(
	    "AddToMenu MenuEffects%s \"&4. Effect Frame3D\" %sEffect Frame3D");
	...
	/* Still to be done:
	   Use of FvwmForms for Help.  (Need to fix line spacing in FvwmForms
	   first).
	*/
}
...
SaveConfig(void)
{
	...
	*/
	...
	if (config_file == NULL) {
		sprintf(msg, "%s: Open config file <%s> for write failed. \
Save not done! Error\n",
...
DefineForm(void)
{
	...
	for (i = 0; i < NUM_EFFECTS; i++) { /* for all effects */
		...
		if (Animate.resize
		    == effects[i].function) {	  /* compare to curr setting */
...
#define CMD1V(TEXT, VAR)                       \
	...
	CMD1V(
	    "*Form%sChoice FRAME3D FRAME3D %s \"Frame3d\"", effects[4].button);
