...
DumpButtons(button_info *b)
{
	...
	if (b != UberButton) {
		...
		fvwm_debug(__func__, "0x%lx(%ix%i@(%i,%i)): ", (unsigned long)b,
		    b->BWidth, b->BHeight, buttonXPos(b, button),
		    buttonYPos(b, button));
	} else {
		fvwm_debug(__func__, "0x%lx(%ix%i@): ", (unsigned long)b,
		    b->BWidth, b->BHeight);
	}

	if (b->flags.b_Font)
		fvwm_debug(__func__, "Font(%s,0x%lx) ", b->font_string,
		    (unsigned long)b->Ffont);
	...
	if (b->flags.b_Icon)
		fvwm_debug(
		    __func__, "Icon(%s,%i) ", b->icon_file, (int)b->IconWin);
	if (b->flags.b_Icon)
	...
	if (b->flags.b_Container) {
		...
		fvwm_debug(__func__,
		    ...
		    b->c->allocated_buttons, b->c->width, b->c->height,
		    b->c->xpos, b->c->ypos);
		/*
...
SaveButtons(button_info *b)
{
	...
	if (b->flags.b_Action) {
		...
		for (i = 1; i < 4; i++) {
			if (b->action[i])
				fvwm_debug(__func__, "Action (Mouse %i) `%s` ",
				    i, b->action[i]);
		}
	}

	if (b->flags.b_Container) {
		fvwm_debug(__func__, "Container (Columns %i Rows %i ",
		    b->c->num_columns, b->c->num_rows);

		if (b->c->flags.b_Font)
			fvwm_debug(__func__, "Font %s ", b->c->font_string);
		if (b->c->flags.b_Fore)
			fvwm_debug(__func__, "Fore %s ", b->c->fore);
		if (b->c->flags.b_Back)
			fvwm_debug(__func__, "Back %s ", b->c->back);
		if (b->c->flags.b_Frame)
			fvwm_debug(__func__, "Frame %i ", b->c->framew);
		if (b->c->flags.b_Padding)
			fvwm_debug(
			    __func__, "Padding %i %i ", b->c->xpad, b->c->ypad);
