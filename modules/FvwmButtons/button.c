...
{
	return (b->flags.b_Swallow || b->flags.b_Panel) ? (b->swallow & b_Count)
							: 0;
}
...
alloc_buttonlist(button_info *ub, int num)
{
	...
	if (num >= ub->c->allocated_buttons) {
		...
		if (num < old || old > (old + 32)) /* test for num<old or for
						      signed overflow */
		{
			fvwm_debug(__func__,
...
PlaceAndExpandButton(int x, int y, button_info *b, button_info *ub)
{
	...
	if (x + b->BWidth > c->num_columns) {
		fvwm_debug(
		    __func__, "%s: Button too wide. giving up\n", MyName);
		fvwm_debug(__func__,
