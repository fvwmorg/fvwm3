...
int
buttonXPos(const button_info *b, int i)
{
...
int
buttonYPos(const button_info *b, int i)
{
...
int
buttonWidth(const button_info *b)
{
...
int
buttonHeight(const button_info *b)
{
...
int
buttonSwallowCount(button_info *b)
{
	return (b->flags.b_Swallow || b->flags.b_Panel) ?
	    (b->swallow & b_Count) :
	    0;
}
...
**/
void
buttonInfo(const button_info *b, int *x, int *y, int *px, int *py, int *f)
{
...
buttonXPad(button_info *b)
{
	...
	if (b->flags.b_Container ||
	    b->flags.b_Swallow) /* Normally no padding for these     */
		return 0;
	while ((b = b->parent))
...
**/
Pixel
buttonFore(const button_info *b)
{
	if (b->flags.b_Fore)
		return b->fc;
	while ((b = b->parent)) {
...
**/
Pixel
buttonBack(const button_info *b)
{
	if (b->flags.b_Back)
		return b->bc;
	while ((b = b->parent)) {
...
int
buttonBackgroundButton(button_info *b, button_info **r_b)
{
	button_info *tmpb, *pb;
	Bool	     done = False;
	...
	while (tmpb->parent != NULL && !done) {
		...
		} else if (tmpb->flags.b_Swallow &&
		    buttonSwallowCount(b) == 3 && tmpb->flags.b_Colorset) {
			done = True;
		} else if (tmpb->flags.b_Colorset) {
			done = True;
		} else if (!tmpb->flags.b_IconBack &&
		    !tmpb->flags.b_IconParent && !tmpb->flags.b_Swallow &&
...
ShuffleButtons(button_info *ub)
{
	...
	for (i = 0; i < num_items; i++)
		actual_buttons_used += local_buttons[i]->BWidth *
		    local_buttons[i]->BHeight;
	...
	if (!c->flags.b_SizeFixed || !(c->num_rows) || !(c->num_columns)) {
		...
		if (c->num_columns == 0)
			c->num_columns = 1 +
			    (actual_buttons_used - 1) / c->num_rows;
		if (c->num_rows == 0)
			c->num_rows = 1 +
			    (actual_buttons_used - 1) / c->num_columns;
...
**/
button_info *
get_xy_button(button_info *ub, int row, int column)
{
	int i;
...
select_button(button_info *ub, int x, int y)
{
	...
	return select_button(b, x + ub->c->xpos - buttonXPos(b, i),
	    y + ub->c->ypos - buttonYPos(b, i));
}

void
get_button_root_geometry(rectangle *r, const button_info *b)
{
	int    x;
	int    y;
	int    f;
	...
	XTranslateCoordinates(Dpy, MyWindow, Root, r->x, r->y, &r->x, &r->y,
	    &win);
