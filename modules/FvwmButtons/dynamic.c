...
expand_button_vars(const button_info *b, const char *line)
{
	...
	expanded_line = module_expand_action(Dpy, screen, (char *)line, &r,
	    fore, back);
...
parse_button_id(char **line)
{
	...
	*line = NULL;
	...
	if (*s == '+') {
		...
		if (b == NULL) {
			show_error("Button at column %d row %d not found\n", x,
			    y);
			return NULL;
		}
	} else if (isdigit(*s)) {
...
static char *actions[] = { "Silent", "ChangeButton", "ExpandButtonVars",
	"PressButton", NULL };
...
parse_message_line(char *line)
{
	...
	case 1:
		...
		while (rest && rest[0] != '\0') {
			...
			rest = GetQuotedString(rest, &option_pair, ",", NULL,
			    NULL, NULL);
			while (isspace(*rest)) {
				rest++;
			}
			...
			option = GetTokenIndex(option_pair, button_options, -1,
			    &value0);
			if (option < 0) {
				show_error(
