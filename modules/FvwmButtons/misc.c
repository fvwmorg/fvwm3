...
#define minAspectY hints->min_aspect.y
	...
	if (hints->flags & PAspect) {
		if (minAspectX * dheight > minAspectY * dwidth) {
			delta = makemult(dheight -
				minAspectY * dwidth / minAspectX,
			    yinc);
			if (dheight - delta >= minHeight) {
				dheight -= delta;
			} else {
				delta = makemult(minAspectX * dheight /
					    minAspectY -
					dwidth,
				    xinc);
		...
		if (maxAspectX * dheight < maxAspectY * dwidth) {
			delta = makemult(dwidth -
				maxAspectX * dheight / maxAspectY,
			    xinc);
			if (dwidth - delta >= minWidth) {
				dwidth -= delta;
			} else {
				delta = makemult(dwidth * maxAspectY /
					    maxAspectX -
					dheight,
				    yinc);
