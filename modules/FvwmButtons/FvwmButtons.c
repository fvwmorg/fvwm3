...
#define MW_EVENTS                                                 \
	(ExposureMask | StructureNotifyMask | ButtonReleaseMask | \
	    ...
	    PointerMotionMask | KeyReleaseMask | KeyPressMask |   \
	    ButtonMotionMask | 0)
...
#define PA_EVENTS (StructureNotifyMask | 0)
...
static Window GetRealGeometry(Display *, Window, int *, int *, unsigned int *,
    unsigned int *, unsigned int *, unsigned int *);
static void   GetPanelGeometry(Bool get_big, button_info *b, int lb, int tb,
      int rb, int bb, int *x, int *y, int *w, int *h);
		...
		if ((buttonSwallowCount(b) == 3) && swin) {
			...
			if (!IsThereADestroyEvent(
				...
				if (!(buttonSwallow(b) & b_NoClose)) {
					...
					XReparentWindow(Dpy, swin, Root, b->x,
					    b->y);
...
GetButtonAction(button_info *b, int n)
{
	...
	act = module_expand_action(Dpy, screen, b->action[n], &r,
	    UberButton->c->fore, UberButton->c->back);
...
main(int argc, char **argv)
{
	...
	fvwmSetSignalMask(sigmask(SIGPIPE) | sigmask(SIGINT) | sigmask(SIGHUP) |
	...
	*/
	...
	MyWindow = XCreateWindow(Dpy, Root, 0, 0, 1, 1, 0, Pdepth, InputOutput,
	    Pvisual, CWColormap | CWBackPixmap | CWBorderPixel, &xswa);
	...
	if (!XGetGeometry(Dpy, MyWindow, &root, &x, &y, (unsigned int *)&Width,
...
static button_info *
handle_new_position(button_info *b, int pos_x, int pos_y)
{
...
void
ButtonPressProcess(button_info *b, char **act)
{
...
Loop(void)
{
	...
	while (!isTerminated) {
		if (My_FNextEvent(Dpy, &Event)) {
			if (FShapesSupported &&
			    Event.type == FShapeEventBase + FShapeNotify) {
				...
				if (UberButton->c->flags.b_TransBack) {
					SetTransparentBackground(UberButton,
					    Width, Height);
				}
				continue;
			}
			...
			case Expose: {
				...
				while (FCheckTypedWindowEvent(Dpy, MyWindow,
				    Expose, &Event)) {
			...
			case ConfigureNotify: {
				...
				if (!XGetGeometry(Dpy, MyWindow, &root, &x, &y,
					&tw, &th, &border_width, &depth)) {
					break;
				}
				if (tw != Width || th != Height) {
					Width  = tw;
					Height = th;
					SetButtonSize(UberButton, Width,
					    Height);
					button = -1;
					ub     = UberButton;
				...
				} else if (Event.xconfigure.window ==
					MyWindow &&
			...
			case EnterNotify:
				b = handle_new_position(b, Event.xcrossing.x,
				    Event.xcrossing.y);
				break;

			case MotionNotify:
				b = handle_new_position(b, Event.xmotion.x,
				    Event.xmotion.y);
				break;
			...
			case LeaveNotify:
				if (Event.xcrossing.x < 0 ||
				    ...
				    Event.xcrossing.y < 0 ||
				    Event.xcrossing.y >= Height ||

				    /* We get LeaveNotify events when the mouse
				     * enters a swallowed window of FvwmButtons,
				     * but we're not interested in these
				     * situations. */
				    !select_button(UberButton, x, y)
					 ->flags.b_Swallow ||

				    /* But we're interested in those situations
				     * when the mouse enters a window which
				     * overlaps with the swallowed window. */
				    Event.xcrossing.subwindow != None) {
					if (ActiveButton) {
						b	     = ActiveButton;
						ActiveButton = NULL;
						RedrawButton(b, DRAW_FORCE,
						    NULL);
					}
					if (CurrentButton) {
						RedrawButton(b, DRAW_FORCE,
			...
			case KeyPress:
				XLookupString(&Event.xkey, buffer, 10, &keysym,
				    0);
				if (keysym != XK_Return &&
				    keysym != XK_KP_Enter &&
				    keysym != XK_Linefeed)
			...
			case ButtonPress:
				...
				if (CurrentButton) {
					b	      = CurrentButton;
					CurrentButton = NULL;
					ActiveButton = select_button(UberButton,
					    x, y);
					RedrawButton(b, DRAW_FORCE, NULL);
					if (ActiveButton != b) {
						RedrawButton(ActiveButton,
						    DRAW_FORCE, NULL);
						b = ActiveButton;
					}
					break;
				...
				CurrentButton = b = select_button(UberButton, x,
				    y);
			...
			case UnmapNotify:
				...
				while (NextButton(&ub, &b, &button, 0)) {
					if (b->flags.b_Panel &&
						...
						b->PanelWin) {
						/* A panel has been unmapped,
						 * update the button */
						b->newflags.panel_mapped =
...
**/
int
LoadIconFile(const char *s, FvwmPicture **p, int cset)
{
...
{
	int i, x = 0, y = 0, ix, iy, tx, ty, hix, hiy, htx, hty, pix, piy, ptx,
	       pty;
	FlocaleFont *Ffont;
	...
	if (b->flags.b_Font) {
		...
		if (strncasecmp(b->font_string, "none", 4) == 0) {
			b->Ffont = NULL;
		} else if (!(b->Ffont = FlocaleLoadFont(Dpy, b->font_string,
				 MyName))) {
			b->flags.b_Font = 0;
		}
	}
	...
	if (b->flags.b_Container && b->c->flags.b_Font) {
		...
		if (strncasecmp(b->c->font_string, "none", 4) == 0) {
			b->c->Ffont = NULL;
		} else if (!(b->c->Ffont = FlocaleLoadFont(Dpy,
				 b->c->font_string, MyName))) {
			b->c->flags.b_Font = 0;
		}
	}
...
#ifdef DEBUG_LOADDATA
		fvwm_debug(__func__, "%s: Loading: Back to container 0x%06x",
		    MyName, (ushort)b);
#endif
	...
	/* $ix & $iy are dimensions of Icon
	   ...
	   Note that if No ActiveIcon is specified, Icon is displayed during
	   hover. Similarly for ActiveTitle, PressIcon & PressTitle. */
	...
	if (b->flags.b_ActiveIcon &&
	    LoadIconFile(b->active_icon_file, &b->activeicon,
		buttonColorset(b))) {
#ifdef DEBUG_LOADDATA
		fvwm_debug(__func__, ", active icon \"%s\"",
		    b->active_icon_file);
#endif
	...
	if (b->flags.b_PressIcon &&
	    LoadIconFile(b->press_icon_file, &b->pressicon,
		buttonColorset(b))) {
#ifdef DEBUG_LOADDATA
	...
	if (b->flags.b_Title && (Ffont = buttonFont(b))) {
		...
		if (buttonJustify(b) & b_Horizontal) {
			tx = buttonXPad(b) +
			    FlocaleTextWidth(Ffont, b->title, strlen(b->title));
			ty = Ffont->height;
		} else {
			tx = FlocaleTextWidth(Ffont, b->title,
			    strlen(b->title));
			ty = Ffont->height;
		}
	}
	...
	if (b->flags.b_ActiveTitle && (Ffont = buttonFont(b))) {
		...
		if (buttonJustify(b) & b_Horizontal) {
			htx = buttonXPad(b) +
			    FlocaleTextWidth(Ffont, b->activeTitle,
				strlen(b->activeTitle));
			hty = Ffont->height;
		} else {
			htx = FlocaleTextWidth(Ffont, b->activeTitle,
			    strlen(b->activeTitle));
			hty = Ffont->height;
		}
	} else {
	...
	if (b->flags.b_PressTitle && (Ffont = buttonFont(b))) {
		...
		if (buttonJustify(b) & b_Horizontal) {
			ptx = buttonXPad(b) +
			    FlocaleTextWidth(Ffont, b->pressTitle,
				strlen(b->pressTitle));
			pty = Ffont->height;
		} else {
			ptx = FlocaleTextWidth(Ffont, b->pressTitle,
			    strlen(b->pressTitle));
			pty = Ffont->height;
		}
	} else {
...
static void
HandlePanelPress(button_info *b)
{
	int		  x1, y1, w1, h1;
	int		  x2, y2, w2, h2;
	int		  px, py, pw, ph, pbw;
	...
	if (is_mapped &&
		...
		XTranslateCoordinates(Dpy, b->PanelWin, Root, b->x, b->y, &b->x,
		    &b->y, &JunkW);
	} else {
		XWMHints wmhints;

		/* Make sure the icon is unmapped first. Needed to work properly
		 * with shaded and iconified windows. */
	...
	/* We're sure that the window is mapped and decorated now. Find the top
	 * level ancestor now. */
	MyXGrabServer(Dpy);
	lb = 0;
	tb = 0;
	...
	if (!b->panel_flags.ignore_lrborder ||
	    !b->panel_flags.ignore_tbborder) {
		ancestor = GetTopAncestorWindow(Dpy, b->PanelWin);
		if (ancestor) {
			if (XGetGeometry(Dpy, ancestor, &JunkW, &ax, &ay,
				(unsigned int *)&aw, (unsigned int *)&ah,
				(unsigned int *)&abw, &d) &&
			    XGetGeometry(Dpy, b->PanelWin, &JunkW, &px, &py,
				(unsigned int *)&pw, (unsigned int *)&ph,
				(unsigned int *)&pbw, &d) &&
			    XTranslateCoordinates(Dpy, b->PanelWin, ancestor, 0,
				0, &px, &py, &JunkW)) {
				/* calculate the 'border' width in all four
				 * directions */
				if (!b->panel_flags.ignore_lrborder) {
					lb = max(px, 0) + abw;
					rb = max((aw + abw) - (px + pw), 0) +
					    abw;
				}
				if (!b->panel_flags.ignore_tbborder) {
					tb = max(py, 0) + abw;
					bb = max((ah + abw) - (py + ph), 0) +
					    abw;
				}
			}
		}
	...
	snprintf(cmd, sizeof(cmd),
	    "Silent WindowId 0x%08x (!Sticky) MoveToDesk 0", (int)b->PanelWin);
	SendInfo(fd, cmd, b->PanelWin);
	SlideWindow(Dpy, b->PanelWin, x1, y1, w1, h1, x2, y2, w2, h2, steps,
	    b->slide_delay_ms, NULL, b->panel_flags.smooth,
	...
	CurrentButton = tmp;
	XSync(Dpy, 0);
	/* Give fvwm a chance to update the window.  Otherwise the window may
	 * end up too small.  This doesn't prevent this completely, but makes it
	 * much less likely. */
	usleep(250000);
...
**/
void
CreateUberButtonWindow(button_info *ub, int maxx, int maxy)
{
	XSizeHints    mysizehints;
	XGCValues     gcv;
	unsigned long gcm;
	...
	if (mysizehints.flags & USPosition) {
		fvwm_debug(__func__, "create(%i,%i,%u,%u,1,%u,%u)...", wx, wy,
		    mysizehints.width, mysizehints.height, (ushort)fore_pix,
		    (ushort)back_pix);
	} else {
		fvwm_debug(__func__, "create(-,-,%u,%u,1,%u,%u)...",
		    mysizehints.width, mysizehints.height, (ushort)fore_pix,
		    (ushort)back_pix);
	}
#endif

	XMoveResizeWindow(Dpy, MyWindow, wx, wy, mysizehints.width,
	    mysizehints.height);
	...
	if (ub->c->flags.b_Colorset) {
		SetWindowBackground(Dpy, MyWindow, mysizehints.width,
		    mysizehints.height, &Colorset[ub->c->colorset], Pdepth,
		    NormalGC, True);
	} else if (ub->c->flags.b_IconBack && !ub->c->flags.b_TransBack) {
		XSetWindowBackgroundPixmap(Dpy, MyWindow,
		    ub->c->backicon->picture);
	} else {
...
**/
int
My_FNextEvent(Display *Dpy, XEvent *event)
{
	fd_set	       in_fdset;
	...
	if (fvwmSelect(fd_width, SELECT_FD_SET_CAST & in_fdset, 0, 0,
		...
		if ((buttonSwallowCount(b) == 1) && b->flags.b_Hangon &&
			...
			if (b->spawn) {
				...
				fvwm_debug(__func__,
				    ...
				    "spawning own\n");
#endif
				p = module_expand_action(Dpy, screen, b->spawn,
				    NULL, UberButton->c->fore,
				    UberButton->c->back);
				if (p) {
					exec_swallow(p, b);
					free(p);
...
void
change_swallowed_window_colorset(button_info *b, Bool do_clear)
{
	Window	w	 = SwallowedWindow(b);
	Window *children = None;
	int	nchildren;

	nchildren = GetEqualSizeChildren(Dpy, w, Pdepth,
	    XVisualIDFromVisual(Pvisual), Pcmap, &children);
	SetWindowBackground(Dpy, w, buttonWidth(b), buttonHeight(b),
	    &Colorset[b->colorset], Pdepth, NormalGC, do_clear);
	while (nchildren-- > 0) {
		SetWindowBackground(Dpy, children[nchildren], buttonWidth(b),
		    buttonHeight(b), &Colorset[b->colorset], Pdepth, NormalGC,
		    do_clear);
	}
	if (children) {
		XFree(children);
...
static void
send_bg_change_to_module(button_info *b, XEvent *Event)
{
...
static void
update_root_transparency(XEvent *Event)
{
	button_info *ub, *b;
	int	     button;
	...
	if (bg_is_transparent) {
		SetWindowBackground(Dpy, MyWindow, UberButton->c->width,
		    UberButton->c->height, &Colorset[UberButton->c->colorset],
...
static void
recursive_change_colorset(container_info *c, int colorset, XEvent *Event)
{
	int	     button;
	button_info *ub, *b;
	button_info *tmpb;
...
static void
change_colorset(int colorset, XEvent *Event)
{
	if (UberButton->c->flags.b_Colorset &&
		...
		SetWindowBackground(Dpy, MyWindow, UberButton->c->width,
		    UberButton->c->height, &Colorset[colorset], Pdepth,
		    NormalGC, True);
			...
			while (NextButton(&ub, &b, &button, 0)) {
				...
				if ((buttonSwallowCount(b) == 3) && swin) {
					snprintf(cmd, sizeof(cmd),
					    ...
					    MX_PROPERTY_CHANGE_SWALLOW, 1, swin,
					    s);
					SendText(fd, cmd, 0);
				}
			}
	...
	while (NextButton(&ub, &b, &button, 0)) {
		if (b->flags.b_Hangon && matchWildcards(b->hangon, cbody)) {
			...
			if (buttonSwallowCount(b) == 1) {
				...
				b->IconWinParent = GetRealGeometry(Dpy,
				    SwallowedWindow(b), &b->x, &b->y, &b->w,
				    &b->h, &b->bw, &d);
...
**/
Window
GetRealGeometry(Display *dpy, Window win, int *x, int *y, unsigned int *w,
...
static void
GetPanelGeometry(Bool get_big, button_info *b, int lb, int tb, int rb, int bb,
    int *x, int *y, int *w, int *h)
{
	...
	case SLIDE_CONTEXT_PB: /* slide relatively to the panel button */
		...
		bw = buttonWidth(b);
		bh = buttonHeight(b);
		XTranslateCoordinates(Dpy, MyWindow, Root, bx, by, &bx, &by,
		    &JunkWin);
		break;
	case SLIDE_CONTEXT_MODULE: /* slide relatively to FvwmButtons */
	{
		...
		if (XGetGeometry(Dpy, MyWindow, &JunkWin, &bx, &by,
			(unsigned int *)&bw, (unsigned int *)&bh,
			(unsigned int *)&dum, &dum2)) {
			XTranslateCoordinates(Dpy, MyWindow, Root, bx, by, &bx,
			    &by, &JunkWin);
			break;
		} else {
...
swallow(unsigned long *body)
{
	...
	while (NextButton(&ub, &b, &button, 0)) {
		...
		if ((swin == (Window)body[0]) && (buttonSwallowCount(b) == 2)) {
			...
			p = GetRealGeometry(Dpy, swin, &b->x, &b->y, &b->w,
			    &b->h, &b->bw, &d);
#ifdef DEBUG_HANGON
			fprintf(stderr,
			...
			if (b->flags.b_Swallow) {
				...
				XReparentWindow(Dpy, swin, MyWindow, -32768,
				    -32768);
				...
				if (buttonSwallow(b) & b_UseTitle) {
					...
					b->flags.b_Title = 1;
					FlocaleGetNameProperty(XGetWMName, Dpy,
					    swin, &temp);
					if (temp.name != NULL) {
						CopyString(&b->title,
						    temp.name);
				...
				XSync(Dpy, 0);
				if (b->swallow & b_FvwmModule) {
					/* if we swallow a module we send an
					 * avertisment */
					char cmd[256];
				...
				if (FShapesSupported) {
					if (UberButton->c->flags.b_TransBack) {
						SetTransparentBackground(
						    UberButton, Width, Height);
						FShapeSelectInput(Dpy, swin,
						    FShapeNotifyMask);
					}
				}
				/* Redraw and force cleaning the background to
				 * erase the old button title. */
				RedrawButton(b, DRAW_FORCE, NULL);
...
void
exec_swallow(char *action, button_info *b)
{
