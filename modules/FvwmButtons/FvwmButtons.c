...
/* --------------------------- external functions -------------------------- */
extern void
DumpButtons(button_info *);
extern void
SaveButtons(button_info *);
...
void
SetButtonSize(button_info *, int, int);
/* main */
void
Loop(void);
void
RedrawWindow(void);
void
RecursiveLoadData(button_info *, int *, int *);
void
CreateUberButtonWindow(button_info *, int, int);
int
...
void
CheckForHangon(unsigned long *);
static Window
GetRealGeometry(Display *, Window, int *, int *, unsigned int *, unsigned int *,
    unsigned int *, unsigned int *);
static void
GetPanelGeometry(Bool get_big, button_info *b, int lb, int tb, int rb, int bb,
    int *x, int *y, int *w, int *h);
void
swallow(unsigned long *);
void
AddButtonAction(button_info *, int, char *);
char *
GetButtonAction(button_info *, int);
static void
...
#ifdef DEBUG_HANGON
						fvwm_debug(
						    __func__, "now deleted\n");
#endif
					}
				} else {
#ifdef DEBUG_HANGON
					fvwm_debug(
					    __func__, "now unswallowed\n");
#endif
...
AddButtonAction(button_info *b, int n, char *action)
{
	...
	if (!b || n < 0 || n > NUMBER_OF_EXTENDED_MOUSE_BUTTONS || !action) {
		fvwm_debug(
		    __func__, "%s: BUG: AddButtonAction failed\n", MyName);
		exit(2);
	}
	if (b->flags.b_Action) {
		if (b->action[n]) {
...
main(int argc, char **argv)
{
	...
	if (Root == None) {
		fvwm_debug(
		    __func__, "%s: Screen %d is not valid\n", MyName, screen);
		exit(1);
	}
	...
	*/
	...
	if (UberButton->c->num_buttons == 0) {
		fvwm_debug(
		    __func__, "%s: No buttons defined. Quitting\n", MyName);
		exit(0);
	}
	...
	if (!XGetGeometry(Dpy, MyWindow, &root, &x, &y, (unsigned int *)&Width,
		...
		&depth)) {
		fvwm_debug(
		    __func__, "%s: Failed to get window geometry\n", MyName);
		exit(0);
	}
	...
	SetMessageMask(fd, M_NEW_DESK | M_END_WINDOWLIST | M_MAP | M_WINDOW_NAME
			       | M_RES_CLASS | M_CONFIG_INFO | M_END_CONFIG_INFO
			       | M_RES_NAME | M_SENDCONFIG | M_ADD_WINDOW
			       | M_CONFIGURE_WINDOW | M_STRING);
		...
		if (My_FNextEvent(Dpy, &Event)) {
			...
			case ConfigureNotify: {
				...
				fev_sanitise_configure_notify(
				    &Event.xconfigure);
				while (FCheckTypedWindowEvent(
				    Dpy, MyWindow, ConfigureNotify, &event)) {
			...
			case MotionNotify:
				b = handle_new_position(
				    b, Event.xmotion.x, Event.xmotion.y);
				break;
...
RecursiveLoadData(button_info *b, int *maxx, int *maxy)
{
	...
	fvwm_debug(
	    __func__, "%s: Loading: Button 0x%06x: colors", MyName, (ushort)b);
#endif
	...
	if (b->flags.b_Font) {
		...
		if (strncasecmp(b->font_string, "none", 4) == 0) {
			b->Ffont = NULL;
		} else if (!(b->Ffont = FlocaleLoadFont(
				 Dpy, b->font_string, MyName))) {
			b->flags.b_Font = 0;
		}
	}
	...
	if (b->flags.b_Container && b->c->flags.b_Font) {
		...
		if (strncasecmp(b->c->font_string, "none", 4) == 0) {
			b->c->Ffont = NULL;
		} else if (!(b->c->Ffont = FlocaleLoadFont(
				 Dpy, b->c->font_string, MyName))) {
			b->c->flags.b_Font = 0;
		}
	}
	...
	if (b->flags.b_ActiveIcon
	    && LoadIconFile(
		b->active_icon_file, &b->activeicon, buttonColorset(b))) {
#ifdef DEBUG_LOADDATA
		fvwm_debug(
		    __func__, ", active icon \"%s\"", b->active_icon_file);
#endif
	...
	if (b->flags.b_PressIcon
	    && LoadIconFile(
		b->press_icon_file, &b->pressicon, buttonColorset(b))) {
#ifdef DEBUG_LOADDATA
	...
	if (b->flags.b_ActiveTitle && (Ffont = buttonFont(b))) {
		...
		if (buttonJustify(b) & b_Horizontal) {
			...
			htx = FlocaleTextWidth(
			    Ffont, b->activeTitle, strlen(b->activeTitle));
			hty = Ffont->height;
		}
	} else {
		htx = tx;
	...
	if (b->flags.b_PressTitle && (Ffont = buttonFont(b))) {
		...
		if (buttonJustify(b) & b_Horizontal) {
			ptx = buttonXPad(b)
			      + FlocaleTextWidth(
				  Ffont, b->pressTitle, strlen(b->pressTitle));
			pty = Ffont->height;
		} else {
			ptx = FlocaleTextWidth(
			    Ffont, b->pressTitle, strlen(b->pressTitle));
			pty = Ffont->height;
		}
	} else {
	...
	if (!b->panel_flags.ignore_lrborder
		...
		if (ancestor) {
			if (XGetGeometry(Dpy, ancestor, &JunkW, &ax, &ay,
				...
				(unsigned int *)&pbw, &d)
			    && XTranslateCoordinates(Dpy, b->PanelWin, ancestor,
				0, 0, &px, &py, &JunkW)) {
	...
	while (NextButton(&ub, &b, &button, 0)) {
		if (b->flags.b_Hangon && matchWildcards(b->hangon, cbody)) {
			...
			if (buttonSwallowCount(b) == 1) {
				...
				b->IconWinParent =
				    GetRealGeometry(Dpy, SwallowedWindow(b),
...
swallow(unsigned long *body)
{
	...
	while (NextButton(&ub, &b, &button, 0)) {
		...
		if ((swin == (Window)body[0]) && (buttonSwallowCount(b) == 2)) {
			...
			p = GetRealGeometry(
			    Dpy, swin, &b->x, &b->y, &b->w, &b->h, &b->bw, &d);
#ifdef DEBUG_HANGON
			fprintf(stderr,
...
exec_swallow(char *action, button_info *b)
{
	...
	sprintf(cmd, "FSMExecFuncWithSessionManagment \"%s\" \"%s\" \"%s\"",
