    ...
    Pixel shadow, int rev)
{
	...
	RelieveRectangle(
	    Dpy, wn, x, y, w - 1, h - 1, reliefGC, shadowGC, width);
}
	...
	if (buttonSwallowCount(b) == 3 && b->flags.b_Swallow) {
		...
		if (iw > 0 && ih > 0) {
			...
			if (!b->flags.b_Top)
				iy += (ih - b->icon_h) / 2;
			XMoveResizeWindow(
			    Dpy, b->IconWin, ix, iy, b->icon_w, b->icon_h);
		} else {
...
RedrawButton(button_info *b, int draw, XEvent *pev)
{
		...
		for (n = 0; n < 4; n++) {
			if (b->action[n]) {
				if (strncasecmp(
					...
					break;
				} else if (strncasecmp(b->action[n], "Desk", 4)
					   == 0) {
					k = sscanf(
					    &b->action[n][4], "%d%d", &i, &j);
					if (k == 2 && i == 0 && j == new_desk) {
						rev = 1;
					}
					break;
				} else if (strncasecmp(
					       b->action[n], "GotoDesk", 8)
					   == 0) {
					k = sscanf(
					    &b->action[n][8], "%d%d", &i, &j);
					if (k == 2 && i == 0 && j == new_desk) {
						rev = 1;
					}
					break;
	...
	if (clean && BW > 2 * f && BH > 2 * f) {
		...
		do_draw = (!pev
			   || frect_get_intersection(x + f, y + f, BW - 2 * f,
			       BH - 2 * f, pev->xexpose.x, pev->xexpose.y,
			       pev->xexpose.width, pev->xexpose.height, &clip));
		...
		if (b->flags.b_Container) {
			if (b->c->flags.b_Colorset) {
				...
				if ((lw > 0 && lh > 0)
				    && (!pev
					|| frect_get_intersection(lx, ly, lw,
					    lh, pev->xexpose.x, pev->xexpose.y,
					    pev->xexpose.width,
