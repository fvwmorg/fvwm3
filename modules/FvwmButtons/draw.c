...
**/
void
RelieveButton(Window wn, int width, int x, int y, int w, int h, Pixel relief,
    Pixel shadow, int rev)
{
	...
	RelieveRectangle(Dpy, wn, x, y, w - 1, h - 1, reliefGC, shadowGC,
	    width);
}
	...
	if (buttonSwallowCount(b) == 3 && b->flags.b_Swallow) {
		...
		if (iw > 0 && ih > 0) {
			if (!(buttonSwallow(b) & b_NoHints)) {
				if (!FGetWMNormalHints(Dpy, b->IconWin,
					b->hints, &supplied))
					b->hints->flags = 0;
			...
			if (!b->flags.b_Top)
				iy += (ih - b->icon_h) / 2;
			XMoveResizeWindow(Dpy, b->IconWin, ix, iy, b->icon_w,
			    b->icon_h);
		} else {
...
static int
buttonBGColorset(button_info *b)
{
	if (b->flags.b_Hangon) {
...
**/
void
RedrawButton(button_info *b, int draw, XEvent *pev)
{
	int	   i, j, k, BH, BW;
	int	   f, of, x, y, px, py;
	int	   ix, iy, iw, ih;
	...
	if (b->flags.b_Action) {
		...
		for (n = 0; n < 4; n++) {
			if (b->action[n]) {
				if (strncasecmp(b->action[n], "GotoDeskAndPage",
					15) == 0) {
					...
					break;
				} else if (strncasecmp(b->action[n], "Desk",
					       4) == 0) {
					k = sscanf(&b->action[n][4], "%d%d", &i,
					    &j);
					if (k == 2 && i == 0 && j == new_desk) {
						rev = 1;
					}
					break;
				} else if (strncasecmp(b->action[n], "GotoDesk",
					       8) == 0) {
					k = sscanf(&b->action[n][8], "%d%d", &i,
					    &j);
					if (k == 2 && i == 0 && j == new_desk) {
						rev = 1;
					}
	...
	if (clean && BW > 2 * f && BH > 2 * f) {
		...
		do_draw = (!pev ||
		    frect_get_intersection(x + f, y + f, BW - 2 * f, BH - 2 * f,
			pev->xexpose.x, pev->xexpose.y, pev->xexpose.width,
			pev->xexpose.height, &clip));

		if (b->flags.b_Container) {
			if (b->c->flags.b_Colorset) {
				if (do_draw) {
					cleaned = True;
					SetClippedRectangleBackground(Dpy,
					    MyWindow, x + f, y + f, BW - 2 * f,
					    BH - 2 * f, &clip,
					    &Colorset[b->c->colorset], Pdepth,
					    NormalGC);
				}
				...
				if ((lw > 0 && lh > 0) &&
				    (!pev ||
					frect_get_intersection(lx, ly, lw, lh,
			...
			if (do_draw) {
				cleaned = True;
				SetClippedRectangleBackground(Dpy, MyWindow,
				    x + f, y + f, BW - 2 * f, BH - 2 * f, &clip,
				    &Colorset[b->colorset], Pdepth, NormalGC);
			}
#if 0
			...
			XClearArea(Dpy, MyWindow, clip.x, clip.y, clip.width,
			    clip.height, False);
			if (cs >= 0) {
				SetClippedRectangleBackground(Dpy, MyWindow, x,
				    y, BW, BH, &clip, &Colorset[cs], Pdepth,
				    NormalGC);
			} else if (b->flags.b_Back &&
			    !UberButton->c->flags.b_Colorset) {
				XFillRectangle(Dpy, MyWindow, NormalGC, clip.x,
			...
			if (UberButton->c->flags.b_TransBack &&
				...
				if (transGC == NULL) {
					transGC = fvwmlib_XCreateGC(Dpy,
					    shapeMask, 0, &gcv);
				}
				XSetClipMask(Dpy, transGC, None);
				XSetForeground(Dpy, transGC, 0);
				XFillRectangle(Dpy, shapeMask, transGC, x, y, w,
				    h);
				...
				if (!icon || icon->mask == None || cs >= 0) {
					...
					int xx, yy, ww, hh;
					GetIconPosition(b, icon, &xx, &yy, &ww,
					    &hh);
	...
	if (title == NULL && b->flags.b_Panel &&
		...
		if (b->slide_direction == SLIDE_GEOMETRY) {
			...
			RelieveRectangle(Dpy, MyWindow, ix, iy, iw - 1, ih - 1,
			    NormalGC, ShadowGC, 1);
		} else {
			...
			if (b->indicator_sunkraise == 2) {
				DrawTrianglePattern(Dpy, MyWindow, ShadowGC,
				    NormalGC, None, ix, iy, iw, ih, 0, dir, 1,
				    0, 0);
			} else {
				DrawTrianglePattern(Dpy, MyWindow, NormalGC,
				    ShadowGC, None, ix, iy, iw, ih, 0, dir, 1,
				    0, 0);
			}
		}
	} /* panel indicator */
...
**/
void
DrawTitle(button_info *b, Window win, GC gc, XEvent *pev)
{
	int	       BH;
	int	       ix, iy, iw, ih;
	...
	if (just == 0 ||
	    ((justify & b_Horizontal) && b->flags.b_Right)) /* Left */
	{
		xpos = ix;
	...
	if (*s && l > 0 && BH >= Ffont->height) /* Clip it somehow? */
	{
		...
		/* Or else center vertically */
			...
			FwinString.y = iy +
			    (ih + Ffont->ascent - Ffont->descent) / 2;
		}
		...
		if (pev) {
			if (!frect_get_intersection(FwinString.x,
				FwinString.y - Ffont->ascent, i, Ffont->height,
				pev->xexpose.x, pev->xexpose.y,
		...
		XSetClipRectangles(Dpy, FwinString.gc, 0, 0, &clip, 1,
		    Unsorted);
		...
		if (0) {
			XClearArea(Dpy, win, clip.x, clip.y, clip.width,
			    clip.height, False);
		}
