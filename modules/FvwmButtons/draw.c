...
**/
void RedrawButton(button_info *b, int clean, XEvent *pev)
{
	int i,j,k,BH,BW;
	int f,x,y,px,py;
	int ix,iy,iw,ih;
	...
	if (FftSupport && (b->flags&b_Title)
	    && Ffont && Ffont->fftf.fftfont != NULL)
	{
		XClearArea(Dpy, MyWindow, ix, iy, iw, ih, False);
		clean = 1;
	}
	...
	if(clean && BW>2*f && BH>2*f)
	{
		...
		{
			XFillRectangle(
				Dpy,MyWindow,NormalGC,x+f,y+f,BW-2*f,BH-2*f);
		}
		else if (clean == 2)
		{
			XClearArea(
				Dpy, MyWindow, x+f, y+f, BW-2*f, BH-2*f, False);
		}
	}
