...
ParseBoxSize(char **ss, flags_type *flags)
{
	...
	*ss = GetNextTokenIndex(*ss, opts, 0, &m);
	...
	default:
		flags->b_SizeFixed = 0;
		flags->b_SizeSmart = 0;
		fvwm_debug(__func__, "%s: Illegal boxsize option \"%s\"\n",
		    MyName, s);
		break;
	}
	return;
...

{
	...
	char *positionopts[] = { "button", "module", "root", "center", "left",
		"top", "right", "bottom", "noplr", "noptb", "mlr", "mtb",
		NULL };
	...
	while (*s && *s != ')') {
		...
		case 20: /* indicator */
			...
			*indicator_size		       = 0;
			sscanf(s, "%d %s%n", indicator_size, indicator_in_out,
			    &n);
...
ParseButton(button_info **uberb, char *s)
{
	...
	if (*s == '(' && s++) {
		char *opts[] = { "back", "fore", "font", "title", "icon",
			...
			"actionignoresclientwindow", "actiononpress",
			"container", "end", "nosize", "size", "left", "right",
			"center", "colorset", "action", "id", "activeicon",
			"activetitle", "pressicon", "presstitle",
			"activecolorset", "presscolorset", "top", NULL };
		s	     = trimleft(s);
		while (*s && *s != ')') {
			...
			if (isdigit(*s) || *s == '+' || *s == '-') {
				...
				geom = seekright(&s);
				if (geom) {
					flags = XParseGeometry(geom, &x, &y, &w,
					    &h);
					if (flags & WidthValue) {
						b->BWidth = w;
					}
			...
			case 8: /* Panel */
				...
				if (is_swallow &&
					...
					if (b->spawn != NULL &&
					    (strncasecmp(b->spawn, "module",
						 6) == 0 ||
						strncasecmp(b->spawn, "fvwm",
						    4) == 0)) {
