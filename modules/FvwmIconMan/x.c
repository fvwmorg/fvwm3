...
find_frame_window(Window win, int *off_x, int *off_y)
{
	...
	while (1) {
		if (!XQueryTree(
			theDisplay, win, &root, &parent, &junkw, &junki)) {
			fvwm_debug(__func__,
		...
		if (!XGetWindowAttributes(theDisplay, win, &attr)) {
			fvwm_debug(__func__,
			    "%s: find_frame_window: XGetWindowAttrigutes "
			    "failed.\n",
			    MyName);
			return win;
		}
...
xevent_loop(void)
{
	...
	while (FPending(theDisplay)) {
		...
		case KeyPress:
			...
			theEvent.xkey.keycode = XKeysymToKeycode(
			    theDisplay, fvwm_KeycodeToKeysym(theDisplay,
      ...
      fprintf (stderr, "Expose: %i,%i,%i,%i %i\n",
	       ...
	       theEvent.xexpose.height, theEvent.xexpose.count);
			...
			ex2 = max(
			    ex2, theEvent.xexpose.x + theEvent.xexpose.width);
			ey2 = max(
			    ey2, theEvent.xexpose.y + theEvent.xexpose.height);
	      ...
	      fprintf (stderr, "\t\tExpose Purge: %i,%i,%i,%i %i\n",
		       ...
		       theEvent.xexpose.count);
				...
				ex  = min(ex, theEvent.xexpose.x);
				ey  = min(ey, theEvent.xexpose.y);
				ex2 = max(ex2, theEvent.xexpose.x
						   + theEvent.xexpose.width);
				ey2 = max(ey2, theEvent.xexpose.y
      ...
      fprintf (
		...
		case ConfigureNotify: {
			...
			while (FPending(theDisplay)
			       && FCheckTypedEvent(
				   theDisplay, ConfigureNotify, &theEvent)) {
...
X_init_manager(int man_id)
{
	...
	if (man->fontname == NULL)
		...
		man->FButtonFont =
		    FlocaleLoadFont(theDisplay, man->fontname, MyName);
	...
	if (man->tips_fontname == NULL)
		man->tips_conf->Ffont =
		    FlocaleLoadFont(theDisplay, NULL, MyName);
	else
		man->tips_conf->Ffont =
		    FlocaleLoadFont(theDisplay, man->tips_fontname, MyName);
	...
	FScreenGetScrRect(NULL, FSCREEN_PRIMARY, &man->managed_g.x,
	...
	if (man->geometry_str) {
		...
		FScreenGetScrRect(&arg, FSCREEN_BY_NAME, &man->managed_g.x,
...
create_manager_window(int man_id)
{
	...
	sizehints.flags |=
	    PBaseSize | PMinSize | PMaxSize | PWinGravity | PResizeInc;
	...
	for (i = 0; i < NUM_CONTEXTS; i++) {
		if (man->colorsets[i] != -2) {
			...
			man->backContext[i] = fvwmlib_XCreateGC(
		...
		if (man->colorsets[i] >= 0) {
			...
			man->pixmap[i] = None;
			XSetFillStyle(
			    theDisplay, man->backContext[i], FillSolid);
			if (i == DEFAULT) {
...
recreate_background(WinManager *man, Contexts i)
{
	...
	if ((i == DEFAULT && Colorset[cset].pixmap)
		...
		if (i == DEFAULT) {
		   ...
		   && !CSET_IS_TRANSPARENT_PR_TINT(cset)) {
		man->pixmap[i] =
		    CreateBackgroundPixmap(theDisplay, man->theWindow,
	...
	for (j = 0; j < globals.num_managers; j++) {
		...
		for (i = 0; i < NUM_CONTEXTS; i++) {
			...
			XSetForeground(
			    theDisplay, man->hiContext[i], man->forecolor[i]);
			XSetBackground(
			    theDisplay, man->flatContext[i], man->forecolor[i]);
			XSetForeground(
			    theDisplay, man->flatContext[i], man->backcolor[i]);
			if (!PictureUseBWOnly()) {
