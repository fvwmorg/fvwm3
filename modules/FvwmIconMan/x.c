...
find_frame_window(Window win, int *off_x, int *off_y)
{
	...
	while (1) {
		if (!XQueryTree(theDisplay, win, &root, &parent, &junkw,
			&junki)) {
			fvwm_debug(__func__,
			    "%s: find_frame_window: XQueryTree failed.\n",
			    MyName);
			return win;
		}
		if (junkw)
...
xevent_loop(void)
{
	...
	while (FPending(theDisplay)) {
		...
		if (!man) {
			ConsoleDebug(X11,
			    "Event doesn't belong to a manager\n");
			continue;
		}
		...
		case KeyPress:
			...
			theEvent.xkey.keycode = XKeysymToKeycode(theDisplay,
      ...
      fprintf (stderr, "Expose: %i,%i,%i,%i %i\n",
	       ...
	       theEvent.xexpose.height, theEvent.xexpose.count);
			...
			ex2 = max(ex2,
			    theEvent.xexpose.x + theEvent.xexpose.width);
			ey2 = max(ey2,
			    theEvent.xexpose.y + theEvent.xexpose.height);

			/* we use "clip redrawing": but this speedup redrawing
			 * without adding to much flickering */
			while (FCheckTypedWindowEvent(theDisplay,
			    theEvent.xany.window, Expose, &theEvent)) {
#if 0
      ...
      fprintf (
		...
		case EnterNotify:
			...
			b = xy_to_button(man, theEvent.xcrossing.x,
			    theEvent.xcrossing.y);
		...
		case ConfigureNotify: {
			/* when send_event is true it means a move has taken
			 * place, transparent windows must be refreshed since
			 * the server won't do this, we must not miss ones of
			 * these */
			...
			while (FPending(theDisplay) &&
			    FCheckTypedEvent(theDisplay, ConfigureNotify,
				&theEvent)) {
      ...
      fprintf (stderr, "Configure: %i,%i,%i,%i %i\n",
	       ...
	       moved);
			...
			if (moved || recreate_bg) {
				...
				if (!force_redraw) {
					r = draw_transparent_buttons(man,
					    (moved && !recreate_bg), False);
					if (r) {
		...
		case MotionNotify:
			...
			b = xy_to_button(man, theEvent.xmotion.x,
			    theEvent.xmotion.y);
		...
		default:
			if (FShapesSupported &&
			    theEvent.type == FShapeEventBase + FShapeNotify) {
...
X_init_manager(int man_id)
{
	...
	if (man->fontname == NULL)
		...
		man->FButtonFont = FlocaleLoadFont(theDisplay, man->fontname,
		    MyName);
	...
	if (man->tips_fontname != NULL)
		man->tips_conf->Ffont = FlocaleLoadFont(theDisplay,
		    man->tips_fontname, MyName);
	else if (man->fontname != NULL)
		man->tips_conf->Ffont = FlocaleLoadFont(theDisplay,
		    man->fontname, MyName);
	else
		man->tips_conf->Ffont = FlocaleLoadFont(theDisplay, NULL,
		    MyName);
	...
	for (i = 0; i < NUM_CONTEXTS; i++) {
		...
		if (man->colorsets[i] >= 0) {
			...
			man->backcolor[i] =
			    Colorset[man->colorsets[DEFAULT]].bg;
		} else if (man->backColorName[i]) {
			if (!lookup_color(man->backColorName[i],
				&man->backcolor[i])) {
		...
		if (man->colorsets[i] >= 0) {
			...
			man->forecolor[i] =
			    Colorset[man->colorsets[DEFAULT]].fg;
		} else if (man->foreColorName[i]) {
			if (!lookup_color(man->foreColorName[i],
				&man->forecolor[i])) {
		...
		if (!PictureUseBWOnly()) {
			if (man->colorsets[i] >= 0) {
				man->shadowcolor[i] =
				    Colorset[man->colorsets[i]].shadow;
				man->hicolor[i] =
				    Colorset[man->colorsets[i]].hilite;
			} else if (man->colorsets[i] == -2) {
				man->shadowcolor[i] =
				    Colorset[man->colorsets[DEFAULT]].shadow;
				man->hicolor[i] =
				    Colorset[man->colorsets[DEFAULT]].hilite;
			} else {
				man->shadowcolor[i] = GetShadow(
				    man->backcolor[i]);
	...
	if (man->button_geometry_str) {
		int val;
		x = y = width = height = 0;
		val = XParseGeometry(man->button_geometry_str, &x, &y, &width,
		    &height);
	...
	if (man->geometry_str) {
		geometry_mask = FScreenParseGeometry(man->geometry_str,
	...
	if (globals.transient) {
		...
		if (FQueryPointer(theDisplay, theRoot, &dummyroot, &dummychild,
		...
		FScreenGetScrRect(&fscr, FSCREEN_XYPOS, &man->managed_g.x,
		    &man->managed_g.y, &man->managed_g.width,
		    &man->managed_g.height);
...
create_manager_window(int man_id)
{
	...
	FScreenGetScrRect(NULL, FSCREEN_GLOBAL, &man->managed_g.x,
	    &man->managed_g.y, &man->managed_g.width, &man->managed_g.height);

	man->theWindow = XCreateWindow(theDisplay, theRoot, sizehints.x,
	    sizehints.y, man->geometry.width, man->geometry.height, 0, Pdepth,
	    InputOutput, Pvisual, winattrmask, &winattr);
	...
	for (i = 0; i < NUM_CONTEXTS; i++) {
		if (man->colorsets[i] != -2) {
			...
			man->backContext[i] = fvwmlib_XCreateGC(theDisplay,
			    man->theWindow, gcmask, &gcval);
			XSetForeground(theDisplay, man->backContext[i],
			    man->backcolor[i]);
			XSetLineAttributes(theDisplay, man->backContext[i],
			    line_width, line_style, cap_style, join_style);
		}
		gcmask		  = GCForeground;
		gcval.foreground  = man->forecolor[i];
		man->hiContext[i] = fvwmlib_XCreateGC(theDisplay,
		    man->theWindow, gcmask, &gcval);
		if (man->FButtonFont->font != NULL)
			XSetFont(theDisplay, man->hiContext[i],
			    man->FButtonFont->font->fid);
		XSetForeground(theDisplay, man->hiContext[i],
		    man->forecolor[i]);

		gcmask		    = GCForeground | GCBackground;
		gcval.foreground    = man->backcolor[i];
		gcval.background    = man->forecolor[i];
		man->flatContext[i] = fvwmlib_XCreateGC(theDisplay,
		    man->theWindow, gcmask, &gcval);
		if (!PictureUseBWOnly()) {
			gcmask		      = GCForeground | GCBackground;
			gcval.foreground      = man->hicolor[i];
			gcval.background      = man->backcolor[i];
			man->reliefContext[i] = fvwmlib_XCreateGC(theDisplay,
			    man->theWindow, gcmask, &gcval);

			gcmask		      = GCForeground | GCBackground;
			gcval.foreground      = man->shadowcolor[i];
			gcval.background      = man->backcolor[i];
			man->shadowContext[i] = fvwmlib_XCreateGC(theDisplay,
			    man->theWindow, gcmask, &gcval);
		}
		if (man->colorsets[i] >= 0) {
			...
			man->pixmap[i] = None;
			XSetFillStyle(theDisplay, man->backContext[i],
			    FillSolid);
			if (i == DEFAULT) {
...
void
recreate_background(WinManager *man, Contexts i)
{
	...
	if ((i == DEFAULT && Colorset[cset].pixmap) ||
		...
		man->pixmap[i] = CreateBackgroundPixmap(theDisplay,
		    man->theWindow, man->geometry.width, man->geometry.height,
		    &Colorset[cset], Pdepth, man->backContext[i], False);
	...
	} else if (Colorset[cset].pixmap &&
	    !CSET_IS_TRANSPARENT_PR_TINT(cset)) {
		man->pixmap[i] = CreateBackgroundPixmap(theDisplay,
		    man->theWindow, man->geometry.boxwidth,
		    man->geometry.boxheight, &Colorset[cset], Pdepth,
		    man->backContext[i], False);
		XSetTile(theDisplay, man->backContext[i], man->pixmap[i]);
		XSetFillStyle(theDisplay, man->backContext[i], FillTiled);
	} else {
...
change_colorset(int color)
{
	...
	for (j = 0; j < globals.num_managers; j++) {
		...
		for (i = 0; i < NUM_CONTEXTS; i++) {
			...
			if (!man->hiContext[i] || !man->flatContext[i] ||
			    !man->reliefContext[i] || !man->shadowContext[i]) {
			...
			XSetForeground(theDisplay, man->hiContext[i],
			    man->forecolor[i]);
			XSetBackground(theDisplay, man->flatContext[i],
			    man->forecolor[i]);
			XSetForeground(theDisplay, man->flatContext[i],
			    man->backcolor[i]);
			if (!PictureUseBWOnly()) {
				XSetBackground(theDisplay,
				    man->reliefContext[i], man->backcolor[i]);
				XSetForeground(theDisplay,
				    man->reliefContext[i], man->hicolor[i]);
				XSetBackground(theDisplay,
				    man->shadowContext[i], man->backcolor[i]);
				XSetForeground(theDisplay,
				    man->shadowContext[i], man->shadowcolor[i]);
			}
			recreate_background(man, i);
		}
		force_manager_redraw(man);
...
void
recreate_transparent_bgs(WinManager *man)
{
	int i, color;
