...
count_nonsticky_in_hashtab(void *arg)
{
	...
	if (!(IS_STICKY_ACROSS_DESKS(win) || IS_STICKY_ACROSS_PAGES(win))
	    && !(WINDATA_ICONIFIED(win)
		 && (IS_ICON_STICKY_ACROSS_PAGES(win)
...
{
	copy_string(
	    &win->monitor, (char *)&(body->add_config_data.monitor_name));
	...
	win->geometry_set = 1;
	memcpy(
	    &(win->flags), &(body->add_config_data.flags), sizeof(win->flags));
	{
...
focus_change(FvwmPacketBody *body)
{
	...
	if (fvwm_focus_win && fvwm_focus_win->button
...
static void
visible_icon_name(FvwmPacketBody *body)
{
	WinData *win;
	Ulong	 app_id;
	Uchar *	 name = body->name_data.name.name;
...
static void
visible_name(FvwmPacketBody *body)
{
	WinData *win;
	Ulong	 app_id;
	Uchar *	 name = body->name_data.name.name;
...
static void
icon_location(FvwmPacketBody *body)
{
	Ulong	 app_id = body->minimal_data.app_id;
	WinData *win;
...
static void
window_shade(FvwmPacketBody *body, int do_shade)
{
	Window	    w;
	WinManager *man;
...
static void
property_change(FvwmPacketBody *body)
{
	WinManager *man = NULL;
	int	    j;
	...
	if (body->property_data.arg == MX_PROPERTY_CHANGE_BACKGROUND) {
		...
		if (man->swallowed) {
			...
			if (sscanf((char *)body->property_data.str, "%lu", &u)
			    == 1) {
