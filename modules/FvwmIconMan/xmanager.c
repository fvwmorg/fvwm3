			...
			while (i <= g->rows) {
				if (n * man->max_button_width <= ret.width) {
					...
					break;
					   ...
					   <= ret.width) {
					ret.cols =
					    ret.width / man->max_button_width;
					break;
				}
				i++;
			}
...
set_num_buttons(ButtonArray *buttons, int n)
{
	...
	if (n > buttons->num_buttons) {
		...
		buttons->buttons =
		    fxrealloc((void *)buttons->buttons, n, sizeof(Button *));
	...
	if (FShapesSupported) {
		...
		if (cols == 0 || n % cols == 0) {
			...
			if (man->geometry.dir & GROW_DOWN) {
				...
				rects[0].width = cols * man->geometry.boxwidth;
				rects[0].height =
				    (num_visible_rows(n, cols) - 1)
				    * man->geometry.boxheight;
				rects[1].x = 0;
				rects[1].y = rects[0].height;
				rects[1].width =
				    (n % cols) * man->geometry.boxwidth;
    ...
    GC context2)
{
	...
	if ((abs(relief) > (man->geometry.boxheight / 2))
		...
		relief = min(
		    man->geometry.boxheight / 2, man->geometry.boxwidth / 2);
...
draw_manager(WinManager *man)
{
	...
	if (man->buttons.num_windows == 0) {
		...
		/* I was having the problem where when the shape changed the
		   ...
		   the expose.
		   How can I tell when I am going to reliably get an expose
		   event? */
...
find_windows_spot(WinData *win)
{
	...
	if (man->sort != SortNone) {
		...
		if (win->button) {
			...
			if (cur - 1 >= 0
			    && compare_windows(
				   man->sort, win, bp[cur - 1]->drawn_state.win)
				   < 0) {
				start	   = cur - 1;
				finish	   = -1;
				cmp_dir	   = -1;
...
get_tips(WinManager *man, Button *b)
{
	...
	if (man->tips_formatstring && b->drawn_state.win) {
		CopyString(&s, make_display_string(b->drawn_state.win,
				   man->tips_formatstring, 0));
		free_str = s;
	} else {
