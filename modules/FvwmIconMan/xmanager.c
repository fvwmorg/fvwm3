...
static Region
GetRegion(int x, int y, int w, int h)
{
	Region	   region;
...
index_to_box(WinManager *man, int index)
{
	...
	if (man->geometry.dir & GROW_DOWN) {
		...
		index += (man->geometry.rows - num_visible_rows(n, cols)) *
		    cols;
		return index;
	}
}
...
figure_geometry(WinManager *man)
{
	...
	if (man->geometry.dir & GROW_FIXED) {
		...
		if (man->max_button_width_columns > 0) {
			man->max_button_width = ret.width /
			    man->max_button_width_columns;
		}
...
static void
release_manager_size(WinManager *man)
{
	XSizeHints size;
	long	   mask;
...
resize_window(WinManager *man)
{
	...
	/* Wait until the window has resized -- there could be a timing issue
	 * and we don't want to warn about incorrect sizehints when this is
	 * still happening.
	 */
	while (counter &&
	    (attribs.width != man->geometry.width ||
...
static char *
make_display_string(WinData *win, char *format)
{
...
set_num_buttons(ButtonArray *buttons, int n)
{
	...
	if (n > buttons->num_buttons) {
		...
		buttons->buttons = fxrealloc((void *)buttons->buttons, n,
		    sizeof(Button *));
...
void
set_win_picture(WinData *win, Pixmap picture, Pixmap mask, Pixmap alpha,
...
set_win_iconified(WinData *win, int iconified)
{
	...
	if (win->button != NULL && (win->iconified != iconified)) {
		...
		if (do_animate && win->manager->AnimCommand &&
			...
			XTranslateCoordinates(theDisplay,
			    win->manager->theWindow, theRoot, win->button->x,
			    win->button->y, &abs_x, &abs_y, &junkw);
			w = win->button->w;
			h = win->button->h;
			if (man_data) {
			...
			if (iconified) {
				snprintf(string, sizeof(string),
				    "%s %d %d %d %d %d %d %d %d",
				    win->manager->AnimCommand, (int)win->x,
				    (int)win->y, (int)win->width,
				    (int)win->height, abs_x, abs_y, w, h);
			} else {
				snprintf(string, sizeof(string),
				    "%s %d %d %d %d %d %d %d %d",
				    win->manager->AnimCommand, abs_x, abs_y, w,
				    h, (int)win->x, (int)win->y,
...
clear_empty_region(WinManager *man)
{
	...
	if (n == 0 || rows * cols == 0 /* just be to safe */) {
		...
		if (n % cols == 0) {
			rects[0].x = 0;
			rects[0].y = num_visible_rows(n, cols) *
	...
	if (CSET_IS_TRANSPARENT_PR_PURE(cset)) {
		for (n = 0; n < num_rects; n++) {
			if (rects[n].width > 0 && rects[n].height > 0) {
				XClearArea(theDisplay, man->theWindow,
		...
		for (n = 0; n < num_rects; n++) {
			if (rects[n].width > 0 && rects[n].height > 0) {
				SetRectangleBackground(theDisplay,
				    man->theWindow, rects[n].x, rects[n].y,
...
set_shape(WinManager *man)
{
	if (FShapesSupported) {
		...
		if (cols == 0 || n % cols == 0) {
			...
			rects[0].height = num_visible_rows(n, cols) *
			    man->geometry.boxheight;
...
static void
draw_button_background(WinManager *man, XRectangle bounding,
    Contexts button_state)
{
	int cset = man->colorsets[button_state];

	if (cset >= 0) {
		SetRectangleBackground(theDisplay, man->theWindow, bounding.x,
		    bounding.y, bounding.width, bounding.height,
		    &Colorset[cset], Pdepth, man->backContext[button_state]);
	} else if (man->backContext[button_state] == None) {
		XClearArea(theDisplay, man->theWindow, bounding.x, bounding.y,
		    bounding.width, bounding.height, False);
	} else {
		XFillRectangle(theDisplay, man->theWindow,
		    man->backContext[button_state], bounding.x, bounding.y,
    ...
    Contexts contextId)
{
	if ((iconified == 0) || (man->relief_thickness == 0))
		...
		RelieveRectangle(theDisplay, man->theWindow, g->icon_x,
		    g->icon_y, g->icon_w - 1, g->icon_h - 1,
    ...
    XRectangle bounding)
{
	...
	frect_get_intersection(bounding.x, bounding.y, bounding.width,
	    bounding.height, g->icon_x, g->icon_y, g->icon_w, g->icon_h,
	    &inter);
	...
	if (FMiniIconsSupported && man->draw_icons && win->pic.picture) {
		if (iconified == 0 && man->draw_icons != 2) {
			...
			PGraphicsRenderPicture(theDisplay, man->theWindow,
			    &win->pic, &fra, man->theWindow,
			    man->hiContext[contextId], None, None, p_x, p_y,
			    inter.width, inter.height, inter.x, inter.y,
...
check_in_window(WinData *win)
{
	...
	if (win->manager && win->complete) {
		is_state_selected = ((!win->manager->showonlyiconic ||
					 win->iconified) &&
		...
		if (win->manager->usewinlist && DO_SKIP_WINDOW_LIST(win))
			in_viewport = 0;
		if ((win->manager->showonlyfocused &&
			win->state != FOCUS_CONTEXT) &&
		    (win->manager->showonlyfocused &&
			win->state != FOCUS_SELECT_CONTEXT))
			in_viewport = 0;
...
static void
get_gcs(WinManager *man, int state, int iconified, GC *context1, GC *context2)
{
	GC gc1, gc2;
    ...
    GC context2)
{
	...
	if ((abs(relief) > (man->geometry.boxheight / 2)) ||
		...
		relief = min(man->geometry.boxheight / 2,
		    man->geometry.boxwidth / 2);
	...
	if (state == BUTTON_EDGEUP || state == BUTTON_EDGEDOWN) {
		RelieveRectangle(theDisplay, man->theWindow, g->button_x,
		    g->button_y, g->button_w - 1, g->button_h - 1, context1,
		    context2, relief);
		RelieveRectangle(theDisplay, man->theWindow, g->button_x + 2,
		    g->button_y + 2, g->button_w - 5, g->button_h - 5, context2,
		    context1, relief);
	} else {
		RelieveRectangle(theDisplay, man->theWindow, g->button_x,
		    g->button_y, g->button_w - 1, g->button_h - 1, context1,
...
draw_button(WinManager *man, int button, int force)
{
	...
	if (dirty) {
		...
		if (dirty & STRING_CHANGED) {
			ConsoleDebug(X11, "\tString changed: %s\n",
			    win->display_string);
			copy_string(&b->drawn_state.display_string,
			    win->display_string);
	...
	if (draw_background || draw_icon || draw_string) {
		...
		b_region = GetRegion(bounding.x, bounding.y, bounding.width,
		    bounding.height);
		...
		if (clear_old_pic) {
			...
			if (!cleared_button) {
				...
				frect_get_intersection(bounding.x, bounding.y,
				    bounding.width, bounding.height, g.icon_x,
				    g.icon_y, g.icon_w, g.icon_h, &r);
				if (r.width > 0) {
					draw_button_background(man, r,
					    button_state);
				}
			}
		}
		...
		if (draw_background) {
			...
			if (!PictureUseBWOnly()) {
				get_gcs(man, button_state, win->iconified,
				    &context1, &context2);
		...
		if (draw_string) {
			...
			tr = GetRegion(g.text_x, g.text_y, g.text_w, g.text_h);
			XIntersectRegion(tr, b_region, tr);
			XSetRegion(theDisplay, man->hiContext[button_state],
			    tr);
			if (!cleared_button) {
				XRectangle r;

				frect_get_intersection(bounding.x, bounding.y,
				    bounding.width, bounding.height, g.text_x,
				    g.text_y, g.text_w, g.text_h, &r);
			...
			FlocaleDrawString(theDisplay, man->FButtonFont,
			    FwinString, 0);
			XDestroyRegion(tr);
			XSetClipMask(theDisplay, man->hiContext[button_state],
			    None);
		}
...
draw_manager(WinManager *man)
{
	...
	if (!man->flags.is_shaded && man->flags.needs_resize_after_unshade) {
	    ...
	    (man->buttons.dirty_flags & NUM_WINDOWS_CHANGED)) {
		ConsoleDebug(X11, "\tresizing manager\n");
	...
	if (FShapesSupported && man->shaped) {
		if (redraw_all || (man->dirty_flags & SHAPE_CHANGED)) {
			FShapeCombineRectangles(theDisplay, man->theWindow,
			    FShapeBounding, 0, 0, man->shape.rects,
			    man->shape.num_rects, FShapeSet, Unsorted);
			FShapeCombineRectangles(theDisplay, man->theWindow,
			    FShapeClip, 0, 0, man->shape.rects,
			    man->shape.num_rects, FShapeSet, Unsorted);
			shape_changed	= 1;
			update_geometry = 1;
		}
...
Bool
draw_transparent_buttons(WinManager *man, Bool only_moved, Bool clear_only)
{
	Button **bp;
	Contexts button_state;
	int	 i, cset;
	...
	for (i = 0; i < man->buttons.num_windows; i++) {
		...
		if ((!CSET_IS_TRANSPARENT(cset) &&
			!CSET_IS_TRANSPARENT_PR_TINT(
			    man->colorsets[DEFAULT])) ||
		    (only_moved && !man_bg_transparent &&
			!CSET_IS_TRANSPARENT_ROOT(cset))) {
			continue;
		}
		if (clear_only) {
			XClearArea(theDisplay, man->theWindow, bp[i]->x,
			    bp[i]->y, bp[i]->w, bp[i]->h, True);
			r = True;
		} else {
...
compute_weight(WinData *win)
{
	...
	for (i = 0; i < man->weighted_sorts_len; i++) {
		...
		if (sort->resname &&
		    !matchWildcards(sort->resname, win->resname)) {
			continue;
		}
		if (sort->classname &&
		    !matchWildcards(sort->classname, win->classname)) {
			continue;
		}
		if (sort->titlename &&
		    !matchWildcards(sort->titlename, win->titlename)) {
			continue;
		}
		if (sort->iconname &&
		    !matchWildcards(sort->iconname, win->iconname)) {
			continue;
		}
		return sort->weight;
	}
...
compare_windows(SortType type, WinData *a, WinData *b)
{
	...
	if (type == SortId) {
		...
		return strcasecmp((a->display_string) ? a->display_string : "",
	...
	if (offset > 0) {
		for (i = finish; i >= start; i--) {
			...
			if (bp[i + offset]->drawn_state.win)
				bp[i + offset]->drawn_state.win->button =
				    bp[i + offset];
		}
		for (i = 0; i < offset; i++)
...
resort_windows_button(WinData *win)
{
	...
	if (new_spot != cur_spot) {
		...
		if (selected_index >= 0) {
			move_highlight(man,
			    man->buttons.buttons[selected_index]);
		}
	}
}
...
man_exposed(WinManager *man, XEvent *theEvent)
{
	...
	if (0 && FHaveShapeExtension && man->shaped) {
		 ...
		 * to match which expose event results from which shape
		 * change.*/
		/* seems fixed ? olicha */
		if (man->buttons.num_windows) {
			for (i = 0; i < man->buttons.num_windows; i++) {
	...
	if (man->buttons.num_windows) {
		for (i = 0; i < man->buttons.num_windows; i++) {
			...
			if (fvwmrect_do_rectangles_intersect(&r1, &r2)) {
				bp[i]->drawn_state.ex = max(r1.x, r2.x);
				bp[i]->drawn_state.ey = max(r1.y, r2.y);
				bp[i]->drawn_state.ew = min(r1.x + r1.width,
							    r2.x + r2.width) -
				    max(r1.x, r2.x);
				bp[i]->drawn_state.eh = min(r1.y + r1.height,
							    r2.y + r2.height) -
				    max(r1.y, r2.y);
				bp[i]->drawn_state.dirty_flags |= REDRAW_BUTTON;
			}
...
static char *
get_tips(WinManager *man, Button *b)
{
	ButtonGeometry g;
	char	      *s;
	...
	if (man->tips_formatstring && b->drawn_state.win) {
		CopyString(&s,
		    make_display_string(b->drawn_state.win,
			man->tips_formatstring));
		free_str = s;
	} else {
...
void
tips_on(WinManager *man, Button *b)
{
	char *tips_str;

	if ((tips_str = get_tips(man, b)) != NULL) {
		FTipsOn(theDisplay, man->theWindow, man->tips_conf, (void *)b,
...
void
tips_update_label(WinManager *man)
{
	char *tips_str;
