...
int
check_resolution(WinManager *manager, WinData *win)
{
	...
	if ((manager->res.type & NO_SHOW_SCREEN) &&
	    (manager->scr == NULL ||
...
static int
iconmanager_show(WinManager *man, char *tname, char *iname, char *rname,
    char *cname)
{
	StringEl *string;
...
check_win_complete(WinData *p)
{
	...
	if (p->geometry_set && p->monitor && p->resname && p->classname &&
	    p->iconname && p->titlename && p->app_id_set) {
		p->complete = 1;
