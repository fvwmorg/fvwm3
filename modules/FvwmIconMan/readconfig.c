...
print_args(int numargs, BuiltinArg *args)
{
	...
	for (i = 0; i < numargs; i++) {
		...
		case IntArg:
			ConsoleDebug(CONFIG, "Int: %d ",
			    args[i].value.int_value);
			break;

		case StringArg:
			ConsoleDebug(CONFIG, "String: %s ",
...
print_binding(Binding *binding)
{
	...
	while (func) {
		...
		if (i > num_builtins) {
			ConsoleDebug(CONFIG, "\tFunction: not found 0x%x ",
			    (unsigned int)func->func);
		}
...
{
	Function     *ftype = fxmalloc(sizeof(Function));
	char	     *ptr, *name, *tok;
	int	      j, flag;
	...
	if (builtin_functions_i) {
		...
		for (j = 0;
			...
			case IntArg:
				ptr = DoGetNextToken(ptr, &tok, NULL, ",",
				    pstop_char);
				if (!tok) {
					ConsoleMessage(
					    "%s: too few arguments\n",
		...
		for (f = tail; f; f = f->prev) {
			for (j = 0; j < (f->numargs); ++j) {
				if (f->args[j].type == JmpArg) {
					ConsoleMessage(
					    "Attempt to jump to non-existant label %s; "
...
ParseMouseEntry(char *tline)
{
	...
	if (token != NULL) {
		...
		if (n1 == 1 &&
		    (button < 0 || button > NUMBER_OF_EXTENDED_MOUSE_BUTTONS)) {
			/* syntax error */
			n1 = 0;
		}
		Free(token);
	...
	if ((mods & AnyModifier) && (mods & (~AnyModifier))) {
		ConsoleMessage(
		    "Binding specified AnyModifier and other modifiers too. "
		    "Excess modifiers will be ignored.");
	}

	new		= fxcalloc(1, sizeof(Binding));
...
ParseKeyEntry(char *tline)
{
	...
	if ((mods & AnyModifier) && (mods & (~AnyModifier))) {
		ConsoleMessage(
		    "Binding specified AnyModifier and other modifiers too. Excess modifiers will be ignored.");
	}
	...
	for (i = min; i <= max; i++) {
		if (fvwm_KeycodeToKeysym(theDisplay, i, 0, 0) == keysym) {
			...
			ConsoleDebug(CONFIG, "Key: %d %s %d %s\n", i,
			    new->key_name, mods, (char *)new->Action);
		}
	}
	return new;
}
...
read_next_cmd(ReadOption flag)
{
	...
	case READ_OPTION:
		...
		if (!*cur_pos) {
			status = READ_LINE;
		} else if (isdigit(*retstring) ||
		    !strncmp(retstring, "transient", 9)) {
			status = READ_OPTION;
		} else if (*cur_pos) {
			status = READ_ARG;
		}
...
static void
handle_resolution_config(int man, char *line)
{
...
static void
add_weighted_sort(WinManager *man, WeightedSort *weighted_sort)
{
	WeightedSort *p;

	if (man->weighted_sorts_len == man->weighted_sorts_size) {
		man->weighted_sorts_size += 16;
		man->weighted_sorts = fxrealloc((char *)man->weighted_sorts,
...
read_in_resources(void)
{
			...
			if (!strcasecmp(option1, "action")) {
				...
				if (!strcasecmp(p, "mouse")) {
					...
					ConsoleMessage("Bad line: %s\n",
					    current_line);
					ConsoleMessage(
					    "This isn't a valid action name: %s\n",
					    p);
					continue;
				}
				...
				SET_MANAGER(manager, max_button_width_columns,
				    0);
			} else if (!strcasecmp(option1,
				...
				if (!p) {
					ConsoleMessage("Bad line: %s\n",
					    current_line);
					ConsoleMessage(
					    "Need argument to showtransient\n");
					continue;
				}
				if (!strcasecmp(p, "true")) {
					i = 1;
				...
				if (!p) {
					ConsoleMessage("Bad line: %s\n",
					    current_line);
					ConsoleMessage(
					    "Need argument to showonlyfocused\n");
					continue;
				}
				if (!strcasecmp(p, "true")) {
					i = 1;
				...
				if (!p) {
					ConsoleMessage("Bad line: %s\n",
					    current_line);
					ConsoleMessage(
					    "Need argument to showonlyicons\n");
					continue;
				}
				if (!strcasecmp(p, "true")) {
					i = 1;
				...
				SET_MANAGER(manager, fontname,
				    copy_string(&globals.managers[id].fontname,
					f));
				free(f);
				...
				ConsoleMessage(
				    "Use ManagerGeometry and ButtonGeometry.\n");
				...
				SET_MANAGER(manager, iconname,
				    copy_string(&globals.managers[id].iconname,
					token));
				Free(token);
				...
				if (!strcasecmp(p, "name")) {
					...
					/* Old options */
					ConsoleMessage(
					    "FvwmIconMan: sort option no longer takes "
					    "true or false value\n"
				...
				while (p) {
					if (!strncasecmp(p, "resource=", 9)) {
						copy_string(
						    &weighted_sort.resname,
						    p + 9);
					} else if (!strncasecmp(p,
						       "class=", 6)) {
						copy_string(
						    &weighted_sort.classname,
						    p + 6);
					} else if (!strncasecmp(p,
						       "title=", 6)) {
						copy_string(
						    &weighted_sort.titlename,
						    p + 6);
					} else if (!strncasecmp(p,
						       "icon=", 5)) {
						copy_string(
						    &weighted_sort.iconname,
						    p + 5);
					} else {
						ConsoleMessage(
						    "Unknown sortweight field: %s\n",
						    p);
						ConsoleMessage("Bad line: %s\n",
						    current_line);
					}
				...
				if (manager == -1) {
					for (i = 0; i < globals.num_managers;
					     i++) {
						add_weighted_sort(
						    &globals.managers[i],
						    &weighted_sort);
				...
				SET_MANAGER(manager, AnimCommand,
				    copy_string(
					&globals.managers[id].AnimCommand,
					token));
				Free(token);
				...
				handle_button_config(manager, ICON_CONTEXT,
				    option1);
			} else if (!strcasecmp(option1,
				       "iconandselectButton")) {
				handle_button_config(manager,
				    ICON_SELECT_CONTEXT, option1);
			} else if (!strcasecmp(option1, "plainButton")) {
				handle_button_config(manager, PLAIN_CONTEXT,
				    option1);
			} else if (!strcasecmp(option1, "selectButton")) {
				handle_button_config(manager, SELECT_CONTEXT,
				    option1);
			} else if (!strcasecmp(option1, "focusButton")) {
				handle_button_config(manager, FOCUS_CONTEXT,
				    option1);
			} else if (!strcasecmp(option1,
				       "focusandselectButton")) {
				handle_button_config(manager,
				    FOCUS_SELECT_CONTEXT, option1);
			} else if (!strcasecmp(option1, "titlebutton")) {
				handle_button_config(manager, TITLE_CONTEXT,
				    option1);
				...
				SET_MANAGER(manager, colorsets[TITLE_CONTEXT],
				    n);
				AllocColorset(n);
			} else if (!strcasecmp(option1,
				...
				SET_MANAGER(manager,
				    colorsets[FOCUS_SELECT_CONTEXT], n);
				AllocColorset(n);
				...
				SET_MANAGER(manager, colorsets[FOCUS_CONTEXT],
				    n);
				AllocColorset(n);
				...
				SET_MANAGER(manager, colorsets[SELECT_CONTEXT],
				    n);
				AllocColorset(n);
				...
				SET_MANAGER(manager, colorsets[PLAIN_CONTEXT],
				    n);
				AllocColorset(n);
				...
				SET_MANAGER(manager, colorsets[ICON_CONTEXT],
				    n);
				AllocColorset(n);
			} else if (!strcasecmp(option1,
				...
				SET_MANAGER(manager,
				    colorsets[ICON_SELECT_CONTEXT], n);
				AllocColorset(n);
				...
				SET_MANAGER(manager, tips_fontname,
				    copy_string(
					&globals.managers[id].tips_fontname,
					f));
				free(f);
				...
				SET_MANAGER(manager, tips_conf->mapped_delay,
				    n);
				...
				SET_MANAGER(manager, tips_formatstring,
				    copy_string(
					&globals.managers[id].tips_formatstring,
					token));
#if 0
	flags = parse_format_dependencies(token);
	SET_MANAGER(manager, format_depend, flags);
	Free(token);
				...
				SET_MANAGER(manager, tips_conf->border_width,
				    n);
				...
				if (n < 1) {
					ConsoleMessage(
					    "A tips offset must be positive: %s\n",
					    p);
					ConsoleMessage("Bad line: %s\n",
					    current_line);
					continue;
				...
				SET_MANAGER(manager,
				    tips_conf->justification_offset, n);
				...
				SET_MANAGER(manager, tips_conf->justification,
				    i);
			} else {
				ConsoleMessage("Bad line: %s\n", current_line);
...
void
process_dynamic_config_line(char *line)
{
	int	    manager = 0;
	char	   *token;
	...
	if (token && isdigit(*token)) {
		if (extract_int(token, &manager) == 0 || manager <= 0 ||
		    manager > globals.num_managers) {
			manager = 0;
		}
		free(token);
