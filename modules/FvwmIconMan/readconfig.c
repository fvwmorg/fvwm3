	...
	if (builtin_functions_i) {
		...
		for (j = 0;
			...
			case IntArg:
				...
				if (extract_int(
					tok, &ftype->args[j].value.int_value)
				    == 0) {
					ConsoleMessage(
...
read_in_resources(void)
{
			...
			if (!strcasecmp(option1, "action")) {
				...
				for (i = 0; i < NUM_CONTEXTS; i++) {
					if (i != DEFAULT && i != TITLE_CONTEXT)
						SET_MANAGER(
						    manager, colorsets[i], -2);
					else
						SET_MANAGER(
						    manager, colorsets[i], n);
				}
				AllocColorset(n);
				...
				SET_MANAGER(
				    manager, max_button_width_columns, 0);
			} else if (!strcasecmp(
				       option1, "maxbuttonwidthbycolumns")) {
				p = read_next_cmd(READ_ARG);
				if (!p) {
					n = 0;
				} else if (extract_int(p, &n) == 0) {
				...
				SET_MANAGER(
				    manager, max_button_width_columns, n);
				...
				if (!FMiniIconsSupported) {
					ConsoleMessage("DrawIcons support not "
						       "compiled in\n");
				} else {
					p = read_next_cmd(READ_ARG);
					if (!p) {
				...
				if (!p) {
					ConsoleMessage(
					    "Bad line: %s\n", current_line);
					ConsoleMessage("Need argument to "
						       "showonlyfocused\n");
					continue;
				}
				if (!strcasecmp(p, "true")) {
				...
				ConsoleDebug(
				    CONFIG, "Setting shownoicons to: %d\n", i);
				...
				SET_MANAGER(manager, fontname,
				    copy_string(
					&globals.managers[id].fontname, f));
				free(f);
				...
				handle_button_config(
				    manager, ICON_CONTEXT, option1);
			} else if (!strcasecmp(
				       option1, "iconandselectButton")) {
				handle_button_config(
				    manager, ICON_SELECT_CONTEXT, option1);
				...
				SET_MANAGER(
				    manager, colorsets[TITLE_CONTEXT], n);
				AllocColorset(n);
			} else if (!strcasecmp(
				       option1, "focusandselectcolorset")) {
				p = read_next_cmd(READ_ARG);
				if (!p) {
					ConsoleMessage(
					    "Bad line: %s\n", current_line);
				...
				SET_MANAGER(
				    manager, colorsets[FOCUS_CONTEXT], n);
				AllocColorset(n);
				...
				SET_MANAGER(
				    manager, colorsets[SELECT_CONTEXT], n);
				AllocColorset(n);
				...
				SET_MANAGER(
				    manager, colorsets[PLAIN_CONTEXT], n);
				AllocColorset(n);
				...
				SET_MANAGER(
				    manager, colorsets[ICON_CONTEXT], n);
				AllocColorset(n);
			} else if (!strcasecmp(
				       option1, "iconandselectcolorset")) {
				p = read_next_cmd(READ_ARG);
				if (!p) {
					ConsoleMessage(
					    "Bad line: %s\n", current_line);
				...
				SET_MANAGER(
				    manager, colorsets[ICON_SELECT_CONTEXT], n);
				AllocColorset(n);
				...
				SET_MANAGER(
				    manager, tips_conf->mapped_delay, n);
	...
	Free(token);
				...
				SET_MANAGER(
				    manager, tips_conf->border_width, n);
				...
				if (n < 1) {
					ConsoleMessage("A tips offset must be "
						       "positive: %s\n",
					    p);
					ConsoleMessage(
					    "Bad line: %s\n", current_line);
				...
				SET_MANAGER(
				    manager, tips_conf->placement_offset, n);
				p = read_next_cmd(READ_ARG);
				if (!p) {
					ConsoleMessage(
					    "Bad line: %s\n", current_line);
				...
				SET_MANAGER(
				    manager, tips_conf->justification, i);
			} else {
				ConsoleMessage("Bad line: %s\n", current_line);
