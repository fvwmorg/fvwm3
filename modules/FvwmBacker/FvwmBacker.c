...
#define ERR
#define fvwm_msg(t, l, f) \
	fprintf(stderr, "[fvwm][FvwmBacker]: <<ERROR>> %s\n", f)

unsigned long
BackerGetColor(char *color);
...
main(int argc, char **argv)
{
	...
	SetMessageMask(fvwm_fd, M_NEW_PAGE | M_NEW_DESK | M_CONFIG_INFO
...
void
DeleteRootAtoms(Display *dpy2, Window root2)
{
	Atom	       type;
	int	       format;
...
void
SetRootAtoms(Display *dpy2, Window root2, Pixmap RootPix)
{
	long l_root_pix;

	l_root_pix = RootPix;
	if (RetainPixmap && RootPix != None) {
		XChangeProperty(dpy2, root2, XA_ESETROOT_PMAP_ID, XA_PIXMAP, 32,
		    PropModeReplace, (unsigned char *)&l_root_pix, 1);
		XChangeProperty(dpy2, root2, XA_XROOTPMAP_ID, XA_PIXMAP, 32,
		    PropModeReplace, (unsigned char *)&l_root_pix, 1);
	} else {
		XChangeProperty(dpy2, root2, XA_XSETROOT_ID, XA_PIXMAP, 32,
		    PropModeReplace, (unsigned char *)&l_root_pix, 1);
	}
}

void
SetDeskPageBackground(const Command *c)
{
	Display *dpy2  = NULL;
	Window	 root2 = None;
	int	 screen2;
	...
	case 2: /* colorset */
		...
		if (!dpy2) {
			fvwm_debug(
			    __func__, "Fail to create a forking dpy, Exit!");
			exit(2);
		}
		...
		case 2:
			...
			if (CSET_IS_TRANSPARENT(c->colorset)) {
				...
				pix = CreateBackgroundPixmap(dpy2, root2,
				...
				SetWindowBackground(dpy2, root2, MyDisplayWidth,
				    MyDisplayHeight, &Colorset[c->colorset],
				    DefaultDepth(dpy2, screen2),
		...
		case 1: /* Process a solid color request */
			if (RetainPixmap) {
				...
				gc		= fvwmlib_XCreateGC(
					 dpy2, root2, GCForeground, &xgcv);
				pix = XCreatePixmap(dpy2, root2, 1, 1,
				    DefaultDepth(dpy2, screen2));
				XFillRectangle(dpy2, pix, gc, 0, 0, 1, 1);
				XFreeGC(dpy2, gc);
			}
 ...
 */
void
ExecuteMatchingCommands(int colorset, int changed)
{
...
ParseConfigLine(char *line)
{
	...
	if (strlen(line) > 1) {
		if (strncasecmp(line, configPrefix, cpl) == 0) {
			if (strncasecmp(line + cpl, "RetainPixmap", 12) == 0) {
				RetainPixmap = True;
			} else if (strncasecmp(
				       line + cpl, "DoNotRetainPixmap", 17)
				   == 0) {
				RetainPixmap = False;
			} else {
				AddCommand(line + cpl);
	    ...
	    *parens_run
		...
		if (StrEquals(name, "Desk")) {
			if (GetNextToken(option_val, &value) == NULL) {
				fvwm_debug(
				    __func__, "Desk option requires a value");
				return 1;
			}
			if (!StrEquals(value, "*")) {
			...
			*do_ignore_desk = False;
			...
			if ((option_val = GetNextToken(option_val, &value))
			    == NULL) {
				fvwm_debug(
				    __func__, "Page option requires 2 values");
				return 1;
			}
			if (!StrEquals(value, "*"))
				this->x = atoi(value);
			...
			if (GetNextToken(option_val, &value) == NULL) {
				fvwm_debug(
				    __func__, "Desk option requires 2 values");
				return 1;
			}
			if (!StrEquals(value, "*"))
				this->y = atoi(value);
...
AddCommand(char *line)
{
	...
	if (strncasecmp(line, "Desk", 4) == 0) {
		...
		if (*line == '(') {
			...
			if (line == NULL) {
				fvwm_debug(
				    __func__, "Syntax error: no closing brace");
				free(this);
				return;
			}
	...
	if (strncasecmp(line, "-solid", 6) == 0) {
		...
		this->solidColor = (!token || !*token) ? BlackPixel(dpy, screen)
