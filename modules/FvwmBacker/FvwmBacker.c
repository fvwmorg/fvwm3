...
main(int argc, char **argv)
{
	...
	SetMessageMask(fvwm_fd,
	    M_NEW_PAGE | M_NEW_DESK | M_CONFIG_INFO | M_END_CONFIG_INFO |
		M_SENDCONFIG);
	...
	case 2: /* colorset */
		...
		case 2:
			...
			if (CSET_IS_TRANSPARENT(c->colorset)) {
				...
				if (pix != None) {
					XSetWindowBackgroundPixmap(dpy2, root2,
					    pix);
		...
		case 1: /* Process a solid color request */
			if (RetainPixmap) {
				...
				gc		= fvwmlib_XCreateGC(dpy2, root2,
						 GCForeground, &xgcv);
	...
	if (strlen(line) > 1) {
		if (strncasecmp(line, configPrefix, cpl) == 0) {
			if (strncasecmp(line + cpl, "RetainPixmap", 12) == 0) {
				RetainPixmap = True;
			} else if (strncasecmp(line + cpl, "DoNotRetainPixmap",
				       17) == 0) {
				RetainPixmap = False;
			} else {
				AddCommand(line + cpl);
...
Bool
ParseNewCommand(char *parens, Command *this, int *do_ignore_desk,
    int *do_ignore_page)
{
	char *option;
	char *parens_run;
...
AddCommand(char *line)
{
	...
	if (strncasecmp(line, "Desk", 4) == 0) {
		...
		if (*line == '(') {
			...
			line++;
			line = GetQuotedString(line, &parens, ")", NULL, NULL,
			    NULL);
			if (line == NULL) {
				fvwm_debug(__func__,
			...
			error = ParseNewCommand(parens, this, &do_ignore_desk,
			    &do_ignore_page);
			if (error) {
				free(this);
				return;
