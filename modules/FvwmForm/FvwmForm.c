...
FormVarsCheck(char **p)
{
	if (CF.have_env_var) { /* if cmd line vars */
		if (strlen(*p) + 200 >
			...
			CF.expand_buffer_size = strlen(*p) +
			    2000;		/* new size */
...
ParseDefaults(char *buf)
{
	...
	if (strncasecmp(buf, "*FvwmFormDefault", 16) == 0) {
		...
		e = FindToken(p, def_table,
		    struct CommandTable);	 /* find cmd in table */
		if (e != 0) {			 /* if its valid */
...
ct_ActivateOnPress(char *cp)
{
	...
	if (strcmp(option, "on") == 0 || strcmp(option, "true") == 0 ||
	    strcmp(option, "1") == 0 || (cp == 0 || strcmp(option, "")) == 0) {
...
AssignDrawTable(Item *adt_item)
{
	...
	for (find_dt = CF.roots_dt;	   /* start at front */
		...
		last_dt = find_dt;
		if ((strcasecmp(match_text_fore,
			 find_dt->dt_color_names[c_fg]) == 0) &&
		    (strcasecmp(match_text_back,
			 find_dt->dt_color_names[c_bg]) == 0) &&
		    (strcasecmp(match_item_fore,
			 find_dt->dt_color_names[c_item_fg]) == 0) &&
		    (strcasecmp(match_item_back,
			 find_dt->dt_color_names[c_item_bg]) == 0) &&
	...
	new_dt->dt_Ffont = FlocaleLoadFont(dpy, new_dt->dt_font_name,
	    module->name);
...
ReadFormData(void)
{
	...
	char  cmd_buffer[200];
	snprintf(cmd_buffer, sizeof(cmd_buffer), "read %s Quiet",
	    CF.file_to_read);
...
ct_Choice(char *cp)
{
	...
	item->header.size_x = item->header.dt_ptr->dt_Ffont->height +
	    4 * TEXT_SPC +
	    FlocaleTextWidth(item->header.dt_ptr->dt_Ffont, item->choice.text,
		item->choice.n);
...
ReadDefaults(void)
{
	...
	SendText(Channel, "*FvwmFormDefaultRead y",
	    0); /* remember you read it */
		...
		case L_LEFTRIGHT:
			...
			if (extra == 0) {
				if (line->n < 2) { /* same as L_CENTER */
					width = (CF.max_width - line->size_x) /
						2 +
					    ITEM_HSPC;
					for (i = 0; i < line->n; i++) {
...
RedrawFrame(XEvent *pev)
{
	...
	for (item = root_item_ptr; item != 0;
		...
		case I_CHOICE:
			...
			dt_ptr->dt_Fstr->x   = item->header.pos_x + TEXT_SPC +
			    item->header.size_y;
...
RedrawText(Item *item)
{
	...
	CheckAlloc(item,
	    item->header.dt_ptr); /* alloc colors and fonts needed */
	item->header.dt_ptr->dt_Fstr->len = item->text.n;
	if ((p = memchr(item->text.value, '\0',
		 item->header.dt_ptr->dt_Fstr->len)) != NULL)
	...
	item->header.dt_ptr->dt_Fstr->y	  = item->header.pos_y +
	    (CF.padVText / 2) + item->header.dt_ptr->dt_Ffont->ascent;
...
RedrawTimeout(Item *item)
{
	...
	*tmpbptr = '\0';
	...
	if ((p = memchr(item->timeout.text, '\0',
		 item->header.dt_ptr->dt_Fstr->len)) != NULL)
...
RedrawItem(Item *item, int click, XEvent *pev)
{
	...
	case I_INPUT:
		...
		if (len > item->input.size)
			...
			item->header.dt_ptr->dt_Fstr->len = item->input.size -
			    len;
			FlocaleDrawString(dpy, item->header.dt_ptr->dt_Ffont,
			    item->header.dt_ptr->dt_Fstr, FWS_HAVE_LENGTH);
		}
	...
	for (item = root_item_ptr->header.next; item != NULL;
		...
		if (item->header.win != None) {
			...
			if (CSET_IS_TRANSPARENT_PR_PURE(itemcolorset)) {
				XClearArea(dpy, item->header.win, 0, 0, 0, 0,
				    False);
				if (do_draw) {
...
DoCommand(Item *cmd)
{
	...
	for (k = 0; k < cmd->button.n; k++) {
		char *parsed_command;
		/* construct command */
		parsed_command = ParseCommand(0, cmd->button.commands[k], '\0',
		    &dn, &sp);
		myfprintf(
...
OpenWindows(void)
{
	...
	if (!CF.pointer[button_in_pointer]) {
		CF.pointer[button_in_pointer] = XCreateFontCursor(dpy,
		    XC_hand2);
	}
	...
	for (item = root_item_ptr; item != 0;
		...
		case I_INPUT:
			...
			XChangeWindowAttributes(dpy, item->header.win, CWCursor,
			    &xswa);
			if (itemcolorset >= 0) {
				SetWindowBackground(dpy, item->header.win,
				    item->header.size_x, item->header.size_y,
...
TimerHandler(evutil_socket_t fd, short ev, void *arg)
{
	...
	if (timer->timeout.timeleft <= 0) {
		...
		/* construct command */
		parsed_command = ParseCommand(0, timer->timeout.command, '\0',
		    &dn, &sp);
...
main(int argc, char **argv)
{
	...
	if (strcasecmp(module->name, "FvwmForm") !=
		...
		snprintf(cmd, sizeof(cmd), "read %s Quiet",
