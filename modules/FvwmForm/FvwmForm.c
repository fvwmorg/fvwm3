...
FormVarsCheck(char **);
static void
TimerHandler(evutil_socket_t, short int, void *);
static int
ErrorHandler(Display *dpy, XErrorEvent *error_event);
static void
SetupTimer(void)
{
...
ct_ButtonFont(char *);
static void
ct_ButtonInPointer(char *);
static void
ct_ButtonInPointerBack(char *);
static void
ct_ButtonInPointerFore(char *);
static void
ct_ButtonPointer(char *);
static void
ct_ButtonPointerBack(char *);
static void
ct_ButtonPointerFore(char *);
static void
ct_Choice(char *);
static void
ct_Command(char *);
...
ct_Text(char *);
static void
ct_InputPointer(char *);
static void
ct_InputPointerBack(char *);
static void
ct_InputPointerFore(char *);
static void
ct_Timeout(char *);
static void
ct_TimeoutFont(char *);
...
ct_Colorset(char *);
static void
ct_ItemColorset(char *);

/* Must be in Alphabetic order (caseless) */
static struct CommandTable ct_table[] = {
    {"ActivateOnPress", ct_ActivateOnPress}, {"Back", ct_Back},
    {"Button", ct_Button}, {"ButtonFont", ct_ButtonFont},
    ...
    {"ButtonPointerFore", ct_ButtonPointerFore}, {"Choice", ct_Choice},
    {"Colorset", ct_Colorset}, {"Command", ct_Command}, {"Font", ct_Font},
    {"Fore", ct_Fore}, {"Geometry", ct_Geometry}, {"GrabServer", ct_GrabServer},
    {"Input", ct_Input}, {"InputFont", ct_InputFont},
    {"InputPointer", ct_InputPointer},
    {"InputPointerBack", ct_InputPointerBack},
    {"InputPointerFore", ct_InputPointerFore}, {"ItemBack", ct_ItemBack},
    {"ItemColorset", ct_ItemColorset}, {"ItemFore", ct_ItemFore},
    {"Line", ct_Line}, {"Message", ct_Message}, {"PadVText", ct_padVText},
    {"Position", ct_Position}, {"Selection", ct_Selection},
    {"Separator", ct_Separator}, {"Text", ct_Text}, {"Timeout", ct_Timeout},
    {"TimeoutFont", ct_TimeoutFont}, {"Title", ct_Title},
    {"UseData", ct_UseData}, {"WarpPointer", ct_WarpPointer}};
...
static struct CommandTable def_table[] = {
    {"ActivateOnPress", ct_ActivateOnPress}, {"Back", ct_Back},
    {"ButtonFont", ct_ButtonFont}, {"ButtonInPointer", ct_ButtonInPointer},
    {"ButtonInPointerBack", ct_ButtonInPointerBack},
    {"ButtonInPointerFore", ct_ButtonInPointerFore},
    {"ButtonPointer", ct_ButtonPointer},
    {"ButtonPointerBack", ct_ButtonPointerBack},
    {"ButtonPointerFore", ct_ButtonPointerFore}, {"Colorset", ct_Colorset},
    {"Font", ct_Font}, {"Fore", ct_Fore}, {"InputFont", ct_InputFont},
    {"InputPointer", ct_InputPointer},
    {"InputPointerBack", ct_InputPointerBack},
    {"InputPointerFore", ct_InputPointerFore}, {"ItemBack", ct_ItemBack},
    {"ItemColorset", ct_ItemColorset}, {"ItemFore", ct_ItemFore},
    {"Read", ct_Read}, {"TimeoutFont", ct_TimeoutFont}};

/* If there were vars on the command line, do env var sustitution on
   all input. */
static void
FormVarsCheck(char **p)
{
	if (CF.have_env_var) { /* if cmd line vars */
		if (strlen(*p) + 200
		    > CF.expand_buffer_size) { /* fast and loose */
			CF.expand_buffer_size =
			    strlen(*p) + 2000;	/* new size */
			if (CF.expand_buffer) { /* already have one */
				CF.expand_buffer = fxrealloc(CF.expand_buffer,
				    CF.expand_buffer_size,
				    sizeof(CF.expand_buffer));
			} else { /* first time */
				CF.expand_buffer =
				    fxmalloc(CF.expand_buffer_size);
			}
		}
		strcpy(CF.expand_buffer, *p);
		*p = CF.expand_buffer;
		envExpand(*p,
		    CF.expand_buffer_size); /* expand the command in place */
	}
}

static void
ParseDefaults(char *buf)
{
	char *		     p;
	...
	if (strncasecmp(buf, "*FvwmFormDefault", 16) == 0) {
		...
		e = FindToken(
		    p, def_table, struct CommandTable); /* find cmd in table */
		if (e != 0) {				/* if its valid */
...
ParseConfigLine(char *buf)
{
		  ...
		  sizeof(XINERAMA_CONFIG_STRING)-1) == 0)
  {
	...
	if (strncasecmp(buf, mname, module->namelen + 1)
	    != 0) { /* If its not for me */
		return;
...
/* Expands item array */
static void
ExpandArray(Line *this_line)
{
	if (this_line->n + 1
	    >= this_line->item_array_size) { /* no empty space */
		this_line->item_array_size +=
		    ITEMS_PER_EXPANSION; /* get bigger */
		this_line->items = fxrealloc((void *)this_line->items,
...
AddToLine(Item *newItem)
{
	...
	if (cur_line->size_y < newItem->header.size_y) { /* new item bigger */
		cur_line->size_y =
		    newItem->header.size_y; /* then line is bigger */
	}
}
...
ct_ActivateOnPress(char *cp)
{
	...
	option[j] = 0;
	if (strcmp(option, "on") == 0 || strcmp(option, "true") == 0
	    || strcmp(option, "1") == 0
	    || (cp == 0 || strcmp(option, "")) == 0) {
		CF.activate_on_press = 1;
	} else if (strcmp(option, "off") == 0 || strcmp(option, "false") == 0
		   || strcmp(option, "0") == 0) {
...
static void
ct_GrabServer(char *cp)
{
	CF.grab_server = 1;
}
static void
ct_WarpPointer(char *cp)
{
	CF.warp_pointer = 1;
}
static void
...
static void
ct_ButtonInPointer(char *cp)
{
	int cursor;
	cursor = fvwmCursorNameToIndex(cp);
	if (cursor == -1) {
		fprintf(
		    stderr, "ButtonInPointer: invalid cursor name %s\n", cp);
	} else {
		CF.pointer[button_in_pointer] = XCreateFontCursor(dpy, cursor);
	}
}
static void
ct_ButtonPointer(char *cp)
{
	int cursor;
	cursor = fvwmCursorNameToIndex(cp);
	if (cursor == -1) {
...
static void
color_arg(Ptr_color *color_cell, char *color_name);
static void
color_arg(Ptr_color *color_cell, char *color_name)
{
...
static void
ct_ButtonInPointerBack(char *cp)
{
	color_arg(&CF.p_c[button_in_back], cp);
}
static void
ct_ButtonInPointerFore(char *cp)
{
	color_arg(&CF.p_c[button_in_fore], cp);
}
static void
ct_ButtonPointerBack(char *cp)
{
	color_arg(&CF.p_c[button_back], cp);
}
static void
ct_ButtonPointerFore(char *cp)
{
	color_arg(&CF.p_c[button_fore], cp);
}
static void
ct_InputPointerBack(char *cp)
{
	color_arg(&CF.p_c[input_back], cp);
}
static void
ct_InputPointerFore(char *cp)
{
...
static void
CheckAlloc(Item *this_item, DrawTable *dt)
{
	XGCValues xgcv;
	...
	if (Pdepth < 2) {
		...
		dt->dt_colors[c_itemlo] =
		    (itemcolorset < 0) ? GetShadow(dt->dt_colors[c_item_bg])
				       : Colorset[itemcolorset].shadow;
		dt->dt_colors[c_itemhi] =
		    (itemcolorset < 0) ? GetHilite(dt->dt_colors[c_item_bg])
...
AssignDrawTable(Item *adt_item)
{
	...
	for (find_dt = CF.roots_dt;	   /* start at front */
		...
		last_dt = find_dt;
		if ((strcasecmp(match_text_fore, find_dt->dt_color_names[c_fg])
			== 0)
		    && (strcasecmp(
			    match_text_back, find_dt->dt_color_names[c_bg])
			== 0)
		    && (strcasecmp(
			    match_item_fore, find_dt->dt_color_names[c_item_fg])
			== 0)
		    && (strcasecmp(
			    match_item_back, find_dt->dt_color_names[c_item_bg])
			== 0)
		    && (strcasecmp(match_font, find_dt->dt_font_name)
			== 0)) { /* Match */
			adt_item->header.dt_ptr =
			    find_dt; /* point item to drawtable */
			return;	     /* done */
		}		     /* end match */
	...
	new_dt->dt_Ffont =
	    FlocaleLoadFont(dpy, new_dt->dt_font_name, module->name);
	...
	myfprintf((stderr, "Created drawtable with %s %s %s %s %s\n",
	    ...
	    new_dt->dt_color_names[c_item_bg], new_dt->dt_font_name));
...
ct_Timeout(char *cp)
{
	...
	item->header.size_x = FlocaleTextWidth(item->header.dt_ptr->dt_Ffont,
				  item->timeout.text, item->timeout.len)
			      + 2 * TEXT_SPC;
	item->header.size_y =
	    item->header.dt_ptr->dt_Ffont->height + CF.padVText;
	myfprintf(
	    (stderr, "Timeout %d \"%s\" [%d, %d]\n", item->timeout.timeleft,
...
ct_Input(char *cp)
{
	...
	item->header.size_x =
	    item->header.dt_ptr->dt_Ffont->max_char_width * item->input.size
	    + 2 * TEXT_SPC + 2 * BOX_SPC;
	item->header.size_y =
	    item->header.dt_ptr->dt_Ffont->height + 3 * TEXT_SPC + 2 * BOX_SPC;
	myfprintf((stderr, "Input size_y is %d\n", item->header.size_y));

	if (CF.cur_input == 0) { /* first input field */
		item->input.next_input =
		    item; /* ring, next field is first field */
		item->input.prev_input =
		    item;	       /* ring, prev field is first field */
		CF.first_input = item; /* save loc of first item */
	} else {		       /* not first field */
		CF.cur_input->input.next_input =
		    item; /* old next ptr point to this item */
		item->input.prev_input =
		    CF.cur_input; /* current items prev is old item */
		item->input.next_input =
		    CF.first_input; /* next is first item */
		CF.first_input->input.prev_input =
		    item; /* prev in first is this item */
	}
...
ct_UseData(char *cp)
{
	...
	if (*CF.leading == 0) {
		fprintf(
		    stderr, "UseData command missing second arg, Leading\n");
...
ReadFormData(void)
{
	...
	while (GetConfigLine(Channel, &line_buf),
	    line_buf) { /* while there is some */
		if (strncasecmp(line_buf, CF.leading, leading_len)
		    == 0) { /* leading = */
			...
			PutDataInForm(
			    line_buf
			    + leading_len); /* pass arg, space, value */
	...
	if (CF.cur_input != 0) {
		...
		do {
			if (strcasecmp(var_name, item->header.name) == 0) {
				...
				strcpy(item->input.init_value,
				    cp); /* new initial value in field */
				if (item->input.value)
					free(item->input.value);
				item->input.buf	  = var_len + 1;
				item->input.value = fxmalloc(item->input.buf);
				strcpy(item->input.value,
				    cp);	/* new value in field */
	...
	do {					/* for all lines */
		for (i = 0; i < line->n; i++) { /* all items on line */
			...
			if (item->type == I_CHOICE) { /* choice is good */
				if (strcasecmp(var_name, item->header.name)
				    == 0) { /* match */
					item->choice.init_on = 0;
					if (strncasecmp(cp, "on", 2) == 0) {
						item->choice.init_on =
						    1; /* set default state */
						free(var_name); /* goto's have
								   their uses */
						return;
					}
				}
	...
	for (item = root_item_ptr; item != 0;
		...
		if (item->type == I_SELECT) { /* selection is good */
			if (strcasecmp(var_name, item->header.name)
				...
				Item *choice_ptr;
				for (i = 0; i < item->selection.n; i++) {
					choice_ptr = item->selection.choices[i];
					/* if the choice value matches the
					 * selection */
					if (strcmp(choice_ptr->choice.value,
						var_value)
					    == 0) {
...
ct_Choice(char *cp)
{
	...
	item->header.size_y =
	    item->header.dt_ptr->dt_Ffont->height + 2 * TEXT_SPC;
...
ct_Button(char *cp)
{
	...
	item->header.size_y =
	    item->header.dt_ptr->dt_Ffont->height + 2 * TEXT_SPC + 2 * BOX_SPC;
	...
	myfprintf((stderr, "Created button, fore %s, bg %s, text %s\n",
	    ...
	    item->header.dt_ptr->dt_color_names[c_item_bg], item->button.text));
}
static void
ct_Command(char *cp)
{
...
ReadDefaults(void)
{
	...
	SendText(
	    Channel, "*FvwmFormDefaultRead y", 0); /* remember you read it */
		...
		case L_LEFTRIGHT:
			...
			if (extra == 0) {
				if (line->n < 2) { /* same as L_CENTER */
					...
					extra = (CF.max_width - line->size_x)
						/ (line->n - 1);
					width = ITEM_HSPC;
					for (i = 0; i < line->n; i++) {
						line->items[i]->header.pos_x =
...
Restart(void)
{
	...
	for (item = root_item_ptr; item != 0;
		...
		case I_INPUT:
			...
			if (item->input.value
			    && item->input.value[0] != 0) { /* ? to save */
				if (item->input.value_history_ptr
				    == 0) { /* no history yet */
					item->input.value_history_ptr =
					    fxcalloc(sizeof(char *), 50);
					item->input.value_history_ptr[0] =
					    fxstrdup(item->input.value);
					...
					int prior;
					prior =
					    item->input.value_history_count - 1;
					if (prior < 0) {
						for (prior = VH_SIZE - 1;
						     CF.cur_input->input
							 .value_history_ptr
					...
					myfprintf((stderr,
					    ...
					    item->input
						.value_history_ptr[prior]));

					if (strcmp(item->input.value,
						item->input
						    .value_history_ptr[prior])
					    != 0) { /* different value */
						if (item->input.value_history_ptr
...
RedrawFrame(XEvent *pev)
{
	...
	if (FftSupport) {
		...
		while (item != 0 && !clear) {
			if ((item->type == I_TEXT || item->type == I_CHOICE)
			    && item->header.dt_ptr->dt_Ffont->fftf.fftfont
				   != NULL)
				clear = True;
			item = item->header.next;
		}
		if (clear && pev) {
			XClearArea(dpy, CF.frame, pev->xexpose.x,
			    pev->xexpose.y, pev->xexpose.width,
			    pev->xexpose.height, False);
		} else {
	...
	for (item = root_item_ptr; item != 0;
		...
		case I_CHOICE:
			...
			dt_ptr->dt_Fstr->x =
			    item->header.pos_x + TEXT_SPC + item->header.size_y;
			dt_ptr->dt_Fstr->y = item->header.pos_y + TEXT_SPC
					     + dt_ptr->dt_Ffont->ascent;
			dt_ptr->dt_Fstr->len		    = item->choice.n;
			dt_ptr->dt_Fstr->flags.has_colorset = False;
			if (itemcolorset >= 0) {
				dt_ptr->dt_Fstr->colorset =
				    &Colorset[itemcolorset];
				dt_ptr->dt_Fstr->flags.has_colorset = True;
			}
			FlocaleDrawString(dpy, dt_ptr->dt_Ffont,
			    dt_ptr->dt_Fstr, FWS_HAVE_LENGTH);
			break;
		case I_SEPARATOR:
			myfprintf(
			    (stderr, "redraw_frame calling RedrawSeparator\n"));
...
void
RedrawText(Item *item)
{
	char *p;

	CheckAlloc(
	    item, item->header.dt_ptr); /* alloc colors and fonts needed */
	item->header.dt_ptr->dt_Fstr->len = item->text.n;
	if ((p = memchr(
		 item->text.value, '\0', item->header.dt_ptr->dt_Fstr->len))
	    != NULL)
	...
	item->header.dt_ptr->dt_Fstr->y =
	    item->header.pos_y + (CF.padVText / 2)
	...
	FlocaleDrawString(dpy, item->header.dt_ptr->dt_Ffont,
...
RedrawSeparator(Item *item)
{
	...
	CheckAlloc(
	    item, item->header.dt_ptr); /* alloc colors and fonts needed */
	if (item->header.dt_ptr
	    && item->header.dt_ptr->dt_colors[c_itemlo]) { /* safety */
		XSetForeground(dpy, item->header.dt_ptr->dt_item_GC,
		    item->header.dt_ptr->dt_colors[c_itemlo]);
		XDrawLine(dpy, item->header.win,
		    item->header.dt_ptr->dt_item_GC, 0, 0, item->header.size_x,
		    0);
		XSetForeground(dpy, item->header.dt_ptr->dt_item_GC,
		    item->header.dt_ptr->dt_colors[c_itemhi]);
		XDrawLine(dpy, item->header.win,
		    item->header.dt_ptr->dt_item_GC, 0, 1, item->header.size_x,
		    1);
...
void
RedrawTimeout(Item *item)
{
	char *p;
	char *tmpbuf, *tmpptr, *tmpbptr;
	int   reallen;

	XClearArea(dpy, CF.frame, item->header.pos_x, item->header.pos_y,
	    item->header.size_x, item->header.size_y, False);

	tmpbuf	= fxmalloc(item->timeout.len + 6);
	tmpbptr = tmpbuf;
	for (tmpptr = item->timeout.text;
	     *tmpptr != '\0' && !(tmpptr[0] == '%' && tmpptr[1] == '%');
	     tmpptr++) {
		*tmpbptr = *tmpptr;
		tmpbptr++;
	}
	...
	*tmpbptr = '\0';
	...
	item->header.size_x =
	    FlocaleTextWidth(item->header.dt_ptr->dt_Ffont, tmpbuf, reallen)
	    + 2 * TEXT_SPC;
	item->header.size_y =
	    item->header.dt_ptr->dt_Ffont->height + CF.padVText;
	...
	if ((p = memchr(
		 item->timeout.text, '\0', item->header.dt_ptr->dt_Fstr->len))
	    != NULL)
...
/* redraw an item */
void
RedrawItem(Item *item, int click, XEvent *pev)
{
	int		dx, dy, len, x;
	...
	myfprintf((stderr, "%s: RedrawItem expose x=%d/y=%d h=%d/w=%d\n",
	    module->name, (int)pev->xexpose.x, (int)pev->xexpose.y,
	    (int)pev->xexpose.width, (int)pev->xexpose.height));
	...
	if (pev) {
		...
		r.width	   = pev->xexpose.width;
		r.height   = pev->xexpose.height;
		text_inter = frect_get_intersection(r.x, r.y, r.width, r.height,
		    inter.x, inter.y, inter.width, inter.height, &inter);
	} else {
	...
	case I_INPUT:
		...
		if (frect_get_intersection(r.x, r.y, r.width, r.height, inter.x,
			inter.y, inter.width, inter.height, &inter)) {
			XClearArea(dpy, item->header.win, inter.x, inter.y,
		...
		XDrawSegments(dpy, item->header.win,
		    item->header.dt_ptr->dt_item_GC, xsegs, 4);
		XSetForeground(dpy, item->header.dt_ptr->dt_item_GC,
		    item->header.dt_ptr->dt_colors[c_itemhi]);
		...
		if (len > item->input.size)
			...
			item->header.dt_ptr->dt_Fstr->y =
			    BOX_SPC + TEXT_SPC
			    + item->header.dt_ptr->dt_Ffont->ascent;
			item->header.dt_ptr->dt_Fstr->len =
			    item->input.size - len;
			FlocaleDrawString(dpy, item->header.dt_ptr->dt_Ffont,
			    item->header.dt_ptr->dt_Fstr, FWS_HAVE_LENGTH);
		}
		...
		item->header.dt_ptr->dt_Fstr->y =
		    BOX_SPC + TEXT_SPC + item->header.dt_ptr->dt_Ffont->ascent;
		...
		myfprintf(
		    (stderr, "Just drew input field. click %d\n", (int)click));
		break;
	case I_CHOICE:
		...
		if (item->choice.on) {
			...
			if (item->choice.sel->selection.key == IS_MULTIPLE) {
				...
				XDrawSegments(dpy, item->header.win,
				    item->header.dt_ptr->dt_item_GC, xsegs, 2);
			} else {
				XDrawArc(dpy, item->header.win,
				    item->header.dt_ptr->dt_item_GC, 5, 5,
	...
	case I_BUTTON:
		...
		if (FftSupport) {
			if (item->header.dt_ptr->dt_Ffont->fftf.fftfont != NULL)
				XClearArea(dpy, item->header.win, inter.x,
				    inter.y, inter.width, inter.height, False);
		}
		FlocaleDrawString(dpy, item->header.dt_ptr->dt_Ffont,
		    item->header.dt_ptr->dt_Fstr, FWS_HAVE_LENGTH);
		myfprintf(
		    (stderr, "Just put %s into a button\n", item->button.text));
		break;
	case I_SEPARATOR:
		RedrawSeparator(item);
		break;
...
void
UpdateRootTransapency(Bool pr_only, Bool do_draw)
{
	Item *item;
	...
	if (CSET_IS_TRANSPARENT(colorset)) {
		if (CSET_IS_TRANSPARENT_PR_PURE(colorset)) {
			...
			SetWindowBackground(dpy, CF.frame, CF.max_width,
			    CF.total_height, &Colorset[(colorset)], Pdepth,
...
DoCommand(Item *cmd)
{
	...
	for (k = 0; k < cmd->button.n; k++) {
		...
		myfprintf(
		    (stderr, "Final command[%d]: [%s]\n", k, parsed_command));

		/* send command */
		if (parsed_command[0] == '!') { /* If command starts with ! */
			int n;

			n = system(parsed_command
	...
	if (CF.last_error) { /* if form has last_error field */
		memset(CF.last_error->text.value, ' ',
		    CF.last_error->text.n); /* clear */
		/* To do this more elegantly, the window resize logic should
		   recalculate size_x for the Message as the window resizes.
		   Right now, just clear a nice wide area. dje */
		XClearArea(dpy, CF.frame, CF.last_error->header.pos_x,
		    CF.last_error->header.pos_y,
		    /* CF.last_error->header.size_x, */
		    2000, CF.last_error->header.size_y, False);
	} /* end form has last_error field */
	if (cmd->button.key == IB_QUIT) {
		if (CF.grab_server)
			XUngrabServer(dpy);
		/* This is a temporary bug workaround for the pipe drainage
		 * problem */
		SendQuitNotification(Channel); /* let commands complete */
		/* Note how the window is withdrawn, but execution continues
		   until the quit notifcation catches up with this module...
...
OpenWindows(void)
{
	...
	myfprintf((stderr,
	    ...
	    (int)CF.screen_background, (int)GetColor(screen_background_color),
	    (int)Colorset[colorset].bg, (int)colorset));
	...
	if (CF.have_geom) {
		...
		if (CF.yneg) {
			y = DisplayHeight(dpy, screen) - CF.total_height
			    + CF.gy;
		...
		FScreenCenterOnScreen(NULL, FSCREEN_CURRENT, &x, &y,
		    CF.max_width, CF.total_height);
	}
	...
	myfprintf((stderr,
	    ...
	    screen_background_color, (int)xswa.background_pixel,
	    (int)BlackPixel(dpy, screen)));
	CF.frame = XCreateWindow(dpy, root, x, y, CF.max_width, CF.total_height,
	...
	for (item = root_item_ptr; item != 0;
		...
		case I_INPUT:
			...
			CheckAlloc(item,
			    item->header
				.dt_ptr); /* alloc colors and fonts needed */
			item->header.win = XCreateSimpleWindow(dpy, CF.frame,
			    item->header.pos_x, item->header.pos_y,
			    item->header.size_x, item->header.size_y, 0,
			    CF.screen_background,
			    item->header.dt_ptr->dt_colors[c_item_bg]);
		...
		case I_SEPARATOR:
			myfprintf((stderr,
			    "Checking alloc during OpenWindow on separator\n"));
			CheckAlloc(item,
			    item->header
				.dt_ptr); /* alloc colors and fonts needed */
			item->header.size_x = CF.max_width - 6;
			myfprintf((stderr, "Create win %d/%d %d/%d\n", 3,
			    item->header.pos_y, item->header.size_x, 2));
			item->header.win = XCreateSimpleWindow(dpy, CF.frame, 3,
			    item->header.pos_y, item->header.size_x, 2, 0,
			    CF.screen_background,
...
static void
process_message(unsigned long, unsigned long *); /* proto */
static void
ParseActiveMessage(char *); /* proto */
...
process_message(unsigned long type, unsigned long *body)
{
	...
	case M_STRING:
		if (CF.last_error) { /* if form has message area */
			/* ignore form size, its OK to write outside the window
			 * boundary */
			int   msg_len;
			char *msg_ptr;
			...
			memset(CF.last_error->text.value, ' ',
			    CF.last_error->text.n); /* clear */
			XClearArea(dpy, CF.frame, CF.last_error->header.pos_x,
			    CF.last_error->header.pos_y, 2000,
			...
			if (CF.last_error->text.n
			    <= msg_len) { /* if message wont fit */
				CF.last_error->text.value = fxrealloc(
				    CF.last_error->text.value, msg_len * 2,
...
static struct CommandTable am_table[] = {
    {"Map", am_Map}, {"Stop", am_Stop}, {"UnMap", am_UnMap}};

/* This is similar to the other 2 "Parse" functions. */
static void
ParseActiveMessage(char *buf)
{
	char *		     p;
	struct CommandTable *e;
	if (buf[strlen(buf) - 1] == '\n') {  /* if line ends with newline */
	...
	if (strncasecmp(buf, "Colorset", 8) == 0) {
		...
		if (n == colorset || n == itemcolorset) {
			...
			if (colorset >= 0) {
				SetWindowBackground(dpy, CF.frame, CF.max_width,
				    CF.total_height, &Colorset[(colorset)],
				    Pdepth, root_item_ptr->header.dt_ptr->dt_GC,
				    False);
				RedrawFrame(NULL);
			}
...
am_Map(char *cp)
{
	...
	XMapSubwindows(dpy, CF.frame);
	if (CF.warp_pointer) {
		FWarpPointer(dpy, None, CF.frame, 0, 0, 0, 0, CF.max_width / 2,
		    CF.total_height - 1);
	}
...
MainLoop(void)
{
	...
	while (!isTerminated) {
		/* TA:  20091219:  Automatically flush the buffer from the
		 * XServer and process each request as we receive them.
		 */
		while (FPending(dpy))
			ReadXServer();
		...
		/* TA:  20091219:  Using XFlush() here was always a nasty hack!
		 * See comments above.
		 */
		/*XFlush(dpy);*/
...
TimerHandler(evutil_socket_t fd, short ev, void *arg)
{
	...
	if (timer->timeout.timeleft <= 0) {
		...
		/* construct command */
		parsed_command =
		    ParseCommand(0, timer->timeout.command, '\0', &dn, &sp);
		...
		if (CF.last_error) { /* if form has last_error field */
			memset(CF.last_error->text.value, ' ',
			    CF.last_error->text.n); /* clear */
			/* To do this more elegantly, the window resize logic
			   should recalculate size_x for the Message as the
			   window resizes.	Right now, just clear a nice
			   wide area. dje */
			XClearArea(dpy, CF.frame, CF.last_error->header.pos_x,
			    CF.last_error->header.pos_y,
			    /* CF.last_error->header.size_x, */
...
main(int argc, char **argv)
{
	...
	if (module == NULL) {
		fprintf(stderr, "FvwmForm Version " VERSION
	...
	fvwmSetSignalMask(
	    sigmask(SIGPIPE) | sigmask(SIGTERM) | sigmask(SIGINT));
#endif
	...
	if (strcasecmp(module->name, "FvwmForm")
	    != 0) { /* if not already read */
		sprintf(cmd, "read %s Quiet",
		    module->name); /* read quiet modules config */
	...
	SetMessageMask(
	    Channel, M_SENDCONFIG | M_CONFIG_INFO | M_ERROR | M_STRING);
