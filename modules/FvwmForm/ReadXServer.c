...
ReadXServer(void)
{
		...
		if (event.xany.window == CF.frame) {
			...
			case ConfigureNotify: /* has window be reconfigured */
			{
				...
				fev_sanitise_configure_notify(
				    &event.xconfigure);
				while (FCheckTypedWindowEvent(dpy, CF.frame,
				    ConfigureNotify, &tmpe)) {
					fev_sanitise_configure_notify(
					    &tmpe.xconfigure);
	...
	break;
			...
			case KeyPress: /* we do text input here */
				n = XLookupString(&event.xkey, (char *)buf,
				    sizeof(buf), &ks, NULL);
				keypress = buf[0];
				...
				switch (
				    ks) { /* regular key, may need adjustment */
				case XK_Tab:
#ifdef XK_XKB_KEYS
				case XK_ISO_Left_Tab:
#endif
					if (event.xkey.state &
					    ShiftMask) { /* shifted key */
						buf[0] = '\020'; /* chg shift
								    tab to ^P */
					}
					break;
				...
				case '\005': /* ^E */
					...
					if ((CF.cur_input->input.left =
						    CF.rel_cursor -
						    CF.cur_input->input.size) <
					    0)
						CF.cur_input->input.left = 0;
					CF.abs_cursor = CF.rel_cursor -
				...
				case '\020': /* ^P previous field */
					...
					CF.cur_input =
					    old_item->input
						.prev_input; /* new current
								input fld */
					RedrawItem(old_item, 1, NULL);
					CF.rel_cursor = CF.abs_cursor =
			...
			redraw_newcursor: {
				XSetForeground(dpy,
				    ...
				    CF.cur_input->header.dt_ptr
					->dt_colors[c_item_bg]);
				/* Since DrawString is being used, I changed
				   this to clear the entire input field.  dje
				   10/24/99. */
			...
			redraw: {
				...
				CF.cur_input->header.dt_ptr->dt_Fstr->win =
				    CF.cur_input->header.win;
				CF.cur_input->header.dt_ptr->dt_Fstr->gc =
				    CF.cur_input->header.dt_ptr->dt_item_GC;
				CF.cur_input->header.dt_ptr->dt_Fstr->flags
				    .has_colorset = False;
				if (itemcolorset >= 0) {
					...
					CF.cur_input->header.dt_ptr->dt_Fstr
					    ->flags.has_colorset = True;
				}
				CF.cur_input->header.dt_ptr->dt_Fstr->str =
				    CF.cur_input->input.value;
				CF.cur_input->header.dt_ptr->dt_Fstr->x =
				    BOX_SPC + TEXT_SPC;
				CF.cur_input->header.dt_ptr->dt_Fstr->y =
				    BOX_SPC + TEXT_SPC +
				...
				XDrawLine(dpy, CF.cur_input->header.win,
				    CF.cur_input->header.dt_ptr->dt_item_GC, x,
				    BOX_SPC, x, dy - BOX_SPC);
				myfprintf(
				    (stderr, "Line %d/%d - %d/%d (char)\n", x,
					BOX_SPC, x, dy - BOX_SPC));
			}
			no_redraw:
		...
		for (item = root_item_ptr; item != 0;
			...
			if (event.xany.window == item->header.win) {
				...
				case Expose: {
					int ex	= event.xexpose.x;
					int ey	= event.xexpose.y;
					int ex2 = event.xexpose.x +
					    event.xexpose.width;
					int ey2 = event.xexpose.y +
					    event.xexpose.height;
					while (FCheckTypedWindowEvent(dpy,
					    item->header.win, Expose, &event)) {
						ex  = min(ex, event.xexpose.x);
						ey  = min(ey, event.xexpose.y);
						ex2 = max(ex2,
						    event.xexpose.x +
				...
				case ButtonPress:
					...
					if (item->type == I_BUTTON) {
						...
						if (CF.activate_on_press) {
							usleep(
							    MICRO_S_FOR_10MS); /* make sure its visible */
							RedrawItem(item, 0,
							    NULL); /* pop button
								      out */
							DoCommand(
...
process_history(int direction)
{
	...
	CF.cur_input->input.value = fxstrdup(
	    CF.cur_input->input.value_history_ptr[count]);
...
static int
process_tabtypes(unsigned char *buf)
{
	Item *item, *old_item;
	...
	for (; item != 0; item = item->header.next) { /* find next input item */
		if (item->type == I_INPUT) {
			old_item     = CF.cur_input;
			CF.cur_input = item;
			RedrawItem(old_item, 1, NULL);
...
static void
process_regular_char_input(unsigned char *buf)
{
	char *sp, *dp, *ep;
	...
	if (++(CF.cur_input->input.n) >= CF.cur_input->input.buf) {
		...
		CF.cur_input->input.value = fxrealloc(CF.cur_input->input.value,
		    CF.cur_input->input.buf, sizeof(CF.cur_input->input.value));
	}
...
/* Process a paste.  This can be any size.
   ...
   */
static void
process_paste_request(XEvent *event, Item *item)
{
	Atom	       actual_type;
	int	       actual_format;
	...
	do {
		if (XGetWindowProperty(dpy, DefaultRootWindow(dpy),
			XA_CUT_BUFFER0, nread / 4, 1024L, /* offset, length */
			False,				  /* delete */
		...
		if (actual_type !=
		    XA_STRING) { /* if something other than text */
			return;	 /* give up */
		}
		for (c = data; c != data + nitems; c++) { /* each char */
			...
			case '\t': /* TAB */
				if (CF.cur_input ==
				    CF.cur_input->input
					.next_input) { /* 1 ip field */
					process_regular_char_input(
					    (unsigned char
						    *)" "); /* paste space */
				} else {
					process_tabtypes(
					    c); /* jump to the next field */
				}
			case '\015': /* LINEFEED */
			case '\016': /* ^N */
				process_tabtypes(
		...
		if (!item->choice.on) {
			for (i = 0; i < sel->selection.n; i++) {
				if (sel->selection.choices[i]->choice.on) {
					sel->selection.choices[i]->choice.on =
					    0;
