...
static struct fvwm_msg	 *handle_packet(unsigned long, unsigned long *,
      unsigned long);
...
handle_packet(unsigned long type, unsigned long *body, unsigned long len)
{
	...
	case M_CONFIGURE_WINDOW: {
		...
		cJSON_AddStringToObject(content, "window", xwid);
		cJSON_AddNumberToObject(content, "title_height",
		    cwp->title_height);
		cJSON_AddNumberToObject(content, "border_width",
		    cwp->border_width);
		...
		if ((hints = cJSON_CreateObject()) == NULL)
			goto out;
		cJSON_AddNumberToObject(hints, "base_width",
		    cwp->hints_base_width);
		cJSON_AddNumberToObject(hints, "base_height",
		    cwp->hints_base_height);
		cJSON_AddNumberToObject(hints, "inc_width",
		    cwp->hints_width_inc);
		cJSON_AddNumberToObject(hints, "inc_height",
		    cwp->hints_height_inc);
		cJSON_AddNumberToObject(hints, "orig_inc_width",
		    cwp->orig_hints_width_inc);
		cJSON_AddNumberToObject(hints, "orig_inc_height",
		    cwp->orig_hints_height_inc);
		cJSON_AddNumberToObject(hints, "min_width",
		    cwp->hints_min_width);
		cJSON_AddNumberToObject(hints, "min_height",
		    cwp->hints_min_height);
		cJSON_AddNumberToObject(hints, "max_width",
		    cwp->hints_max_width);
		cJSON_AddNumberToObject(hints, "max_height",
		    cwp->hints_max_height);
		...
		cJSON_AddNumberToObject(ewmh, "desktop",
		    cwp->ewmh_hint_desktop);
		cJSON_AddNumberToObject(ewmh, "window_type",
		    cwp->ewmh_window_type);
...
main(int argc, char **argv)
{
	...
	fc.read_ev = event_new(base, fc.fd[1], EV_READ | EV_PERSIST, fvwm_read,
	    NULL);
