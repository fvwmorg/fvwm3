...
main(int argc, char **argv)
{
	...
	if (module == NULL) {
		fprintf(stderr, "FvwmAuto Version " VERSION
	...
	if (n < module->user_argc && module->user_argv[n]) {
		...
		/* -passid option */
		if (n < module->user_argc && *module->user_argv[n]
		    && StrEquals(module->user_argv[n], "-passid")) {
			do_pass_id = True;
			n++;
		}
		if (n < module->user_argc && *module->user_argv[n]
		    && StrEquals(module->user_argv[n], "-menterleave")) {
			/* enterleave mode */
			use_leave_mode = True;
			use_enter_mode = True;
			n++;
		} else if (n < module->user_argc && *module->user_argv[n]
			   && StrEquals(module->user_argv[n], "-menter")) {
			/* enter mode */
			use_leave_mode = False;
			use_enter_mode = True;
			n++;
		} else if (n < module->user_argc && *module->user_argv[n]
			   && StrEquals(module->user_argv[n], "-mfocus")) {
			/* focus mode */
			use_leave_mode = False;
	...
	if (matchWildcards("*Raise*", rl) || matchWildcards("*Lower*", rl)) {
	...
	*/
	...
	while (!isTerminated) {
		...
		if (FD_ISSET(fd[1], &in_fdset)) {
			...
			myfprintf(
			    (stderr, "pw = 0x%x, fw=0x%x, rw = 0x%x, lw=0x%x\n",
			...
			case M_FOCUS_CHANGE:
				...
				if (focus_win != raised_win) {
					myfprintf(
					    (stderr, "focus on new window\n"));
					have_new_window	 = 1;
					raise_window_now = 0;
				} else {
					myfprintf(
					    (stderr, "focus on old window\n"));
				}
				break;
			...
			case M_RAISE_WINDOW:
				...
				if (have_new_window
				    && focus_win == raised_win) {
					myfprintf((stderr, "its the old window:"
							   " don't raise\n"));
					have_new_window = 0;
				}
				break;
