...
list_configure(unsigned long *body)
{
	...
	if ((module->window == cfgpacket->frame)
		...
		if (FScreenIsEnabled()) {
			free(target.monitor);
			target.monitor =
			    fxstrdup(monitor_by_output((int)target.monitor_id)
					 ->si->name);
		}
...
list_end(void)
{
	...
	if ((Ffont = FlocaleLoadFont(dpy, font_string, module->name)) == NULL) {
		fvwm_debug(
		    __func__, "%s: cannot load font, exiting\n", module->name);
		exit(1);
	}
...
change_window_name(char *str)
{
	...
	if (XStringListToTextProperty(&str, 1, &name) == 0) {
		fvwm_debug(
		    __func__, "%s: cannot allocate window name", module->name);
		return;
	}
...
MakeList(void)
{
	...
	AddToList(
	    "StickyPageIcon:", (IS_ICON_STICKY_ACROSS_PAGES(targ) ? yes : no));
	AddToList(
	    "StickyDeskIcon:", (IS_ICON_STICKY_ACROSS_DESKS(targ) ? yes : no));
	...
	if (HAS_EWMH_INIT_WM_DESKTOP(targ) == EWMH_STATE_HAS_HINT
		...
		char desk_buf[32];
		snprintf(
		    desk_buf, 32, "StartOnDesk %lu ", target.ewmh_hint_desktop);
