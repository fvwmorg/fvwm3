...
static EventMask mw_events = ExposureMask | ButtonPressMask | KeyPressMask |
...
main(int argc, char **argv)
{
	...
	if (!(dpy = XOpenDisplay(display_name))) {
		fvwm_debug(__func__, "%s: can't open display %s", module->name,
	...
	while (tline != (char *)0) {
		...
		if (strncasecmp(tline, mname, module->namelen + 1) == 0) {
			...
			} else if (strncasecmp(tline, "MinimalLayer", 12) ==
			    0) {
				char *layer_str = PeekToken(&tline[12], NULL);
				if (layer_str == NULL) {
					minimal_layer = default_layer;
				} else if (sscanf(layer_str, "%d",
					       &minimal_layer) != 1) {
					if (strncasecmp(layer_str, "none", 4) ==
					    0) {
						minimal_layer = -1;
					} else {
						minimal_layer = default_layer;
	...
	if (module->window == 0) {
		fvwmlib_get_target_window(dpy, screen, module->name,
		    &(module->window), True);
	}
...
list_configure(unsigned long *body)
{
	...
	if ((module->window == cfgpacket->frame) ||
		...
		target.monitor	  = fxstrdup("unknown");
		{
			...
			target.monitor = fxstrdup(
			    monitor_by_output((int)target.monitor_id)
				->si->name);
		}
		...
		memcpy(&target.flags, &(cfgpacket->flags),
...
list_property_change(unsigned long *body)
{
	if (body[0] == MX_PROPERTY_CHANGE_BACKGROUND && body[2] == 0 &&
		...
		if (UsePixmapDrawing) {
			...
			UpdateBackgroundTransparency(dpy, main_win, main_width,
			    main_height, &Colorset[(colorset)], Pdepth, gc,
			    True);
		}
	}
}
...
list_config_info(unsigned long *body)
{
	...
	if (StrEquals(token, "Colorset") && colorset >= 0 &&
		...
		if (UsePixmapDrawing) {
			...
			SetWindowBackground(dpy, main_win, main_width,
			    main_height, &Colorset[colorset], Pdepth, gc, True);
		}
	}
	free(token);
}
...
ProcessXEvent(int x, int y)
{
	...
	while (FPending(dpy)) {
		...
		case Expose:
			...
			if (FftSupport) {
				XClearArea(dpy, main_win, ex, ey, ex2 - ex,
				    ey2 - ey, False);
			}
		...
		case ButtonRelease:
			if (is_button_pressed) {
				if (is_button_pressed == 2 &&
					 ...
					 * button 2 is pressed */
					SetMessageMask(fd,
					    M_CONFIGURE_WINDOW | M_WINDOW_NAME |
						M_ICON_NAME | M_RES_CLASS |
						M_RES_NAME | M_END_WINDOWLIST |
						M_CONFIG_INFO |
						M_END_CONFIG_INFO |
						M_SENDCONFIG);
					SendText(fd, "Send_WindowList", 0);
					...
					DestroyList();
					fvwmlib_get_target_window(dpy, screen,
					    module->name, &(module->window),
					    True);
					found = 0;
					return 1;
				} else {
		...
		case ReparentNotify:
			if (minimal_layer >= 0) {
				snprintf(buf, sizeof(buf), "Layer 0 %d",
				    my_layer);
		...
		case ConfigureNotify:
			...
			if ((Event.xconfigure.send_event ||
				...
				if (UsePixmapDrawing) {
					PixmapDrawWindow(main_width,
					    main_height);
					do_eat_expose = True;
				} else if (colorset == -1) {
					do_eat_expose = True;
				} else if (UpdateBackgroundTransparency(dpy,
					       main_win, main_width,
					       main_height,
				...
				if (do_eat_expose == True &&
				    is_initial_cn == False) {
					while (FCheckTypedEvent(dpy, Expose,
					    &Event)) {
						/* nothing */
					}
				}
...
list_end(void)
{
	...
	mysizehints.win_gravity		 = NorthWestGravity;
	{
		...
		if (!FQueryPointer(dpy, Root, &JunkW, &JunkW, &x, &y, &JunkC,
			&JunkC, &JunkM)) {
	...
	if (Pdepth < 2) {
		...
		back_pix = (colorset < 0) ? GetColor(BackColor) :
	...
	main_win   = XCreateWindow(dpy, Root, x, y, mysizehints.width,
	      mysizehints.height, 0, Pdepth, InputOutput, Pvisual,
 ...
 */
void
DrawItems(Drawable d, int x, int y, int w, int h)
{
	int	     fontheight, i = 0;
...
PixmapDrawWindow(int w, int h)
{
	...
	if (colorset >= 0) {
		...
		cs_pix = CreateBackgroundPixmap(dpy, main_win, w, h,
		    &Colorset[(colorset)], Pdepth, gc, False);
		if (cs_pix == ParentRelative) {
			pix = cs_pix;
		} else {
			pix = CreateTiledPixmap(dpy, cs_pix, 0, 0, w, h, Pdepth,
			    gc);
...
AddToList(char *s1, char *s2)
{
	...
	item	   = fxmalloc(sizeof(struct Item));
	item->col1 = s1;
	item->col2 = s2;
	item->next = NULL;
...
MakeList(void)
{
...

	{
		...
		Atom	  _XA_WM_TAKE_FOCUS = XInternAtom(dpy, "WM_TAKE_FOCUS",
			 False);
	...
	if (HAS_EWMH_INIT_WM_DESKTOP(targ) == EWMH_STATE_HAS_HINT &&
		...
		char desk_buf[32];
		snprintf(desk_buf, 32, "StartOnDesk %lu ",
		    target.ewmh_hint_desktop);
