...
FuncGetFore(int *NbArg, long *TabArg)
{
	...
	xasprintf(&tmp, "%02x%02x%02x", color.red >> 8, color.green >> 8,
	    color.blue >> 8);
	return tmp;
}
...
ReceivFromScript(int *NbArg, long *TabArg)
{
	...
	if ((send >= 0) && (send < 99)) {
		if (x11base->TabScriptId[send] != NULL) {
			ASend = XInternAtom(dpy, x11base->TabScriptId[send],
			    True);
			if (ASend == None)
				fvwm_debug(__func__,
				    "[%s][ReceivFromScript]: <<WARNING>> Error with atome\n",
	...
	/* Get message */
	XConvertSelection(dpy, ASend, AReceiv, propriete, x11base->win,
	    CurrentTime);
...
ChangeForeColor(int NbArg, long *TabArg)
{
	...
	if (tabxobj[IdItem]->colorset >= 0) {
		tabxobj[IdItem]->TabColor[back] = GetColor(
		    tabxobj[IdItem]->backcolor);
		tabxobj[IdItem]->TabColor[hili] = GetColor(
		    tabxobj[IdItem]->hilicolor);
		tabxobj[IdItem]->TabColor[shad] = GetColor(
		    tabxobj[IdItem]->shadcolor);
	}
...
ChangeBackColor(int NbArg, long *TabArg)
{
	...
	if (tabxobj[IdItem]->colorset >= 0) {
		tabxobj[IdItem]->TabColor[fore] = GetColor(
		    tabxobj[IdItem]->forecolor);
		tabxobj[IdItem]->TabColor[hili] = GetColor(
		    tabxobj[IdItem]->hilicolor);
		tabxobj[IdItem]->TabColor[shad] = GetColor(
		    tabxobj[IdItem]->shadcolor);
	...
	if (tabxobj[IdItem]->TypeWidget != SwallowExec) {
		XSetWindowBackground(dpy, tabxobj[IdItem]->win,
...
ChangeColorset(int NbArg, long *TabArg)
{
	...
	/* Free color */
	if (tabxobj[IdItem]->colorset < 0) {
		PictureFreeColors(dpy, Pcmap, &tabxobj[IdItem]->TabColor[fore],
		    1, 0, True);
		PictureFreeColors(dpy, Pcmap, &tabxobj[IdItem]->TabColor[back],
		    1, 0, True);
		PictureFreeColors(dpy, Pcmap, &tabxobj[IdItem]->TabColor[hili],
		    1, 0, True);
		PictureFreeColors(dpy, Pcmap, &tabxobj[IdItem]->TabColor[shad],
		    1, 0, True);
	}
	sscanf(arg[1], "%d", &i);
...
ChangeWindowTitleFromArg(int NbArg, long *TabArg)
{
	...
	if (x11base->TabArg[argVal] != NULL) {
		arg = fxcalloc(strlen(x11base->TabArg[argVal]) + 1,
		    sizeof(char));
...
WriteToFile(int NbArg, long *TabArg)
{
	...
	if (CurrPos == size) {
		...
		if (OldPID == getpid()) {
			...
			while (((strncmp(StrEnd, &buf[CurrPos],
				    strlen(StrEnd))) != 0) &&
			    (CurrPos < size)) {
				CurrPos++;
			}
		...
		} else /* Replacing old commands */
		{
			...
			CurrPos2 = CurrPos;
			while (((strncmp(StrEnd, &buf[CurrPos2],
				    strlen(StrEnd))) != 0) &&
			    (CurrPos2 < size)) {
				CurrPos2++;
			}
...
Key(int NbArg, long *TabArg)
{
	...
	for (i = j + 1; i < NbArg; i++) {
		...
		str = fxrealloc((void *)str, strlen(str) + strlen(tmp) + 1,
		    sizeof(str));
		str = strcat(str, tmp);
		free(tmp);
	}
