...
/* GetMinValue */
static char *
FuncGetMinValue(int *NbArg, long *TabArg)
{
	char *tmp;
	long  Id;
...
/* GetMaxValue */
static char *
FuncGetMaxValue(int *NbArg, long *TabArg)
{
	char *tmp;
	long  Id;
...
/* GetFore */
static char *
FuncGetFore(int *NbArg, long *TabArg)
{
	char * tmp;
	long   Id;
	XColor color;
	...
	sprintf(tmp, "%02x%02x%02x", color.red >> 8, color.green >> 8,
	    color.blue >> 8);
	return tmp;
}
...
/* GetBack */
static char *
FuncGetBack(int *NbArg, long *TabArg)
{
	char * tmp;
	long   Id;
	XColor color;
...
/* GetHili */
static char *
FuncGetHili(int *NbArg, long *TabArg)
{
	char * tmp;
	long   Id;
	XColor color;
...
/* GetShad */
static char *
FuncGetShad(int *NbArg, long *TabArg)
{
	char * tmp;
	long   Id;
	XColor color;
...
FuncGetOutput(int *NbArg, long *TabArg)
{
	...
	if ((strcmp(Command, cmndbuf)) || ((time(NULL) - TimeCom) > 1)
	    || (TimeCom == 0)) {
...
LaunchScript(int *NbArg, long *TabArg)
{
	...
	if (x11base->NbChild < 98) {
		i = 16;
		do {
			sprintf(x11base->TabScriptId[x11base->NbChild + 2],
			    "%s%x", x11base->TabScriptId[1], i);
			MyAtom = XInternAtom(dpy,
			    x11base->TabScriptId[x11base->NbChild + 2], False);
			i++;
	...
	execstr = fxcalloc(
	    strlen(module->name) + strlen(arg)
		+ strlen(x11base->TabScriptId[x11base->NbChild + 2]) + 5,
	    sizeof(char));
...
ReceivFromScript(int *NbArg, long *TabArg)
{
	...
	if ((send >= 0) && (send < 99)) {
		if (x11base->TabScriptId[send] != NULL) {
			ASend =
			    XInternAtom(dpy, x11base->TabScriptId[send], True);
			if (ASend == None)
				fvwm_debug(__func__,
				    "[%s][ReceivFromScript]: <<WARNING>> Error "
				    "with atome\n",
				    ScriptName);
		} else
			return msg;
	...
	/* Get message */
	XConvertSelection(
	    dpy, ASend, AReceiv, propriete, x11base->win, CurrentTime);
	...
	if (event.xselection.property != None)
		if (event.xselection.selection == ASend) {
			XGetWindowProperty(dpy, event.xselection.requestor,
			    event.xselection.property, 0L, 8192L, False,
			    event.xselection.target, &type, &format, &longueur,
			    &octets_restant, &donnees);
			if (longueur > 0) {
				msg = fxrealloc((void *)msg,
				    (longueur + 1) * sizeof(char),
				    sizeof((void *)msg));
...
/* GetPid */
static char *
FuncGetPid(int *NbArg, long *TabArg)
{
	char *str;
	pid_t pid;
...
static char *
FuncSendMsgAndGet(int *NbArg, long *TabArg)
{
	...
	/* unlock the receiver, wait IN_FIFO_TIMEOUT * IN_FIFO_NBR_OF_TRY so
	 * that * the receiver has the time to go in its communication loop at
	 * startup   * or if it do an other job than waiting for a cmd */
	i = 0;
	while (1) {
		if (access(in_fifo, W_OK) == 0) {
			if ((filedes = open(in_fifo, O_WRONLY)) == 0) {
				fvwm_debug(__func__,
				    "[%s][GetMsgAndGet]: <<WARNING>> cannot "
				    "open the in fifo %s\n",
				    ScriptName, in_fifo);
				err = 1;
				break;
			...
			l = strlen(cmd);
			while (l > i) {
				if ((j = write(filedes, cmd + i, l - i))
				    == -1) {
					fvwm_debug(__func__,
					    "[%s][GetMsgAndGet]: <<WARNING>> "
					    "write error on %s\n",
					    ScriptName, in_fifo);
					err = 1;
				}
			...
			if (i > IN_FIFO_NBR_OF_TRY) {
				fvwm_debug(__func__,
				    "[%s][GetMsgAndGet]: <<WARNING>> No in "
				    "fifo %s for communication %s\n",
...
/* Parse */
static char *
FuncParse(int *NbArg, long *TabArg)
{
	char *string, *str;
	char  num[4];
	int   index, toklen, l;
...
/* GetLastString */
static char *
FuncGetLastString(int *NbArg, long *TabArg)
{
	char *str;
...
ChangeValue(int NbArg, long *TabArg)
{
	...
	tabxobj[TabIdObj[atoi(arg[0])]]->DrawObj(
	    tabxobj[TabIdObj[atoi(arg[0])]], NULL);
	free(arg[0]);
	free(arg[1]);
}
...
ChangeFont(int NbArg, long *TabArg)
{
	...
	if ((Ffont = FlocaleLoadFont(dpy, tabxobj[IdItem]->font, ScriptName))
	    == NULL) {
		fvwm_debug(__func__,
		...
		if (Ffont->font != NULL)
			XSetFont(dpy, tabxobj[IdItem]->gc,
			    tabxobj[IdItem]->Ffont->font->fid);
	}
...
/* ChangeLocaleTitle */
static void
ChangeLocaleTitle(int NbArg, long *TabArg)
{
	int   i = 0;
	char *arg[2];
	int   IdItem;
...
ChangeForeColor(int NbArg, long *TabArg)
{
	...
	if (tabxobj[IdItem]->colorset < 0)
		PictureFreeColors(dpy, Pcmap,
		    (void *)(&(tabxobj[IdItem])->TabColor[fore]), 1, 0, True);
	...
	if (tabxobj[IdItem]->colorset >= 0) {
		tabxobj[IdItem]->TabColor[back] =
		    GetColor(tabxobj[IdItem]->backcolor);
		tabxobj[IdItem]->TabColor[hili] =
		    GetColor(tabxobj[IdItem]->hilicolor);
		tabxobj[IdItem]->TabColor[shad] =
		    GetColor(tabxobj[IdItem]->shadcolor);
	}
...
ChangeBackColor(int NbArg, long *TabArg)
{
	...
	if (tabxobj[IdItem]->colorset < 0)
		PictureFreeColors(dpy, Pcmap,
		    (void *)(&(tabxobj[IdItem])->TabColor[back]), 1, 0, True);
	...
	if (tabxobj[IdItem]->colorset >= 0) {
		tabxobj[IdItem]->TabColor[fore] =
		    GetColor(tabxobj[IdItem]->forecolor);
		tabxobj[IdItem]->TabColor[hili] =
		    GetColor(tabxobj[IdItem]->hilicolor);
		tabxobj[IdItem]->TabColor[shad] =
		    GetColor(tabxobj[IdItem]->shadcolor);
	...
	if (tabxobj[IdItem]->TypeWidget != SwallowExec) {
		XSetWindowBackground(
		    dpy, tabxobj[IdItem]->win, tabxobj[IdItem]->TabColor[back]);
		XClearWindow(dpy, tabxobj[IdItem]->win);
	}
	tabxobj[IdItem]->DrawObj(tabxobj[IdItem], NULL);
...
/* ChangeMainColorset */
static void
ChangeMainColorset(int i)
{
	/* Free color */
	if (x11base->colorset < 0) {
		PictureFreeColors(
		    dpy, Pcmap, &x11base->TabColor[fore], 1, 0, True);
		PictureFreeColors(
		    dpy, Pcmap, &x11base->TabColor[back], 1, 0, True);
		PictureFreeColors(
		    dpy, Pcmap, &x11base->TabColor[hili], 1, 0, True);
		PictureFreeColors(
		    dpy, Pcmap, &x11base->TabColor[shad], 1, 0, True);
	}
	x11base->colorset = i;
	AllocColorset(i);
	...
	SetWindowBackground(dpy, x11base->win, x11base->size.width,
	    x11base->size.height, &Colorset[i], Pdepth, x11base->gc, True);
}

/* ChangeMainColorset */
static void
ChangeColorset(int NbArg, long *TabArg)
{
	int   i = 0;
	char *arg[2];
	int   IdItem;
	...
	if (tabxobj[IdItem]->colorset < 0) {
		PictureFreeColors(
		    dpy, Pcmap, &tabxobj[IdItem]->TabColor[fore], 1, 0, True);
		PictureFreeColors(
		    dpy, Pcmap, &tabxobj[IdItem]->TabColor[back], 1, 0, True);
		PictureFreeColors(
		    dpy, Pcmap, &tabxobj[IdItem]->TabColor[hili], 1, 0, True);
		PictureFreeColors(
		    dpy, Pcmap, &tabxobj[IdItem]->TabColor[shad], 1, 0, True);
	}
	sscanf(arg[1], "%d", &i);
	...
	if (tabxobj[IdItem]->TypeWidget != SwallowExec) {
		SetWindowBackground(dpy, tabxobj[IdItem]->win,
		    tabxobj[IdItem]->width, tabxobj[IdItem]->height,
		    &Colorset[i], Pdepth, tabxobj[IdItem]->gc, False);
		XClearWindow(dpy, tabxobj[IdItem]->win);
	}
	tabxobj[IdItem]->DrawObj(tabxobj[IdItem], NULL);
...
/* ChangeWindowTitle */
static void
ChangeWindowTitle(int NbArg, long *TabArg)
{
	...
	XChangeProperty(dpy, x11base->win, XA_WM_NAME, XA_STRING, 8,
	    PropModeReplace, (unsigned char *)arg, strlen(arg));
	free(arg);
}

/* ChangeWindowTitleFromArg */
static void
ChangeWindowTitleFromArg(int NbArg, long *TabArg)
{
	...
	if (x11base->TabArg[argVal] != NULL) {
		arg =
		    fxcalloc(strlen(x11base->TabArg[argVal]) + 1, sizeof(char));
...
/* MyWarpPointer */
static void
MyWarpPointer(int NbArg, long *TabArg)
{
	int   i = 0;
	char *arg;
	int   IdItem;
	...
	for (j = 0; j < NbArg - 2; j++) {
		if (TabArg[j] > 100000) /* Number coding case */
		{
			...
			CurrArg++;
			   ...
			   < -200000) /* Comparison function ID case */
		{
...
Loop(int NbArg, long *TabArg)
{
	...
	TabVVar[TabArg[0]] = fxrealloc(
	    TabVVar[TabArg[0]], sizeof(char) * 10, sizeof(TabVVar[TabArg[0]]));
...
WriteToFile(int NbArg, long *TabArg)
{
	...
	if (CurrPos == size) {
		...
		if (OldPID == getpid()) {
			sprintf(str, "%d\n", OldPID);
			while (((strncmp(StrEnd, &buf[CurrPos], strlen(StrEnd)))
				   != 0)
			       && (CurrPos < size)) {
				CurrPos++;
			}
			memmove(&buf[CurrPos + strlen(arg[1])], &buf[CurrPos],
			    strlen(buf) - CurrPos);
		...
		} else /* Replacing old commands */
		{
			...
			CurrPos2 = CurrPos;
			while (
			    ((strncmp(StrEnd, &buf[CurrPos2], strlen(StrEnd)))
				!= 0)
			    && (CurrPos2 < size)) {
				CurrPos2++;
			}
			sprintf(str, "%d\n%s", getpid(), arg[1]);
			memmove(&buf[CurrPos + strlen(str)], &buf[CurrPos2],
			    strlen(buf) - CurrPos2);
			buf[strlen(buf)
			    - ((CurrPos2 - CurrPos) - strlen(str))] = '\0';
...
SendToScript(int NbArg, long *TabArg)
{
	...
	if ((BuffSend.NbMsg < 40)
	    && (XGetSelectionOwner(dpy, myatom) != None)) {
...
Key(int NbArg, long *TabArg)
{
	...
	for (i = j + 1; i < NbArg; i++) {
		...
		str = fxrealloc(
		    (void *)str, strlen(str) + strlen(tmp) + 1, sizeof(str));
		str = strcat(str, tmp);
		free(tmp);
	}
	...
	if (keysym == 0) {
		fvwm_debug(__func__, "[%s][Key]: No such key: %s", ScriptName,
		    key_string);
		error = 1;
	}
	...
	AddBinding(dpy, &BindingsList, BIND_KEYPRESS, 0, keysym, key_string,
	    modifier, C_WINDOW, (void *)action, NULL, NULL);
	free(key_string);
	free(in_modifier);
}
...
static char *
FuncGettext(int *NbArg, long *TabArg)
{
	char *str;
	char *string;
