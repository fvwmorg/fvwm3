	...
	while (!isTerminated && (BuffSend.NbMsg > 0) && (NbEssai < 10000)) {
		...
		if (fvwmSelect(x_fd + 1, &in_fdset, NULL, NULL, &tv) > 0) {
			if (FD_ISSET(x_fd, &in_fdset)) {
				if (FCheckTypedEvent(dpy, SelectionRequest,
					&event))
...
ReadConfig(char *ScriptName)
{
	...
	snprintf(s, sizeof(s), "%s%s%s", ScriptPath, (!*ScriptPath ? "" : "/"),
	    ScriptName);
	yyin = fopen(s, "r");
...
ParseOptions(void)
{
	...
	while (tline != (char *)0) {
		if (strlen(tline) > 1) {
			...
			} else if (strncasecmp(tline, "*FvwmScriptDefaultBack",
				       22) == 0) {
				x11base->colorset = -1;
				CopyString(&x11base->backcolor, &tline[22]);
			} else if (strncasecmp(tline, "*FvwmScriptDefaultFore",
				       22) == 0) {
				x11base->colorset = -1;
			...
			} else if (strncasecmp(tline, "*FvwmScriptDefaultFont",
				       22) == 0)
				CopyStringWithQuotes(&x11base->font,
				    &tline[22]);
			else if (strncasecmp(tline, "Colorset", 8) == 0)
				LoadColorset(&tline[8]);
			else if (strncasecmp(tline, "*FvwmScriptDefaultCursor",
				     24) == 0) {
...
LoadIcon(struct XObj *xobj)
{
	...
	if (depth <= 1) {
		...
		temp	       = XCreatePixmap(dpy, x11base->win, xobj->icon_w,
			      xobj->icon_h, Pdepth);
...
SendMsgToScript(XEvent event)
{
	...
	if (event.xselectionrequest.selection == Sender) {
		...
		FSendEvent(dpy, evnt_sel.xselection.requestor, False, 0,
		    &evnt_sel);
	}
}
...
ReadXServer(void)
{
	...
	while (FEventsQueued(dpy, QueuedAfterReading)) {
		...
		case Expose:
			...
			while (FCheckTypedWindowEvent(dpy, event.xany.window,
			    Expose, &event)) {
		...
		case KeyPress:
			...
			isTab = 0;
			find  = False;
			XLookupString(&event.xkey, (char *)buf, sizeof(buf),
			    &ks, NULL);
			event.xkey.keycode = XKeysymToKeycode(dpy,
			    fvwm_KeycodeToKeysym(dpy, event.xkey.keycode, 0,
				0));
...
main(int argc, char **argv)
{
	...
	SetMessageMask(fd,
	    M_NEW_DESK | M_END_WINDOWLIST | M_STRING | M_MAP | M_RES_NAME |
		M_RES_CLASS | M_CONFIG_INFO | M_END_CONFIG_INFO |
		M_WINDOW_NAME | M_SENDCONFIG);
