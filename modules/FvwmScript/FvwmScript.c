...
ReadConfig(char *ScriptName)
{
	...
	/* This is legacy support.  The first try is the command line file name
	   with the scriptfile directory.  If the scriptfile directory wasn't
	   set it ends up trying the command line file name undecorated, which
	   could be a full path, or it could be relative to the current
	   directory. Not very pretty, dje 12/26/99 */
		...
		if (strlen(tline) > 1) {
			if (strncasecmp(tline, "ImagePath", 9) == 0)
				...
				CopyString(&ScriptPath, &tline[15]);
				     ...
				     tline, "*FvwmScriptDefaultColorset", 26)
				 == 0) {
				LoadColorset(&tline[26]);
				x11base->colorset = atoi(&tline[26]);
			} else if (strncasecmp(
				       tline, "*FvwmScriptDefaultBack", 22)
				   == 0) {
				x11base->colorset = -1;
				CopyString(&x11base->backcolor, &tline[22]);
			} else if (strncasecmp(
				       tline, "*FvwmScriptDefaultFore", 22)
				   == 0) {
				x11base->colorset = -1;
				CopyString(&x11base->forecolor, &tline[22]);
			} else if (strncasecmp(
				       tline, "*FvwmScriptDefaultHilight", 25)
				   == 0) {
				x11base->colorset = -1;
				CopyString(&x11base->hilicolor, &tline[25]);
			} else if (strncasecmp(
				       tline, "*FvwmScriptDefaultShadow", 24)
				   == 0) {
				x11base->colorset = -1;
				CopyString(&x11base->shadcolor, &tline[24]);
			} else if (strncasecmp(
				       tline, "*FvwmScriptDefaultFont", 22)
				   == 0)
				CopyStringWithQuotes(
				    &x11base->font, &tline[22]);
			else if (strncasecmp(tline, "Colorset", 8) == 0)
				LoadColorset(&tline[8]);
			else if (strncasecmp(
				     tline, "*FvwmScriptDefaultCursor", 24)
				 == 0) {
...
LoadIcon(struct XObj *xobj)
{
	...
	if (xobj->alloc_pixels != NULL && xobj->nalloc_pixels > 0) {
		PictureFreeColors(dpy, Pcmap, xobj->alloc_pixels,
		    xobj->nalloc_pixels, 0, False);
		xobj->alloc_pixels  = NULL;
		xobj->nalloc_pixels = 0;
	}
	if ((xobj->icon) == NULL)
		return;
	if ((path = PictureFindImageFile(xobj->icon, imagePath, R_OK))
	    == NULL) {
		return;
	}
	if (!PImageLoadPixmapFromFile(dpy, x11base->win, path,
		&xobj->iconPixmap, &xobj->icon_maskPixmap,
		&xobj->icon_alphaPixmap, &xobj->icon_w, &xobj->icon_h, &depth,
		&xobj->nalloc_pixels, &xobj->alloc_pixels, 0, fpa)) {
		fvwm_debug(__func__,
	...
	if (depth <= 1) {
		...
		m_gc	       = fvwmlib_XCreateGC(
		      dpy, x11base->win, GCForeground | GCBackground, &gcv);
		temp = XCreatePixmap(
		    dpy, x11base->win, xobj->icon_w, xobj->icon_h, Pdepth);
		XCopyPlane(dpy, xobj->iconPixmap, temp, m_gc, 0, 0,
		    xobj->icon_w, xobj->icon_h, 0, 0, 1);
		XFreePixmap(dpy, xobj->iconPixmap);
		XFreeGC(dpy, m_gc);
...
OpenWindow(void)
{
	...
	x11base->win = XCreateWindow(dpy, x11base->root, x11base->size.x,
	    x11base->size.y, x11base->size.width, x11base->size.height, 0,
	...
	if (x11base->colorset >= 0)
		SetWindowBackground(dpy, x11base->win, x11base->size.width,
		    x11base->size.height, &Colorset[x11base->colorset], Pdepth,
		    x11base->gc, True);

	/* Select which events are received by the window */
	XSelectInput(dpy, x11base->win,
	...
	if (x11base->size.x != -1) {
		...
		IndicNorm->flags =
		    PSize | PMinSize | PMaxSize | PBaseSize | PPosition;
	} else
	...
	XSetWMProperties(dpy, x11base->win, &Name, &Name, NULL, 0, IndicNorm,
	    IndicWM, &classHints);
...
ExecBloc(Bloc *bloc)
{
	...
	for (i = 0; i <= bloc->NbInstr; i++) {
		TabCom[bloc->TabInstr[i].Type](
		    bloc->TabInstr[i].NbArg, bloc->TabInstr[i].TabArg);
	}
}
...
BuildGUI(int IsFather)
{
	...
	for (i = 0; i < nbobj; i++) {
		...
		else if ((*tabobj)[i].backcolor == NULL
			 && (*tabobj)[i].forecolor == NULL
			 && (*tabobj)[i].shadcolor == NULL
			 && (*tabobj)[i].hilicolor == NULL)
			tabxobj[i]->colorset = x11base->colorset;
		else
			tabxobj[i]->colorset = -1;
 ...
 */
void
SendMsgAndString(char *action, char *type)
{
	int   val[2];
	char *token = NULL;
	char *arg1  = NULL;
	...
	if (StrEquals(type, "SendString") || StrEquals(type, "CheckBinding")) {
		...
		if (StrEquals(type, "ChangeWindowTitle")) {
			...
			if (arg2 == NULL) {
				...
				XChangeProperty(dpy, x11base->win, XA_WM_NAME,
				    XA_STRING, 8, PropModeReplace,
				    (unsigned char *)arg1, strlen(arg1));

			} else {
				if (XFetchName(dpy, x11base->win, &windowName)
				    == 0) {
					fvwm_debug(__func__,
					    "[%s][SendMsgAndString]: <<ERROR>> "
					    "Can't find the title of a "
					    "window\n",
					    module->name);
				} else {
					if (StrEquals(arg2, windowName)) {
						XChangeProperty(dpy,
						    x11base->win, XA_WM_NAME,
						    XA_STRING, 8,
						    PropModeReplace,
						    (unsigned char *)arg1,
						    strlen(arg1));
...
SendMsgToScript(XEvent event)
{
	...
	if (event.xselectionrequest.selection == Sender) {
		i = 0;
		while ((i < BuffSend.NbMsg)
		       && (event.xselectionrequest.target != Receiver)) {
		...
		if (event.xselectionrequest.target == Receiver) {
			...
			XChangeProperty(dpy, evnt_sel.xselection.requestor,
			    evnt_sel.xselection.property,
			    evnt_sel.xselection.target, 8, PropModeReplace,
			...
			if (BuffSend.NbMsg > 0) {
				memmove(&BuffSend.TabMsg[i],
				    &BuffSend.TabMsg[i + 1],
				    (BuffSend.NbMsg - i) * sizeof(TypeName));
			}
		} else {
		...
		FSendEvent(
		    dpy, evnt_sel.xselection.requestor, False, 0, &evnt_sel);
	}
}

static void
UpdateRootTransparency(Bool pr_only)
{
	int i;
	...
	if (CSET_IS_TRANSPARENT(x11base->colorset)) {
		if (CSET_IS_TRANSPARENT_PR_PURE(x11base->colorset)) {
			XClearArea(dpy, x11base->win, 0, 0, 0, 0, True);
			   ...
			   || CSET_IS_TRANSPARENT_PR(x11base->colorset)) {
			SetWindowBackground(dpy, x11base->win,
			    x11base->size.width, x11base->size.height,
			    &Colorset[x11base->colorset], Pdepth, x11base->gc,
			    True);
		}
	}
	for (i = 0; i < nbobj; i++) {
		...
		if (CSET_IS_TRANSPARENT_PR_PURE(tabxobj[i]->colorset)) {
			XClearArea(dpy, tabxobj[i]->win, 0, 0, 0, 0, True);
			   ...
			   || CSET_IS_TRANSPARENT_PR(tabxobj[i]->colorset)) {
			SetWindowBackground(dpy, tabxobj[i]->win,
			    tabxobj[i]->width, tabxobj[i]->height,
	...
	while (FEventsQueued(dpy, QueuedAfterReading)) {
		...
		case Expose:
			...
			for (i = 0; i < nbobj; i++) {
				...
				/* handle exposes on x11base that need an object
				 * to render */
				if (event.xexpose.window == x11base->win) {
					/* redraw first menu item to get the 3d
					 * menubar */
					for (i = 0; i < nbobj; i++) {
						if (Menu
						    == tabxobj[i]->TypeWidget) {
							tabxobj[i]->DrawObj(
							    tabxobj[i], &event);
							break;
						}
					}
					/* redraw all Rectangles and
					 * SwallowExec's */
					for (i = 0; i < nbobj; i++) {
						if ((Rectangle
							== tabxobj[i]
		...
		case KeyPress:
			...
			find  = False;
			XLookupString(
			    &event.xkey, (char *)buf, sizeof(buf), &ks, NULL);
			event.xkey.keycode = XKeysymToKeycode(
			    dpy, fvwm_KeycodeToKeysym(
				     dpy, event.xkey.keycode, 0, 0));

			/* check for bindings defined by the Key instruction */
			tmp.res_class = tmp.res_name = "root";
			if ((action = CheckBinding(BindingsList,
				 event.xkey.keycode, event.xkey.state, LockMask,
				 C_WINDOW, BIND_KEYPRESS, &tmp, tmp.res_class))
			    != NULL) {
			...
			if (isTab > 0) {
				...
				while (!find) {
					...
					if (!IS_HIDDEN(tabxobj[i])
					    && !HAS_NO_FOCUS(tabxobj[i])
					    && tabxobj[i]->TypeWidget
						   != Rectangle
					    && tabxobj[i]->TypeWidget
						   != HDipstick
				...
				if (find)
					FWarpPointer(dpy, x11base->win,
					    tabxobj[i]->win, 0, 0, 0, 0,
		...
		case PropertyNotify:
			if (event.xproperty.atom == XA_CUT_BUFFER0)
				...
				break;
			if (i > 0) {
				Scrapt = fxrealloc((void *)Scrapt,
				    sizeof(char) * (i + 1), sizeof(Scrapt));
			...
			if (FD_ISSET(fd[1], &in_fdset)) {
				...
				if (packet->type == M_CONFIG_INFO) {
					...
					if (StrEquals(token, "Colorset")) {
						/* track all colorset changes
						 * and update display if
						 * necessary */
						n = LoadColorset(line);
						for (i = 0; i < nbobj; i++) {
							if (n
					...
					for (i = 0; i < nbobj; i++)
						tabxobj[i]->ProcessMsg(
						    tabxobj[i], packet->type,
						    packet->body);
			}
...
void
ReadFvwmScriptArg(int argc, char **argv, int IsFather)
{
	int  i;
	Atom myatom;
...
main(int argc, char **argv)
{
	...
	SetMessageMask(fd, M_NEW_DESK | M_END_WINDOWLIST | M_STRING | M_MAP
			       | M_RES_NAME | M_RES_CLASS | M_CONFIG_INFO
	...
	signal(SIGINT,
	    TerminateHandler); /* cleanup on other ways of closing too */
