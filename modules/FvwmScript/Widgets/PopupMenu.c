...
InitPopupMenu(struct XObj *xobj)
{
	...
	for (i = 1; i <= xobj->value3; i++) {
		...
		if (xobj->width <
		    FlocaleTextWidth(xobj->Ffont, str, strlen(str)) + 34)
			xobj->width = FlocaleTextWidth(xobj->Ffont, str,
					  strlen(str)) +
			    34;
		free(str);
	}
...
void
DrawPopupMenu(struct XObj *xobj, XEvent *evp)
{
	XSegment segm[4];
	char	*str;
	int	 x, y;
	...
	x   = GetXTextPosition(xobj, xobj->width,
	      FlocaleTextWidth(xobj->Ffont, str, strlen(str)), 25, 8, 8);
...
EvtMousePopupMenu(struct XObj *xobj, XButtonEvent *EvtButton)
{
	...
	/* Create menu window */
	XTranslateCoordinates(dpy, *xobj->ParentWin, Root, xobj->x, yMenu, &x,
	    &y, &Win1);
	if (x < 0)
		x = 0;
	if (y < 0)
	...
	WinPop = XCreateWindow(dpy, Root, x, y, xobj->width, hMenu, 0, Pdepth,
	...
	XGrabPointer(dpy, WinPop, True, GRAB_EVMASK, GrabModeAsync,
	...
	while (End) {
		FQueryPointer(dpy, Root, &Win1, &Win2, &x1, &y1, &x2, &y2,
		    &modif);
		/* Find current option */
		y2 = y2 - y;
		x2 = x2 - x;
		...
		if (newvalue != oldvalue) {
			UnselectMenu(xobj, WinPop, hOpt, oldvalue, xobj->width,
			    asc, 0);
		...
		case KeyPress:
			...
			if (ks == XK_Escape) {
				...
				FWarpPointer(dpy, None, None, 0, 0, 0, 0, 0,
				    -hOpt);
		...
		case ButtonRelease:
			if (start_time != 0 &&
			    event.xbutton.time - start_time < MENU_DRAG_TIME) {
				while_mask = ButtonPress;
				start_time = 0;
			}
