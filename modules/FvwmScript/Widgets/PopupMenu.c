...
EvtMousePopupMenu(struct XObj *xobj, XButtonEvent *EvtButton)
{
	...
	XTranslateCoordinates(
	    dpy, *xobj->ParentWin, Root, xobj->x, yMenu, &x, &y, &Win1);
	if (x < 0)
		x = 0;
	if (y < 0)
		y = 0;
	...
	while (End) {
		FQueryPointer(
		    dpy, Root, &Win1, &Win2, &x1, &y1, &x2, &y2, &modif);
		/* Find current option */
		y2 = y2 - y;
		x2 = x2 - x;
		/* compute xobj->value */
		...
		if (newvalue != oldvalue) {
			UnselectMenu(
			    xobj, WinPop, hOpt, oldvalue, xobj->width, asc, 0);
