...
InitList(struct XObj *xobj)
{
	...
	NbVisCell =
	    (xobj->height - 2 - 3 * BdWidth) / (xobj->Ffont->height + 3);
...
void
DrawCellule(struct XObj *xobj, int NbCell, int NbVisCell, int HeightCell,
    int asc, XEvent *evp)
{
	XRectangle r, inter;
	char *	   Title;
	...
	for (i = xobj->value2; i < xobj->value2 + NbVisCell; i++) {
		...
		do_draw = True;
		if (evp) {
			if (!frect_get_intersection(r.x - 2,
				r.y + (i - xobj->value2) * HeightCell + 2,
				r.width + 4, HeightCell - 2, evp->xexpose.x,
				evp->xexpose.y, evp->xexpose.width,
				evp->xexpose.height, &inter)) {
				do_draw = False;
			}
		} else {
			inter.x	    = r.x - 2; /* r.x */
		...
		if (strlen(Title) != 0) {
			...
			if (xobj->value == i) {
				...
				XFillRectangle(dpy, xobj->win, xobj->gc,
				    inter.x, inter.y, inter.width,
				    inter.height);
				MyDrawString(dpy, xobj, xobj->win, x, y, Title,
				    fore, back, shad, !xobj->flags[1], &r, evp);
			} else {
				XClearArea(dpy, xobj->win, inter.x, inter.y,
				    inter.width, inter.height, False);
				MyDrawString(dpy, xobj, xobj->win, x, y, Title,
				    fore, hili, back, !xobj->flags[1], &r, evp);
			}
		} else {
			XClearArea(dpy, xobj->win, inter.x, inter.y,
			    inter.width, inter.height, False);
...
void
DrawList(struct XObj *xobj, XEvent *evp)
{
	int	   NbVisCell, NbCell;
	int	   HeightCell;
	XRectangle r;
...
EvtMouseList(struct XObj *xobj, XButtonEvent *EvtButton)
{
	...
	if (PtInRect(pt, rect)) {
		...
		do {
			FQueryPointer(dpy, *xobj->ParentWin, &Win1, &Win2, &x1,
			    &y1, &x2, &y2, &modif);
			pt.y = y2 - xobj->y;
			pt.x = x2 - xobj->x;
			if (PtInRect(pt, rect)) {
				if (In) {
					...
					if (xobj->value2 < 1)
						...
						DrawCellule(xobj, NbCell,
						    NbVisCell, HeightCell,
						    xobj->Ffont->ascent, NULL);
						DrawVSbList(
						    xobj, NbCell, NbVisCell, 1);
					}
				} else {
					In = 1;
					DrawVSbList(xobj, NbCell, NbVisCell, 1);
			...
			select(32, SELECT_FD_SET_CAST & in_fdset, NULL, NULL,
			    NULL);
	...
	if (PtInRect(pt, rect)) {
		...
		rectT.x = rect.x;
		rectT.y =
		    rect.y + (xobj->value2 - 1) * (rect.height - 8) / NbCell;
		if (NbVisCell < NbCell)
			rectT.height =
			    NbVisCell * (rect.height - 8) / NbCell + 8;
		else
		...
		if (PtInRect(pt, rectT)) {
			...
			do {
				...
				NPosCell =
				    (pt.y - rect.y) * NbCell / (rect.height);
				...
				if (xobj->value2 != NPosCell) {
					...
					DrawCellule(xobj, NbCell, NbVisCell,
					    HeightCell, xobj->Ffont->ascent,
					    NULL);
					DrawVSbList(xobj, NbCell, NbVisCell, 0);
				}
				FD_ZERO(&in_fdset);
				FD_SET(x_fd, &in_fdset);
				select(32, SELECT_FD_SET_CAST & in_fdset, NULL,
				    NULL, NULL);
			} while (!FCheckTypedEvent(dpy, ButtonRelease, &event));
...
EvtKeyList(struct XObj *xobj, XKeyEvent *EvtKey)
{
	...
	if (ks == XK_Return) {
		if (PtInRect(pt, rect)) {
			...
			pt.y = pt.y - rect.y;
			NPosCell =
			    xobj->value2 + (pt.y / (xobj->Ffont->height + 3));
			if (NPosCell > CountOption(xobj->title))
				NPosCell = 0;
			else if (NPosCell >= xobj->value2 + NbVisCell) {
				NPosCell--;
		...
		if (xobj->value2 == 1) {
			if (PtInRect(pt, rect)) {
				/* Find cell */
				pt.y	 = pt.y - rect.y;
				NPosCell = xobj->value2
					   + (pt.y / (xobj->Ffont->height + 3));
				if (NPosCell > CountOption(xobj->title))
					NPosCell = 0;
				if (NPosCell > 1) {
					FWarpPointer(dpy, None, None, 0, 0, 0,
					    0, 0, -HeightCell);
				}
			}
		} else {
