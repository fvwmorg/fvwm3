...
InitList(struct XObj *xobj)
{
	...
	NbVisCell = (xobj->height - 2 - 3 * BdWidth) /
	    (xobj->Ffont->height + 3);
...
EvtMouseList(struct XObj *xobj, XButtonEvent *EvtButton)
{
	...
	if (PtInRect(pt, rect)) {
		...
		rectT.x = rect.x;
		rectT.y = rect.y +
		    (xobj->value2 - 1) * (rect.height - 8) / NbCell;
		if (NbVisCell < NbCell)
			rectT.height = NbVisCell * (rect.height - 8) / NbCell +
			    8;
		else
