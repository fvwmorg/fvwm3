...
InitItemDraw(struct XObj *xobj)
{
	...
	if (xobj->icon == NULL) {
		if (strlen(xobj->title) != 0) {
			...
			minWidth  = FlocaleTextWidth(xobj->Ffont, xobj->title,
					strlen(xobj->title)) +
			    2;
		...
		/* title and icon */
		if (xobj->icon_w > FlocaleTextWidth(xobj->Ffont, xobj->title,
				       strlen(xobj->title)) +
			2) {
			...
			if (xobj->width < FlocaleTextWidth(xobj->Ffont,
					      xobj->title,
					      strlen(xobj->title)) +
				2)
				xobj->width = FlocaleTextWidth(xobj->Ffont,
						  xobj->title,
						  strlen(xobj->title)) +
				    2;
		}
...
DrawItemDraw(struct XObj *xobj, XEvent *evp)
{
	...
	if (evp) {
		XClearArea(dpy, xobj->win, evp->xexpose.x, evp->xexpose.y,
		    evp->xexpose.width, evp->xexpose.height, False);
	} else {
		XClearArea(dpy, xobj->win, 0, 0, xobj->width, xobj->height,
		    False);
	}
...
EvtMouseItemDraw(struct XObj *xobj, XButtonEvent *EvtButton)
{
	...
	FQueryPointer(dpy, *xobj->ParentWin, &Win1, &Win2, &x1, &y1, &x2, &y2,
	    &modif);
