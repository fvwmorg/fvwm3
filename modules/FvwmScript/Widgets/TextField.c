...
DrawTextField(struct XObj *xobj, XEvent *evp)
{
	...
	offset	= getByteOffsetBoundsCheck(xobj->Ffont, xobj->title,
	     xobj->value);
	...
	XClearArea(dpy, xobj->win, 2, 2, xobj->width - 4, xobj->height - 4,
	    False);
	XSetForeground(dpy, xobj->gc, xobj->TabColor[fore]);
	...
	if (FlocaleTextWidth(xobj->Ffont, xobj->title + offset3,
		...
		right -= FlocaleStringNumberOfBytes(xobj->Ffont,
		    xobj->title +
			FlocaleStringCharToByteOffset(xobj->Ffont, xobj->title,
			    FlocaleStringByteToCharOffset(xobj->Ffont,
				xobj->title, right) -
				1));
	}
...
EvtMouseTextField(struct XObj *xobj, XButtonEvent *EvtButton)
{
	...
	case Button1:
		...
		while ((curs_pos < strlen(xobj->title + start_pos)) &&
		    (x2 > FlocaleTextWidth(xobj->Ffont, xobj->title + start_pos,
			      curs_pos) +
			    8)) {
			curs_pos += FlocaleStringNumberOfBytes(xobj->Ffont,
		...
		if (xobj->value != xobj->value2) {
			...
			Scrapt = (char *)realloc((void *)Scrapt,
			    (strlen(str) + 2) * sizeof(char));
			Scrapt = strcpy(Scrapt, str);
			free(str);
			x11base->HaveXSelection = True;
			XSetSelectionOwner(dpy, XA_PRIMARY, x11base->win,
			    EvtButton->time);
...
EvtKeyTextField(struct XObj *xobj, XKeyEvent *EvtKey)
{
	...
	if ((xobj->value != new_value) || (xobj->value2 != new_value)) {
		...
		x2 = FlocaleTextWidth(xobj->Ffont, xobj->title,
		    strlen(xobj->title));
