...
InitTextField(struct XObj *xobj)
{
	...
	i = FlocaleTextWidth(xobj->Ffont, xobj->title, strlen(xobj->title))
	    + 40;
	if (xobj->width < i)
		xobj->width = i;
...
/* text cursor drawing */
void
DrawPointTxt(struct XObj *xobj, unsigned int pixel)
{
#define dec 2
	int	 x, y;
	...
	offset_abs = FlocaleStringCharToByteOffset(
	    xobj->Ffont, xobj->title, xobj->value);
	offset_rel = FlocaleStringCharToByteOffset(
	    xobj->Ffont, xobj->title, xobj->value3);
	x = FlocaleTextWidth(
		xobj->Ffont, xobj->title + offset_rel, offset_abs - offset_rel)
	    + 5;
...
void
DrawTextField(struct XObj *xobj, XEvent *evp)
{
	int x1, y1;
	int x2, l;
	int nl	  = 0;
	...
	offset2 =
	    getByteOffsetBoundsCheck(xobj->Ffont, xobj->title, xobj->value2);
	offset3 =
	    getByteOffsetBoundsCheck(xobj->Ffont, xobj->title, xobj->value3);
	...
	XClearArea(
	    dpy, xobj->win, 2, 2, xobj->width - 4, xobj->height - 4, False);
	XSetForeground(dpy, xobj->gc, xobj->TabColor[fore]);
	/* computation of the first visible character */
	while (l - nl >= 1
	       && FlocaleTextWidth(xobj->Ffont, xobj->title + nl, offset - nl)
		      > (xobj->width - 10)) {
	...
	{
		xobj->value3 =
		    getCharOffsetBoundsCheck(xobj->Ffont, xobj->title, nl);
		/*FlocaleStringByteToCharOffset(xobj->Ffont,
		  xobj->title, nl);*/
		offset3 = nl;
	} else if (xobj->value3 > xobj->value) {
		xobj->value3--;
		offset3 = FlocaleStringCharToByteOffset(
		    xobj->Ffont, xobj->title, xobj->value3);
	}
	...
	while (right < l
	       && FlocaleTextWidth(
		      xobj->Ffont, xobj->title + offset3, right - offset3)
		      <= xobj->width - 10) {
		right += FlocaleStringNumberOfBytes(
		    xobj->Ffont, xobj->title + right);
	}
	...
	if (FlocaleTextWidth(
		...
		right -= FlocaleStringNumberOfBytes(xobj->Ffont,
		    xobj->title
			+ FlocaleStringCharToByteOffset(xobj->Ffont,
			    xobj->title,
	...
	{
		x1 = FlocaleTextWidth(
		    xobj->Ffont, &xobj->title[offset3], offset - offset3);
		x2 = FlocaleTextWidth(
		    xobj->Ffont, &xobj->title[offset], offset2 - offset);
	} else /* Curseur apres la souris / cursor after the mouse */
	{
		x1 = FlocaleTextWidth(
		    xobj->Ffont, &xobj->title[offset3], offset2 - offset3);
		x2 = FlocaleTextWidth(
		    xobj->Ffont, &xobj->title[offset2], offset - offset2);
	}
	XFillRectangle(dpy, xobj->win, xobj->gc, x1 + 5, 7, x2,
	    y1 + xobj->Ffont->descent - 2);
...
EvtMouseTextField(struct XObj *xobj, XButtonEvent *EvtButton)
{
	...
	case Button1:
		...
		start_pos = getByteOffsetBoundsCheck(
		    xobj->Ffont, xobj->title, xobj->value3);
		selection_pos = getByteOffsetBoundsCheck(
		    xobj->Ffont, xobj->title, xobj->value3);
		DrawPointTxt(xobj, xobj->TabColor[fore]);
		...
		while (ButPress) {
			...
			case MotionNotify:
				...
				while (
				    (curs_pos < strlen(xobj->title + start_pos))
				    && (x2 > FlocaleTextWidth(xobj->Ffont,
				...
				if (PosCurs > (xobj->value2 - xobj->value3)) {
					/* select made "forward" */
					rect.x = FlocaleTextWidth(xobj->Ffont,
					    xobj->title + start_pos,
					    selection_pos - start_pos);
					rect.y = 0;
					rect.width =
					    FlocaleTextWidth(xobj->Ffont,
						xobj->title + start_pos,
						curs_pos)
					    - rect.x + 1;
	...
	case Button2: /* Paste text */
		...
		if (!x11base->HaveXSelection) {
			...
			while (
			    !(FCheckTypedEvent(dpy, SelectionNotify, &event)))
				;
			if (event.xselection.property != None)
				if (event.xselection.selection == XA_PRIMARY) {
					XGetWindowProperty(dpy,
					    ...
					    &format, &longueur, &octets_restant,
					    &donnees);
					if (longueur > 0) {
						Scrapt = (char *)realloc(
						    (void *)Scrapt,
	...
	case Button3: /* Push on third button */
		...
		while ((PosCurs < strlen(xobj->title))
		       && (x2 > FlocaleTextWidth(xobj->Ffont,
				    xobj->title + xobj->value3, PosCurs)
				    + 8))
			PosCurs++;
...
EvtKeyTextField(struct XObj *xobj, XKeyEvent *EvtKey)
{
		...
		if (strcmp(carks, "Right") == 0) {
			...
			/* otherwise step forward as many bytes as the
			   next charcter at the insertion point is wide */
				...
				NewPos += FlocaleStringNumberOfBytes(
				    xobj->Ffont, xobj->title + NewPos);
			...
			/* otherwise step back as many bytes as the chacter
			   behind the insertion point */
				...
				NewPos = FlocaleStringCharToByteOffset(
				    xobj->Ffont, xobj->title, xobj->value - 1);
			...
			if (NewPos > 0) {
				...
				PrevPos = FlocaleStringCharToByteOffset(
				    xobj->Ffont, xobj->title, xobj->value - 1);
				memmove(&xobj->title[PrevPos],
				    &xobj->title[NewPos], Size - NewPos + 1);
				xobj->title = (char *)realloc(
				    xobj->title, (Size - (NewPos - PrevPos) + 1)
						     * sizeof(char));
				NewPos = PrevPos;
				SendMsg(xobj, SingleClic);
			}
	...
	if ((xobj->value != new_value) || (xobj->value2 != new_value)) {
		...
		XFillRectangle(dpy, xobj->win, xobj->gc, x2 + 4, 4,
		    xobj->width - x2 - 8, xobj->height - 8);
		...
		/*FlocaleStringByteToCharOffset(xobj->Ffont,
							  ...
							  NewPos);*/
		xobj->value2 =
		    new_value; /*FlocaleStringByteToCharOffset(xobj->Ffont,
					       xobj->title,
					       NewPos);*/
		DrawPointTxt(xobj, xobj->TabColor[fore]);
		DrawTextField(xobj, NULL);
