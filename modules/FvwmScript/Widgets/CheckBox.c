...
InitCheckBox(struct XObj *xobj)
{
	...
	xobj->width  = FlocaleTextWidth(xobj->Ffont, xobj->title,
			   strlen(xobj->title)) +
	    30;
...
void
DrawCheckBox(struct XObj *xobj, XEvent *evp)
{
	XSegment segm[2];
	...
	/* draw the cross */
	if (xobj->value) {
		XSetLineAttributes(dpy, xobj->gc, 2, LineSolid, CapProjecting,
		    JoinMiter);
		segm[0].x1 = 5;
		segm[0].y1 = 5;
		...
		XSetLineAttributes(dpy, xobj->gc, 1, LineSolid, CapRound,
		    JoinMiter);
	}
}
	...
	while (End) {
		...
		case EnterNotify:
			...
			if (WinBut == 0) {
				WinBut = Win2;
				/* Mouse on button */
				DrawReliefRect(0, xobj->Ffont->ascent - 11,
				    xobj->height, xobj->height, xobj, shad,
				    hili);
				In = 1;
			} else {
				if (Win2 == WinBut) {
					/* Mouse on button */
					DrawReliefRect(0,
					    xobj->Ffont->ascent - 11,
					    xobj->height, xobj->height, xobj,
					    shad, hili);
					In = 1;
				} else if (In) {
					In = 0;
		...
		case LeaveNotify:
			...
			if (Win2 == WinBut) {
				...
				/* Mouse not on button */
				DrawReliefRect(0, xobj->Ffont->ascent - 11,
				    xobj->height, xobj->height, xobj, hili,
				    shad);
				In = 0;
			}
			break;
		case ButtonRelease:
			...
			/* Mouse not on button */
			if (In) {
				/* Send an empty message of SingleClic type, for
				 * a mouse click */
			...
			if (xobj->value) {
				XSetLineAttributes(dpy, xobj->gc, 2, LineSolid,
				...
				XSetForeground(dpy, xobj->gc,
				    xobj->TabColor[fore]);
				XDrawSegments(dpy, xobj->win, xobj->gc, segm,
				    2);
				XSetLineAttributes(dpy, xobj->gc, 1, LineSolid,
				    CapRound, JoinMiter);
			} else {
				XClearArea(dpy, xobj->win, 4, 4,
				    xobj->height - 8, xobj->height - 8, False);
			}
			break;
		}
	}
...
EvtKeyCheckBox(struct XObj *xobj, XKeyEvent *EvtKey)
{
	...
	if (ks == XK_Return) {
		...
		if (xobj->value) {
			...
			XClearArea(dpy, xobj->win, 4, 4, xobj->height - 8,
			    xobj->height - 8, False);
		}
	}
}
