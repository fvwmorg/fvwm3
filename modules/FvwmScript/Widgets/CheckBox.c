...
InitCheckBox(struct XObj *xobj)
{
	...
	xobj->width =
	    FlocaleTextWidth(xobj->Ffont, xobj->title, strlen(xobj->title))
	    + 30;
...
DrawCheckBox(struct XObj *xobj, XEvent *evp)
{
	...
	if (xobj->value) {
		XSetLineAttributes(
		    dpy, xobj->gc, 2, LineSolid, CapProjecting, JoinMiter);
		segm[0].x1 = 5;
		segm[0].y1 = 5;
		...
		XSetLineAttributes(
		    dpy, xobj->gc, 1, LineSolid, CapRound, JoinMiter);
	}
}
