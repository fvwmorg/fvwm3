...
DrawMiniScroll(struct XObj *xobj, XEvent *evp)
{
	...
	DrawReliefRect(
	    -1, -1, xobj->width + 2, xobj->height + 2, xobj, hili, shad);
...
EvtMouseMiniScroll(struct XObj *xobj, XButtonEvent *EvtButton)
{
	...
	do {
		if (count == 1) {
			...
			ts  = tv->tv_sec;
			while (
			    ((tv->tv_usec - tus) + (tv->tv_sec - ts) * 1000000)
			    < 16667 * 16)
				gettimeofday(tv, NULL);
			free(tv);
			count++;
		...
		y2 = y2 - xobj->y;
		x2 = x2 - xobj->x;
		if ((x2 > 0) && (x2 < xobj->width) && (y2 > 0)
		    && (y2 < xobj->height / 2)) {
			/* top arrow */
			if (Pos == 1) {
				tv = (struct timeval *)calloc(
				    1, sizeof(struct timeval));
				gettimeofday(tv, NULL);
				tus = tv->tv_usec;
				ts  = tv->tv_sec;
				while (((tv->tv_usec - tus)
		...
		} else if ((x2 > 0) && (x2 < xobj->width)
			   && (y2 > xobj->height / 2) && (y2 < xobj->height)) {
			if (Pos == -1) {
				tv = (struct timeval *)calloc(
				    1, sizeof(struct timeval));
				gettimeofday(tv, NULL);
