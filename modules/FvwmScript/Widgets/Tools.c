...
int
getByteOffsetBoundsCheck(FlocaleFont *flf, char *str, int offset)
{
...
int
getCharOffsetBoundsCheck(FlocaleFont *flf, char *str, int offset)
{
 ...
 */
void
MyDrawString(Display *dpy, struct XObj *xobj, Window win, int x, int y,
    char *str, unsigned long ForeC, unsigned long HiC, unsigned long BackC,
    int WithRelief, XRectangle *clip, XEvent *evp)
{
	Region	   region = None;
	XRectangle inter;
	Bool	   do_draw = True;
	...
	if (evp && clip) {
		if (!frect_get_intersection(clip->x, clip->y, clip->width,
			clip->height, evp->xexpose.x, evp->xexpose.y,
			evp->xexpose.width, evp->xexpose.height, &inter)) {
			do_draw = False;
		}
	} else if (clip) {
		inter.x	     = clip->x;
 ...
 */
int
GetXTextPosition(struct XObj *xobj, int obj_width, int str_len, int left_offset,
    int center_offset, int right_offset)
{
	int position;
	int x;
...
void
UnselectMenu(struct XObj *xobj, Window WinPop, int hOpt, int value,
    unsigned int width, int asc, int start)
{
	...
	len = strlen(str);
	x   = GetXTextPosition(
	      xobj, width, FlocaleTextWidth(xobj->Ffont, str, len), 8, 0, 8);
 ...
 */
void
SelectMenu(struct XObj *xobj, Window WinPop, int hOpt, int value)
{
	XSegment     segm[2];
	unsigned int i;
	int	     x, y;
 ...
 */
void
DrawIconStr(int offset, struct XObj *xobj, int DoRedraw, int l_offset,
    int c_offset, int r_offset, XRectangle *str_clip, XRectangle *icon_clip,
    XEvent *evp)
{
	int	   i, j;
	char *	   str;
	...
	if (evp) {
		if (!frect_get_intersection(clear_r.x, clear_r.y, clear_r.width,
	...
	if (DoRedraw && do_clear) {
		XClearArea(dpy, xobj->win, inter.x, inter.y, inter.width,
		    inter.height, False);
	}

	str = GetMenuTitle(xobj->title, 1);
	len = strlen(str);
	i   = GetXTextPosition(xobj, xobj->width,
	      FlocaleTextWidth(xobj->Ffont, str, len), l_offset, c_offset,
	      r_offset);
	...
	if (len > 0 && xobj->iconPixmap == None) {
		...
		MyDrawString(dpy, xobj, xobj->win, i, j, str, fore, hili, back,
		...
		if (evp && icon_clip) {
			if (!frect_get_intersection(icon_clip->x, icon_clip->y,
				icon_clip->width, icon_clip->height,
				evp->xexpose.x, evp->xexpose.y,
				evp->xexpose.width, evp->xexpose.height,
				&inter)) {
				do_draw_icon = False;
			} else if (!frect_get_intersection(inter.x, inter.y,
				       inter.width, inter.height, ix, iy,
				       xobj->icon_w, xobj->icon_h, &ir)) {
				do_draw_icon = False;
			}
		} else if (icon_clip) {
			if (!frect_get_intersection(icon_clip->x, icon_clip->y,
				icon_clip->width, icon_clip->height, ix, iy,
				xobj->icon_w, xobj->icon_h, &ir)) {
				do_draw_icon = False;
			}
		} else if (evp) {
			if (!frect_get_intersection(evp->xexpose.x,
				evp->xexpose.y, evp->xexpose.width,
				evp->xexpose.height, ix, iy, xobj->icon_w,
				xobj->icon_h, &ir)) {
				do_draw_icon = False;
			}
		} else {
		...
		if (len > 0) {
			...
			MyDrawString(dpy, xobj, xobj->win, i, j, str, fore,
			    hili, back, !xobj->flags[1], str_clip, evp);
		}
		/* Draw icon */
		...
		if (do_draw_icon) {
			PGraphicsRenderPixmaps(dpy, xobj->win, xobj->iconPixmap,
			    xobj->icon_maskPixmap, xobj->icon_alphaPixmap,
			    Pdepth, &fra, xobj->win, xobj->gc, None, None,
			    ir.x - ix, ir.y - iy, ir.width, ir.height, ir.x,
			    ir.y, ir.width, ir.height, False);
		}
	}
	free(str);
}
...
InsertText(struct XObj *xobj, char *str, int SizeStr)
{
	...
	NewPos =
	    getByteOffsetBoundsCheck(xobj->Ffont, xobj->title, xobj->value);
	Size = strlen(xobj->title);
	xobj->title =
	    (char *)realloc(xobj->title, (1 + SizeStr + Size) * sizeof(char));
...
GetText(struct XObj *xobj, int End)
{
	...
	if (End > xobj->value) {
		a = getByteOffsetBoundsCheck(
		    xobj->Ffont, xobj->title, xobj->value);
		b = getByteOffsetBoundsCheck(xobj->Ffont, xobj->title, End);
	} else {
		b = getByteOffsetBoundsCheck(
		    xobj->Ffont, xobj->title, xobj->value);
		a = getByteOffsetBoundsCheck(xobj->Ffont, xobj->title, End);
	}
