...
DrawPMenu(struct XObj *xobj, Window WinPop, int h, int StrtOpt)
{
	...
	for (i = StrtOpt; i <= xobj->value3; i++)
		UnselectMenu(xobj, WinPop, h, i, width, xobj->Ffont->ascent,
		    StrtOpt);
}
    ...
    unsigned int width, int asc, int start)
{
	...
	y += asc + 4;
	len = strlen(str);
	x   = GetXTextPosition(xobj, width,
	      FlocaleTextWidth(xobj->Ffont, str, len), 8, 0, 8);
...
InsertText(struct XObj *xobj, char *str, int SizeStr)
{
	...
	xobj->title = (char *)realloc(xobj->title,
	    (1 + SizeStr + Size) * sizeof(char));
