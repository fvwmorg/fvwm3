...
/*
 * Function for HScrollBar
 */
void
DrawThumbH(struct XObj *xobj, XEvent *evp)
{
	int	 x, y, w, h;
	XSegment segm;
	char	 str[20];
...
HideThumbH(struct XObj *xobj)
{
	...
	/* clear "text" */
	XClearArea(dpy, xobj->win, 0, 0, xobj->width, xobj->height / 2 - 15,
	    False);
}
...
EvtMouseHScrollBar(struct XObj *xobj, XButtonEvent *EvtButton)
{
	...
	do {
		...
		if (oldx != x2) {
			oldx = x2;
			/* Compute xobj->value */
			newvalue = (x2 - 15) * xobj->width /
				(xobj->width - 36) *
				(xobj->value3 - xobj->value2) / (xobj->width) +
			    xobj->value2;
...
EvtKeyHScrollBar(struct XObj *xobj, XKeyEvent *EvtKey)
{
	...
	if (ks == XK_Left && xobj->value > 0) {
	    ...
	    xobj->value <
		xobj->width * (xobj->value3 - xobj->value2) / (xobj->width) +
		    xobj->value2) {
		HideThumbH(xobj);
		xobj->value++;
		DrawThumbH(xobj, NULL);
