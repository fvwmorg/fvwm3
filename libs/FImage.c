...
FShmInit(Display *dpy)
{
	...
	FShmImagesSupported = XQueryExtension(
	    dpy, "MIT-SHM", &FShmMajorOpCode, &FShmEventBase, &FShmErrorBase);
}
    ...
    unsigned int depth, int format, unsigned int width, unsigned int height)
{
	...
	fim->shminfo->shmaddr = fim->im->data =
	    Fshmat(fim->shminfo->shmid, 0, 0);
    ...
    unsigned int width, unsigned int height)
{
	...
	if (!fim->im) {
		if ((fim->im = XCreateImage(dpy, visual, depth, ZPixmap, 0, 0,
			...
			fim->im->data =
			    fxmalloc(fim->im->bytes_per_line * height);
		} else {
			free(fim);
		}
	}
