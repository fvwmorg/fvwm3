 ...
 *
 **/
char *searchPath(const char *path, const char *filename, const char *suffix,
    int type);
...
void PictureInitCMapRoot(Display *dpy, Bool init_color_limit,
...
/** Search for file along pathlist.  If pathlist is NULL, will use the current
    ...
    more details.  **/
char *PictureFindImageFile(const char *filename, const char *pathlist,
    int mode);
...
#define FPAM_NO_ALLOC_PIXELS                                                    \
	(1)			     /* do not return the allocated pixels      \
