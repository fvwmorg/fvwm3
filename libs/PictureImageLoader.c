...
PImageLoadSvg(FIMAGE_CMD_ARGS)
{
	...
	while (*render_opts) {
		...
		default:
			j = 0;
			if (sscanf(render_opts, "%dx%n%d%n", &b1, &j, &b2,
				&i) >= 2 &&
			    i > j) {
				w = b1;
				h = b2;
			...
			} else if (sscanf(render_opts, "%d%d%n", &b1, &b2,
				       &i) >= 2) {
				dw += b1;
				dh += b2;
			}
...
PImageLoadPng(FIMAGE_CMD_ARGS)
{
	...
	rewind(f);
	Fpng_ptr = Fpng_create_read_struct(FPNG_LIBPNG_VER_STRING, NULL, NULL,
	    NULL);
	if (!Fpng_ptr) {
		fclose(f);
		return False;
    ...
    FvwmPictureAttributes fpa)
{
	...
	if (alpha && !(fpa.mask & FPAM_NO_ALPHA) && alpha_depth) {
		alpha_limit = 0;
		a_fim = FCreateFImage(dpy, Pvisual, alpha_depth, ZPixmap, width,
		    height);
	}
    ...
    FvwmPictureAttributes fpa)
{
	...
	/* Bitmap fallback */
		     ...
		     (unsigned int *)height, pixmap, NULL,
		     NULL) == BitmapSuccess) {
		*depth = 1;
		*mask  = None;
    ...
    int y_hot)
{
	...
	if ((fcis = FcursorFilenameLoadImages(path,
	...
	/* Get cursor data from the regular image loader */
		...
		/* Create monochrome cursor from argb data */
			...
			cursor = XCreatePixmapCursor(dpy, src, msk, &(c[0]),
			    &(c[1]), x_hot, y_hot);
			XFreePixmap(dpy, src);
			XFreePixmap(dpy, msk);
		}
