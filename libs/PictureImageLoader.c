...
#define FIMAGE_PASS_ARGS dpy, path, argb_data, width, height
...
PImageLoader Loaders[] = {{"xpm", PImageLoadXpm}, {"svg", PImageLoadSvg},
    {"png", PImageLoadPng}, {NULL, 0}};

/* ---------------------------- exported variables (globals) --------------- */

/* ---------------------------- local functions ---------------------------- */

static Bool PImageLoadArgbDataFromFile(FIMAGE_CMD_ARGS)
{
...
static Bool PImageLoadSvg(FIMAGE_CMD_ARGS)
{
	...
	while (*render_opts) {
		...
		default:
			j = 0;
			if (sscanf(render_opts, "%dx%n%d%n", &b1, &j, &b2, &i)
				>= 2
			    && i > j) {
				w = b1;
				h = b2;
			...
			} else if (sscanf(render_opts, "%d%d%n", &b1, &b2, &i)
				   >= 2) {
				dw += b1;
				dh += b2;
			}
	...
	for (i = 0; i < w * h; i++) {
		if ((a_value = (data[i] >> 030) & 0xff)) {
			...
			data[i] = (a_value << 030) | (r_value << 020)
				  | (g_value << 010) | b_value;
		}
	}
...
static Bool PImageLoadPng(FIMAGE_CMD_ARGS)
{
	...
	rewind(f);
	Fpng_ptr =
	    Fpng_create_read_struct(FPNG_LIBPNG_VER_STRING, NULL, NULL, NULL);
	if (!Fpng_ptr) {
		fclose(f);
		return False;
	}
	...
	Fpng_get_IHDR(Fpng_ptr, Finfo_ptr, (Fpng_uint_32 *)(&w32),
	    (Fpng_uint_32 *)(&h32), &bit_depth, &color_type, &interlace_type,
	    NULL, NULL);
	...
	*height = h = (int)h32;
	...
	if (hasa)
		...
		/* we want ARGB */
		/* note form raster:
		 * thanks to mustapha for helping debug this on PPC Linux
		 * remotely by sending across screenshots all the time and me
		 * figuring out form them what the hell was up with the colors
...
static Bool PImageLoadXpm(FIMAGE_CMD_ARGS)
{
	...
	for (i = 0; i < xpm_im.ncolors; i++) {
		...
		if (XParseColor(dpy, Pcmap, visual_color, &color)) {
			colors[i] = 0xff000000 | ((color.red << 8) & 0xff0000)
				    | ((color.green) & 0xff00)
				    | ((color.blue >> 8) & 0xff);
		} else {
			colors[i] = 0;
    ...
    FvwmPictureAttributes fpa)
{
	...
	fim = FCreateFImage(dpy, Pvisual,
	    (fpa.mask & FPAM_MONOCHROME) ? 1 : Pdepth, ZPixmap, width, height);
	if (!fim) {
		return False;
	}
	if (mask) {
		m_fim = FCreateFImage(dpy, Pvisual, 1, ZPixmap, width, height);
	}
	...
	if (!(fpa.mask & FPAM_MONOCHROME)) {
		...
		pica = PictureOpenImageColorAllocator(dpy, Pcmap, width, height,
	...
	for (j = 0; j < height; j++) {
		for (i = 0; i < width; i++, data++) {
			...
			if (a > alpha_limit) {
				...
				else if ((0x99 * c.red + 0x12D * c.green
					     + 0x3A * c.blue)
					 >> 16) {
    ...
    Display *dpy, Window win, char *path, int x_hot, int y_hot)
{
	...
	/* Get cursor data from the regular image loader */
	else if (PImageLoadArgbDataFromFile(
		     dpy, path, &data, &width, &height)) {
		Pixmap		      src;
		Pixmap		      msk = None;
		FvwmPictureAttributes fpa;
		...
		if (x_hot < 0 || x_hot >= width || y_hot < 0
		    || y_hot >= height) {
			FxpmImage xpm_im;
			FxpmInfo  xpm_info;
...
Bool
PImageLoadPixmapFromXpmData(Display *dpy, Window win, int color_limit,
    char **data, Pixmap *pixmap, Pixmap *mask, int *width, int *height,
    int *depth)
{
	...
	xpm_attributes.valuemask = FxpmCloseness | FxpmExtensions | FxpmVisual
