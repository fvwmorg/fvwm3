...
#define FLC_IS_TRANSLIT_SUPPORTED(fc)                              \
	((fc) != NULL &&                                           \
	    (fc)->translit_csname != FLC_TRANSLIT_NOT_SUPPORTED && \
 ...
 */
void FlocaleInit(int category, const char *local, const char *modifier,
    const char *module);
 ...
 */
void FlocaleDrawUnderline(Display *dpy, FlocaleFont *flf, FlocaleWinString *fws,
    int coffset);

/*
 * Get the position for shadow text
 */
void FlocaleInitGstpArgs(flocale_gstp_args *args, FlocaleFont *flf,
    FlocaleWinString *fws, int start_x, int start_y);

Bool FlocaleGetShadowTextPosition(int *x, int *y, flocale_gstp_args *args);
