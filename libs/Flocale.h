...
#define FLC_GET_ICONV_CHARSET(fc)                \
	((fc) != NULL && (fc)->iconv_index >= 0) \
	    ? (fc)->locale[(fc)->iconv_index]    \
	    : NULL
...
#define FLF_SHADOW_HEIGHT(flf)               \
	(FLF_SHADOW_FULL_SIZE((flf))         \
	    * (FLF_MULTIDIR_HAS_UPPER((flf)) \
		+ FLF_MULTIDIR_HAS_BOTTOM((flf))))
...
typedef struct _FlocaleFont
{
	...
	FlocaleCharset
	    *str_fc; /* fvwm charset of the strings to be displayed */
...
void
FlocaleInit(
    int category, const char *local, const char *modifier, const char *module);
 ...
 */
void
FlocaleDrawString(Display *dpy, FlocaleFont *ff, FlocaleWinString *fstring,
    unsigned long flags);
 ...
 */
int
FlocaleTextWidth(FlocaleFont *ff, char *str, int sl);
