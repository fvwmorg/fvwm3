...
void   CollectBindingList(Display *dpy, Binding **pblist_src,
      Binding **pblist_dest, Bool *ret_are_similar_bindings_left, binding_t type,
      int button, KeySym keysym, int modifiers, int contexts, char *windowName);
int    AddBinding(Display *dpy, Binding **pblist, binding_t type, int button,
...
Bool   RemoveMatchingBinding(Display *dpy, Binding **pblist, binding_t type,
...
void   GrabAllWindowKeysAndButtons(Display *dpy, Window w, Binding *blist,
      unsigned int contexts, unsigned int dead_modifiers, Cursor cursor,
      Bool fGrab);
void   GrabWindowKeyOrButton(Display *dpy, Window w, Binding *binding,
      unsigned int contexts, unsigned int dead_modifiers, Cursor cursor,
      Bool fGrab);
