...
FlocaleCharset FlocaleCharsetTable[] = {CT_ENTRY("ARMSCII-8", armscii_8, NULL),
    ...
    CT_ENTRY("KOI8-U", koi8_u, NULL), CT_ENTRY("KSC5601.1987-0", ksc5601, NULL),
    ...
    CT_ENTRY("MULELAO-1", mulelao_1, NULL), CT_ENTRY("SJISX-1", sjisx_1, NULL),
...
FlocaleInit_X_Charset(Display *dpy, const char *module)
{
	...
	if (om && XGetOMValues(om, XNRequiredCharSet, &cs, NULL) == NULL) {
		if (cs.charset_count > 0) {
			...
			FLCXOMCharsetList =
			    fxmalloc(sizeof(FlocaleCharset) * cs.charset_count);
...
FlocaleCharsetSetFlocaleCharset(
    Display *dpy, FlocaleFont *flf, char *hints, char *encoding, char *module)
{
	char *charset	  = NULL;
	char *iconv	  = NULL;
	...
	if (flf->fc == NULL) {
		if (FftSupport && flf->fftf.fftfont != NULL) {
			...
			if (FLCXOMCharset != NULL) {
				...
				if (XFontsOfFontSet(flf->fontset, &fs_list, &ml)
				    > 0) {
					flf->fc = FLCXOMCharset =
	...
	if (encoding != NULL) {
		...
		if (flf->fftf.str_encoding != NULL) {
			flf->str_fc =
			    FlocaleCharsetOfXCharset(flf->fftf.str_encoding);
			if (flf->str_fc == NULL) {
