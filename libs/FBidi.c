    ...
    Bool *is_rtl, int *out_len, superimpose_char_t *comb_chars, int *l_to_v)
{
	...
	logical_unicode_str = fxmalloc((str_len + 1) * sizeof(FriBidiChar));

	/* convert to unicode first */
	str_len = fribidi_charset_to_unicode(fribidi_charset,
	    (char *)logical_str, str_len, logical_unicode_str);

	visual_unicode_str = fxmalloc((str_len + 1) * sizeof(FriBidiChar));
	...
	(void)!fribidi_log2vis(logical_unicode_str, str_len, &pbase_dir,
	...
	if (comb_chars != NULL) {
		for (i = 0;
			...
			comb_chars[i].position = str_len != 0 ?
	...
	if (l_to_v != NULL) {
		...
		orig_len    = i;
		l_to_v_temp = fxmalloc(orig_len * sizeof(int));
	...
	visual_str = fxmalloc((4 * str_len + 1) * sizeof(char));

	/* convert from unicode finally */
	*out_len = fribidi_unicode_to_charset(fribidi_charset,
	    visual_unicode_str, str_len, visual_str);
