...
static Status
SetAuthentication(int count, FIceListenObj *listenObjs,
    FIceAuthDataEntry **authDataEntries)
{
	FILE *addfp    = NULL;
	FILE *removefp = NULL;
	char *path;
	...
	*authDataEntries = fxmalloc(count * 2 * sizeof(FIceAuthDataEntry));
	...
	for (i = 0; i < count * 2; i += 2) {
		...
		(*authDataEntries)[i].auth_data = FIceGenerateMagicCookie(
		    MAGIC_COOKIE_LEN);
		...
		(*authDataEntries)[i + 1].auth_data = FIceGenerateMagicCookie(
		    MAGIC_COOKIE_LEN);
    ...
    FIcePointer *watch_data)
{
	...
	if (opening) {
		...
		*watch_data	    = (FIcePointer)fice_conn;
		fsm_ice_conn_list   = flist_append_obj(fsm_ice_conn_list,
		      fice_conn);
		fcntl(fice_conn->fd, F_SETFD, FD_CLOEXEC);
	} else {
		fice_conn	  = (fsm_ice_conn_t *)*watch_data;
		fsm_ice_conn_list = flist_remove_obj(fsm_ice_conn_list,
		    fice_conn);
		free(fice_conn);
	}
}
...
RegisterClientProc(FSmsConn smsConn, FSmPointer managerData, char *previousId)
{
	...
	if (send_save) {
		FSmsSaveYourself(smsConn, FSmSaveLocal, False,
		    FSmInteractStyleNone, False);
	}
...
static void
CloseConnectionProc(FSmsConn smsConn, FSmPointer managerData, int count,
    char **reasonMsgs)
{
	fsm_client_t *client = (fsm_client_t *)managerData;
...
static void
SetPropertiesProc(FSmsConn smsConn, FSmPointer managerData, int numProps,
    FSmProp **props)
{
	int i;
...
static void
DeletePropertiesProc(FSmsConn smsConn, FSmPointer managerData, int numProps,
    char **propNames)
{
	int i;
    ...
    FSmsCallbacks *callbacksRet, char **failureReasonRet)
{
	...
	*maskRet |= FSmsSaveYourselfP2RequestProcMask;
	...
	callbacksRet->save_yourself_phase2_request.manager_data = (FSmPointer)
	    nc;
...
NewConnectionMsg(int i)
{
	...
	if (!ice_conn) {
		...
		pending_ice_conn_list = flist_append_obj(pending_ice_conn_list,
		    ice_conn);
...
GetClientID(Display *dpy, Window window)
{
	...
	if (!_XA_WM_CLIENT_LEADER) {
		_XA_WM_CLIENT_LEADER = XInternAtom(dpy, "WM_CLIENT_LEADER",
		    False);
	}
	if (!_XA_SM_CLIENT_ID) {
...
set_session_manager(Display *dpy, Window window, char *sm)
{
	...
	if (!_XA_SESSION_MANAGER) {
		_XA_SESSION_MANAGER = XInternAtom(dpy, "SESSION_MANAGER",
		    False);
	}
	if (!_XA_WM_CLIENT_LEADER) {
		_XA_WM_CLIENT_LEADER = XInternAtom(dpy, "WM_CLIENT_LEADER",
...
fsm_init(char *module)
{
	...
	if (!FIceListenForConnections(&numTransports, &slistenObjs, 256,
		errormsg)) {
		fvwm_debug(__func__,
		    "[%s][fsm_init] <<ERROR>> -- "
