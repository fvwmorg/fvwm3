...
static XftFont *
FftGetRotatedFont(Display *dpy, FftFontType *ft, rotation_t rotation)
{
	...
	if ((FcPatternGetMatrix(rotated_pat, FC_MATRIX, 0, &pm) ==
		FcResultMatch) &&
	    pm) {
		/* rotate the matrice */
...
void
FftGetFontHeights(FftFontType *fftf, int *height, int *ascent, int *descent)
{
...
void
FftCloseFont(Display *dpy, FftFontType *fftf)
{
	int i = 0;
	for (i = 0; i < 4; i++) {
		if (fftf->fftfont[i])
...
FftFontType *
FftGetFont(Display *dpy, char *fontname, char *module)
{
	...
	/* safety check */
	if (FcPatternGetMatrix(load_pat, FC_MATRIX, 0, &a) == FcResultMatch &&
	    a != NULL) {
		FcMatrix b;
		Bool	 cm = False;
		memset(&b, 0, sizeof(FcMatrix));
...
static FcPattern *
MatchCacheFont(FcChar32 code, FcPattern *src, FcResult *result)
{
	unsigned short hash = FcPatternHash(src);
	int	       i    = 0;
	hash ^= code; /* Because FcPatternHash() does not hash charset due to
			 its cost. */

	for (i = g_font_cache.fifo_start; i < g_font_cache.fifo_end; i++) {
		int idx = i % CACHE_ENTIRES;
		if (g_font_cache.hashes[idx] != hash)
			continue;
...
static void
AddCacheFont(FcChar32 code, FcPattern *src, FcPattern *match)
{
	while (
	    g_font_cache.fifo_end - g_font_cache.fifo_start >= CACHE_ENTIRES) {
...
static void
MatchFont(Display *dpy, XftFont *font, int *xoff, int *yoff, const char *p,
    const int len, void (*render_char)(void *, XftCharFontSpec *),
    void     *user_data)
{
	...
	for (off = 0; off < len; off += codelen) {
		...
		codelen = FcUtf8ToUcs4((const FcChar8 *)p + off, &sp.ucs4,
		    len - off);
		if (codelen <= 0) {
			fvwm_debug("XFT", "Malformed UTF8 char %x",
			    (char)*(p + off));
			codelen = 1;
			continue;
		}
		...
		if (!XftCharExists(dpy, font, sp.ucs4)) {
			...
			if (!template) {
				...
				FcPatternGetDouble(font->pattern, FC_SIZE, 0,
				    &font_size);
				FcPatternGetInteger(font->pattern, FC_WEIGHT, 0,
				    &weight);
				FcPatternGetInteger(font->pattern, FC_SLANT, 0,
				    &slant);
				template = FcPatternBuild(NULL, FC_SIZE,
				    FcTypeDouble, font_size, FC_WEIGHT,
				    FcTypeInteger, weight, FC_SLANT,
			...
			FcPatternAddCharSet(template, FC_CHARSET,
			    fallback_charset);

			/* Lookup from the cache first! */
			FcResult   result	= FcResultMatch;
			FcPattern *font_pattern = MatchCacheFont(sp.ucs4,
			    template, &result);
			if (!font_pattern) {
				font_pattern = XftFontMatch(dpy,
				    XDefaultScreen(dpy), template, &result);
				if (font_pattern)
					AddCacheFont(sp.ucs4, template,
					    font_pattern);
			}
			FcCharSetDestroy(fallback_charset);

			if (result == FcResultMatch) {
				XftFont *fallback_font = XftFontOpenPattern(dpy,
				    font_pattern);
				if (fallback_font)
...
static void
RenderCharFontSpec(void *p, XftCharFontSpec *sp)
{
	struct CharFontSpecBatch *batch = p;

	if (batch->size == NR_CHARFONTSPEC || sp == NULL) {
		XftDrawCharFontSpec(batch->draw, batch->color, batch->specs,
		    batch->size);
		int i = 0;
...
void
FftDrawString(Display *dpy, FlocaleFont *flf, FlocaleWinString *fws, Pixel fg,
    Pixel fgsh, Bool has_fg_pixels, int len, unsigned long flags)
{
	XftDraw		 *draw = NULL;
	char		 *str;
	XGCValues	  vr;
	...
	alpha_factor = ((fws->flags.has_colorset) ?
		((float)fws->colorset->fg_alpha_percent / 100) :
		1);
	...
	if (flf->shadow_size != 0 && has_fg_pixels) {
		while (FlocaleGetShadowTextPosition(&xt, &yt, &gstp_args)) {
			int mxt = xt, myt = yt;
			MatchFont(dpy, uf, &mxt, &myt, str, len,
			    &RenderCharFontSpec, &batch);
...
int
FftTextWidth(Display *dpy, FlocaleFont *flf, char *str, int len)
{
	int xoff = 0, yoff = 0;
	MatchFont(dpy, flf->fftf.fftfont[0], &xoff, &yoff, str, len, NULL,
	    NULL);
	return xoff;
}

void
FftPrintPatternInfo(FftFontType *ft)
{
	...
	for (i = 0; i < 4; i++) {
		...
		if (i == 0) {
			...
			if (FcPatternGetMatrix(f->pattern, FC_MATRIX, 0, &pm) ==
				FcResultMatch &&
			    pm) {
				fvwm_debug("xft",
				    "	   matrix: (%f %f %f %f)\n", pm->xx,
