...
static FftChar16 *
FftUtf8ToFftString16(unsigned char *str, int len, int *nl)
{
	FftChar16 *new;
	int i = 0, j = 0;

	new = fxmalloc((len + 1) * sizeof(FftChar16));
	while (i < len && str[i] != 0) {
		if (str[i] <= 0x7f) {
			new[j] = str[i];
...
static void
FftSetupEncoding(Display *dpy, FftFontType *fftf, char *encoding, char *module)
{
	FlocaleCharset *fc;
	...
	if (FftSupportUseXft2) {
		...
		} else if ((fc = FlocaleCharsetGetDefaultCharset(dpy, NULL))
			       != NULL
...
static FftFont *
FftGetRotatedFont(Display *dpy, FftFont *f, rotation_t rotation)
{
...
void
FftGetFontWidths(FlocaleFont *flf, int *max_char_width)
{
	FGlyphInfo extents;
	...
	if (FftUtf8Support && FLC_ENCODING_TYPE_IS_UTF_8(flf->fc)) {
		FftTextExtentsUtf8(
		    fftdpy, flf->fftf.fftfont, (FftChar8 *)"W", 1, &extents);
	} else {
		FftTextExtents8(
		    fftdpy, flf->fftf.fftfont, (FftChar8 *)"W", 1, &extents);
	}
...
FftGetFont(Display *dpy, char *fontname, char *module)
{
	...
	if ((load_pat = FftFontMatch(dpy, fftscreen, src_pat, &result))
	    == NULL) {
		goto bail;
	}
	/* safety check */
	if (FftPatternGetMatrix(load_pat, FFT_MATRIX, 0, &a) == FftResultMatch
	    && a) {
		FftMatrix b;
		Bool	  cm = False;
	...
	fftf			  = fxmalloc(sizeof(FftFontType));
    ...
    Pixel fgsh, Bool has_fg_pixels, int len, unsigned long flags)
{
	...
	typedef void (*DrawStringFuncType)(FftDraw * fftdraw, FftColor * fft_fg,
	    FftFont * uf, int xt, int yt, char *str, int len);
	...
	if (fws->flags.text_rotation == ROTATION_90) /* CW */
	{
		if (fftf->fftfont_rotated_90 == NULL) {
			fftf->fftfont_rotated_90 = FftGetRotatedFont(
			    dpy, fftf->fftfont, fws->flags.text_rotation);
		}
		...
		if (fftf->fftfont_rotated_180 == NULL) {
			fftf->fftfont_rotated_180 = FftGetRotatedFont(
			    dpy, fftf->fftfont, fws->flags.text_rotation);
		}
		...
		if (fftf->fftfont_rotated_270 == NULL) {
			fftf->fftfont_rotated_270 = FftGetRotatedFont(
			    dpy, fftf->fftfont, fws->flags.text_rotation);
		}
...
int
FftTextWidth(FlocaleFont *flf, char *str, int len)
{
	FGlyphInfo extents;
	int	   result = 0;
	...
	if (FftUtf8Support && FLC_ENCODING_TYPE_IS_UTF_8(flf->fc)) {
		FftTextExtentsUtf8(
		    fftdpy, flf->fftf.fftfont, (FftChar8 *)str, len, &extents);
		result = extents.xOff;
	} else if (FLC_ENCODING_TYPE_IS_UTF_8(flf->fc)) {
		FftChar16 *new;
		int nl;
		...
		FftTextExtents16(
		    fftdpy, flf->fftf.fftfont, (FftChar16 *)str, len, &extents);
		result = extents.xOff;
	} else if (FLC_ENCODING_TYPE_IS_USC_4(flf->fc)) {
		FftTextExtents32(
		    fftdpy, flf->fftf.fftfont, (FftChar32 *)str, len, &extents);
		result = extents.xOff;
	} else {
		FftTextExtents8(
		    fftdpy, flf->fftf.fftfont, (FftChar8 *)str, len, &extents);
...
void
FftPrintPatternInfo(FftFont *f, Bool vertical)
{
