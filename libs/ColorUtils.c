...
static XColor *
GetShadowOrHiliteColor(Pixel background, float light, float dark, float factor)
{
	long	     brightness;
	...
	if (brightness < XmDEFAULT_DARK_THRESHOLD * PCT_BRIGHTNESS) {
		scolor.red   = (unsigned short)(0xffff -
		      ((0xffff - red) * dark + 50) / 100);
		scolor.green = (unsigned short)(0xffff -
		    ((0xffff - green) * dark + 50) / 100);
		scolor.blue  = (unsigned short)(0xffff -
		     ((0xffff - blue) * dark + 50) / 100);
	}
	/* For "light" background, make everything a fixed %age darker */
	else if (brightness > XmDEFAULT_LIGHT_THRESHOLD * PCT_BRIGHTNESS) {
		scolor.red   = (unsigned short)((red * light + 50) / 100);
		scolor.green = (unsigned short)((green * light + 50) / 100);
		scolor.blue  = (unsigned short)((blue * light + 50) / 100);
	}
...
GetTintedColor(Pixel in, Pixel tint, int percent)
{
	...
	scolor.red   = (unsigned short)(((100 - percent) * scolor.red +
					    tint_color.red * percent) /
	      100);
	scolor.green = (unsigned short)(((100 - percent) * scolor.green +
					    tint_color.green * percent) /
	    100);
	scolor.blue  = (unsigned short)(((100 - percent) * scolor.blue +
					    tint_color.blue * percent) /
	     100);
	return &scolor;
}
    ...
    Bool use_hash, int adj)
{
	...
	/* Lighten color */
	if (adj > 0 && adj <= 100) {
		color.red   = (int)(((100 - adj) * color.red + adj * 65535) /
		      100);
		color.green = (int)(((100 - adj) * color.green + adj * 65535) /
		    100);
		color.blue  = (int)(((100 - adj) * color.blue + adj * 65535) /
		     100);
	}
	/* Darken color */
	if (adj >= -100 && adj < 0) {
	...
	if (!use_hash) {
		n = sprintf(output, "rgb:%04x/%04x/%04x", (int)color.red,
		    (int)color.green, (int)color.blue);
	} else {
		n = sprintf(output, "#%02x%02x%02x", (int)(color.red / 256),
...
static char *colorset_names[] = { "$[fg.cs", "$[bg.cs", "$[hilight.cs",
	"$[shadow.cs", "$[fgsh.cs", NULL };
