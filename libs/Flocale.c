...
static void
FlocaleParseShadow(char *str, int *shadow_size, int *shadow_offset,
...
static int
FlocaleChar2bOneCharToUtf8(XChar2b c, char *buf)
{
	int	       len;
	char	       byte1 = c.byte1;
	char	       byte2 = c.byte2;
...
/* return number of bytes of character at current position
   (pointed to by str) */
int
FlocaleStringNumberOfBytes(FlocaleFont *flf, const char *str)
{
	int bytes = 0;
...
/* given a string, font specifying its locale and a byte offset gives
   character offset */
int
FlocaleStringByteToCharOffset(FlocaleFont *flf, const char *str, int offset)
{
...
/* like above but reversed, ie. return byte offset corresponding to given
   charater offset */
int
FlocaleStringCharToByteOffset(FlocaleFont *flf, const char *str, int coffset)
{
	const char *curr_ptr = str;
	int	    i;
	int	    len	   = strlen(str);
...
int
FlocaleStringCharLength(FlocaleFont *flf, const char *str)
{
	int i, len;
...
static XChar2b *
FlocaleUtf8ToUnicodeStr2b(char *str, int len, int *nl)
{
	XChar2b *str2b = NULL;
	int	 i = 0, j = 0, t;
...
static XChar2b *
FlocaleStringToString2b(Display *dpy, FlocaleFont *flf, char *str, int len,
    int *nl)
{
	XChar2b *str2b	  = NULL;
	char	*tmp	  = NULL;
	...
	if (flf->fc && StrEquals(flf->fc->x, "jisx0208.1983-0")) {
		tmp = FiconvCharsetToCharset(dpy, flf->fc,
		    FlocaleCharsetGetEUCJPCharset(), str, len);
		if (tmp != NULL) {
			free_str = True;
			str	 = tmp;
			len	 = strlen(tmp);
...
static char *
FlocaleEncodeString(Display *dpy, FlocaleFont *flf, char *str, int *do_free,
    int len, int *nl, int *is_rtl, superimpose_char_t **comb_chars,
    int **l_to_v)
{
	char	   *str1, *str2, *str3;
	int	    len1;
	...
	*nl	 = len;
	...
	if (flf->str_fc == NULL || flf->fc == NULL || flf->fc == flf->str_fc) {
		do_iconv = False;
	}
	...
	if (FiconvSupport && do_iconv) {
		str2 = FiconvCharsetToCharset(dpy, flf->str_fc, flf->fc,
		    (const char *)str1, len);
		if (str2 == NULL) {
			/* fail to convert */
			return str1;
...
static void
FlocaleEncodeWinString(Display *dpy, FlocaleFont *flf, FlocaleWinString *fws,
    int *do_free, int *len, superimpose_char_t **comb_chars, int **l_to_v)
{
	int len2b;
	fws->e_str = FlocaleEncodeString(dpy, flf, fws->str, do_free, *len, len,
	    NULL, comb_chars, l_to_v);
	fws->str2b = NULL;

	if (flf->font != None) {
		if (FLC_ENCODING_TYPE_IS_UTF_8(flf->fc)) {
			fws->str2b = FlocaleUtf8ToUnicodeStr2b(fws->e_str, *len,
			    &len2b);
		} else if (flf->flags.is_mb) {
			fws->str2b = FlocaleStringToString2b(dpy, flf,
			    fws->e_str, *len, &len2b);
		}
	}
}
...
static void
FlocaleFontStructDrawString(Display *dpy, FlocaleFont *flf, Drawable d, GC gc,
    int x, int y, Pixel fg, Pixel fgsh, Bool has_fg_pixels,
    FlocaleWinString *fws, int len, Bool image)
{
	int		  xt = x;
	int		  yt = y;
	int		  is_string16;
	...
	if (image) {
		/* for rotated drawing */
		FSwitchDrawImageString(is_string16, dpy, d, gc, x, y,
		...
		if (flf->shadow_size != 0 && has_fg_pixels == True) {
			...
			while (FlocaleGetShadowTextPosition(&xt, &yt,
			    &gstp_args)) {
				FSwitchDrawString(is_string16, dpy, d, gc, xt,
				    yt, fws->e_str, fws->str2b, len);
			}
		}
		...
		xt = gstp_args.orig_x;
		yt = gstp_args.orig_y;
		FSwitchDrawString(is_string16, dpy, d, gc, xt, yt, fws->e_str,
		    fws->str2b, len);
	}
 ...
 */
static void
FlocaleRotateDrawString(Display *dpy, FlocaleFont *flf, FlocaleWinString *fws,
    ...
    superimpose_char_t *comb_chars, int *pixel_pos)
{
	...
	if (flf->font != NULL) {
		...
		FlocaleFontStructDrawString(dpy, flf, canvas_pix, font_gc, 0,
		    height - descent, fg, fgsh, has_fg_pixels, fws, len, True);
	} else if (flf->fontset != None) {
		XmbDrawString(dpy, canvas_pix, flf->fontset, font_gc, 0,
	...
	if (comb_chars != NULL) {
		while (
			...
			int   out_len;
			char *buf2 = FiconvUtf8ToCharset(dpy, flf->str_fc,
			...
			if (flf->fontset != None) {
				XmbDrawString(dpy, canvas_pix, flf->fontset,
				    fws->gc, offset, height - descent, buf2,
				    strlen(buf2));
				...
				if (FLC_ENCODING_TYPE_IS_UTF_8(flf->fc)) {
					...
					tmp_fws.str2b = FlocaleStringToString2b(
					    dpy, flf, tmp_fws.e_str, curr_len,
					    &out_len);
				} else {
					out_len = strlen(buf2);
				}
				XSetFont(dpy, font_gc, flf->font->fid);
				FlocaleFontStructDrawString(dpy, flf,
				    canvas_pix, font_gc, offset,
				    height - descent, fg, fgsh, has_fg_pixels,
				    &tmp_fws, out_len, True);
			}
	...
	if ((image = XCreateImage(dpy, Pvisual, 1, XYBitmap, 0,
		 (char *)normal_data, normal_w, normal_h, 8, 0)) == NULL) {
		return;
	}
	image->byte_order = image->bitmap_bit_order = MSBFirst;

	/* extract character from canvas */
	XGetSubImage(dpy, canvas_pix, 0, 0, normal_w, normal_h, 1, XYPixmap,
	    image, 0, 0);
	...
	if ((rotated_image = XCreateImage(dpy, Pvisual, 1, XYBitmap, 0,
		 (char *)rotated_data, rotated_w, rotated_h, 8, 0)) == NULL) {
		return;
	}
	...
	for (j = 0; j < rotated_h; j++) {
		for (i = 0; i < rotated_w; i++) {
			...
			if (fws->flags.text_rotation == ROTATION_270)
				val = normal_data[i * normal_len +
					  (normal_w - j - 1) / 8] &
				    (128 >> ((normal_w - j - 1) % 8));

			else if (fws->flags.text_rotation == ROTATION_180)
				val = normal_data[(normal_h - j - 1) *
					      normal_len +
					  (normal_w - i - 1) / 8] &
				    (128 >> ((normal_w - i - 1) % 8));

			else /* ROTATION_90 */
				val = normal_data[(normal_h - i - 1) *
					      normal_len +
					  j / 8] &
				    (128 >> (j % 8));

			if (val)
				rotated_data[j * rotated_len + i / 8] |= (128 >>
				    (i % 8));
		}
	}
	...
	XPutImage(dpy, rotated_pix, font_gc, rotated_image, 0, 0, 0, 0,
	    rotated_w, rotated_h);
	...
	case ROTATION_180:
		xpfg = fws->x;
		ypfg = fws->y - min_offset + FLF_SHADOW_BOTTOM_SIZE(flf);
		break;
	case ROTATION_270:
		/* CCW */
		xpfg = fws->x - min_offset;
...
static char *
FlocaleGetFullNameOfFontStruct(Display *dpy, XFontStruct *font)
{
	char	     *full_name = NULL;
	unsigned long value;
...
static char *
FlocaleGetCharsetOfFontStruct(Display *dpy, XFontStruct *font)
{
	int   i	      = 0;
	int   count   = 0;
	char *charset = NULL;
...
static char *
FlocaleGetCharsetFromName(char *name)
{
	int   l, i, e;
	char *charset;
...
static char *
FlocaleFixNameForFontSet(Display *dpy, char *name, char *module)
{
	char	    *new_name;
	char	    *charset;
...
static FlocaleFont *
FlocaleGetFftFont(Display *dpy, char *fontname, char *encoding, char *module)
{
	...
	FftGetFontHeights(&flf->fftf, &flf->height, &flf->ascent,
	    &flf->descent);
...
static FlocaleFont *
FlocaleGetFontSet(Display *dpy, char *fontname, char *encoding, char *module)
{
...
static FlocaleFont *
FlocaleGetFont(Display *dpy, char *fontname, char *encoding, char *module)
{
...
static FlocaleFont *
FlocaleGetFontOrFontSet(Display *dpy, char *fontname, char *encoding,
    char *fullname, char *module)
{
	FlocaleFont *flf = NULL;
	...
	if (strlen(fontname) > 3 && strncasecmp("xft:", fontname, 4) == 0) {
		if (FftSupport) {
			flf = FlocaleGetFftFont(dpy, fontname + 4, encoding,
			    module);
		}
		if (flf) {
...
static void
FlocaleSetlocaleForX(int category, const char *locale, const char *module)
{
...
void
FlocaleInit(int category, const char *locale, const char *modifiers,
    const char *module)
{
	...
	fvwm_debug(__func__, "[%s][FlocaleInit] locale: %s, modifier: %s\n",
 ...
 * fonts loading
 */
char *prefix_list[] = { "Shadow=", "StringEncoding=", NULL };

FlocaleFont *
FlocaleLoadFont(Display *dpy, char *fontname, char *module)
{
	...
	while ((i = GetTokenIndex(str, prefix_list, -1, &str)) > -1) {
		...
		switch (i) {
		case 0: /* shadow= */
			FlocaleParseShadow(opt_str, &shadow_size,
			    &shadow_offset, &shadow_dir, fontname, module);
			break;
		case 1: /* encoding= */
			if (encoding != NULL) {
				free(encoding);
	...
	while (!flf && (fn && *fn)) {
		flf = FlocaleGetFontOrFontSet(dpy, fn, encoding, fontname,
		    module);
		if (fn != NULL && fn != mb_fallback_font &&
		    fn != fallback_font) {
			free(fn);
	...
	if (flf == NULL) {
		...
		if (Flocale != NULL) {
			...
			if ((flf = FlocaleGetFontSet(dpy, mb_fallback_font,
				 NULL, module)) != NULL) {
				flf->name = mb_fallback_font;
			}
		}
		if (flf == NULL) {
			...
			if ((flf = FlocaleGetFont(dpy, fallback_font, NULL,
				 module)) != NULL) {
				flf->name = fallback_font;
			} else if (!ask_default) {
				fvwm_debug(__func__,
	...
	if (flf != NULL) {
		...
		if (flf->str_fc == FlocaleCharsetGetUnknownCharset()) {
			flf->str_fc = FlocaleCharsetGetDefaultCharset(dpy,
			    module);
		}
...
void
FlocaleUnloadFont(Display *dpy, FlocaleFont *flf)
{
	...
	if (flf->name != NULL && !StrEquals(flf->name, mb_fallback_font) &&
...
void
FlocaleInitGstpArgs(flocale_gstp_args *args, FlocaleFont *flf,
    FlocaleWinString *fws, int start_x, int start_y)
{
	args->step	   = 0;
...
Bool
FlocaleGetShadowTextPosition(int *x, int *y, flocale_gstp_args *args)
{
	if (args->step == 0) {
	...
	if (args->direction == MULTI_DIR_C) {
		...
		args->inter_step = (tx + args->outer_offset) +
		...
		*y = args->orig_y + ty;
		...
		gravity_rotate_xy(args->rot, args->x_sign, args->y_sign,
...
void
FlocaleDrawString(Display *dpy, FlocaleFont *flf, FlocaleWinString *fws,
    unsigned long flags)
{
	int		    len;
	Bool		    do_free = False;
	...
	FlocaleEncodeWinString(dpy, flf, fws, &do_free, &len, &comb_chars,
	    NULL);
	curr_str = fws->e_str;
	for (char_len = 0, i = 0; i < len && curr_str[i] != 0;
	     char_len++, i += curr_len) {
	...
	if (comb_chars != NULL &&
		...
		for (i = 0, curr_pixel_pos = 0; i < char_len;
		     i++, curr_str += curr_len) {
	...
	if (fws->flags.text_rotation != ROTATION_0 &&
		...
		FlocaleRotateDrawString(dpy, flf, fws, fg, fgsh, has_fg_pixels,
		    len, comb_chars, pixel_pos);
	} else if (FftSupport && flf->fftf.fftfont[0] != NULL) {
		FftDrawString(dpy, flf, fws, fg, fgsh, has_fg_pixels, len,
		    flags);
	} else if (flf->fontset != None) {
		int xt = fws->x;
		int yt = fws->y;
		...
		xt = gstp_args.orig_x;
		yt = gstp_args.orig_y;
		XmbDrawString(dpy, fws->win, flf->fontset, fws->gc, xt, yt,
		    fws->e_str, len);
	} else if (flf->font != None) {
		FlocaleFontStructDrawString(dpy, flf, fws->win, fws->gc, fws->x,
		    fws->y, fg, fgsh, has_fg_pixels, fws, len, False);
	}
	...
	if (comb_chars != NULL) {
		while (
			...
			curr_len = FlocaleChar2bOneCharToUtf8(comb_chars[i].c,
			    buf);
			buf2	 = FiconvUtf8ToCharset(dpy, flf->str_fc,
				(const char *)buf, curr_len);
			if (buf2 == NULL) {
				/* if conversion failed, combinational char
				   is not representable in current charset */
			...
			if (FftSupport && flf->fftf.fftfont[0] != NULL) {
				...
				FftDrawString(dpy, flf, &tmp_fws, fg, fgsh,
				...
				if (flf->shadow_size != 0) {
					XSetForeground(dpy, fws->gc, fgsh);
					while (FlocaleGetShadowTextPosition(&xt,
					    &yt, &gstp_args)) {
						XmbDrawString(dpy, fws->win,
						    flf->fontset, fws->gc, xt,
						    yt, buf2, strlen(buf2));
					}
				}
				XSetForeground(dpy, fws->gc, fg);
				...
				FlocaleFontStructDrawString(dpy, flf, fws->win,
				    fws->gc, fws->x + offset, fws->y, fg, fgsh,
				    has_fg_pixels, &tmp_fws, out_len, False);
			}
...
void
FlocaleDrawUnderline(Display *dpy, FlocaleFont *flf, FlocaleWinString *fws,
    int offset)
{
	...
	FlocaleEncodeWinString(dpy, flf, fws, &do_free, &len, NULL, &l_to_v);
	...
	off1 = FlocaleTextWidth(flf, fws->e_str, voffset) +
	    ((voffset == 0) ? FLF_SHADOW_LEFT_SIZE(flf) :
...
int
FlocaleTextWidth(FlocaleFont *flf, char *str, int sl)
{
	int		    result = 0;
	char		   *tmp_str;
	int		    new_l, do_free;
	...
	if (sl == 1) {
		...
		tmp_str = FlocaleEncodeString(Pdpy, flf, str, &do_free, sl,
		    &new_l, NULL, &comb_chars, NULL);
	}
	...
	if (strlen(tmp_str) == 0 && comb_chars &&
		...
		if (FLC_ENCODING_TYPE_IS_UTF_8(flf->fc) || flf->flags.is_mb) {
			...
			if (FLC_ENCODING_TYPE_IS_UTF_8(flf->fc))
				str2b = FlocaleUtf8ToUnicodeStr2b(tmp_str,
				    new_l, &nl);
			else
				str2b = FlocaleStringToString2b(Pdpy, flf,
				    tmp_str, new_l, &nl);
			if (str2b != NULL) {
...
int
FlocaleGetMinOffset(FlocaleFont *flf, rotation_t rotation)
{
	int min_offset;
 ...
 */
void
FlocaleGetNameProperty(Status(func)(Display *, Window, XTextProperty *),
    Display *dpy, Window w, FlocaleNameString *ret_name)
{
	char	    **list;
	int	      num;
...
void
FlocaleFreeNameProperty(FlocaleNameString *ptext)
{
...
Bool
FlocaleTextListToTextProperty(Display *dpy, char **list, int count,
    XICCEncodingStyle style, XTextProperty *text_prop_return)
{
	int ret = False;

	if (Flocale != NULL) {
		ret = XmbTextListToTextProperty(dpy, list, count, style,
		    text_prop_return);
		if (ret == XNoMemory) {
			ret = False;
		} else {
	...
	if (!ret) {
		if (XStringListToTextProperty(list, count, text_prop_return) ==
		    0) {
			ret = False;
		} else {
			ret = True;
 ...
 */
void
FlocalePrintLocaleInfo(Display *dpy, int verbose)
{
	FlocaleFont    *flf   = FlocaleFontList;
	int		count = 0;
	FlocaleCharset *cs;
	...
	fflush(stderr);
	fflush(stdout);
	fvwm_debug(__func__, "fvwm info on locale:\n");
	fvwm_debug(__func__, "  locale: %s, Modifier: %s\n",
	    (Flocale) ? Flocale : "", (Fmodifiers) ? Fmodifiers : "");
	cs = FlocaleCharsetGetDefaultCharset(dpy, NULL);
	fvwm_debug(__func__, "  Default Charset:  X: %s, Iconv: %s, Bidi: %s\n",
	    cs->x,
	    (cs->iconv_index >= 0) ? cs->locale[cs->iconv_index] :
				     "Not defined",
	    (cs->bidi) ? "Yes" : "No");
	...
	fvwm_debug(__func__, "  Number of loaded font: %i\n", count);
	if (verbose) {
		count = 0;
		flf   = FlocaleFontList;
		while (flf) {
			cs = flf->fc;
			fvwm_debug(__func__, "  * Font number %i\n", count);
			fvwm_debug(__func__, "    fvwm info:\n");
			fvwm_debug(__func__, "      Name: %s\n",
			    (flf->name) ? flf->name : "");
			fvwm_debug(__func__, "      Cache count: %i\n",
			    flf->count);
			fvwm_debug(__func__, "      Type: ");
			if (flf->font) {
				fvwm_debug(__func__, "FontStruct\n");
			} else if (flf->fontset) {
				fvwm_debug(__func__, "FontSet\n");
			} else {
				fvwm_debug(__func__, "XftFont\n");
			}
			fvwm_debug(__func__,
			    "      Charset:  X: %s, Iconv: %s, "
			    "Bidi: %s\n",
			    cs->x,
			    (cs->iconv_index >= 0) ?
				cs->locale[cs->iconv_index] :
				"Not defined",
			    (cs->bidi) ? "Yes" : "No");
			fvwm_debug(__func__,
			    "      height: %i, ascent: %i, "
			    "descent: %i\n",
			    flf->height, flf->ascent, flf->descent);
			fvwm_debug(__func__,
			    "      shadow size: %i, "
			...
			if (verbose >= 2) {
				if (flf->fftf.fftfont[0] != NULL) {
					...
					fvwm_debug(__func__, "    X info:\n");
					n = XFontsOfFontSet(flf->fontset,
