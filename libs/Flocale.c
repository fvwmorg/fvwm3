    ...
    int *do_free, int *len, superimpose_char_t **comb_chars, int **l_to_v)
{
	...
	fws->e_str = FlocaleEncodeString(
	    dpy, flf, fws->str, do_free, *len, len, NULL, comb_chars, l_to_v);
	fws->str2b = NULL;

	if (flf->font != None) {
		if (FLC_ENCODING_TYPE_IS_UTF_8(flf->fc)) {
			fws->str2b =
			    FlocaleUtf8ToUnicodeStr2b(fws->e_str, *len, &len2b);
		} else if (flf->flags.is_mb) {
    ...
    FlocaleWinString *fws, int len, Bool image)
{
	...
	if (image) {
		...
		FSwitchDrawImageString(
		    is_string16, dpy, d, gc, x, y, fws->e_str, fws->str2b, len);
	} else {
    ...
    superimpose_char_t *comb_chars, int *pixel_pos)
{
	...
	if ((image = XCreateImage(dpy, Pvisual, 1, XYBitmap, 0,
		 (char *)normal_data, normal_w, normal_h, 8, 0))
	    == NULL) {
		return;
	}
	...
	if ((rotated_image = XCreateImage(dpy, Pvisual, 1, XYBitmap, 0,
		 (char *)rotated_data, rotated_w, rotated_h, 8, 0))
	    == NULL) {
		return;
	}
	...
	for (j = 0; j < rotated_h; j++) {
		for (i = 0; i < rotated_w; i++) {
			...
			if (fws->flags.text_rotation == ROTATION_270)
				...
				val =
				    normal_data[(normal_h - j - 1) * normal_len
						+ (normal_w - i - 1) / 8]
				    & (128 >> ((normal_w - i - 1) % 8));

			else /* ROTATION_90 */
				val =
				    normal_data[(normal_h - i - 1) * normal_len
						+ j / 8]
				    & (128 >> (j % 8));
...
FlocaleGetFontOrFontSet(
    Display *dpy, char *fontname, char *encoding, char *fullname, char *module)
{
	FlocaleFont *flf = NULL;
...
void
FlocaleInit(
    int category, const char *locale, const char *modifiers, const char *module)
{
	...
	if (flf == NULL) {
		...
		if (Flocale != NULL) {
			...
			if ((flf = FlocaleGetFontSet(
				 dpy, mb_fallback_font, NULL, module))
			    != NULL) {
				flf->name = mb_fallback_font;
			}
		}
...
void
FlocaleDrawString(
    Display *dpy, FlocaleFont *flf, FlocaleWinString *fws, unsigned long flags)
{
	int		    len;
	Bool		    do_free = False;
	Pixel		    fg = 0, fgsh = 0;
	...
	if (comb_chars != NULL
		...
		/* the second condition is actually redundant,
		   ...
		   string */
		pixel_pos =
		    fxmalloc((char_len != 0 ? char_len : 1) * sizeof(int));
    ...
    Display *dpy, FlocaleFont *flf, FlocaleWinString *fws, int offset)
{
	...
	FlocaleEncodeWinString(
	    dpy, flf, fws, &do_free, &len, &comb_chars, &l_to_v);
