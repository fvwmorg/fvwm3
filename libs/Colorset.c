...
LoadColorset(char *line)
{
	...
	if (sscanf(line,
		...
		&do_dither_icon, &icon_tint_percent, &icon_alpha_percent)
	    != 20)
		return -1;
    ...
    unsigned int depth, GC gc, Bool clear_area)
{
	...
	if (FHaveShapeExtension && colorset->shape_mask) {
		...
		if (mask != None) {
			FShapeCombineMask(
			    dpy, win, FShapeBounding, 0, 0, mask, FShapeSet);
    ...
    colorset_t *colorset, unsigned int depth, GC gc, Bool clear_area)
{
	if (!CSETS_IS_TRANSPARENT(colorset)) {
		...
		SetWindowBackgroundWithOffset(
		    dpy, win, 0, 0, width, height, colorset, depth, gc, True);
	} else {
    ...
    Bool is_shape_mask)
{
	...
	if (colorset->pixmap == ParentRelative && !is_shape_mask
		...
		XTranslateCoordinates(
		    dpy, win, DefaultRootWindow(dpy), x, y, &sx, &sy, &dummy);
		...
		if (is_bad_gc == 0 && CSETS_IS_TRANSPARENT_ROOT_PURE(colorset)
		    && colorset->tint_percent > 0) {
			FvwmRenderAttributes fra;
	...
	if (cs_pixmap == None) {
		...
		if (pixmap && big_pixmap) {
			XCopyArea(dpy, big_pixmap, pixmap, gc, x, y, width,
			    height, 0, 0);
		}
		if (big_pixmap) {
    ...
    colorset_t *colorset, unsigned int depth, GC gc, Bool is_shape_mask)
{
	return CreateOffsetBackgroundPixmap(
	    dpy, win, 0, 0, width, height, colorset, depth, gc, is_shape_mask);
}
    ...
    GC gc)
{
	...
	if (!colorset->pixmap) {
		...
		if (stretch_x || stretch_y) {
			...
			pixmap2 =
			    CreateTiledPixmap(dpy, pixmap, colorset->width,
				colorset->height, width, height, depth, gc);
			XFreePixmap(dpy, pixmap);
			pixmap = pixmap2;
		}
