...
static int
get_aspect_dimensions(int *ret_w, int *ret_h, int dest_w, int dest_h, int src_w,
    int src_h)
{
	double ax;
	double ay;
...
AllocColorset(int n)
{
	...
	Colorset = fxrealloc((void *)Colorset, ++n, sizeof(colorset_t));
...
DumpColorset(int n, colorset_t *cs)
{
	snprintf(csetbuf, sizeof(csetbuf),
	    ...
	    cs->icon_tint, cs->pixmap, cs->shape_mask, cs->fg_alpha_percent,
	    cs->width, cs->height, cs->pixmap_type, cs->shape_width,
	    cs->shape_height, cs->shape_type, cs->tint_percent,
	    cs->do_dither_icon, cs->icon_tint_percent, cs->icon_alpha_percent);
	return csetbuf;
}
...
LoadColorset(char *line)
{
	...
	if (sscanf(line,
		...
		&shape_mask, &fg_alpha_percent, &width, &height, &pixmap_type,
		&shape_width, &shape_height, &shape_type, &tint_percent,
		&do_dither_icon, &icon_tint_percent, &icon_alpha_percent) != 20)
		return -1;
    ...
    int height, unsigned int depth)
{
	...
	XCopyGC(dpy, gc,
	    GCFunction | GCPlaneMask | GCSubwindowMode | GCClipXOrigin |
    ...
    unsigned int depth, GC gc, Bool clear_area)
{
	...
	if (0 == width || 0 == height) {
		if (!XGetGeometry(dpy, win, &XID_int.junk, &XID_int.i_junk,
			&XID_int.i_junk, (unsigned int *)&width,
			(unsigned int *)&height, &XID_int.ui_junk,
			&XID_int.ui_junk)) {
			return;
		}
	}
	...
	if (FHaveShapeExtension && colorset->shape_mask) {
		mask = CreateBackgroundPixmap(dpy, None, width, height,
		    colorset, 1, None, True);
		if (mask != None) {
			FShapeCombineMask(dpy, win, FShapeBounding, 0, 0, mask,
			    FShapeSet);
	...
	if (!colorset->pixmap) {
		...
		pixmap = CreateOffsetBackgroundPixmap(dpy, win, x_off, y_off,
	...
	if (cs_t->pixmap == None) {
		...
		*h = cs_t->height;
		...
		case PIXMAP_STRETCH_ASPECT:
			get_aspect_dimensions(w, h, width, height, cs_t->width,
			    cs_t->height);
			break;
    ...
    Bool is_shape_mask)
{
	...
	if (colorset->pixmap == ParentRelative && !is_shape_mask &&
		...
		MyXGrabServer(dpy);
		pixmap = PGraphicsCreateTransparency(dpy, win, &fra, gc, x, y,
		    width, height, True);
		MyXUngrabServer(dpy);
		if (pixmap == None) {
		...
		if (CSETS_IS_TRANSPARENT_ROOT_PURE(colorset)) {
			...
			if (!XGetGeometry(dpy, colorset->pixmap, &XID_int.junk,
		...
		fill_gc		 = fvwmlib_XCreateGC(dpy, win,
		...
		if (is_bad_gc == 0 &&
			...
			fra.tint	 = colorset->tint;
			fra.tint_percent = colorset->tint_percent;
			PGraphicsRenderPixmaps(dpy, win, pixmap, None, None,
			    Pdepth, &fra, pixmap, fill_gc, None, None, 0, 0,
	...
	if (!is_shape_mask) {
		...
		cs_width       = colorset->width;
		cs_height      = colorset->height;
		cs_keep_aspect = (colorset->pixmap_type ==
		...
		if (shape_gc == None) {
			...
			shape_gc = fvwmlib_XCreateGC(dpy, win,
	...
	if (cs_pixmap == None) {
		...
		trim_side  = get_aspect_dimensions(&big_width, &big_height,
		     width, height, cs_width, cs_height);
		big_pixmap = CreateStretchPixmap(dpy, cs_pixmap, cs_width,
		    cs_height, depth, big_width, big_height, gc);
		...
		if (pixmap && big_pixmap) {
			XCopyArea(dpy, big_pixmap, pixmap, gc, px, py, width,
			    height, 0, 0);
		}
		if (big_pixmap) {
		...
		if (!is_shape_mask) {
			pixmap = XCreatePixmap(dpy, cs_pixmap, cs_width,
			    cs_height, depth);
			if (pixmap) {
				XCopyArea(dpy, cs_pixmap, pixmap, gc, 0, 0,
				    cs_width, cs_height, 0, 0);
			}
		} else {
			/* can't tile masks, create a tiled version of the
			 * mask */
			pixmap = CreateTiledPixmap(dpy, cs_pixmap, cs_width,
    ...
    GC gc)
{
	...
	if (CSETS_IS_TRANSPARENT_PR_TINT(colorset)) {
		...
		PGraphicsTintRectangle(dpy, win, colorset->tint,
		    colorset->tint_percent, win, True, gc, None, None, dest_x,
	...
	if (FHaveShapeExtension && colorset->shape_mask != None) {
		clipmask = CreateBackgroundPixmap(dpy, 0, width, height,
		    colorset, 1, None, True);
		if (clipmask) {
			...
			clip_gc		   = fvwmlib_XCreateGC(dpy, win,
				       GCClipXOrigin | GCClipYOrigin | GCClipMask, &xgcv);
			draw_gc		   = clip_gc;
		}
	}
	...
	if (!colorset->pixmap) {
		...
		pixmap = CreateBackgroundPixmap(dpy, win, width, height,
		    colorset, depth, gc, False);
		if (keep_aspect) {
			/* nothing to do */
		}
		if (stretch_x || stretch_y) {
			if (!stretch_x && colorset->width != width) {
				pixmap2 = CreateStretchXPixmap(dpy, pixmap,
				    colorset->width, height, depth, width, gc);
				XFreePixmap(dpy, pixmap);
				pixmap = pixmap2;
			}
			if (!stretch_y && colorset->height != height) {
				pixmap2 = CreateStretchYPixmap(dpy, pixmap,
				    width, colorset->height, depth, height, gc);
				XFreePixmap(dpy, pixmap);
				pixmap = pixmap2;
			}
		} else {
			pixmap2 = CreateTiledPixmap(dpy, pixmap,
			    colorset->width, colorset->height, width, height,
			    depth, gc);
			XFreePixmap(dpy, pixmap);
			pixmap = pixmap2;
		}
		...
		if (pixmap) {
			...
			XCopyArea(dpy, pixmap, win, draw_gc, dest_x - x,
			    dest_y - y, dest_w, dest_h, dest_x, dest_y);
			XFreePixmap(dpy, pixmap);
		}
	}
