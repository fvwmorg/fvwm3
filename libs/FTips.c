...
__initialize_window(Display *dpy)
{
	...
	win = XCreateWindow(dpy, DefaultRootWindow(dpy), 0, 0, 1, 1,
	    current_config->border_width, Pdepth, InputOutput, Pvisual,
	    valuemask, &attributes);

	_net_um_window_type = XInternAtom(dpy, "_NET_UM_WINDOW_TYPE", False);
	_net_um_window_type_tooltips = XInternAtom(dpy,
...
__map_window(Display *dpy)
{
	...
	FScreenGetScrRect(fsarg, FSCREEN_GLOBAL, &screen_g.x, &screen_g.y,
	    &screen_g.width, &screen_g.height);

	new_g.height =
	    ((current_config->Ffont) ? current_config->Ffont->height : 0) + 1;
	new_g.width = 4;

	if (label && current_config->Ffont) {
		new_g.width += FlocaleTextWidth(current_config->Ffont, label,
		    strlen(label));
	}
	...
	XTranslateCoordinates(dpy, win_for, DefaultRootWindow(dpy), x, y,
	...
	if (placement == FTIPS_PLACEMENT_RIGHT ||
		...
		if (new_g.x < 2) {
			...
			x = box.x - current_config->placement_offset -
			    new_g.width - (2 * current_config->border_width) -
			    1;
			XTranslateCoordinates(dpy, win_for,
			    DefaultRootWindow(dpy), x, y, &x1, &y1, &dummy);
			/* */
...
FTipsNewConfig(void)
{
	...
	fc = fxcalloc(1, sizeof(ftips_config));
