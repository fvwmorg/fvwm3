...
FRenderCreateShadePicture(Display *dpy, Window win, int alpha_percent)
{
	...
	if (shade_picture
		...
		FRenderFillRectangle(
		    dpy, FRenderPictOpSrc, shade_picture, &frc, 0, 0, 1, 1);
    ...
    FRenderPicture dest_picture, int dest_x, int dest_y, int dest_w, int dest_h)
{
	...
	if (tint_picture
		...
		FRenderFillRectangle(
		    dpy, FRenderPictOpSrc, tint_picture, &frc_tint, 0, 0, 1, 1);
		saved_tint	   = tint;
    ...
    Bool do_repeat)
{
	...
	if (Pdepth != depth && pixmap) {
		pixmap_copy = PictureBitmapToPixmap(
		    dpy, win, pixmap, Pdepth, gc, src_x, src_y, src_w, src_h);
		src_x = src_y = 0;
	}
	...
	if (added_alpha_percent >= 100) {
		if (alpha != None) {
			 ...
			 */
			if (!(shade_picture =
				    FRenderCreateShadePicture(dpy, win, 100))) {
				goto bail;
			}
			alpha_x = alpha_y = 0;
		}
	...
	dest_picture =
	    FRenderCreatePicture(dpy, d, PFrenderVisualFormat, 0, &pa);
