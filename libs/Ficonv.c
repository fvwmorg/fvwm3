...
static Bool
is_iconv_supported(char *c1, char *c2)
{
	Ficonv_t cd1, cd2;
	Bool	 r = False;
...
static int
is_translit_supported(char *c1, char *c2)
{
	Ficonv_t cd;
	char	*fname;
...
static int
set_default_iconv_charsets(FlocaleCharset *fc)
{
	int i = 0, j = 0;
...
static void
set_iconv_charset_index(FlocaleCharset *fc)
{
	int i = 0;
	...
	while (FLC_GET_LOCALE_CHARSET(fc, i) != NULL) {
		if (is_iconv_supported(FLC_GET_ICONV_CHARSET(
					   FLCIconvUtf8Charset),
			FLC_GET_LOCALE_CHARSET(fc, i))) {
			FLC_SET_ICONV_INDEX(fc, i);
			if (is_translit_supported(FLC_GET_ICONV_CHARSET(
						      FLCIconvUtf8Charset),
				FLC_GET_LOCALE_CHARSET(fc, i))) {
				FLC_SET_ICONV_TRANSLIT_CHARSET(fc,
				    fxstrdup(translit_csname(
					FLC_GET_LOCALE_CHARSET(fc, i))));
			} else {
				FLC_SET_ICONV_TRANSLIT_CHARSET(fc,
				    FLC_TRANSLIT_NOT_SUPPORTED);
...
static char *
convert_charsets(const char *in_charset, const char *out_charset,
    const char *in, unsigned int in_size)
{
	...
	insize		   = len;
	outp = dest = fxmalloc(outbuf_size);
	...
	for (is_finished = 0; is_finished == 0;) {
		...
		nconv = Ficonv(cd, (ICONV_ARG_CONST char **)&inptr, &insize,
...
static void
FiconvInit(Display *dpy, const char *module)
{
	int suc = False;
...
FiconvSetupConversion(Display *dpy, FlocaleCharset *fc)
{
...
char *
FiconvUtf8ToCharset(Display *dpy, FlocaleCharset *fc, const char *in,
    unsigned int in_size)
{
	...
	fvwm_debug(__func__, "[FiconvUtf8ToCharset] conversion from %s to %s\n",
	    FLC_DEBUG_GET_ICONV_CHARSET(FLCIconvUtf8Charset),
	    FLC_DEBUG_GET_ICONV_CHARSET(my_fc));
#endif
	...
	if (FLC_ENCODING_TYPE_IS_UTF_8(my_fc)) {
		...
		out = convert_charsets(FLC_GET_ICONV_CHARSET(
					   FLCIconvUtf8Charset),
		    to_cs, in, in_size);
	}
...
char *
FiconvCharsetToUtf8(Display *dpy, FlocaleCharset *fc, const char *in,
    unsigned int in_size)
{
	char	       *out   = NULL;
	FlocaleCharset *my_fc = NULL;
...
#if FLOCALE_DEBUG_ICONV
	fvwm_debug(__func__, "[FiconvCharsetToUtf8] conversion from %s to %s\n",
	    FLC_DEBUG_GET_ICONV_CHARSET(my_fc),
	    FLC_DEBUG_GET_ICONV_CHARSET(FLCIconvUtf8Charset));
#endif
	...
	if (FLC_ENCODING_TYPE_IS_UTF_8(my_fc)) {
		...
		out = convert_charsets(FLC_GET_ICONV_CHARSET(my_fc),
		    FLC_GET_ICONV_CHARSET(FLCIconvUtf8Charset), in, in_size);
	}
	return out;
}
    ...
    FlocaleCharset *out_fc, const char *in, unsigned int in_size)
{
	...
	if (!FLC_ENCODING_TYPE_IS_UTF_8(my_in_fc)) {
		tmp = FiconvCharsetToUtf8(dpy, my_in_fc, (const char *)in,
		    in_size);
		if (tmp != NULL) {
			free_tmp = True;
			tmp_len	 = strlen(tmp);
	...
	if (!FLC_ENCODING_TYPE_IS_UTF_8(my_out_fc)) {
		out = FiconvUtf8ToCharset(dpy, my_out_fc, (const char *)tmp,
		    tmp_len);
		if (free_tmp) {
			free(tmp);
		}
