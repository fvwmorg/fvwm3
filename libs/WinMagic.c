    ...
    Colormap colormap, Window **ret_children)
{
	...
	for (i = 0, j = 0; i < nchildren; i++) {
		if (XGetWindowAttributes(dpy, children[i], &cxwa) && cxwa.x == 0
		    && cxwa.y == 0 && cxwa.width == pxwa.width
