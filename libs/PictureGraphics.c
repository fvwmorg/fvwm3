...
/* ---------------------------- local functions ---------------------------- */
static int
FSetBackingStore(Display *dpy, Window win, int backing_store)
{
...
static void
PCopyArea(Display *dpy, Pixmap pixmap, Pixmap mask, int depth, Drawable d,
    GC gc, int src_x, int src_y, int src_w, int src_h, int dest_x, int dest_y)
{
	XGCValues     gcv;
	unsigned long gcm;
	GC	      my_gc = None;
	...
	if (depth == Pdepth) {
		...
		XCopyArea(dpy, pixmap, d, (my_gc != None) ? my_gc : gc, src_x,
		...
		XCopyPlane(dpy, pixmap, d, (my_gc != None) ? my_gc : gc, src_x,
		    src_y, src_w, src_h, dest_x, dest_y, 1);
	}
	if (my_gc != None) {
		XFreeGC(dpy, my_gc);
	} else {
...
static void
PTileRectangle(Display *dpy, Window win, Pixmap pixmap, Pixmap mask, int depth,
    int src_x, int src_y, Drawable d, GC gc, GC mono_gc, int dest_x, int dest_y,
    int dest_w, int dest_h)
{
	...
	if (mask != None) {
		...
		XFillRectangle(dpy, tile_mask,
		    (mono_gc != None) ? mono_gc : my_mono_gc, src_x, src_y,
		    dest_w, dest_h);
		if (mono_gc != None) {
	...
	if (depth != Pdepth) {
		...
		XChangeGC(dpy, (mono_gc != None) ? mono_gc : my_mono_gc, gcm,
		    &gcv);
		my_pixmap = XCreatePixmap(dpy, win, dest_w, dest_h, 1);
		XFillRectangle(dpy, my_pixmap,
		    (mono_gc != None) ? mono_gc : my_mono_gc, 0, 0, dest_w,
		    dest_h);
		...
		XChangeGC(dpy, (mono_gc != None) ? mono_gc : my_mono_gc, gcm,
		    &gcv);
		XCopyPlane(dpy, my_pixmap, d, (my_gc != None) ? my_gc : gc, 0,
		    0, dest_w, dest_h, dest_x, dest_y, 1);
		if (my_pixmap != None) {
			XFreePixmap(dpy, my_pixmap);
		}
	} else {
		XChangeGC(dpy, (gc != None) ? gc : my_gc, gcm, &gcv);
		XFillRectangle(dpy, d, (gc != None) ? gc : my_gc, dest_x,
...
static void
PGrabImageErrorHandler(void)
{
	PGrabImageError = True;
}

static FImage *
PGrabXImage(Display *dpy, Drawable d, int x, int y, int w, int h,
    Bool d_is_a_window)
{
...
static Pixmap
PCreateRenderPixmap(Display *dpy, Window win, Pixmap pixmap, Pixmap mask,
    Pixmap alpha, int depth, int added_alpha_percent, Pixel tint,
    int tint_percent, Bool d_is_a_window, Drawable d, GC gc, GC mono_gc,
    GC alpha_gc, int src_x, int src_y, int src_w, int src_h, int dest_x,
    int dest_y, int dest_w, int dest_h, Bool do_repeat, int *new_w, int *new_h,
    Bool *new_do_repeat, Pixmap *new_mask)
{
	...
	*new_do_repeat = do_repeat;
	...
	if (depth != Pdepth) {
		...
		if (pixmap_copy && gc) {
			XCopyPlane(dpy, pixmap, pixmap_copy, gc, src_x, src_y,
			    src_w, src_h, 0, 0, 1);
		} else {
			error = True;
			goto bail;
	...
	if (src_pix == ParentRelative) {
		pixmap_fim = PGrabXImage(dpy, d, dest_x, dest_y, dest_w, dest_h,
		    d_is_a_window);
	} else {
		pixmap_fim = FGetFImage(dpy, src_pix, Pvisual, Pdepth, src_x,
	...
	if (mask != None) {
		mask_fim = FGetFImage(dpy, mask, Pvisual, 1, src_x, src_y,
		    src_w, src_h, AllPlanes, ZPixmap);
		if (!mask_fim) {
			error = True;
			goto bail;
		}
	...
	if (alpha != None) {
		alpha_fim = FGetFImage(dpy, alpha, Pvisual,
		    FRenderGetAlphaDepth(), src_x, src_y, src_w, src_h,
		    AllPlanes, ZPixmap);
		if (!alpha_fim) {
			error = True;
			goto bail;
	...
	if (alpha != None || added_alpha_percent < 100) {
		dest_fim = PGrabXImage(dpy, d, dest_x, dest_y, dest_w, dest_h,
		    d_is_a_window);
		/* accept this error */
	}
	...
	*new_h = h;
	...
	out_fim = FCreateFImage(dpy, Pvisual, Pdepth, ZPixmap, w, h);
	if (gc == None) {
		gc = PictureDefaultGC(dpy, win);
	}
	...
	colors = fxmalloc(n_src_w * n_src_h * sizeof(XColor));
	if (dest_fim) {
		dest_colors = fxmalloc(w * h * sizeof(XColor));
	}
	am = fxmalloc(n_src_w * n_src_h * sizeof(unsigned short));
	...
	if (make_new_mask) {
		*new_mask = XCreatePixmap(dpy, win, w, h, 1);
		if (*new_mask) {
			new_mask_fim = FCreateFImage(dpy, Pvisual, 1, ZPixmap,
			    w, h);
			if (!new_mask_fim) {
				error = True;
				goto bail;
			...
			if (mono_gc == None) {
				mono_gc = fvwmlib_XCreateGC(dpy, *new_mask, 0,
				    NULL);
		...
		for (i = 0; i < n_src_w; i++, m++) {
			if (am[m] > 0) {
				...
				if (dest_fim) {
					...
					c.pixel = XGetPixel(pixmap_fim->im, i,
					    j);
				}
				if (*new_mask) {
	...
	if (*new_mask && mono_gc) {
		FPutFImage(dpy, *new_mask, mono_gc, new_mask_fim, 0, 0, 0, 0, w,
		    h);
	}
...
static Pixmap
PCreateDitherPixmap(Display *dpy, Window win, Drawable src, Pixmap mask,
    int depth, GC gc, int in_width, int in_height, int out_width,
    int out_height)
{
	FImage	      *src_fim;
	FImage	      *mask_fim = NULL;
	...
	if (!(src_fim = FGetFImage(dpy, src, Pvisual, depth, 0, 0, in_width,
		  in_height, AllPlanes, ZPixmap))) {
		return None;
	}
	if (mask != None) {
		mask_fim = FGetFImage(dpy, mask, Pvisual, 1, 0, 0, in_width,
		    in_height, AllPlanes, ZPixmap);
		if (!mask_fim) {
			FDestroyFImage(dpy, mask_fim);
			return None;
	...
	out_fim = FCreateFImage(dpy, Pvisual, Pdepth, ZPixmap, out_width,
	    out_height);
	if (gc == None) {
		gc = PictureDefaultGC(dpy, win);
	}
	...
	colors = fxmalloc(out_width * out_height * sizeof(XColor));
	cm     = fxmalloc(out_width * out_height * sizeof(char));
	...
	for (j = 0; j < out_height; j++, y++) {
		...
		for (i = 0; i < out_width; i++, x++) {
			...
			if (mask_fim != NULL &&
				...
				cm[m++]		  = 255;
				colors[k++].pixel = XGetPixel(src_fim->im, x,
				    y);
			}
		}
	}
	...
	for (j = 0; j < out_height; j++) {
		for (i = 0; i < out_width; i++) {
			...
			if (cm[m] > 0) {
				c = colors[k++];
				PictureAllocColorAllProp(Pdpy, Pcmap, &c, i, j,
				    False, False, True);
			} else {
	...
	FPutFImage(dpy, out_pix, gc, out_fim, 0, 0, 0, 0, out_width,
	    out_height);
	FDestroyFImage(dpy, out_fim);
...
Pixmap
PictureBitmapToPixmap(Display *dpy, Window win, Pixmap src, int depth, GC gc,
    int src_x, int src_y, int src_w, int src_h)
{
	...
	if (dest && gc) {
		XCopyPlane(dpy, src, dest, gc, src_x, src_y, src_w, src_h, 0, 0,
		    1);
	}
    ...
    int dest_x, int dest_y, int dest_w, int dest_h, int do_repeat)
{
	...
	if (t_fra.tint_percent > 0 || t_fra.added_alpha_percent < 100 ||
		...
		if (FRenderRender(dpy, win, pixmap, mask, alpha, depth,
			t_fra.added_alpha_percent, t_fra.tint,
			t_fra.tint_percent, d, gc, alpha_gc, src_x, src_y,
			src_w, src_h, dest_x, dest_y, dest_w, dest_h,
			do_repeat)) {
			return;
		}
	}
	...
	if (t_fra.tint_percent > 0 || t_fra.added_alpha_percent < 100 ||
		...
		xrs_pixmap = PCreateRenderPixmap(dpy, win, pixmap, mask, alpha,
		    depth, t_fra.added_alpha_percent, t_fra.tint,
		    t_fra.tint_percent, d_is_a_window, d, gc, mono_gc, alpha_gc,
		    src_x, src_y, src_w, src_h, dest_x, dest_y, dest_w, dest_h,
		    do_repeat, &new_w, &new_h, &new_do_repeat, &xrs_mask);
		if (xrs_pixmap) {
			src_x	  = 0;
			src_y	  = 0;
			src_w	  = new_w;
	...
	if (do_repeat) {
		PTileRectangle(dpy, win, tmp_pixmap, tmp_mask, depth, src_x,
		    src_y, d, gc, mono_gc, dest_x, dest_y, dest_w, dest_h);
	} else {
		PCopyArea(dpy, tmp_pixmap, tmp_mask, depth, d, gc, src_x, src_y,
...
{
	PGraphicsRenderPixmaps(dpy, win, p->picture, p->mask, p->alpha,
	    p->depth, fra, d, gc, mono_gc, alpha_gc, src_x, src_y, src_w, src_h,
	    dest_x, dest_y, dest_w, dest_h, do_repeat);
}

void
PGraphicsCopyPixmaps(Display *dpy, Pixmap pixmap, Pixmap mask, Pixmap alpha,
    int depth, Drawable d, GC gc, int src_x, int src_y, int src_w, int src_h,
    int dest_x, int dest_y)
{
	PGraphicsRenderPixmaps(dpy, None, pixmap, mask, alpha, depth, 0, d, gc,
	    None, None, src_x, src_y, src_w, src_h, dest_x, dest_y, src_w,
	    src_h, False);
}
...
PGraphicsCopyFvwmPicture(Display *dpy, FvwmPicture *p, Drawable d, GC gc,
    int src_x, int src_y, int src_w, int src_h, int dest_x, int dest_y)
{
	PGraphicsRenderPicture(dpy, None, p, 0, d, gc, None, None, src_x, src_y,
	    src_w, src_h, dest_x, dest_y, src_w, src_h, False);
}

void
PGraphicsTileRectangle(Display *dpy, Window win, Pixmap pixmap, Pixmap mask,
    Pixmap alpha, int depth, Drawable d, GC gc, GC mono_gc, int src_x,
    int src_y, int src_w, int src_h, int dest_x, int dest_y, int dest_w,
    int dest_h)
{
	PGraphicsRenderPixmaps(dpy, win, pixmap, mask, alpha, depth, 0, d, gc,
	    mono_gc, None, src_x, src_y, dest_w, dest_h, dest_x, dest_y, dest_w,
	    dest_h, True);
}
    ...
    int dest_width, int dest_height, GC gc, GC mono_gc, GC alpha_gc)
{
	...
	pixmap = CreateStretchPixmap(dpy, src->picture, src->width, src->height,
	    src->depth, dest_width, dest_height, gc);
	if (!pixmap) {
		return NULL;
	}
	if (src->mask) {
		mask = CreateStretchPixmap(dpy, src->mask, src->width,
		    src->height, 1, dest_width, dest_height, mono_gc);
	}
	if (src->alpha) {
		alpha = CreateStretchPixmap(dpy, src->alpha, src->width,
		    src->height, FRenderGetAlphaDepth(), dest_width,
    ...
    int dest_width, int dest_height, GC gc, GC mono_gc, GC alpha_gc)
{
	...
	pixmap = CreateTiledPixmap(dpy, src->picture, src->width, src->height,
	    dest_width, dest_height, src->depth, gc);
	if (!pixmap) {
		return NULL;
	}
	if (src->mask) {
		mask = CreateTiledPixmap(dpy, src->mask, src->width,
		    src->height, dest_width, dest_height, 1, mono_gc);
	}
	if (src->alpha) {
		alpha = CreateTiledPixmap(dpy, src->alpha, src->width,
		    src->height, dest_width, dest_height,
...
Pixmap
PGraphicsCreateTransparency(Display *dpy, Window win, FvwmRenderAttributes *fra,
    GC gc, int x, int y, int width, int height, Bool parent_relative)
{
	...
	if (parent_relative) {
		...
		if (!XGetGeometry(dpy, win, &root, (int *)&junk, (int *)&junk,
	...
	if (XRenderSupport && FRenderGetExtensionSupported()) {
		...
		if (FRenderRender(dpy, win, ParentRelative, None, None, Pdepth,
			100, fra->tint, fra->tint_percent, r, gc, None, gx, gy,
			gw, gh, 0, 0, gw, gh, False)) {
			goto bail;
		}
		XFreePixmap(dpy, r);
	}
	r = PCreateRenderPixmap(dpy, win, ParentRelative, None, None, Pdepth,
	    100, fra->tint, fra->tint_percent, True, win, gc, None, None, gx,
	    gy, gw, gh, gx, gy, gw, gh, False, &dummy, &dummy, &dummy, &dp);
...
void
PGraphicsTintRectangle(Display *dpy, Window win, Pixel tint, int tint_percent,
    ...
    int dest_x, int dest_y, int dest_w, int dest_h)
{
	...
	if (FRenderRender(dpy, win, ParentRelative, None, None, Pdepth, 100,
		tint, tint_percent, win, gc, None, dest_x, dest_y, dest_w,
		dest_h, dest_x, dest_y, dest_w, dest_h, False)) {
	...
	if (dest_is_a_window) {
		...
		p = PGraphicsCreateTransparency(dpy, dest, &fra, gc, dest_x,
		    dest_y, dest_w, dest_h, False);
		if (p) {
			XCopyArea(dpy, p, dest, gc, 0, 0, dest_w, dest_h,
			    dest_x, dest_y);
			XFreePixmap(dpy, p);
		}
	}
...
Pixmap
PGraphicsCreateTranslucent(Display *dpy, Window win, FvwmRenderAttributes *fra,
    GC gc, int x, int y, int width, int height)
{
	...
	}
	{
		...
		MyXGrabServer(dpy);
		XCopyArea(dpy, DefaultRootWindow(dpy), root_pix, my_gc, gx, gy,
		    gw, gh, 0, 0);
		MyXUngrabServer(dpy);
		XFreeGC(dpy, my_gc);
	}
	if (XRenderSupport && FRenderGetExtensionSupported()) {
		r = XCreatePixmap(dpy, win, gw, gh, Pdepth);
		if (FRenderRender(dpy, win, root_pix, None, None, Pdepth,
			t_fra.added_alpha_percent, t_fra.tint,
			t_fra.tint_percent, r, gc, None, 0, 0, gw, gh, 0, 0, gw,
			gh, False)) {
			goto bail;
		}
		XFreePixmap(dpy, r);
	...
	r = PCreateRenderPixmap(dpy, win, root_pix, None, None, Pdepth, 100,
	    fra->tint, fra->tint_percent, True, win, gc, None, None, 0, 0, gw,
	    gh, gx, gy, gw, gh, False, &dummy, &dummy, &dummy, &dp);
...
Pixmap
PGraphicsCreateDitherPixmap(Display *dpy, Window win, Drawable src, Pixmap mask,
    int depth, GC gc, int in_width, int in_height, int out_width,
    int out_height)
{
	return PCreateDitherPixmap(dpy, win, src, mask, depth, gc, in_width,
