    ...
    int dest_w, int dest_h)
{
	...
	if (depth != Pdepth) {
		...
		XChangeGC(
		    dpy, (mono_gc != None) ? mono_gc : my_mono_gc, gcm, &gcv);
    ...
    Display *dpy, Drawable d, int x, int y, int w, int h, Bool d_is_a_window)
{
	...
	if (try_to_grab) {
		fim = FGetFImage(
		    dpy, d, Pvisual, Pdepth, x, y, w, h, AllPlanes, ZPixmap);
		if (PGrabImageError) {
#if 0
    ...
    Bool *new_do_repeat, Pixmap *new_mask)
{
	...
	*new_do_repeat = do_repeat;
	...
	if (alpha != None) {
		alpha_fim =
		    FGetFImage(dpy, alpha, Pvisual, FRenderGetAlphaDepth(),
			src_x, src_y, src_w, src_h, AllPlanes, ZPixmap);
		if (!alpha_fim) {
			error = True;
			goto bail;
		}
	...
	*new_h = h;
	...
	if (make_new_mask) {
		*new_mask = XCreatePixmap(dpy, win, w, h, 1);
		if (*new_mask) {
			...
			if (mono_gc == None) {
				mono_gc =
				    fvwmlib_XCreateGC(dpy, *new_mask, 0, NULL);
	...
	if (*new_mask && mono_gc) {
		FPutFImage(
		    dpy, *new_mask, mono_gc, new_mask_fim, 0, 0, 0, 0, w, h);
	}
    ...
    int out_height)
{
	...
	out_fim =
	    FCreateFImage(dpy, Pvisual, Pdepth, ZPixmap, out_width, out_height);
	if (gc == None) {
		gc = PictureDefaultGC(dpy, win);
	}
		...
		for (i = 0; i < out_width; i++) {
			...
			if (cm[m] > 0) {
				...
				PictureAllocColorAllProp(
				    Pdpy, Pcmap, &c, i, j, False, False, True);
			} else {
    ...
    int src_x, int src_y, int src_w, int src_h)
{
	...
	if (dest && gc) {
		XCopyPlane(
		    dpy, src, dest, gc, src_x, src_y, src_w, src_h, 0, 0, 1);
	}
    ...
    int dest_x, int dest_y, int dest_w, int dest_h)
{
	...
	if (dest_is_a_window) {
		...
		p		 = PGraphicsCreateTransparency(
			   dpy, dest, &fra, gc, dest_x, dest_y, dest_w, dest_h, False);
		if (p) {
