...
static int
fvwmrect_do_intervals_intersect(int x1, int width1, int x2, int width2)
{
...
int
fvwmrect_do_rectangles_intersect(rectangle *r, rectangle *s)
{
	if (r->x + r->width <= s->x) {
		return 0;
	}
 ...
 * diff_g. */
void
fvwmrect_subtract_rectangles(rectangle *rdiff, rectangle *r1, rectangle *r2)
{
	rdiff->x      = r1->x - r2->x;
	rdiff->y      = r1->y - r2->y;
...
int
fvwmrect_move_into_rectangle(rectangle *move_rec, rectangle *target_rec)
{
	int has_changed = 0;

	if (!fvwmrect_do_intervals_intersect(move_rec->x, move_rec->width,
	...
	if (!fvwmrect_do_intervals_intersect(move_rec->y, move_rec->height,
...
int
fvwmrect_intersect_xrectangles(XRectangle *r1, XRectangle *r2)
{
