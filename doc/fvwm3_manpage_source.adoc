...
Fvwm / Mwm / Win, BorderWidth, RoundedCorners, SlightlyRoundedCorners,
...
equal to 6 pixels.
+
_SlightlyRoundedCorners_ is analogous but the radii
are equal to 3 pixels.
+
...
_HandleWidth_, _CornerLength_, _RoundedCorners_, _RoundedCornersTop_,
_RoundedCornersBottom_, _SlightlyRoundedCorners_, _!Icon_ / _Icon_, _MiniIcon_,
...
If using _RoundedCorners_ and its relatives simultaneously, please
take care that _CornerLength_ value is not less that all the radii.
+
_RoundedCorners_ takes one to four numeric arguments which are the radii,
in pixels, of circle quarters at the corners. If less than four arguments
are supplied, they are reasonably repeated. _RoundedCorners_ with
no arguments means the default which is 6 pixels. There is also
_SlightlyRoundedCorners_ that sets the radii to 3 pixels.
+
