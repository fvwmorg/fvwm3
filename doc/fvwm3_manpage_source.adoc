...
*Style* _stylename_ _options_ ...::
	The *Style* command is used to set attributes of a window.
+
_stylename_ can be a window's name, class, visible name, or resource
string. It may contain the wildcards '{asterisk}' and '?', which are matched
according to standard globbing rules, although unlike globs, these are the
only supported wildcard modifiers.
+
Multiple options in a single *Style* command are read from left to right
as if they were issued one after each other in separate commands. A given
style always overrides all conflicting styles that have been issued earlier
(or further left on the same style line).
+
Note: windows that have no name (WM_NAME) are given a name of
_Untitled_, and windows that do not have a class (WM_CLASS, res_class)
are given class _NoClass_ and those that do not have a resource
(WM_CLASS, res_name) are given resource _NoResource_.
+
If a window has the resource _fvwmstyle_ set, the value of that
...
Options affecting a window's appearance:

[%autowidth.stretch]
|====
|_Option_ |_Description_
|BorderWidth|Sets the width of the rendered border, in pixels.  A value of _0_
disables the border.
|HandleWidth|Sets the width of the rendered handles, in pixels.  _Handles_
refers to the corners of a window.  A value of _0_ disables the handles.
|CornerLength|Sets the size of the handles on a window.
|BorderColorset|Takes eight positive integers and applies the _colorsets_ to
those regions of a window's border.  The ordering is:
+
....
