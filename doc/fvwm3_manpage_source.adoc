...
These are controlled by *FvwmEvent*.
...
+
The _ToolTip_ flag, followed by a text argument, sets the text of a hover
tool tip shown over the window border while it is in the given
_state_. _!ToolTip_ clears any tip text previously set for that state.
+

....
BorderStyle Active -- Tip "Drag to resize"
....
...
+
The _ToolTip_ flag, followed by a text argument, sets the text of a hover
tool tip shown while the pointer rests over this button or title bar in the
given _state_. _!ToolTip_ clears any tip text previously set for that _state_.
+

....
ButtonStyle 2 Active -- Tip "Close"
TitleStyle Active -- Tip "%n"
....

+
As with any other text argument containing spaces, quote the _Tip_
text (e.g. with double quotes) to keep it as one argument.
+
...
customizable tool bar), *FvwmPrompt* (to execute fvwm commands
