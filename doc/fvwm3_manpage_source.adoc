...
*Test* [(_test-conditions_)] _command_:::
	...
	pattern_, _StrMatch string1 string2_, _EdgeHasPointer direction_,
	_EdgeIsActive direction_, _Start_, _Init_, _Restart_, _Exit_, _Quit_,
	_ToRestart_, _True_, _False_, _F_, _R_, _W_, _X_ and _I_. A test-condition
...
+
The _StrMatch_ _string1_ _string2_ test-condition is true if the
two strings are equal ignoring case. This is useful to test if
a variable expansion is equal to a certain value, for example to
test if the current monitor's name is "DP-2" use:
+
