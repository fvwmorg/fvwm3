...
*FvwmPager: MonitorLabels::
  Tells FvwmPager to display a row of monitor labels below each desktop
  label. The monitor label on the desktop the monitor is currently viewing
  will be highlighted using the hilight color. Clicking on a monitor
  label will move that monitor to the selected desktop. This option is
  best used with 'DesktopConfiguration shared' to be able to control
  which monitor is moved to a selected desktop. Note that if this option
  is on, the desktop and monitor labels will always be shown, even if
  'Font' is set to 'none' (which falls back to the default font).

*FvwmPager: NoMonitorLabels::
  This turns off the row of monitor labels. Useful to negate
  '*FvwmPager: MonitorLabels'.
