...
*FvwmEvent: Cmd command::
  This determines the default _fvwm_ command (if any) that is to be
  called with the event parameters.
+
_FvwmEvent_ uses the value of the _Cmd_ configuration option, appends
any configured event handling action (details below), and passes the
resulting command to _fvwm_ for execution when it receives an event.
For example, with the the following configuration:
+
....
...
....
+
_fvwm_ would execute the command
+
....
        Exec /rsynth/say window closed
....
+
when a window is destroyed.
+
You can also use _fvwm_'s builtin _Echo_ command as _*FvwmEvent: Cmd_ to
...
....
	...
	*FvwmEvent: Cmd
	...
	# execute external programs only
	*FvwmEvent: Cmd Exec

	# execute fvwm user functions only
	*FvwmEvent: Cmd Function
....

*FvwmEvent: PassArgs::
*FvwmEvent: PassId::
  Specifies that _FvwmEvent_ should append additional arguments to the
  event action at end of the command line. For details see the event table
...
For most events this is the
  windowID of the window that the event refers to, passed as a
  hexadecimal string preceded by 0x.
+
Some events do not get any arguments, like _new_page_, and some events
get special arguments:
+
_new_desk_: gets the new desk number as a decimal number
+
_monitor_changed_, _monitor_disabled_, _monitor_enabled_,
_monitor_focus_: get two arguments, the decimal monitor number and the
monitor name
