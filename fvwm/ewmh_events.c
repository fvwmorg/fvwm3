    ...
    FvwmWindow *fw, XEvent *ev, window_style *style, unsigned long any)
{
	...
	if (move) {
		execute_function_override_window(NULL, NULL, "Move", 0, fw);
	} else {
		execute_function_override_window(NULL, NULL, "Resize", 0, fw);
	}
    ...
    FvwmWindow *fw, XEvent *ev, window_style *style, unsigned long any)
{
	...
	if (ev != NULL) {
		...
		if ((IS_EWMH_FULLSCREEN(fw) && !DO_EWMH_USE_STACKING_HINTS(fw))
...
ewmh_WMStrut(FvwmWindow *fw, XEvent *ev, window_style *style, unsigned long any)
{
	...
	val = ewmh_AtomGetByName(
	    FW_W(fw), "_NET_WM_STRUT", EWMH_ATOM_LIST_PROPERTY_NOTIFY, &size);
