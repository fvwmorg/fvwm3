...
#define SHOW_DEFAULT                                               \
	(SHOW_GEOMETRY | SHOW_ALLDESKS | SHOW_NORMAL | SHOW_ICONIC \
	    | SHOW_STICKY_ACROSS_PAGES | SHOW_STICKY_ACROSS_DESKS)

static char *
get_desk_title(int desk, unsigned long flags, Bool is_top_title)
{
	char *		desk_name;
	char *		tlabel;
	...
	if (desk_name != NULL) {
		...
		sprintf(tlabel, _("Desk: %d%s"), desk,
		    (is_top_title && (flags & SHOW_GEOMETRY)) ? _("\tGeometry")
							      : "");
	}
...
static int
visibleCompare(const FvwmWindow **a, const FvwmWindow **b)
{
	return strcasecmp((*a)->visible_name, (*b)->visible_name);
}

static int
iconCompare(const FvwmWindow **a, const FvwmWindow **b)
{
...
static int
classCompare(const FvwmWindow **a, const FvwmWindow **b)
{
...
static int
resourceCompare(const FvwmWindow **a, const FvwmWindow **b)
{
	int result = strcasecmp((*a)->class.res_class, (*b)->class.res_class);
	if (result) {
		return result;
...
static int
classIconCompare(const FvwmWindow **a, const FvwmWindow **b)
{
	int result = strcasecmp((*a)->class.res_class, (*b)->class.res_class);
	if (result) {
		return result;
...
static int
resourceIconCompare(const FvwmWindow **a, const FvwmWindow **b)
{
	int result = strcasecmp((*a)->class.res_class, (*b)->class.res_class);
	if (result) {
		return result;
...
static int
compareReverse(const FvwmWindow **a, const FvwmWindow **b)
{
	return -compare(a, b);
}
...
void CMD_WindowList(F_CMD_ARGS)
{
	...
	if (action && *action) {
		...
		opts = get_menu_options(action, w, fw, NULL, NULL, NULL, &mops);
		...
		while (opts && *opts) {
			...
			if (StrEquals(tok, "NoHotkeys")) {
				...
				fvwm_debug(
				    __func__, "UseListSkip is deprecated. "
					      "Please use \"UseSkipList\".\n");
				skiplist_mode = 1;
				...
				/* deprecated as of 02-May-2007 (SS) */
				fvwm_debug(
				    __func__, "OnlyListSkip is deprecated. "
					      "Please use \"OnlySkipList\".\n");
				skiplist_mode = 2;
	...
	if (flags & (SHOW_ALPHABETIC | SORT_BYCLASS | SORT_BYRESOURCE)) {
		...
		switch (flags
			& (SHOW_ALPHABETIC | SHOW_ICONNAME | SORT_BYCLASS
			    | SORT_BYRESOURCE)) {
	...
	while (next_desk != INT_MAX) {
		...
		for (ii = 0; ii < numWindows; ii++) {
			...
			if (first_desk && flags & TITLE_FOR_ALL_DESKS) {
				...
				AddToMenu(
				    mr, tlabel, "TITLE", False, False, False);
				free(tlabel);
			}
			first_desk = False;
			...
			if (flags & SHOW_INFONOTGEO) {
				...
				dheight =
				    t->g.frame.height - b.total_size.height;
				dwidth = t->g.frame.width - b.total_size.width;
	...
	if (mret.rc == MENU_DOUBLE_CLICKED && default_action
	    && *default_action) {
