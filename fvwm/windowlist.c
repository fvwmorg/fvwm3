...
CMD_WindowList(F_CMD_ARGS)
{
	...
	while (next_desk != INT_MAX) {
		...
		for (ii = 0; ii < numWindows; ii++) {
			...
			xasprintf(&tfunc, "WindowId %lu %s %lu", FW_W(t), ffunc,
			    FW_W(t));
	...
	if (sor_keyname &&
		...
		MST_SELECT_ON_RELEASE_KEY(mr) = XKeysymToKeycode(dpy,
		    FvwmStringToKeysym(dpy, sor_keyname));
	}
	...
	mp.flags.is_triggered_by_keypress = (!default_action &&
