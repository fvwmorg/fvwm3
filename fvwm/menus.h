...
struct MenuRoot *FollowMenuContinuations(struct MenuRoot *mr,
    struct MenuRoot					**pmrPrior);
...
char *get_menu_options(char *action, Window w, FvwmWindow *fw, XEvent *e,
    struct MenuRoot *mr, struct MenuItem *mi, struct MenuOptions *pops);
Bool  DestroyMenu(struct MenuRoot *mr, Bool do_recreate,
...
void  update_transparent_menu_bg(struct MenuRepaintTransparentParameters *prtm,
     int current_x, int current_y, int step_x, int step_y, int end_x, int end_y);
void  repaint_transparent_menu(struct MenuRepaintTransparentParameters *prtmp,
