...
EWMH_SetVisibleName(FvwmWindow *fw, Bool is_icon_name)
{
	...
	if (is_icon_name) {
		if ((fw->icon_name_count == 0 || !USE_INDEXED_ICON_NAME(fw)) &&
		...
		if ((fw->name_count == 0 || !USE_INDEXED_WINDOW_NAME(fw)) &&
	...
	val = (unsigned char *)FiconvCharsetToUtf8(dpy, fc, tmp_str,
	    strlen(tmp_str));
 ...
 */
int
EWMH_WMIconName(FvwmWindow *fw, XEvent *ev, window_style *style,
    unsigned long any)
{
	int		size = 0;
	char	       *val;
	...
	tmp_str = (char *)FiconvUtf8ToCharset(dpy, fc, (const char *)val, size);
	free(val);
	if (tmp_str == NULL) {
		SET_HAS_EWMH_WM_ICON_NAME(fw, 0);
		return 0;
...
EWMH_SetDesktopNames(struct monitor *m)
{
	...
	for (i = 0; i < nbr; i++) {
		names[i] = (unsigned char *)FiconvCharsetToUtf8(dpy, NULL,
