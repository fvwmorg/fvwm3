...
ewmh_atom ewmh_atom_allowed_actions[] = {
    ...
    ENTRY("_NET_WM_ACTION_STICK", XA_ATOM, ewmh_AllowsYes), {NULL, 0, 0, 0}};
...
void *
ewmh_AtomGetByName(
    Window win, const char *atom_name, ewmh_atom_list_name list, int *size)
{
	ewmh_atom *a;
	void *	   data = NULL;
...
EWMH_SetDesktopGeometry(struct monitor *m)
{
	...
	val[0] =
	    m->virtual_scr.VxMax + m->si->w; // m->virtual_scr.MyDisplayWidth;
	val[1] =
	    m->virtual_scr.VyMax + m->si->h; // m->virtual_scr.MyDisplayHeight;
...
EWMH_GetBaseStrutIntersection(
    struct monitor *m, int x11, int y11, int x12, int y12, Bool use_percent)
{
    ...
    struct monitor *m, int x11, int y11, int x12, int y12, Bool use_percent)
{
	...
	return ewmh_GetStrutIntersection(
	    m, x11, y11, x12, y12, left, right, top, bottom, use_percent);
}
...
ewmh_AllowsFullScreen(
    FvwmWindow *fw, XEvent *ev, window_style *style, unsigned long any)
{
	if (!is_function_allowed(F_MAXIMIZE, NULL, fw, RQORIG_PROGRAM_US, False)
...
EWMH_DLOG(char *msg, ...)
{
	...
	prev_time  = time_val;
	sprintf(buffer, "%.2d:%.2d:%.2d %6ld", t_ptr->tm_hour, t_ptr->tm_min,
	    t_ptr->tm_sec, time_taken);
...
EWMH_fullscreen(FvwmWindow *fw)
{
	...
	sprintf(cmd, "ResizeMoveMaximize %dp %dp +%dp +%dp ewmhiwa",
	    scr_g.width, scr_g.height, scr_g.x - b.top_left.width + page_x,
