...
ewmh_atom ewmh_atom_client_win[] = {
	...
	ENTRY("_NET_WM_STATE", XA_ATOM, ewmh_WMState), {NULL, 0, 0, 0}
};
...
ewmh_atom ewmh_atom_window_type[] = {
	...
	ENTRY("_NET_WM_WINDOW_TYPE_NOTIFICATION", XA_ATOM,
	    ewmh_HandleNotification),
	{NULL, 0, 0, 0}
};
...
ewmh_atom ewmh_atom_fixed_property[] = {
	...
	ENTRY("_NET_WM_WINDOW_TYPE", XA_ATOM, None), {NULL, 0, 0, 0}
};
...
ewmh_atom ewmh_atom_property_notify[] = {
	...
	ENTRY("_NET_WM_STRUT", XA_CARDINAL, ewmh_WMStrut), {NULL, 0, 0, 0}
};
...
ewmh_atom ewmh_atom_fvwm_root[] = {
	...
	ENTRY("_NET_WORKAREA", XA_CARDINAL, None), {NULL, 0, 0, 0}
};
...
ewmh_atom ewmh_atom_fvwm_win[] = {
	...
	ENTRY("_NET_WM_VISIBLE_NAME", None, None), {NULL, 0, 0, 0}
};

#define NUMBER_OF_ATOM_LISTS 9
#define L_ENTRY(x, y)                               \
	{                                           \
		x, y, sizeof(y) / sizeof(ewmh_atom) \
	}
ewmh_atom_list atom_list[] = { L_ENTRY(EWMH_ATOM_LIST_CLIENT_ROOT,
...
EWMH_DLOG(char *msg, ...)
{
	if (EWMH_DEBUG) {
		...
		if (start_time == 0) {
			/* get clock ticks */
			prev_time = start_time = (unsigned int)times(
			    &not_used_tms);
		}
		/* get clock ticks */
...
static ewmh_atom *
get_ewmh_atom_by_name(const char *atom_name, ewmh_atom_list_name list_name)
{
	ewmh_atom *a	= NULL;
	Bool	   done = 0;
	int	   i	= 0;
	...
	while (!done && atom_list[i].name != EWMH_ATOM_LIST_END) {
		if (atom_list[i].name == list_name ||
			...
			a = (ewmh_atom *)bsearch(atom_name, atom_list[i].list,
			    atom_list[i].size - 1, sizeof(ewmh_atom), compare);
...
ewmh_atom *
ewmh_GetEwmhAtomByAtom(Atom atom, ewmh_atom_list_name list_name)
{
	int i = 0;
...
void
ewmh_ChangeProperty(Window w, const char *atom_name, ewmh_atom_list_name list,
...
void
ewmh_DeleteProperty(Window w, const char *atom_name, ewmh_atom_list_name list)
{
	ewmh_atom *a;
...
static void *
atom_get(Window win, Atom to_get, Atom type, int *size)
{
	...
	retval = NULL;
	length = 0x7fffffff;
	ok     = XGetWindowProperty(dpy, win, to_get, 0L, length, False, type,
...
void *
ewmh_AtomGetByName(Window win, const char *atom_name, ewmh_atom_list_name list,
    int *size)
{
	ewmh_atom *a;
	void	  *data = NULL;
...
void
EWMH_SetCurrentDesktop(struct monitor *m)
{
	long val;
	...
	if (val < 0 ||
	    (val >= m->ewmhc.MaxDesktops && m->ewmhc.MaxDesktops != 0)) {
		return;
	}
...
void
EWMH_SetNumberOfDesktops(struct monitor *m)
{
	long val;
	...
	if (m->virtual_scr.CurrentDesk >= m->ewmhc.CurrentNumberOfDesktops &&
		...
		m->ewmhc.CurrentNumberOfDesktops = m->virtual_scr.CurrentDesk +
		    1;
	}
...
void
EWMH_SetDesktopViewPort(struct monitor *m)
{
	...
	ewmh_ChangeProperty(Scr.Root, "_NET_DESKTOP_VIEWPORT",
	    EWMH_ATOM_LIST_CLIENT_ROOT, (unsigned char *)&val, i * 2);

	return;
}

void
EWMH_SetDesktopGeometry(struct monitor *m)
{
	long val[2];

	/* FIXME: needs broadcast for global. */

	val[0] = m->virtual_scr.VxMax +
	    m->si->w; // m->virtual_scr.MyDisplayWidth;
	val[1] = m->virtual_scr.VyMax +
	    m->si->h; // m->virtual_scr.MyDisplayHeight;
	ewmh_ChangeProperty(Scr.Root, "_NET_DESKTOP_GEOMETRY",
	    EWMH_ATOM_LIST_CLIENT_ROOT, (unsigned char *)&val, 2);
...
void
EWMH_SetWMDesktop(FvwmWindow *fw)
{
...
void
EWMH_SetWMState(FvwmWindow *fw, Bool do_restore)
{
	...
	if (i > 0) {
		ewmh_ChangeProperty(FW_W(fw), "_NET_WM_STATE",
		    EWMH_ATOM_LIST_CLIENT_WIN, (unsigned char *)wm_state, i);
	} else {
...
void
ewmh_AddToKdeSysTray(FvwmWindow *fw)
{
	int	 size = 0;
	CARD32	*val;
	KstItem *t;

	val = ewmh_AtomGetByName(FW_W(fw), "_KDE_NET_WM_SYSTEM_TRAY_WINDOW_FOR",
...
void
EWMH_ManageKdeSysTray(Window w, int type)
{
	KstItem *t;
...
void
EWMH_SetClientList(struct monitor *m)
{
	Window	   *wl = NULL;
	FvwmWindow *fw;
	int	    nbr = 0;
...
void
EWMH_SetClientListStacking(struct monitor *m)
{
	Window	   *wl = NULL;
	FvwmWindow *fw;
	int	    nbr = 0;
	...
	for (fw = Scr.FvwmRoot.stack_next; fw != &Scr.FvwmRoot;
	     fw = fw->stack_next) {
		nbr++;
	}
	i = nbr - 1;
...
void
ewmh_SetWorkArea(struct monitor *m)
{
...
void
ewmh_ComputeAndSetWorkArea(struct monitor *m)
{
	...
	if (Scr.bo.do_debug_randr) {
		fvwm_debug(__func__,
		    "monitor '%s': {l: %d, r: %d, t: %d, b: %d} "
		    "{x: %d, y: %d, w: %d, h: %d}\n",
		    m->si->name, left, right, top, bottom, x, y, width, height);
	}

	if (m->Desktops->ewmh_working_area.x != x ||
...
void
ewmh_HandleDynamicWorkArea(struct monitor *m)
{
	...
	if (m->Desktops->ewmh_dyn_working_area.x != x ||
...
void
EWMH_UpdateWorkArea(struct monitor *m)
{
...
void
EWMH_GetWorkAreaIntersection(FvwmWindow *fw, int *x, int *y, int *w, int *h,
    int type)
{
	struct monitor *m = (fw && fw->m) ? fw->m : monitor_get_current();
...
static float
get_intersection(int x11, int y11, int x12, int y12, int x21, int y21, int x22,
    int y22, Bool use_percent)
{
	float ret = 0;
	int   xl, xr, yt, yb;
	...
	if (use_percent && (x22 - x21) * (y22 - y21) != 0 &&
	    (x12 - x11) * (y12 - y11) != 0) {
		ret = 100 *
		    max(ret / ((x22 - x21) * (y22 - y21)),
			ret / ((x12 - x11) * (y12 - y11)));
	}
...
static float
ewmh_GetStrutIntersection(struct monitor *m, int x11, int y11, int x12, int y12,
    int left, int right, int top, int bottom, Bool use_percent)
{
	float ret = 0;
	int   x21, y21, x22, y22;
	...
	ret += get_intersection(x11, y11, x12, y12, x21, y21, x22, y22,
	    use_percent);
	/* right */
...
float
EWMH_GetBaseStrutIntersection(struct monitor *m, int x11, int y11, int x12,
    int y12, Bool use_percent)
{
	return ewmh_GetStrutIntersection(m, x11, y11, x12, y12,
	    m->ewmhc.BaseStrut.left, m->ewmhc.BaseStrut.right,
	    m->ewmhc.BaseStrut.top, m->ewmhc.BaseStrut.bottom, use_percent);
}
    ...
    Bool use_percent)
{
	...
	return ewmh_GetStrutIntersection(m, x11, y11, x12, y12, left, right,
	    top, bottom, use_percent);
}
 ...
 */
void
EWMH_SetFrameStrut(FvwmWindow *fw)
{
	long	     val[4];
	size_borders b;
	...
	ewmh_ChangeProperty(FW_W(fw), "_KDE_NET_WM_FRAME_STRUT",
	    EWMH_ATOM_LIST_FVWM_WIN, (unsigned char *)&val, 4);
	ewmh_ChangeProperty(FW_W(fw), "_NET_FRAME_EXTENTS",
	    EWMH_ATOM_LIST_FVWM_WIN, (unsigned char *)&val, 4);
 ...
 */
Bool
ewmh_AllowsYes(FvwmWindow *fw, XEvent *ev, window_style *style,
    unsigned long any)
{
	return True;
}
...
Bool
ewmh_AllowsClose(FvwmWindow *fw, XEvent *ev, window_style *style,
    unsigned long any)
{
	return is_function_allowed(F_CLOSE, NULL, fw, RQORIG_PROGRAM_US, False);
}

Bool
ewmh_AllowsFullScreen(FvwmWindow *fw, XEvent *ev, window_style *style,
    unsigned long any)
{
	if (!is_function_allowed(F_MAXIMIZE, NULL, fw, RQORIG_PROGRAM_US,
		False) ||
	    !is_function_allowed(F_MOVE, NULL, fw, RQORIG_PROGRAM_US, False) ||
	    !is_function_allowed(F_RESIZE, NULL, fw, RQORIG_PROGRAM_US, True)) {
		return False;
	}
...
Bool
ewmh_AllowsMinimize(FvwmWindow *fw, XEvent *ev, window_style *style,
    unsigned long any)
{
	return is_function_allowed(F_ICONIFY, NULL, fw, RQORIG_PROGRAM_US,
	    False);
}

Bool
ewmh_AllowsMaximize(FvwmWindow *fw, XEvent *ev, window_style *style,
    unsigned long any)
{
	return is_function_allowed(F_MAXIMIZE, NULL, fw, RQORIG_PROGRAM_US,
	    False);
}

Bool
ewmh_AllowsMove(FvwmWindow *fw, XEvent *ev, window_style *style,
    unsigned long any)
{
...
Bool
ewmh_AllowsResize(FvwmWindow *fw, XEvent *ev, window_style *style,
    unsigned long any)
{
	return is_function_allowed(F_RESIZE, NULL, fw, RQORIG_PROGRAM_US,
	    False);
}

void
EWMH_SetAllowedActions(FvwmWindow *fw)
{
	...
	if (i > 0) {
		ewmh_ChangeProperty(FW_W(fw), "_NET_WM_ALLOWED_ACTIONS",
...
int
ewmh_HandleDesktop(FvwmWindow *fw, XEvent *ev, window_style *style,
    unsigned long any)
{
...
int
ewmh_HandleDialog(FvwmWindow *fw, XEvent *ev, window_style *style,
    unsigned long any)
{
...
int
ewmh_HandleDock(FvwmWindow *fw, XEvent *ev, window_style *style,
    unsigned long any)
{
...
int
ewmh_HandleMenu(FvwmWindow *fw, XEvent *ev, window_style *style,
    unsigned long any)
{
...
int
ewmh_HandleNormal(FvwmWindow *fw, XEvent *ev, window_style *style,
    unsigned long any)
{
...
int
ewmh_HandleToolBar(FvwmWindow *fw, XEvent *ev, window_style *style,
    unsigned long any)
{
...
int
ewmh_HandleNotification(FvwmWindow *fw, XEvent *ev, window_style *style,
    unsigned long any)
{
...
void
ewmh_HandleWindowType(FvwmWindow *fw, window_style *style)
{
	CARD32	    *val;
	unsigned int nitems;
	...
	val = ewmh_AtomGetByName(FW_W(fw), "_NET_WM_WINDOW_TYPE",
	    EWMH_ATOM_LIST_FIXED_PROPERTY, &size);
	if (val == NULL) {
		return;
	}
 ...
 */
static int
ksmserver_workarround(FvwmWindow *fw)
{
...
void
EWMH_GetStyle(FvwmWindow *fw, window_style *style)
{
...
static void
ewmh_check_wm_pid(FvwmWindow *fw)
{
	int	size = 0;
	CARD32 *val;

	fw->ewmh_window_type = 0;
	val		     = ewmh_AtomGetByName(FW_W(fw), "_NET_WM_PID",
			     EWMH_ATOM_LIST_FIXED_PROPERTY, &size);
	if (val != NULL) {
		free(val);
...
void
EWMH_RestoreInitialStates(FvwmWindow *fw, int event_type)
{
	...
	if (HAS_EWMH_INIT_WM_DESKTOP(fw) == EWMH_STATE_HAS_HINT) {
		...
		ewmh_DeleteProperty(FW_W(fw), "_NET_WM_DESKTOP",
		    EWMH_ATOM_LIST_CLIENT_WIN);
	}
 ...
 */
static int
set_all_atom_in_list(ewmh_atom *list)
{
	int l = 0;
...
EWMH_Init(struct monitor *m)
{
	...
	ewmh_ChangeProperty(Scr.NoFocusWin, "_NET_WM_NAME",
	    EWMH_ATOM_LIST_PROPERTY_NOTIFY, (unsigned char *)&utf_name, 4);
...
EWMH_fullscreen(FvwmWindow *fw)
{
	...
	if (IS_ICONIFIED(fw)) {
		...
		execute_function_override_window(NULL, NULL, "Iconify off",
		    NULL, 0, fw);
	}
	if (IS_SHADED(fw)) {
		...
		execute_function_override_window(NULL, NULL, "WindowShade off",
		    NULL, 0, fw);
	...
	FScreenGetScrRect(&fscr, FSCREEN_XYPOS, &scr_g.x, &scr_g.y,
	    &scr_g.width, &scr_g.height);
	get_window_borders(fw, &b);
	get_page_offset_check_visible(&page_x, &page_y, fw);
	snprintf(cmd, sizeof(cmd),
	    "ResizeMoveMaximize %dp %dp +%dp +%dp ewmhiwa", scr_g.width,
	    scr_g.height, scr_g.x - b.top_left.width + page_x,
