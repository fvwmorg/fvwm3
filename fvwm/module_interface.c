	...
	*(bp++) = fev_get_evtime();
	...
	for (; num > 0; --num) {
		...
		if (bodylen >= FvwmPacketMaxSize_byte) {
			fvwm_debug(__func__, "packet too long %ld %ld",
			    (long)bodylen, (long)FvwmPacketMaxSize_byte);
			break;
		}
		if (arglen > 0) {
...
__get_allowed_actions(const FvwmWindow *fw)
{
	...
	act.is_movable	   = is_function_allowed(F_MOVE, NULL, fw,
		RQORIG_PROGRAM_US, False);
	act.is_deletable   = is_function_allowed(F_DELETE, NULL, fw,
	      RQORIG_PROGRAM_US, False);
	act.is_destroyable = is_function_allowed(F_DESTROY, NULL, fw,
	    RQORIG_PROGRAM_US, False);
	act.is_closable	   = is_function_allowed(F_CLOSE, NULL, fw,
	       RQORIG_PROGRAM_US, False);
	act.is_maximizable = is_function_allowed(F_MAXIMIZE, NULL, fw,
	    RQORIG_PROGRAM_US, False);
	act.is_resizable   = is_function_allowed(F_RESIZE, NULL, fw,
	      RQORIG_PROGRAM_US, False);
	act.is_iconifiable = is_function_allowed(F_ICONIFY, NULL, fw,
	    RQORIG_PROGRAM_US, False);
...
*/
#define CONFIGARGS(_fw)                                                        \
	35, (unsigned long)(-sizeof(Window)), &FW_W(*(_fw)),                   \
	    (unsigned long)(-sizeof(Window)), &FW_W_FRAME(*(_fw)),             \
	    (unsigned long)(-sizeof(void *)), &(_fw), (unsigned long)(0),      \
	    &(*(_fw))->g.frame.x, (unsigned long)(0), &(*(_fw))->g.frame.y,    \
	    (unsigned long)(0), &(*(_fw))->g.frame.width, (unsigned long)(0),  \
	    &(*(_fw))->g.frame.height, (unsigned long)(0), &(*(_fw))->Desk,    \
	    (unsigned long)(0), &(*(_fw))->m->si->rr_output,                   \
	    (unsigned long)(sizeof(unsigned long)), (*(_fw))->m->si->name,     \
	    (unsigned long)(0), &(*(_fw))->layer, (unsigned long)(0),          \
	    &(*(_fw))->hints.base_width, (unsigned long)(0),                   \
	    &(*(_fw))->hints.base_height, (unsigned long)(0),                  \
	    &(*(_fw))->hints.width_inc, (unsigned long)(0),                    \
	    &(*(_fw))->hints.height_inc, (unsigned long)(0),                   \
	    &(*(_fw))->orig_hints.width_inc, (unsigned long)(0),               \
	    &(*(_fw))->orig_hints.height_inc, (unsigned long)(0),              \
	    &(*(_fw))->hints.min_width, (unsigned long)(0),                    \
	    &(*(_fw))->hints.min_height, (unsigned long)(0),                   \
	    &(*(_fw))->hints.max_width, (unsigned long)(0),                    \
	    &(*(_fw))->hints.max_height, (unsigned long)(-sizeof(Window)),     \
	    &FW_W_ICON_TITLE(*(_fw)), (unsigned long)(-sizeof(Window)),        \
	    &FW_W_ICON_PIXMAP(*(_fw)), (unsigned long)(0),                     \
	    &(*(_fw))->hints.win_gravity, (unsigned long)(-sizeof(Pixel)),     \
	    &(*(_fw))->colors.fore, (unsigned long)(-sizeof(Pixel)),           \
	    &(*(_fw))->colors.back, (unsigned long)(0),                        \
	    &(*(_fw))->ewmh_hint_layer,                                        \
	    (unsigned long)(sizeof(unsigned long)),                            \
	    &(*(_fw))->ewmh_hint_desktop, (unsigned long)(0),                  \
	    &(*(_fw))->ewmh_window_type, (unsigned long)(sizeof(short)),       \
	    &(*(_fw))->title_thickness, (unsigned long)(sizeof(short)),        \
	    &(*(_fw))->boundary_width, (unsigned long)(sizeof(short)), &dummy, \
	    (unsigned long)(sizeof(short)), &dummy,                            \
	    (unsigned long)(sizeof((*(_fw))->flags)), &(*(_fw))->flags,        \
...
static unsigned long *
make_named_packet(int *len, unsigned long event_type, const char *name, int num,
    ...)
{
	unsigned long *body;
	va_list	       ap;
...
void
SendName(fmodule *module, unsigned long event_type, unsigned long data1,
...
void
BroadcastMonitorList(fmodule *this)
{
	char		*name;
	struct monitor	*m;
	fmodule_list_itr moditr;
	...
	while ((module = module_list_itr_next(&moditr)) != NULL) {
		TAILQ_FOREACH (m, &monitor_q, entry) {
			if (m->flags & MONITOR_DISABLED)
				continue;
			xasprintf(&name,
			    "Monitor %s %d %d %d %d %d %d %d %d %d %d %d %d",
			    m->si->name, (int)m->si->rr_output,
...
void
SendFvwmPicture(fmodule *module, unsigned long event_type, unsigned long data1,
    unsigned long data2, unsigned long data3, FvwmPicture *picture, char *name)
{
	unsigned long *body;
	unsigned long  data4 = 0, data5 = 0, data6 = 0, data7 = 0, data8 = 0,
		      data9 = 0;
	int l;
	...
	TAILQ_FOREACH (m, &monitor_q, entry) {
		...
		for (t = Scr.FvwmRoot.next; t != NULL; t = t->next) {
			...
			if (IS_ICONIFIED(t) && !IS_ICON_UNMAPPED(t)) {
				...
				if (rc == True) {
					SendPacket(mod, M_ICONIFY, 7,
					    (long)FW_W(t), (long)FW_W_FRAME(t),
					    (unsigned long)t, (long)r.x,
					    (long)r.y, (long)r.width,
					    (long)r.height);
				}
			}
			if ((IS_ICONIFIED(t)) && (IS_ICON_UNMAPPED(t))) {
		...
		if (Scr.Hilite == NULL) {
			...
			BroadcastPacket(M_FOCUS_CHANGE, 5,
			    (long)FW_W(Scr.Hilite), (long)FW_W(Scr.Hilite),
			    (unsigned long)True,
			    (long)Scr.Hilite->hicolors.fore,
			    (long)Scr.Hilite->hicolors.back);
		}
