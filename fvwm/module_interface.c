...
__get_allowed_actions(const FvwmWindow *fw)
{
	...
	act.is_destroyable =
	    is_function_allowed(F_DESTROY, NULL, fw, RQORIG_PROGRAM_US, False);
	act.is_closable =
	    is_function_allowed(F_CLOSE, NULL, fw, RQORIG_PROGRAM_US, False);
	act.is_maximizable =
	    is_function_allowed(F_MAXIMIZE, NULL, fw, RQORIG_PROGRAM_US, False);
	act.is_resizable =
	...
	while ((module = module_list_itr_next(&moditr)) != NULL) {
		TAILQ_FOREACH(m, &monitor_q, entry)
		{
			...
			xasprintf(&name,
			    ...
			    m->virtual_scr.MyDisplayHeight, m->virtual_scr.Vx,
			    m->virtual_scr.Vy, m->virtual_scr.VxMax,
			    m->virtual_scr.VyMax, m->si->x, m->si->y, m->si->w,
			    m->si->h);
    ...
    unsigned long data2, char *string)
{
	...
	while ((module = module_list_itr_next(&moditr)) != NULL) {
		SendName(
		    module, MX_PROPERTY_CHANGE, argument, data1, data2, string);
	}
...
module_input_execute(struct fmodule_input *input)
{
	...
	memset(&e, 0, sizeof(e));
	if (XFindContext(dpy, input->window, FvwmContext, (caddr_t *)&ecc.w.fw)
	    == XCNOENT) {
		ecc.w.fw      = NULL;
		input->window = None;
	}
	...
	exc = exc_create_context(&ecc, ECC_TYPE | ECC_ETRIGGER | ECC_FW | ECC_W
					   | ECC_WCONTEXT | ECC_MODULE);
