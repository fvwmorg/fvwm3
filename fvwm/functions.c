...
execute_complex_function(cond_rc_t *cond_rc, const exec_context_t *exc,
    char *action, Bool *desperate, Bool has_ref_window_moved);
...
static void
__execute_function(cond_rc_t *cond_rc, const exec_context_t *exc, char *action,
    FUNC_FLAGS_TYPE exec_flags, char *args[], Bool has_ref_window_moved)
{
	...
	if (!(exec_flags & FUNC_DONT_EXPAND_COMMAND)) {
		expaction = expand_vars(taction, arguments,
    ...
    const exec_context_t *exc, char *args[], Bool has_ref_window_moved)
{
	...
	for (fi = func->first_item; fi != NULL && cond_rc->break_levels == 0;) {
		...
		if (c == cond) {
			__execute_function(cond_rc, exc, fi->action,
...
static void
__cf_cleanup(int *depth, char **arguments, cond_rc_t *cond_rc)
{
	int i;
    ...
    char *action, Bool *desperate, Bool has_ref_window_moved)
{
	...
	if (func == NULL) {
		if (*desperate == 0) {
			fvwm_debug(__func__, "No such function %s", action);
		}
		return;
	}
	if (!depth) {
	...
	*desperate = 0;
	...
	if (!GrabEm(CRS_NONE, GRAB_NORMAL)) {
		func->use_depth--;
		fvwm_debug(__func__,
		    "Grab failed in function %s,"
		    " unable to execute immediate action",
		    action);
		__cf_cleanup(&depth, arguments, cond_rc);
		return;
	}
	if (has_immediate) {
		...
		__run_complex_function_items(cond_rc, CF_IMMEDIATE, func, exc2,
		    arguments, has_ref_window_moved);
		exc_destroy_context(exc2);
	}
	for (fi = func->first_item; fi != NULL && cond_rc->break_levels == 0;
	     fi = fi->next_item) {
	...
	if (do_run_late_immediate) {
		...
		__run_complex_function_items(cond_rc, CF_LATE_IMMEDIATE, func,
		    exc2, arguments, has_ref_window_moved);
		exc_destroy_context(exc2);
...
DestroyFunction(FvwmFunction *func)
{
	...
	if (func->use_depth != 0) {
		fvwm_debug(__func__, "Function %s is in use (depth %d)",
...
void
execute_function(cond_rc_t *cond_rc, const exec_context_t *exc, char *action,
...
void
execute_function_override_wcontext(cond_rc_t *cond_rc,
...
void
execute_function_override_window(cond_rc_t *cond_rc, const exec_context_t *exc,
    char *action, FUNC_FLAGS_TYPE exec_flags, FvwmWindow *fw)
{
	const exec_context_t * exc2;
	exec_context_changes_t ecc;
...
void
find_func_t(char *action, short *func_t, unsigned char *flags)
{
	int   j, len = 0;
	char *endtok = action;
	Bool  matched;
