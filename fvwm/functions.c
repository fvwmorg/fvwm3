...
static void execute_complex_function(cond_rc_t *cond_rc,
    const exec_context_t *exc, cmdparser_context_t *pc, FvwmFunction *func,
    ...
    Bool has_ref_window_moved)
{
	 ...
	 * returning */
	rc = cmdparser_hooks->create_context(&pc, caller_pc, xaction,
	...
	}
	{
		...
		if (func != NULL) {
			...
			if (PARSER_DEBUG) {
				fvwm_debug(__func__,
				    "!!! --> find bif '%s' -> %p", func, bif);
			}
			err_func = func;
		} else {
			if (PARSER_DEBUG) {
				cmdparser_hooks->debug(&pc,
				    "!!!E no command name");
			}
			bif	 = NULL;
			err_func = "";
	...
	exec_type = cmdparser_hooks->find_something_to_execute(&pc, &bif,
	    &complex_function);
	if (PARSER_DEBUG) {
	...
	case CP_EXECTYPE_BUILTIN_FUNCTION: {
		...
		if (PARSER_DEBUG) {
			...
			fvwm_debug(__func__, "!!!pc.cline: '%s'", pc.cline);
			fvwm_debug(__func__, "!!!bif: '%s' 0x%x", bif->keyword,
			    bif->flags);
		}
		...
		if ((bif->flags & FUNC_NEEDS_WINDOW) &&
			...
			b = DeferExecution(&ecc, &mask, bif->cursor,
			    exc->x.elast->type,
		...
		if (PARSER_DEBUG) {
			fvwm_debug(__func__, "!!!execute '%s' (fw %p)",
	...
	case CP_EXECTYPE_COMPLEX_FUNCTION: {
		...
		ecc.w.wcontext = exc->w.wcontext;
		exc2	       = exc_clone_context(exc, &ecc, mask);
		execute_complex_function(func_rc, exc2, &pc, complex_function,
    ...
    Bool is_button_pressed, int *ret_button)
{
	...
	while (
	    (total < Scr.ClickTime && fev_get_evtime() - t0 < Scr.ClickTime) ||
	    !may_time_out) {
...
__run_complex_function_items(cond_rc_t *cond_rc, char cond, FvwmFunction *func,
    ...
    char *all_pos_args_string, char *pos_arg_tokens[], int *run_item_count,
    Bool has_ref_window_moved)
{
	char	      c;
	FunctionItem *fi;
	...
	if (!(!has_ref_window_moved && PressedW &&
		XTranslateCoordinates(dpy, PressedW, Scr.Root, 0, 0, &x0, &y0,
		    &JunkChild))) {
		x0 = y0 = 0;
	}
	...
	for (fi = func->first_item; fi != NULL && cond_rc->break_levels == 0;) {
		...
		if (c == cond) {
			...
			if (!has_ref_window_moved && PressedW &&
			    XTranslateCoordinates(dpy, PressedW, Scr.Root, 0, 0,
				&x, &y, &JunkChild)) {
...
static void
__cf_cleanup(FvwmFunction *func, int *depth, int *run_item_count,
    ...
    cmdparser_context_t *pc, FvwmFunction *func, Bool has_ref_window_moved)
{
	...
	if (has_immediate) {
		...
		__run_complex_function_items(cond_rc, CF_IMMEDIATE, func, exc2,
		    pc, all_pos_args_string, pos_arg_tokens, &run_item_count,
		    has_ref_window_moved);
		exc_destroy_context(exc2);
	}
	for (fi = func->first_item; fi != NULL && cond_rc->break_levels == 0;
	...
	if (do_run_late_immediate) {
		...
		__run_complex_function_items(cond_rc, CF_LATE_IMMEDIATE, func,
		    ...
		    &run_item_count, has_ref_window_moved);
		exc_destroy_context(exc2);
...
ungrab_exit:
	func->use_depth--;
	__cf_cleanup(func, &depth, &run_item_count, all_pos_args_string,
...
void
execute_function_override_wcontext(F_CMD_ARGS, func_flags_t exec_flags,
    int wcontext)
{
	const exec_context_t  *exc2;
	exec_context_changes_t ecc;
...
void
execute_function_override_window(F_CMD_ARGS, func_flags_t exec_flags,
    FvwmWindow *fw)
{
	const exec_context_t  *exc2;
	exec_context_changes_t ecc;
	...
	if (exc != NULL) {
		...
		ecc.type = EXCT_NULL;
		exc2	 = exc_create_context(&ecc,
			ECC_TYPE | ECC_FW | ECC_W | ECC_WCONTEXT);
	}
	execute_function(F_PASS_ARGS_WITH_EXC(exc2), exec_flags);
	exc_destroy_context(exc2);
...
AddToFunction(FvwmFunction *func, char *action)
{
	...
	if (func->num_items >= MAX_FUNCTION_ITEMS) {
		fvwm_debug(__func__, "Function too big: '%s' (%d)", func->name,
		    func->num_items);
		return;
	}
...
CMD_EchoFuncDefinition(F_CMD_ARGS)
{
	...
	if (bif != NULL) {
		fvwm_debug(__func__, "function '%s' is a built in command",
		    token);
		free(token);
