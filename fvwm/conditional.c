 ...
 */
static FvwmWindow *
Circulate(FvwmWindow *sf, char *action, int Direction, char **restofline)
{
	int		    pass = 0;
...
static void
circulate_cmd(F_CMD_ARGS, int new_context, int circ_dir, Bool do_use_found,
...
select_cmd(F_CMD_ARGS)
{
	...
	if (MatchesConditionMask(fw, &mask) && restofline) {
		...
		execute_function_override_wcontext(cond_rc, exc, restofline, pc,
		    0, C_WINDOW);
...
static Bool
cond_check_access(char *file, int type, Bool im)
{
	char *full_file;
	char *path = NULL;
...
CreateConditionMask(char *flags, WindowConditionMask *mask)
{
	...
	while (condition) {
		...
		if (StrEquals(cond, "AcceptsFocus")) {
			...
			mask->my_flags.needs_focus = (on) ? NEEDS_TRUE :
							    NEEDS_FALSE;
		} else if (StrEquals(cond, "HasPointer")) {
			mask->my_flags.needs_pointer = (on) ? NEEDS_TRUE :
							      NEEDS_FALSE;
			...
			struct namelist	      *p;
			char		      *condp = fxstrdup(cond);

			pp		     = fxmalloc(sizeof *pp);
...
MatchesConditionMask(FvwmWindow *fw, WindowConditionMask *mask)
{
	...
	/* special treatment for FixedSize, because more than just
	   ...
	   hints etc.) */
	if (IS_SIZE_FIXED(mask) && mask->flag_mask.common.s.is_size_fixed &&
	    is_function_allowed(F_RESIZE, NULL, fw, RQORIG_PROGRAM_US, False)) {
		return False;
	}
	if (!IS_SIZE_FIXED(mask) && mask->flag_mask.common.s.is_size_fixed &&
	    !is_function_allowed(F_RESIZE, NULL, fw, RQORIG_PROGRAM_US,
		False)) {
		return False;
	}
	if (IS_FIXED(mask) && mask->flag_mask.common.s.is_fixed &&
	...
	if (IS_UNCLOSABLE(mask) && mask->flag_mask.common.s.is_unclosable &&
	    (is_function_allowed(F_CLOSE, NULL, fw, RQORIG_PROGRAM_US, False) ||
		is_function_allowed(F_DELETE, NULL, fw, RQORIG_PROGRAM_US,
		    False) ||
	...
	if (!IS_UNCLOSABLE(mask) && mask->flag_mask.common.s.is_unclosable &&
	    (!is_function_allowed(F_CLOSE, NULL, fw, RQORIG_PROGRAM_US,
		 False) &&
		!is_function_allowed(F_DELETE, NULL, fw, RQORIG_PROGRAM_US,
		    False) &&
		!is_function_allowed(F_DESTROY, NULL, fw, RQORIG_PROGRAM_US,
	...
	if (IS_ICONIFIED(fw) && IS_TRANSIENT(fw) &&
	...
	if (mask->my_flags.do_check_desk ||
		...
		is_on_desk = (fw->Desk == m->virtual_scr.CurrentDesk);
	}
	is_on_page = 1;
	if (mask->my_flags.do_check_page ||
	    mask->my_flags.do_check_desk_and_page) {
		if (!mask->my_flags.do_not_check_screen) {
			is_on_page = !!FScreenIsRectangleOnScreen(NULL,
			    FSCREEN_CURRENT, &(fw->g.frame));
		} else {
			is_on_page = !!IsRectangleOnThisPage(m, &(fw->g.frame),
	...
	if (mask->my_flags.do_check_global_page ||
	    mask->my_flags.do_check_desk_and_global_page) {
		is_on_global_page = !!IsRectangleOnThisPage(m, &(fw->g.frame),
		    m->virtual_scr.CurrentDesk);
	}
...
static void
direction_cmd(F_CMD_ARGS, Bool is_scan)
{
	rectangle	    my_g;
	rectangle	    his_g;
	int		    my_cx;
	...
	if (dir == -1 || dir > DIR_ALL_MASK) {
		fvwm_debug(__func__, "Invalid direction %s", (tmp) ? tmp : "");
		if (cond_rc != NULL) {
			cond_rc->rc = COND_RC_ERROR;
		}
		return;
	...
	if (is_scan) {
		...
		if (dir2 == -1 || dir2 > DIR_NW || (dir < 4) != (dir2 < 4) ||
		    (abs(dir - dir2) & 1) != 1) {
	...
	if (fw && is_pointer_relative == False) {
		...
		if (FQueryPointer(dpy, Scr.Root, &JunkRoot, &JunkChild, &my_g.x,
	...
	for (tfw = Scr.FvwmRoot.next; tfw != NULL; tfw = tfw->next) {
		...
		case DIR_NE:
			...
			distance = (dir == DIR_N || dir == DIR_NE) ? -his_cy :
								     his_cy;
			break;
		case DIR_E:  /* E */
		case DIR_SE: /* SE */
		...
		case DIR_NW: /* NW */
			...
			distance = (dir == DIR_W || dir == DIR_NW) ? -his_cx :
								     his_cx;
			break;
		case DIR_C:
			offset	 = 0;
	...
	if (fw_best) {
		...
		execute_function_override_window(cond_rc, exc, restofline, pc,
		    0, fw_best);
	} else if (cond_rc != NULL) {
		cond_rc->rc = COND_RC_NO_MATCH;
	}
...
static int
__rc_matches_rcstring_consume(char **ret_rest, cond_rc_t *cond_rc, char *action)
{
	...
	if (flags == NULL) {
		...
		if (StrEquals(flags, "1") || StrEquals(flags, "match")) {
			match_rc = COND_RC_OK;
		} else if (StrEquals(flags, "0") ||
		    StrEquals(flags, "nomatch")) {
			match_rc = COND_RC_NO_MATCH;
		} else if (StrEquals(flags, "-1") ||
		    StrEquals(flags, "error")) {
			match_rc = COND_RC_ERROR;
		} else if (StrEquals(flags, "-2") ||
		    StrEquals(flags, "break")) {
...
CMD_All(F_CMD_ARGS)
{
	...
	if (do_reverse) {
		for (i = num - 1; i >= 0; i--) {
			execute_function_override_window(cond_rc, exc,
			    restofline, pc, 0, g[i]);
		}
	} else {
		for (i = 0; i < num; i++) {
 ...
 */
void
CMD_Direction(F_CMD_ARGS)
{
	direction_cmd(F_PASS_ARGS, False);
}

void
CMD_ScanForWindow(F_CMD_ARGS)
{
	direction_cmd(F_PASS_ARGS, True);
}

void
CMD_WindowId(F_CMD_ARGS)
{
	FvwmWindow	   *t;
	char		   *token;
	char		   *naction;
	...
	if (token && StrEquals(token, "root")) {
		...
		if (!token ||
		    GetIntegerArguments(token, NULL, &screen, 1) != 1) {
			screen = Scr.screen;
		} else {
			action = naction;
		}
	...
	if (!t) {
		...
		if (use_condition) {
		...
		} else if (XGetGeometry(dpy, win, &JunkRoot, &JunkX, &JunkY,
			...
			if (action != NULL) {
				...
				exc2	       = exc_clone_context(exc, &ecc,
					      ECC_FW | ECC_W | ECC_WCONTEXT);
				execute_function(cond_rc, exc2, action, pc,
				    FUNC_IS_UNMANAGED);
				exc_destroy_context(exc2);
			}
		} else {
...
static Bool
match_version(char *version, char *operator)
{
	static int fvwm_version = -1;
	const int  v		= ver(version);

	if (fvwm_version < 0) {
		char *tmp    = fxstrdup(VERSION);
...
CMD_Test(F_CMD_ARGS)
{
	...
	while (condition) {
		...
		if (StrEquals(cond, "True")) {
			...
			if (pattern) {
				char *ver;
				flags_ptr = GetNextSimpleOption(flags_ptr,
				    &ver);
				if (ver == NULL) {
					match = matchWildcards(pattern,
					    VERSION);
				} else {
			...
			if (var_name) {
				const char *value;
				if ((strlen(var_name) > 10) &&
				    (memcmp(var_name, "infostore.", 10) == 0)) {
					value = get_metainfo_value(
					    var_name + 10);
				} else {
					value = getenv(var_name);
			...
			if (dirname) {
				dir = gravity_parse_dir_argument(dirname, NULL,
				    DIR_NONE);
				if (dir == DIR_NONE) {
