...
MatchesConditionMask(FvwmWindow *fw, WindowConditionMask *mask)
{
	...
	if (IS_UNICONIFIABLE(mask) && mask->flag_mask.common.s.is_uniconifiable
	    && is_function_allowed(
	...
	if (IS_UNMAXIMIZABLE(mask) && mask->flag_mask.common.s.is_unmaximizable
	    && is_function_allowed(
	...
	if (mask->my_flags.do_check_page
		...
		if (FScreenIsEnabled() && !mask->my_flags.do_not_check_screen) {
			...
			is_on_page = !!IsRectangleOnThisPage(
			    m, &(fw->g.frame), m->virtual_scr.CurrentDesk);
		}
	}
	is_on_global_page = 1;
...
direction_cmd(F_CMD_ARGS, Bool is_scan)
{
	...
	for (tfw = Scr.FvwmRoot.next; tfw != NULL; tfw = tfw->next) {
		...
		case DIR_NE:
			...
			distance =
			    (dir == DIR_N || dir == DIR_NE) ? -his_cy : his_cy;
			break;
		case DIR_E:  /* E */
		case DIR_SE: /* SE */
			forward = True;
		...
		case DIR_NW: /* NW */
			...
			distance =
			    (dir == DIR_W || dir == DIR_NW) ? -his_cx : his_cx;
			break;
		case DIR_C:
			offset	 = 0;
			tx	 = (float)his_cx;
	...
	if (!t) {
		...
		if (use_condition) {
			...
			if (action != NULL) {
				...
				execute_function(
				    cond_rc, exc2, action, FUNC_IS_UNMANAGED);
				exc_destroy_context(exc2);
			}
		} else {
...
match_version(char *version, char *operator)
{
	...
	if (v < 0) {
		fvwm_debug(
		    __func__, "match_version: Invalid version: %s\n", version);
		return False;
	}
	...
	while (condition) {
		...
		if (StrEquals(cond, "True")) {
			...
			if (pattern) {
				char *ver;
				flags_ptr =
				    GetNextSimpleOption(flags_ptr, &ver);
				if (ver == NULL) {
					match =
					    matchWildcards(pattern, VERSION);
				} else {
			...
			match =
			    exc->type == EXCT_INIT || exc->type == EXCT_RESTART;
			...
			if (var_name) {
				...
				if (memcmp(var_name, "infostore.", 10) == 0)
					value =
					    get_metainfo_value(var_name + 10);
				else
			...
			if (var_name) {
				 ...
				 */
				flags_ptr =
				    GetNextSimpleOption(flags_ptr, &pattern);
				if (!value) {
					value = "";
				}
				if (pattern) {
