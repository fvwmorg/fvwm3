...
static void
set_ewmhc_desktop_values(struct monitor *, int, int *);
...
void CMD_EwmhNumberOfDesktops(F_CMD_ARGS)
{
	...
	if ((num != 1 && num != 2) || val[0] < 1
		...
		fvwm_debug(__func__, "Bad arguments to EwmhNumberOfDesktops");
		return;
	}
...
void CMD_EwmhBaseStruts(F_CMD_ARGS)
{
	...
	if (GetIntegerArguments(action, NULL, val, 4) != 4 || val[0] < 0
	    || val[1] < 0 || val[2] < 0 || val[3] < 0) {
		fvwm_debug(
		    __func__, "EwmhBaseStruts needs four positive arguments");
		return;
	}
