...
SaveWindowStates(FILE *f)
{
	...
	for (ewin = get_next_window_in_stack_ring(&Scr.FvwmRoot);
		...
		if (!client_id) {
			...
			if (XGetCommand(
				dpy, FW_W(ewin), &wm_command, &wm_command_count)
		...
		if (client_id && window_role) {
			...
			if (wm_command && wm_command_count > 0) {
				fprintf(
				    f, "  [WM_COMMAND] %i", wm_command_count);
		...
		gravity_get_naked_geometry(
		    ewin->hints.win_gravity, ewin, &save_g, &ewin->g.normal);
...
matchWin(FvwmWindow *w, Match *m)
{
	...
	fvwm_debug(__func__, "\twin(%s, %s, %s, %s, %s,", w->class.res_name,
	    w->class.res_class, w->name.name,
	...
	fvwm_debug(__func__, "\tmat(%s, %s, %s, %s, %s,", m->res_name,
	    m->res_class, m->wm_name, (m->client_id) ? m->client_id : "(null)",
...
save_state_file(char *filename)
{
	...
	success = do_preserve_state ? SaveVersionInfo(f) && SaveWindowStates(f)
					  && SaveGlobalState(f)
				    : 1;
...
set_sm_properties(FSmcConn sm_conn, char *filename, char hint)
{
	...
	prop5.vals =
	    (FSmPropValue *)malloc((g_argc + 2) * sizeof(FSmPropValue));
	numVals = 0;
	for (i = 0; i < g_argc; i++) {
	...
	if (filename) {
		...
		prop6.vals =
		    (FSmPropValue *)malloc((g_argc + 6) * sizeof(FSmPropValue));
		...
		if (is_xsm_detected) {
			...
			*/
			char *discardCommand =
			    alloca((10 + strlen(filename)) * sizeof(char));
			...
			prop7.num_vals = 3;
			prop7.vals =
			    (FSmPropValue *)malloc(3 * sizeof(FSmPropValue));
	...
	for (i = 0; i < num_match; i++) {
		if (!matches[i].used && matchWin(ewin, &matches[i])) {
			...
			if (!Restarting) {
				...
				SET_ICON_SUPPRESSED(
				    ewin, IS_ICON_SUPPRESSED(&(matches[i])));
...
SessionInit(void)
{
	...
	sm_conn =
	    FSmcOpenConnection(NULL, &context, FSmProtoMajor, FSmProtoMinor,
...
quit_session(void)
{
	...
	FSmcRequestSaveYourself(sm_conn, FSmSaveLocal, True /* shutdown */,
...
save_session(void)
{
	...
	FSmcRequestSaveYourself(sm_conn, FSmSaveBoth, False /* shutdown */,
	    FSmInteractStyleAny, False /* fast */, True /* global */);
	return True;
...
save_quit_session(void)
{
	...
	FSmcRequestSaveYourself(sm_conn, FSmSaveBoth, True /* shutdown */,
	    FSmInteractStyleAny, False /* fast */, True /* global */);
	return True;
