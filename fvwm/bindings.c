...
static int
ParseBinding(Display *dpy, Binding **pblist, char *tline, binding_t type,
    ...
    unsigned short *buttons_grabbed, Bool is_silent)
{
	...
	if (BIND_IS_KEY_BINDING(type)) {
		...
		if (keysym == 0) {
			if (!is_silent) {
				fvwm_debug(
				    __func__, "No such key: %s", key_string);
			}
			return 0;
		}
	}
	...
	if ((context & C_MENU) == C_MENU) {
		menu_binding(dpy, type, button, keysym, context, modifier,
		    action, window_name);
	...
	*/
	/* BEGIN remove */
	CollectBindingList(dpy, pblist, &rmlist, &are_similar_bindings_left,
	...
	if (rmlist != NULL) {
		...
		bcontext =
		    bind_get_bound_button_contexts(pblist, buttons_grabbed);
		update_nr_buttons(
		    bcontext, nr_left_buttons, nr_right_buttons, True);
	}
	...
	rc = AddBinding(dpy, pblist, type, button, keysym, key_string, modifier,
	    context, (void *)action, NULL, window_name);
...
print_bindings(void)
{
	...
	for (b = Scr.AllBindings; b != NULL; b = b->NextBinding) {
		...
		default:
			fvwm_debug(
			    __func__, "invalid binding type %d", b->type);
			continue;
		}
		...
		}
		{
			...
			context_string =
			    charmap_table_to_string(b->Context, win_contexts);
