...
static int
activate_binding(Binding *binding, binding_t type, Bool do_grab)
{
	FvwmWindow *t;
	Bool	    rc = 0;
	...
	for (t = Scr.FvwmRoot.next; t != NULL; t = t->next) {
		if (!IS_EWMH_DESKTOP(FW_W(t)) &&
			...
			GrabWindowKey(dpy, FW_W_FRAME(t), binding,
			    C_WINDOW | C_DECOR, GetUnusedModifiers(), do_grab);
		}
		if (binding->Context & C_ICON) {
			if (FW_W_ICON_TITLE(t) != None) {
				GrabWindowKeyOrButton(dpy, FW_W_ICON_TITLE(t),
				    binding, C_ICON, GetUnusedModifiers(), None,
				    do_grab);
			}
			if (FW_W_ICON_PIXMAP(t) != None) {
				GrabWindowKeyOrButton(dpy, FW_W_ICON_PIXMAP(t),
				    binding, C_ICON, GetUnusedModifiers(), None,
				    do_grab);
			}
		}
		if (IS_EWMH_DESKTOP(FW_W(t)) &&
			...
			GrabWindowKeyOrButton(dpy, FW_W_PARENT(t), binding,
			    C_EWMH_DESKTOP, GetUnusedModifiers(), None,
			    do_grab);
		}
	}
...
static int
bind_get_bound_button_contexts(Binding **pblist,
...
static void
__rebind_global_key(Binding **pblist, int Button_Key)
{
	Binding *b;
...
static int
ParseBinding(Display *disp, Binding **pblist, char *tline, binding_t type,
    ...
    unsigned short *buttons_grabbed, Bool is_silent)
{
			...
			if (button > NUMBER_OF_MOUSE_BUTTONS) {
				if (!is_silent) {
					fvwm_debug(__func__,
					    ...
					    " this warning, use:\n"
					    "  Silent Mouse %s",
					    button, NUMBER_OF_MOUSE_BUTTONS,
					    tline);
				}
			}
		}
	...
	if (wcontext_string_to_wcontext(context_string, &context) &&
	    !is_silent) {
	...
	if (BIND_IS_KEY_BINDING(type)) {
		...
		if (keysym == 0) {
			if (!is_silent) {
				fvwm_debug(__func__, "No such key: %s",
				    key_string);
			}
			return 0;
		}
	...
	if ((context & C_MENU) == C_MENU) {
		menu_binding(disp, type, button, keysym, context, modifier,
		    action, window_name);
	...
	*/
	/* BEGIN remove */
	CollectBindingList(disp, pblist, &rmlist, &are_similar_bindings_left,
	    type, button, keysym, modifier, context, window_name);
	if (rmlist != NULL) {
		int bcontext;
		...
		bcontext = bind_get_bound_button_contexts(pblist,
		    buttons_grabbed);
	...
	rc = AddBinding(disp, pblist, type, button, keysym, key_string,
	    modifier, context, (void *)action, NULL, window_name);

	return rc;
}

static void
binding_cmd(F_CMD_ARGS, binding_t type)
{
	Binding	      *b;
	int	       count;
	...
	count = ParseBinding(dpy, &Scr.AllBindings, action, type,
	    &Scr.nr_left_buttons, &Scr.nr_right_buttons, &btg,
	    Scr.flags.are_functions_silent);
...
print_bindings(void)
{
	...
	for (b = Scr.AllBindings; b != NULL; b = b->NextBinding) {
		...
		case BIND_BUTTONRELEASE:
			...
			break;
		default:
			fvwm_debug(__func__, "invalid binding type %d",
			    b->type);
			continue;
		}
		...
		}
		{
			...
			mod_string = charmap_table_to_string(MaskUsedModifiers(
								 b->Modifier),
			    key_modifiers);
			context_string = charmap_table_to_string(b->Context,
			    win_contexts);
...
void
CMD_IgnoreModifiers(F_CMD_ARGS)
{
	char *token;
	...
	if (!token) {
		...
		fvwm_debug(__func__, "illegal modifier in line %s\n", action);
	}
