...
const func_t func_table[] = {
    ...
    CMD_ENT(
	"addtitlestyle", CMD_AddTitleStyle, F_ADD_TITLE_STYLE, FUNC_DECOR, 0),
    ...
    CMD_ENT("borderstyle", CMD_BorderStyle, F_BORDERSTYLE, FUNC_DECOR, 0),
    ...
    CMD_ENT("buttonstyle", CMD_ButtonStyle, F_BUTTON_STYLE, FUNC_DECOR, 0),
    /* - Define a window button look (will be reworked) */

    CMD_ENT("changedecor", CMD_ChangeDecor, F_CHANGE_DECOR, FUNC_NEEDS_WINDOW,
	CRS_SELECT),
    /* - Attach decor to a window (will be obsolete) */

    CMD_ENT("changemenustyle", CMD_ChangeMenuStyle, F_CHANGE_MENUSTYLE, 0, 0),
    ...
    CMD_ENT("close", CMD_Close, F_CLOSE, FUNC_NEEDS_WINDOW, CRS_DESTROY),
    ...
    CMD_ENT("defaultcolorset", CMD_DefaultColorset, F_DFLT_COLORSET, 0, 0),
    ...
    CMD_ENT("delete", CMD_Delete, F_DELETE, FUNC_NEEDS_WINDOW, CRS_DESTROY),
    ...
    CMD_ENT("destroy", CMD_Destroy, F_DESTROY, FUNC_NEEDS_WINDOW, CRS_DESTROY),
    ...
    CMD_ENT(
	"destroymenustyle", CMD_DestroyMenuStyle, F_DESTROY_MENUSTYLE, 0, 0),
    /* - Delete menu style defined using MenuStyle */

    CMD_ENT(
	"destroymoduleconfig", CMD_DestroyModuleConfig, F_DESTROY_MOD, 0, 0),
    ...
    CMD_ENT(
	"edgeleavecommand", CMD_EdgeLeaveCommand, F_EDGE_LEAVE_COMMAND, 0, 0),
    /* - Bind one or another screen edge to an fvwm action */
    ...
    CMD_ENT("fakekeypress", CMD_FakeKeypress, F_FAKE_KEYPRESS, 0, 0),
    /* - Send a keyboard event to a window */

    CMD_ENT("flipfocus", CMD_FlipFocus, F_FLIP_FOCUS, FUNC_NEEDS_WINDOW,
	CRS_SELECT),
    /* - Focus a window without rotating windowlist order */

    CMD_ENT("focus", CMD_Focus, F_FOCUS, FUNC_NEEDS_WINDOW, CRS_SELECT),
    ...
    CMD_ENT(
	"hidegeometrywindow", CMD_HideGeometryWindow, F_HIDEGEOMWINDOW, 0, 0),
    ...
    CMD_ENT("iconify", CMD_Iconify, F_ICONIFY, FUNC_NEEDS_WINDOW, CRS_SELECT),
    ...
    CMD_ENT("ignoremodifiers", CMD_IgnoreModifiers, F_IGNORE_MODIFIERS, 0, 0),
    ...
    CMD_ENT("layer", CMD_Layer, F_LAYER, FUNC_NEEDS_WINDOW, CRS_SELECT),
    ...
    CMD_ENT("lower", CMD_Lower, F_LOWER, FUNC_NEEDS_WINDOW, CRS_SELECT),
    /* - Lower a window within a layer */

    CMD_ENT(
	"maximize", CMD_Maximize, F_MAXIMIZE, FUNC_NEEDS_WINDOW, CRS_SELECT),
    ...
    CMD_ENT(
	"modulelistenonly", CMD_ModuleListenOnly, F_MODULE_LISTEN_ONLY, 0, 0),
    /* - Invoke an fvwm module */

    CMD_ENT("modulepath", CMD_ModulePath, F_MODULE_PATH, 0, 0),
    /* - Modify the directories to search for an fvwm module */

    CMD_ENT("modulesynchronous", CMD_ModuleSynchronous, F_MODULE_SYNC, 0, 0),
    ...
    CMD_ENT("move", CMD_Move, F_MOVE, FUNC_NEEDS_WINDOW, CRS_MOVE),
    /* - Move a window */

    CMD_ENT("movethreshold", CMD_MoveThreshold, F_MOVE_THRESHOLD, 0, 0),
    /* - Set number of pixels in a click and a hold vs. a drag */

    CMD_ENT("movetodesk", CMD_MoveToDesk, F_MOVE_TO_DESK, FUNC_NEEDS_WINDOW,
	CRS_SELECT),
    /* - Move a window to another desk same page */

    CMD_ENT("movetopage", CMD_MoveToPage, F_MOVE_TO_PAGE, FUNC_NEEDS_WINDOW,
	CRS_SELECT),
    ...
    CMD_ENT("placeagain", CMD_PlaceAgain, F_PLACEAGAIN, FUNC_NEEDS_WINDOW,
	CRS_SELECT),
    ...
    CMD_ENT("raise", CMD_Raise, F_RAISE, FUNC_NEEDS_WINDOW, CRS_SELECT),
    /* - Raise a window in a layer */

    CMD_ENT("raiselower", CMD_RaiseLower, F_RAISELOWER, FUNC_NEEDS_WINDOW,
	CRS_SELECT),
    ...
    CMD_ENT("refreshwindow", CMD_RefreshWindow, F_REFRESH, FUNC_NEEDS_WINDOW,
	CRS_SELECT),
    ...
    CMD_ENT("resize", CMD_Resize, F_RESIZE, FUNC_NEEDS_WINDOW, CRS_RESIZE),
    ...
    CMD_ENT("resizemove", CMD_ResizeMove, F_RESIZEMOVE, FUNC_NEEDS_WINDOW,
	CRS_RESIZE),
    /* - Resize and move in one operation */

    CMD_ENT("resizemovemaximize", CMD_ResizeMoveMaximize, F_RESIZEMOVE_MAXIMIZE,
	FUNC_NEEDS_WINDOW, CRS_RESIZE),
    ...
    CMD_ENT("savequitsession", CMD_SaveQuitSession, F_SAVE_QUIT_SESSION, 0, 0),
    ...
    CMD_ENT("send_reply", CMD_Send_Reply, F_SEND_REPLY, FUNC_DONT_REPEAT, 0),
    /* - Internal, used for module communication */
    ...
    CMD_ENT(
	"sendtomodule", CMD_SendToModule, F_SEND_STRING, FUNC_DONT_REPEAT, 0),
    ...
    CMD_ENT("state", CMD_State, F_STATE, FUNC_NEEDS_WINDOW, CRS_SELECT),
    /* - Control user defined window states */

    CMD_ENT("status", CMD_Status, F_STATUS, 0, 0),
    /* - Sets if we send status. */

    CMD_ENT("stick", CMD_Stick, F_STICK, FUNC_NEEDS_WINDOW, CRS_SELECT),
	...
	FUNC_NEEDS_WINDOW | FUNC_ALLOW_UNMANAGED, CRS_SELECT),
    ...
    CMD_ENT("windowshade", CMD_WindowShade, F_WINDOW_SHADE, FUNC_NEEDS_WINDOW,
	CRS_SELECT),
    /* - Shade/unshade a window */

    CMD_ENT(
	"windowshadeanimate", CMD_WindowShadeAnimate, F_SHADE_ANIMATE, 0, 0),
    /* - (obsolete, use Style * WindowShadeSteps) */

    CMD_ENT("windowstyle", CMD_WindowStyle, F_WINDOW_STYLE, FUNC_NEEDS_WINDOW,
	CRS_SELECT),
