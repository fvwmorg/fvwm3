 ...
 */
static void
draw_separator(Window w, GC TopGC, GC BottomGC, int x1, int y, int x2)
{
 ...
 */
static void
draw_tear_off_bar(Window w, GC TopGC, GC BottomGC, int x1, int y, int x2)
{
	XGCValues xgcv;
	int	  width;
	int	  offset;
...
static void
draw_highlight_background(struct MenuPaintItemParameters *mpip, int x, int y,
    int width, int height, colorset_t *cs, GC gc)
{
	if (cs->pixmap && cs->pixmap_type != PIXMAP_TILED) {
		Pixmap p;

		p = CreateOffsetBackgroundPixmap(dpy, mpip->w, 0, 0, width,
		    height, cs, Pdepth, gc, False);
		...
		default:
			XCopyArea(dpy, p, mpip->w, gc, 0, 0, width, height, x,
			    y);
			break;
		}
		XFreePixmap(dpy, p);
...
void
menuitem_free(struct MenuItem *mi)
{
	int i;
...
struct MenuItem *
menuitem_clone(struct MenuItem *mi)
{
	MenuItem *new_mi;
	int	  i;

	/* copy everything */
	new_mi = fxmalloc(sizeof(MenuItem));
	...
	if (MI_ACTION(mi) != NULL) {
		MI_ACTION(new_mi) = fxstrdup(MI_ACTION(mi));
	}
	for (i = 0; i < MAX_MENU_ITEM_LABELS; i++) {
		if (MI_LABEL(mi)[i] != NULL) {
			MI_LABEL(new_mi)[i] = fxstrdup(MI_LABEL(mi)[i]);
		}
	}
	...
	for (i = 0; i < MAX_MENU_ITEM_MINI_ICONS; i++) {
		if (MI_MINI_ICON(mi)[i] != NULL) {
			MI_MINI_ICON(new_mi)
			[i] = PCloneFvwmPicture(MI_MINI_ICON(mi)[i]);
		}
	}
 ...
 * through mipst. */
void
menuitem_get_size(struct MenuItem *mi, struct MenuItemPartSizesT *mipst,
    FlocaleFont *font, Bool do_reverse_icon_order)
{
	...
	if (MI_IS_POPUP(mi)) {
		...
		if (!is_formatted && MI_LABEL(mi)[0] != NULL) {
			...
			w = FlocaleTextWidth(font, MI_LABEL(mi)[0],
 ...
 */
void
menuitem_paint(struct MenuItem *mi, struct MenuPaintItemParameters *mpip)
{
	...
	for (i = 0; i < mpip->used_mini_icons; i++) {
		...
		y -= font->height + ST_ITEM_GAP_ABOVE(ms) +
		    ST_ITEM_GAP_BELOW(ms);
		if (y > 1) {
			text_y += y / 2;
		}
	}
	...
	if (MI_SHOULD_BE_GREYED(mi) ||
		...
		!is_function_allowed(MI_FUNC_TYPE(mi), MI_LABEL(mi)[0],
		    mpip->fw, RQORIG_PROGRAM_US, False))) {
	...
	if (is_item_selected &&
		...
		if (MDIM_HILIGHT_WIDTH(*dim) - 2 * relief_thickness > 0) {
			...
			if (ST_DO_HILIGHT_BACK(ms)) {
				draw_highlight_background(mpip, lit_x_start,
				    y_offset + relief_thickness,
				    lit_x_end - lit_x_start,
				    y_height - relief_thickness, &Colorset[cs],
				    gcs.back_gc);
				item_cleared = True;
			}
		}
	} else if (MI_WAS_DESELECTED(mi) &&
	    (relief_thickness > 0 || ST_DO_HILIGHT_BACK(ms) ||
		...
		/* Undo the hilighting. */
		x1 = min(MDIM_HILIGHT_X_OFFSET(*dim), MDIM_ITEM_X_OFFSET(*dim));
		x2 = max(MDIM_HILIGHT_X_OFFSET(*dim) + MDIM_HILIGHT_WIDTH(*dim),
	...
	if (is_item_selected && relief_thickness > 0) {
		...
		if (MDIM_HILIGHT_WIDTH(*dim) - 2 * relief_thickness > 0) {
			 ...
			 * MI_HEIGHT(mi) + 1 */
			RelieveRectangle(dpy, mpip->w,
			    MDIM_HILIGHT_X_OFFSET(*dim), y_offset,
			    MDIM_HILIGHT_WIDTH(*dim) - 1,
	...
	if (MI_IS_SEPARATOR(mi)) {
		if (sx1 < sx2) {
			/* It's a separator. */
			draw_separator(mpip->w, gcs.shadow_gc, gcs.hilight_gc,
		...
		tx2 = MDIM_ITEM_X_OFFSET(*dim) + MDIM_ITEM_WIDTH(*dim) - 1 -
		    relief_thickness - MENU_TEAR_OFF_BAR_X_OFFSET;
		if (tx1 < tx2) {

			/* It's a tear off bar. */
			draw_tear_off_bar(mpip->w, gcs.shadow_gc,
			    gcs.hilight_gc, tx1,
			    y_offset + relief_thickness +
				MENU_TEAR_OFF_BAR_Y_OFFSET,
			    tx2);
		}
		/* Nothing else to do. */
		return;
		...
		if (ST_TITLE_UNDERLINES(ms) > 0 && !mpip->flags.is_first_item) {
			...
			y = y_offset + add;
			if (sx1 < sx2) {
				draw_separator(mpip->w, gcs.shadow_gc,
				    gcs.hilight_gc, sx1, y, sx2);
			}
		}
		/* Underline the title. */
		...
		default:
			for (i = ST_TITLE_UNDERLINES(ms); i-- > 0;) {
				...
				XDrawLine(dpy, mpip->w, gcs.shadow_gc, sx1, y,
				    sx2, y);
			}
			break;
		}
	...
	if (!item_cleared && mpip->ev) {
		int u, v;
		if (!frect_get_seg_intersection(mpip->ev->xexpose.y,
			mpip->ev->xexpose.height, b.y, b.height, &u, &v)) {
			/* empty intersection */
			empty_inter = True;
		}
		b.y	 = u;
	...
	for (i = MAX_MENU_ITEM_LABELS; i-- > 0;) {
		if (!empty_inter && MI_LABEL(mi)[i] && *(MI_LABEL(mi)[i])) {
			...
			b.width = text_width = FlocaleTextWidth(font, fws->str,
			    strlen(fws->str));
			...
			if (!item_cleared && mpip->ev) {
				...
				if (frect_get_seg_intersection(
					mpip->ev->xexpose.x,
					mpip->ev->xexpose.width, fws->x,
					text_width, &s_x, &s_w)) {
					b.x	= s_x;
					b.width = s_w;
					region	= XCreateRegion();
					XUnionRectWithRegion(&b, region,
					    region);
	...
	if (MI_IS_POPUP(mi)) {
		...
		y = y_offset +
		    (y_height - MENU_TRIANGLE_HEIGHT + relief_thickness) / 2;

		if (ST_TRIANGLES_USE_FORE(ms)) {
			DrawTrianglePattern(dpy, mpip->w, tmp_gc, tmp_gc,
			...
			DrawTrianglePattern(dpy, mpip->w, gcs.hilight_gc,
			    gcs.shadow_gc, tmp_gc, MDIM_TRIANGLE_X_OFFSET(*dim),
	...
	if (MI_PICTURE(mi)) {
		...
		if (!item_cleared && mpip->ev) {
			if (!frect_get_intersection(mpip->ev->xexpose.x,
				mpip->ev->xexpose.y, mpip->ev->xexpose.width,
				mpip->ev->xexpose.height, b.x, b.y, b.width,
				b.height, &b)) {
				draw_picture = False;
			}
		}
		if (draw_picture) {
			if (!item_cleared &&
				...
				(tmp_cs >= 0 &&
					...
					100))) {
				XClearArea(dpy, mpip->w, b.x, b.y, b.width,
				    b.height, False);
			}
			PGraphicsRenderPicture(dpy, mpip->w, MI_PICTURE(mi),
			    &fra, mpip->w, tmp_gc, Scr.MonoGC, Scr.AlphaGC,
			    b.x - x, b.y - y, b.width, b.height, b.x, b.y,
			    b.width, b.height, False);
		}
	}
	...
	for (i = 0; i < mpip->used_mini_icons; i++) {
		 ...
		 * style. */
		k = (ST_USE_LEFT_SUBMENUS(ms)) ? mpip->used_mini_icons - 1 - i :
						 i;
		...
		if (MI_MINI_ICON(mi)[i]) {
			...
			if (!item_cleared && mpip->ev) {
				if (!frect_get_intersection(mpip->ev->xexpose.x,
					...
					mpip->ev->xexpose.height, b.x, b.y,
					b.width, b.height, &b)) {
					draw_picture = False;
				}
			}
			if (draw_picture) {
				...
				PGraphicsRenderPicture(dpy, mpip->w,
				    MI_MINI_ICON(mi)[i], &fra, mpip->w, tmp_gc,
				    Scr.MonoGC, Scr.AlphaGC,
				    b.x - MDIM_ICON_X_OFFSET(*dim)[k], b.y - y,
				    b.width, b.height, b.x, b.y, b.width,
				    b.height, False);
			}
		}
	}
...
int
menuitem_middle_y_offset(struct MenuItem *mi, struct MenuStyle *ms)
{
	int r;
