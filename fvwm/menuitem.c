    ...
    MenuPaintItemParameters *mpip, int x, int y, int w, int h)
{
	...
	if (!ST_HAS_MENU_CSET(ms) && ST_FACE(ms).type == GradientMenu
    ...
    int width, int height, colorset_t *cs, GC gc)
{
	if (cs != NULL && cs->pixmap && cs->pixmap_type != PIXMAP_TILED) {
		...
		p = CreateOffsetBackgroundPixmap(
		    dpy, mpip->w, 0, 0, width, height, cs, Pdepth, gc, False);
		...
		default:
			XCopyArea(
			    dpy, p, mpip->w, gc, 0, 0, width, height, x, y);
			break;
		}
		XFreePixmap(dpy, p);
	} else {
...
menuitem_paint(struct MenuItem *mi, struct MenuPaintItemParameters *mpip)
{
	...
	if (!IS_EWMH_DESKTOP_FW(mpip->fw)
	    && !is_function_allowed(MI_FUNC_TYPE(mi), MI_LABEL(mi)[0], mpip->fw,
	...
	if (is_item_selected
		...
		clear_menu_item_background(mpip, x1, y_offset + d, x2 - x1,
		...
		if (MDIM_HILIGHT_WIDTH(*dim) > 0
			...
			draw_highlight_background(mpip, lit_x_start,
			    ...
			    lit_x_end - lit_x_start,
			    y_height - relief_thickness,
			    (cs >= 0 ? &Colorset[cs] : NULL), gcs.back_gc);
			item_cleared = True;
		}
	}
	...
	for (i = MAX_MENU_ITEM_LABELS; i-- > 0;) {
		if (!empty_inter && MI_LABEL(mi)[i] && *(MI_LABEL(mi)[i])) {
			...
			b.width	     = text_width =
			    FlocaleTextWidth(font, fws->str, strlen(fws->str));
