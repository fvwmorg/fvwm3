    ...
    window_parts draw_parts, Bool has_focus, Bool is_border, Bool do_change_gcs)
{
	...
	if (has_focus) {
		...
		if (DFS_FACE_TYPE(df->style) == TiledPixmapButton
			...
			cd->texture_pixmap_width =
			    GetDecor(t, BorderStyle.active.u.p->width);
			cd->texture_pixmap_height =
			    GetDecor(t, BorderStyle.active.u.p->height);
		} else if (DFS_FACE_TYPE(df->style) == ColorsetButton) {
			cd->bg_border_cs =
			    GetDecor(t, BorderStyle.active.u.acs.cs);
		}
		...
		if (DFS_FACE_TYPE(df->style) == TiledPixmapButton
			...
			cd->texture_pixmap_width =
			    GetDecor(t, BorderStyle.inactive.u.p->width);
			cd->texture_pixmap_height =
			    GetDecor(t, BorderStyle.inactive.u.p->height);
		} else if (DFS_FACE_TYPE(df->style) == ColorsetButton) {
			cd->bg_border_cs =
			    GetDecor(t, BorderStyle.inactive.u.acs.cs);
		}
...
border_get_changed_border_parts(
    FvwmWindow *fw, rectangle *old_sidebar_g, rectangle *new_sidebar_g, int cs)
{
    ...
    rectangle *new_g, Bool do_hilight, border_relief_descr *br)
{
	...
	frame_get_sidebar_geometry(
	    fw, borderstyle, new_g, &br->sidebar_g, &has_x_marks, &has_y_marks);
	if (has_x_marks == True) {
    ...
    rectangle *old_g, rectangle *new_g, window_parts force_draw_parts)
{
	...
	if ((draw_parts & PART_TITLE) == PART_NONE && td->layout.title_g.x >= 0
...
border_get_border_relief_size_descr(
    border_relief_size_descr *ret_size_descr, FvwmWindow *fw, Bool do_hilight)
{
	DecorFaceStyle *borderstyle;
...
static void
border_draw_x_mark(
    border_relief_descr *br, int x, int y, Pixmap dest_pix, Bool do_draw_shadow)
{
	int k;
	int length;
	GC  gc;
    ...
    pixmap_background_type *bg, common_decorations_type *cd)
{
	...
	do_tile =
	    (bg->flags.use_pixmap && bg->pixmap.flags.is_tiled) ? True : False;
    ...
    titlebar_descr *td, Window w, int width, int height, int cs)
{
	...
	p = CreateBackgroundPixmap(
	    dpy, w, my_w, my_h, &Colorset[cs], Pdepth, Scr.BordersGC, False);
	...
	*free_bg_pixmap = False;
	...
	if (cd->texture_pixmap) {
		...
		if (CSET_IS_TRANSPARENT_ROOT(cd->bg_border_cs)) {
			...
			bg->pixmap.p =
			    XCreatePixmap(dpy, cd->dynamic_cd.frame_pixmap,
				part_g->width, part_g->height, Pdepth);
	...
	if (dcd->bar_pixmaps[bs].bps[i].p == None) {
		...
		while (b < BS_MaxButtonState) {
			...
			while (
				...
				|| dcd->bar_pixmaps[b].bps[j].mp_pic
				       != mp_pic)) {
				j++;
			}
...
border_mp_get_titlebar_descr(FvwmWindow *fw, titlebar_descr *td, DecorFace *df)
{
	...
	if (td->has_vt) {
		...
		td->full_right_main_g.height =
		    after_space + td->right_buttons_g.height;
		...
		td->full_right_main_g.width =
		    after_space + td->right_buttons_g.width;
    ...
    rectangle *right_of_text_g, rectangle *left_end_g, rectangle *right_end_g)
{
	...
	right_of_text =
	    border_mp_get_length(td, pm, acs, sf, TBMP_RIGHT_OF_TEXT);
    ...
    Pixmap dest_pix, Window w)
{
	...
	if (td->length > 0) {
		if (TBMP_HAS_PART(TBMP_LEFT_MAIN, pm, acs, solid_flags)
		    && td->left_main_g.width > 0
		    && td->left_main_g.height > 0) {
		...
		if (TBMP_HAS_PART(TBMP_RIGHT_MAIN, pm, acs, solid_flags)
		    && td->right_main_g.width > 0
		    && td->right_main_g.height > 0) {
			border_mp_render_into_pixmap(td, td->cd, pm, acs,
		...
		if (TBMP_HAS_PART(TBMP_UNDER_TEXT, pm, acs, solid_flags)
		    && td->under_text_g.width > 0
		    && td->under_text_g.height > 0) {
			border_mp_render_into_pixmap(td, td->cd, pm, acs,
	...
	case MultiPixmap: /* for UseTitleStyle only */
	{
		...
		if (cs >= 0 && use_title_style && g.width > 0 && g.height > 0
		    ...
		    && (bg.pixmap.p = border_get_bar_pixmaps(
			    dcd, &g, bs, cs, NULL, part, stretch, NULL))
			   != None) {
			bg.pixmap.g.width	 = g.width;
			bg.pixmap.g.height	 = g.height;
			bg.pixmap.flags.is_tiled = 1;
			...
			bg.pixmap.flags.is_tiled = 1;
			   ...
			   && border_get_bar_pixmaps(dcd, &g, bs, -1, pm[part],
				  part, stretch, &full_pic)
				  != None
			   && full_pic) {
			bg.pixmap.p	   = full_pic->picture;
			bg.pixmap.shape	   = full_pic->mask;
	...
	case ColorsetButton: {
		...
		if (use_title_style && !CSET_IS_TRANSPARENT_ROOT(cs)
			...
			GetWindowBackgroundPixmapSize(
			    cs_t, w_g->width, w_g->height, &bg_w, &bg_h);
			bg.pixmap.g.width  = bg_w;
			bg.pixmap.g.height = bg_h;
			bg.pixmap.g.x	   = 0;
			bg.pixmap.g.y	   = 0;
...
border_set_button_pixmap(
    FvwmWindow *fw, titlebar_descr *td, int button, Pixmap *dest_pix, Window w)
{
	pixmap_background_type bg;
	unsigned int	       mask;
	int		       is_left_button;
...
border_draw_title_stick_lines(
    FvwmWindow *fw, titlebar_descr *td, title_draw_descr *tdd, Pixmap dest_pix)
{
	int	   i;
	int	   num;
	int	   min;
	...
	num = (int)(fw->title_thickness / WINDOW_TITLE_STICK_VERT_DIST / 2) * 2
	      - 1;
	...
	right_x =
	    ((under_text_length == 0) ? td->offset + td->length
	...
	if (right_w < WINDOW_TITLE_STICK_MIN_WIDTH) {
		...
		right_x =
		    fw->title_length - WINDOW_TITLE_STICK_MIN_WIDTH - 1
		    - td->right_end_length
    ...
    title_draw_descr *tdd, FlocaleWinString *fstr, Pixmap dest_pix)
{
	...
	XFillRectangle(dpy, dest_pix, td->cd->relief_gc, td->offset - 2, 0,
	...
	XDrawLine(dpy, dest_pix, tdd->sgc,
	    ...
	    SWAP_ARGS(
		has_vt, td->offset + td->length + 1, fw->title_thickness));
...
#define ROTATE_RECTANGLE(rot, r, vs_frame, vs_titlebar, vs_title)             \
	{                                                                     \
		...
		case ROTATION_90: /* cw */                                    \
			if (vs_frame) {                                       \
				tr.x =                                        \
				    td->frame_g.height - (r->y + r->height);  \
			} else if (vs_titlebar) {                             \
    ...
    rectangle *old_g, rectangle *new_g, Bool do_hilight, titlebar_descr *ret_td)
{
	...
	if (ret_td->has_an_upsidedown_rotation) {
		if (ret_td->has_vt) {
			...
			ret_td->left_buttons_g.y =
			    ret_td->layout.title_g.y
			...
			ret_td->left_buttons_g.x =
			    ret_td->layout.title_g.x
	...
	if (fw->visible_name != (char *)NULL) {
		ret_td->length =
		    FlocaleTextWidth(fw->title_font, fw->visible_name,
    ...
    rectangle *old_g, rectangle *new_g, Bool do_hilight)
{
	...
	draw_parts =
	    border_get_titlebar_descr(cd, fw, pressed_parts, pressed_button,
		force_draw_parts, clear_parts, old_g, new_g, do_hilight, &td);
	if ((draw_parts & PART_TITLE) != PART_NONE
