    ...
    char **ret_cmd)
{
	char *optlist[] = {"MenuClose", "MenuEnterContinuation",
	    ...
	    "MenuCursorLeft", "MenuCursorRight", "MenuSelectItem", "MenuScroll",
	    "MenuTearOff", "MenuCloseAndExec", NULL};
	int   index;
	char *options;
	int   num;
	int   suffix[2];
	...
	*ret_cmd = NULL;
	...
	case 1: /* MenuEnterContinuation */
		*saction =
		    (MR_CONTINUATION_MENU(mr) != NULL) ? SA_CONTINUE : SA_ENTER;
		break;
    ...
    int context, int modifier, char *action, char *menu_style)
{
	 ...
	 */
	/* BEGIN remove */
	CollectBindingList(dpy, menu_bindings, &rmlist, &dummy, type, button,
	...
	rc = AddBinding(dpy, menu_bindings, type, button, keysym, NULL,
    ...
    double_keypress *pdkp, int *ret_menu_x, int *ret_menu_y)
{
	...
	} else /* Should be KeyPressed */
	{
		 ...
		 * Use XlookupString */
		index =
		    XLookupString(&(event->xkey), &ckeychar, 1, &keysym, NULL);
	...
	if (event->type == KeyPress && keysym == XK_Escape && with_control == 0
	    && with_shift == 0 && with_meta == 0) {
	...
	case SA_FIRST:
		if (do_skip_section) {
			*pmi_current =
			    get_selectable_item_from_section(mr, items_to_move);
		} else {
			*pmi_current =
			    get_selectable_item_from_index(mr, items_to_move);
		}
		if (*pmi_current != NULL) {
			pmret->rc = MENU_NEWITEM;
		} else {
	...
	case SA_LAST:
		if (do_skip_section) {
			...
			*pmi_current =
			    get_selectable_item_from_section(mr, index);
			if (*pmi_current != NULL) {
				pmret->rc = MENU_NEWITEM;
			} else {
				pmret->rc = MENU_NOP;
			...
			if (index > 0) {
				...
				*pmi_current =
				...
				pmret->rc =
				    (*pmi_current) ? MENU_NEWITEM : MENU_NOP;
			} else {
				pmret->rc = MENU_NOP;
			}
		}
	...
	case SA_SELECT:
		...
		return;
	case SA_ABORT:
		pmret->rc = (MR_IS_TEAR_OFF_MENU(mr)) ? MENU_KILL_TEAR_OFF_MENU
						      : MENU_ABORTED;
		return;
	case SA_TEAROFF:
		pmret->rc =
