 ...
 */
static int
get_selectable_item_index(MenuRoot *mr, MenuItem *mi_target, int *ret_sections)
{
	int	  i = 0;
	int	  s = 0;
	MenuItem *mi;
...
static MenuItem *
get_selectable_item_from_index(MenuRoot *mr, int index)
{
	int	  i = -1;
	MenuItem *mi;
...
static MenuItem *
get_selectable_item_from_section(MenuRoot *mr, int section)
{
	int	  i = 0;
	MenuItem *mi;
	MenuItem *mi_last_ok	     = NULL;
	...
	for (mi = MR_FIRST_ITEM(mr); mi && (i <= section || mi_last_ok == NULL);
	     mi = MI_NEXT_ITEM(mi)) {
		if (MI_IS_SELECTABLE(mi)) {
...
static int
get_selectable_item_count(MenuRoot *mr, int *ret_sections)
{
	int count;
...
static Binding *
__menu_binding_is_mouse(Binding *blist, XEvent *event, int context)
{
	Binding *b;
	int	 real_mod;
...
static void
parse_menu_action(struct MenuRoot *mr, const char *action,
    menu_shortcut_action *saction, int *items_to_move, int *do_skip_section,
    char **ret_cmd)
{
	char *optlist[] = { "MenuClose", "MenuEnterContinuation",
		"MenuEnterSubmenu", "MenuLeaveSubmenu", "MenuMoveCursor",
		"MenuCursorLeft", "MenuCursorRight", "MenuSelectItem",
		"MenuScroll", "MenuTearOff", "MenuCloseAndExec", NULL };
	int   index;
	char *options;
	int   num;
	int   suffix[2];
	...
	*ret_cmd = NULL;
	...
	case 1: /* MenuEnterContinuation */
		*saction = (MR_CONTINUATION_MENU(mr) != NULL) ? SA_CONTINUE :
								SA_ENTER;
		break;
	...
	case 4: /* MenuMoveCursor */
		num = GetSuffixedIntegerArguments(options, NULL, count, 2, "s",
		    suffix);
		if (num == 2) {
	...
	case 10: /* MenuCloseAndExecute */
		...
		*ret_cmd = options;
		break;
	default:
		fvwm_debug(__func__, "unknown action '%s'", action);
		*saction = SA_NONE;
	}
...
static Binding *
__menu_binding_is_key(Binding *blist, XEvent *event, int context)
{
	Binding *b;

	for (b = blist; b != NULL; b = b->NextBinding) {
		if (BIND_IS_KEY_BINDING(b->type) &&
...
menu_binding_is_mouse(XEvent *event, int context)
{
	...
	if (b == NULL) {
		b = __menu_binding_is_mouse(menu_bindings_fallback, event,
		    context);
	}
...
menu_binding_is_key(XEvent *event, int context)
{
	...
	if (b == NULL) {
		b = __menu_binding_is_key(menu_bindings_fallback, event,
		    context);
	}
...
int
menu_binding(Display *disp, binding_t type, int button, KeySym keysym,
    int context, int modifier, char *action, char *menu_style)
{
	 ...
	 */
	/* BEGIN remove */
	CollectBindingList(disp, menu_bindings, &rmlist, &dummy, type, button,
	    keysym, modifier, context, menu_style);
	if (rmlist != NULL) {
	...
	rc = AddBinding(disp, menu_bindings, type, button, keysym, NULL,
    ...
    double_keypress *pdkp, int *ret_menu_x, int *ret_menu_y)
{
	...
	if (mi_current) {
		...
		if (menu_get_geometry(mr, &JunkRoot, &menu_x, &menu_y,
			&menu_width, &menu_height, &JunkBW, &JunkDepth)) {
			is_geometry_known = 1;
			if (FQueryPointer(dpy, Scr.Root, &JunkRoot, &JunkChild,
				&mx, &my, &JunkX, &JunkY, &JunkMask) == 0) {
				/* pointer is on a different screen */
				mx = 0;
				my = 0;
			} else if (mx >= menu_x && mx < menu_x + menu_width &&
	...
	if (event->type == ButtonRelease) {
		...
		if (binding != NULL) {
			parse_menu_action(mr, binding->Action, &saction,
			    &items_to_move, &do_skip_section, &command);
		}
		index	 = 0;
		ikeychar = 0;
	...
	if (index == 1 && isascii(ikeychar) && isgraph(ikeychar) &&
		...
		do {
			if (MI_HAS_HOTKEY(mi) && !MI_IS_TITLE(mi) &&
				...
				key = (MI_LABEL(mi)[(int)MI_HOTKEY_COLUMN(
				    mi)])[MI_HOTKEY_COFFSET(mi)];
				key = tolower(key);
				if (ikeychar == key) {
					if (++countHotkey == 1) {
						new_item = mi;
			...
			mi = (mi == MR_LAST_ITEM(mr)) ? MR_FIRST_ITEM(mr) :
							MI_NEXT_ITEM(mi);
		} while (mi != mi1);
	...
	if (event->type == KeyPress && keysym == XK_Escape &&
	    with_control == 0 && with_shift == 0 && with_meta == 0) {
	...
	if (!mi_current &&
		...
		if (is_geometry_known) {
			if (my < menu_y + MST_BORDER_WIDTH(mr)) {
			    ...
			    menu_y + menu_height - MST_BORDER_WIDTH(mr)) {
				saction = SA_LAST;
			} else {
				saction = SA_WARPBACK;
			}
	...
	case SA_LEAVE:
		pmret->rc = (MR_IS_TEAR_OFF_MENU(mr)) ? MENU_NOP : MENU_POPDOWN;
		break;
	case SA_FIRST:
		if (do_skip_section) {
			*pmi_current = get_selectable_item_from_section(mr,
			    items_to_move);
		} else {
			*pmi_current = get_selectable_item_from_index(mr,
			    items_to_move);
		}
		if (*pmi_current != NULL) {
			pmret->rc = MENU_NEWITEM;
		...
		if (do_skip_section) {
			...
			*pmi_current = get_selectable_item_from_section(mr,
			...
			if (index > 0) {
				...
				*pmi_current =
				    get_selectable_item_from_index(mr, index);
				pmret->rc = (*pmi_current) ? MENU_NEWITEM :
							     MENU_NOP;
			} else {
				pmret->rc = MENU_NOP;
			}
	...
	case SA_MOVE_ITEMS:
		if (do_skip_section) {
			...
			index = get_selectable_item_index(mr, mi_current, NULL);
			if (index == 0)
				/* wraparound */
				index = get_selectable_item_count(mr, NULL);
			else {
			...
			index = get_selectable_item_index(mr, mi_current,
				    NULL) +
			    items_to_move;
		...
		if (do_skip_section) {
			...
			if (items_to_move > 0 && new_item == mi_current) {
				new_item = get_selectable_item_from_index(mr,
				    0);
			}
		}
		if (new_item) {
	...
	case SA_SELECT:
		...
		return;
	case SA_ABORT:
		pmret->rc = (MR_IS_TEAR_OFF_MENU(mr)) ?
		    MENU_KILL_TEAR_OFF_MENU :
		    MENU_ABORTED;
		return;
	case SA_TEAROFF:
		pmret->rc = (MR_IS_TEAR_OFF_MENU(mr)) ? MENU_NOP :
							MENU_TEAR_OFF;
		return;
	case SA_SCROLL:
		...
		if (new_item) {
			*pmi_current = new_item;
			...
			if (MST_MOUSE_WHEEL(mr) == MMW_POINTER) {
				...
				menu_y += my -
				    menuitem_middle_y_offset(new_item,
					MR_STYLE(mr));
	...
	if (saction != SA_SCROLL && pmret->rc == MENU_NEWITEM) {
		...
		if (menu_y < 0 ||
			...
			menu_y	    = (menu_y < 0) ? 0 :
						     MR_SCREEN_Y(mr) +
				     MR_SCREEN_HEIGHT(mr) - menu_height;
			pmret->rc   = MENU_NEWITEM_MOVEMENU;
			*ret_menu_x = menu_x;
			*ret_menu_y = menu_y;
		}
