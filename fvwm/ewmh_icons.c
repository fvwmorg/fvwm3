...
ewmh_WMIcon(FvwmWindow *fw, XEvent *ev, window_style *style, unsigned long any)
{
	...
	list = ewmh_AtomGetByName(
	    FW_W(fw), "_NET_WM_ICON", EWMH_ATOM_LIST_PROPERTY_NOTIFY, &size);
	...
	if (list == NULL || HAS_EWMH_WM_ICON_HINT(fw) != EWMH_TRUE_ICON) {
		...
		if (DO_EWMH_DONATE_ICON(fw)
		    && (new_list =
			       ewmh_SetWmIconFromPixmap(fw, list, &size, False))
			   != NULL) {
...
EWMH_DoUpdateWmIcon(FvwmWindow *fw, Bool mini_icon, Bool icon)
{
	...
	if (FMiniIconsSupported && mini_icon && DO_EWMH_DONATE_MINI_ICON(fw)) {
		if ((dummy = ewmh_SetWmIconFromPixmap(
			 fw, (new_list != NULL) ? new_list : list, &size, True))
		    != NULL) {
			SET_HAS_EWMH_WM_ICON_HINT(fw, EWMH_FVWM_ICON);
			free(dummy);
		}
    ...
    FvwmWindow *fw, CARD32 *orig_icon, int *orig_size, Bool is_mini_icon)
{
	...
	*orig_size = 0;
	...
	if (alpha != None) {
		a_image = XGetImage(
		    dpy, alpha, 0, 0, width, height, AllPlanes, ZPixmap);
	}
...
EWMH_DeleteWmIcon(FvwmWindow *fw, Bool mini_icon, Bool icon)
{
	...
	if (keep_length > 0) {
		...
		memcpy(
		    new_list, &list[keep_start], keep_length * sizeof(CARD32));
	}
