 ...
 */
int
ewmh_WMIcon(FvwmWindow *fw, XEvent *ev, window_style *style, unsigned long any)
{
	CARD32 *list	 = NULL;
	...
	if (list == NULL || HAS_EWMH_WM_ICON_HINT(fw) != EWMH_TRUE_ICON) {
		...
		if (DO_EWMH_DONATE_ICON(fw) &&
		    (new_list = ewmh_SetWmIconFromPixmap(fw, list, &size,
			 False)) != NULL) {
	...
	if (FMiniIconsSupported) {
		if (list == NULL ||
			...
			if (DO_EWMH_DONATE_MINI_ICON(fw) &&
			    (dummy = ewmh_SetWmIconFromPixmap(fw,
				 (new_list != NULL) ? new_list : list, &size,
				 True)) != NULL) {
				SET_HAS_EWMH_WM_ICON_HINT(fw, EWMH_FVWM_ICON);
				free(dummy);
			}
 ...
 */
void
EWMH_DoUpdateWmIcon(FvwmWindow *fw, Bool mini_icon, Bool icon)
{
	CARD32 *list	 = NULL;
	CARD32 *new_list = NULL;
	CARD32 *dummy	 = NULL;
	...
	/* we have to reset */
	if (icon && DO_EWMH_DONATE_ICON(fw)) {
		if ((new_list = ewmh_SetWmIconFromPixmap(fw, list, &size,
			 False)) != NULL) {
			SET_HAS_EWMH_WM_ICON_HINT(fw, EWMH_FVWM_ICON);
		}
	}
	if (FMiniIconsSupported && mini_icon && DO_EWMH_DONATE_MINI_ICON(fw)) {
		if ((dummy = ewmh_SetWmIconFromPixmap(fw,
			 (new_list != NULL) ? new_list : list, &size, True)) !=
		    NULL) {
			SET_HAS_EWMH_WM_ICON_HINT(fw, EWMH_FVWM_ICON);
			free(dummy);
		}
 ...
 */
CARD32 *
ewmh_SetWmIconFromPixmap(FvwmWindow *fw, CARD32 *orig_icon, int *orig_size,
    Bool is_mini_icon)
{
	...
	*orig_size = 0;
	...
	if (FMiniIconsSupported && orig_icon != NULL) {
		int k_width  = (is_mini_icon) ? fw->ewmh_icon_width :
						fw->ewmh_mini_icon_width;
		int k_height = (is_mini_icon) ? fw->ewmh_icon_height :
	...
	if (image == NULL) {
		fvwm_debug(__func__, "cannot create XImage\n");
		if (destroy_icon_pix) {
	...
	if (mask != None) {
		m_image = XGetImage(dpy, mask, 0, 0, width, height, AllPlanes,
		    ZPixmap);
	}
	if (alpha != None) {
		a_image = XGetImage(dpy, alpha, 0, 0, width, height, AllPlanes,
		    ZPixmap);
	}
	*orig_size = (height * width + 2 + keep_length) * sizeof(CARD32);
	new_icon   = fxmalloc(*orig_size);
	if (keep_length > 0) {
	...
	default: /* depth = Pdepth */
	{
		...
		colors = fxmalloc(width * height * sizeof(XColor));
		cm     = fxmalloc(width * height * sizeof(char));
		for (j = 0; j < height; j++) {
			for (i = 0; i < width; i++) {
				if (m_image != NULL &&
				    (XGetPixel(m_image, i, j) == 0)) {
					...
					colors[k++].pixel = XGetPixel(image, i,
					    j);
				} else {
					cm[m++]		  = 255;
					colors[k++].pixel = XGetPixel(image, i,
	...
	ewmh_ChangeProperty(FW_W(fw), "_NET_WM_ICON",
	    EWMH_ATOM_LIST_PROPERTY_NOTIFY, (unsigned char *)new_icon,
 ...
 */
void
EWMH_DeleteWmIcon(FvwmWindow *fw, Bool mini_icon, Bool icon)
{
	CARD32 *list;
	CARD32 *new_list   = NULL;
	int	keep_start = 0, keep_length = 0;
	...
	if (mini_icon && icon) {
		ewmh_DeleteProperty(FW_W(fw), "_NET_WM_ICON",
	...
	if (keep_length > 0) {
		new_list = fxmalloc(keep_length * sizeof(CARD32));
		memcpy(new_list, &list[keep_start],
		    keep_length * sizeof(CARD32));
	}

	if (new_list != NULL) {
		ewmh_ChangeProperty(FW_W(fw), "_NET_WM_ICON",
		    EWMH_ATOM_LIST_PROPERTY_NOTIFY, (unsigned char *)new_list,
		    keep_length);
	} else {
		/*SET_HAS_EWMH_WM_ICON_HINT(fw, EWMH_NO_ICON);*/
		ewmh_DeleteProperty(FW_W(fw), "_NET_WM_ICON",
...
#define SQUARE(X) ((X) * (X))
static void
extract_wm_icon(CARD32 *list, int size, int wanted_w, int wanted_h,
...
int
EWMH_SetIconFromWMIcon(FvwmWindow *fw, CARD32 *list, int size,
    Bool is_mini_icon)
{
	...
	if (!PImageCreatePixmapFromArgbData(dpy, Scr.NoFocusWin, list, start,
		width, height, &pixmap, &mask, &alpha, &nalloc_pixels,
		&alloc_pixels, &no_limit, fpa)) {
		fvwm_debug(__func__, "fail to create a pixmap\n");
		if (free_list) {
			free(list);
		}
		return 0;
	...
	if (width > max_w || height > max_h) {
		...
		if (pixmap) {
			np = CreateStretchPixmap(dpy, pixmap, width, height,
			    Pdepth, wanted_w, wanted_h, Scr.TitleGC);
			XFreePixmap(dpy, pixmap);
			pixmap = np;
		}
		if (mask) {
			nm = CreateStretchPixmap(dpy, mask, width, height, 1,
			    wanted_w, wanted_h, Scr.MonoGC);
			XFreePixmap(dpy, mask);
			mask = nm;
		}
		if (alpha) {
	...
	if (FMiniIconsSupported && is_mini_icon &&
		...
		fw->mini_icon = PCacheFvwmPictureFromPixmap(dpy, Scr.NoFocusWin,
		    name, pixmap, mask, alpha, width, height, nalloc_pixels,
		    alloc_pixels, no_limit);
		if (fw->mini_icon != NULL) {
			fw->mini_pixmap_file = name;
			BroadcastFvwmPicture(M_MINI_ICON, FW_W(fw),
			    FW_W_FRAME(fw), (unsigned long)fw, fw->mini_icon,
			    fw->mini_pixmap_file);
