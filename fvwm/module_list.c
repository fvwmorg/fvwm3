...
module_list_remove(fmodule *module, fmodule_list *list)
{
	...
	for (position = list; *position != NULL;
...
do_execute_module(F_CMD_ARGS, Bool desperate, Bool do_listen_only)
{
	...
	if (fvwm_to_app[0] >= fvwmlib_max_fd || fvwm_to_app[1] >= fvwmlib_max_fd
...
void CMD_ModuleSynchronous(F_CMD_ARGS)
{
	...
	while (!done) {
		...
		do {
			...
			timeout.tv_usec = 1;
			num_fd		= fvwmSelect(
			     fvwmlib_max_fd, &in_fdset, &out_fdset, 0, &timeout);
		...
		if (FPending(dpy)
			...
			escape =
			    CheckBinding(Scr.AllBindings, tmpevent.xkey.keycode,
				tmpevent.xkey.state, GetUnusedModifiers(),
				context, BIND_KEYPRESS, class, name);
			if (escape != NULL) {
