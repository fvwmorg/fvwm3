...
static inline void msg_mask_set(msg_masks_t *msg_mask, unsigned long m1,
    unsigned long m2);
static void	   set_message_mask(msg_masks_t *mask, unsigned long msg);

void
module_kill_all(void)
{
...
static inline void
module_list_insert(fmodule *module, fmodule_list *list)
{
...
static inline fmodule *
module_list_remove(fmodule *module, fmodule_list *list)
{
	fmodule_store **pos;
...
static inline void
module_list_destroy(fmodule_list *list)
{
	fmodule_store *current;
	fmodule_store *next;
...
static fmodule *
do_execute_module(F_CMD_ARGS, Bool desperate, Bool do_listen_only)
{
	...
	if (fvwm_to_app[0] >= fvwmlib_max_fd ||
	...
	if (val > 0) {
		...
		if (fcntl(MOD_READFD(module), F_SETFD, 1) == -1) {
			fvwm_debug(__func__, "module close-on-exec failed");
		}
		if (MOD_WRITEFD(module) >= 0 &&
		...
		if (!Pdefault) {
			...
			snprintf(visualid, sizeof(visualid),
			    "FVWM_VISUALID=%lx", XVisualIDFromVisual(Pvisual));
...
void
module_kill(fmodule *module)
{
	module_list_insert(module_list_remove(module, &module_list),
	    &death_row);
...
void
PositiveWrite(fmodule *module, unsigned long *ptr, int size)
{
	msg_masks_t mask;
	...
	if (IS_MESSAGE_IN_MASK(&(MOD_SYNCMASK(module)), ptr[1]) &&
		...
		while (!done) {
			...
			do {
				 ...
				 */
				rc = fvwmSelect(channel + 1, &readSet, NULL,
				    NULL, &timeout);
...
module_receive(fmodule *module)
{
	...
	if (size > MAX_MODULE_INPUT_TEXT_LEN) {
		fvwm_debug(__func__,
		    "Module(%p) command is too big (%ld), limit is %d", module,
		    size, MAX_MODULE_INPUT_TEXT_LEN);
	...
	if (n < sizeof(cont)) {
		fvwm_debug(__func__,
		    "Module %p, Size Problems (read: %d, size: %d)", module, n,
		    (int)sizeof(cont));
		goto err;
	}
	if (cont == 0) {
...
void
module_input_discard(fmodule_input *input)
{
	if (input == NULL) {
		return;
	}
...
Bool
module_input_expect(fmodule_input *input, char *expect)
{
...
void
module_list_itr_init(fmodule_list_itr *itr)
{
	*itr = module_list;
...
fmodule *
module_list_itr_next(fmodule_list_itr *itr)
{
	fmodule *module;
...
static void
KillModuleByName(char *name, char *alias)
{
 ...
 */
char *
skipModuleAliasToken(const char *string)
{
#define is_valid_first_alias_char(ch) (isalpha(ch) || (ch) == '/')
#define is_valid_alias_char(ch)                                         \
	(is_valid_first_alias_char(ch) || isalnum(ch) || (ch) == '-' || \
	    (ch) == '.' || (ch) == '/')
...
static inline void
msg_mask_set(msg_masks_t *msg_mask, unsigned long m1, unsigned long m2)
{
	msg_mask->m1 = m1;
	msg_mask->m2 = m2;
 ...
 */
static void
set_message_mask(msg_masks_t *mask, unsigned long msg)
{
...
static void
DeleteMessageQueueBuff(fmodule *module)
{
	mqueue_object_type *obj;

	if (fqueue_get_first(&(MOD_PIPEQUEUE(module)), (void **)&obj) == 1) {
		/* remove from queue */
		fqueue_remove_or_operate_from_front(&(MOD_PIPEQUEUE(module)),
		    NULL, NULL, NULL, NULL);
...
void
FlushMessageQueue(fmodule *module)
{
	mqueue_object_type *obj;
	char		   *dptr;
	int		    a;
...
void
CMD_ModuleListenOnly(F_CMD_ARGS)
{
...
void
CMD_ModuleSynchronous(F_CMD_ARGS)
{
	int	 sec = 0;
	char	*next;
	char	*token;
	...
	while (!done) {
		...
		do {
			...
			timeout.tv_sec	= 0;
			timeout.tv_usec = 1;
			num_fd		= fvwmSelect(fvwmlib_max_fd, &in_fdset,
				     &out_fdset, 0, &timeout);
		...
		if (FPending(dpy) &&
			...
			context = GetContext(NULL, exc->w.fw, &tmpevent, &w);
			if (exc->w.fw != NULL) {
			...
			escape = CheckBinding(Scr.AllBindings,
			    ...
			    GetUnusedModifiers(), context, BIND_KEYPRESS, class,
			    name);
			if (escape != NULL) {
...
void
CMD_set_nograb_mask(F_CMD_ARGS)
{
	unsigned long val;
