...
{
	ret_grav->decor_grav =
	    gravity_combine_xy_grav(grav_x->decor_grav, grav_y->decor_grav);
	ret_grav->title_grav =
	    gravity_combine_xy_grav(grav_x->title_grav, grav_y->title_grav);
	ret_grav->lbutton_grav =
	    gravity_combine_xy_grav(grav_x->lbutton_grav, grav_y->lbutton_grav);
	ret_grav->rbutton_grav =
	    gravity_combine_xy_grav(grav_x->rbutton_grav, grav_y->rbutton_grav);
    ...
    Bool do_send_configure_notify, Bool do_force, Bool is_application_request)
{
	...
	if (do_send_configure_notify && !IS_SHADED(fw)) {
		SendConfigureNotify(
		    fw, new_g.x, new_g.y, new_g.width, new_g.height, 0, True);
	}
	/* get things updated */
	XFlush(dpy);
...
frame_get_titlebar_compression(FvwmWindow *fw, rectangle *frame_g)
{
	...
	need_space =
	    (fw->nr_left_buttons + fw->nr_right_buttons) * fw->title_thickness
    ...
    rectangle *ret_hidden_g)
{
	...
	if (do_unhide == False) {
		...
		ret_hidden_g->x = (dir_x == DIR_E) ? -mra->client_g.width
							 + mra->parent_s.width
						   : 0;
		ret_hidden_g->y = (dir_y == DIR_S) ? -mra->client_g.height
							 + mra->parent_s.height
						   : 0;
		target_g	= &mra->next_g;
	} else {
...
frame_mrs_setup_draw_decorations(FvwmWindow *fw, mr_args_internal *mra)
{
	...
	} else /* lazy shading */
	{
		...
		if (mra->current_step == 1) {
			...
			lazy_g.width =
			    max(mra->current_g.width, mra->end_g.width);
			lazy_g.height =
			    max(mra->current_g.height, mra->end_g.height);
			border_draw_decorations(fw, PART_ALL,
...
frame_mrs_resize_move_windows(FvwmWindow *fw, mr_args_internal *mra)
{
	...
	if (!mra->flags.is_shading) {
		...
		if (mra->mode == FRAME_MR_SCROLL) {
			...
			if (mra->dstep_g.y == 0) {
				y -=
				    mra->dstep_g.height - mra->minimal_h_offset;
				;
			}
    ...
    rectangle *diff_g, frame_title_layout_t *title_layout)
{
	...
	if (HAS_VERTICAL_TITLE(fw)) {
		...
		if (GET_TITLE_DIR(fw) == DIR_N) {
			...
			tb_y =
			    frame_g->height - b.bottom_right.height - tb_thick;
			if (diff_g != NULL) {
				tb_y -= diff_g->height;
			}
		}
    ...
    rectangle *start_g, rectangle *end_g, int anim_steps, int shade_dir)
{
	...
	mra->flags.is_setup =
	    (mra->mode == FRAME_MR_FORCE_SETUP || mra->mode == FRAME_MR_SETUP);
	...
	if (mra->flags.is_setup && mra->delta_g.x == 0 && mra->delta_g.y == 0
		...
		frame_set_decor_gravities(
		    fw, &mra->grav, (mra->flags.do_set_bit_gravity) ? 1 : 0);
	}
...
frame_free_move_resize_args(FvwmWindow *fw, frame_move_resize_args mr_args)
{
	...
	if (mra->flags.do_update_shape) {
		/* unset shape */
		FShapeCombineMask(
		    dpy, FW_W_FRAME(fw), FShapeBounding, 0, 0, None, FShapeSet);
	}
	...
	if (mra->flags.do_restore_gravity) {
		 ...
		 */
		frame_set_decor_gravities(
		    fw, &mra->grav, (mra->flags.do_set_bit_gravity) ? 2 : 0);
	} else {
...
void
frame_setup_window(
    FvwmWindow *fw, int x, int y, int w, int h, Bool do_send_configure_notify)
{
	rectangle g;
