...
{
	ret_grav->decor_grav   = gravity_combine_xy_grav(grav_x->decor_grav,
	      grav_y->decor_grav);
	ret_grav->title_grav   = gravity_combine_xy_grav(grav_x->title_grav,
	      grav_y->title_grav);
	ret_grav->lbutton_grav = gravity_combine_xy_grav(grav_x->lbutton_grav,
	    grav_y->lbutton_grav);
	ret_grav->rbutton_grav = gravity_combine_xy_grav(grav_x->rbutton_grav,
	    grav_y->rbutton_grav);
	ret_grav->parent_grav  = gravity_combine_xy_grav(grav_x->parent_grav,
	     grav_y->parent_grav);
	ret_grav->client_grav  = gravity_combine_xy_grav(grav_x->client_grav,
	     grav_y->client_grav);
...
static void
get_resize_decor_gravities_one_axis(frame_decor_gravities_type *ret_grav,
    direction_t title_dir, frame_move_resize_mode axis_mode,
    direction_t neg_dir, direction_t pos_dir, int is_moving)
{
	int title_grav;
	int neg_grav;
	int pos_grav;
    ...
    size_borders *bs, rectangle *ret_titlebar_g)
{
	...
	case DIR_E:
		...
		ret_titlebar_g->height = frame_g->height -
		    bs->total_size.height;
		break;
	case DIR_N:
	case DIR_S:
...
static void
frame_setup_border(FvwmWindow *fw, rectangle *frame_g, window_parts setup_parts,
    rectangle *diff_g)
{
	XWindowChanges xwc;
	Window	       w;
...
static void
frame_setup_titlebar(FvwmWindow *fw, rectangle *frame_g,
    window_parts setup_parts, rectangle *diff_g)
{
	...
	for (i = 0; i < NUMBER_OF_TITLE_BUTTONS; i++) {
		if (FW_W_BUTTON(fw, i) != None &&
			...
			XMoveResizeWindow(dpy, FW_W_BUTTON(fw, i),
	...
	if (setup_parts & PART_TITLE) {
		XMoveResizeWindow(dpy, FW_W_TITLE(fw), title_layout.title_g.x,
		    title_layout.title_g.y, title_layout.title_g.width,
...
static void
__frame_setup_window(FvwmWindow *fw, rectangle *frame_g,
    Bool do_send_configure_notify, Bool do_force, Bool is_application_request)
{
	...
	if (is_resized || do_force) {
		...
		mr_args = frame_create_move_resize_args(fw, mode, NULL, &new_g,
		    0, DIR_NONE);
		...
		fw->g.frame = *frame_g;
		if ((draw_parts = border_get_transparent_decorations_part(
			 fw)) != PART_NONE) {
	...
	if (do_send_configure_notify && !IS_SHADED(fw)) {
		SendConfigureNotify(fw, new_g.x, new_g.y, new_g.width,
		    new_g.height, 0, True);
	}
	/* get things updated */
	XFlush(dpy);
...
static void
frame_reparent_hide_windows(Window w)
{
	int i;
...
static Bool
frame_is_parent_hidden(FvwmWindow *fw, rectangle *frame_g)
{
	size_borders b;
 ...
 * pixel. */
static int
frame_get_titlebar_compression(FvwmWindow *fw, rectangle *frame_g)
{
	size_borders b;
	int	     space;
	int	     need_space;
 ...
 */
static void
frame_get_resize_decor_gravities(frame_decor_gravities_type *ret_grav,
    direction_t title_dir, frame_move_resize_mode rmode, rectangle *delta_g)
{
	...
	get_resize_decor_gravities_one_axis(&grav_x, title_dir_x, rmode, DIR_W,
	    DIR_E, (delta_g->x != 0));
	get_resize_decor_gravities_one_axis(&grav_y, title_dir_y, rmode, DIR_N,
	    DIR_S, (delta_g->y != 0));
...
static void
frame_set_decor_gravities(FvwmWindow *fw, frame_decor_gravities_type *grav,
...
static void
frame_restore_client_gravities(FvwmWindow *fw)
{
	XSetWindowAttributes xcwa;
	long		     valuemask;
...
static void
frame_next_move_resize_args(frame_move_resize_args mr_args)
{
...
static rectangle *
frame_get_hidden_pos(FvwmWindow *fw, mr_args_internal *mra, Bool do_unhide,
...
static void
frame_update_hidden_window_pos(FvwmWindow *fw, mr_args_internal *mra,
    Bool do_unhide)
{
	rectangle *target_g;
	rectangle  hidden_g;
	...
	XMoveResizeWindow(dpy, FW_W_PARENT(fw), mra->b_g.top_left.width,
...
static void
frame_prepare_animation_shape(FvwmWindow *fw, mr_args_internal *mra,
    int parent_x, int parent_y)
{
	...
	gravity_move_resize_parent_child(mra->grav.parent_grav, &mra->dstep_g,
	    &parent_g);
	client_g = mra->client_g;
	frame_get_hidden_pos(fw, mra, True, &client_g);
	client_g.x += parent_g.x;
	client_g.y += parent_g.y;
	FShapeCombineShape(dpy, Scr.NoFocusWin, FShapeBounding, client_g.x,
	    client_g.y, FW_W(fw), FShapeBounding, FShapeSet);
	if (HAS_TITLE(fw)) {
		rectangle  tb_g;
		XRectangle rect;

		SUPPRESS_UNUSED_VAR_WARNING(rect);
		frame_get_titlebar_dimensions_only(fw, &mra->next_g,
		    &mra->b_no_title_g, &tb_g);
		/* windows w/ titles */
		rect.x	    = tb_g.x;
		rect.y	    = tb_g.y;
		rect.width  = tb_g.width;
		rect.height = tb_g.height;
		FShapeCombineRectangles(dpy, Scr.NoFocusWin, FShapeBounding, 0,
...
static void
frame_mrs_prepare_vars(FvwmWindow *fw, mr_args_internal *mra)
{
	Bool dummy;
	int  i;
	...
	frame_get_sidebar_geometry(fw, NULL, &mra->next_g, &mra->next_sidebar_g,
	    &dummy, &dummy);
	fvwmrect_subtract_rectangles(&mra->dstep_g, &mra->next_g,
	    &mra->current_g);
	mra->next_titlebar_compression = frame_get_titlebar_compression(fw,
	    &mra->next_g);
	mra->step_flags.is_hidden = (frame_is_parent_hidden(fw, &mra->next_g) ==
	    True);
	mra->step_flags.do_hide_parent	 = ((!mra->step_flags.was_hidden ||
						mra->flags.do_force) &&
	      mra->step_flags.is_hidden);
	mra->step_flags.do_unhide_parent = ((mra->step_flags.was_hidden ||
						mra->flags.do_force) &&
	    !mra->step_flags.is_hidden);
...
static void
frame_mrs_hide_changing_parts(mr_args_internal *mra)
{
	int l_add;
	int t_add;
	int r_add;
	...
	if (w > 0 && h > 0) {
		XMoveResizeWindow(dpy, hide_wins.w[2], 0,
		    mra->current_g.height - mra->b_g.bottom_right.height -
			b_add,
		    w, h);
	...
	h = mra->current_g.height;
	if (w > 0 && h > 0) {
		XMoveResizeWindow(dpy, hide_wins.w[3],
		    mra->current_g.width - mra->b_g.bottom_right.width - r_add,
		    0, w, h);
...
static void
frame_mrs_hide_unhide_parent(FvwmWindow *fw, mr_args_internal *mra)
{
	...
	if (mra->step_flags.do_unhide_parent) {
		...
		XChangeWindowAttributes(dpy, FW_W_PARENT(fw), CWWinGravity,
		    &xswa);
	}
...
static void
frame_mrs_hide_unhide_parent2(FvwmWindow *fw, mr_args_internal *mra)
{
	XSetWindowAttributes xswa;
...
static void
frame_mrs_setup_draw_decorations(FvwmWindow *fw, mr_args_internal *mra)
{
	...
	} else /* lazy shading */
	{
		...
		if (mra->current_step == 1) {
			...
			lazy_g.width  = max(mra->current_g.width,
			     mra->end_g.width);
			lazy_g.height = max(mra->current_g.height,
			    mra->end_g.height);
			border_draw_decorations(fw, PART_ALL,
...
static void
frame_mrs_resize_move_windows(FvwmWindow *fw, mr_args_internal *mra)
{
	...
	if (!mra->flags.is_shading) {
		if (!mra->step_flags.is_hidden &&
			...
			XMoveResizeWindow(dpy, FW_W(fw), 0, 0,
		...
		if (mra->mode == FRAME_MR_SCROLL) {
			if (mra->dstep_g.x == 0) {
				x -= mra->dstep_g.width - mra->minimal_w_offset;
			}
			if (mra->dstep_g.y == 0) {
		...
		if (mra->flags.do_update_shape) {
			...
			FShapeCombineShape(dpy, FW_W_FRAME(fw), FShapeBounding,
			    0, 0, Scr.NoFocusWin, FShapeBounding, FShapeUnion);
		}
	}
	XMoveResizeWindow(dpy, FW_W_FRAME(fw), mra->next_g.x, mra->next_g.y,
...
static void
frame_move_resize_step(FvwmWindow *fw, mr_args_internal *mra)
{
...
static void
frame_has_handles_and_tiled_border(FvwmWindow *fw, int *ret_has_handles,
    int *ret_has_tiled_border)
{
	...
	*ret_has_tiled_border = (DFS_FACE_TYPE(df->style) ==
				    TiledPixmapButton) ||
...
static int
frame_get_shading_laziness(FvwmWindow *fw, mr_args_internal *mra)
{
	int has_handles;
	...
	frame_has_handles_and_tiled_border(fw, &has_handles,
	    &has_tiled_pixmap_border);
	if (HAS_TITLE(fw) && has_tiled_pixmap_border) {
		using_border_style = border_is_using_border_style(fw,
...
void
frame_reshape_border(FvwmWindow *fw)
{
	int	   grav;
	int	   off_x = 0;
	int	   off_y = 0;
	...
	gravity_translate_to_northwest_geometry_no_bw(grav, fw, &naked_g,
	    &naked_g);
	...
	if (IS_SHADED(fw)) {
		...
		frame_force_setup_window(fw, fw->g.frame.x, fw->g.frame.y,
		    fw->g.frame.width, fw->g.frame.height, False);
	} else {
...
frame_init(void)
{
	...
	for (i = 0; i < 4; i++) {
		hide_wins.w[i] = XCreateWindow(dpy, Scr.Root, -1, -1, 1, 1, 0,
...
void
frame_destroyed_frame(Window frame_w)
{
...
void
frame_get_titlebar_dimensions(FvwmWindow *fw, rectangle *frame_g,
    rectangle *diff_g, frame_title_layout_t *title_layout)
{
	size_borders b;
	int	     i;
	int	     tb_length;
	...
	if (HAS_VERTICAL_TITLE(fw)) {
		...
		if (GET_TITLE_DIR(fw) == DIR_W) {
			...
			tb_x = frame_g->width - b.bottom_right.width - tb_thick;
			if (diff_g != NULL) {
				tb_x -= diff_g->width;
			}
		}
...
void
frame_get_sidebar_geometry(FvwmWindow *fw, DecorFaceStyle *borderstyle,
    rectangle *frame_g, rectangle *ret_g, Bool *ret_has_x_marks,
    Bool *ret_has_y_marks)
{
	int	     min_w;
	size_borders b;
...
int
frame_window_id_to_context(FvwmWindow *fw, Window w, int *ret_num)
{
	int context = C_ROOT;
	...
	*ret_num = -1;
	...
	if (w == FW_W_TITLE(fw)) {
	    ...
	    (w == FW_W(Scr.EwmhDesktop) || w == FW_W_PARENT(Scr.EwmhDesktop) ||
		w == FW_W_FRAME(Scr.EwmhDesktop))) {
		context = C_EWMH_DESKTOP;
	} else if (w == FW_W(fw) || w == FW_W_PARENT(fw) ||
	    w == FW_W_FRAME(fw)) {
		context = C_WINDOW;
...
frame_create_move_resize_args(FvwmWindow *fw, frame_move_resize_mode mr_mode,
    rectangle *start_g, rectangle *end_g, int anim_steps, int shade_dir)
{
	...
	if (!IS_SHADED(fw)) {
		rc = XGetGeometry(dpy, FW_W(fw), &JunkRoot, &mra->client_g.x,
		    &mra->client_g.y, (unsigned int *)&mra->client_g.width,
		    (unsigned int *)&mra->client_g.height,
		    (unsigned int *)&JunkBW, (unsigned int *)&JunkDepth);
		if (rc == True) {
			rc = XTranslateCoordinates(dpy, FW_W_PARENT(fw),
			    Scr.Root, mra->client_g.x, mra->client_g.y,
			    &mra->client_g.x, &mra->client_g.y, &JunkChild);
		}
		if (rc == False) {
	...
	frame_get_sidebar_geometry(fw, NULL, &mra->start_g,
	    &mra->curr_sidebar_g, &dummy, &dummy);
	mra->end_g		       = *end_g;
	mra->current_g		       = mra->start_g;
	mra->next_g		       = mra->end_g;
	mra->curr_titlebar_compression = frame_get_titlebar_compression(fw,
	    &mra->start_g);
	fvwmrect_subtract_rectangles(&mra->delta_g, &mra->end_g, &mra->start_g);
	...
	mra->step_flags.was_hidden = (frame_is_parent_hidden(fw,
					  &mra->start_g) == True);
	mra->flags.is_setup	   = (mra->mode == FRAME_MR_FORCE_SETUP ||
	...
	mra->flags.do_update_shape = (FShapesSupported &&
	    mra->flags.is_shading && fw->wShaped);
	...
	frame_get_resize_decor_gravities(&mra->grav, GET_TITLE_DIR(fw),
	    mra->mode, &mra->delta_g);
 ...
 */
void
frame_update_move_resize_args(frame_move_resize_args mr_args, rectangle *end_g)
{
	mr_args_internal *mra;
...
void
frame_free_move_resize_args(FvwmWindow *fw, frame_move_resize_args mr_args)
{
	mr_args_internal *mra;
	...
	if (mra->flags.is_lazy_shading) {
		border_draw_decorations(fw, PART_ALL,
		    (mra->w_with_focus != None) ? True : False, True, CLEAR_ALL,
		    &mra->current_g, &mra->end_g);
	} else if (mra->trans_parts != PART_NONE) {
		border_draw_decorations(fw, mra->trans_parts,
		    (mra->w_with_focus != None) ? True : False, True, CLEAR_ALL,
		    &mra->current_g, &mra->end_g);
	}
	...
	if (mra->flags.do_update_shape) {
		/* unset shape */
		FShapeCombineMask(dpy, FW_W_FRAME(fw), FShapeBounding, 0, 0,
		    None, FShapeSet);
	}
...
void
frame_move_resize(FvwmWindow *fw, frame_move_resize_args mr_args)
{
	mr_args_internal *mra;
	...
	/* animation */
	for (i = 1; i <= mra->anim_steps;
	     i++, frame_next_move_resize_args(mra)) {
		mra->current_step = i;
 ...
 ************************************************************************/
void
frame_setup_window(FvwmWindow *fw, int x, int y, int w, int h,
...
void
frame_setup_window_app_request(FvwmWindow *fw, int x, int y, int w, int h,
    Bool do_send_configure_notify)
{
	rectangle g;
...
void
frame_force_setup_window(FvwmWindow *fw, int x, int y, int w, int h,
    Bool do_send_configure_notify)
{
	rectangle g;
 ...
 ****************************************************************************/
void
frame_setup_shape(FvwmWindow *fw, int w, int h, int shape_mode)
{
	rectangle    r;
	size_borders b;
	...
	if (!shape_mode) {
		...
		get_window_borders(fw, &b);
		FShapeCombineShape(dpy, FW_W_FRAME(fw), FShapeBounding,
		    b.top_left.width, b.top_left.height, FW_W(fw),
		...
		if (FW_W_TITLE(fw)) {
			...
			SUPPRESS_UNUSED_VAR_WARNING(rect);
			FShapeCombineRectangles(dpy, FW_W_FRAME(fw),
			    FShapeBounding, 0, 0, &rect, 1, FShapeUnion,
			    Unsorted);
		}
	}
