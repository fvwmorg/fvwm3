...
static void
__update_windowlist(
    FvwmWindow *fw, fpol_set_focus_by_t set_by, int is_focus_by_flip_focus_cmd)
{
...
__activate_window_by_command(F_CMD_ARGS, int is_focus_by_flip_focus_cmd)
{
	...
	if (!do_not_warp) {
		...
		if (cx < 0 || cx >= m->virtual_scr.MyDisplayWidth || cy < 0
		    || cy >= m->virtual_scr.MyDisplayHeight) {
			int dx;
			int dy;
...
restore_focus_after_unmap(const FvwmWindow *fw, Bool do_skip_marked_transients)
{
	...
	if (focus_is_focused(fw)) {
		set_focus_to =
		    __restore_focus_after_unmap(fw, do_skip_marked_transients);
	}
...
focus_force_refresh_focus(const FvwmWindow *fw)
{
	...
	if (XGetWindowAttributes(dpy, FW_W(fw), &winattrs)) {
		XSelectInput(
		    dpy, FW_W(fw), winattrs.your_event_mask & ~FocusChangeMask);
...
void CMD_WarpToWindow(F_CMD_ARGS)
{
	...
	if (exc->w.wcontext != C_UNMANAGED) {
		if (n == 2) {
			warp_to_fvwm_window(
			    exc, val1, val1_unit, val2, val2_unit, do_raise);
		} else {
