...
void
_focus_set(Window w, FvwmWindow *fw)
{
...
static Bool
focus_get_fpol_context_flag(fpol_context_t *fpol_context, int context)
{
	int flag;
 ...
 */
static void
__try_program_focus(Window w, const FvwmWindow *fw)
{
...
static Bool
__try_forbid_user_focus(Window w, FvwmWindow *fw)
{
	if (fw == NULL ||
	...
	if (WM_TAKES_FOCUS(fw)) {
		...
		border_draw_decorations(fw, PART_ALL, False, False, CLEAR_ALL,
		    NULL, NULL);
	}
...
static Bool
__check_allow_focus(Window w, FvwmWindow *fw, fpol_set_focus_by_t set_by)
{
	FvwmWindow     *sf;
	struct monitor *m;
	...
	if (!FP_IS_LENIENT(FW_FOCUS_POLICY(fw)) &&
	    !focus_does_accept_input_focus(fw) && sf != NULL &&
	    sf->Desk == m->virtual_scr.CurrentDesk) {
...
static void
__update_windowlist(FvwmWindow *fw, fpol_set_focus_by_t set_by,
...
static Bool
__try_other_screen_focus(const FvwmWindow *fw)
{
 ...
 */
static void
__set_focus_to_fwin(Window w, FvwmWindow *fw, sftfwin_args_t *args)
{
	FvwmWindow     *sf;
	struct monitor *m;
	...
	if (fw && !args->do_forbid_warp) {
		if (IS_ICONIFIED(fw)) {
			...
			if (!rc ||
			    !IsRectangleOnThisPage(fw->m, &r, fw->Desk)) {
				fw = NULL;
				w  = Scr.NoFocusWin;
			}
		} else if (!IsRectangleOnThisPage(fw->m, &(fw->g.frame),
			       fw->Desk)) {
			fw = NULL;
			w  = Scr.NoFocusWin;
		}
...
static void
set_focus_to_fwin(Window w, FvwmWindow *fw, sftfwin_args_t *args)
{
	FvwmWindow *sf;
 ...
 */
static void
warp_to_fvwm_window(const exec_context_t *exc, int warp_x, int x_unit,
    int warp_y, int y_unit, int do_raise)
{
	int		dx, dy;
	int		cx, cy;
	int		x, y;
	...
	FWarpPointerUpdateEvpos(exc->x.elast, dpy, None, Scr.Root, 0, 0, 0, 0,
	    x, y);
	if (do_raise) {
		RaiseWindow(t, False);
	}
	...
	if (t->g.frame.x + t->g.frame.width < 0 ||
		...
		frame_setup_window(t, 0, 0, t->g.frame.width, t->g.frame.height,
		    False);
...
static Bool
focus_query_grab_buttons(FvwmWindow *fw, Bool client_entered)
{
	Bool		flag;
	Bool		is_focused;
...
static FvwmWindow *
__restore_focus_after_unmap(const FvwmWindow *fw,
    Bool				      do_skip_marked_transients)
{
	...
	if (t != NULL && FP_DO_RELEASE_FOCUS_TRANSIENT(FW_FOCUS_POLICY(fw)) &&
		...
		for (t = fw->next; t != NULL && set_focus_to == NULL;
		     t = t->next) {
			if (!FP_DO_OVERRIDE_RELEASE_FOCUS(FW_FOCUS_POLICY(t)) &&
 ...
 */
static void
__activate_window_by_command(F_CMD_ARGS, int is_focus_by_flip_focus_cmd)
{
	int		  cx;
	int		  cy;
	Bool		  do_not_warp;
	...
	if (!do_not_warp) {
		...
		if (cx < 0 || cx >= monitor_get_all_widths() || cy < 0 ||
		    cy >= monitor_get_all_heights()) {
			int dx;
			int dy;

			dx = ((cx + m->virtual_scr.Vx) /
				 monitor_get_all_widths()) *
			    monitor_get_all_widths();
			dy = ((cy + m->virtual_scr.Vy) /
				 monitor_get_all_heights()) *
			    monitor_get_all_heights();
			MoveViewport(m, dx, dy, True);
		}
...
static void
__focus_grab_one_button(FvwmWindow *fw, int button, int grab_buttons)
{
	Bool do_grab;
	...
	if (do_grab) {
		XGrabButton(dpy, button + 1, AnyModifier, FW_W_PARENT(fw), True,
...
Bool
focus_does_accept_input_focus(const FvwmWindow *fw)
{
	return (!fw || !fw->wmhints || !(fw->wmhints->flags & InputHint) ||
	    fw->wmhints->input);
}

Bool
focus_is_focused(const FvwmWindow *fw)
{
	return (fw && fw == ScreenFocus);
}

Bool
focus_query_click_to_raise(FvwmWindow *fw, Bool is_focused, int context)
{
	fpol_context_t *c;
...
Bool
focus_query_click_to_focus(FvwmWindow *fw, int context)
{
	fpol_context_t *c;
...
focus_query_open_grab_focus(FvwmWindow *fw, FvwmWindow *focus_win)
{
	...
	if (IS_TRANSIENT(fw) && FW_W_TRANSIENTFOR(fw) != Scr.Root) {
		...
		if (FP_DO_GRAB_FOCUS(FW_FOCUS_POLICY(fw)) &&
		    (focus_win == NULL ||
			!FP_DO_OVERRIDE_GRAB_FOCUS(
			    FW_FOCUS_POLICY(focus_win)))) {
			return True;
		}
	}
...
static void
__focus_grab_buttons(FvwmWindow *fw, Bool client_entered)
{
	int  i;
...
void
focus_grab_buttons(FvwmWindow *fw)
{
	__focus_grab_buttons(fw, False);
}

void
focus_grab_buttons_client_entered(FvwmWindow *fw)
{
	__focus_grab_buttons(fw, True);
}

void
focus_grab_buttons_on_layer(int layer)
{
	FvwmWindow *fw;
...
void
_SetFocusWindow(FvwmWindow *fw, Bool do_allow_force_broadcast,
...
void
_ReturnFocusWindow(FvwmWindow *fw)
{
	sftfwin_args_t sf_args;
...
void
_DeleteFocus(Bool do_allow_force_broadcast)
{
	sftfwin_args_t sf_args;
...
void
restore_focus_after_unmap(const FvwmWindow *fw, Bool do_skip_marked_transients)
{
	extern FvwmWindow *colormap_win;
	FvwmWindow	  *set_focus_to = NULL;

	if (focus_is_focused(fw)) {
		set_focus_to = __restore_focus_after_unmap(fw,
		    do_skip_marked_transients);
	}
 ...
 * if necessary */
void
focus_grab_buttons_on_pointer_window(void)
{
	Window	    w;
	FvwmWindow *fw;

	if (!FQueryPointer(dpy, Scr.Root, &JunkRoot, &w, &JunkX, &JunkY, &JunkX,
		&JunkY, &JunkMask)) {
...
void
focus_force_refresh_focus(const FvwmWindow *fw)
{
...
void
refresh_focus(const FvwmWindow *fw)
{
...
CMD_WarpToWindow(F_CMD_ARGS)
{
	...
	if (exc->w.wcontext != C_UNMANAGED) {
		...
		if (n == 2) {
			...
			if (!XGetGeometry(dpy, exc->w.w, &JunkRoot, &wx, &wy,
		...
		FWarpPointerUpdateEvpos(exc->x.elast, dpy, None, exc->w.w, 0, 0,
		    0, 0, x, y);
	}
