...
get_shaded_geometry_with_dir(
    FvwmWindow *fw, rectangle *small_g, rectangle *big_g, direction_t shade_dir)
{
...
update_relative_geometry(FvwmWindow *fw)
{
	get_relative_geometry(
	    fw, &fw->g.frame, (IS_MAXIMIZED(fw)) ? &fw->g.max : &fw->g.normal);
	if (IS_SHADED(fw)) {
    ...
    int xmotion, int ymotion, int flags)
{
	...
	if (min.height < fw->min_window_height - b.total_size.height) {
		min.height = fw->min_window_height - b.total_size.height;
	}
	...
	if (max.height > fw->max_window_height - b.total_size.height) {
		max.height = fw->max_window_height - b.total_size.height;
	}
	...
	if (!(flags & CS_ROUND_UP)) {
		d.width = ((d.width - base.width) / inc.width) * inc.width
			  + base.width;
		d.height = ((d.height - base.height) / inc.height) * inc.height
			   + base.height;
	} else {
		...
		d.width = ((d.width - base.width + inc.width - 1) / inc.width)
			  ...
			  + base.width;
		d.height =
		    ((d.height - base.height + inc.height - 1) / inc.height)
			* inc.height
		    + base.height;
	...
	if ((flags & CS_ROUND_UP) && e != NULL && e->type == MotionNotify) {
		if (xmotion > 0 && e->xmotion.x_root < round_up.width) {
			d.width -= inc.width;
			   ...
			   && e->xmotion.x_root >= m->virtual_scr.MyDisplayWidth
						       - round_up.width) {
			d.width -= inc.width;
		}
	...
	if (fw->hints.flags & PAspect) {
		__cs_handle_aspect_ratio(
		    &d, fw, d, base, inc, min, max, xmotion, ymotion, flags);
	}
...
set_icon_position(FvwmWindow *fw, int x, int y)
{
	...
	if (fw->icon_g.picture_w_g.width > 0 && !HAS_NO_ICON_TITLE(fw)) {
		fw->icon_g.title_w_g.x -=
		    (fw->icon_g.title_w_g.width - fw->icon_g.picture_w_g.width)
		    / 2;
...
void
get_page_offset_rectangle(
    FvwmWindow *fw, int *ret_page_x, int *ret_page_y, rectangle *r)
{
	struct monitor *m = (fw && fw->m) ? fw->m : monitor_get_current();
...
get_page_offset_check_visible(int *ret_page_x, int *ret_page_y, FvwmWindow *fw)
{
	...
	fprintf(stderr, "%s: MON: %s {page_x: %d, page_y: %d}\n", __func__,
	    fw->m->si->name, *ret_page_x, *ret_page_y);
