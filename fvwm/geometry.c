...
void
gravity_get_naked_geometry(int gravity, FvwmWindow *t, rectangle *dest_g,
    rectangle *orig_g)
{
	int	     xoff;
	int	     yoff;
...
void
gravity_add_decoration(int gravity, FvwmWindow *t, rectangle *dest_g,
    rectangle *orig_g)
{
	size_borders b;
	...
	get_window_borders(t, &b);
	*dest_g = *orig_g;
	gravity_resize(gravity, dest_g, b.total_size.width,
	    b.total_size.height);
...
void
get_relative_geometry(FvwmWindow *fw, rectangle *rel_g, rectangle *abs_g)
{
...
void
get_absolute_geometry(FvwmWindow *fw, rectangle *abs_g, rectangle *rel_g)
{
...
void
gravity_translate_to_northwest_geometry(int gravity, FvwmWindow *t,
    rectangle *dest_g, rectangle *orig_g)
{
	...
	dest_g->x = orig_g->x -
	    ((xoff + 1) *
		(orig_g->width - 1 + 2 * t->attr_backup.border_width)) /
		2;
	dest_g->y = orig_g->y -
	    ((yoff + 1) *
		(orig_g->height - 1 + 2 * t->attr_backup.border_width)) /
		2;
...
void
gravity_translate_to_northwest_geometry_no_bw(int gravity, FvwmWindow *t,
    rectangle *dest_g, rectangle *orig_g)
{
...
void
get_title_geometry(FvwmWindow *fw, rectangle *ret_g)
{
	size_borders b;
	size_borders nt;
	int	     w;
...
void
get_title_gravity_factors(FvwmWindow *fw, int *ret_fx, int *ret_fy)
{
	switch (GET_TITLE_DIR(fw)) {
	case DIR_N:
		*ret_fx = 0;
...
Bool
get_title_button_geometry(FvwmWindow *fw, rectangle *ret_g, int context)
{
	int bnum;
	...
	if (XGetGeometry(dpy, FW_W_BUTTON(fw, bnum), &JunkRoot, &ret_g->x,
		&ret_g->y, (unsigned int *)&ret_g->width,
		(unsigned int *)&ret_g->height, (unsigned int *)&JunkBW,
		(unsigned int *)&JunkDepth) == 0) {
		return False;
	}
	XTranslateCoordinates(dpy, FW_W_FRAME(fw), Scr.Root, ret_g->x, ret_g->y,
	    &ret_g->x, &ret_g->y, &JunkChild);

	return True;
}

void
get_title_font_size_and_offset(FvwmWindow *fw, direction_t title_dir,
    Bool is_left_title_rotated_cw, Bool is_right_title_rotated_cw,
    Bool is_top_title_rotated, Bool is_bottom_title_rotated, int *size,
    int *offset)
{
	int	   decor_size;
	int	   extra_size;
	...
	case DIR_S:
	default:
		is_rotated = (title_dir == DIR_N) ? is_top_title_rotated :
	...
	min_offset = FlocaleGetMinOffset(fw->title_font, draw_rotation);
...
void
get_icon_corner(FvwmWindow *fw, rectangle *ret_g)
{
	switch (GET_TITLE_DIR(fw)) {
	case DIR_N:
	case DIR_W:
	...
	case DIR_S:
		ret_g->x = fw->g.frame.x;
		ret_g->y = fw->g.frame.y + fw->g.frame.height - ret_g->height;
		break;
	case DIR_E:
		ret_g->x = fw->g.frame.x + fw->g.frame.width - ret_g->width;
		ret_g->y = fw->g.frame.y;
		break;
	}
...
void
get_shaded_geometry(FvwmWindow *fw, rectangle *small_g, rectangle *big_g)
{
	size_borders b;
...
void
get_shaded_geometry_with_dir(FvwmWindow *fw, rectangle *small_g,
    rectangle *big_g, direction_t shade_dir)
{
...
void
get_unshaded_geometry(FvwmWindow *fw, rectangle *ret_g)
{
	if (IS_SHADED(fw)) {
		if (IS_MAXIMIZED(fw)) {
			*ret_g = fw->g.max;
...
void
get_shaded_client_window_pos(FvwmWindow *fw, rectangle *ret_g)
{
	rectangle    big_g;
	size_borders b;
...
void
get_window_borders(const FvwmWindow *fw, size_borders *borders)
{
	...
	borders->total_size.width = borders->top_left.width +
	    borders->bottom_right.width;
	borders->total_size.height = borders->top_left.height +
	    borders->bottom_right.height;
...
void
get_window_borders_no_title(const FvwmWindow *fw, size_borders *borders)
{
	borders->top_left.width	     = fw->boundary_width;
	borders->bottom_right.width  = fw->boundary_width;
	borders->top_left.height     = fw->boundary_width;
...
void
set_window_border_size(FvwmWindow *fw, int used_width)
{
	if (used_width <= 0) {
		fw->boundary_width	    = 0;
...
Bool
is_window_border_minimal(FvwmWindow *fw)
{
	size_borders nt;
...
void
get_client_geometry(FvwmWindow *fw, rectangle *ret_g)
{
 ...
 * state */
void
update_relative_geometry(FvwmWindow *fw)
{
	get_relative_geometry(fw, &fw->g.frame,
	    (IS_MAXIMIZED(fw)) ? &fw->g.max : &fw->g.normal);
	if (IS_SHADED(fw)) {
		get_shaded_geometry(fw, &fw->g.frame, &fw->g.frame);
	}
...
void
update_absolute_geometry(FvwmWindow *fw)
{
	rectangle      *dest_g;
	rectangle	frame_g;
	struct monitor *m;
 ...
 * more apart. */
void
maximize_adjust_offset(FvwmWindow *fw)
{
	int off_x;
	int off_y;
	int dh;
...
static void
__cs_handle_aspect_ratio(size_rect *ret_s, FvwmWindow *fw, size_rect s,
    const size_rect base, const size_rect inc, size_rect min, size_rect max,
    int xmotion, int ymotion, int flags)
{
	...
	do {
		...
		if (r < rmin) {
			...
			delta = MAKEMULT(s.height - s.width / rmin, inc.height);
			if (s.height - delta >= min.height) {
				s.height -= delta;
			} else {
				delta = MAKEMULT(s.height * rmin - s.width,
				    inc.width);
				if (s.width + delta <= max.width) {
					s.width += delta;
				}
		...
		if (r > rmax) {
			...
			if (s.width - delta >= min.width) {
				...
				delta = MAKEMULT(s.width / rmax - s.height,
				    inc.height);
				if (s.height + delta <= max.height) {
					s.height += delta;
				}
 ...
 */
void
constrain_size(FvwmWindow *fw, const XEvent *e, int *widthp, int *heightp,
    int xmotion, int ymotion, int flags)
{
	int	     tmp;
	window_style style;
	...
	if (!(flags & CS_ROUND_UP)) {
		d.width = ((d.width - base.width) / inc.width) * inc.width +
		    base.width;
		...
		d.width = ((d.width - base.width + inc.width - 1) / inc.width) *
			inc.width +
		    base.width;
	...
	if (fw->hints.flags & PAspect) {
		__cs_handle_aspect_ratio(&d, fw, d, base, inc, min, max,
		    xmotion, ymotion, flags);
	}
 ...
 */
void
gravity_constrain_size(int gravity, FvwmWindow *t, rectangle *rect, int flags)
{
	int new_width  = rect->width;
	int new_height = rect->height;

	if (IS_MAXIMIZED(t) && (flags & CS_UPDATE_MAX_DEFECT)) {
		gravity_resize(gravity, rect, t->g.max_defect.width,
	...
	constrain_size(t, NULL, &new_width, &new_height, 0, 0, flags);
	if (rect->width != new_width || rect->height != new_height) {
		gravity_resize(gravity, rect, new_width - rect->width,
		    new_height - rect->height);
	}
...
Bool
get_visible_icon_title_geometry(FvwmWindow *fw, rectangle *ret_g)
{
...
Bool
get_icon_title_geometry(FvwmWindow *fw, rectangle *ret_g)
{
...
Bool
get_visible_icon_picture_geometry(FvwmWindow *fw, rectangle *ret_g)
{
	if (fw->icon_g.picture_w_g.width == 0 || IS_ICON_UNMAPPED(fw) ||
	    !IS_ICONIFIED(fw)) {
		memset(ret_g, 0, sizeof(*ret_g));
		return False;
	}
...
Bool
get_icon_picture_geometry(FvwmWindow *fw, rectangle *ret_g)
{
...
Bool
get_visible_icon_geometry(FvwmWindow *fw, rectangle *ret_g)
{
...
void
get_icon_geometry(FvwmWindow *fw, rectangle *ret_g)
{
	/* valid geometry? */
	if (fw->icon_g.picture_w_g.width > 0) {
		*ret_g = fw->icon_g.picture_w_g;
	...
	/* valid position? */
	else if (fw->icon_g.picture_w_g.x != 0 ||
...
Bool
get_visible_window_or_icon_geometry(FvwmWindow *fw, rectangle *ret_g)
{
	if (IS_ICONIFIED(fw)) {
...
void
move_icon_to_position(FvwmWindow *fw)
{
	...
	if (!HAS_NO_ICON_TITLE(fw)) {
		...
		XMoveWindow(dpy, FW_W_ICON_TITLE(fw), fw->icon_g.title_w_g.x,
	...
	if (draw_title_w || draw_picture_w) {
		DrawIconWindow(fw, draw_title_w, draw_picture_w, False,
		    draw_picture_w, NULL);
	}
...
void
broadcast_icon_geometry(FvwmWindow *fw, Bool do_force)
{
	rectangle g;
	Bool	  rc;
	...
	if (rc == True && (!IS_ICON_UNMAPPED(fw) || do_force == True)) {
		BroadcastPacket(M_ICON_LOCATION, 7, (long)FW_W(fw),
		    (long)FW_W_FRAME(fw), (unsigned long)fw, (long)g.x,
		    (long)g.y, (long)g.width, (long)g.height);
	}
...
void
modify_icon_position(FvwmWindow *fw, int dx, int dy)
{
 ...
 * layout */
void
set_icon_position(FvwmWindow *fw, int x, int y)
{
	if (fw->icon_g.picture_w_g.width > 0) {
	...
	if (fw->icon_g.picture_w_g.width > 0 && !HAS_NO_ICON_TITLE(fw)) {
		fw->icon_g.title_w_g.x -= (fw->icon_g.title_w_g.width -
					      fw->icon_g.picture_w_g.width) /
		    2;
		fw->icon_g.title_w_g.y += fw->icon_g.picture_w_g.height;
...
void
set_icon_picture_size(FvwmWindow *fw, int w, int h)
{
	if (fw->icon_g.picture_w_g.width > 0) {
...
void
resize_icon_title_height(FvwmWindow *fw, int dh)
{
	if (!HAS_NO_ICON_TITLE(fw)) {
...
void
get_page_offset_rectangle(FvwmWindow *fw, int *ret_page_x, int *ret_page_y,
    rectangle *r)
{
	struct monitor *m = (fw && fw->m) ? fw->m : monitor_get_current();
	...
	*ret_page_x = truncate_to_multiple(r->x + r->width / 2 + xoff,
			  monitor_get_all_widths()) -
	    xoff;
	*ret_page_y = truncate_to_multiple(r->y + r->height / 2 + yoff,
...
void
get_page_offset(int *ret_page_x, int *ret_page_y, FvwmWindow *fw)
{
	rectangle r;
...
void
get_page_offset_check_visible(int *ret_page_x, int *ret_page_y, FvwmWindow *fw)
{
