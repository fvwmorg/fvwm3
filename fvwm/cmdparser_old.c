...
static void
ocp_debug(cmdparser_context_t *c, const char *msg)
{
	int i;
...
static int
ocp_create_context(cmdparser_context_t *dest_c, cmdparser_context_t *caller_c,
    char *line, char *all_pos_args_string, char *pos_arg_tokens[])
{
	...
	if (all_pos_args_string != NULL) {
		...
		for (i = 0;
		     i < CMDPARSER_NUM_POS_ARGS && pos_arg_tokens[i] != NULL;
		     i++) {
...
static void
ocp_destroy_context(cmdparser_context_t *c)
{
...
static int
ocp_handle_line_start(cmdparser_context_t *c)
{
	if (OCP_DEBUG) {
...
static cmdparser_prefix_flags_t
ocp_handle_line_prefix(cmdparser_context_t *c)
{
	...
	if (OCP_DEBUG) {
		fvwm_debug(__func__, "cl '%s', tok '%s', rl '%s'", c->cline,
		    token, restofline);
	}
	while (token != NULL) {
		if (OCP_DEBUG) {
...
static int
ocp_is_module_config(cmdparser_context_t *c)
{
	return (c->command != NULL && *c->command == '*') ? 1 : 0;
}

static const char *
ocp_parse_command_name(cmdparser_context_t *c, void *func_rc, const void *exc)
{
	GetNextToken(c->cline, &c->command);
	if (OCP_DEBUG) {
		fvwm_debug(__func__, "c->command '%s'",
		    (c->command) ? c->command : "(nil)");
	}
	if (c->command != NULL) {
		char *tmp = c->command;
		...
		c->command = expand_vars(c->command, c, False, False, func_rc,
		    exc);
		if (OCP_DEBUG) {
			fvwm_debug(__func__, "c->command '%s'",
			    (c->command) ? c->command : "(nil)");
...
static void
ocp_expand_command_line(cmdparser_context_t *c, int is_addto, void *func_rc,
    const void *exc)
{
	c->expline = expand_vars(c->cline, c, is_addto, ocp_is_module_config(c),
	    func_rc, exc);
	c->cline   = c->expline;
	c->do_free_expline = 1;
...
static void
ocp_release_expanded_line(cmdparser_context_t *c)
{
	c->do_free_expline = 0;
...
static cmdparser_execute_type_t
ocp_find_something_to_execute(cmdparser_context_t *c,
    const func_t **ret_builtin, FvwmFunction **ret_complex_function)
{
	...
	*ret_complex_function = NULL;
	...
	if (*ret_builtin != NULL) {
		if ((*ret_builtin)->func_c == F_FUNCTION) {
			...
			if (OCP_DEBUG) {
				fvwm_debug(__func__, "func cline '%s'",
				    c->cline);
			}
			free(c->command);
			c->command	    = NULL;
	...
	}
	{
		...
		if (OCP_DEBUG) {
			fvwm_debug(__func__, "lookup cf '%s'",
			    complex_function_name);
		}
		*ret_complex_function = find_complex_function(
		    complex_function_name);
		if (OCP_DEBUG) {
			fvwm_debug(__func__, "lookup cf '%s' -> %p",
...
static cmdparser_hooks_t old_parser_hooks = { ocp_create_context,
	ocp_handle_line_start, ocp_handle_line_prefix, ocp_parse_command_name,
	ocp_is_module_config, ocp_expand_command_line,
	ocp_release_expanded_line, ocp_find_something_to_execute,
	ocp_destroy_context, ocp_debug };

/* ---------------------------- interface functions ------------------------ */

/* return the hooks structure of the old parser */
const cmdparser_hooks_t *
cmdparser_old_get_hooks(void)
{
