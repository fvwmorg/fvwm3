...
static Bool
__validate_titleformat_string(const char *formatstr)
{
	const char *fmt;
...
static int
blockor(char *dest, char *blk1, char *blk2, int length)
{
	int  i;
	char result = 0;
...
static int
blockand(char *dest, char *blk1, char *blk2, int length)
{
	int  i;
	char result = 0;
...
static int
blockunmask(char *dest, char *blk1, char *blk2, int length)
{
	int  i;
...
static int
blockissubset(char *sub, char *super, int length)
{
	int i;
...
static int
blocksintersect(char *blk1, char *blk2, int length)
{
	int i;
...
static int
style_ids_are_equal(style_id_t *a, style_id_t *b)
{
...
static int
style_id_equals_id(window_style *s, style_id_t *id)
{
	return style_ids_are_equal(&SGET_ID(*s), id);
}

static int
styles_have_same_id(window_style *s, window_style *t)
{
	return style_ids_are_equal(&SGET_ID(*s), &SGET_ID(*t));
}

static int
fw_match_style_id(FvwmWindow *fw, style_id_t s_id)
{
...
static int
one_fw_can_match_both_ids(window_style *s, window_style *t)
{
...
static void
remove_icon_boxes_from_style(window_style *pstyle)
{
...
static void
copy_icon_boxes(icon_boxes **pdest, icon_boxes *src)
{
...
static int
Get_TBLR(char *token, unsigned char *IconFill)
{
	/* init */
	if (StrEquals(token, "B") || StrEquals(token, "BOT") ||
	    StrEquals(token, "BOTTOM")) {
		/* turn on bottom and verical */
		*IconFill = ICONFILLBOT | ICONFILLHRZ;
	} else if (StrEquals(token, "T") || StrEquals(token, "TOP")) {
		/* turn on vertical */
		*IconFill = ICONFILLHRZ;
	} else if (StrEquals(token, "R") || StrEquals(token, "RGT") ||
	    StrEquals(token, "RIGHT")) {
		/* turn on right bit */
		*IconFill = ICONFILLRGT;
	} else if (StrEquals(token, "L") || StrEquals(token, "LFT") ||
...
static void
cleanup_style_defaults(window_style *style)
{
	int   i;
	char *dflt;
	char *mask;
...
static void
merge_styles(window_style *merged_style, window_style *add_style,
    Bool do_free_src_and_alloc_copy)
{
	...
	if (add_style->flag_mask.has_icon) {
		if (do_free_src_and_alloc_copy) {
			...
			SSET_ICON_NAME(*merged_style,
				...
				fxstrdup(SGET_ICON_NAME(*add_style)) :
				NULL);
		} else {
			SSET_ICON_NAME(*merged_style,
			    SGET_ICON_NAME(*add_style));
	...
	if (S_HAS_ICON_FONT(SCF(*add_style))) {
		if (do_free_src_and_alloc_copy) {
			...
			SSET_ICON_FONT(*merged_style,
				...
				fxstrdup(SGET_ICON_FONT(*add_style)) :
				NULL);
		} else {
			SSET_ICON_FONT(*merged_style,
			    SGET_ICON_FONT(*add_style));
	...
	if (add_style->flags.has_icon_size_limits) {
		...
		SSET_ICON_RESIZE_TYPE(*merged_style,
		    SGET_ICON_RESIZE_TYPE(*add_style));
	}
	if (add_style->flags.has_min_window_size) {
		SSET_MIN_WINDOW_WIDTH(*merged_style,
		    SGET_MIN_WINDOW_WIDTH(*add_style));
		SSET_MIN_WINDOW_WIDTH_IS_C(*merged_style,
		    SGET_MIN_WINDOW_WIDTH_IS_C(*add_style));
		SSET_MIN_WINDOW_HEIGHT(*merged_style,
		    SGET_MIN_WINDOW_HEIGHT(*add_style));
		SSET_MIN_WINDOW_HEIGHT_IS_C(*merged_style,
		    SGET_MIN_WINDOW_HEIGHT_IS_C(*add_style));
	}
	if (add_style->flags.has_max_window_size) {
		SSET_MAX_WINDOW_WIDTH(*merged_style,
		    SGET_MAX_WINDOW_WIDTH(*add_style));
		SSET_MAX_WINDOW_WIDTH_IS_C(*merged_style,
		    SGET_MAX_WINDOW_WIDTH_IS_C(*add_style));
		SSET_MAX_WINDOW_HEIGHT(*merged_style,
		    SGET_MAX_WINDOW_HEIGHT(*add_style));
		SSET_MAX_WINDOW_HEIGHT_IS_C(*merged_style,
		    SGET_MAX_WINDOW_HEIGHT_IS_C(*add_style));
	}
	if (add_style->flags.has_icon_background_relief) {
		SSET_ICON_BACKGROUND_RELIEF(*merged_style,
		    SGET_ICON_BACKGROUND_RELIEF(*add_style));
	}
	if (add_style->flags.has_icon_background_padding) {
		SSET_ICON_BACKGROUND_PADDING(*merged_style,
		    SGET_ICON_BACKGROUND_PADDING(*add_style));
	}
	if (add_style->flags.has_icon_title_relief) {
		SSET_ICON_TITLE_RELIEF(*merged_style,
		    SGET_ICON_TITLE_RELIEF(*add_style));
	}
	if (add_style->flags.has_window_shade_steps) {
		SSET_WINDOW_SHADE_STEPS(*merged_style,
		    SGET_WINDOW_SHADE_STEPS(*add_style));
	}
	if (add_style->flags.has_snap_attraction) {
		SSET_SNAP_PROXIMITY(*merged_style,
		    SGET_SNAP_PROXIMITY(*add_style));
		SSET_SNAP_MODE(*merged_style, SGET_SNAP_MODE(*add_style));
	}
	if (add_style->flags.has_snap_grid) {
		SSET_SNAP_GRID_X(*merged_style, SGET_SNAP_GRID_X(*add_style));
		SSET_SNAP_GRID_Y(*merged_style, SGET_SNAP_GRID_Y(*add_style));
	}
	if (add_style->flags.has_edge_delay_ms_move) {
		SSET_EDGE_DELAY_MS_MOVE(*merged_style,
		    SGET_EDGE_DELAY_MS_MOVE(*add_style));
	}
	if (add_style->flags.has_edge_delay_ms_resize) {
		SSET_EDGE_DELAY_MS_RESIZE(*merged_style,
		    SGET_EDGE_DELAY_MS_RESIZE(*add_style));
	}
	if (add_style->flags.has_edge_resistance_move) {
		SSET_EDGE_RESISTANCE_MOVE(*merged_style,
		    SGET_EDGE_RESISTANCE_MOVE(*add_style));
	}
	if (add_style->flags.has_edge_resistance_screen_move) {
		SSET_EDGE_RESISTANCE_SCREEN_MOVE(*merged_style,
	...
	if (SHAS_ICON_BOXES(&add_style->flag_mask)) {
		...
		if (do_free_src_and_alloc_copy) {
			...
			copy_icon_boxes(&SGET_ICON_BOXES(*merged_style),
			    SGET_ICON_BOXES(*add_style));
		} else {
			SSET_ICON_BOXES(*merged_style,
			    SGET_ICON_BOXES(*add_style));
		}
	}
	...
	if (add_style->flags.has_placement_position_string) {
		if (do_free_src_and_alloc_copy) {
			...
			SSET_PLACEMENT_POSITION_STRING(*merged_style,
	...
	if (add_style->flags.has_initial_map_command_string) {
		if (do_free_src_and_alloc_copy) {
			SAFEFREE(
			    SGET_INITIAL_MAP_COMMAND_STRING(*merged_style));
			SSET_INITIAL_MAP_COMMAND_STRING(*merged_style,
			    strdup(
...
static void
free_style_mask(window_style *style, style_flags *mask)
{
...
static void
add_style_to_list(window_style *new_style)
{
...
static void
remove_style_from_list(window_style *style, Bool do_free_style)
{
	window_style *prev;
	window_style *next;
...
static int
remove_all_of_style_from_list(style_id_t style_id)
{
...
__simplify_style_list(void)
{
	...
	for (cur = last_style_in_list, has_modified = 0; cur;
		...
		while (cmp) {
			...
			if (blockissubset((char *)&cmp->flag_mask,
				(char *)&sumflags, sizeof(style_flags)) &&
			    blockissubset((char *)&cmp->flag_default,
				(char *)&sumdflags, sizeof(style_flags))) {
			 ...
			 * style */
			blockor((char *)&dummyflags, (char *)&sumdflags,
			    (char *)&sumflags, sizeof(style_flags));
			free_style_mask(cmp, &dummyflags);
			if (!blocksintersect((char *)&cmp->flag_mask,
				(char *)&interflags, sizeof(style_flags)) &&
			    !blocksintersect((char *)&cmp->flag_default,
				(char *)&interflags, sizeof(style_flags))) {
				...
				blockor((char *)&sumflags, (char *)&sumflags,
				    (char *)&cmp->flag_mask,
				    sizeof(style_flags));
				blockor((char *)&sumdflags, (char *)&sumflags,
				    (char *)&cmp->flag_default,
				    sizeof(style_flags));
				...
				cmp	     = tmp;
				has_modified = 1;
			} else if (!blocksintersect((char *)&cur->flag_mask,
				       (char *)&interflags,
				       sizeof(style_flags)) &&
			    !blocksintersect((char *)&cur->flag_default,
				(char *)&interflags, sizeof(style_flags))) {
...
static void
style_set_old_focus_policy(window_style *ps, int policy)
{
	focus_policy_t fp;
	...
	if (!FP_DO_FOCUS_BY_PROGRAM(S_FOCUS_POLICY(SCM(*ps)))) {
		FPS_FOCUS_BY_PROGRAM(S_FOCUS_POLICY(SCF(*ps)),
	...
	if (!FP_DO_RELEASE_FOCUS(S_FOCUS_POLICY(SCM(*ps)))) {
		FPS_RELEASE_FOCUS(S_FOCUS_POLICY(SCF(*ps)),
	...
	if (!FP_DO_RAISE_FOCUSED_CLIENT_CLICK(S_FOCUS_POLICY(SCM(*ps)))) {
		FPS_RAISE_FOCUSED_CLIENT_CLICK(S_FOCUS_POLICY(SCF(*ps)),
	...
	if (!FP_DO_RAISE_UNFOCUSED_CLIENT_CLICK(S_FOCUS_POLICY(SCM(*ps)))) {
		FPS_RAISE_UNFOCUSED_CLIENT_CLICK(S_FOCUS_POLICY(SCF(*ps)),
	...
	if (!FP_DO_RAISE_FOCUSED_DECOR_CLICK(S_FOCUS_POLICY(SCM(*ps)))) {
		FPS_RAISE_FOCUSED_DECOR_CLICK(S_FOCUS_POLICY(SCF(*ps)),
	...
	if (!FP_DO_RAISE_UNFOCUSED_DECOR_CLICK(S_FOCUS_POLICY(SCM(*ps)))) {
		FPS_RAISE_UNFOCUSED_DECOR_CLICK(S_FOCUS_POLICY(SCF(*ps)),
	...
	if (!FP_DO_PASS_FOCUS_CLICK(S_FOCUS_POLICY(SCM(*ps)))) {
		FPS_PASS_FOCUS_CLICK(S_FOCUS_POLICY(SCF(*ps)),
	...
	if (!FP_DO_PASS_RAISE_CLICK(S_FOCUS_POLICY(SCM(*ps)))) {
		FPS_PASS_RAISE_CLICK(S_FOCUS_POLICY(SCF(*ps)),
	...
	if (!FP_DO_ALLOW_FUNC_FOCUS_CLICK(S_FOCUS_POLICY(SCM(*ps)))) {
		FPS_ALLOW_FUNC_FOCUS_CLICK(S_FOCUS_POLICY(SCF(*ps)),
	...
	if (!FP_DO_ALLOW_FUNC_RAISE_CLICK(S_FOCUS_POLICY(SCM(*ps)))) {
		FPS_ALLOW_FUNC_RAISE_CLICK(S_FOCUS_POLICY(SCF(*ps)),
	...
	if (!FP_DO_WARP_POINTER_ON_FOCUS_FUNC(S_FOCUS_POLICY(SCM(*ps)))) {
		FPS_WARP_POINTER_ON_FOCUS_FUNC(S_FOCUS_POLICY(SCF(*ps)),
	...
	if (!FP_DO_SORT_WINDOWLIST_BY(S_FOCUS_POLICY(SCM(*ps)))) {
		FPS_SORT_WINDOWLIST_BY(S_FOCUS_POLICY(SCF(*ps)),
...
static char *
style_parse_button_style(window_style *ps, char *button_string, int on)
{
	int   button;
	char *rest;
...
static Bool
style_parse_focus_policy_style(char *option, char *rest, char **ret_rest,
    Bool is_reversed, focus_policy_t *f, focus_policy_t *m, focus_policy_t *c)
{
	char *optlist[] = { "SortWindowlistByFocus", "FocusClickButtons",
		"FocusClickModifiers", "ClickRaisesFocused",
		"ClickDecorRaisesFocused", "ClickIconRaisesFocused",
		"ClickRaisesUnfocused", "ClickDecorRaisesUnfocused",
		"ClickIconRaisesUnfocused", "ClickToFocus", "ClickDecorToFocus",
		"ClickIconToFocus", "EnterToFocus", "LeaveToUnfocus",
		"FocusByProgram", "FocusByFunction",
		...
		"AllowRaiseClickFunction", "GrabFocus", "GrabFocusTransient",
		"OverrideGrabFocus", "ReleaseFocus", "ReleaseFocusTransient",
		"OverrideReleaseFocus", NULL };
	Bool  found;
	int   val;
	int   index;
	char *token;
	...
	case 0:
		...
		FPS_SORT_WINDOWLIST_BY(*f,
		    (val) ? FPOL_SORT_WL_BY_FOCUS : FPOL_SORT_WL_BY_OPEN);
	...
	case 1:
		...
		if (token != NULL && *token != 0) {
			fvwm_debug(__func__, "illegal mouse button '%c'",
			    *token);
...
static char *
style_parse_icon_size_style(char *option, char *rest, window_style *ps)
{
	int vals[4];
	int i;
...
static char *
style_parse_icon_box_style(icon_boxes **ret_ib, char *option, char *rest,
    window_style *ps)
{
	...
	if (num == 4) {
		...
		if (l > 0 && l < 24) {
			...
			geom_flags = FScreenParseGeometryWithScreen(option,
			    &IconBoxes->IconBox[0], &IconBoxes->IconBox[1],
			...
			if (width == 0 || !(geom_flags & WidthValue)) {
				...
				if (geom_flags & YNegative) {
					IconBoxes->IconBox[1] =
					    /* neg y coord */
					    IconBoxes->IconBox[1] - height - 2;
					/* save for later */
					IconBoxes->IconSign[1] = '-';
					IconBoxes->IconSign[3] = '-';
				}
				/* x + wid = right x */
				IconBoxes->IconBox[2] = width +
				    IconBoxes->IconBox[0];
				/* y + height = bottom y */
				IconBoxes->IconBox[3] = height +
				    IconBoxes->IconBox[1];
...
static char *
style_parse_icon_grid_style(char *option, char *rest, window_style *ps,
    icon_boxes *ib)
{
	int val[4];
	int num;
...
static char *
style_parse_icon_fill_style(char *option, char *rest, window_style *ps,
    icon_boxes *ib)
{
...
static int
style_set_border_colorset(window_style *ps, char *rest, int type)
{
	int vals[BP_SIZE];
	int this_many;
	int this_cs;
...
static Bool
style_parse_one_style_option(char *token, char *rest, char **ret_rest,
    char *prefix, window_style *ps, icon_boxes **cur_ib)
{
	...
	case 'c':
		if (StrEquals(token, "CascadePlacement")) {
			...
			*val = -1;
			...
			FPS_RAISE_FOCUSED_CLIENT_CLICK(S_FOCUS_POLICY(SCF(*ps)),
			    on);
			FPS_RAISE_FOCUSED_CLIENT_CLICK(S_FOCUS_POLICY(SCM(*ps)),
			    1);
			FPS_RAISE_FOCUSED_CLIENT_CLICK(S_FOCUS_POLICY(SCC(*ps)),
			    1);
			FPS_RAISE_UNFOCUSED_CLIENT_CLICK(S_FOCUS_POLICY(
							     SCF(*ps)),
			    on);
			FPS_RAISE_UNFOCUSED_CLIENT_CLICK(S_FOCUS_POLICY(
							     SCM(*ps)),
	...
	case 'f':
		if (strncasecmp(token, "fp", 2) == 0) {
			...
			found = style_parse_focus_policy_style(token + 2, rest,
			    &rest, (on) ? False : True,
	...
	case 'h':
		if (StrEquals(token, "HintOverride")) {
			...
			*val = -1;
			...
			if (style_set_border_colorset(ps, rest, BS_BC_HI) !=
			    0) {
				fvwm_debug(__func__,
				    "HilightBorderColorset can "
	...
	case 'm':
		if (StrEquals(token, "ManualPlacement")) {
			...
			num = 0;
			if (on != 0 && rest != NULL) {
				num = sscanf(rest, "%f %f %f %f %f %f", &f[0],
				    &f[1], &f[2], &f[3], &f[4], &f[5]);
				for (i = 0; i < num; i++) {
					PeekToken(rest, &rest);
					if (f[i] < 0) {
			...
			num = GetSuffixedIntegerArguments(rest, &rest, val, 2,
			    "pc", tmpno);
			if (num != 2) {
				val[0] = 0;
				val[1] = 0;
			...
			int   i;
			char *methodlist[] = { "AutoDetect", "UseGravity",
				"IgnoreGravity", NULL };
			...
			SCR_MOTION_METHOD(
			    &ps->flag_mask) = WS_CR_MOTION_METHOD_MASK;
			SCR_MOTION_METHOD(
			    &ps->change_mask) = WS_CR_MOTION_METHOD_MASK;
		} else {
			found = False;
		}
		break;
	...
	case 's':
		if (StrEquals(token, "SMARTPLACEMENT")) {
			...
			SSET_LAYER(*ps,
			    (on) ? Scr.BottomLayer : Scr.DefaultLayer);
			ps->flags.use_layer	  = 1;
			ps->flag_mask.use_layer	  = 1;
			ps->change_mask.use_layer = 1;
			...
			if (spargs == 1) {
				...
				fvwm_debug(__func__, "bad StartsOnDesk arg: %s",
				    rest);
			}
		}
		/* StartsOnPage is like StartsOnDesk-Plus */
			...
			char *rest2;
			spargs = GetIntegerArguments(rest, &rest2, tmpno, 3);
			...
			if (spargs == 2 || spargs == 3) {
				if (spargs == 3) {
					 ...
					 * special case of -1  */
					SSET_START_PAGE_X(*ps,
					    (tmpno[1] > -1) ? tmpno[1] + 1 :
							      tmpno[1]);
					SSET_START_PAGE_Y(*ps,
					    (tmpno[2] > -1) ? tmpno[2] + 1 :
							      tmpno[2]);
				} else {
					SSET_START_PAGE_X(*ps,
			...
			if (spargs == 1 && tmpno[0] >= 0 && tmpno[0] <= 31) {
				...
				fvwm_debug(__func__, "bad State arg: %s", rest);
			}
			...
			do {
				...
				if (GetIntegerArguments(rest, &rest, &ival,
					1) != 1) {
					break;
				}
				if (ival >= 0) {
	...
	case 'w':
		if (StrEquals(token, "WindowListSkip")) {
			...
			S_SET_WINDOWSHADE_LAZINESS(SCM(*ps),
			    WINDOWSHADE_LAZY_MASK);
			S_SET_WINDOWSHADE_LAZINESS(SCC(*ps),
			    WINDOWSHADE_LAZY_MASK);
		} else if (StrEquals(token, "WindowShadeAlwaysLazy")) {
			S_SET_WINDOWSHADE_LAZINESS(SCF(*ps),
...
static void
parse_and_set_window_style(char *action, char *prefix, window_style *ps)
{
	char *option;
	char *token;
	char *rest;
	...
	while (action && *action && *action != '\n') {
		...
		found = style_parse_one_style_option(token, rest, &rest, prefix,
		    ps, &cur_ib);

		if (found == False) {
			fvwm_debug(__func__, "Bad style option: %s", option);
			...
			if (*rest) {
				fvwm_debug(__func__,
				    "Unconsumed argument in %s: %s", option,
				    rest);
			}
		}
		free(option);
...
static void
__style_command(F_CMD_ARGS, char *prefix, Bool is_window_style)
{
...
void
check_window_style_change(FvwmWindow *t, update_win *flags,
    window_style *ret_style)
{
	...
	/*  is_fixed */
	if (S_IS_FIXED(SCC(*ret_style)) || S_IS_FIXED_PPOS(SCC(*ret_style)) ||
...
void
update_window_color_style(FvwmWindow *fw, window_style *pstyle)
{
	int cs = 0;
...
void
update_window_color_hi_style(FvwmWindow *fw, window_style *pstyle)
{
	int cs = 0;
...
print_styles(int verbose)
{
	...
	for (nptr = all_styles; nptr != NULL; nptr = SGET_NEXT_STYLE(*nptr)) {
		...
		if (SGET_ICON_FONT(*nptr)) {
			...
			if (verbose > 1) {
				fvwm_debug(__func__, "        Icon Font: %s\n",
				    SGET_ICON_FONT(*nptr));
			}
		}
		if (SGET_ICON_NAME(*nptr)) {
			...
			if (verbose > 1) {
				fvwm_debug(__func__, "        Icon Name: %s\n",
				    SGET_ICON_NAME(*nptr));
			}
		}
...
void
CMD_DestroyWindowStyle(F_CMD_ARGS)
{
	style_id_t s_id;
