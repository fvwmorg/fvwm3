...
fw_match_style_id(FvwmWindow *fw, style_id_t s_id)
{
	if (SID_GET_HAS_NAME(s_id)) {
		...
		if (fw->style_name != NULL
		    && matchWildcards(SID_GET_NAME(s_id), fw->style_name)
			   == 1) {
			return 1;
		}
	}
    ...
    Bool do_free_src_and_alloc_copy)
{
	...
	if (add_style->flag_mask.has_color_fore) {
		if (do_free_src_and_alloc_copy) {
			...
			SSET_FORE_COLOR_NAME(
			    *merged_style, SGET_FORE_COLOR_NAME(*add_style));
		}
	}
	if (add_style->flag_mask.has_color_back) {
		if (do_free_src_and_alloc_copy) {
			...
			SSET_BACK_COLOR_NAME(
			    *merged_style, SGET_BACK_COLOR_NAME(*add_style));
		}
	}
	...
	if (add_style->flags.has_edge_resistance_xinerama_move) {
		SSET_EDGE_RESISTANCE_XINERAMA_MOVE(*merged_style,
...
style_set_old_focus_policy(window_style *ps, int policy)
{
	...
	if (!FP_DO_ALLOW_FUNC_FOCUS_CLICK(S_FOCUS_POLICY(SCM(*ps)))) {
		FPS_ALLOW_FUNC_FOCUS_CLICK(
		    S_FOCUS_POLICY(SCF(*ps)), FP_DO_ALLOW_FUNC_FOCUS_CLICK(fp));
	...
	if (!FP_DO_ALLOW_FUNC_RAISE_CLICK(S_FOCUS_POLICY(SCM(*ps)))) {
		FPS_ALLOW_FUNC_RAISE_CLICK(
		    S_FOCUS_POLICY(SCF(*ps)), FP_DO_ALLOW_FUNC_RAISE_CLICK(fp));
	...
	if (!FP_DO_SORT_WINDOWLIST_BY(S_FOCUS_POLICY(SCM(*ps)))) {
		FPS_SORT_WINDOWLIST_BY(
		    S_FOCUS_POLICY(SCF(*ps)), FP_DO_SORT_WINDOWLIST_BY(fp));
...
style_parse_button_style(window_style *ps, char *button_string, int on)
{
	...
	if (button < 0 || button >= NUMBER_OF_TITLE_BUTTONS) {
		fvwm_debug(
		    __func__, "Button and NoButton styles require an argument");
	} else {
		if (on) {
    ...
    Bool is_reversed, focus_policy_t *f, focus_policy_t *m, focus_policy_t *c)
{
	char *optlist[] = {"SortWindowlistByFocus", "FocusClickButtons",
	    ...
	    "FocusByProgram", "FocusByFunction", "FocusByFunctionWarpPointer",
	    "Lenient", "PassFocusClick", "PassRaiseClick",
	    "IgnoreFocusClickMotion", "IgnoreRaiseClickMotion",
	    "AllowFocusClickFunction", "AllowRaiseClickFunction", "GrabFocus",
	    "GrabFocusTransient", "OverrideGrabFocus", "ReleaseFocus",
	    "ReleaseFocusTransient", "OverrideReleaseFocus", NULL};
	Bool  found;
	int   val;
	int   index;
	char *token;
	...
	case 0:
		...
		FPS_SORT_WINDOWLIST_BY(
		    *f, (val) ? FPOL_SORT_WL_BY_FOCUS : FPOL_SORT_WL_BY_OPEN);
	...
	case 1:
		...
		if (token != NULL && *token != 0) {
			fvwm_debug(
			    __func__, "illegal mouse button '%c'", *token);
...
style_parse_icon_size_style(char *option, char *rest, window_style *ps)
{
	...
	case 4:
		...
		break;
	default:
		fvwm_debug(__func__, "IconSize requires exactly 0, 2 or 4"
				     " numerical arguments");
		break;
	}
    ...
    char *option, char *rest, window_style *ps, icon_boxes *ib)
{
	...
	if ((IconFill_1 & ICONFILLHRZ) == (IconFill_2 & ICONFILLHRZ)) {
		fvwm_debug(__func__, "IconFill must specify a horizontal"
    ...
    char *prefix, window_style *ps, icon_boxes **cur_ib)
{
	...
	case 'a':
		if (StrEquals(token, "ACTIVEPLACEMENT")) {
			...
			ps->change_mask.placement_mode |= PLACE_RANDOM;
			       ...
			       token, "ACTIVEPLACEMENTHONORSSTARTSONPAGE")) {
	...
	case 'i':
		if (StrEquals(token, "Icon")) {
			...
			*val = ICON_RELIEF_WIDTH;
			...
			rest = style_parse_icon_grid_style(
			    token, rest, ps, *cur_ib);
		} else if (StrEquals(token, "ICONFILL")) {
			rest = style_parse_icon_fill_style(
			    token, rest, ps, *cur_ib);
		} /* end iconfill */
	...
	case 'm':
		if (StrEquals(token, "ManualPlacement")) {
			...
			ps->change_mask.placement_mode = PLACE_MASK;
			       ...
			       token, "ManualPlacementHonorsStartsOnPage")) {
			ps->flags.manual_placement_honors_starts_on_page = on;
			ps->flag_mask.manual_placement_honors_starts_on_page =
			    1;
			ps->change_mask.manual_placement_honors_starts_on_page =
			...
			int   i;
			char *methodlist[] = {
			    "AutoDetect", "UseGravity", "IgnoreGravity", NULL};
	...
	case 's':
		if (StrEquals(token, "SMARTPLACEMENT")) {
			...
			if (spargs == 1) {
				...
				fvwm_debug(
				    __func__, "bad StartsOnDesk arg: %s", rest);
			}
		}
		/* StartsOnPage is like StartsOnDesk-Plus */
		else if (StrEquals(token, "STARTSONPAGE")) {
			char *ret_rest;
			spargs = GetIntegerArguments(rest, &ret_rest, tmpno, 3);
			...
			if (spargs == 2 || spargs == 3) {
				if (spargs == 3) {
					 ...
					 * special case of -1  */
					SSET_START_PAGE_X(
					    *ps, (tmpno[1] > -1) ? tmpno[1] + 1
								 : tmpno[1]);
					SSET_START_PAGE_Y(
					    *ps, (tmpno[2] > -1) ? tmpno[2] + 1
								 : tmpno[2]);
				} else {
					SSET_START_PAGE_X(
					    *ps, (tmpno[0] > -1) ? tmpno[0] + 1
								 : tmpno[0]);
					SSET_START_PAGE_Y(
					    *ps, (tmpno[1] > -1) ? tmpno[1] + 1
								 : tmpno[1]);
	...
	case 'u':
		if (StrEquals(token, "UsePPosition")) {
			...
			if (!token) {
				fvwm_debug(
				    __func__, "UseStyle needs an argument");
				break;
			}
			hit = 0;
...
print_styles(int verbose)
{
	...
	for (nptr = all_styles; nptr != NULL; nptr = SGET_NEXT_STYLE(*nptr)) {
		...
		if (SGET_BACK_COLOR_NAME(*nptr)) {
			...
			if (verbose > 1) {
				fvwm_debug(__func__, "        Back Color: %s\n",
		...
		if (SGET_FORE_COLOR_NAME(*nptr)) {
			...
			if (verbose > 1) {
				fvwm_debug(__func__, "        Fore Color: %s\n",
