...
static void
draw_move_resize_grid(int x, int y, int width, int height);
 ...
 */
static int
get_outline_rects(XRectangle *rects, int x, int y, int width, int height)
{
	int i;
	int n;
	int m;
...
draw_move_resize_grid(int x, int y, int width, int height)
{
	...
	if (move_resize_grid.flags.is_enabled && x == move_resize_grid.geom.x
	...
	if (move_resize_grid.flags.is_enabled
		...
		nrects += get_outline_rects(&(rects[0]),
	...
	if (width && height) {
		...
		nrects +=
		    get_outline_rects(&(rects[nrects]), x, y, width, height);
	}
	if (nrects > 0) {
...
switch_move_resize_grid(Bool state)
{
	if (state == False) {
		...
		if (move_resize_grid.geom.width
			...
			draw_move_resize_grid(move_resize_grid.geom.x,
...
static int
__get_shift(int val, float factor)
{
	int shift;
 ...
 */
int
GetMoveArguments(FvwmWindow *fw, char **paction, int w, int h, int *pFinalX,
    int *pFinalY, Bool *fWarp, Bool *fPointer, Bool fKeep)
{
	char *		s1    = NULL;
	char *		s2    = NULL;
	char *		token = NULL;
	...
	if (s1 && StrEquals(s1, "screen")) {
		...
		FScreenGetScrRect(
		    &parg, FSCREEN_BY_NAME, &scr_x, &scr_y, &scr_w, &scr_h);
		action = GetNextToken(action, &s1);
	}
	...
	if (use_working_area) {
		EWMH_GetWorkAreaIntersection(NULL, &scr_x, &scr_y, &scr_w,
		    &scr_h, EWMH_USE_WORKING_AREA);
	}
	...
	if (s1 != NULL && s2 != NULL) {
		...
		if (fKeep == True && StrEquals(s1, "keep")) {
			retval++;
		} else if (GetOnePositionArgument(s1, *pFinalX, w, pFinalX,
			       (float)scr_w / 100, scr_w, scr_x, True)) {
			retval++;
		}
		if (fKeep == True && StrEquals(s2, "keep")) {
			retval++;
		} else if (GetOnePositionArgument(s2, *pFinalY, h, pFinalY,
			       (float)scr_h / 100, scr_h, scr_y, False)) {
			retval++;
		}
		if (retval == 0) {
		...
		while (*pFinalY < 0) {
			*pFinalY =
			    fw->m->virtual_scr.MyDisplayHeight + *pFinalY;
		}
	}
...
static int
GetResizeArguments(FvwmWindow *fw, char **paction, int x, int y, int w_base,
    int h_base, int w_inc, int h_inc, size_borders *sb, int *pFinalW,
    int *pFinalH, direction_t *ret_dir, Bool *is_direction_fixed,
    ...
    Bool *detect_automatic_direction)
{
	...
	*detect_automatic_direction = False;
	...
	if (StrEquals(token, "bottomright") || StrEquals(token, "br")) {
		...
		n = GetMoveArguments(
		    fw, &naction, 0, 0, &nx, &ny, NULL, NULL, True);
		if (n < 2) {
			return 0;
		}
		*pFinalW = nx - x + 1;
	...
	*paction = naction;
	...
	m = fw->m;
	n = 0;
	n += ParseOneResizeArgument(s1, m->virtual_scr.MyDisplayWidth,
	    m->Desktops->ewmh_working_area.width,
	    m->Desktops->ewmh_dyn_working_area.width, w_base, w_inc, w_add,
	    pFinalW);
	n += ParseOneResizeArgument(s2, m->virtual_scr.MyDisplayHeight,
	    m->Desktops->ewmh_working_area.height,
	    m->Desktops->ewmh_dyn_working_area.height, h_base, h_inc, h_add,
	    pFinalH);
...
static int
GetResizeMoveArguments(FvwmWindow *fw, char **paction, int w_base, int h_base,
    int w_inc, int h_inc, size_borders *sb, int *pFinalX, int *pFinalY,
    int *pFinalW, int *pFinalH, Bool *fWarp, Bool *fPointer)
{
	...
	if (GetResizeArguments(fw, &action, *pFinalX, *pFinalY, w_base, h_base,
		w_inc, h_inc, sb, pFinalW, pFinalH, &dir, &dummy, &dummy,
		&dummy, &dummy)
	    < 2) {
		return 0;
	}
	if (GetMoveArguments(fw, &action, *pFinalW, *pFinalH, pFinalX, pFinalY,
		fWarp, NULL, True)
	    < 2) {
		return 0;
	}
...
position_geometry_window(const XEvent *eventp)
{
	...
	if (Scr.gs.do_emulate_mwm) {
		FScreenCenterOnScreen(&fscr, FSCREEN_CURRENT, &x, &y,
		    sizew_g.width, sizew_g.height);
	} else {
...
resize_geometry_window(void)
{
	...
	Scr.SizeStringWidth = FlocaleTextWidth(Scr.DefaultFont,
	...
	if (cset >= 0) {
		SetWindowBackground(dpy, Scr.SizeWindow, w, h, &Colorset[cset],
    ...
    const FvwmWindow *tmp_win, const XEvent *eventp, int x, int y, int Init)
{
	...
	if (Init) {
		...
		XClearArea(dpy, Scr.SizeWindow, GEOMETRY_WINDOW_BW,
 ...
 */
static void
DisplaySize(const FvwmWindow *tmp_win, const XEvent *eventp, int width,
...
static Bool resize_move_window(F_CMD_ARGS)
{
	...
	n = GetResizeMoveArguments(fw, &action, fw->hints.base_width,
...
static void
InteractiveMove(Window *win, const exec_context_t *exc, int *FinalX,
    int *FinalY, Bool do_start_at_pointer)
{
	...
	if (do_start_at_pointer) {
		if (FQueryPointer(dpy, Scr.Root, &JunkRoot, &JunkChild, &DragX,
	...
	if (IS_ICONIFIED(exc->w.fw)) {
		...
		areapct = 100.0;
		areapct *=
		    ((float)DragWidth / (float)m->virtual_scr.MyDisplayWidth);
		areapct *=
		    ((float)DragHeight / (float)m->virtual_scr.MyDisplayHeight);
		/* round up */
		areapct += 0.1;
		if (Scr.OpaqueSize < 0
	...
	__move_loop(exc, XOffset, YOffset, DragWidth, DragHeight, FinalX,
	    FinalY, do_move_opaque, CRS_MOVE);
	if (!Scr.gs.do_hide_position_window) {
 ...
 * entry */
static void
AnimatedMoveAnyWindow(FvwmWindow *fw, Window w, int startX, int startY,
    int endX, int endY, Bool fWarpPointerToo, int cmsDelay, float *ppctMovement,
    MenuRepaintTransparentParameters *pmrtp)
{
	...
	do {
		...
		if (pmrtp != NULL) {
			update_transparent_menu_bg(pmrtp, lastX, lastY,
			    currentX, currentY, endX, endY);
		}
		XMoveWindow(dpy, w, currentX, currentY);
		if (pmrtp != NULL) {
			repaint_transparent_menu(
			    pmrtp, first, currentX, currentY, endX, endY, True);
		...
		if (fWarpPointerToo == True) {
			...
			FWarpPointer(dpy, None, Scr.Root, 0, 0, 0, 0, pointerX,
			    pointerY);
		}
		if (fw && !IS_SHADED(fw)
			...
			SendConfigureNotify(fw, currentX, currentY,
		...
		if (FCheckMaskEvent(dpy,
			...
			if (pmrtp != NULL) {
				repaint_transparent_menu(
				    pmrtp, first, endX, endY, endX, endY, True);
			}
			break;
		}
		lastX = currentX;
...
AnimatedMoveOfWindow(Window w, int startX, int startY, int endX, int endY,
    ...
    MenuRepaintTransparentParameters *pmrtp)
{
	AnimatedMoveAnyWindow(NULL, w, startX, startY, endX, endY,
	    fWarpPointerToo, cmsDelay, ppctMovement, pmrtp);
...
void
AnimatedMoveFvwmWindow(FvwmWindow *fw, Window w, int startX, int startY,
    int endX, int endY, Bool fWarpPointerToo, int cmsDelay, float *ppctMovement)
{
	AnimatedMoveAnyWindow(fw, w, startX, startY, endX, endY,
...
placement_binding(int button, KeySym keysym, int modifier, char *action)
{
	if (keysym != 0) {
		/* fixme */
		fvwm_debug(
		    __func__, "sorry, placement keybindings not allowed. yet.");
		return 1;
	}
	if (modifier != 0) {
		/* fixme */
 ...
 */
void
__move_icon(FvwmWindow *fw, int x, int y, int old_x, int old_y,
...
__move_window(F_CMD_ARGS, Bool do_animate, int mode)
{
	...
	if (!XGetGeometry(dpy, w, &JunkRoot, &x, &y, (unsigned int *)&width,
	...
	if (mode == MOVE_PAGE) {
		...
		FinalX = x;
		FinalY = y;
		n      = GetMoveArguments(fw, &action, width, height, &FinalX,
			 &FinalY, &fWarp, &fPointer, True);
	...
	if (w == FW_W_FRAME(fw)) {
		...
		frame_setup_window(fw, FinalX, FinalY, fw->g.frame.width,
		    fw->g.frame.height, True);
		if (fWarp & !do_animate) {
			FWarpPointer(dpy, None, None, 0, 0, 0, 0, FinalX - x,
			    FinalY - y);
		}
		if (IS_MAXIMIZED(fw)) {
			fw->g.max.x += dx;
...
static void
DoSnapAttract(FvwmWindow *fw, int Width, int Height, int *px, int *py)
{
	int		nyt, nxl;
	rectangle	self;
	int		score_x;
	...
	if (fw->snap_attraction.proximity > 0
		...
		for (tmp = Scr.FvwmRoot.next; tmp; tmp = tmp->next) {
			...
			if (other.x >= scr_w || other.x + other.width <= 0
			    || other.y >= scr_h
	...
	if (fw->snap_attraction.proximity > 0
		...
		/* vertically */
		if (!(scr_w < (*px) || (*px + self.width) < 0)) {
			if (*py + self.height >= scr_h
			    && *py + self.height
				   < scr_h + fw->snap_attraction.proximity) {
				update_pos(
				    &score_y, &nyt, *py, scr_h - self.height);
			}
			if (*py + self.height
				>= scr_h - fw->snap_attraction.proximity
			...
			if ((*py <= 0)
			    && (*py > -fw->snap_attraction.proximity)) {
				update_pos(&score_y, &nyt, *py, 0);
			}
			if ((*py <= fw->snap_attraction.proximity)
			    && (*py > 0)) {
				update_pos(&score_y, &nyt, *py, 0);
			}
		} /* vertically */
		/* horizontally */
		if (!(scr_h < (*py) || (*py + self.height) < 0)) {
			if (*px + self.width >= scr_w
			    && *px + self.width
				   < scr_w + fw->snap_attraction.proximity) {
				update_pos(
				    &score_x, &nxl, *px, scr_w - self.width);
			}
			if (*px + self.width
				>= scr_w - fw->snap_attraction.proximity
			    && *px + self.width < scr_w) {
	...
	if (fw->edge_resistance_move > 0) {
		/* snap to right edge */
		if (*px + Width > scr_w
		    && *px + Width < scr_w + fw->edge_resistance_move) {
			*px = scr_w - Width;
		}
		/* snap to left edge */
		...
		/* snap to bottom edge */
		if (*py + Height > scr_h
		    && *py + Height < scr_h + fw->edge_resistance_move) {
			*py = scr_h - Height;
		}
		/* snap to top edge */
	...
	if (fw->edge_resistance_xinerama_move > 0 && FScreenIsEnabled()) {
		...
		FScreenGetResistanceRect(*px, *py, Width, Height, &scr_x0,
		    &scr_y0, &scr_x1, &scr_y1);

		/* snap to right edge */
		if (scr_x1 < scr_w && *px + Width >= scr_x1
		    && *px + Width
		...
		/* snap to left edge */
		else if (scr_x0 > 0 && *px <= scr_x0
			 && scr_x0 - *px < fw->edge_resistance_xinerama_move) {
			*px		    = scr_x0;
			do_recalc_rectangle = True;
		}
		if (do_recalc_rectangle) {
			 ...
			 * in Y direction. */
			FScreenGetResistanceRect(*px, *py, Width, Height,
			    &scr_x0, &scr_y0, &scr_x1, &scr_y1);
		}
		/* snap to bottom edge */
		if (scr_y1 < scr_h && *py + Height >= scr_y1
		    && *py + Height
			   < scr_y1 + fw->edge_resistance_xinerama_move) {
			*py = scr_y1 - Height;
		}
		/* snap to top edge */
		else if (scr_y0 > 0 && *py <= scr_y0
			 && scr_y0 - *py < fw->edge_resistance_xinerama_move) {
			*py = scr_y0;
		}
	}
 ...
 */
Bool
__move_loop(const exec_context_t *exc, int XOffset, int YOffset, int Width,
    int Height, int *FinalX, int *FinalY, Bool do_move_opaque, int cursor)
{
	...
	vx = m->virtual_scr.Vx;
	vy = m->virtual_scr.Vy;
	dx = m->virtual_scr.EdgeScrollX ? m->virtual_scr.EdgeScrollX
					: m->virtual_scr.MyDisplayWidth;
	dy = m->virtual_scr.EdgeScrollY ? m->virtual_scr.EdgeScrollY
	...
	if (FQueryPointer(dpy, Scr.Root, &JunkRoot, &JunkChild, &xl, &yt,
	...
	if (!IS_ICONIFIED(fw)
	    && ((!do_move_opaque && !Scr.gs.do_emulate_mwm)
		|| !IS_MAPPED(fw))) {
	...
	while (!is_finished && bad_window != FW_W(fw)) {
		...
		while (rc != -1
			...
			rc = HandlePaging(&le, dx, dy, &xl, &yt, &delta_x,
			    &delta_y, False, False, True,
		 ...
		 * mouseless operation */
		if (e.type == KeyPress) {
			Keyboard_shortcuts(&e, fw, &x_virtual_offset,
		...
		case KeyPress:
			...
			do_snap = nosnap_enabled && (e.xkey.state & Mod1Mask)
				      ? False
				      : True;
		...
		case ButtonRelease:
			...
			if (!(((xl < 0 && xl2 >= 0) || (xl >= 0 && xl2 < 0)
				  ...
				  || (yt >= 0 && yt2 < 0))
				&& (abs(xl - xl2)
					> m->virtual_scr.MyDisplayWidth / 2
				    || abs(yt - yt2)
					   > m->virtual_scr.MyDisplayHeight
						 / 2))) {
				xl = xl2;
		...
		case MotionNotify:
			...
			do_snap = nosnap_enabled && (e.xmotion.state & Mod1Mask)
				      ? False
				      : True;
			xl	= e.xmotion.x_root;
			yt	= e.xmotion.y_root;
			...
			for (paged = 0; paged <= 1; paged++) {
				...
				if (paged == 0) {
					...
					yt = e.xmotion.y_root;
					fev_get_last_event(&le);
					HandlePaging(&le, dx, dy, &xl, &yt,
					    &delta_x, &delta_y, False, False,
					    False, fw->edge_delay_ms_move);
					if (delta_x) {
						x_virtual_offset = 0;
					}
					xl += XOffset;
		...
		if (do_move_opaque && !IS_ICONIFIED(fw) && !IS_SHADED(fw)
			...
			if (!sent_cn || cnx != xl || cny != yt) {
				...
				sent_cn = True;
				SendConfigureNotify(
				    fw, xl, yt, Width, Height, 0, False);
#ifdef FVWM_DEBUG_MSGS
				fvwm_debug(__func__,
				    "Sent ConfigureNotify (w %d, h %d)", Width,
				    Height);
#endif
			}
		}
...
static char *hide_options[] = {"never", "move", "resize", NULL};
...
void CMD_XorValue(F_CMD_ARGS)
{
	...
	*/
	...
	gcv.foreground =
	    (val) ? (val) : (PictureBlackPixel() ^ PictureWhitePixel());
 ...
 */
static direction_t
__resize_get_dir_from_resize_quadrant(int x_off, int y_off, int px, int py)
{
...
static void
__resize_get_dir_proximity(int *ret_xmotion, int *ret_ymotion, FvwmWindow *fw,
    int x_off, int y_off, int px, int py, int *warp_x, int *warp_y,
    Bool find_nearest_border)
{
	int	    tx;
	int	    ty;
	direction_t dir;
	...
	*warp_x = *warp_y = -1;
	...
	dir = __resize_get_dir_from_resize_quadrant(x_off, y_off, px, py);
...
static void
__resize_get_refpos(int *ret_x, int *ret_y, int xmotion, int ymotion, int w,
    int h, FvwmWindow *fw)
{
	if (xmotion > 0) {
		*ret_x = 0;
 ...
 */
static void
__resize_step(const exec_context_t *exc, int x_root, int y_root, int *x_off,
    ...
    int *ymotionp, Bool do_resize_opaque, Bool is_direction_fixed)
{
	...
	if (action) {
		...
		constrain_size(exc->w.fw, exc->x.elast, &drag->width,
		    &drag->height, *xmotionp, *ymotionp, CS_ROUND_UP);
		if (*xmotionp == 1) {
		...
		if (!do_resize_opaque) {
			draw_move_resize_grid(drag->x, drag->y, drag->width - 1,
			    drag->height - 1);
		} else {
	...
	DisplaySize(
	    exc->w.fw, exc->x.elast, drag->width, drag->height, False, False);
...
static Bool __resize_window(F_CMD_ARGS)
{
	...
	dx = mon->virtual_scr.EdgeScrollX ? mon->virtual_scr.EdgeScrollX
					  : mon->virtual_scr.MyDisplayWidth;
	dy = mon->virtual_scr.EdgeScrollY ? mon->virtual_scr.EdgeScrollY
	...
	if (fev_get_evpos_or_query(dpy, Scr.Root, exc->x.etrigger, &px, &py)
	    ...
	    || XTranslateCoordinates(
		   dpy, Scr.Root, ResizeWindow, px, py, &px, &py, &JunkChild)
		   == False) {
		/* pointer is on a different screen - that's okay here */
		px = 0;
		py = 0;
	...
	get_window_borders(fw, &b);
	n = GetResizeArguments(fw, &action, fw->g.frame.x, fw->g.frame.y,
	    fw->hints.base_width, fw->hints.base_height, fw->hints.width_inc,
	    fw->hints.height_inc, &b, &(drag->width), &(drag->height), &dir,
	    &is_direction_fixed, &do_warp_to_border,
	...
	if (PressedW != Scr.Root && xmotion == 0 && ymotion == 0
	    && !called_from_title) {
		__resize_get_dir_proximity(&xmotion, &ymotion, fw, orig->width,
	...
	if (!IS_SHADED(fw)) {
		__resize_get_refpos(&ref_x, &ref_y, xmotion, ymotion,
		    orig->width, orig->height, fw);
	} else {
		...
		__resize_get_refpos(&ref_x, &ref_y, xmotion, ymotion,
		    fw->g.frame.width, fw->g.frame.height, fw);
	}
	x_off = 0;
	y_off = 0;
	if (do_warp_to_border == True) {
		...
		if (automatic_border_direction == True
		    && (warp_x >= 0 && warp_y >= 0) && !IS_SHADED(fw)) {
			dx = warp_x;
			dy = warp_y;
		}

		/* warp the pointer to the border */
		FWarpPointer(dpy, None, ResizeWindow, 0, 0, 1, 1, dx, dy);
		XFlush(dpy);
	...
	if (exc->x.elast->type == KeyPress) {
		...
		xo = 0;
		yo = 0;
		__resize_step(exc, stashed_x, stashed_y, &xo, &yo, drag, orig,
	...
	while (!is_finished && bad_window != FW_W(fw)) {
		...
		while (
			...
			rc = HandlePaging(&ev, dx, dy, &x, &y, &delta_x,
			    &delta_y, False, False, True,
		...
		case MotionNotify:
			...
			if (!fForceRedraw) {
				...
				__resize_step(exc, x, y, &x_off, &y_off, drag,
				    orig, &xmotion, &ymotion, do_resize_opaque,
				    is_direction_fixed);
				is_resized = True;
				/* need to move the viewport */
				HandlePaging(&ev, dx, dy, &x, &y, &delta_x,
		...
		if (!is_done) {
			...
			dispatch_event(&ev);
			if (!do_resize_opaque) {
				draw_move_resize_grid(drag->x, drag->y,
	...
	if (is_aborted || bad_window == FW_W(fw)) {
		...
		if (stashed_x >= 0) {
			FWarpPointer(dpy, None, Scr.Root, 0, 0, 0, 0, stashed_x,
			    stashed_y);
		}
		if (was_maximized) {
		...
		if (do_resize_opaque) {
			...
			ymotion = 1;
			g	= sorig;
			__resize_step(exc, sorig.x, sorig.y, &xo, &yo, &g, orig,
			    &xmotion, &ymotion, do_resize_opaque, True);
		}
...
static void
move_sticky_window_to_same_page(FvwmWindow *fw, int *x11, int *x12, int *y11,
    int *y12, int x21, int x22, int y21, int y22)
{
	struct monitor *m = fw->m;
...
static void
MaximizeHeight(FvwmWindow *win, int win_width, int win_x, int *win_height,
    int *win_y, Bool grow_up, Bool grow_down, int top_border, int bottom_border,
    int *layers)
{
	struct monitor *mon;
	FvwmWindow *	cwin;
	...
	for (cwin = Scr.FvwmRoot.next; cwin; cwin = cwin->next) {
		...
		if (is_window_sticky_across_pages(cwin)) {
			move_sticky_window_to_same_page(cwin, &x21, &x22,
			    &new_y1, &new_y2, x11, x12, y11, y12);
		}
...
static void
MaximizeWidth(FvwmWindow *win, int *win_width, int *win_x, int win_height,
    ...
    int right_border, int *layers)
{
	...
	for (cwin = Scr.FvwmRoot.next; cwin; cwin = cwin->next) {
		...
		y22 = y21 + g.height;
		if (is_window_sticky_across_pages(cwin)) {
			move_sticky_window_to_same_page(cwin, &new_x1, &new_x2,
			    &y21, &y22, x11, x12, y11, y12);
		}
...
static void
unmaximize_fvwm_window(FvwmWindow *fw)
{
	char *	  cmd;
	rectangle new_g;
...
void CMD_Maximize(F_CMD_ARGS)
{
	...
	while (!global_flag_parsed) {
		...
		if (!token) {
			...
			if (StrEquals(token, "screen")) {
				...
				FScreenGetScrRect(&arg, FSCREEN_BY_NAME, &scr_x,
				    &scr_y, &scr_w, &scr_h);
			} else if (StrEquals(token, "ewmhiwa")) {
	...
	if (!ignore_working_area) {
		EWMH_GetWorkAreaIntersection(
		    fw, &scr_x, &scr_y, &scr_w, &scr_h, EWMH_MAXIMIZE_MODE(fw));
	}
#if 0
	...
	} else /* maximize */
	{
		...
		if (grow_up || grow_down) {
			MaximizeHeight(fw, new_g.width, new_g.x, &new_g.height,
		...
		if (grow_left || grow_right) {
			MaximizeWidth(fw, &new_g.width, &new_g.x, new_g.height,
			    new_g.y, grow_left, grow_right, page_x + scr_x,
			    page_x + scr_x + scr_w, layers);
		} else if (val1 > 0) {
...
stick_across_pages(F_CMD_ARGS, int toggle)
{
	...
	if (IS_STICKY_ACROSS_PAGES(fw)) {
		...
		if (!IsRectangleOnThisPage(
			fw->m, &fw->g.frame, fw->m->virtual_scr.CurrentDesk)) {
			action = "";
			__move_window(F_PASS_ARGS, False, MOVE_PAGE);
			UPDATE_FVWM_SCREEN(fw);
		}
	...
	if (IS_STICKY_ACROSS_DESKS(fw)) {
		...
		if (fw->Desk != fw->m->virtual_scr.CurrentDesk) {
			do_move_window_to_desk(
			    fw, fw->m->virtual_scr.CurrentDesk);
		}
