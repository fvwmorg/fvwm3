...
float rgpctMovementDefault[32] = {
	-.01, 0, .01, .03, .08, .18, .3, .45, .60, .75, .85, .90, .94, .97, .99,
	1.0
	/* must end in 1.0 */
};
...
static rectangle sizew_g = { -1, -1, -1, -1 };
 ...
 */
static int
get_outline_rects(XRectangle *rects, rectangle geom)
{
	int i;
	int n;
	int m;
...
static void
draw_move_resize_grid(rectangle geom)
{
	int	   nrects = 0;
	XRectangle rects[10];
...
void
switch_move_resize_grid(Bool state)
{
...
static int
ParsePositionArgumentSuffix(float *ret_factor, char *suffix, float wfactor,
    float sfactor)
{
	int n;
...
static void
move_to_next_monitor(FvwmWindow *fw, rectangle *win_r, bool ewmh,
    direction_t dir)
{
	position	page;
	struct monitor *m;
...
static void
shuffle_win_to_closest(FvwmWindow *fw, char **action, position *pFinal,
    Bool *fWarp)
{
	direction_t dir;
	...
	/* Get direction(s) */
	while (token) {
		dir = gravity_parse_dir_argument(*action, &naction, DIR_NONE);
		...
		case DIR_N:
			...
			grow_to_closest_type(fw, &cwin, bound, layers, snap,
			    false);
...
static int
get_shift(int val, float factor)
{
	int shift;
...
static int
GetOnePositionArgument(char *s1, int window_pos, int window_size,
    int *pFinalPos, float sfactor, int screen_size, int screen_pos, Bool is_x)
{
	int   final_pos;
	int   pos_change = 0;
	float wfactor;
	...
	default:
		...
		if (*s1 != 0) {
			...
			s1 += n;
			/* parse suffix */
			n = ParsePositionArgumentSuffix(&f, s1, wfactor,
			    sfactor);
			s1 += n;
	...
	if (final_pos > MAX_X_WINDOW_POSITION ||
		...
		fvwm_debug(__func__, "new position is out of range: %d",
		    final_pos);
		if (final_pos > MAX_X_WINDOW_POSITION) {
 ...
 */
int
GetMoveArguments(FvwmWindow *fw, char **paction, size_rect s, position *pFinal,
    Bool *fWarp, Bool *fPointer, Bool fKeep)
{
	...
	if (s1 && StrEquals(s1, "screen")) {
		...
		FScreenGetScrRect(&parg, FSCREEN_BY_NAME, &scr_pos.x,
		    &scr_pos.y, &scr_w, &scr_h);
		action = GetNextToken(action, &s1);
	}
	...
	if (use_working_area) {
		EWMH_GetWorkAreaIntersection(NULL, &scr_pos.x, &scr_pos.y,
		    &scr_w, &scr_h, EWMH_USE_WORKING_AREA);
	}
	...
	if (s1 != NULL && s2 != NULL) {
		...
		if (fKeep == True && StrEquals(s1, "keep")) {
			retval++;
		} else if (GetOnePositionArgument(s1, pFinal->x, s.width,
			       &(pFinal->x), (float)scr_w / 100, scr_w,
			       scr_pos.x, True)) {
			retval++;
		}
		if (fKeep == True && StrEquals(s2, "keep")) {
			retval++;
		} else if (GetOnePositionArgument(s2, pFinal->y, s.height,
			       &(pFinal->y), (float)scr_h / 100, scr_h,
			       scr_pos.y, False)) {
			retval++;
		}
		if (retval == 0) {
...
static int
ParseOneResizeArgument(char *arg, int scr_size, int wa_size, int dwa_size,
    int base_size, int size_inc, int add_size, int *ret_size)
{
	float factor;
	int   val;
	int   add_base_size = 0;
	...
	if (strcmp(arg, "w") == 0) {
	    ...
	    sscanf(arg, "%d", &val) == 1) {
		size_change = (int)(val * factor + 0.5) + add_size +
		    add_base_size - *ret_size;
	} else {
		return 0;
	}
...
static int
GetResizeArguments(FvwmWindow *fw, char **paction, position p,
    size_rect base_size, size_rect inc_size, size_borders *sb,
    size_rect *pFinalSize, direction_t *ret_dir, Bool *is_direction_fixed,
    ...
    Bool *detect_automatic_direction)
{
	...
	*detect_automatic_direction = False;
	...
	if (StrEquals(token, "bottomright") || StrEquals(token, "br")) {
		position  pn = { p.x + pFinalSize->width - 1,
	...
	for (;; token = PeekToken(naction, &naction)) {
		if (StrEquals(token, "frame")) {
			...
			*ret_dir = gravity_parse_dir_argument(naction, &naction,
			    DIR_NONE);
			if (*ret_dir != DIR_NONE) {
	...
	*paction = naction;
	...
	m = fw->m;
	n = 0;
	n += ParseOneResizeArgument(s1, monitor_get_all_widths(),
	    m->Desktops->ewmh_working_area.width,
	    m->Desktops->ewmh_dyn_working_area.width, base_size.width,
	    inc_size.width, add_size.width, &(pFinalSize->width));
	n += ParseOneResizeArgument(s2, monitor_get_all_heights(),
	    m->Desktops->ewmh_working_area.height,
	    m->Desktops->ewmh_dyn_working_area.height, base_size.height,
	    inc_size.height, add_size.height, &(pFinalSize->height));
...
static int
GetResizeMoveArguments(FvwmWindow *fw, char **paction, size_rect base_size,
    size_rect inc_size, size_borders *sb, position *pFinalPos,
    size_rect *pFinalSize, Bool *fWarp, Bool *fPointer)
{
	char	   *action = *paction;
	direction_t dir;
	Bool	    dummy;
	...
	if (GetResizeArguments(fw, &action, *pFinalPos, base_size, inc_size, sb,
		pFinalSize, &dir, &dummy, &dummy, &dummy, &dummy) < 2) {
		return 0;
	}
	if (GetMoveArguments(fw, &action, *pFinalSize, pFinalPos, fWarp, NULL,
		True) < 2) {
		return 0;
	}
	*paction = action;
...
static void
position_geometry_window(const XEvent *eventp)
{
	position	p;
	size_rect	sgn    = { 1, 1 };
	...
	if (Scr.SizeWindow.is_configured) {
		rectangle scr_g;
		FScreenGetScrRect(&fscr, screen, &scr_g.x, &scr_g.y,
		...
		p.x = scr_g.x +
		    sgn.width *
			(Scr.SizeWindow.x * (scr_g.width - sizew_g.width)) /
			100;
		...
		p.y = scr_g.y +
		    sgn.height *
			(Scr.SizeWindow.y * (scr_g.height - sizew_g.height)) /
			100;
		...
		FScreenCenterOnScreen(&fscr, screen, &p.x, &p.y, sizew_g.width,
		    sizew_g.height);
	} else {
...
resize_geometry_window(void)
{
	...
	Scr.SizeWindow.StringWidth = FlocaleTextWidth(Scr.DefaultFont,
	    GEOMETRY_WINDOW_STRING, sizeof(GEOMETRY_WINDOW_STRING) - 1);
	...
	if (cset >= 0) {
		SetWindowBackground(dpy, Scr.SizeWindow.win, s.width, s.height,
		    &Colorset[cset], Pdepth, Scr.StdGC, False);
	}
}

static void
display_string(Bool Init, char *str)
{
	...
	if (Init) {
		...
		if (fstr.flags.has_colorset) {
			...
			our_relief_gc  = fvwmlib_XCreateGC(dpy, Scr.NoFocusWin,
			     GCForeground, &gcv);
			gcv.foreground = fstr.colorset->shadow;
			our_shadow_gc  = fvwmlib_XCreateGC(dpy, Scr.NoFocusWin,
			     GCForeground, &gcv);
		}
	} else {
		/* just clear indside the relief lines to reduce flicker */
		XClearArea(dpy, Scr.SizeWindow.win, GEOMETRY_WINDOW_BW,
	...
	if (Pdepth >= 2) {
		RelieveRectangle(dpy, Scr.SizeWindow.win, 0, 0,
	...
	offset = (Scr.SizeWindow.StringWidth -
		     FlocaleTextWidth(Scr.DefaultFont, str, strlen(str))) /
	    2;
...
static void
DisplayPosition(const FvwmWindow *tmp_win, const XEvent *eventp, position p,
    int Init)
{
	char		str[100];
	fscreen_scr_arg fscr;
	...
	fscr.xypos.x = p.x;
	fscr.xypos.y = p.y;
	FScreenTranslateCoordinates(NULL, FSCREEN_GLOBAL, &fscr, FSCREEN_XYPOS,
	    &p.x, &p.y);
...
static void
DisplaySize(const FvwmWindow *tmp_win, const XEvent *eventp, size_rect size,
    Bool Init, Bool resetLast)
{
	...
	(void)snprintf(str, sizeof(str), GEOMETRY_WINDOW_SIZE_STRING, d.width,
	    d.height);
	display_string(Init, str);
...
resize_move_window(F_CMD_ARGS)
{
	...
	/* gotta have a window */
	w = FW_W_FRAME(fw);
	if (!XGetGeometry(dpy, w, &JunkRoot, &p.x, &p.y,
		(unsigned int *)&final_size.width,
		(unsigned int *)&final_size.height, (unsigned int *)&JunkBW,
	...
	{
		size_rect base = { fw->hints.base_width,
		...
		n = GetResizeMoveArguments(fw, &action, base, inc, &b,
		    &final_pos, &final_size, &fWarp, &fPointer);
	}
	if (n < 4) {
		return False;
	}
	...
	if (IS_MAXIMIZED(fw)) {
		...
		SET_MAXIMIZED(fw, 0);
		border_draw_decorations(fw, PART_BUTTONS, (fw == Scr.Hilite),
		    True, CLEAR_ALL, NULL, NULL);
	}
	d.x = final_pos.x - fw->g.frame.x;
	d.y = final_pos.y - fw->g.frame.y;
	/* size will be less or equal to requested */
	constrain_size(fw, NULL, &final_size.width, &final_size.height, 0, 0,
	    0);
	if (IS_SHADED(fw)) {
		frame_setup_window(fw, final_pos.x, final_pos.y,
	...
	if (fWarp) {
		FWarpPointer(dpy, None, None, 0, 0, 0, 0, final_pos.x - p.x,
		    final_pos.y - p.y);
	}
	if (IS_MAXIMIZED(fw)) {
		fw->g.max.x += d.x;
...
static void
InteractiveMove(Window *win, const exec_context_t *exc, position *pFinal,
    Bool do_start_at_pointer)
{
	...
	if (do_start_at_pointer) {
		if (FQueryPointer(dpy, Scr.Root, &JunkRoot, &JunkChild, &drag.x,
		 ...
		 * faked. */
		fev_get_evpos_or_query(dpy, Scr.Root, exc->x.elast, &drag.x,
		    &drag.y);
	}

	MyXGrabServer(dpy);
	if (!XGetGeometry(dpy, w, &JunkRoot, &orig_drag.x, &orig_drag.y,
	...
	if (IS_ICONIFIED(exc->w.fw)) {
		...
		areapct = 100.0;
		areapct *= ((float)drag.width /
		    (float)monitor_get_all_widths());
		areapct *= ((float)drag.height /
 ...
 * entry */
static void
AnimatedMoveAnyWindow(FvwmWindow *fw, Window w, position start, position end,
    ...
    MenuRepaintTransparentParameters *pmrtp)
{
	...
	if (start.x < 0 || start.y < 0) {
		if (!XGetGeometry(dpy, w, &JunkRoot, &current.x, &current.y,
	...
	do {
		...
		if (pmrtp != NULL) {
			update_transparent_menu_bg(pmrtp, last.x, last.y,
			    current.x, current.y, end.x, end.y);
		}
		XMoveWindow(dpy, w, current.x, current.y);
		if (pmrtp != NULL) {
			repaint_transparent_menu(pmrtp, first, current.x,
			    current.y, end.x, end.y, True);
		} else if (draw_parts != PART_NONE) {
			border_draw_decorations(fw, draw_parts,
			    ((fw == get_focus_window())) ? True : False, True,
			    CLEAR_ALL, NULL, NULL);
		}
		...
		if (fWarpPointerToo == True) {
			...
			FWarpPointer(dpy, None, Scr.Root, 0, 0, 0, 0, pointer.x,
			    pointer.y);
		}
		if (fw && !IS_SHADED(fw) &&
		    !Scr.bo.do_disable_configure_notify) {
			/* send configure notify event for windows that care
			 * about their location */
			SendConfigureNotify(fw, current.x, current.y,
		...
		if (FCheckMaskEvent(dpy,
			...
			if (pmrtp != NULL) {
				update_transparent_menu_bg(pmrtp, last.x,
				    last.y, current.x, current.y, end.x, end.y);
			}
			XMoveWindow(dpy, w, end.x, end.y);
			if (pmrtp != NULL) {
				repaint_transparent_menu(pmrtp, first, end.x,
				    end.y, end.x, end.y, True);
			}
			break;
		}
		last.x = current.x;
...
void
AnimatedMoveOfWindow(Window w, position start, position end,
    Bool fWarpPointerToo, int cmsDelay, float *ppctMovement,
    MenuRepaintTransparentParameters *pmrtp)
{
	AnimatedMoveAnyWindow(NULL, w, start, end, fWarpPointerToo, cmsDelay,
	    ppctMovement, pmrtp);
...
void
AnimatedMoveFvwmWindow(FvwmWindow *fw, Window w, position start, position end,
    Bool fWarpPointerToo, int cmsDelay, float *ppctMovement)
{
	AnimatedMoveAnyWindow(fw, w, start, end, fWarpPointerToo, cmsDelay,
	    ppctMovement, NULL);
...
int
placement_binding(int button, KeySym keysym, int modifier, char *action)
{
	if (keysym != 0) {
		/* fixme */
		fvwm_debug(__func__,
	...
	if (strcmp(action, "-") == 0 ||
		...
		if (keysym == 0) /* must be button binding */
		{
			if (button == 0) {
			    ...
			    button <= NUMBER_OF_EXTENDED_MOUSE_BUTTONS) {
				move_drag_finish_button_mask |= (1
				    << (button - 1));
				move_interactive_finish_button_mask |= (1
				    << (button - 1));
			}
		}
		...
		fvwm_debug(__func__, "invalid action %s", action);
	}
 ...
 */
void
__move_icon(FvwmWindow *fw, position new, position old, Bool do_move_animated,
    Bool do_warp_pointer)
{
	...
	if (do_move_animated) {
		...
		AnimatedMoveOfWindow(tw, start, t, do_warp_pointer, -1, NULL,
		    NULL);
		do_warp_pointer = 0;
	}
	if (has_icon_title) {
	...
	if (do_warp_pointer) {
		FWarpPointer(dpy, None, None, 0, 0, 0, 0, new.x - old.x,
		    new.y - old.y);
	}
...
static void
__move_window(F_CMD_ARGS, Bool do_animate, int mode)
{
	position    final = { 0, 0 };
	int	    n;
	position    p;
	...
	if (mode == MOVE_PAGE) {
		...
		final.x = p.x;
		final.y = p.y;
		n = GetMoveArguments(fw, &action, sz, &final, &fWarp, &fPointer,
		    True);
	...
	if (w == FW_W_FRAME(fw)) {
		...
		if (do_animate) {
			...
			AnimatedMoveFvwmWindow(fw, w, start, final, fWarp, -1,
			    NULL);
		}
		frame_setup_window(fw, final.x, final.y, fw->g.frame.width,
		    fw->g.frame.height, True);
		if (fWarp & !do_animate) {
			char *cmd = "WarpToWindow 50 50";
			execute_function_override_window(NULL, exc, cmd, NULL,
			    0, fw);
		}
		if (IS_MAXIMIZED(fw)) {
			fw->g.max.x += d.x;
...
static void
update_pos(int *dest_score, int *dest_pos, int current_pos, int requested_pos)
{
	int requested_score;
...
static void
DoSnapGrid(FvwmWindow *fw, rectangle self, position *new)
{
	...
	if (fw->snap_grid_x > 1 && self.x != grid_x)
		new->x = (self.x - grid_x <= fw->snap_grid_x / 2) ?
		    grid_x :
		    grid_x + fw->snap_grid_x;

	if (fw->snap_grid_y > 1 && self.y != grid_y)
		new->y = (self.y - grid_y <= fw->snap_grid_y / 2) ?
		    grid_y :
		    grid_y + fw->snap_grid_y;
}
static void
DoSnapMonitor(FvwmWindow *fw, rectangle self, position *new, int snapd,
    bool resist, bool all_edges)
{
	position mon;
	position mon_br;
	int	 tmp;
	...
	TAILQ_FOREACH (m, &monitor_q, entry) {
		...
		if (!(br.x < mon.x || self.x > mon_br.x) &&
			...
			if (self.y <= tmp && self.y >= mon.y - snapd)
				update_pos(&score.y, &(new->y), self.y, mon.y);
		}
		...
		if (!(br.y < mon.y || self.y > mon_br.y) &&
			...
			if (self.x <= tmp && self.x >= mon.x - snapd)
				update_pos(&score.x, &(new->x), self.x, mon.x);
		}
	}
}
static void
DoSnapWindow(FvwmWindow *fw, rectangle self, position *new)
{
	struct {
		int x;
		int y;
	...
	FvwmWindow *tmp;
	int maskout = (SNAP_SCREEN | SNAP_SCREEN_WINDOWS | SNAP_SCREEN_ICONS |
	    SNAP_SCREEN_ALL);
	size_rect scr;
	...
	for (tmp = Scr.FvwmRoot.next; tmp; tmp = tmp->next) {
		...
		if (other.x >= scr.width || other.x + other.width <= 0 ||
		    other.y >= scr.height || other.y + other.height <= 0) {
...
static void
DoSnapAttract(FvwmWindow *fw, size_rect sz, position *p)
{
	...
	bool snap_mon  = (fw->snap_attraction.mode & SNAP_SCREEN_ALL ||
			     fw->snap_attraction.mode &
				 (SNAP_SCREEN & icon_mask)) ?
	     true :
	     false;
	bool snap_win  = (fw->snap_attraction.mode &
			     (SNAP_ICONS | SNAP_WINDOWS | SNAP_SAME)) ?
	     true :
	     false;
	...
	if (fw->edge_resistance_move > 0) {
		new.x = NOSNAP;
		new.y = NOSNAP;
		DoSnapMonitor(fw, self, &new, fw->edge_resistance_move, true,
		    false);
		if (new.x != NOSNAP)
			p->x = new.x;
		if (new.y != NOSNAP)
	...
	if (fw->edge_resistance_screen_move > 0) {
		new.x = NOSNAP;
		new.y = NOSNAP;
		DoSnapMonitor(fw, self, &new, fw->edge_resistance_screen_move,
		    true, true);
		if (new.x != NOSNAP)
			p->x = new.x;
		if (new.y != NOSNAP)
 ...
 */
Bool
__move_loop(const exec_context_t *exc, position offset, size_rect sz,
    position *pFinal, Bool do_move_opaque, int cursor)
{
	...
	d.x = m->virtual_scr.EdgeScrollX ? m->virtual_scr.EdgeScrollX :
					   monitor_get_all_widths();
	d.y = m->virtual_scr.EdgeScrollY ? m->virtual_scr.EdgeScrollY :
	...
	if (!XGetGeometry(dpy, move_w, &JunkRoot, &bak.x, &bak.y,
		(unsigned int *)&JunkWidth, (unsigned int *)&JunkHeight,
		(unsigned int *)&JunkBW, (unsigned int *)&JunkDepth)) {
	...
	if (FQueryPointer(dpy, Scr.Root, &JunkRoot, &JunkChild, &p.x, &p.y,
	...
	if (!IS_ICONIFIED(fw) &&
		...
		rectangle r = { p.x, p.y, sz.width - 1, sz.height - 1 };
	...
	while (!is_finished && bad_window != FW_W(fw)) {
		...
		while (rc != -1 &&
			...
			p.x -= offset.x;
			p.y -= offset.y;
			delay = (is_alt_mode_enabled) ? 0 :
							fw->edge_delay_ms_move;
			rc    = HandlePaging(&le, d, &p, &delta, False, False,
			       True, delay);
		...
		if (e.type == MotionNotify || e.type == EnterNotify ||
		    e.type == LeaveNotify) {
		 ...
		 * mouseless operation */
		if (e.type == KeyPress) {
			Keyboard_shortcuts(&e, fw, &virtual_offset.x,
		...
		case ButtonRelease:
			...
			if (!(((p.x < 0 && p2.x >= 0) ||
				  ...
				  (p.y >= 0 && p2.y < 0)) &&
				(abs(p.x - p2.x) >
					monitor_get_all_widths() / 2 ||
				    abs(p.y - p2.y) >
					monitor_get_all_heights() / 2))) {
				p.x = p2.x;
				p.y = p2.y;
			}
		...
		case MotionNotify:
			...
			for (paged = 0; paged <= 1; paged++) {
				if (!do_move_opaque) {
					rectangle r = { p.x, p.y, sz.width - 1,
						sz.height - 1 };
					...
					if (IS_ICONIFIED(fw)) {
						...
						broadcast_icon_geometry(fw,
						    False);
					} else {
						XMoveWindow(dpy, FW_W_FRAME(fw),
						    p.x, p.y);
					}
				}
				...
				if (paged == 0) {
					...
					p.y   = e.xmotion.y_root;
					fev_get_last_event(&le);
					HandlePaging(&le, d, &p, &delta, False,
					    False, False, delay);
					if (delta.x) {
						virtual_offset.x = 0;
					}
		...
		if (do_move_opaque && !IS_ICONIFIED(fw) && !IS_SHADED(fw) &&
		    !Scr.bo.do_disable_configure_notify) {
			/* send configure notify event for windows that care
			...
			if (!sent_cn || cn.x != p.x || cn.y != p.y) {
				cn	= p;
				sent_cn = True;
				SendConfigureNotify(fw, p.x, p.y, sz.width,
				    sz.height, 0, False);
				if (DEBUG_CONFIGURENOTIFY) {
					fvwm_debug(__func__,
					    "Sent ConfigureNotify"
...
static void
set_geom_win_visible_val(char *token, bool val)
{
...
static bool
set_geom_win_position_val(char *s, int *coord, bool *neg, bool *rel)
{
	int val, n;
...
CMD_GeometryWindow(F_CMD_ARGS)
{
	...
	while ((token = PeekToken(action, &action)) != NULL) {
		if (StrEquals(token, "hide")) {
			set_geom_win_visible_val(PeekToken(action, &action),
			    false);
		}
...
void
CMD_HideGeometryWindow(F_CMD_ARGS)
{
	char *cmd;

	fvwm_debug(__func__,
	    "HideGeometryWindow is deprecated.  "
	    "Converting to use: GeometryWindow hide %s",
	    action);
...
CMD_XorValue(F_CMD_ARGS)
{
	...
	gcm = GCFunction | GCLineWidth | GCForeground | GCFillStyle |
	    GCSubwindowMode;
 ...
 */
static direction_t
__resize_get_dir_from_resize_quadrant(position off, position p)
{
...
static void
__resize_get_dir_from_window(position *ret_motion, FvwmWindow *fw,
    Window context_w)
{
...
static void
__resize_get_dir_proximity(position *ret_motion, FvwmWindow *fw, position off,
...
static void
__resize_get_refpos(position *ret, position motion, size_rect sz,
    FvwmWindow *fw)
{
	if (motion.x > 0) {
		ret->x = 0;
 ...
 */
static void
__resize_step(const exec_context_t *exc, position root, position *off,
    ...
    Bool do_resize_opaque, Bool is_direction_fixed)
{
	...
	p2.x = root.x - off->x;
	root.x += off->x;
	if (is_direction_fixed == True &&
	    (pmotion->x != 0 || pmotion->y != 0)) {
		dir.x = pmotion->x;
	} else if (p2.x <= orig->x ||
	...
	if (action) {
		 ...
		 * rubberband */
		constrain_size(exc->w.fw, exc->x.elast, &drag->width,
		    &drag->height, pmotion->x, pmotion->y, CS_ROUND_UP);
		if (pmotion->x == 1) {
		...
		if (!do_resize_opaque) {
			rectangle r = { drag->x, drag->y, drag->width - 1,
				drag->height - 1 };

			draw_move_resize_grid(r);
		} else {
			frame_setup_window(exc->w.fw, drag->x, drag->y,
...
__resize_window(F_CMD_ARGS)
{
	...
	d.x = mon->virtual_scr.EdgeScrollX ? mon->virtual_scr.EdgeScrollX :
					     monitor_get_all_widths();
	d.y = mon->virtual_scr.EdgeScrollY ? mon->virtual_scr.EdgeScrollY :
					     monitor_get_all_heights();

	bad_window   = False;
	ResizeWindow = FW_W_FRAME(fw);
	if (fev_get_evpos_or_query(dpy, Scr.Root, exc->x.etrigger, &p2.x,
		&p2.y) == False ||
	    XTranslateCoordinates(dpy, Scr.Root, ResizeWindow, p2.x, p2.y,
		&p2.x, &p2.y, &JunkChild) == False) {
		/* pointer is on a different screen - that's okay here */
		p2.x = 0;
		p2.y = 0;
	}
	...
	get_window_borders(fw, &b);
	{
		...
		n = GetResizeArguments(fw, &action, fp, base, inc, &b, &sz,
	...
	if (n == 2) {
		...
		if (IS_SHADED(fw)) {
			...
			SET_MAXIMIZED(fw, 0);
			constrain_size(fw, NULL, &drag->width, &drag->height,
			    motion.x, motion.y, 0);
			gravity_resize(fw->hints.win_gravity, &new_g,
			    drag->width - new_g.width,
			    drag->height - new_g.height);
			fw->g.normal = new_g;
			get_shaded_geometry(fw, &shaded_g, &new_g);
			frame_setup_window(fw, shaded_g.x, shaded_g.y,
			    shaded_g.width, shaded_g.height, False);
		} else {
			new_g = fw->g.frame;
			SET_MAXIMIZED(fw, 0);
			constrain_size(fw, NULL, &drag->width, &drag->height,
			...
			gravity_resize(fw->hints.win_gravity, &new_g,
			    drag->width - new_g.width,
			    drag->height - new_g.height);
			frame_setup_window(fw, new_g.x, new_g.y, drag->width,
			    drag->height, False);
		}
		update_absolute_geometry(fw);
		maximize_adjust_offset(fw);
	...
	if (!XGetGeometry(dpy, (Drawable)ResizeWindow, &JunkRoot, &drag->x,
		&drag->y, (unsigned int *)&drag->width,
		(unsigned int *)&drag->height, (unsigned int *)&JunkBW,
		(unsigned int *)&JunkDepth)) {
		UngrabEm(GRAB_NORMAL);
		if (!do_resize_opaque) {
			MyXUngrabServer(dpy);
	...
	if (do_resize_opaque) {
		mr_args = frame_create_move_resize_args(fw, FRAME_MR_OPAQUE,
		    &fw->g.frame, &fw->g.frame, 0, DIR_NONE);
	} else {
		Scr.flags.is_wire_frame_displayed = True;
	}
	MyXGrabKeyboard(dpy);
	...
	if (PressedW != Scr.Root && motion.x == 0 && motion.y == 0 &&
		...
		__resize_get_dir_proximity(&motion, fw, toff, p2, &warp,
	...
	if (do_warp_to_border == True) {
		...
		if (automatic_border_direction == True &&
		    (warp.x >= 0 && warp.y >= 0) && !IS_SHADED(fw)) {
			dd.x = warp.x;
			dd.y = warp.y;
		}
	...
	if (exc->x.elast->type == KeyPress) {
		...
		if (FQueryPointer(dpy, Scr.Root, &JunkRoot, &JunkChild,
			&stashed.x, &stashed.y, &JunkX, &JunkY,
			&JunkMask) == False) {
			/* pointer is on a different screen */
			stashed.x = 0;
			stashed.y = 0;
		}
		__resize_step(exc, stashed, &toff, drag, orig, &motion,
	...
	while (!is_finished && bad_window != FW_W(fw)) {
		...
		while (rc != -1 &&
			...
			rc    = HandlePaging(&ev, d, &p, &delta, False, False,
			       True, delay);
			if (rc == 1) {
				/* Fake an event to force window reposition */
		...
		if (rc == -1) {
			FMaskEvent(dpy,
			    evmask | EnterWindowMask | LeaveWindowMask, &ev);
		}
		...
		case MotionNotify:
			...
			if (!fForceRedraw) {
				...
				__resize_step(exc, p, &off, drag, orig, &motion,
				    do_resize_opaque, is_direction_fixed);
				is_resized = True;
				/* need to move the viewport */
				HandlePaging(&ev, d, &p, &delta, False, False,
				    False, delay);
			}
	...
	if (is_aborted || bad_window == FW_W(fw)) {
		...
		if (stashed.x >= 0) {
			FWarpPointer(dpy, None, Scr.Root, 0, 0, 0, 0, stashed.x,
			    stashed.y);
		}
		if (was_maximized) {
		...
		if (do_resize_opaque) {
			...
			motion.x = 1;
			motion.y = 1;
			__resize_step(exc, psorig, &toff, &g, orig, &motion,
			    do_resize_opaque, True);
		}
		...
		constrain_size(fw, exc->x.elast, &drag->width, &drag->height,
		...
		if (do_resize_opaque) {
			...
			frame_setup_window(fw, new_g.x, new_g.y, new_g.width,
			    new_g.height, False);
		}
		if (IS_SHADED(fw)) {
...
Bool
is_window_sticky_across_pages(FvwmWindow *fw)
{
...
Bool
is_window_sticky_across_desks(FvwmWindow *fw)
{
...
static void
move_sticky_window_to_same_page(FvwmWindow *fw, position *p11, position *p12)
{
	position p21;
	position p22;
	position page;
 ...
 */
static void
grow_to_closest_type(FvwmWindow *fw, rectangle *win_r, rectangle bound,
    int *layers, int type, bool consider_touching)
{
	FvwmWindow *twin;
	rectangle   other;
	int maskout = (SNAP_SCREEN | SNAP_SCREEN_WINDOWS | SNAP_SCREEN_ICONS |
...
static void
unmaximize_fvwm_window(FvwmWindow *fw, cmdparser_context_t *pc)
{
	char	 *cmd;
	rectangle new_g;
	 ...
	 * geometry.
	 */
	get_relative_geometry(fw, &new_g,
	    fw->fullscreen.was_maximized ? &fw->fullscreen.g.max :
					   &fw->g.normal);
	...
	frame_setup_window(fw, new_g.x, new_g.y, new_g.width, new_g.height,
	    True);
	...
	if (fw->fullscreen.is_shaded) {
		execute_function_override_window(NULL, NULL, "WindowShade on",
		    NULL, 0, fw);
	...
	if (fw->fullscreen.is_iconified) {
		execute_function_override_window(NULL, NULL, "Iconify on", NULL,
		    0, fw);
...
static void
maximize_fvwm_window(FvwmWindow *fw, rectangle *geometry)
{
	SET_MAXIMIZED(fw, 1);
	fw->g.max_defect.width	= 0;
	fw->g.max_defect.height = 0;
	constrain_size(fw, NULL, &geometry->width, &geometry->height, 0, 0,
	    CS_UPDATE_MAX_DEFECT);
	fw->g.max = *geometry;
	if (IS_SHADED(fw)) {
		get_shaded_geometry(fw, geometry, &fw->g.max);
	}
	frame_setup_window(fw, geometry->x, geometry->y, geometry->width,
...
CMD_Maximize(F_CMD_ARGS)
{
	...
	if (!is_function_allowed(F_MAXIMIZE, NULL, fw, RQORIG_PROGRAM_US,
		False)) {
		XBell(dpy, 0);
		return;
	}
	...
	while (!global_flag_parsed) {
		...
		if (!token) {
			...
			if (StrEquals(token, "screen")) {
				...
				FScreenGetScrRect(&arg, FSCREEN_BY_NAME, &scr.x,
				    &scr.y, &scr.width, &scr.height);
			} else if (StrEquals(token, "ewmhiwa")) {
				...
				n = GetIntegerArguments(taction, &action,
				    layers, 2);
				if (n != 2) {
					layers[0] = -1;
					layers[1] = -1;
	...
	if (!is_screen_given) {
		...
		FScreenGetScrRect(&fscr, FSCREEN_XYPOS, &scr.x, &scr.y,
	...
	if (!ignore_working_area) {
		EWMH_GetWorkAreaIntersection(fw, &scr.x, &scr.y, &scr.width,
		    &scr.height, EWMH_MAXIMIZE_MODE(fw));
	}
#if 0
	...
	} else /* maximize */
	{
		...
		if (grow_up || grow_down) {
			...
			if (grow_down) {
				bound.height = page.y + scr.y + scr.height -
				    bound.y;
			}
		...
		if (grow_left || grow_right) {
			...
			if (grow_right) {
				bound.width = page.x + scr.x + scr.width -
				    bound.x;
			}
			grow_to_closest_type(fw, &new_g, bound, layers,
			    SNAP_NONE, true);
...
void
CMD_ResizeMoveMaximize(F_CMD_ARGS)
{
	rectangle	normal_g;
	rectangle	max_g;
	Bool		was_resized;
...
int
stick_across_pages(F_CMD_ARGS, int toggle)
{
	FvwmWindow *fw = exc->w.fw;
...
int
stick_across_desks(F_CMD_ARGS, int toggle)
{
	FvwmWindow *fw = exc->w.fw;
...
static void
__handle_stick_exit(FvwmWindow *fw, int do_not_draw, int do_silently)
{
	if (do_not_draw == 0) {
		border_draw_decorations(fw, PART_TITLE | PART_BUTTONS,
		    (Scr.Hilite == fw), True, CLEAR_ALL, NULL, NULL);
	}
	if (!do_silently) {
		BroadcastConfig(M_CONFIGURE_WINDOW, fw);
		EWMH_SetWMState(fw, False);
...
void
handle_stick_across_pages(F_CMD_ARGS, int toggle, int do_not_draw,
    int do_silently)
{
	FvwmWindow *fw = exc->w.fw;
	int	    did_change;
...
void
handle_stick_across_desks(F_CMD_ARGS, int toggle, int do_not_draw,
    int do_silently)
{
	FvwmWindow *fw = exc->w.fw;
	int	    did_change;
...
void
handle_stick(F_CMD_ARGS, int toggle_page, int toggle_desk, int do_not_draw,
    int do_silently)
{
	FvwmWindow *fw = exc->w.fw;
	int	    did_change;
...
void
CMD_Stick(F_CMD_ARGS)
{
	int toggle;

	toggle = ParseToggleArgument(action, &action, -1, 0);
	if (toggle == -1 &&
	    IS_STICKY_ACROSS_DESKS(exc->w.fw) !=
...
void
CMD_StickAcrossPages(F_CMD_ARGS)
{
	int toggle;
...
void
CMD_StickAcrossDesks(F_CMD_ARGS)
{
	int toggle;
