 ...
 */
static void
change_grab_cursor(int cursor)
{
	if (cursor != None) {
		XChangeActivePointerGrab(dpy, GRAB_EVMASK,
...
Bool
GrabEm(int cursor, int grab_context)
{
	int	      i	  = 0;
	int	      val = 0;
	int	      rep;
	...
	if (grab_count[GRAB_ALL] > grab_count[GRAB_PASSIVE]) {
		...
		if (grab_context == GRAB_FREEZE_CURSOR) {
			if (XGrabPointer(dpy,
				(PressedW != None) ? PressedW : Scr.NoFocusWin,
				True, GRAB_EVMASK, GrabModeAsync, GrabModeAsync,
	...
	XFlush(dpy);
	while (i < rep &&
	    (val = XGrabPointer(dpy, grab_win, True, GRAB_EVMASK, GrabModeAsync,
		       GrabModeAsync, None,
...
void
fvwm_msg_report_app_and_workers(void)
{
	fvwm_debug(__func__,
...
void
set_last_added_item(last_added_item_t type, void *item)
{
...
void
NewFontAndColor(FlocaleFont *flf, Pixel color, Pixel backcolor)
{
 ...
 */
void
Keyboard_shortcuts(XEvent *ev, FvwmWindow *fw, int *x_defect, int *y_defect,
    int ReturnEvent)
{
	int x_move_size = 0;
	int y_move_size = 0;
...
Bool
check_if_fvwm_window_exists(FvwmWindow *fw)
{
	FvwmWindow *t;
...
Bool
IsRectangleOnThisPage(struct monitor *m, const rectangle *rec, int desk)
{
#if 0
...
get_pointer_fvwm_window(void)
{
	...
	if (FQueryPointer(dpy, Scr.Root, &JunkRoot, &win, &JunkX, &JunkY, &x,
		&y, &JunkMask) == False) {
	...
	for (t = NULL; win != Scr.Root && win != None; win = ancestor) {
		...
		children = NULL;
		if (!XQueryTree(dpy, win, &root, &ancestor, &children,
			&nchildren)) {
			return NULL;
		}
		if (children) {
...
print_g(char *text, rectangle *g)
{
	...
	fvwm_debug(__func__, "%4d %4d %4dx%4d (%4d - %4d, %4d - %4d)\n", g->x,
	    g->y, g->width, g->height, g->x, g->x + g->width, g->y,
	    g->y + g->height);
