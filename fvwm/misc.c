...
GrabEm(int cursor, int grab_context)
{
	...
	if (grab_count[GRAB_ALL] > grab_count[GRAB_PASSIVE]) {
		...
		if (grab_context == GRAB_FREEZE_CURSOR) {
			if (XGrabPointer(dpy,
				...
				None, Scr.FvwmCursors[CRS_DEFAULT], CurrentTime)
			    != GrabSuccess) {
				return False;
			}
			return True;
	...
	XFlush(dpy);
	while (i < rep
	       && (val = XGrabPointer(dpy, grab_win, True, GRAB_EVMASK,
...
void
Keyboard_shortcuts(
    XEvent *ev, FvwmWindow *fw, int *x_defect, int *y_defect, int ReturnEvent)
{
	int x_move_size = 0;
	int y_move_size = 0;
