...
static const unsigned int default_cursors[CRS_MAX] = {
	None, XC_top_left_corner, /* CRS_POSITION */
...
CMD_CursorStyle(F_CMD_ARGS)
{
	...
	if (my_nc > -1) {
		...
		if ((nc < 0) || (nc >= XC_num_glyphs) || ((nc % 2) != 0)) {
			fvwm_debug(__func__, "Bad cursor number %s", newcursor);
			free(newcursor);
		...
		if (StrEquals("none", newcursor) ||
			...
			XSetForeground(dpy, Scr.MonoGC,
			    (tolower(*newcursor) == 'n') ? 0 : 1);
			XFillRectangle(dpy, Scr.ScratchMonoPixmap, Scr.MonoGC,
			    0, 0, 1, 1);
			cursor = XCreatePixmapCursor(dpy, Scr.ScratchMonoPixmap,
			...
			if (!path) {
				fvwm_debug(__func__, "Cursor %s not found",
				    newcursor);
				free(newcursor);

				return;
			}
			if (GetIntegerArguments(action, &tmp, hotspot, 2) ==
			    2) {
				action = tmp;
			}
			cursor = PImageLoadCursorFromFile(dpy, Scr.Root, path,
			    hotspot[0], hotspot[1]);
			free(path);
		}
	}
	if (!cursor) {
		fvwm_debug(__func__, "Cannot load cursor: %s", newcursor);
		free(newcursor);
	...
	for (fw2 = Scr.FvwmRoot.next; fw2; fw2 = fw2->next) {
		...
		for (i = 0; i < 4; i++) {
			SafeDefineCursor(FW_W_CORNER(fw2, i),
			    Scr.FvwmCursors[CRS_TOP_LEFT + i]);
			SafeDefineCursor(FW_W_SIDE(fw2, i),
			    Scr.FvwmCursors[CRS_TOP + i]);
		}
		for (i = 0; i / 2 < Scr.nr_left_buttons; i += 2) {
			SafeDefineCursor(FW_W_BUTTON(fw2, i),
		...
		if (index == CRS_DEFAULT) {
			SafeDefineCursor(FW_W_FRAME(fw2),
			    Scr.FvwmCursors[CRS_DEFAULT]);
			SafeDefineCursor(FW_W_PARENT(fw2),
			    Scr.FvwmCursors[CRS_DEFAULT]);
			if (IS_ICONIFIED(fw2)) {
				if (!HAS_NO_ICON_TITLE(fw2)) {
					SafeDefineCursor(FW_W_ICON_TITLE(fw2),
					    Scr.FvwmCursors[CRS_DEFAULT]);
				}
				if (FW_W_ICON_PIXMAP(fw2) != None) {
					SafeDefineCursor(FW_W_ICON_PIXMAP(fw2),
					    Scr.FvwmCursors[CRS_DEFAULT]);
				}
			}
		}
	...
	SafeDefineCursor(m->PanFrameRight.win, Scr.FvwmCursors[CRS_RIGHT_EDGE]);
 ...
 * functions. */
void
CMD_BusyCursor(F_CMD_ARGS)
{
	char *option	= NULL;
	char *optstring = NULL;
	char *args	= NULL;
	int   flag	= -1;
	char *optlist[] = { "read", "wait", "modulesynchronous", "dynamicmenu",
		"*", NULL };

	while (action && *action != '\0') {
		action = GetQuotedString(action, &optstring, ",", NULL, NULL,
		    NULL);
		if (!optstring) {
			break;
		}
		...
		free(optstring);
		if (flag == -1) {
			fvwm_debug(__func__, "error in boolean specification");
			free(option);
			break;
		}
		...
		default:
			fvwm_debug(__func__, "unknown context '%s'", option);
			break;
		}
		free(option);
	}
