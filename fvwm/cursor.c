...
void CMD_CursorStyle(F_CMD_ARGS)
{
	...
	if (my_nc > -1) {
		...
		if (StrEquals("none", newcursor)
			...
			XFillRectangle(
			    dpy, Scr.ScratchMonoPixmap, Scr.MonoGC, 0, 0, 1, 1);
			...
			if (!path) {
				fvwm_debug(
				    __func__, "Cursor %s not found", newcursor);
				free(newcursor);
	...
	if (fore && back) {
		...
		XRecolorCursor(
		    dpy, Scr.FvwmCursors[index], &(colors[0]), &(colors[1]));
	}
	if (fore) {
		free(fore);
	}
	...
	for (fw2 = Scr.FvwmRoot.next; fw2; fw2 = fw2->next) {
		...
		for (i = 0; i < 4; i++) {
			...
			SafeDefineCursor(
			    FW_W_SIDE(fw2, i), Scr.FvwmCursors[CRS_TOP + i]);
		}
