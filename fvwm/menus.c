...
#define SCTX_SET_MI(ctx, item) \
	((ctx).type = SCTX_MENU_ITEM, (ctx).menu_item.menu_item = (item))
#define SCTX_GET_MI(ctx) \
	((ctx).type == SCTX_MENU_ITEM ? (ctx).menu_item.menu_item : NULL)
#define SCTX_SET_MR(ctx, root) \
	((ctx).type = SCTX_MENU_ROOT, (ctx).menu_root.menu_root = (root))
#define SCTX_GET_MR(ctx) \
	((ctx).type == SCTX_MENU_ROOT ? (ctx).menu_root.menu_root : NULL)

#define MR_IS_TRANSLUCENT(mr) \
	(!MR_IS_TEAR_OFF_MENU(mr) && MST_IS_TRANSLUCENT(mr))
#define MR_IS_TRANSPARENT(mr) \
	(MR_IS_TRANSLUCENT(mr) || CSET_IS_TRANSPARENT(MST_CSET_MENU((mr))))
...
typedef enum { SCTX_MENU_ROOT, SCTX_MENU_ITEM } string_context_type_t;
...
typedef struct {
	char delimiter;
	Bool (*string_handler)(char *string, char delimiter,
...
static void
__menu_execute_function(const exec_context_t **pexc, char *action)
{
	...
	if (!check_if_fvwm_window_exists((*pexc)->w.fw)) {
		...
		ecc.w.w	       = None;
		ecc.w.wcontext = 0;
		exc	       = exc_clone_context(*pexc, &ecc,
...
static Bool
pointer_in_active_item_area(int x_offset, MenuRoot *mr)
{
	...
	if (MST_USE_LEFT_SUBMENUS(mr)) {
		return (x_offset <= MR_ITEM_X_OFFSET(mr) + MR_ITEM_WIDTH(mr) -
...
static Bool
pointer_in_passive_item_area(int x_offset, MenuRoot *mr)
{
	float ratio = (float)MST_ACTIVE_AREA_PERCENT(mr) / 100.0;
...
static void
warp_pointer_to_title(MenuRoot *mr)
{
...
static MenuItem *
warp_pointer_to_item(MenuRoot *mr, MenuItem *mi, Bool do_skip_title)
{
	if (do_skip_title) {
...
static void
get_menu_repaint_transparent_parameters(MenuRepaintTransparentParameters *pmrtp,
    MenuRoot *mr, FvwmWindow *fw)
{
	pmrtp->mr = mr;
	pmrtp->fw = fw;
...
static void
animated_move_back(MenuRoot *mr, Bool do_warp_pointer, FvwmWindow *fw)
{
	...
	if (menu_get_geometry(mr, &JunkRoot, &start.x, &start.y, &JunkWidth,
		...
		if (MR_IS_TRANSPARENT(mr)) {
			transparent_bg = True;
			get_menu_repaint_transparent_parameters(&mrtp, mr, fw);
			if (MR_IS_TRANSLUCENT(mr) && MR_SUBMENU_ITEM(mr)) {
				MenuRoot *smr;
				smr = seek_submenu_instance(mr,
				    MR_SUBMENU_ITEM(mr));
				if (smr) {
		...
		AnimatedMoveOfWindow(MR_WINDOW(mr), start, end, do_warp_pointer,
		    -1, NULL, (transparent_bg) ? &mrtp : NULL);
		MR_XANIMATION(mr) = 0;
	}
...
static void
move_any_menu(MenuRoot *mr, MenuParameters *pmp, int endX, int endY)
{
	if (MR_IS_TEAR_OFF_MENU(mr)) {
		 ...
		 * are used. */
		AnimatedMoveFvwmWindow(pmp->tear_off_root_menu_window,
		    FW_W_FRAME(pmp->tear_off_root_menu_window), start, end,
		    False, 0, &fFull);
	} else {
		int x;
		int y;
		...
		if (CSET_IS_TRANSPARENT(ST_CSET_MENU(MR_STYLE(mr)))) {
			...
			get_menu_repaint_transparent_parameters(&mrtp, mr,
			    (*pmp->pexc)->w.fw);
			update_transparent_menu_bg(&mrtp, x, y, endX, endY,
			    endX, endY);
			XMoveWindow(dpy, MR_WINDOW(mr), endX, endY);
			repaint_transparent_menu(&mrtp, False, endX, endY, endX,
			    endY, True);
		} else {
			XMoveWindow(dpy, MR_WINDOW(mr), endX, endY);
		}
...
static MenuRoot *
seek_submenu_instance(MenuRoot *parent_menu, MenuItem *parent_item)
{
	MenuRoot *mr;
...
static Bool
is_submenu_mapped(MenuRoot *parent_menu, MenuItem *parent_item)
{
...
static MenuRoot *
mr_popup_for_mi(MenuRoot *mr, MenuItem *mi)
{
	char	 *menu_name;
	MenuRoot *menu = NULL;
    ...
    Window p_child, int p_rx, int p_ry)
{
	...
	if (p_rx < 0) {
		if (!FQueryPointer(dpy, Scr.Root, &JunkRoot, &Child, &root_x,
			&root_y, &JunkX, &JunkY, &JunkMask)) {
	...
	if (!XTranslateCoordinates(dpy, Scr.Root, MR_WINDOW(mr), root_x, root_y,
		&x, &y, &JunkChild)) {
		return NULL;
	}
	...
	for (mi = MR_FIRST_ITEM(mr); mi; mi = MI_NEXT_ITEM(mi)) {
		...
		a = (MI_PREV_ITEM(mi) && MI_IS_SELECTABLE(MI_PREV_ITEM(mi))) ?
		    r / 2 :
		    0;
...
static Bool
is_double_click(Time t0, MenuItem *mi, MenuParameters *pmp, MenuReturn *pmret,
 ...
 */
static void
scanForHotkeys(MenuItem *it, int column)
{
	char *start;
	char *s;
	char *t;
...
static void
__copy_down(char *remove_from, char *remove_to)
{
	char *t1;
	char *t2;
...
static int
__check_for_delimiter(char *s, const string_def_t *string_defs)
{
	int type;
...
static void
scanForStrings(char *instring, const string_def_t *string_defs,
    string_context_t *context)
{
	...
	for (s = instring; *s != '\0'; s++) {
		if (type < 0) {
			...
			s[0]	 = '\0';
			is_valid = string_defs[type].string_handler(string,
			    string_defs[type].delimiter, context);
			/* restore the string */
...
/* Side picture support: this scans for a color int the menu name
   for colorization */
static Bool
__scan_for_color(char *name, char type, string_context_t *context)
{
...
static Bool
__scan_for_pixmap(char *name, char type, string_context_t *context)
{
	FvwmPicture	     *p;
	...
	case '%':
		...
		while (current_mini_icon < MAX_MENU_ITEM_MINI_ICONS) {
			if (MI_MINI_ICON(
				SCTX_GET_MI(*context))[current_mini_icon]) {
				current_mini_icon++;
			} else {
				break;
			}
	...
	fpa.mask = 0;
	p	 = PCacheFvwmPicture(dpy, Scr.NoFocusWin, NULL, name, fpa);
	if (!p) {
		fvwm_debug(__func__, "Couldn't load image from %s", name);
		fvwm_debug(__func__,
...
static void
unlink_item_from_menu(MenuRoot *mr, MenuItem *mi)
{
	MenuItem *next;
	MenuItem *prev;
 ...
 * menu. */
static void
append_item_to_menu(MenuRoot *mr, MenuItem *mi, Bool do_replace_title)
{
	if (MR_FIRST_ITEM(mr) == NULL) {
		...
		if (MI_IS_TITLE(MR_FIRST_ITEM(mr))) {
			...
			if (MI_NEXT_ITEM(MR_FIRST_ITEM(mr)) != NULL) {
				MI_PREV_ITEM(
				    MI_NEXT_ITEM(MR_FIRST_ITEM(mr))) = mi;
			}
...
static void
clone_menu_item_list(MenuRoot *dest_mr, MenuRoot *src_mr)
{
	MenuItem *mi;
	MenuItem *cloned_mi;
	MenuRoot *mr;
	...
	for (mr = src_mr; mr != NULL; mr = MR_CONTINUATION_MENU(mr)) {
		...
		for (mi = MR_FIRST_ITEM(mr); mi != NULL;
		     mi = MI_NEXT_ITEM(mi)) {
...
static void
calculate_item_sizes(MenuSizingParameters *msp)
{
	MenuItem	  *mi;
	MenuItemPartSizesT mipst;
	int		   i;
	Bool do_reverse_icon_order = (MST_USE_LEFT_SUBMENUS(msp->menu)) ? True :
									  False;
	...
	for (mi = MR_FIRST_ITEM(msp->menu); mi != NULL; mi = MI_NEXT_ITEM(mi)) {
		if (MI_IS_TITLE(mi)) {
			menuitem_get_size(mi, &mipst, MST_PTITLEFONT(msp->menu),
			    do_reverse_icon_order);
		} else {
			menuitem_get_size(mi, &mipst, MST_PSTDFONT(msp->menu),
			    do_reverse_icon_order);
		}
		/* adjust maximums */
 ...
 */
static void
size_menu_horizontally(MenuSizingParameters *msp)
{
	MenuItem *mi;
	...
	int	 *item_order[MAX_MENU_ITEM_LABELS + MAX_MENU_ITEM_MINI_ICONS +
	...
	while (*format && !done) {
		...
		case '%':
			...
			if (sscanf(format, "%d.%d%n", &gap_left, &gap_right,
				&chars) >= 2 ||
			...
			case 'r':
				...
				if (msp->max.i.label_width[columns_placed] <=
				    0) {
					columns_placed++;
					break;
				}
			...
			case 'i':
				...
				if (msp->max.i.icon_width[icons_placed] > 0) {
					...
					[icons_placed] = x;
					x += msp->max.i
						 .icon_width[icons_placed] +
					    gap_right;
					item_order[used_objects++] = &(
					    MR_ICON_X_OFFSET(
						msp->menu)[icons_placed]);
			...
			case '\t':
				x += MENU_TAB_WIDTH *
				    FlocaleTextWidth(MST_PSTDFONT(msp->menu),
					" ", 1);
				break;
			case ' ':
				/* Advance the x position. */
				x += FlocaleTextWidth(MST_PSTDFONT(msp->menu),
				    format, 1);
				break;
			default:
		...
		case '\t':
			x += MENU_TAB_WIDTH *
			    FlocaleTextWidth(MST_PSTDFONT(msp->menu), " ", 1);
			break;
		case ' ':
			/* Advance the x position. */
			x += FlocaleTextWidth(MST_PSTDFONT(msp->menu), format,
			    1);
			break;
		default:
			/* Ignore unknown characters. */
	...
	if (!triangle_placed) {
		MR_TRIANGLE_X_OFFSET(msp->menu) = 2 *
		    MR_SCREEN_WIDTH(msp->menu);
	}
	...
	if (relief_end_placed) {
		MR_HILIGHT_WIDTH(msp->menu) = MR_HILIGHT_WIDTH(msp->menu) -
		    MR_HILIGHT_X_OFFSET(msp->menu);
	} else {
...
static int
calc_more_item_height(MenuSizingParameters *msp)
{
	int height;
...
static int
calc_normal_item_height(MenuSizingParameters *msp, MenuItem *mi)
{
	int height;

	height = MST_ITEM_GAP_ABOVE(msp->menu) + MST_ITEM_GAP_BELOW(msp->menu) +
	    MST_RELIEF_THICKNESS(msp->menu);
 ...
 */
static Bool
size_menu_vertically(MenuSizingParameters *msp)
{
	MenuItem *mi;
	int	  y;
	int	  cItems;
	...
	MR_ITEM_TEXT_Y_OFFSET(msp->menu) = MST_PSTDFONT(msp->menu)->ascent +
	    relief_thickness + MST_ITEM_GAP_ABOVE(msp->menu);
	...
	for (cItems = 0, mi = MR_FIRST_ITEM(msp->menu); mi != NULL;
		...
		Bool last_item_has_relief = (MI_PREV_ITEM(mi)) ?
		...
		if (MI_IS_TITLE(mi)) {
			...
			default:
				...
				MI_HEIGHT(mi) += MENU_UNDERLINE_HEIGHT *
		...
		menu_height = y + MST_BORDER_WIDTH(msp->menu) +
		...
		if (menu_height > MR_SCREEN_HEIGHT(msp->menu)) {
			...
			tempname = fxmalloc((10 + strlen(t)) * sizeof(char));
			...
			MR_LAST_ITEM(menuContinuation)	= MR_LAST_ITEM(
			     msp->menu);
			MR_ITEMS(menuContinuation) = MR_ITEMS(msp->menu) -
			    cItems;
			...
			AddToMenu(msp->menu, name, tempname,
 ...
 */
static void
merge_continuation_menus(MenuRoot *mr)
{
	...
	while (MR_CONTINUATION_MENU(mr) != NULL) {
		...
		MI_NEXT_ITEM(MI_PREV_ITEM(MR_LAST_ITEM(mr))) = MR_FIRST_ITEM(
		    cont);
		MI_PREV_ITEM(MR_FIRST_ITEM(cont)) = MI_PREV_ITEM(
		    MR_LAST_ITEM(mr));
 ...
 */
static void
make_menu_window(MenuRoot *mr, Bool is_tear_off)
{
	...
	if (MR_WINDOW(mr) != None) {
		...
		XChangeWindowAttributes(dpy, MR_WINDOW(mr), valuemask,
		    &attributes);
	} else {
		/* create a new window */
		...
		MR_WINDOW(mr) = XCreateWindow(MR_CREATE_DPY(mr), Scr.Root, 0, 0,
		    w, h, 0, Pdepth, InputOutput, Pvisual, valuemask,
		    &attributes);
 ...
 */
static void
make_menu(MenuRoot *mr, Bool is_tear_off)
{
 ...
 * changed. */
static void
update_menu(MenuRoot *mr, MenuParameters *pmp)
{
	int		sw, sh, sx, sy;
 ...
 */
static void
clone_menu_root_static(MenuRoot *dest_mr, MenuRoot *src_mr)
{
	...
	if (MR_MISSING_SUBMENU_FUNC(src_mr)) {
		MR_MISSING_SUBMENU_FUNC(dest_mr) = fxstrdup(
		    MR_MISSING_SUBMENU_FUNC(src_mr));
	}
	if (MR_HAS_SIDECOLOR(src_mr)) {
		MR_SIDECOLOR(dest_mr) = fvwmlib_clone_color(
		    MR_SIDECOLOR(src_mr));
	}
...
static int
float_to_int_with_tolerance(float f)
{
	int low;
...
static void
get_xy_from_position_hints(struct MenuPosHints *ph, int width, int height,
    int context_width, Bool do_reverse_x, int *ret_x, int *ret_y)
{
	float x_add;
	float y_add;
 ...
 */
static char *
get_one_menu_position_argument(char *action, int x, int w, int *pFinalX,
    int *x_offset, float *width_factor, float *context_width_factor,
...
static void
get_popup_options(MenuParameters *pmp, MenuItem *mi, MenuOptions *pops)
{
	if (!mi) {
		return;
	}
	...
	get_menu_options(SkipNTokens(MI_ACTION(mi), 2), MR_WINDOW(pmp->menu),
	    NULL, NULL, pmp->menu, mi, pops);
 ...
 */
static void
paint_side_pic(MenuRoot *mr, XEvent *pevent)
{
	GC	     gc;
	FvwmPicture *sidePic;
	int	     ys;
	...
	if (pevent != NULL && pevent->type == Expose) {
		if (pevent->xexpose.x + pevent->xexpose.width <
		...
		if (pevent->xexpose.y + pevent->xexpose.height < bw ||
	...
	if (MR_HAS_SIDECOLOR(mr) || MST_HAS_SIDE_COLOR(mr)) {
		...
		XClearArea(dpy, MR_WINDOW(mr), MR_SIDEPIC_X_OFFSET(mr), yt,
		    sidePic->width, h, False);
	}
	PGraphicsRenderPicture(dpy, MR_WINDOW(mr), sidePic, 0, MR_WINDOW(mr),
	    gc, Scr.MonoGC, Scr.AlphaGC, 0, ys, sidePic->width, h,
 ...
 */
static void
get_menu_paint_item_parameters(MenuPaintItemParameters *mpip, MenuRoot *mr,
    MenuItem *mi, FvwmWindow *fw, XEvent *pevent, Bool do_redraw_menu_border)
{
	mpip->ms		     = MR_STYLE(mr);
	mpip->w			     = MR_WINDOW(mr);
 ...
 */
static void
paint_menu(MenuRoot *mr, XEvent *pevent, FvwmWindow *fw)
{
	MenuItem  *mi;
	...
	if (MR_IS_PAINTED(mr) && pevent &&
		...
		RelieveRectangle(dpy, MR_WINDOW(mr), 0, 0, MR_WIDTH(mr) - 1,
		    MR_HEIGHT(mr) - 1,
		    (Pdepth < 2) ? SHADOW_GC(MST_MENU_INACTIVE_GCS(mr)) :
	...
	if (MR_IS_TRANSLUCENT(mr)) {
		...
		if (MR_IS_BACKGROUND_SET(mr) == False) {
			trans = PGraphicsCreateTranslucent(dpy, MR_WINDOW(mr),
			    &fra, BACK_GC(ST_MENU_INACTIVE_GCS(ms)), mr->d->x,
			    mr->d->y, MR_WIDTH(mr), MR_HEIGHT(mr));
		...
		SetWindowBackground(dpy, MR_WINDOW(mr), MR_WIDTH(mr),
		    MR_HEIGHT(mr), &Colorset[ST_CSET_MENU(ms)], Pdepth,
	...
	for (mi = MR_FIRST_ITEM(mr); mi != NULL; mi = MI_NEXT_ITEM(mi)) {
		...
		if (do_draw) {
			...
			get_menu_paint_item_parameters(&mpip, mr, NULL, fw,
			    pevent, True);
			mpip.flags.is_first_item = (MR_FIRST_ITEM(mr) == mi);
...
static void
select_menu_item(MenuRoot *mr, MenuItem *mi, Bool select, FvwmWindow *fw)
{
...
static int
get_left_popup_x_position(MenuRoot *mr, MenuRoot *submenu, int x)
{
	if (MST_USE_LEFT_SUBMENUS(mr)) {
...
static int
get_right_popup_x_position(MenuRoot *mr, MenuRoot *submenu, int x)
{
	if (MST_USE_LEFT_SUBMENUS(mr)) {
...
static void
get_prefered_popup_position(MenuRoot *mr, MenuRoot *submenu, int *px, int *py,
    Bool *pprefer_left_submenus)
{
	int	  menu_x, menu_y;
	MenuItem *mi = NULL;

	if (!menu_get_geometry(mr, &JunkRoot, &menu_x, &menu_y, &JunkWidth,
		&JunkHeight, &JunkBW, &JunkDepth)) {
		*px		       = 0;
		*py		       = 0;
		*pprefer_left_submenus = False;
		fvwm_debug(__func__, "can't get geometry of menu %s",
		    MR_NAME(mr));
		return;
	}
	/* set the direction flag */
	*pprefer_left_submenus = (MR_HAS_POPPED_UP_LEFT(mr) ||
	    (MST_USE_LEFT_SUBMENUS(mr) && !MR_HAS_POPPED_UP_RIGHT(mr)));
	/* get the x position */
	...
	if (mi) {
		*py = menu_y + MI_Y_OFFSET(mi) - MST_BORDER_WIDTH(mr) +
...
static int
do_menus_overlap(MenuRoot *mr, int x, int y, int width, int height,
    int h_tolerance, int v_tolerance, int s_tolerance,
    Bool allow_popup_offset_tolerance)
{
	...
	if (!menu_get_geometry(mr, &JunkRoot, &prior_x, &prior_y, &prior_width,
	...
	if (allow_popup_offset_tolerance) {
		...
		if (MST_USE_LEFT_SUBMENUS(mr)) {
			...
			prior_width *= (float)(MST_POPUP_OFFSET_PERCENT(mr)) /
			    100.0;
		}
	}
	if (MST_USE_LEFT_SUBMENUS(mr)) {
 ...
 */
static int
pop_menu_up(MenuRoot **pmenu, MenuParameters *pmp, MenuRoot *parent_menu,
    ...
    Bool *ret_overlap, Window popdown_window)
{
	...
	if (pops->flags.has_poshints &&
		...
		get_xy_from_position_hints(&(pops->pos_hints), MR_WIDTH(mr),
		    MR_HEIGHT(mr), (parent_menu) ? MR_WIDTH(parent_menu) : 0,
	...
	if (!pops->flags.has_poshints && fw && parent_menu == NULL &&
		...
		if (has_context) {
			...
			if (gx == 0 && x < button_g.x) {
			    ...
			    x + MR_WIDTH(mr) >= button_g.x + button_g.width) {
				x = button_g.x + button_g.width - MR_WIDTH(mr);
			}
			if (gy == 0 && y < button_g.y) {
				y = button_g.y;
			} else if (gy == 0 &&
			    y + MR_HEIGHT(mr) >= button_g.y + button_g.height) {
			...
			fscr.xypos.x			  = cx;
			fscr.xypos.y			  = cy;
			if (FScreenGetScrRect(&fscr, FSCREEN_XYPOS, &JunkX,
				&JunkY, &JunkWidth, &JunkHeight)) {
				/* use current cx/cy */
			} else if (FQueryPointer(dpy, Scr.Root, &JunkRoot,
				       &JunkChild, &cx, &cy, &JunkX, &JunkY,
				       &JunkMask)) {
...

	{
		...
		/* clip to screen */
		FScreenClipToScreen(&fscr, FSCREEN_XYPOS, &x, &y, MR_WIDTH(mr),
		    MR_HEIGHT(mr));
	...
	if (parent_menu) {
		...
		x_overlap = do_menus_overlap(parent_menu, x, y, MR_WIDTH(mr),
		    MR_HEIGHT(mr), bwp, bwp, bwp, True);
	}
	...
	if (parent_menu == NULL ||
	    !menu_get_geometry(parent_menu, &JunkRoot, &prev_x, &prev_y,
		...
		x_clipped_overlap = do_menus_overlap(parent_menu, x, y,
		    MR_WIDTH(mr), MR_HEIGHT(mr), bwp, bwp, bwp, True);
		if (x_clipped_overlap &&
			...
			left_x	= get_left_popup_x_position(parent_menu, mr,
			     prev_x);
			right_x = get_right_popup_x_position(parent_menu, mr,
			    prev_x);
			...
			if (MST_IS_ANIMATED(mr)) {
				...
				if (MR_IS_TEAR_OFF_MENU(parent_menu)) {
					...
					if (XGetGeometry(dpy, w, &JunkRoot, &cx,
						&cy, (unsigned int *)&JunkWidth,
				...
				}
				{
					...
					AnimatedMoveOfWindow(w, start, end,
					    True, -1, NULL,
			 ...
			 */
				...
				use_left_as_last_resort = (left_x >
				    scr_x + scr_w - right_x - MR_WIDTH(mr));
				...
				if (!may_use_left && !may_use_right) {
				    ...
				    (prefer_left_submenu || !may_use_right)) {
					do_use_left = True;
				} else {
					do_use_left = False;
				}
	...
	MST_USAGE_COUNT(mr)++;
	if (ret_overlap) {
		*ret_overlap = do_menus_overlap(parent_menu, x, y, MR_WIDTH(mr),
				   MR_HEIGHT(mr), 0, 0, 0, False) ?
		    True :
		    False;
	}
	...
	if (do_warp_to_item) {
		...
		warp_pointer_to_item(mr, MR_FIRST_ITEM(mr),
		    True /* skip Title */);
 ...
 */
static void
pop_menu_down(MenuRoot **pmr, MenuParameters *pmp)
{
	MenuItem *mi;
	...
	if (MR_STORED_PIXELS(*pmr).d_pixels != NULL) {
		PictureFreeColors(dpy, Pcmap, MR_STORED_PIXELS(*pmr).d_pixels,
 ...
 */
static void
pop_menu_down_and_repaint_parent(MenuRoot **pmr, Bool *fSubmenuOverlaps,
    MenuParameters *pmp)
{
	...
	if (*fSubmenuOverlaps && parent) {
		...
		event.type = Expose;
		if (!menu_get_geometry(*pmr, &JunkRoot, &mr_x, &mr_y, &mr_width,
			&mr_height, &JunkBW, &JunkDepth) ||
		    !menu_get_geometry(parent, &JunkRoot, &parent_x, &parent_y,
			...
			if (event.xexpose.x + event.xexpose.width >
			    parent_width) {
				event.xexpose.width = parent_width -
				    event.xexpose.x;
			}
			...
			if (event.xexpose.y + event.xexpose.height >
			    parent_height) {
				event.xexpose.height = parent_height -
				    event.xexpose.y;
			}
...
static void
__mloop_init(MenuParameters *pmp, MenuReturn *pmret, mloop_evh_input_t *in,
    mloop_evh_data_t *med, mloop_static_info_t *msi, MenuOptions *pops)
{
	...
	if (FQueryPointer(dpy, Scr.Root, &JunkRoot, &JunkChild, &(msi->x_init),
...
static void
__mloop_get_event_timeout_loop(MenuParameters *pmp, mloop_evh_input_t *in,
    mloop_evh_data_t *med, mloop_static_info_t *msi)
{
	XEvent e = *(*pmp->pexc)->x.elast;

	while (!FPending(dpy) || !FCheckMaskEvent(dpy, msi->event_mask, &e)) {
		Bool is_popup_timed_out	  = (MST_POPUP_DELAY(pmp->menu) > 0 &&
		      med->popup_delay_10ms++ >= MST_POPUP_DELAY(pmp->menu) + 1);
		Bool is_popdown_timed_out = (MST_POPDOWN_DELAY(pmp->menu) > 0 &&
		    in->mrPopdown &&
		...
		if ((in->mrPopdown || in->mrPopup) &&
		    (is_popdown_timed_out ||
			MST_DO_POPDOWN_IMMEDIATELY(pmp->menu))) {
			MenuRoot *m = (in->mrPopdown) ? in->mrPopdown :
							in->mrPopup;
			...
			if (!m || med->mi != MR_PARENT_ITEM(m)) {
				...
				if (MST_DO_POPUP_IMMEDIATELY(pmp->menu)) {
					in->mif.do_popup_now		= True;
					in->mif.is_popped_up_by_timeout = True;
				} else if (!MST_DO_POPUP_IMMEDIATELY(
					       pmp->menu) &&
...
static mloop_ret_code_t
__mloop_get_event(MenuParameters *pmp, MenuReturn *pmret, mloop_evh_input_t *in,
    mloop_evh_data_t *med, mloop_static_info_t *msi)
{
	XEvent e = *(*pmp->pexc)->x.elast;
	...
	if (pmp->event_propagate_to_submenu) {
		...
		if ((*pmp->pexc)->x.elast->type == KeyPress) {
			...
			if (FQueryPointer(dpy, Scr.Root, &JunkRoot, &JunkChild,
				&e.xkey.x_root, &e.xkey.y_root, &JunkX, &JunkY,
				&e.xkey.state) == False) {
				/* pointer is on a different screen */
				e.xkey.x_root = 0;
				e.xkey.y_root = 0;
	    ...
	    FCheckTypedWindowEvent(dpy,
		FW_W_PARENT(pmp->tear_off_root_menu_window), ClientMessage,
		&e)) {
		...
		if (in->mif.do_force_reposition) {
			...
			if (in->mif.do_force_popup ||
				...
				__mloop_get_event_timeout_loop(pmp, in, med,
				    msi);
			} else {
	...
	if (e.type == MotionNotify) {
		...
		while (FCheckMaskEvent(dpy,
			   ButtonMotionMask | ButtonReleaseMask, &e) &&
...
static mloop_ret_code_t
__mloop_handle_event(MenuParameters *pmp, MenuReturn *pmret,
    double_keypress *pdkp, mloop_evh_input_t *in, mloop_evh_data_t *med,
    mloop_static_info_t *msi)
{
	...
	case ButtonRelease:
		...
		med->mi = find_entry(pmp, &med->x_offset, &med->mrMi,
		...
		if (med->mrMi != NULL) {
			...
			menu_shortcuts(med->mrMi, pmp, pmret,
			    (*pmp->pexc)->x.elast, &med->mi, pdkp, &menu_x,
			    &menu_y);
			if (pmret->rc == MENU_NEWITEM_MOVEMENU) {
				...
				med->mi = find_entry(pmp, NULL, NULL, None, -1,
				    -1);
				pmret->rc = MENU_NEWITEM;
			}
		} else {
		...
		case MENU_SELECTED:
			...
			if (is_double_click(msi->t0, med->mi, pmp, pmret, pdkp,
	...
	case KeyPress:
		...
		if (pmret->flags.is_menu_posted) {
			...
			pmret->flags.is_menu_posted = 0;
			(void)find_entry(pmp, &l_x_offset, &l_mrMi, None, -1,
			    -1);
			if (l_mrMi != NULL) {
				if (pmp->menu != l_mrMi) {
			...
			e.xkey.y_root = menuitem_middle_y_offset(med->mi,
			    MR_STYLE(pmp->menu));
			fev_fake_event(&e);
		}
		...
		/* now handle the actual key press */
		{
			...
			menu_shortcuts(pmp->menu, pmp, pmret,
			    (*pmp->pexc)->x.elast, &med->mi, pdkp, &menu_x,
			    &menu_y);
			if (pmret->rc == MENU_NEWITEM_MOVEMENU) {
	...
	case MotionNotify: {
		...
		if (in->mif.has_mouse_moved == False) {
			if (FQueryPointer(dpy, Scr.Root, &JunkRoot, &p_child,
				&p_rx, &p_ry, &JunkX, &JunkY,
				&JunkMask) == False) {
				/* pointer is on a different screen */
				p_rx = 0;
				p_ry = 0;
		...
		med->mi = find_entry(pmp, &med->x_offset, &med->mrMi, p_child,
		    p_rx, p_ry);
		if (pmret->flags.is_menu_posted &&
	...
	case ClientMessage:
		if ((*pmp->pexc)->x.elast->xclient.format == 32 &&
		    ...
		    (*pmp->pexc)->x.elast->xclient.window ==
			FW_W_PARENT(pmp->tear_off_root_menu_window)) {
...
static void
__mloop_select_item(MenuParameters *pmp, mloop_evh_input_t *in,
    mloop_evh_data_t *med, Bool does_submenu_overlap,
...
static void
__mloop_wants_popup(MenuParameters *pmp, mloop_evh_input_t *in,
    mloop_evh_data_t *med, MenuRoot *mrMiPopup)
{
	...
	if (do_it_now) {
		...
		if (in->mrPopup == NULL || in->mrPopup != mrMiPopup) {
			...
			warp_pointer_to_item(in->mrPopup,
			    MR_FIRST_ITEM(in->mrPopup), True);
		}
	}
...
static mloop_ret_code_t
__mloop_make_popup(MenuParameters *pmp, MenuReturn *pmret,
    ...
    Bool *pdoes_submenu_overlap)
{
	...
	if (!is_submenu_mapped(pmp->menu, med->mi)) {
		...
		get_prefered_popup_position(pmp->menu, in->mrPopup, &x, &y,
		    &prefer_left_submenus);
		/* Note that we don't care if popping up the menu works. If it
		 * doesn't we'll catch it below. */
		pop_menu_up(&in->mrPopup, pmp, pmp->menu, med->mi, pmp->pexc, x,
		    y, prefer_left_submenus, in->mif.do_popup_and_warp, pops,
...
static mloop_ret_code_t
__mloop_get_mi_actions(MenuParameters *pmp, MenuReturn *pmret,
    double_keypress *pdkp, mloop_evh_input_t *in, mloop_evh_data_t *med,
    mloop_static_info_t *msi, MenuRoot *mrMiPopup, Bool *pdoes_submenu_overlap,
    Bool *pdoes_popdown_submenu_overlap)
{
	...
	if (med->mrMi == in->mrPopup) {
		...
		if ((*pmp->pexc)->x.elast->type == ButtonPress &&
		    is_double_click(msi->t0, med->mi, pmp, pmret, pdkp,
			in->mif.has_mouse_moved)) {
			pmret->rc = MENU_DOUBLE_CLICKED;
			return MENU_MLOOP_RET_END;
...
static void
__mloop_do_popdown(MenuParameters *pmp, mloop_evh_input_t *in,
    Bool *pdoes_popdown_submenu_overlap)
{
	if (in->mrPopdown) {
		pop_menu_down_and_repaint_parent(&in->mrPopdown,
...
static mloop_ret_code_t
__mloop_do_popup(MenuParameters *pmp, MenuReturn *pmret, mloop_evh_input_t *in,
    mloop_evh_data_t *med, MenuOptions *pops, MenuRoot *mrMiPopup,
    Bool *pdoes_submenu_overlap, Bool *pdoes_popdown_submenu_overlap)
{
	if (!MR_IS_PAINTED(pmp->menu)) {
		/* draw the parent menu if it is not already drawn */
		FCheckWeedTypedWindowEvents(dpy, MR_WINDOW(pmp->menu), Expose,
		    NULL);
	...
	if (med->mrMi == pmp->menu && mrMiPopup == NULL &&
		...
		menu_name = PeekToken(SkipNTokens(MI_ACTION(med->mi), 1), NULL);
		if (!menu_name) {
			menu_name = "";
		}
		is_complex_function = functions_is_complex_function(
		    missing_action);
		if (is_complex_function) {
			/*TA:  FIXME!  Use xasprintf()!! */
			char *action_ptr;
			action = fxmalloc(strlen("Function") + 3 +
	...
	if (!in->mrPopup) {
		...
		if (__mloop_make_popup(pmp, pmret, in, med, pops,
			pdoes_submenu_overlap) == MENU_MLOOP_RET_END) {
			return MENU_MLOOP_RET_END;
		}
	} /* else (in->mrPopup) */
	...
	if (in->mrPopup) {
		if (MR_PARENT_MENU(in->mrPopup) == pmp->menu) {
			med->mi = find_entry(pmp, NULL, &med->mrMi, None, -1,
			    -1);
...
static mloop_ret_code_t
__mloop_do_menu(MenuParameters *pmp, MenuReturn *pmret, double_keypress *pdkp,
    mloop_evh_input_t *in, mloop_evh_data_t *med, MenuOptions *pops,
    Bool *pdoes_submenu_overlap)
{
	MenuParameters mp;
	...
	if (MST_DO_UNMAP_SUBMENU_ON_POPDOWN(pmp->menu) &&
		...
		pop_menu_down_and_repaint_parent(&in->mrPopup,
		    pdoes_submenu_overlap, pmp);
		in->mi_with_popup	   = NULL;
		MR_SUBMENU_ITEM(pmp->menu) = NULL;
		if (in->mrPopup == in->mrPopdown) {
			in->mrPopdown = NULL;
...
static mloop_ret_code_t
__mloop_handle_action_with_mi(MenuParameters *pmp, MenuReturn *pmret,
    double_keypress *pdkp, mloop_evh_input_t *in, mloop_evh_data_t *med,
    mloop_static_info_t *msi, MenuOptions *pops, Bool *pdoes_submenu_overlap,
    Bool *pdoes_popdown_submenu_overlap)
{
	MenuItem *tmi;
	MenuRoot *tmrMi;
	...
	if (med->mi != MR_SELECTED_ITEM(pmp->menu) && med->mrMi == pmp->menu) {
		...
		__mloop_select_item(pmp, in, med, *pdoes_submenu_overlap,
	...
	if (__mloop_get_mi_actions(pmp, pmret, pdkp, in, med, msi, mrMiPopup,
		pdoes_submenu_overlap,
		pdoes_popdown_submenu_overlap) == MENU_MLOOP_RET_END) {
		return MENU_MLOOP_RET_END;
	}
	...
	if (in->mif.do_popup) {
		if (__mloop_do_popup(pmp, pmret, in, med, pops, mrMiPopup,
			pdoes_submenu_overlap,
			pdoes_popdown_submenu_overlap) == MENU_MLOOP_RET_END) {
			return MENU_MLOOP_RET_END;
		}
	...
	if (in->mif.do_menu) {
		...
		rc = __mloop_do_menu(pmp, pmret, pdkp, in, med, pops,
		    pdoes_submenu_overlap);
...
static mloop_ret_code_t
__mloop_handle_action_without_mi(MenuParameters *pmp, MenuReturn *pmret,
    double_keypress *pdkp, mloop_evh_input_t *in, mloop_evh_data_t *med,
    mloop_static_info_t *msi, MenuOptions *pops, Bool *pdoes_submenu_overlap,
    Bool *pdoes_popdown_submenu_overlap)
{
...
static void
__mloop_exit_warp_back(MenuParameters *pmp)
{
	MenuRoot *tmrMi;
...
static void
__mloop_exit_select_in_place(MenuParameters *pmp, mloop_evh_data_t *med,
    MenuOptions *pops)
{
	MenuRoot	 *submenu;
	XWindowAttributes win_attribs;
	...
	if (submenu && MR_WINDOW(submenu) != None &&
	    ...
	    menu_get_geometry(submenu, &JunkRoot,
		...
		get_prefered_popup_position(pmp->menu, submenu,
		    &last_saved_pos_hints.pos_hints.x,
...
static void
__mloop_exit_selected(MenuParameters *pmp, MenuReturn *pmret,
...
static void
__mloop_exit(MenuParameters *pmp, MenuReturn *pmret, double_keypress *pdkp,
...
static void
__menu_loop(MenuParameters *pmp, MenuReturn *pmret, double_keypress *pdkp)
{
	...
	for (is_finished = False; !is_finished;) {
		...
		mloop_ret = __mloop_handle_event(pmp, pmret, pdkp, &mei, &med,
		    &msi);
		switch (mloop_ret) {
		case MENU_MLOOP_RET_END:
			is_finished = True;
		...
		if (med.mi != NULL) {
			mloop_ret = __mloop_handle_action_with_mi(pmp, pmret,
			    pdkp, &mei, &med, &msi, &mops,
			    &does_submenu_overlap,
			    &does_popdown_submenu_overlap);
		} else {
			mloop_ret = __mloop_handle_action_without_mi(pmp, pmret,
			    pdkp, &mei, &med, &msi, &mops,
			    &does_submenu_overlap,
			    &does_popdown_submenu_overlap);
		}
...
static char *
menu_strip_tear_off_title(MenuRoot *mr)
{
	MenuItem *mi;
	int	  i;
	int	  len;
...
static int
_pred_menu_window_weed_events(Display *display, XEvent *event, XPointer arg)
{
	switch (event->type) {
...
static void
menu_tear_off(MenuRoot *mr_to_copy)
{
	MenuRoot		*mr;
	MenuStyle		*ms;
	XEvent			 ev;
	...
	if (MR_WINDOW(mr_to_copy) != None) {
		XSync(dpy, 0);
		FWeedIfWindowEvents(dpy, MR_WINDOW(mr_to_copy),
	...
	if (!ms) {
		...
		fvwm_debug(__func__, "impossible to create %s menu style",
		    buffer);
		free(buffer);
	...
	make_menu(mr, True);
	/* set position */
	if (menu_get_geometry(mr_to_copy, &JunkRoot, &x, &y, &JunkWidth,
		&JunkHeight, &JunkBW, &JunkDepth)) {
		add_mask = PPosition;
		XMoveWindow(dpy, MR_WINDOW(mr), x, y);
	} else {
	...
	menuclasshints.res_name = (name != NULL) ? name : fxstrdup(MR_NAME(mr));
	...
	XSetWMProperties(dpy, MR_WINDOW(mr), &menunametext, &menunametext, NULL,
	    0, &menusizehints, NULL, &menuclasshints);
	...
	ea.exc	       = exc_create_context(&ecc,
		    ECC_TYPE | ECC_ETRIGGER | ECC_W | ECC_WCONTEXT);
...
static void
do_menu_close_tear_off_menu(MenuRoot *mr, MenuParameters mp)
{
	pop_menu_down(&mr, &mp);
...
void
menus_remove_style_from_menus(MenuStyle *ms)
{
	MenuRoot *mr;
...
void
menu_enter_tear_off_menu(const exec_context_t *exc)
{
	MenuRoot	      *mr;
	...
	if (XFindContext(dpy, FW_W(exc->w.fw), MenuContext, (caddr_t *)&mr) ==
	    XCNOENT) {
		return;
	}
	ecc.w.fw       = NULL;
...
void
menu_close_tear_off_menu(FvwmWindow *fw)
{
	MenuRoot	      *mr;
	MenuParameters	       mp;
	const exec_context_t  *exc;
	...
	if (XFindContext(dpy, FW_W(fw), MenuContext, (caddr_t *)&mr) ==
	    XCNOENT) {
		return;
	}
	memset(&mp, 0, sizeof(mp));
...
Bool
menu_redraw_transparent_tear_off_menu(FvwmWindow *fw, Bool pr_only)
{
	MenuRoot  *mr;
	MenuStyle *ms = NULL;
	int	   cs;
	...
	return UpdateBackgroundTransparency(dpy, MR_WINDOW(mr), MR_WIDTH(mr),
	    MR_HEIGHT(mr), &Colorset[ST_CSET_MENU(ms)], Pdepth,
	    FORE_GC(MST_MENU_INACTIVE_GCS(mr)), True);
}
 ...
 */
void
do_menu(MenuParameters *pmp, MenuReturn *pmret)
{
	int		x;
	int		y;
	...
	if (FQueryPointer(dpy, Scr.Root, &JunkRoot, &JunkChild, &x, &y, &JunkX,
	...
	if (!pmp->flags.is_already_mapped) {
		...
		if (pmp->flags.is_submenu) {
			...
			get_prefered_popup_position(pmp->parent_menu, pmp->menu,
			    &x, &y, &prefer_left_submenus);
		} else {
			fscreen_scr_arg fscr;
			...
			fscr.xypos.x = x;
			fscr.xypos.y = y;
			FScreenGetScrRect(&fscr, FSCREEN_XYPOS, &scr_x, &scr_y,
			    &scr_w, &scr_h);
			x -= menudim_middle_x_offset(&MR_DIM(pmp->menu));
			y -= menuitem_middle_y_offset(MR_FIRST_ITEM(pmp->menu),
			    MR_STYLE(pmp->menu));
			if (x < scr_x) {
				x = scr_x;
			}
		...
		if (!pop_menu_up(&(pmp->menu), pmp, pmp->parent_menu, NULL,
		...
		if (key_press) {
			warp_pointer_to_item(pmp->menu,
			    MR_FIRST_ITEM(pmp->menu), True /* skip Title */);
		}
	}
	...
	if (pmp->flags.is_submenu) {
		...
		dkp.timestamp = (key_press && pmp->flags.has_default_action) ?
		    t0 :
		    0;
	}
	if (!pmp->flags.is_submenu && indirect_depth == 0) {
	...
	for (do_menu_interaction = True; do_menu_interaction == True;) {
		...
		if (pmp->pops->flags.do_tear_off_immediately == 1) {
			...
			if (!pmp->flags.is_submenu) {
				XSelectInput(dpy, Scr.NoFocusWin,
				    XEVMASK_MENUNFW);
				XFlush(dpy);
			}
		...
		if (!pmp->flags.is_submenu && x_start >= 0 && y_start >= 0 &&
			 ...
			 * end it */
			FWarpPointer(dpy, 0, Scr.Root, 0, 0,
		...
		if (!pmp->flags.is_submenu) {
			if (pmret->rc == MENU_DONE) {
				if (pmp->ret_paction && *pmp->ret_paction) {
					indirect_depth++;
					/* Execute the action */
					__menu_execute_function(pmp->pexc,
					    *pmp->ret_paction);
					indirect_depth--;
					free(*pmp->ret_paction);
...
Bool
menu_expose(XEvent *event, FvwmWindow *fw)
{
	MenuRoot *mr = NULL;
 ...
 */
void
update_transparent_menu_bg(MenuRepaintTransparentParameters *prtm,
    int current_x, int current_y, int step_x, int step_y, int end_x, int end_y)
{
	MenuRoot  *mr;
	MenuStyle *ms;
	Bool	   last = False;
	...
	if (MR_IS_TRANSLUCENT(mr)) {
		...
		if (current_x == step_x) {
			...
			if (current_y < step_y) {
				...
				tmp = PGraphicsCreateTranslucent(dpy,
				    MR_WINDOW(mr), &fra,
				    BACK_GC(ST_MENU_INACTIVE_GCS(ms)),
				    current_x, current_y + MR_HEIGHT(mr),
				    MR_WIDTH(mr), delta_y);
				XCopyArea(dpy, tmp, trans, my_gc, 0, 0,
				    MR_WIDTH(mr), delta_y, 0,
				    MR_HEIGHT(mr) - delta_y);
			} else {
				XCopyArea(dpy, MR_WINDOW(mr), trans, my_gc, 0,
				    0, MR_WIDTH(mr), MR_HEIGHT(mr) + delta_y, 0,
				    -delta_y);
				tmp = PGraphicsCreateTranslucent(dpy,
				    MR_WINDOW(mr), &fra,
				    BACK_GC(ST_MENU_INACTIVE_GCS(ms)),
				    current_x, step_y, MR_WIDTH(mr), -delta_y);
				XCopyArea(dpy, tmp, trans, my_gc, 0, 0,
			...
			trans = PGraphicsCreateTranslucent(dpy, MR_WINDOW(mr),
			    &fra, BACK_GC(ST_MENU_INACTIVE_GCS(ms)), step_x,
			    step_y, MR_WIDTH(mr), MR_HEIGHT(mr));
			XMapRaised(dpy, MR_WINDOW(mr));
		}
		...
		if (current_x == step_x) {
			/* Redraw the border */
			RelieveRectangle(dpy, MR_WINDOW(mr), 0, 0,
			    MR_WIDTH(mr) - 1, MR_HEIGHT(mr) - 1,
			    (Pdepth < 2) ?
		...
		SetWindowBackgroundWithOffset(dpy, MR_WINDOW(mr),
		    step_x - current_x, step_y - current_y, MR_WIDTH(mr),
		    MR_HEIGHT(mr), &Colorset[ST_CSET_MENU(ms)], Pdepth,
 ...
 */
void
repaint_transparent_menu(MenuRepaintTransparentParameters *prtm, Bool first,
    int x, int y, int end_x, int end_y, Bool is_bg_set)
{
	MenuItem  *mi;
	MenuRoot  *mr;
	MenuStyle *ms;
	...
	for (mi = MR_FIRST_ITEM(mr); mi != NULL; mi = MI_NEXT_ITEM(mi)) {
		if (mi == MR_SELECTED_ITEM(mr) && MST_DO_HILIGHT_BACK(mr)) {
			...
			if (left > 0) {
				XClearArea(dpy, MR_WINDOW(mr),
				    MR_ITEM_X_OFFSET(mr), MI_Y_OFFSET(mi), left,
				    MI_HEIGHT(mi) + MST_RELIEF_THICKNESS(mr),
				    0);
			}
			h = MI_HEIGHT(mi);
			continue;
	...
	XClearArea(dpy, MR_WINDOW(mr), MR_ITEM_X_OFFSET(mr),
	    MST_BORDER_WIDTH(mr), MR_ITEM_WIDTH(mr), s_h, 0);
	if (e_h != 0) {
		XClearArea(dpy, MR_WINDOW(mr), MR_ITEM_X_OFFSET(mr),
		    s_h + h + MST_RELIEF_THICKNESS(mr) + MST_BORDER_WIDTH(mr),
	...
	for (mi = MR_FIRST_ITEM(mr); mi != NULL; mi = MI_NEXT_ITEM(mi)) {
		...
		get_menu_paint_item_parameters(&mpip, mr, NULL, prtm->fw, NULL,
		    True);
		mpip.flags.is_first_item = (MR_FIRST_ITEM(mr) == mi);
		menuitem_paint(mi, &mpip);
	}
	 ...
	 * pic */
	if ((MR_SIDEPIC(mr) || MST_SIDEPIC(mr)) && !MR_HAS_SIDECOLOR(mr) &&
...
Bool
DestroyMenu(MenuRoot *mr, Bool do_recreate, Bool is_command_request)
{
	MenuItem *mi, *tmp2;
	MenuRoot *tmp, *prev;
	Bool	  in_list = True;
...
{
	*pmrPrior = NULL;
	while ((mr != NULL) && (MR_CONTINUATION_MENU(mr) != NULL)) {
		*pmrPrior = mr;
 ...
 */
void
AddToMenu(MenuRoot *mr, char *item, char *action, Bool fPixmapsOk, Bool fNoPlus,
    Bool is_continuation_item)
{
	...
	for (i = 0; i < MAX_MENU_ITEM_LABELS; i++, start = end) {
		...
		if (MI_LABEL(tmp)[i] != NULL) {
			if (fPixmapsOk) {
				...
				SCTX_SET_MI(ctx, tmp);
				scanForStrings(MI_LABEL(tmp)[i], item_pixmaps,
				    &ctx);
			}
		...
		MI_LABEL_STRLEN(tmp)
		[i] = (MI_LABEL(tmp)[i]) ? strlen(MI_LABEL(tmp)[i]) : 0;
	} /* for */
	...
	MI_IS_SELECTABLE(tmp) = ((MI_HAS_TEXT(tmp) || MI_HAS_PICTURE(tmp) ||
				     MI_IS_TEAR_OFF_BAR(tmp)) &&
	    !MI_IS_TITLE(tmp));
...
NewMenuRoot(char *name)
{
	...
	scanForStrings(MR_NAME(mr), root_strings, &ctx);
	MR_STYLE(mr)	     = menustyle_get_default_style();
	...
	for (t = Scr.FvwmRoot.next; t != NULL; t = t->next) {
		...
		if (IS_TEAR_OFF_MENU(t) &&
			...
			if (ST_CSET_MENU(ms2) == cset) {
				SetWindowBackground(dpy, MR_WINDOW(mr),
				    MR_WIDTH(mr), MR_HEIGHT(mr),
				    &Colorset[ST_CSET_MENU(ms2)], Pdepth,
				    FORE_GC(MST_MENU_INACTIVE_GCS(mr)), False);
				XClearArea(dpy, MR_WINDOW(mr), 0, 0,
...
void
change_mr_menu_style(MenuRoot *mr, char *stylename)
{
	MenuStyle *ms = NULL;
...
void
add_another_menu_item(char *action)
{
	MenuRoot *mr;
	MenuRoot *mrPrior;
	char	 *rest, *item;
 ...
 */
char *
get_menu_options(char *action, Window w, FvwmWindow *fw, XEvent *e,
    MenuRoot *mr, MenuItem *mi, MenuOptions *pops)
{
	...
	while (action != NULL && *action != 0 && once_more) {
		...
		if (StrEquals(tok, "context")) {
			...
			if (fw) {
				if (IS_ICONIFIED(fw)) {
					context_window = FW_W_ICON_TITLE(fw);
				} else {
			...
			flags = FScreenParseGeometryWithScreen(tok, &x, &y,
			...
			if (XFindContext(dpy, w, MenuContext,
				...
				if (mr) {
					/* the parent menu */
					pops->pos_hints.is_menu_relative = True;
					pops->pos_hints.menu_width = MR_WIDTH(
					    mr);
				}
			}
		} else {
		...
		if (fRectangleContext) {
			...
			!XGetGeometry(dpy, context_window, &JunkRoot, &JunkX,
			    ...
			    (unsigned int *)&JunkDepth) ||
			!XTranslateCoordinates(dpy, context_window, Scr.Root, 0,
			    0, &x, &y, &JunkChild))) {
		...
		taction = get_one_menu_position_argument(naction, x, width,
		    &(pops->pos_hints.x), &(pops->pos_hints.x_offset),
		...
		naction = get_one_menu_position_argument(taction, y, height,
		    &(pops->pos_hints.y), &dummy_int,
	...
	if (is_action_empty) {
		if (!pops->pos_hints.has_screen_origin) {
			pops->pos_hints.has_screen_origin = 1;
			if (!fev_get_evpos_or_query(dpy, Scr.Root, e,
				&pops->pos_hints.screen_origin_x,
				&pops->pos_hints.screen_origin_y)) {
				pops->pos_hints.screen_origin_x = 0;
				pops->pos_hints.screen_origin_y = 0;
