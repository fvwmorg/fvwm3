...
typedef struct
{
	char delimiter;
	Bool (*string_handler)(
	    char *string, char delimiter, string_context_t *user_data);
} string_def_t;
...
animated_move_back(MenuRoot *mr, Bool do_warp_pointer, FvwmWindow *fw)
{
	...
	if (menu_get_geometry(mr, &JunkRoot, &act_x, &act_y, &JunkWidth,
		...
		AnimatedMoveOfWindow(MR_WINDOW(mr), act_x, act_y,
		    act_x - MR_XANIMATION(mr), act_y, do_warp_pointer, -1, NULL,
...
move_any_menu(MenuRoot *mr, MenuParameters *pmp, int endX, int endY)
{
	if (MR_IS_TEAR_OFF_MENU(mr)) {
		...
		AnimatedMoveFvwmWindow(pmp->tear_off_root_menu_window,
		    FW_W_FRAME(pmp->tear_off_root_menu_window), -1, -1, endX,
		    endY, False, 0, &fFull);
	} else {
		int x;
		int y;
...
static void
scanForStrings(
    char *instring, const string_def_t *string_defs, string_context_t *context)
{
	char *s;
	int   type;
	char *string;
	...
	if (MR_FIRST_ITEM(mr) == NULL) {
		...
		if (MI_IS_TITLE(MR_FIRST_ITEM(mr))) {
			...
			if (MI_NEXT_ITEM(MR_FIRST_ITEM(mr)) != NULL) {
				MI_PREV_ITEM(MI_NEXT_ITEM(MR_FIRST_ITEM(mr))) =
				    mi;
			}
	...
	while (*format && !done) {
		...
		case '%':
			...
			if (sscanf(format, "%d.%d%n", &gap_left, &gap_right,
				...
				&& chars > 0)
			    || sscanf(format, "%d%n", &gap_left, &chars) >= 1
			    || sscanf(format, ".%d%n", &gap_right, &chars)
				   >= 1) {
			...
			case 's':
				...
				sidepic_space =
				    msp->max.sidepic_width
				    + ((sidepic_is_left) ? gap_left
							 : gap_right);
			...
			case 'i':
				...
				if (msp->max.i.icon_width[icons_placed] > 0) {
					...
					[icons_placed] = x;
					x += msp->max.i.icon_width[icons_placed]
					     + gap_right;
					item_order[used_objects++] =
					    &(MR_ICON_X_OFFSET(
	...
	for (mi = MR_FIRST_ITEM(msp->menu); mi != NULL; mi = MI_NEXT_ITEM(mi)) {
		for (i = 0; i < MAX_MENU_ITEM_LABELS; i++) {
			...
			if (!MI_IS_TITLE(mi) || !MI_IS_TITLE_CENTERED(mi)) {
				...
				MI_LABEL_OFFSET(mi)
				[i] =
				    menudim_middle_x_offset(&MR_DIM(msp->menu))
...
calc_more_item_height(MenuSizingParameters *msp)
{
	...
	height = MST_PSTDFONT(msp->menu)->height + MST_ITEM_GAP_ABOVE(msp->menu)
...
size_menu_vertically(MenuSizingParameters *msp)
{
	...
	for (cItems = 0, mi = MR_FIRST_ITEM(msp->menu); mi != NULL;
		...
		Bool last_item_has_relief =
		    (MI_PREV_ITEM(mi)) ? MI_IS_SELECTABLE(MI_PREV_ITEM(mi))
				       : False;
		...
		if (MI_IS_TITLE(mi)) {
			...
			/* Tear off bar */
			MI_HEIGHT(mi) =
			    relief_thickness + MENU_TEAR_OFF_BAR_HEIGHT;
		} else {
		...
		if (MI_PICTURE(mi)) {
			if ((MI_HAS_TEXT(mi) && msp->used_item_labels)
				...
				MI_HEIGHT(mi) =
				    MI_PICTURE(mi)->height + relief_thickness;
			}
		}
		y += MI_HEIGHT(mi);
...
make_menu_window(MenuRoot *mr, Bool is_tear_off)
{
	...
	if (MR_WINDOW(mr) != None) {
		...
		MR_WINDOW(mr) =
		    XCreateWindow(MR_CREATE_DPY(mr), Scr.Root, 0, 0, w, h, 0,
...
static void
clear_expose_menu_area(Window win, XEvent *e)
{
	if (e == NULL) {
		XClearWindow(dpy, win);
	} else {
		XClearArea(dpy, win, e->xexpose.x, e->xexpose.y,
		    e->xexpose.width, e->xexpose.height, False);
	}
...
static Bool
paint_menu_gradient_background(MenuRoot *mr, XEvent *pevent)
{
	...
	case H_GRADIENT:
		if (MR_IS_BACKGROUND_SET(mr) == False) {
			...
			dw = (float)(bounds.width / ST_FACE(ms).u.grad.npixels)
			     + 1;
	...
	case V_GRADIENT:
		if (MR_IS_BACKGROUND_SET(mr) == False) {
			...
			if (best_tile_width == 0) {
				...
				if (!XQueryBestTile(dpy, Scr.screen, tw, tw,
			...
			pmap   = XCreatePixmap(dpy, MR_WINDOW(mr),
			      best_tile_width, MR_HEIGHT(mr), Pdepth);
			pmapgc = fvwmlib_XCreateGC(dpy, pmap, gcm, &gcv);
			dh = (float)(bounds.height / ST_FACE(ms).u.grad.npixels)
			     + 1;
			for (i = 0; i < ST_FACE(ms).u.grad.npixels; i++) {
				int y;
				...
				XSetForeground(dpy, pmapgc,
				    ST_FACE(ms).u.grad.xcs[i].pixel);
				XFillRectangle(dpy, pmap, pmapgc, 0, y,
				    best_tile_width, dh);
			}
			XSetWindowBackgroundPixmap(dpy, MR_WINDOW(mr), pmap);
			XFreeGC(dpy, pmapgc);
	...
	case B_GRADIENT: {
		...
		for (i = 0; i < numLines; i++) {
			if ((int)(i * ST_FACE(ms).u.grad.npixels / numLines)
				...
				cindex =
				    i * ST_FACE(ms).u.grad.npixels / numLines;
	...
	default:
		if (MR_IS_BACKGROUND_SET(mr) == False) {
			...
			CalculateGradientDimensions(dpy, MR_WINDOW(mr),
			    ST_FACE(ms).u.grad.npixels,
			    ST_FACE(ms).gradient_type,
			    ST_FACE(ms).u.grad.do_dither, &g_width, &g_height);
			/* draw the gradient directly into the window */
			CreateGradientPixmap(dpy, MR_WINDOW(mr), pmapgc,
			    ST_FACE(ms).gradient_type, g_width, g_height,
			    ST_FACE(ms).u.grad.npixels, ST_FACE(ms).u.grad.xcs,
			    ST_FACE(ms).u.grad.do_dither,
			    &(MR_STORED_PIXELS(mr).d_pixels),
			    &(MR_STORED_PIXELS(mr).d_npixels), pmap, bw, bw,
...
static Bool
paint_menu_pixmap_background(MenuRoot *mr, XEvent *pevent)
{
	MenuStyle *  ms = MR_STYLE(mr);
	...
	XCopyArea(dpy, p->picture, MR_WINDOW(mr), Scr.TransMaskGC, bw, bw,
	    width, height, x, y);
		...
		case GradientMenu:
			if (select == True) {
				...
				if (iy + ih > MR_HEIGHT(mr))
					ih = MR_HEIGHT(mr) - iy;
				/* grab image */
				MR_STORED_ITEM(mr).stored = XCreatePixmap(
				    dpy, Scr.NoFocusWin, mw, ih, Pdepth);
				...
				XSetGraphicsExposures(dpy,
				   ...
				   && MR_STORED_ITEM(mr).width != 0) {
				/* ungrab image */
				XCopyArea(dpy, MR_STORED_ITEM(mr).stored,
				    MR_WINDOW(mr),
				    FORE_GC(MST_MENU_INACTIVE_GCS(mr)), 0, 0,
				...
				e.xexpose.width =
				    MR_WIDTH(mr) - 2 * MST_BORDER_WIDTH(mr);
				e.xexpose.height =
				    MI_HEIGHT(mi)
				    + (MI_IS_SELECTABLE(mi)
					    ? MST_RELIEF_THICKNESS(mr)
...
get_left_popup_x_position(MenuRoot *mr, MenuRoot *submenu, int x)
{
	if (MST_USE_LEFT_SUBMENUS(mr)) {
		return (x - MST_POPUP_OFFSET_ADD(mr) - MR_WIDTH(submenu)
			+ MR_WIDTH(mr) * (100 - MST_POPUP_OFFSET_PERCENT(mr))
			      / 100);
	} else {
    ...
    Bool *pprefer_left_submenus)
{
	...
	if (!menu_get_geometry(mr, &JunkRoot, &menu_x, &menu_y, &JunkWidth,
		...
		*pprefer_left_submenus = False;
		fvwm_debug(
		    __func__, "can't get geometry of menu %s", MR_NAME(mr));
		return;
	}
    ...
    Bool *ret_overlap, Window popdown_window)
{
	...
	if (parent_menu == NULL
		...
		if (x_clipped_overlap
			...
			left_x =
			    get_left_popup_x_position(parent_menu, mr, prev_x);
			right_x =
			    get_right_popup_x_position(parent_menu, mr, prev_x);
			...
			if (MST_IS_ANIMATED(mr)) {
				...
				AnimatedMoveOfWindow(w, prev_x, prev_y, end_x,
				    prev_y, True, -1, NULL,
				    (transparent_bg) ? &mrtp : NULL);
			} /* if (MST_IS_ANIMATED(mr)) */
			 ...
			 */
				...
				may_use_right =
				    (right_x + MR_WIDTH(mr) <= scr_x + scr_w);
    ...
    MenuRoot **pmr, Bool *fSubmenuOverlaps, MenuParameters *pmp)
{
	...
	if (*fSubmenuOverlaps && parent) {
		...
		if (!menu_get_geometry(*pmr, &JunkRoot, &mr_x, &mr_y, &mr_width,
		    ...
		    || !menu_get_geometry(parent, &JunkRoot, &parent_x,
			&parent_y, &parent_width, &parent_height, &JunkBW,
			&JunkDepth)) {
    ...
    mloop_evh_data_t *med, mloop_static_info_t *msi)
{
	...
	while (!FPending(dpy) || !FCheckMaskEvent(dpy, msi->event_mask, &e)) {
		...
		if ((in->mrPopdown || in->mrPopup)
			...
			|| MST_DO_POPDOWN_IMMEDIATELY(pmp->menu))) {
			MenuRoot *m =
			    (in->mrPopdown) ? in->mrPopdown : in->mrPopup;
    ...
    mloop_evh_data_t *med, mloop_static_info_t *msi)
{
	...
	if (e.type == MotionNotify) {
		...
		while (FCheckMaskEvent(
			   dpy, ButtonMotionMask | ButtonReleaseMask, &e)
	...
	case ButtonRelease:
		...
		if (med->mrMi != NULL) {
			...
			if (pmret->rc == MENU_NEWITEM_MOVEMENU) {
				...
				med->mi =
				    find_entry(pmp, NULL, NULL, None, -1, -1);
				pmret->rc = MENU_NEWITEM;
			}
		} else {
    ...
    Bool *pdoes_submenu_overlap, Bool *pdoes_popdown_submenu_overlap)
{
	...
	if (med->mrMi == pmp->menu && mrMiPopup == NULL && MI_IS_POPUP(med->mi)
	...
	if (in->mrPopup) {
		if (MR_PARENT_MENU(in->mrPopup) == pmp->menu) {
			med->mi =
			    find_entry(pmp, NULL, &med->mrMi, None, -1, -1);
    ...
    Bool *pdoes_popdown_submenu_overlap)
{
	...
	if (!MR_SELECTED_ITEM(pmp->menu) || in->mif.is_off_menu_allowed == True
    ...
    mloop_evh_data_t *med, MenuOptions *pops)
{
	...
	if (pmp->ret_paction) {
		if (pmret->rc == MENU_EXEC_CMD) {
			...
			*pmp->ret_paction =
			    (med->mi) ? fxstrdup(MI_ACTION(med->mi)) : NULL;
		}
	}
    ...
    MenuOptions *pops)
{
	...
	if (pmret->rc == MENU_SELECTED
	    && is_double_click(
		msi->t0, med->mi, pmp, pmret, pdkp, in->mif.has_mouse_moved)) {
		pmret->rc = MENU_DOUBLE_CLICKED;
	}
...
__menu_loop(MenuParameters *pmp, MenuReturn *pmret, double_keypress *pdkp)
{
	...
	for (is_finished = False; !is_finished;) {
		...
		mloop_ret =
		    __mloop_handle_event(pmp, pmret, pdkp, &mei, &med, &msi);
		switch (mloop_ret) {
		case MENU_MLOOP_RET_END:
			is_finished = True;
			continue;
		...
		if (med.mi != NULL) {
			mloop_ret =
			    __mloop_handle_action_with_mi(pmp, pmret, pdkp,
				&mei, &med, &msi, &mops, &does_submenu_overlap,
				&does_popdown_submenu_overlap);
		} else {
			mloop_ret =
			    __mloop_handle_action_without_mi(pmp, pmret, pdkp,
				&mei, &med, &msi, &mops, &does_submenu_overlap,
				&does_popdown_submenu_overlap);
		}
...
menu_tear_off(MenuRoot *mr_to_copy)
{
	...
	if (!ms) {
		...
		fvwm_debug(
		    __func__, "impossible to create %s menu style", buffer);
		free(buffer);
...
menu_redraw_transparent_tear_off_menu(FvwmWindow *fw, Bool pr_only)
{
	...
	if (!(IS_TEAR_OFF_MENU(fw)
		&& XFindContext(dpy, FW_W(fw), MenuContext, (caddr_t *)&mr)
		       != XCNOENT
		&& (ms = MR_STYLE(mr)) && ST_HAS_MENU_CSET(ms))) {
		return False;
	}
	...
	for (do_menu_interaction = True; do_menu_interaction == True;) {
		...
		if (!pmp->flags.is_submenu && x_start >= 0 && y_start >= 0
			...
			FWarpPointer(dpy, 0, Scr.Root, 0, 0,
			    ...
			    m->virtual_scr.MyDisplayHeight, x_start, y_start);
			if ((*pmp->pexc)->x.elast->type == KeyPress) {
				XEvent e = *(*pmp->pexc)->x.elast;
...
menu_expose(XEvent *event, FvwmWindow *fw)
{
	...
	if ((XFindContext(dpy, event->xany.window, MenuContext, (caddr_t *)&mr)
		!= XCNOENT)) {
    ...
    int current_x, int current_y, int step_x, int step_y, int end_x, int end_y)
{
	...
	SetWindowBackgroundWithOffset(dpy, MR_WINDOW(mr), step_x - current_x,
...
NewMenuRoot(char *name)
{
	...
	string_def_t root_strings[] = {
	    {'@', __scan_for_pixmap}, {'^', __scan_for_color}, {'\0', NULL}};
	string_context_t ctx;
    ...
    MenuRoot *mr, MenuItem *mi, MenuOptions *pops)
{
	...
	while (action != NULL && *action != 0 && once_more) {
		...
		if (StrEquals(tok, "context")) {
			...
			if (tok == NULL) {
				fvwm_debug(
				    __func__, "missing rectangle geometry");
			...
			if ((flags & (XValue | YValue)) != (XValue | YValue)) {
				free(tok);
				fvwm_debug(
				    __func__, "invalid rectangle geometry");
				if (!pops->pos_hints.has_screen_origin) {
					/* xinerama: emergency fallback */
					pops->pos_hints.has_screen_origin = 1;
					pops->pos_hints.screen_origin_x	  = 0;
			...
			arg.name			  = screen;
			FScreenGetScrRect(
			    &arg, FSCREEN_BY_NAME, &sx, &sy, &sw, &sh);
			...
			context_window = w;
			if (XFindContext(
				dpy, w, MenuContext, (caddr_t *)&dummy_mr)
			    != XCNOENT) {
				if (mr) {
					/* the parent menu */
					pops->pos_hints.is_menu_relative = True;
		...
		if (fRectangleContext) {
			...
			/* nothing else to do */
			   ...
			   && (!fHasContext || !context_window
			       || !XGetGeometry(dpy, context_window, &JunkRoot,
				   &JunkX, &JunkY, (unsigned int *)&width,
				   (unsigned int *)&height,
				   (unsigned int *)&JunkBW,
				   (unsigned int *)&JunkDepth)
			       || !XTranslateCoordinates(dpy, context_window,
				   Scr.Root, 0, 0, &x, &y, &JunkChild))) {
		...
		if (naction == taction) {
			...
			if (fHasContext) {
				fvwm_debug(
				    __func__, "invalid position arguments");
			}
			naction = action;
			taction = action;
			break;
