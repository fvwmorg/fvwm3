...
static void toggle_prev_monitor_state(struct monitor *, struct monitor *,
    FvwmWindow *);
static void tips_handle_enter(FvwmWindow *fw, Window w);
static void tips_handle_leave(Window w);
static void tips_handle_window_destroyed(FvwmWindow *fw);
...
static ftips_config *tips_config = NULL;
static Window tips_active_win = None;
...
{
	tips_handle_window_destroyed(ea->exc->w.fw);
...
static void tips_handle_enter(FvwmWindow *fw, Window w)
{
	...
	context = frame_window_id_to_context(fw, w, &num);
	text = get_decor_tooltip(fw, context, num, (fw == Scr.Hilite) ? 1 : 0);
	if (text == NULL || *text == '\0')
	{
		if (tips_active_win != None)
		{
			FTipsCancel(dpy);
			tips_active_win = None;
		}
		return;
	}
	if (context == C_TITLE)
	{
		...
		if (p != NULL)
		{
			xasprintf(&new_text,
				...
				(int)(p - text),
				text,
				(fw->visible_name != NULL) ? fw->visible_name : "",
				p + 2);
			text = new_text;
			free_text = true;
		}
	...
	XTranslateCoordinates(
		dpy, w, DefaultRootWindow(dpy), 0, 0, &rx, &ry, &dummy);

	if (tips_config->Ffont == NULL)
	{
		tips_config->Ffont = Scr.DefaultFont != NULL ?
			Scr.DefaultFont : FlocaleLoadFont(dpy, NULL, "fvwm");
	}

	tips_active_win = w;
	FTipsOn(
		dpy, DefaultRootWindow(dpy), tips_config, (void *)(long)w, text,
		rx, ry, wa.width, wa.height);
	if (free_text)
	{
		free(text);
	}
...
static void tips_handle_window_destroyed(FvwmWindow *fw)
{
	...
	if (tips_active_win == None || fw == NULL)
	{
		return;
	}
	if (frame_window_id_to_context(fw, tips_active_win, &num) == C_ROOT)
	{
		return;
	}
	FTipsCancel(dpy);
...
void HandleEnterNotify(const evh_args_t *ea)
{
	...
	/* Show a decoration tool tip. */
	tips_handle_enter(fw, ewp->window);
	if (fw)
	{
...
void dispatch_event(XEvent *e)
{
	...
	if (FTipsHandleEvents(dpy, e))
	{
		/* The event was handled by the tool tip window. */
		return;
	}
...
int My_XNextEvent(Display *disp, XEvent *event)
{
	...
	do
	{
		...
		/* Adjust time to wait for tooltip if needed. */
		int tips_ms = (int)FTipsCheck(dpy);
		if (tips_ms > 0 && (ms < 0 || tips_ms < ms))
		{
			ms = tips_ms;
		}
	...
	else
	{
		...
		squeue_execute();
		/* Map a decoration tool tip whose delay just elapsed. */
		FTipsCheck(dpy);
	}
