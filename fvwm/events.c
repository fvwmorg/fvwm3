...
static void
fake_map_unmap_notify(const FvwmWindow *fw, int event_type)
{
	XEvent		  client_event;
...
static int
_pred_weed_accumulate_expose(Display *display, XEvent *ev, XPointer arg)
{
	...
	}
	{
		...
		x1		  = max(ev->xexpose.x + ev->xexpose.width,
	...
	}
	{
		...
		y1		   = max(ev->xexpose.y + ev->xexpose.height,
...
static int
_pred_weed_is_expose(Display *display, XEvent *event, XPointer arg)
{
	return (event->type == Expose);
}

static int
_pred_weed_handle_expose(Display *display, XEvent *event, XPointer arg)
{
...
static int
_pred_weed_event_type(Display *display, XEvent *event, XPointer arg)
{
...
static int
_pred_flush_property_notify_weed(Display *display, XEvent *event, XPointer arg)
{
	flush_property_notify_args *args = (flush_property_notify_args *)arg;
	int			    does_match_window;

	does_match_window = (FEV_HAS_EVENT_WINDOW(event->type) &&
				event->xany.window == args->w) ?
	    1 :
	    0;
	if (does_match_window && event->type == args->event_type &&
	    event->xproperty.atom == args->atom) {
		/* invalidate event and continue weeding */
		return 1;
	} else if (args->do_stop_at_event_type &&
...
static Bool
test_resizing_event(Display *display, XEvent *event, XPointer arg)
{
...
static inline void
_handle_cr_on_unmanaged(XEvent *e)
{
...
static inline void
_handle_cr_on_icon(XEvent *e, FvwmWindow *fw)
{
	XConfigureRequestEvent *cre = &e->xconfigurerequest;
	XWindowChanges		xwc;
	unsigned long		xwcm;
	...
	if (FW_W_ICON_PIXMAP(fw) == cre->window) {
		...
		if ((cre->value_mask & (CWWidth | CWHeight)) ==
		    (CWWidth | CWHeight)) {
			set_icon_picture_size(fw, cre->width + 2 * bw,
			    cre->height + 2 * bw);
		}
	}
...
static inline void
_handle_cr_on_shaped(FvwmWindow *fw)
{
...
static inline void
_handle_cr_restack(int *ret_do_send_event, XEvent *e, FvwmWindow *fw)
{
	XConfigureRequestEvent *cre = &e->xconfigurerequest;
	XWindowChanges		xwc;
	unsigned long		xwcm;
	...
	/* only allow clients to restack windows within their layer */
		...
		if (cre->detail == Below) {
			...
			if (FW_W_ICON_TITLE(fw2) != None) {
				XConfigureWindow(dpy, FW_W_ICON_TITLE(fw2),
				    xwcm, &xwc);
			}
			if (FW_W_ICON_PIXMAP(fw2) != None) {
				XConfigureWindow(dpy, FW_W_ICON_PIXMAP(fw2),
				    xwcm, &xwc);
			}
		}
		...
		if (cre->detail == Above) {
			...
			add_window_to_stack_ring_after(fw,
...
static inline void
_cr_get_static_position(rectangle *ret_g, FvwmWindow *fw, XEvent *e,
    size_borders *b)
{
	XConfigureRequestEvent *cre = &e->xconfigurerequest;
...
static inline void
_cr_get_grav_position(rectangle *ret_g, FvwmWindow *fw, XEvent *e,
    size_borders *b)
{
	XConfigureRequestEvent *cre = &e->xconfigurerequest;
	int			grav_x;
...
static inline void
_cr_detect_icccm_move(FvwmWindow *fw, XEvent *e, size_borders *b)
{
	XConfigureRequestEvent *cre = &e->xconfigurerequest;
	rectangle		grav_g;
	rectangle		static_g;
	...
	if (CR_MOTION_METHOD(fw) != CR_MOTION_METHOD_AUTO) {
		if (Scr.bo.do_debug_cr_motion_method == 1) {
			fvwm_debug(__func__,
			    ...
			    " '%s'\n",
			    HAS_EWMH_WM_PID(fw), fw, fw->visible_name);
		}
		return;
	}
	...
	if (Scr.bo.do_debug_cr_motion_method == 1) {
		fvwm_debug(__func__,
		    "s %3d/%3d %2d/%2d, g %3d/%3d %2d/%2d: ", static_g.x,
		    static_g.y, ds_g.x, ds_g.y, grav_g.x, grav_g.y, dg_g.x,
		    dg_g.y);
	}
	/* check full screen */
	if ((cre->value_mask & (CWX | CWY)) == (CWX | CWY) &&
	    (has_x || has_y) && cre->width == monitor_get_all_widths() &&
	...
	if (has_x && dg_g.x == 0 && ds_g.x != 0 && has_y && dg_g.y == 0 &&
	    ds_g.y != 0) {
	...
	w = (cre->value_mask & CWWidth) ? cre->width + b->total_size.width :
					  fw->g.frame.width;
	h = (cre->value_mask & CWHeight) ? cre->height + b->total_size.height :
					   fw->g.frame.height;
	if (!has_x) {
		mx = CR_MOTION_METHOD_AUTO;
	} else if (static_g.x == 0 ||
	    static_g.x + w == monitor_get_all_widths()) {
	...
	} else if (static_g.y == 0 ||
	    static_g.y + h == monitor_get_all_heights()) {
 ...
 * again. */
static int
_pred_merge_cr(Display *display, XEvent *event, XPointer arg)
{
	...
	case PropertyNotify:
		if (event->xproperty.window == args->w &&
...
static inline int
_merge_cr_moveresize(const evh_args_t *ea, XEvent *ev, FvwmWindow *fw,
    size_borders *b)
{
	_merge_cr_args args;
...
static inline int
_handle_cr_on_client(int *ret_do_send_event, XEvent *e, const evh_args_t *ea,
    FvwmWindow *fw, Bool force, int force_gravity)
{
	...
	if (DEBUG_CRE) {
		fvwm_debug(__func__,
		    ...
		    "ew 0x%08x  '%s'\n",
		    cre->x, (int)(cre->value_mask & CWX), cre->y,
		    (int)(cre->value_mask & CWY), cre->width,
		    (int)(cre->value_mask & CWWidth), cre->height,
		    (int)(cre->value_mask & CWHeight), (int)FW_W_FRAME(fw),
		    (int)FW_W(fw), (int)cre->window,
	...
	if (IS_MAXIMIZED(fw)) {
		...
		*ret_do_send_event = 1;
		d_g.width	   = 0;
		d_g.height	   = 0;
	} else if (!is_function_allowed(F_RESIZE, NULL, fw, RQORIG_PROGRAM,
		       False)) {
		cre->value_mask &= ~(CWWidth | CWHeight);
		*ret_do_send_event = 1;
	}
	...
	if (IS_SHADED(fw)) {
		...
		gravity_dir = gravity_override_dir(gravity_dir, SHADED_DIR(fw));
	...
	if (cre->value_mask & CWWidth) {
		if (cre->width <
		    (WINDOW_FREAKED_OUT_SIZE - b.total_size.width)) {
			d_g.width = cre->width -
	...
	constrain_size(fw, NULL, &constr_dim.width, &constr_dim.height, 0, 0,
	...
	if (new_g.x == current_g.x && new_g.y == current_g.y &&
		...
		*ret_do_send_event = 1;
		...
		frame_setup_window_app_request(fw, new_g.x, new_g.y,
		    new_g.width, new_g.height, False);
...
void
_handle_configure_request(XEvent *e, const evh_args_t *ea, FvwmWindow *fw,
    Bool force, int force_gravity)
{
	...
	if (fw != NULL && cre->window == FW_W(fw)) {
		cn_count = _handle_cr_on_client(&do_send_event, e, ea, fw,
		    force, force_gravity);
	}
...
static Bool
_pred_button_click(Display *display, XEvent *event, XPointer arg)
{
...
static Bool
__test_for_motion(int x0, int y0)
{
	int	     x;
	int	     y;
	unsigned int mask;
	...
	for (x = x0, y = y0; FQueryPointer(dpy, Scr.Root, &JunkRoot, &JunkChild,
...
/* Helper function for __handle_focus_raise_click(). */
static void
__check_click_to_focus_or_raise(hfrc_ret_t *ret_args, const exec_context_t *exc)
{
	...
	if (f.is_client_click && !ret_args->do_focus && !f.is_focused &&
	...
	ret_args->do_raise = focus_query_click_to_raise(fw, f.is_focused,
	    exc->w.wcontext);
...
__handle_focus_raise_click(hfrc_ret_t *ret_args, const exec_context_t *exc)
{
	...
	if (FP_USE_MOUSE_BUTTONS(FW_FOCUS_POLICY(exc->w.fw)) != 0 &&
		...
		FPOL_ANY_MODIFIER &&
	    MaskUsedModifiers(FP_USE_MODIFIERS(FW_FOCUS_POLICY(exc->w.fw))) !=
...
static Bool
__is_bpress_window_handled(const exec_context_t *exc)
{
	Window	      eventw;
	const XEvent *te = exc->x.etrigger;
...
static Bool
__handle_click_to_focus(const exec_context_t *exc)
{
	...
	if (focus_is_focused(exc->w.fw) && !IS_ICONIFIED(exc->w.fw)) {
		border_draw_decorations(exc->w.fw, PART_ALL, True, True,
		    CLEAR_ALL, NULL, NULL);
	}
...
/* Helper function for __handle_bpress_on_managed */
static Bool
__handle_click_to_raise(const exec_context_t *exc)
{
	Bool rc = False;
	int  is_focused;
...
/* Helper function for __handle_bpress_on_managed */
static Bool
__handle_bpress_action(const exec_context_t *exc, char *action)
{
	window_parts part;
	Bool	     do_force;
	Bool	     rc = False;
	...
	border_draw_decorations(exc->w.fw, part, (Scr.Hilite == exc->w.fw),
	...
	if (check_if_fvwm_window_exists(exc->w.fw)) {
		part	 = border_context_to_parts(exc->w.wcontext);
		do_force = (part & PART_TITLEBAR) ? True : False;
		border_draw_decorations(exc->w.fw, part,
		    (Scr.Hilite == exc->w.fw), do_force, CLEAR_ALL, NULL, NULL);
	}
...
static void
__handle_bpress_on_root(const exec_context_t *exc)
{
	char *action;

	PressedW = None;
	/* search for an appropriate mouse binding */
	action = CheckBinding(Scr.AllBindings, exc->x.etrigger->xbutton.button,
...
static void
__handle_bpress_on_unmanaged(const exec_context_t *exc)
{
...
static void
__handle_bpress_on_managed(const exec_context_t *exc)
{
	char		 *action;
	hfrc_ret_t	  f;
	FvwmWindow *const fw = exc->w.fw;
	...
	if (!f.do_forbid_function) {
		/* mouse bindings */
		action = CheckBinding(Scr.AllBindings, e->xbutton.button,
		    e->xbutton.state, GetUnusedModifiers(), exc->w.wcontext,
		    BIND_BUTTONPRESS, &fw->class, fw->name.name);
...
static void
__refocus_stolen_focus_win(const evh_args_t *ea)
{
...
void
HandleButtonPress(const evh_args_t *ea)
{
...
HandleClientMessage(const evh_args_t *ea)
{
	...
	if (te->xclient.message_type == _XA_WM_CHANGE_STATE && fw &&
	    te->xclient.data.l[0] == IconicState && !IS_ICONIFIED(fw)) {
...
void
HandleConfigureRequest(const evh_args_t *ea)
{
...
void
HandleDestroyNotify(const evh_args_t *ea)
{
	destroy_window(ea->exc->w.fw);
	EWMH_ManageKdeSysTray(ea->exc->x.etrigger->xdestroywindow.window,
...
#endif
void
HandleEnterNotify(const evh_args_t *ea)
{
	...
	ENTER_DBG((stderr,
	    ...
	    " detail 0x%x '%s'\n",
	    ++ecount, fw, (int)ewp->window, (int)ewp->subwindow, ewp->mode,
	    ewp->detail, fw ? fw->visible_name : "(none)"));
	if (ewp->window == Scr.Root && ewp->detail == NotifyInferior &&
	    ewp->mode == NotifyNormal) {
		/* pointer left subwindow */
		BroadcastPacket(MX_ENTER_WINDOW, 3, (long)Scr.Root, (long)NULL,
		    (long)NULL);
	} else if (ewp->window == Scr.Root &&
	...
	if (fw) {
		if (ewp->window != FW_W_FRAME(fw) &&
		    ewp->window != FW_W_PARENT(fw) && ewp->window != FW_W(fw) &&
	 ...
	 */
	is_tear_off_menu = (fw && IS_TEAR_OFF_MENU(fw) &&
	    ewp->window == FW_W(fw));
	...
	if (is_pan_frame(ewp->window)) {
		...
		if (edge_command && ewp->mode == NotifyUngrab &&
			 ...
			 * HandlePaging */
			position edge_scroll = { m->virtual_scr.EdgeScrollX,
			...
			Scr.flags.is_pointer_on_this_screen = 1;
			e				    = *te;
			int p = HandlePaging(&e, edge_scroll, &junk, &delta,
...
void
HandleExpose(const evh_args_t *ea)
{
	XEvent		  e;
	FvwmWindow *const fw = ea->exc->w.fw;
...
void
HandleFocusIn(const evh_args_t *ea)
{
	XEvent	      d;
	Window	      w	       = None;
	Window	      focus_w  = None;
	...
	Scr.UnknownWinFocused = None;
	if (fw && Scr.focus_in_requested_window != fw &&
	    !FP_DO_FOCUS_BY_PROGRAM(FW_FOCUS_POLICY(fw)) &&
	    fw->focus_model == FM_GLOBALLY_ACTIVE) {
		if (DEBUG_GLOBALLY_ACTIVE) {
			fprintf(stderr,
			    "prevented globally active fw %p (%s)"
			    " from stealing the focus\n",
			    fw, fw->name.name);
			fprintf(stderr, "window was %p (%s)\n",
	...
	if (!fw) {
		if (w != Scr.NoFocusWin) {
			...
			Scr.StolenFocusWin = (ffw_old != NULL) ? FW_W(ffw_old) :
								 None;
		...
		if (!Scr.bo.do_enable_flickering_qt_dialogs_workaround ||
			...
			border_draw_decorations(Scr.Hilite, PART_ALL, False,
			    True, CLEAR_ALL, NULL, NULL);
			if (Scr.ColormapFocus == COLORMAP_FOLLOWS_FOCUS) {
				if ((Scr.Hilite) &&
				    (!IS_ICONIFIED(Scr.Hilite))) {
	    ...
	    last_focus_fw == None || IS_FOCUS_CHANGE_BROADCAST_PENDING(fw) ||
	    fpol_query_allow_user_focus(&FW_FOCUS_POLICY(fw))) {
		if (w != Scr.NoFocusWin) {
			Scr.StolenFocusWin = (ffw_old != NULL) ? FW_W(ffw_old) :
								 None;
		...
		if (fw != Scr.Hilite &&
		    fpol_query_allow_user_focus(&FW_FOCUS_POLICY(fw))) {
			border_draw_decorations(fw, PART_ALL, True, True,
			    CLEAR_ALL, NULL, NULL);
		}
		focus_w	 = FW_W(fw);
	...
	if (was_nothing_ever_focused || last_focus_fw == None ||
		...
		if (!Scr.bo.do_enable_flickering_qt_dialogs_workaround ||
		    !is_unmanaged_focused) {
			BroadcastPacket(M_FOCUS_CHANGE, 5, (long)focus_w,
			    (long)focus_fw,
			    (unsigned long)IsLastFocusSetByMouse(), (long)fc,
			    (long)bc);
...
void
HandleFocusOut(const evh_args_t *ea)
{
...
void
__handle_key(const evh_args_t *ea, Bool is_press)
{
	char		       *action;
	FvwmWindow	       *sf;
	KeyCode			kc;
	...
	kc = XKeysymToKeycode(dpy,
	    fvwm_KeycodeToKeysym(dpy, te->xkey.keycode, 0, 0));
	...
	action = CheckTwoBindings(&is_second_binding, Scr.AllBindings, kc,
	...
	if (action != NULL) {
		...
		if (is_second_binding == False) {
			...
			exc	       = exc_clone_context(ea->exc, &ecc,
				       ECC_FW | ECC_WCONTEXT);
		}
		execute_function(NULL, exc, action, NULL, 0);
		if (is_second_binding == False) {
	...
	if (sf && te->xkey.window != FW_W(sf)) {
		...
		FSendEvent(dpy, e.xkey.window, False,
...
void
HandleKeyPress(const evh_args_t *ea)
{
	__handle_key(ea, True);
}

void
HandleKeyRelease(const evh_args_t *ea)
{
	__handle_key(ea, False);
}

void
HandleLeaveNotify(const evh_args_t *ea)
{
	...
	ENTER_DBG((stderr,
	    "-------- ln (%d): fw %p w 0x%08x sw 0x%08x"
	    " mode 0x%x detail 0x%x '%s'\n",
	    ++ecount, fw, (int)te->xcrossing.window,
	    (int)te->xcrossing.subwindow, te->xcrossing.mode,
	    te->xcrossing.detail, fw ? fw->visible_name : "(none)"));
	lwp = &te->xcrossing;
	if (lwp->window == Scr.Root && lwp->detail == NotifyInferior &&
	    lwp->mode == NotifyNormal) {
		/* pointer entered subwindow */
		BroadcastPacket(MX_LEAVE_WINDOW, 3, (long)Scr.Root, (long)NULL,
		    (long)NULL);
	} else if (lwp->window == Scr.Root &&
	...
	if (lwp->mode != NotifyNormal) {
		...
		if (lwp->mode == NotifyGrab && fw &&
		    (lwp->window == FW_W_FRAME(fw) || lwp->window == FW_W(fw) ||
...
void
HandleMapNotify(const evh_args_t *ea)
{
	...
	if (IS_ICONIFIED(fw)) {
		...
		BroadcastPacket(M_MAP, 3, (long)FW_W(fw), (long)FW_W_FRAME(fw),
		    (unsigned long)fw);
	}
	...
	border_draw_decorations(fw, PART_ALL,
	    (fw == get_focus_window()) ? True : False, True, CLEAR_ALL, NULL,
	    NULL);
	MyXUngrabServer(dpy);
	SET_MAPPED(fw, 1);
	SET_ICONIFIED(fw, 0);
...
void
HandleMapRequest(const evh_args_t *ea)
{
	if (fFvwmInStartup) {
...
void
HandleMapRequestKeepRaised(const evh_args_t *ea, Window KeepRaised,
    FvwmWindow *ReuseWin, initial_window_options_t *win_opts)
{
	 ...
	 */
	is_on_this_page = IsRectangleOnThisPage(fw->m, &(fw->g.frame),
	    fw->Desk);
	if (KeepRaised != None) {
	...
	if (IS_ICONIFIED(fw))
		...
		default:
			...
			if ((initial_map_command != NULL) &&
				...
				execute_function_override_window(NULL, ea->exc,
				    (char *)initial_map_command, NULL, 0, fw);
			}
			MyXUngrabServer(dpy);
	...
	if (IS_SHADED(fw)) {
		BroadcastPacket(M_WINDOWSHADE, 3, (long)FW_W(fw),
		    (long)FW_W_FRAME(fw), (unsigned long)fw);
	}
...
HandlePropertyNotify(const evh_args_t *ea)
{
	...
	if (te->xproperty.window == Scr.Root &&
		...
		for (t = Scr.FvwmRoot.next; t != NULL; t = t->next) {
			...
			if (draw_title || draw_picture) {
				DrawIconWindow(t, draw_title, draw_picture,
				    False, draw_picture, NULL);
			}
		}
	...
	case XA_WM_NAME: {
		...
		flush_property_notify_stop_at_event_type(te->xproperty.atom,
		    FW_W(fw), 0, 0);
		...
		if (fw->name.name &&
		    strcmp(new_name.name, fw->name.name) == 0) {
	...
	case XA_WM_ICON_NAME: {
		...
		FlocaleGetNameProperty(XGetWMIconName, dpy, FW_W(fw),
		    &new_name);
		if (new_name.name == NULL) {
			FlocaleFreeNameProperty(&new_name);
			return;
		...
		if (new_name.name &&
		    strlen(new_name.name) > MAX_ICON_NAME_LEN) {
			/* limit to prevent hanging X server */
	...
	default:
		if (te->xproperty.atom == _XA_WM_PROTOCOLS) {
			...
			is_on_this_page = IsRectangleOnThisPage(fw->m,
			    &(fw->g.frame), fw->Desk);
...
void
HandleReparentNotify(const evh_args_t *ea)
{
	const XEvent	 *te = ea->exc->x.etrigger;
	FvwmWindow *const fw = ea->exc->w.fw;
...
void
HandleShapeNotify(const evh_args_t *ea)
{
	FvwmWindow *const fw = ea->exc->w.fw;
...
void
HandleUnmapNotify(const evh_args_t *ea)
{
	int		  dstx, dsty;
	Window		  dumwin;
	XEvent		  dummy;
	...
	if (!fw) {
		if (XFindContext(dpy, te->xunmap.window, FvwmContext,
			(caddr_t *)&fw) == XCNOENT) {
			return;
		}
	}
	 ...
	 */
	if (!FCheckTypedWindowEvent(dpy, te->xunmap.window, DestroyNotify,
		&dummy) &&
	    XTranslateCoordinates(dpy, te->xunmap.window, Scr.Root, 0, 0, &dstx,
		&dsty, &dumwin)) {
		MyXGrabServer(dpy);
		SetMapStateProp(fw, WithdrawnState);
		EWMH_RestoreInitialStates(fw, te->type);
		if (FCheckTypedWindowEvent(dpy, te->xunmap.window,
			ReparentNotify, &dummy)) {
			if (fw->attr_backup.border_width) {
				XSetWindowBorderWidth(dpy, te->xunmap.window,
...
void
HandleVisibilityNotify(const evh_args_t *ea)
{
	FvwmWindow *const fw = ea->exc->w.fw;
 ...
 *  before sending. */
void
SendConfigureNotify(FvwmWindow *fw, int x, int y, int w, int h, int bw,
    Bool send_for_frame_too)
{
	...
	if (DEBUG_SEND_CN) {
		fprintf(stderr,
		    ...
		    client_event.xconfigure.height, (int)FW_W_FRAME(fw),
		    (int)FW_W(fw), (int)client_event.xconfigure.window,
		    (fw->name.name) ? fw->name.name : "");
	}
...
int
register_event_group(int event_base, int event_count, PFEH *jump_table)
{
...
InitEventHandlerJumpTable(void)
{
	...
	if (register_event_group(0, LASTEvent, EventHandlerJumpTable)) {
		/* should never happen */
		fvwm_debug(__func__, "Failed to initialize event handlers");
		exit(1);
	}
	if (FShapesSupported) {
		...
		if (register_event_group(FShapeEventBase, FShapeNumberEvents,
			shape_jump_table)) {
			fvwm_debug(__func__,
...
void
events_handle_configure_request(XEvent *cre, FvwmWindow *fw, Bool force,
    int force_gravity)
{
 ...
 */
int
My_XNextEvent(Display *disp, XEvent *event)
{
	...
	do {
		...
		num_fd = fvwmSelect(fvwmlib_max_fd, &in_fdset, &out_fdset, 0,
		    timeoutP);
 ...
 */
int
GetContext(FvwmWindow **ret_fw, FvwmWindow *t, const XEvent *e, Window *w)
{
	int    context;
	Window win;
	Window subw	    = None;
	...
	case KeyRelease:
		...
		if (win == Scr.Root && subw != None) {
			...
			win = subw;
			XTranslateCoordinates(dpy, Scr.Root, subw, x, y, &x, &y,
			    &subw);
	...
	case ButtonRelease:
		...
		if (t && win == FW_W_FRAME(t) && subw != None) {
			 ...
			 * coordinates. */
			win = subw;
			XTranslateCoordinates(dpy, FW_W_FRAME(t), subw, x, y,
			    &x, &y, &subw);
			if (win == FW_W_PARENT(t)) {
				win = subw;
				XTranslateCoordinates(dpy, FW_W_PARENT(t), subw,
				    x, y, &x, &y, &subw);
			}
		}
		break;
...
void
flush_accumulate_expose(Window w, XEvent *e)
{
...
CoerceEnterNotifyOnCurrentWindow(void)
{
	...
	if (XFindContext(dpy, child, FvwmContext, (caddr_t *)&fw) == XCNOENT) {
		...
		XTranslateCoordinates(dpy, Scr.Root, child, e.xcrossing.x_root,
...
int
discard_typed_events(int num_event_types, int *event_types)
{
...
int
flush_property_notify_stop_at_event_type(Atom atom, Window w,
    char do_stop_at_event_type, int stop_at_event_type)
{
	...
	FWeedIfEvents(dpy, _pred_flush_property_notify_weed, (XPointer)&args);
...
void
WaitForButtonsUp(Bool do_handle_expose)
{
	unsigned int mask;
	unsigned int bmask;
...
Bool
is_resizing_event_pending(FvwmWindow *fw)
{
	XEvent		    e;
