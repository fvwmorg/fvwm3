...
fake_map_unmap_notify(const FvwmWindow *fw, int event_type)
{
	...
	XSelectInput(
	    dpy, FW_W(fw), winattrs.your_event_mask & ~StructureNotifyMask);
...
_pred_flush_property_notify_weed(Display *display, XEvent *event, XPointer arg)
{
	...
	does_match_window =
	    (FEV_HAS_EVENT_WINDOW(event->type) && event->xany.window == args->w)
		? 1
		: 0;
...
_handle_cr_restack(int *ret_do_send_event, XEvent *e, FvwmWindow *fw)
{
	...
	if (cre->value_mask & CWSibling) {
		if (XFindContext(dpy, cre->above, FvwmContext, (caddr_t *)&fw2)
		    == XCNOENT) {
			fw2 = NULL;
		}
		if (fw2 == fw) {
	...
	/* only allow clients to restack windows within their layer */
		...
		if (cre->detail == Below) {
			...
			if (FW_W_ICON_PIXMAP(fw2) != None) {
				XConfigureWindow(
				    dpy, FW_W_ICON_PIXMAP(fw2), xwcm, &xwc);
			}
		}
...
_cr_detect_icccm_move(FvwmWindow *fw, XEvent *e, size_borders *b)
{
	...
	/* check full screen */
	if ((cre->value_mask & (CWX | CWY)) == (CWX | CWY) && (has_x || has_y)
	...
	if (!has_x) {
		mx = CR_MOTION_METHOD_AUTO;
		   ...
		   || static_g.x + w == mon->virtual_scr.MyDisplayWidth) {
		mx = CR_MOTION_METHOD_STATIC_GRAV;
	} else if (grav_g.x == 0
		   || grav_g.x + w == mon->virtual_scr.MyDisplayWidth) {
	...
	if (!has_y) {
		my = CR_MOTION_METHOD_AUTO;
		   ...
		   || static_g.y + h == mon->virtual_scr.MyDisplayHeight) {
		my = CR_MOTION_METHOD_STATIC_GRAV;
	} else if (grav_g.y == 0
		   || grav_g.y + h == mon->virtual_scr.MyDisplayHeight) {
    ...
    FvwmWindow *fw, Bool force, int force_gravity)
{
	...
	if (!(cre->value_mask & (CWX | CWY))) {
		/* nothing */
	} else if ((force || CR_MOTION_METHOD(fw) == CR_MOTION_METHOD_USE_GRAV)
		...
		if (cre->value_mask & CWX) {
			ref_x =
			    cre->x - ((grav_x + 1) * b.total_size.width) / 2;
			d_g.x = ref_x - current_g.x;
		}
		if (cre->value_mask & CWY) {
			ref_y =
			    cre->y - ((grav_y + 1) * b.total_size.height) / 2;
	...
	if (cre->value_mask & CWWidth) {
		if (cre->width
			...
			d_g.width =
			    cre->width - (current_g.width - b.total_size.width);
		} else {
			d_g.width	   = 0;
			*ret_do_send_event = 1;
		}
...
__check_click_to_focus_or_raise(hfrc_ret_t *ret_args, const exec_context_t *exc)
{
	...
	f.is_client_click =
	    (exc->w.wcontext == C_WINDOW || exc->w.wcontext == C_EWMH_DESKTOP);
...
HandleEnterNotify(const evh_args_t *ea)
{
	...
	ewp = &te->xcrossing;
	ENTER_DBG((stderr,
	    "++++++++ en (%d): fw %p w 0x%08x sw 0x%08x mode 0x%x detail 0x%x "
	    "'%s'\n",
	...
	if (ewp->window == Scr.Root && ewp->detail == NotifyInferior
		...
		BroadcastPacket(
		    MX_ENTER_WINDOW, 3, (long)Scr.Root, (long)NULL, (long)NULL);
		...
		if (is_initial_ungrab_pending) {
			...
			if (fw && fw == xcrossing_last_grab_window) {
				...
				if (ewp->window != FW_W_FRAME(fw)
				    && ewp->window != FW_W_ICON_TITLE(fw)
				    && ewp->window != FW_W_ICON_PIXMAP(fw)) {
					ENTER_DBG((stderr, "en: exit: NU: not "
							   "frame window\n"));
					focus_grab_buttons(fw);
					return;
				}
	...
	if (is_pan_frame(ewp->window)) {
		...
		if (edge_command && ewp->mode == NotifyUngrab
			...
			Scr.flags.is_pointer_on_this_screen = 1;
			e				    = *te;
			int p = HandlePaging(&e, m->virtual_scr.EdgeScrollX,
			    m->virtual_scr.EdgeScrollY, &JunkX, &JunkY,
	...
	if (ewp->window == FW_W_FRAME(fw) || ewp->window == FW_W_ICON_TITLE(fw)
	    || ewp->window == FW_W_ICON_PIXMAP(fw)) {
...
HandleFocusIn(const evh_args_t *ea)
{
	...
	if (was_nothing_ever_focused || last_focus_fw == None
		...
		if (!Scr.bo.do_enable_flickering_qt_dialogs_workaround
			...
			if (fw != NULL
			    && strcmp(fw->m->si->name, prev_focused_monitor)
				   != 0) {
...
__handle_key(const evh_args_t *ea, Bool is_press)
{
	...
	kc = XKeysymToKeycode(
	    dpy, fvwm_KeycodeToKeysym(dpy, te->xkey.keycode, 0, 0));
...
HandleLeaveNotify(const evh_args_t *ea)
{
	...
	ENTER_DBG((stderr,
	    "-------- ln (%d): fw %p w 0x%08x sw 0x%08x mode 0x%x detail 0x%x "
	    "'%s'\n",
	...
	if (lwp->window == Scr.Root && lwp->detail == NotifyInferior
		...
		BroadcastPacket(
		    MX_LEAVE_WINDOW, 3, (long)Scr.Root, (long)NULL, (long)NULL);
    ...
    FvwmWindow *ReuseWin, initial_window_options_t *win_opts)
{
	 ...
	 */
	is_on_this_page =
	    IsRectangleOnThisPage(fw->m, &(fw->g.frame), fw->Desk);
	if (KeepRaised != None) {
	...
	if (IS_ICONIFIED(fw))
		...
		default:
			...
			if (fw->Desk == m->virtual_scr.CurrentDesk) {
				...
				BroadcastPacket(M_MAP, 3, (long)FW_W(fw),
				    (long)FW_W_FRAME(fw), (unsigned long)fw);
#endif
			}
			...
			if ((initial_map_command != NULL)
			    && (!Restarting
				&& Scr.flags.are_windows_captured)) {
	...
	case XA_WM_HINTS: {
		...
		if ((fw->wmhints->flags & IconPixmapHint)
			...
			ICON_DBG((stderr, "hpn: iph changed (%d) '%s'\n",
			    !!(int)(fw->wmhints->flags & IconPixmapHint),
			    fw->name.name));
		...
		if ((fw->wmhints->flags & IconWindowHint)
			...
			ICON_DBG((stderr, "hpn: iwh changed (%d) '%s'\n",
			    !!(int)(fw->wmhints->flags & IconWindowHint),
			    fw->name.name));
		...
		if (has_icon_window_hint_changed
			...
			if (ICON_OVERRIDE_MODE(fw) == ICON_OVERRIDE) {
				...
				if (has_icon_pixmap_hint_changed) {
					...
					ICON_DBG(
					    (stderr, "hpn: using iwh '%s'\n",
						fw->name.name));
					has_icon_changed = True;
				} else {
					ICON_DBG((stderr,
					    "hpn: iwh not changed, hint "
					    "ignored '%s'\n",
					    fw->name.name));
					has_icon_changed = False;
				}
			...
			if (has_icon_changed) {
				...
				if (!fw->icon_bitmap_file
					...
					fw->icon_bitmap_file =
					    (Scr.DefaultIcon) ? Scr.DefaultIcon
							      : NULL;
				}
...
HandleUnmapNotify(const evh_args_t *ea)
{
	...
	if (!fw) {
		if (XFindContext(
			dpy, te->xunmap.window, FvwmContext, (caddr_t *)&fw)
		    == XCNOENT) {
			return;
		}
	}
	...
	if (!FCheckTypedWindowEvent(
	    ...
	    && XTranslateCoordinates(dpy, te->xunmap.window, Scr.Root, 0, 0,
		&dstx, &dsty, &dumwin)) {
		MyXGrabServer(dpy);
		SetMapStateProp(fw, WithdrawnState);
		EWMH_RestoreInitialStates(fw, te->type);
...
void
SendConfigureNotify(
    FvwmWindow *fw, int x, int y, int w, int h, int bw, Bool send_for_frame_too)
{
	XEvent	     client_event;
	size_borders b;
...
register_event_group(int event_base, int event_count, PFEH *jump_table)
{
	...
	while (
	    position != NULL && position->base + position->count < event_base) {
...
InitEventHandlerJumpTable(void)
{
	...
	if (FShapesSupported) {
		...
		if (register_event_group(FShapeEventBase, FShapeNumberEvents,
			shape_jump_table)) {
			fvwm_debug(
			    __func__, "Failed to init Shape event handler");
		}
	}
...
int
flush_property_notify_stop_at_event_type(
    Atom atom, Window w, char do_stop_at_event_type, int stop_at_event_type)
{
