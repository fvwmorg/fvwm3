...
fprintf(stderr,"hen: t 0x%08x, w 0x%08x, mode %d\n", (int)Tmp_win, (int)Event.xcrossing.window, Event.xcrossing.mode);
  ...
  else if (ewp->mode == NotifyUngrab)
  {
    ...
    if (is_initial_ungrab_pending)
    {
      ...
      xcrossing_last_grab_window = None;
    }
...
fprintf(stderr,"hen: 4\n");
    return;
  }

  if (!(sf = get_focus_window()) || HAS_MOUSE_FOCUS(sf))
  {
    DeleteFocus(1);
  }
...
fprintf(stderr,"hen: 6\n");
  ...
  else if (HAS_NEVER_FOCUS(Tmp_win))
  {
    ...
    {
fprintf(stderr,"hen: 7\n");
      focus_grab_buttons(Tmp_win, False);
      focus_grab_buttons(sf, True);
    }
fprintf(stderr,"hen: 8\n");
  }
...
fprintf(stderr,"hen: 9\n");
  ...
  if (IS_ICONIFIED(Tmp_win) && (ewp->mode == NotifyNormal))
  {
    ...
    DrawIconWindow(Tmp_win);
fprintf(stderr,"hen: 10\n");
  }
fprintf(stderr,"hen: 11\n");
...
void HandleLeaveNotify(void)
{
  ...
  if (Scr.flags.is_wire_frame_displayed)
    return;
fprintf(stderr,"hln: t 0x%08x, w 0x%08x, mode %d\n", (int)Tmp_win, (int)Event.xcrossing.window, Event.xcrossing.mode);
  if (Event.xcrossing.mode != NotifyNormal)
  {
fprintf(stderr,"hln: not nn\n");
    /* Ignore events generated by grabbing or ungrabbing the pointer.  However,
