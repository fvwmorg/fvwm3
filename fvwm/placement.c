...
typedef enum { PL_LOOP_END, PL_LOOP_CONT } pl_loop_rc_t;
...
typedef struct {
	...
	pl_penalty_t (*get_pos_simple)(position *ret_p, struct pl_ret_t *ret,
	    const struct pl_arg_t *arg);
	/* otherwise use these three in a loop */
	pl_loop_rc_t (*get_first_pos)(position *ret_p, struct pl_ret_t *ret,
	    const struct pl_arg_t *arg);
	pl_loop_rc_t (*get_next_pos)(position *ret_p, struct pl_ret_t *ret,
	    const struct pl_arg_t *arg, position hint_p);
	pl_penalty_t (*get_pos_penalty)(position *ret_hint_p,
...
const pl_algo_t minoverlap_placement_algo = { NULL,
	__pl_minoverlap_get_first_pos, __pl_minoverlap_get_next_pos,
	__pl_minoverlap_get_pos_penalty };

const pl_algo_t smart_placement_algo = { NULL, __pl_minoverlap_get_first_pos,
	__pl_minoverlap_get_next_pos, __pl_smart_get_pos_penalty };

const pl_algo_t position_placement_algo = { __pl_position_get_pos_simple };

const pl_algo_t cascade_placement_algo = { __pl_cascade_get_pos_simple };

const pl_algo_t manual_placement_algo = { __pl_manual_get_pos_simple };

/* ---------------------------- exported variables (globals) --------------- */

const pl_penalty_struct default_pl_penalty = { 1, PLACEMENT_AVOID_ONTOP,
	PLACEMENT_AVOID_ICON, PLACEMENT_AVOID_STICKY, PLACEMENT_AVOID_BELOW,
	PLACEMENT_AVOID_EWMH_STRUT };

const pl_percent_penalty_struct default_pl_percent_penalty = {
	PLACEMENT_AVOID_COVER_99, PLACEMENT_AVOID_COVER_95,
	PLACEMENT_AVOID_COVER_85, PLACEMENT_AVOID_COVER_75
};
...
static pl_penalty_t
__pl_position_get_pos_simple(position *ret_p, struct pl_ret_t *ret,
    const struct pl_arg_t *arg)
{
	char *spos;
	char *string_to_free;
	...
	if (spos == NULL || *spos == 0) {
		...
		xasprintf(&spos, DEFAULT_PLACEMENT_POS_CENTER_STRING,
	...
	for (n = -1; i < 2 && n < 2; i++) {
		...
		ret_p->x = 0;
		ret_p->y = 0;
		n    = GetMoveArguments(arg->place_fw, &spos, sz, ret_p, NULL,
		       &fPointer, False);
		spos = DEFAULT_PLACEMENT_POSITION_STRING;
		if (n < 2) {
	...
	if (is_under_mouse) {
		...
		/* TA:  20120316:  Imply the working-area when under the mouse
		 * -- this brings it in-line with making the EWMH working area
		 * the default. Note that "UnderMouse" is a special case,
		 * deliberately.  All other PositionPlacement commands are
		 * deliberately NOT subject to ewmhiwa options.
		 */
		EWMH_GetWorkAreaIntersection(arg->place_fw,
		    (int *)&arg->screen_g.x, (int *)&arg->screen_g.y,
		    (int *)&arg->screen_g.width, (int *)&arg->screen_g.height,
		    EWMH_USE_WORKING_AREA);

		if (ret_p->x + arg->place_fw->g.frame.width >
		    arg->screen_g.x + arg->screen_g.width) {
			ret_p->x = (arg->screen_g.x + arg->screen_g.width) -
			    arg->place_fw->g.frame.width;
		}
		if (ret_p->y + arg->place_fw->g.frame.height >
...
static pl_penalty_t
__pl_cascade_get_pos_simple(position *ret_p, struct pl_ret_t *ret,
    const struct pl_arg_t *arg)
{
	size_borders b;
	int	     w;
...
static pl_penalty_t
__pl_manual_get_pos_simple(position *ret_p, struct pl_ret_t *ret,
    const struct pl_arg_t *arg)
{
	ret_p->x = 0;
	ret_p->y = 0;
	if (GrabEm(CRS_POSITION, GRAB_NORMAL)) {
		...
		MyXGrabServer(dpy);
		if (XGetGeometry(dpy, FW_W(arg->place_fw), &JunkRoot, &JunkX,
		...
		if (__move_loop(arg->exc, m, drag, ret_p, False,
			CRS_POSITION)) {
...
static int
__pl_minoverlap_get_next_x(const pl_arg_t *arg)
{
	FvwmWindow     *other_fw;
	int		xnew;
	int		xtest;
	...
	m = (arg->place_fw && arg->place_fw->m) ? arg->place_fw->m :
						  monitor_get_current();
	x = arg->place_g.x;
	y = arg->place_g.y;
	...
	for (other_fw = Scr.FvwmRoot.next; other_fw != NULL;
	     other_fw = other_fw->next) {
		if (other_fw == arg->place_fw ||
		...
		if (IS_ICONIFIED(other_fw)) {
			...
			if (rc == True && y < g.y + g.height - sticky.y &&
				...
				for (i = start; i <= CP_GET_NEXT_STEP; i++) {
					xtest = (win_left) +
					    g.width * i / CP_GET_NEXT_STEP;
					if (xtest > x) {
						xnew = MIN(xnew, xtest);
					}
				}
			}
		} else if (y < other_fw->g.frame.height + other_fw->g.frame.y -
			    sticky.y &&
		    other_fw->g.frame.y - sticky.y < arg->place_g.height + y &&
...
static int
__pl_minoverlap_get_next_y(const pl_arg_t *arg)
{
	FvwmWindow     *other_fw;
	int		ynew;
	int		ytest;
	...
	mon = (arg->place_fw && arg->place_fw->m) ? arg->place_fw->m :
						    monitor_get_current();
...
static pl_loop_rc_t
__pl_minoverlap_get_first_pos(position *ret_p, struct pl_ret_t *ret,
    const pl_arg_t *arg)
{
...
static pl_loop_rc_t
__pl_minoverlap_get_next_pos(position *ret_p, struct pl_ret_t *ret,
    const struct pl_arg_t *arg, position hint_p)
{
...
static pl_penalty_t
__pl_minoverlap_get_avoidance_penalty(const pl_arg_t *arg, FvwmWindow *other_fw,
    const rectangle *other_g)
{
	pl_penalty_t			 anew;
...
static pl_penalty_t
__pl_minoverlap_get_pos_penalty(position *ret_hint_p, struct pl_ret_t *ret,
    const struct pl_arg_t *arg)
{
	FvwmWindow  *other_fw;
	...
	for (other_fw = Scr.FvwmRoot.next; other_fw != NULL;
		...
		if (arg->place_fw == other_fw ||
		    IS_EWMH_DESKTOP(FW_W(other_fw))) {
			continue;
		}
		/*  RBW - account for sticky windows...  */
		if (other_fw->Desk != arg->place_fw->Desk &&
		...
		if (arg->place_g.x < other_g.x + other_g.width &&
		    arg->place_p2.x > other_g.x &&
		    arg->place_g.y < other_g.y + other_g.height &&
		    arg->place_p2.y > other_g.y) {
			pl_penalty_t anew;

			anew = __pl_minoverlap_get_avoidance_penalty(arg,
			    other_fw, &other_g);
			penalty += anew;
			if (penalty > ret->best_penalty &&
			    ret->best_penalty != -1) {
	...
	{
		const pl_penalty_struct *mypp;
		struct monitor		*m = FindScreenOfXY(arg->place_g.x,
			     arg->place_g.y);
		...
		if (arg->flags.use_ewmh_dynamic_working_areapercent == 1) {
			penalty += EWMH_STRUT_PLACEMENT_PENALTY(mypp) *
			    EWMH_GetStrutIntersection(m, arg->place_g.x,
				arg->place_g.y, arg->place_p2.x,
				arg->place_p2.y, arg->flags.use_percent);
		} else {
			/* EWMH_USE_DYNAMIC_WORKING_AREA, count the base strut
			 */
			penalty += EWMH_STRUT_PLACEMENT_PENALTY(mypp) *
			    EWMH_GetBaseStrutIntersection(m, arg->place_g.x,
				arg->place_g.y, arg->place_p2.x,
				arg->place_p2.y, arg->flags.use_percent);
		}
	}
...
static pl_penalty_t
__pl_smart_get_pos_penalty(position *ret_hint_p, struct pl_ret_t *ret,
    const struct pl_arg_t *arg)
{
	pl_penalty_t p;
...
static int
placement_loop(pl_ret_t *ret, pl_arg_t *arg)
{
	position     next_p;
	pl_penalty_t penalty;
...
static void
__place_get_placement_flags(pl_flags_t *ret_flags, FvwmWindow *fw,
    window_style *pstyle, initial_window_options_t *win_opts, int mode,
    pl_reason_t *reason)
{
	Bool override_ppos;
	Bool override_uspos;
	...
	} else if (!ret_flags->do_honor_starts_on_page && fw->wmhints &&
...
static int
__add_algo(const pl_algo_t **algos, int num_algos, const pl_algo_t *new_algo)
{
...
static int
__place_get_wm_pos(const exec_context_t *exc, window_style *pstyle,
    rectangle *attr_g, pl_flags_t flags, rectangle screen_g,
    pl_start_style_t start_style, int mode, initial_window_options_t *win_opts,
    pl_reason_t *reason, int pdeltax, int pdeltay)
{
	...
	switch (placement_mode) {
	case PLACE_POSITION:
		num_algos = __add_algo(algos, num_algos,
		    &position_placement_algo);
		break;
	case PLACE_TILEMANUAL:
		num_algos = __add_algo(algos, num_algos, &smart_placement_algo);
		num_algos = __add_algo(algos, num_algos,
...
static int
__place_get_nowm_pos(const exec_context_t *exc, window_style *pstyle,
    rectangle *attr_g, pl_flags_t flags, rectangle screen_g,
    pl_start_style_t start_style, int mode, initial_window_options_t *win_opts,
    pl_reason_t *reason, int pdeltax, int pdeltay)
{
	...
	if (SUSE_START_ON_SCREEN(&pstyle->flags) &&
		...
		if (attr_g->x + attr_g->width < screen_g.x ||
			...
			FScreenTranslateCoordinates(&arg, FSCREEN_BY_NAME, NULL,
			    FSCREEN_GLOBAL, &attr_g->x, &attr_g->y);
	...
	/* if the gravity is towards the top, move it by the title height */
	{
		...
		if (mode == PLACE_INITIAL) {
			...
			gravity_resize(fw->hints.win_gravity, &final_g,
...
static int
__place_window(const exec_context_t *exc, window_style *pstyle,
    rectangle *attr_g, pl_start_style_t start_style, int mode,
    initial_window_options_t *win_opts, pl_reason_t *reason)
{
	...
	if (SUSE_START_ON_DESK(&pstyle->flags) ||
		...
		if (win_opts->flags.do_override_ppos &&
			...
			flags.do_honor_starts_on_screen = 0;
			reason->page.reason		= (preason_page_t)
			    PR_PAGE_IGNORE_CAPTURE;
			reason->page.do_ignore_starts_on_page = 1;
			reason->screen.reason = (preason_screen_t)
			    PR_PAGE_IGNORE_CAPTURE;
		}
		/*
	...
	if (SUSE_START_ON_SCREEN(&pstyle->flags)) {
		if (flags.do_honor_starts_on_screen) {
			...
			FScreenGetScrRect(&arg, FSCREEN_BY_NAME, &screen_g.x,
		...
		/* use current screen */
		FScreenGetScrRect(NULL, FSCREEN_CURRENT, &screen_g.x,
		    &screen_g.y, &screen_g.width, &screen_g.height);
	...
	if (SPLACEMENT_MODE(&pstyle->flags) != PLACE_MINOVERLAPPERCENT &&
		 ...
		 */
		EWMH_GetWorkAreaIntersection(fw, &screen_g.x, &screen_g.y,
	...
	if (S_IS_STICKY_ACROSS_DESKS(SFC(pstyle->flags))) {
	    ...
	    flags.do_honor_starts_on_page) {
		fw->Desk = (start_style.desk > -1) ? start_style.desk - 1 :
						     start_style.desk;
		reason->desk.reason = reason->desk.sod_reason;
	} else {
		if ((DO_USE_WINDOW_GROUP_HINT(fw)) && (fw->wmhints) &&
	...
	if (!is_tracking_shared && !win_opts->flags.do_override_ppos &&
	    !DO_NOT_SHOW_ON_MAP(fw)) {
		struct monitor *m = fw->m ? fw->m : monitor_get_current();
	...
	__place_get_placement_flags(&flags, fw, pstyle, win_opts, mode, reason);
	if (flags.do_not_use_wm_placement) {
		rc = __place_get_nowm_pos(exc, pstyle, attr_g, flags, screen_g,
		    start_style, mode, win_opts, reason, pdeltax, pdeltay);
	} else {
		rc = __place_get_wm_pos(exc, pstyle, attr_g, flags, screen_g,
		    start_style, mode, win_opts, reason, pdeltax, pdeltay);
	}
...
static void
__place_handle_x_resources(FvwmWindow *fw, window_style *pstyle,
    pl_reason_t *reason)
{
	int	 client_argc = 0;
	...
	MergeCmdLineResources(&db, table, 4, client_argv[0], &client_argc,
	    client_argv, True);
	...
	if (GetResourceString(db, "page", client_argv[0], &rm_value) &&
	    rm_value.size != 0) {
		spargs = sscanf(rm_value.addr, "%d %d %d", &t1, &t2, &t3);
		switch (spargs) {
		case 1:
			pstyle->flags.use_start_on_desk = 1;
...
static void
__explain_placement(FvwmWindow *fw, pl_reason_t *reason)
{
	...
	*explanation = 0;
	...
	sprintf(s, "  screen: %s: %d %d %dx%d (%s)\n", reason->screen.screen,
	    reason->screen.g.x, reason->screen.g.y, reason->screen.g.width,
	...
	fvwm_debug(__func__, explanation, (int)FW_W(fw), fw->name.name);
...
Bool
setup_window_placement(FvwmWindow *fw, window_style *pstyle, rectangle *attr_g,
...
CMD_PlaceAgain(F_CMD_ARGS)
{
	...
	if (IS_ICONIFIED(fw) && do_place_icon) {
		...
		AnimatedMoveFvwmWindow(fw, FW_W_FRAME(fw), start, end, False,
		    -1, ppctMovement);
	}
	if (fw->Desk != old_desk) {
