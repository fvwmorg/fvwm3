...
static char *button_states[BS_MaxButtonStateName + 1] = { "ActiveUp",
	"ActiveDown", "InactiveUp", "InactiveDown", "ToggledActiveUp",
	"ToggledActiveDown", "ToggledInactiveUp", "ToggledInactiveDown",
	"Active", "Inactive", "ToggledActive", "ToggledInactive", "AllNormal",
...
status_send(void)
{
	...
	TAILQ_FOREACH (m, &monitor_q, entry) {
		...
		di = m->Desktops->next;
		while (di != NULL) {
			if ((individual_d[d_count] = cJSON_CreateObject()) ==
			    NULL)
				goto out;
			cJSON_AddNumberToObject(individual_d[d_count], "number",
			    di->desk);
			cJSON_AddBoolToObject(individual_d[d_count],
			    "is_current",
			    di->desk == m->virtual_scr.CurrentDesk);
			cJSON_AddBoolToObject(individual_d[d_count],
			    "is_urgent", desk_get_fw_urgent(m, di->desk));
			cJSON_AddNumberToObject(individual_d[d_count],
			    "number_of_clients",
			    desk_get_fw_count(m, di->desk));
			cJSON_AddItemToObject(this_desktop, di->name,
...
ReadTitleButton(char *s, TitleButton *tb, Bool append, int button)
{
	...
	if (*s == '(') {
		...
		pstyle = 1;
		if (!(end = strchr(++s, ')'))) {
			fvwm_debug(__func__, "missing parenthesis: %s", s);
			return NULL;
		}
		s   = SkipSpaces(s, NULL, 0);
		len = end - s + 1;
	...
	for (t = Scr.FvwmRoot.next; t; t = t->next) {
		if (t->decor == d) {
			...
			exc2	       = exc_clone_context(exc, &ecc,
				      ECC_FW | ECC_WCONTEXT);
			execute_function(cond_rc, exc2, "ChangeDecor Default",
			    pc, 0);
...
static void
do_title_style(F_CMD_ARGS, Bool do_add)
{
	char	  *parm;
	char	  *prev;
 ...
 */
static char *
ReadMultiPixmapDecor(char *s, DecorFace *df)
{
	static char  *pm_names[TBMP_NUM_PIXMAPS + 1] = { "Main", "LeftMain",
		 "RightMain", "LeftButtons", "RightButtons", "UnderText",
		 "LeftOfText", "RightOfText", "LeftEnd", "RightEnd", "Buttons",
		 NULL };
	FvwmPicture **pm;
	FvwmAcs	     *acs;
	Pixel	     *pixels;
	...
	df->u.mp.pixmaps = pm = fxcalloc(TBMP_NUM_PIXMAPS,
	    sizeof(FvwmPicture *));
	...
	while (pm_id >= 0) {
		...
		if (pm[pm_id] || acs[pm_id].cs >= 0 ||
			...
			fvwm_debug(__func__, "Ignoring: already-specified %s",
			    pm_names[i]);
			continue;
		}
		if (stretched) {
		...
		if (load_pixmap && token) {
			...
			pm[pm_id] = PCacheFvwmPicture(dpy, Scr.NoFocusWin, NULL,
			    token, fpa);
			if (!pm[pm_id]) {
				fvwm_debug(__func__,
				    "Pixmap '%s' could not be loaded", token);
			}
		}
		if (pm_id == TBMP_BUTTONS) {
			...
			if (pm[TBMP_BUTTONS]) {
				pm[TBMP_LEFT_BUTTONS] = PCloneFvwmPicture(
				    pm[TBMP_BUTTONS]);
				acs[TBMP_LEFT_BUTTONS].cs = -1;
				pm[TBMP_RIGHT_BUTTONS]	  = PCloneFvwmPicture(
				       pm[TBMP_BUTTONS]);
				acs[TBMP_RIGHT_BUTTONS].cs = -1;
			} else {
				pm[TBMP_RIGHT_BUTTONS] = pm[TBMP_LEFT_BUTTONS] =
				    NULL;
 ...
 */
static void
DestroyFvwmDecor(FvwmDecor *decor)
{
	int i;
...
static void
SetLayerButtonFlag(int layer, int multi, int set, FvwmDecor *decor,
    TitleButton *tb)
{
	int i;
	int start = 0;
 ...
 */
static void
SetMWMButtonFlag(mwm_flags flag, int multi, int set, FvwmDecor *decor,
    TitleButton *tb)
{
	int i;
	int start = 0;
...
static void
do_button_style(F_CMD_ARGS, Bool do_add)
{
	int	     i;
	int	     multi  = 0;
	int	     button = 0;
	...
	if (parm == NULL || button >= NUMBER_OF_TITLE_BUTTONS || button < 0) {
		fvwm_debug(__func__, "Bad button style (1) in line %s", action);
		return;
	}
		...
		if (!do_add && strcmp(parm, "-") == 0) {
			...
			while (tok) {
				...
				if (StrEquals(tok, "Clear")) {
					...
					SetMWMButtonFlag(MWM_DECOR_MENU, multi,
					    set, decor, tb);
				} else if (StrEquals(tok, "MWMDecorMin")) {
					SetMWMButtonFlag(MWM_DECOR_MINIMIZE,
					    multi, set, decor, tb);
				} else if (StrEquals(tok, "MWMDecorMax")) {
					SetMWMButtonFlag(MWM_DECOR_MAXIMIZE,
					    multi, set, decor, tb);
				} else if (StrEquals(tok, "MWMDecorShade")) {
					SetMWMButtonFlag(MWM_DECOR_SHADE, multi,
					    set, decor, tb);
				} else if (StrEquals(tok, "MWMDecorStick")) {
					SetMWMButtonFlag(MWM_DECOR_STICK, multi,
					    set, decor, tb);
					...
					if (!ltok || !got_number) {
						...
						SetLayerButtonFlag(layer, multi,
						    set, decor, tb);
					}
				...
				for (i = 0; i < NUMBER_OF_TITLE_BUTTONS; ++i) {
					if (((multi & 1) && !(i & 1)) ||
						...
						text = ReadTitleButton(prev,
						    &decor->buttons[i], do_add,
						    i);
					}
				}
			} else if (!(text = ReadTitleButton(prev, tb, do_add,
					 button))) {
				break;
			}
		}
...
static int
update_decorface_colorset(DecorFace *df, int cset)
{
	DecorFace *tdf;
	int	   has_changed = 0;
...
static int
update_titlebutton_colorset(TitleButton *tb, int cset)
{
	int i;
	int has_changed = 0;
...
static void
update_decors_colorset(int cset)
{
	int	   i;
	...
	for (decor = &Scr.DefaultDecor; decor != NULL; decor = decor->next) {
		...
		decor->flags.has_changed |=
		    update_titlebutton_colorset(&(decor->titlebar), cset);
...
static Bool
__parse_vector_line_one_coord(char **ret_action, int *pcoord, int *poff,
    char *action)
{
	int offset;
	int n;
...
static Bool
__parse_vector_line(char **ret_action, int *px, int *py, int *pxoff, int *pyoff,
    int *pc, char *action)
{
	Bool is_valid = True;
	int  offset;
	...
	*ret_action = action;
	if (__parse_vector_line_one_coord(&action, px, pxoff, action) ==
	    False) {
		return False;
	}
	if (*action != 'x') {
	...
	action++;
	if (__parse_vector_line_one_coord(&action, py, pyoff, action) ==
	    False) {
		return False;
	}
	if (*action != '@') {
...
void
refresh_window(Window w, Bool window_update)
{
	...
	w = XCreateWindow(dpy, w, 0, 0, monitor_get_all_widths(),
	    monitor_get_all_heights(), 0, CopyFromParent, CopyFromParent,
...
ApplyDefaultFontAndColors(void)
{
	...
	if (Scr.StdReliefGC) {
		...
		Scr.StdReliefGC = fvwmlib_XCreateGC(dpy, Scr.NoFocusWin, gcm,
		    &gcv);
	}
	...
	if (Scr.StdShadowGC) {
		...
		Scr.StdShadowGC = fvwmlib_XCreateGC(dpy, Scr.NoFocusWin, gcm,
		    &gcv);
	}
...
void
FreeDecorFace(Display *disp, DecorFace *df)
{
	switch (DFS_FACE_TYPE(df->style)) {
	case GradientButton:
		if (df->u.grad.d_pixels != NULL && df->u.grad.d_npixels) {
			PictureFreeColors(disp, Pcmap, df->u.grad.d_pixels,
			...
			PictureFreeColors(disp, Pcmap, p, df->u.grad.npixels, 0,
			    False);
			free(p);
		}
		...
		if (df->u.mp.pixmaps) {
			...
			for (i = 0; i < TBMP_NUM_PIXMAPS; i++) {
				if (df->u.mp.pixmaps[i]) {
					PDestroyFvwmPicture(disp,
					    df->u.mp.pixmaps[i]);
				} else if (!!(df->u.mp.solid_flags & i)) {
					PictureFreeColors(disp, Pcmap,
					    &df->u.mp.pixels[i], 1, 0, False);
				}
			}
 ...
 */
Bool
ReadDecorFace(char *s, DecorFace *df, int button, int verbose)
{
	int   offset;
	char  style[256], *file;
	char *action = s;
	...
	if (strncasecmp(style, "--", 2) != 0) {
		...
		if (strncasecmp(style, "Simple", 6) == 0) {
			...
			for (i = 0; i < vc->num; ++i) {
				...
				if (__parse_vector_line(&s, &x, &y, &xoff,
					&yoff, &c, s) == False) {
					break;
				}
				if (x < 0) {
			 ...
			 * colors etc */
			npixels = ParseGradient(s, &s, &s_colors, &perc,
			    &nsegs);
			while (*s && isspace(*s)) {
				s++;
			}
			...
			xcs = AllocAllGradientColors(s_colors, perc, nsegs,
			    npixels, do_dither);
			if (xcs == None)
				return False;
			df->u.grad.xcs	     = xcs;
			...
			s	 = GetNextToken(s, &file);
			fpa.mask = (Pdepth <= 8) ? FPAM_DITHER : 0; /* ? */
			df->u.p	 = PCacheFvwmPicture(dpy, Scr.NoFocusWin, NULL,
			     file, fpa);
			if (df->u.p == NULL) {
				if (file) {
					if (verbose) {
			...
			if (strncasecmp(style, "Tiled", 5) == 0) {
				...
				DFS_FACE_TYPE(
				    df->style) = StretchedPixmapButton;
	...
	if (file && (strcmp(file, "--") == 0)) {
		...
		while (tok && *tok) {
			...
			if (StrEquals(tok, "Clear")) {
				...
				if (set) {
					DFS_BUTTON_RELIEF(
					    df->style) = DFS_BUTTON_IS_FLAT;
				} else if (DFS_BUTTON_RELIEF(df->style) ==
				    DFS_BUTTON_IS_FLAT) {
					DFS_BUTTON_RELIEF(
					    df->style) = DFS_BUTTON_IS_UP;
				}
			} else if (StrEquals(tok, "Sunk")) {
				if (set) {
					DFS_BUTTON_RELIEF(
					    df->style) = DFS_BUTTON_IS_SUNK;
				} else if (DFS_BUTTON_RELIEF(df->style) ==
				    DFS_BUTTON_IS_SUNK) {
					DFS_BUTTON_RELIEF(
					    df->style) = DFS_BUTTON_IS_UP;
...
void
CMD_RefreshWindow(F_CMD_ARGS)
{
	FvwmWindow *const fw = exc->w.fw;

	refresh_window((exc->w.wcontext == C_ICON) ? FW_W_ICON_TITLE(fw) :
						     FW_W_FRAME(fw),
	    True);
...
CMD_Wait(F_CMD_ARGS)
{
	...
	if (wait_string) {
		...
		if (*rest) {
			...
			for (i = strlen(wait_string) - 1;
			     i >= 0 && isspace(wait_string[i]); i--) {
				wait_string[i] = 0;
			}
		}
	} else {
	...
	while (!done && !isTerminated) {
		...
		if (My_XNextEvent(dpy, &e)) {
			dispatch_event(&e);
			if (XFindContext(dpy, e.xmap.window, FvwmContext,
				(caddr_t *)&t) == XCNOENT) {
				t = NULL;
			}
			...
			if (e.type == MapNotify && e.xmap.event == Scr.Root) {
				...
				if (t &&
				    matchWildcards(wait_string, t->name.name) ==
					True) {
					done = True;
...
CMD_PrintInfo(F_CMD_ARGS)
{
	...
	if (StrEquals(subject, "Colors")) {
		...
		fvwm_debug(__func__, "Unknown subject '%s'", action);
	}
	if (subject) {
		free(subject);
	}
...
void
CMD_IconPath(F_CMD_ARGS)
{
	fvwm_debug(__func__, "IconPath is deprecated; use ImagePath instead.");
}

void
CMD_PixmapPath(F_CMD_ARGS)
{
	fvwm_debug(__func__,
...
void
CMD_ModuleTimeout(F_CMD_ARGS)
{
	int timeout;

	moduleTimeout = DEFAULT_MODULE_TIMEOUT;
	if (GetIntegerArguments(action, NULL, &timeout, 1) == 1 &&
	    timeout > 0) {
...
CMD_HilightColor(F_CMD_ARGS)
{
	...
	if (fore && back) {
		xasprintf(&action, "* HilightFore %s, HilightBack %s", fore,
		    back);
...
void
CMD_HilightColorset(F_CMD_ARGS)
{
	char *newaction;
...
CMD_UpdateDecor(F_CMD_ARGS)
{
	...
	for (fw2 = Scr.FvwmRoot.next; fw2; fw2 = fw2->next) {
		...
		if (found) {
			if (fw2->decor == found) {
				border_draw_decorations(fw2, PART_ALL, True,
				    True, CLEAR_ALL, NULL, NULL);
				border_draw_decorations(fw2, PART_ALL, False,
				    True, CLEAR_ALL, NULL, NULL);
			}
		} else {
			border_draw_decorations(fw2, PART_ALL, True, True,
			    CLEAR_ALL, NULL, NULL);
			border_draw_decorations(fw2, PART_ALL, False, True,
			    CLEAR_ALL, NULL, NULL);
		}
	}
	border_draw_decorations(hilight, PART_ALL, True, True, CLEAR_ALL, NULL,
	    NULL);
}
...
CMD_BugOpts(F_CMD_ARGS)
{
	...
	while (action && *action && *action != '\n') {
		...
		toggle = ParseToggleArgument(SkipNTokens(optstring, 1), NULL, 2,
		    False);
		...
		if (StrEquals(opt, "FlickeringMoveWorkaround")) {
			...
			fvwm_debug(__func__, "Unknown Bug Option '%s'", opt);
		}
	}
...
static Bool
FKeysymToKeycode(Display *disp, KeySym keysym, unsigned int *keycode,
		...
		if (ks == keysym) {
			...
			case 1: /* Shift modifier */
				*modifiers |= ShiftMask;
				break;
			default:
				fvwm_debug(__func__, "Unhandled modifier %d",
				    m);
				break;
			}
			return True;
...
static void
__fake_event(F_CMD_ARGS, FakeEventType type)
{
	char	    *token;
	char	    *optlist[] = { "press", "p", "release", "r", "wait", "w",
		       "modifiers", "m", "depth", "d", NULL };
	unsigned int mask      = 0;
	...
	FQueryPointer(dpy, Scr.Root, &root, &JunkRoot, &JunkX, &JunkY, &JunkX,
	    &JunkY, &mask);
	...
	while (token && action) {
		...
		case 3:
			...
			w	= None;
			child_w = root;
			for (depth = 1; depth != maxdepth && w != child_w &&
			     child_w != None;
			     depth++) {
				w = child_w;
				if (FQueryPointer(dpy, w, &root, &child_w, &rx,
					&ry, &x, &y, &JunkMask) == False) {
		...
		case 5:
			...
			usleep(1000 * val);
			if (FQueryPointer(dpy, Scr.Root, &root, &JunkRoot,
		...
		case 7:
			...
			if (GetIntegerArguments(action, &action, &val, 1) !=
			    1) {
				fvwm_debug(__func__,
				    "Invalid modifier value in \"%s\"", args);
				return;
			}
			do_unset = False;
			if (val < 0) {
		...
		case 9:
			...
			break;
		default:
			fvwm_debug(__func__, "Invalid command (%s) in \"%s\"",
			    token, args);
			return;
		}
		if (action) {
