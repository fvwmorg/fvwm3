...
static char *button_states[BS_MaxButtonStateName + 1] = {"ActiveUp",
    ...
    "ToggledActiveDown", "ToggledInactiveUp", "ToggledInactiveDown", "Active",
    "Inactive", "ToggledActive", "ToggledInactive", "AllNormal", "AllToggled",
    "AllActive", "AllInactive", "AllUp", "AllDown", "AllActiveUp",
    "AllActiveDown", "AllInactiveUp", "AllInactiveDown", NULL};
...
status_send(void)
{
	...
	TAILQ_FOREACH(m, &monitor_q, entry)
	{
		...
		while (di != NULL) {
			...
			BSON_APPEND_INT64(
			    individual_d[d_count], "number", di->desk);
			...
			BSON_APPEND_DOCUMENT(
			    desktops[d_count], di->name, individual_d[d_count]);
			BSON_APPEND_DOCUMENT(
			    desk_doc[d_count], "desktops", desktops[d_count]);
...
status_init_pipe(void)
{
	...
	fvwm_debug(
	    __func__, "Pipe opened: %s (fd: %d)", pipename, fileno(status_fp));
...
__remove_window_decors(F_CMD_ARGS, FvwmDecor *d)
{
	...
	for (t = Scr.FvwmRoot.next; t; t = t->next) {
		if (t->decor == d) {
			...
			exc2 =
			    exc_clone_context(exc, &ecc, ECC_FW | ECC_WCONTEXT);
			execute_function(
...
do_title_style(F_CMD_ARGS, Bool do_add)
{
	...
	for (prev = action; (parm = PeekToken(action, &action));
		...
		if (!do_add && StrEquals(parm, "centered")) {
			...
			action =
			    ReadTitleButton(prev, &decor->titlebar, do_add, -1);
		}
	}
...
ReadMultiPixmapDecor(char *s, DecorFace *df)
{
	...
	df->u.mp.pixmaps    = pm =
	    fxcalloc(TBMP_NUM_PIXMAPS, sizeof(FvwmPicture *));
...
refresh_window(Window w, Bool window_update)
{
	...
	w = XCreateWindow(dpy, w, 0, 0, m->virtual_scr.MyDisplayWidth,
	    m->virtual_scr.MyDisplayHeight, 0, CopyFromParent, CopyFromParent,
...
void
FreeDecorFace(Display *dpy, DecorFace *df)
{
	int i;
	...
	case GradientButton:
		if (df->u.grad.d_pixels != NULL && df->u.grad.d_npixels) {
			PictureFreeColors(dpy, Pcmap, df->u.grad.d_pixels,
...
InitFvwmDecor(FvwmDecor *decor)
{
	...
	for (i = 0; i < BS_MaxButtonState; ++i) {
		DFS_FACE_TYPE(TB_STATE(decor->titlebar)[i].style) =
		    SimpleButton;
	}
...
void CMD_CursorMove(F_CMD_ARGS)
{
	...
	FWarpPointerUpdateEvpos(exc->x.elast, dpy, None, Scr.Root, 0, 0,
	    m->virtual_scr.MyDisplayWidth, m->virtual_scr.MyDisplayHeight, x,
	    y);
...
void CMD_Delete(F_CMD_ARGS)
{
	...
	if (IS_TEAR_OFF_MENU(fw)) {
		 ...
		 * be read from there. */
		send_clientmessage(
		    dpy, FW_W_PARENT(fw), _XA_WM_DELETE_WINDOW, CurrentTime);
...
void CMD_Exec(F_CMD_ARGS)
{
	...
	if (!(fork())) /* child process */
	{
		...
		if (fvwm_setpgrp() == -1) {
			fvwm_debug(
			    __func__, "setpgrp failed (%s)", strerror(errno));
			exit(100);
		}
		if (execl(exec_shell_name, exec_shell_name, "-c", cmd, NULL)
		    == -1) {
			fvwm_debug(
			    __func__, "execl failed (%s)", strerror(errno));
			exit(100);
		}
	}
	free(cmd);
...
void CMD_RefreshWindow(F_CMD_ARGS)
{
	...
	refresh_window(
	    (exc->w.wcontext == C_ICON) ? FW_W_ICON_TITLE(fw) : FW_W_FRAME(fw),
	    True);
...
void CMD_Wait(F_CMD_ARGS)
{
	...
	while (!done && !isTerminated) {
		...
		if (My_XNextEvent(dpy, &e)) {
			dispatch_event(&e);
			if (XFindContext(
				dpy, e.xmap.window, FvwmContext, (caddr_t *)&t)
			    == XCNOENT) {
				t = NULL;
			}
	...
	for (fw2 = Scr.FvwmRoot.next; fw2; fw2 = fw2->next) {
		...
		if (found) {
			...
			border_draw_decorations(
			    fw2, PART_ALL, True, True, CLEAR_ALL, NULL, NULL);
			border_draw_decorations(
			    fw2, PART_ALL, False, True, CLEAR_ALL, NULL, NULL);
		}
	}
	border_draw_decorations(
...
void CMD_GlobalOpts(F_CMD_ARGS)
{
	...
	int   i;
	Bool  is_bugopt;
	char *optlist[]	    = {"WindowShadeShrinks", "WindowShadeScrolls",
	...
	for (action = GetNextSimpleOption(action, &opt); opt;
		...
		if (i > -1) {
			...
			action = tmp;
			fvwm_debug(__func__,
			    "Please replace 'GlobalOpts %s' with '%s %s'.", opt,
			    cmd, replace);
		} else {
			fvwm_debug(__func__, "Unknown Global Option '%s'", opt);
		}
...
void CMD_SetAnimation(F_CMD_ARGS)
{
	...
	if (!opt || sscanf(opt, "%d", &delay) != 1) {
		fvwm_debug(
		    __func__, "Improper milli-second delay as first argument");
		return;
	}
	if (delay > 500) {
		fvwm_debug(__func__,
...
static Bool
FKeysymToKeycode(
    Display *dpy, KeySym keysym, unsigned int *keycode, unsigned int *modifiers)
{
	int m;
	...
	*modifiers = 0;
	...
	for (m = 0; m <= 8; ++m) {
		...
		if (ks == keysym) {
			...
			default:
				fvwm_debug(
				    __func__, "Unhandled modifier %d", m);
				break;
			}
			return True;
		}
...
__fake_event(F_CMD_ARGS, FakeEventType type)
{
	...
	while (token && action) {
		...
		case 3:
			...
			if (type == FakeMouseEvent) {
				e.type =
				    (do_unset) ? ButtonRelease : ButtonPress;
				...
				e.xkey.state |= mask;
				add_mask =
				    (do_unset) ? KeyReleaseMask : KeyPressMask;
			}
