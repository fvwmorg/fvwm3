...
static char *csetopts[] = { "Foreground", "Fore", "fg",
	...
	"Alpha", "PixmapAlpha", /* ~ Alpha */
	"ImageAlpha",		/* ~ Alpha */

	/* Icon stuff */
	"DitherIcon", "NoDitherIcon", "IconTint", "NoIconTint", "IconAlpha",
...
get_root_pixmap(Atom prop)
{
	...
	if (ret == Success && type == XA_PIXMAP && format == 32 &&
	    length == 1 && after == 0) {
...
update_root_pixmap(Atom prop)
{
	...
	if (prop != 0) {
		...
		if (pix &&
		    !XGetGeometry(dpy, pix, &dummy, (int *)&dummy,
			(int *)&dummy, (unsigned int *)&w, (unsigned int *)&h,
...
static void
add_to_junk(Pixmap pixmap)
{
	struct junklist *oldjunk = junk;

	junk	     = fxmalloc(sizeof(struct junklist));
	junk->prev   = oldjunk;
	junk->pixmap = pixmap;
...
static char *
get_simple_color(char *string, char **color, colorset_t *cs, int supplied_color,
...
static void
SafeDestroyPicture(Display *disp, FvwmPicture *picture)
{
...
free_colorset_background(colorset_t *cs, Bool do_free_args)
{
	...
	if (cs->pixels && cs->nalloc_pixels) {
		PictureFreeColors(dpy, Pcmap, cs->pixels, cs->nalloc_pixels, 0,
		    False);
		free(cs->pixels);
		cs->pixels	  = NULL;
		cs->nalloc_pixels = 0;
...
static void
reset_cs_pixmap(colorset_t *cs, GC gc)
{
	if (Pdepth == cs->picture->depth) {
		XCopyArea(dpy, cs->picture->picture, cs->pixmap, gc, 0, 0,
		    cs->width, cs->height, 0, 0);
	} else {
		XCopyPlane(dpy, cs->picture->picture, cs->pixmap, gc, 0, 0,
		    cs->width, cs->height, 0, 0, 1);
	}
...
static void
parse_pixmap(Window win, GC gc, colorset_t *cs, Bool *pixmap_is_a_bitmap)
{
	...
	if (!*pixmap_is_a_bitmap) {
		cs->pixmap = XCreatePixmap(dpy, win, cs->width, cs->height,
		    Pdepth);
	...
	if (cs->pixmap) {
		if (cs->picture->mask != None) {
			...
			cs->mask = XCreatePixmap(dpy, win, cs->width,
			    cs->height, 1);
			if (cs->mask) {
				XCopyArea(dpy, cs->picture->mask, cs->mask,
				    Scr.MonoGC, 0, 0, cs->width, cs->height, 0,
				    0);
...
static void
parse_shape(Window win, colorset_t *cs, int i, char *args,
...
static void
parse_simple_tint(colorset_t *cs, char *args, char **tint, int supplied_color,
	...
	*changed = False;
	...
	if (!(cs->color_flags & supplied_color)) {
		...
		*changed = True;
		...
		fvwm_debug(__func__,
		    "%s must have two arguments a color and an integer", cmd);
		return;
	}
	*changed = True;
	if (*percent > 100) {
...
parse_colorset(int n, char *line)
{
	...
	while (line && *line) {
		...
		case 1: /* Fore */
		case 2: /* fg */
			get_simple_color(args, &fg, cs, FG_SUPPLIED,
			    FG_CONTRAST, "contrast");
			has_fg_changed = True;
			break;
		case 3: /* Background */
		case 4: /* Back */
		case 5: /* bg */
			get_simple_color(args, &bg, cs, BG_SUPPLIED, BG_AVERAGE,
			    "average");
			has_bg_changed = True;
			break;
		case 6: /* Hilight */
		...
		case 11: /* sh */
			...
			break;
		case 12: /* fgsh */
			get_simple_color(args, &fgsh, cs, FGSH_SUPPLIED, 0,
			    NULL);
		...
		case 29: /* NoTint */
			...
			break;
		case 30: /* fgTint */
			parse_simple_tint(cs, args, &fg_tint, FG_TINT_SUPPLIED,
			...
			break;
		case 31: /* bgTint */
			parse_simple_tint(cs, args, &bg_tint, BG_TINT_SUPPLIED,
		...
		case 38: /* DoNotDitherIcon */
			...
			break;
		case 39: /* IconTint */
			parse_simple_tint(cs, args, &icon_tint,
			    ICON_TINT_SUPPLIED, &has_icon_tint_changed,
			    &percent, "IconTint");
	...
	if (do_reload_pixmap) {
		...
		if (cs->pixmap_type == PIXMAP_ROOT_PIXMAP_PURE ||
			...
			cs->pixmap = CreateGradientPixmapFromString(dpy, win,
			    gc, cs->gradient_type, cs->gradient_args, &w, &h,
			    &cs->pixels, &cs->nalloc_pixels, cs->dither);
			cs->width  = w;
			cs->height = h;
		}
		has_pixmap_changed = True;
	...
	if (has_bg_changed ||
		...
		if (cs->color_flags & BG_AVERAGE) {
			...
			if (average_pix == root_pic.pixmap) {
				...
				if (!XGetGeometry(dpy, average_pix, &dummy,
					(int *)&dummy, (int *)&dummy,
					(unsigned int *)&w2,
					(unsigned int *)&h2,
					(unsigned int *)&dummy,
		...
		if ((cs->color_flags & BG_AVERAGE) && average_pix) {
			...
			image = XGetImage(dpy, average_pix, 0, 0, cs->width,
			    cs->height, AllPlanes, ZPixmap);
			if (cs->mask != None) {
				mask_image = XGetImage(dpy, cs->mask, 0, 0,
				    cs->width, cs->height, AllPlanes, ZPixmap);
			}
			...
			if (k == 0) {
				...
				for (i = 0; i < k; i += 256) {
					XQueryColors(dpy, Pcmap, &colors[i],
					    min(k - i, 256));
				}
				...
				color.blue  = dblue / k;
				{
					...
					PictureFreeColors(dpy, Pcmap, &cs->bg,
					    1, 0, True);
	 ...
	 */
	if (has_bg_tint_changed ||
	    (has_bg_changed && cs->bg_tint_percent > 0)) {
		if (cs->bg_tint_percent == 0) {
			...
			PictureFreeColors(dpy, Pcmap, &cs->bg, 1, 0, True);
			cs->bg = GetTintedPixel(cs->bg_saved, cs->bg_tint,
			    cs->bg_tint_percent);
			if (old_bg != cs->bg) {
				have_pixels_changed = True;
				has_bg_changed	    = True;
	 ...
	 */
	if (has_fg_tint_changed ||
	    (has_fg_changed && cs->fg_tint_percent > 0)) {
		if (cs->fg_tint_percent == 0) {
			...
			PictureFreeColors(dpy, Pcmap, &cs->fg, 1, 0, True);
			cs->fg = GetTintedPixel(cs->fg_saved, cs->fg_tint,
			    cs->fg_tint_percent);
			if (old_fg != cs->fg) {
				have_pixels_changed = True;
				has_fg_changed	    = 1;
	...
	if (cs->pixmap != None && cs->pixmap != ParentRelative &&
		...
		XFillRectangle(dpy, temp, gc, 0, 0, cs->width, cs->height);
		...
		PGraphicsRenderPixmaps(dpy, win, cs->pixmap, mask, alpha,
		    Pdepth, &fra, temp, gc, Scr.MonoGC, Scr.AlphaGC, 0, 0,
		    cs->width, cs->height, 0, 0, cs->width, cs->height, False);
...
alloc_colorset(int n)
{
	...
	if (n < nColorsets) {
		...
		Colorset = fxrealloc((void *)Colorset, (n + 1),
		    sizeof(colorset_t));
	...
	while (nColorsets <= n) {
		...
		if (PictureUseBWOnly()) {
			char g_bits[] = { 0x0a, 0x05, 0x0a, 0x05, 0x08, 0x02,
				0x08, 0x02, 0x01, 0x02, 0x04, 0x08 };
			...
			ncs->pixmap    = XCreatePixmapFromBitmapData(dpy,
			       Scr.NoFocusWin, &g_bits[4 * (nColorsets % 3)], 4, 4,
...
void
CMD_CleanupColorsets(F_CMD_ARGS)
{
	cleanup_colorsets();
}
