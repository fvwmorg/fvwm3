...
static void
SafeDestroyPicture(Display *dpy, FvwmPicture *picture)
{
...
parse_pixmap(Window win, GC gc, colorset_t *cs, Bool *pixmap_is_a_bitmap)
{
	...
	if (!*pixmap_is_a_bitmap) {
		cs->pixmap =
		    XCreatePixmap(dpy, win, cs->width, cs->height, Pdepth);
	...
	if (cs->pixmap) {
		if (cs->picture->mask != None) {
			...
			cs->mask =
			    XCreatePixmap(dpy, win, cs->width, cs->height, 1);
			if (cs->mask) {
...
static void
parse_shape(
    Window win, colorset_t *cs, int i, char *args, int *has_shape_changed)
{
	char *		      token;
	FvwmPicture *	      picture;
	FvwmPictureAttributes fpa;
...
parse_colorset(int n, char *line)
{
	...
	while (line && *line) {
		...
		case 5: /* bg */
			get_simple_color(
			    args, &bg, cs, BG_SUPPLIED, BG_AVERAGE, "average");
			has_bg_changed = True;
			break;
		case 6: /* Hilight */
		case 7: /* Hilite */
	...
	if (has_bg_changed
		...
		if ((cs->color_flags & BG_AVERAGE) && average_pix) {
			...
			colors =
			    fxmalloc(cs->width * cs->height * sizeof(XColor));
	...
	if (has_fg_tint_changed
		...
		if (cs->fg_tint_percent == 0) {
			...
			cs->fg = GetTintedPixel(
			    cs->fg_saved, cs->fg_tint, cs->fg_tint_percent);
			if (old_fg != cs->fg) {
				have_pixels_changed = True;
				has_fg_changed	    = 1;
			}
...
alloc_colorset(int n)
{
	...
	if (n < nColorsets) {
		...
		Colorset =
		    fxrealloc((void *)Colorset, (n + 1), sizeof(colorset_t));
