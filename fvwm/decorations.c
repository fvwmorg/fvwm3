...
GetMwmHints(FvwmWindow *t)
{
	...
	if (XGetWindowProperty(dpy, FW_W(t), _XA_MwmAtom, 0L, 32L, False,
		_XA_MwmAtom, &actual_type, &actual_format, &nitems, &bytesafter,
		(unsigned char **)&t->mwm_hints)
	    == Success) {
...
GetOlHints(FvwmWindow *t)
{
	...
	if (XGetWindowProperty(dpy, FW_W(t), _XA_OL_WIN_ATTR, 0L, 32L, False,
		_XA_OL_WIN_ATTR, &actual_type, &actual_format, &nitems,
		&bytesafter, (unsigned char **)&hints)
	    == Success) {
		if (nitems > 0) {
 ...
 */
void
SelectDecor(FvwmWindow *t, window_style *pstyle, short *buttons)
{
	int	      decor;
	int	      i;
	int	      border_width;
	...
	border_width = (SHAS_BORDER_WIDTH(sflags)) ? SGET_BORDER_WIDTH(*pstyle)
						   : DEFAULT_BORDER_WIDTH;
	if (border_width > MAX_BORDER_WIDTH) {
		border_width = MAX_BORDER_WIDTH;
	}
	handle_width = (SHAS_HANDLE_WIDTH(sflags)) ? SGET_HANDLE_WIDTH(*pstyle)
	...
	*buttons = (1 << NUMBER_OF_TITLE_BUTTONS) - 1;
	...
	if (SHAS_OL_DECOR(sflags) && (t->ol_hints & OL_ANY_HINTS)) {
		...
		if (t->ol_hints & OL_DECOR_HEADER) {
			...
			t->functions &=
			    ~(MWM_DECOR_MENU | MWM_FUNC_MINIMIZE
				| MWM_FUNC_MAXIMIZE | MWM_DECOR_TITLE);
			decor &= ~(MWM_DECOR_MENU | MWM_FUNC_MINIMIZE
				   | MWM_FUNC_MAXIMIZE | MWM_DECOR_TITLE);
		}
	...
	if (!(decor & MWM_DECOR_MINIMIZE)) {
		...
		int i;
		for (i = 0; i < NUMBER_OF_TITLE_BUTTONS; ++i) {
			if (TB_HAS_MWM_DECOR_MINIMIZE(
				GetDecor(t, buttons[i]))) {
				*buttons &= ~(1 << i);
			}
		}
	}
...
*/
Bool
is_function_allowed(int function, char *action_string, const FvwmWindow *t,
    request_origin_t request_origin, Bool do_allow_override_mwm_hints)
{
	unsigned int functions;
	char *functionlist[] = {MOVE_STRING, RESIZE_STRING1, RESIZE_STRING2,
	    MINIMIZE_STRING, MINIMIZE_STRING2, MAXIMIZE_STRING, CLOSE_STRING1,
	    CLOSE_STRING2, CLOSE_STRING3, CLOSE_STRING4, NULL};
