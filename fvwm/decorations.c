...
SelectDecor(FvwmWindow *t, window_style *pstyle, short *buttons)
{
	...
	*buttons = (1 << NUMBER_OF_TITLE_BUTTONS) - 1;
	...
	if (t->functions & MWM_FUNC_ALL) {
		...
		t->functions &= ~MWM_FUNC_ALL;
		t->functions = (MWM_FUNC_RESIZE | MWM_FUNC_MOVE |
...
static Bool
__is_resize_allowed(const FvwmWindow *t, int functions,
    ...
    request_origin_t request_origin, Bool do_allow_override_mwm_hints)
{
	...
	char *functionlist[] = { MOVE_STRING, RESIZE_STRING1, RESIZE_STRING2,
		MINIMIZE_STRING, MINIMIZE_STRING2, MAXIMIZE_STRING,
		CLOSE_STRING1, CLOSE_STRING2, CLOSE_STRING3, CLOSE_STRING4,
		NULL };
