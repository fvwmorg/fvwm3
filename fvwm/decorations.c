...
void GetMwmHints(FvwmWindow *t)
{
	...
	if (XGetWindowProperty(
		    dpy, FW_W(t), _XA_MwmAtom, 0L, 20L, False,
...
void GetOlHints(FvwmWindow *t)
{
	...
	if (XGetWindowProperty(
		    dpy, FW_W(t), _XA_OL_WIN_ATTR, 0L, 20L, False,
	...
	if (XGetWindowProperty(
		    dpy, FW_W(t), _XA_OL_DECOR_ADD, 0L, 20L, False,
	...
	if (XGetWindowProperty(
		    dpy, FW_W(t), _XA_OL_DECOR_DEL, 0L, 20L, False,
		    XA_ATOM, &actual_type, &actual_format, &nitems,
		    &bytesafter,(unsigned char **)&hints)==Success)
	{
		for (i = 0; i < nitems; i++)
