    ...
    int freeOldPixmap)
{
	...
	*nry = 0;
	...
	case ICON_RESIZE_TYPE_STRETCHED:
		if (width < newWidth || height < newHeight) {
			...
			resizedPixmap = CreateStretchPixmap(
			    dpy, oldPixmap, width, height, depth, r_w, r_h, gc);
			width  = r_w;
			height = r_h;
		}
		break;
	...
	if (resize_type != ICON_RESIZE_TYPE_ADJUSTED) {
		*icon =
		    XCreatePixmap(dpy, oldPixmap, newWidth, newHeight, depth);
...
GetIconPicture(FvwmWindow *fw, Bool no_icon_window)
{
	...
	if (ICON_OVERRIDE_MODE(fw) == ICON_OVERRIDE) {
		...
		if (fw->wmhints && (fw->wmhints->flags & IconPixmapHint)
			...
			icon_order[3] = 0;
			ICON_DBG(
			    (stderr, "ciw: hint order: iwh iph file '%s'\n",
				fw->name.name));
			...
			icon_order[3] = 3;
			ICON_DBG(
			    (stderr, "ciw: hint order: iwh file iph '%s'\n",
				fw->name.name));
		}
	} else {
	...
	for (i = 0; i < 4 && fw->icon_g.picture_w_g.width == 0
		...
		case 0:
			...
			ICON_DBG((stderr, "ciw: file%s used '%s'\n",
			    (fw->icon_g.picture_w_g.height) ? "" : " not",
			    fw->name.name));
			break;
		case 1:
	...
	if ((IS_ICON_OURS(fw)) && fw->icon_g.picture_w_g.height > 0
		...
		if (resize) {
			...
			ICON_DBG((stderr,
			    ...
			    " %dx%d\n",
			    fw->name.name, fw->icon_g.picture_w_g.width,
			    fw->icon_g.picture_w_g.height, newWidth,
			    newHeight));
			/* Resize the icon Pixmap */
...
CreateIconWindow(FvwmWindow *fw, int def_x, int def_y)
{
	 ...
	 */
	valuemask =
	    CWColormap | CWBorderPixel | CWBackPixel | CWCursor | CWEventMask;
	...
	if (Scr.DefaultColorset >= 0) {
		SetWindowBackground(dpy, FW_W_ICON_TITLE(fw),
		    ...
		    &Colorset[Scr.DefaultColorset], Pdepth, Scr.StdGC, False);
	}
	...
	if (FW_W_ICON_PIXMAP(fw) != None
		...
		XDefineCursor(
		    dpy, FW_W_ICON_PIXMAP(fw), Scr.FvwmCursors[CRS_DEFAULT]);
    ...
    GC Relief, int cs, int title_cs)
{
	...
	if (is_expanded && FW_W_ICON_PIXMAP(fw) != None) {
		...
		if (w_title_w <= fw->icon_g.picture_w_g.width) {
			...
			FScreenGetScrRect(
			    NULL, FSCREEN_CURRENT, &sx, &sy, &sw, &sh);
	...
	if (is_stippled) {
		...
		if (w_stipple == 0) {
			w_stipple =
			    ((w_title_w - 2 * (x_stipple + w_title_text_gap)
				 - w_title)
				+ 1)
			    / 2;
		}
	...
	if (is_stippled) {
		...
		int num =
		    (pseudo_height / ICON_TITLE_STICK_VERT_DIST / 2) * 2 - 1;
    ...
    FvwmWindow *fw, Bool reset_bg, XEvent *pev, GC Shadow, GC Relief, int cs)
{
	...
	if (!pev) {
		...
		if (reset_bg
			...
			set_icon_pixmap_background(fw);
			XClearArea(
			    dpy, FW_W_ICON_PIXMAP(fw), 0, 0, 0, 0, False);
			cleared = True;
		}
	}
    ...
    Bool focus_change, Bool reset_bg, XEvent *pev)
{
		...
		if (pev && pev->xexpose.window != FW_W_ICON_TITLE(fw)) {
			...
			if (!pev) {
				FCheckWeedTypedWindowEvents(
				    dpy, FW_W_ICON_TITLE(fw), Expose, NULL);
			}
			DrawIconTitleWindow(
			    fw, pev, BackColor, Shadow, Relief, cs, title_cs);
		}
	}
	...
	if (focus_change && draw_pixmap) {
		...
		if (!relief_change && (fw->iconPixmap != None)
			...
			relief_change =
			    (draw_colors.hilight != co_draw_colors.hilight)
	...
	if (draw_pixmap && FW_W_ICON_PIXMAP(fw) != None) {
		if (pev && pev->xexpose.window != FW_W_ICON_PIXMAP(fw)) {
			...
			if (!pev) {
				FCheckWeedTypedWindowEvents(
				    dpy, FW_W_ICON_PIXMAP(fw), Expose, NULL);
			}
			DrawIconPixmapWindow(
...
void
AutoPlaceIcon(
    FvwmWindow *t, initial_window_options_t *win_opts, Bool do_move_immediately)
{
	int	    base_x, base_y;
	int	    width, height;
	FvwmWindow *test_fw;
...
GetIconWindow(FvwmWindow *fw)
{
	...
	if (XGetGeometry(dpy, fw->wmhints->icon_window, &JunkRoot, &JunkX,
		...
		(unsigned int *)&bw, (unsigned int *)&JunkDepth)
	    == 0) {
		fvwm_debug(__func__,
		...
		if (t == fw || IS_IN_TRANSIENT_SUBTREE(t)) {
			...
			if (t->Desk == t->m->virtual_scr.CurrentDesk) {
				...
				if (IsRectangleOnThisPage(t->m, &r, t->Desk)
					...
					t->m, &(t->g.frame), t->Desk)) {
					/* Make sure we keep it on screen when
					 * de-iconifying. */
					t->g.frame.x -= truncate_to_multiple(
					    t->g.frame.x,
					    t->m->virtual_scr.MyDisplayWidth);
					t->g.frame.y -= truncate_to_multiple(
					    t->g.frame.y,
					    t->m->virtual_scr.MyDisplayHeight);
	...
	for (t = Scr.FvwmRoot.next; t != NULL; t = t->next) {
		if (t == fw || IS_IN_TRANSIENT_SUBTREE(t)) {
			...
			border_draw_decorations(
			    t, PART_ALL, False, False, CLEAR_ALL, NULL, NULL);
	...
	if (IS_ICONIFIED(fw)) {
		...
		if (toggle == 1) {
			...
			if (!is_function_allowed(
				F_ICONIFY, NULL, fw, RQORIG_PROGRAM, True)) {
				XBell(dpy, 0);
