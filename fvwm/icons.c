...
static void
clear_icon_dimensions(FvwmWindow *fw)
{
	int px;
	int py;
	int tx;
...
int
get_visible_icon_window_count(FvwmWindow *fw)
{
	int count = 0;
...
void
setup_icon_title_size(FvwmWindow *fw)
{
	if (HAS_NO_ICON_TITLE(fw)) {
		...
		fw->icon_g.title_text_width = FlocaleTextWidth(fw->icon_font,
		    fw->visible_icon_name, strlen(fw->visible_icon_name));
 ...
 */
static void
SetIconPixmapSize(Pixmap *icon, int width, int height, int depth, int newWidth,
    int newHeight, Bool force_centering, int resize_type, int *nrx, int *nry,
    int freeOldPixmap)
{
	Pixmap	  oldPixmap;
	...
	*nry = 0;
	...
	case ICON_RESIZE_TYPE_ADJUSTED:
		if (newWidth != width || newHeight != height) {
			*icon = CreateStretchPixmap(dpy, oldPixmap, width,
			    height, depth, newWidth, newHeight, gc);
		}
		break;
	case ICON_RESIZE_TYPE_STRETCHED:
		if (width < newWidth || height < newHeight) {
			...
			resizedPixmap = CreateStretchPixmap(dpy, oldPixmap,
			    width, height, depth, r_w, r_h, gc);
			width	      = r_w;
			height	      = r_h;
		}
		break;
	...
	if (resize_type != ICON_RESIZE_TYPE_ADJUSTED) {
		*icon = XCreatePixmap(dpy, oldPixmap, newWidth, newHeight,
		    depth);
		...
		*nry = (newHeight > height && !force_centering) ?
		    newHeight - height :
		    (newHeight - height) / 2;
		XCopyArea(dpy, (resizedPixmap) ? resizedPixmap : oldPixmap,
		    *icon, gc, 0, 0, width, height, *nrx, *nry);
	}
 ...
 */
void
GetIconPicture(FvwmWindow *fw, Bool no_icon_window)
{
	char icon_order[4];
	int  i;
	...
	for (i = 0; i < 4 && fw->icon_g.picture_w_g.width == 0 &&
		...
		case 2:
			...
			if (HAS_EWMH_WM_ICON_HINT(fw) == EWMH_TRUE_ICON) {
				if (EWMH_SetIconFromWMIcon(fw, NULL, 0,
					False)) {
		...
		if (resize) {
			...
			if (fw->icon_maskPixmap) {
				SetIconPixmapSize(&(fw->icon_maskPixmap),
				    fw->icon_g.picture_w_g.width,
				    fw->icon_g.picture_w_g.height, 1, newWidth,
				    newHeight, force_centering,
				    fw->icon_resize_type, &nrx, &nry,
				    IS_PIXMAP_OURS(fw));
			} else if ((nrx > 0 || nry > 0) && fw->iconDepth > 1) {
				fw->icon_maskPixmap = XCreatePixmap(dpy,
				    fw->iconPixmap, newWidth, newHeight, 1);
				XSetForeground(dpy, Scr.MonoGC, 0);
				XFillRectangle(dpy, fw->icon_maskPixmap,
				    Scr.MonoGC, 0, 0, newWidth, newHeight);
			...
			if (fw->icon_alphaPixmap) {
				SetIconPixmapSize(&(fw->icon_alphaPixmap),
				    fw->icon_g.picture_w_g.width,
				    fw->icon_g.picture_w_g.height,
				    FRenderGetAlphaDepth(), newWidth, newHeight,
				    force_centering, fw->icon_resize_type, &nrx,
				    &nry, IS_PIXMAP_OURS(fw));
			}
 ...
 */
static void
set_icon_pixmap_background(FvwmWindow *fw)
{
	int cs = 0;
	...
	if (fw->iconPixmap != None &&
		...
		IS_PIXMAP_OURS(fw))) {
		if (fw->icon_background_cs >= 0) {
			SetWindowBackground(dpy, FW_W_ICON_PIXMAP(fw),
			    fw->icon_g.picture_w_g.width,
			    fw->icon_g.picture_w_g.height,
			    &Colorset[fw->icon_background_cs], Pdepth,
			    Scr.StdGC, False);
			...
			SetWindowBackground(dpy, FW_W_ICON_PIXMAP(fw),
			    fw->icon_g.picture_w_g.width,
			    fw->icon_g.picture_w_g.height, &Colorset[cs],
 ...
 */
void
CreateIconWindow(FvwmWindow *fw, int def_x, int def_y)
{
	...
	if (HAS_NO_ICON_TITLE(fw)) {
		...
		if (FW_W_ICON_TITLE(fw) == None) {
			FW_W_ICON_TITLE(fw) = XCreateWindow(dpy, Scr.Root,
			    fw->icon_g.title_w_g.x, fw->icon_g.title_w_g.y,
			    fw->icon_g.title_w_g.width,
			    fw->icon_g.title_w_g.height, 0, Pdepth, InputOutput,
			    Pvisual, valuemask, &attributes);
		} else {
			XMoveResizeWindow(dpy, FW_W_ICON_TITLE(fw),
			    fw->icon_g.title_w_g.x, fw->icon_g.title_w_g.y,
			    fw->icon_g.title_w_g.width,
			    fw->icon_g.title_w_g.height);
		}
	}
	SetWindowBackground(dpy, FW_W_ICON_TITLE(fw),
	    fw->icon_g.title_w_g.width, fw->icon_g.title_w_g.height,
	    &Colorset[cs], Pdepth, Scr.StdGC, False);
	...
	if (IS_ICON_OURS(fw) && fw->icon_g.picture_w_g.width > 0 &&
		...
		if (Pdefault || fw->iconDepth == 1 || fw->iconDepth == Pdepth ||
			...
			if (!old_icon_pixmap_w) {
				FW_W_ICON_PIXMAP(fw) = XCreateWindow(dpy,
				    Scr.Root, fw->icon_g.picture_w_g.x,
				    fw->icon_g.picture_w_g.y,
				    fw->icon_g.picture_w_g.width,
				    fw->icon_g.picture_w_g.height, 0, Pdepth,
				    InputOutput, Pvisual, valuemask,
				    &attributes);
			} else {
				FW_W_ICON_PIXMAP(fw) = old_icon_pixmap_w;
				XMoveResizeWindow(dpy, FW_W_ICON_PIXMAP(fw),
			...
			FW_W_ICON_PIXMAP(fw) = XCreateWindow(dpy, Scr.Root,
			    fw->icon_g.picture_w_g.x, fw->icon_g.picture_w_g.y,
			    fw->icon_g.picture_w_g.width,
			    fw->icon_g.picture_w_g.height, 0,
		...
		XChangeWindowAttributes(dpy, FW_W_ICON_PIXMAP(fw), valuemask,
		    &attributes);
		if (!IS_ICON_OURS(fw)) {
			XMoveWindow(dpy, FW_W_ICON_PIXMAP(fw),
			    fw->icon_g.picture_w_g.x, fw->icon_g.picture_w_g.y);
		}
	}
	...
	if (FShapesSupported) {
		if (IS_ICON_SHAPED(fw) && fw->icon_background_cs < 0) {
			...
			FShapeCombineMask(dpy, FW_W_ICON_PIXMAP(fw),
			    FShapeBounding, off, off, fw->icon_maskPixmap,
			    FShapeSet);
			...
			FShapeCombineRectangles(dpy, FW_W_ICON_PIXMAP(fw),
			    FShapeBounding, 0, 0, &r, 1, FShapeSet, 0);
		}
	}

	if (FW_W_ICON_TITLE(fw) != None && FW_W_ICON_TITLE(fw) != old_icon_w) {
		XSaveContext(dpy, FW_W_ICON_TITLE(fw), FvwmContext,
		    (caddr_t)fw);
		XDefineCursor(dpy, FW_W_ICON_TITLE(fw),
		    Scr.FvwmCursors[CRS_DEFAULT]);
		GrabAllWindowKeysAndButtons(dpy, FW_W_ICON_TITLE(fw),
		    Scr.AllBindings, C_ICON, GetUnusedModifiers(),
		    Scr.FvwmCursors[CRS_DEFAULT], True);
		xwc.sibling    = FW_W_FRAME(fw);
		xwc.stack_mode = Below;
		XConfigureWindow(dpy, FW_W_ICON_TITLE(fw),
		    CWSibling | CWStackMode, &xwc);
	}
	if (FW_W_ICON_PIXMAP(fw) != None &&
	    FW_W_ICON_PIXMAP(fw) != old_icon_pixmap_w) {
		XSaveContext(dpy, FW_W_ICON_PIXMAP(fw), FvwmContext,
		    (caddr_t)fw);
		XDefineCursor(dpy, FW_W_ICON_PIXMAP(fw),
		    Scr.FvwmCursors[CRS_DEFAULT]);
		GrabAllWindowKeysAndButtons(dpy, FW_W_ICON_PIXMAP(fw),
		    Scr.AllBindings, C_ICON, GetUnusedModifiers(),
		    Scr.FvwmCursors[CRS_DEFAULT], True);
		xwc.sibling    = FW_W_FRAME(fw);
		xwc.stack_mode = Below;
 ...
 */
static void
DrawIconTitleWindow(FvwmWindow *fw, XEvent *pev, Pixel BackColor, GC Shadow,
    GC Relief, int cs, int title_cs)
{
	...
	is_sticky = (IS_STICKY_ACROSS_PAGES(fw) ||
	    IS_ICON_STICKY_ACROSS_PAGES(fw));
	is_sticky |= (IS_STICKY_ACROSS_DESKS(fw) ||
	    IS_ICON_STICKY_ACROSS_DESKS(fw));
	...
	if (is_expanded && FW_W_ICON_PIXMAP(fw) != None) {
		...
		if (w_title_w <= fw->icon_g.picture_w_g.width) {
			...
			FScreenGetScrRect(NULL, FSCREEN_CURRENT, &sx, &sy, &sw,
			    &sh);
	...
	if (!pev) {
		XMoveResizeWindow(dpy, FW_W_ICON_TITLE(fw),
		    fw->icon_g.title_w_g.x, fw->icon_g.title_w_g.y, w_title_w,
		    ICON_HEIGHT(fw));
	}

	if (title_cs >= 0) {
		SetWindowBackground(dpy, FW_W_ICON_TITLE(fw), w_title_w,
		    ICON_HEIGHT(fw), &Colorset[title_cs], Pdepth, Scr.TitleGC,
		    False);
	} else {
	...
	if (pev || is_stippled) {
		if (pev) {
			if (!frect_get_intersection(pev->xexpose.x,
				pev->xexpose.y, pev->xexpose.width,
				pev->xexpose.height, r.x, r.y, r.width,
				r.height, &clip)) {
				draw_string = False;
			}
		} else {
		...
		if (draw_string) {
			XSetClipRectangles(dpy, Scr.TitleGC, 0, 0, &clip, 1,
			    Unsorted);
	...
	if (!pev) {
		...
		if (x_title - relief >= 1) {
			...
			XClearArea(dpy, FW_W_ICON_TITLE(fw), relief, relief,
			    x_title - relief, ICON_HEIGHT(fw) - 2 * relief,
			    False);
		}
		if (is_stippled) {
			...
			XClearArea(dpy, FW_W_ICON_TITLE(fw),
			    ...
			    w_stipple + 2, ICON_HEIGHT(fw) - 2 * relief, False);
		}
	}
	...
	if (draw_string) {
		if (pev) {
			...
			XClearArea(dpy, FW_W_ICON_TITLE(fw), clip.x, clip.y,
	...
	RelieveRectangle(dpy, FW_W_ICON_TITLE(fw), 0, 0, w_title_w - 1,
	    ICON_HEIGHT(fw) - 1, (fw->icon_title_relief > 0) ? Relief : Shadow,
	...
	if (is_stippled) {
		...
		int num = (pseudo_height / ICON_TITLE_STICK_VERT_DIST / 2) * 2 -
		    1;
		int min = ICON_HEIGHT(fw) / 2 - num * 2 + 1;
		int max = ICON_HEIGHT(fw) / 2 + num * 2 -
		    ICON_TITLE_STICK_VERT_DIST + 1;
		int i;

		for (i = min; w_stipple > 0 && i <= max;
		     i += ICON_TITLE_STICK_VERT_DIST) {
			RelieveRectangle(dpy, FW_W_ICON_TITLE(fw), x_stipple, i,
			    w_stipple - 1, 1, Shadow, Relief,
			    ICON_TITLE_STICK_HEIGHT);
			RelieveRectangle(dpy, FW_W_ICON_TITLE(fw),
			    w_title_w - x_stipple - w_stipple, i, w_stipple - 1,
			    1, Shadow, Relief, ICON_TITLE_STICK_HEIGHT);
		}
	}
...
static void
DrawIconPixmapWindow(FvwmWindow *fw, Bool reset_bg, XEvent *pev, GC Shadow,
    GC Relief, int cs)
{
	XRectangle r, clip;
	Bool	   cleared = False;
	...
	if (!pev) {
		XMoveWindow(dpy, FW_W_ICON_PIXMAP(fw), fw->icon_g.picture_w_g.x,
		    fw->icon_g.picture_w_g.y);
		if (reset_bg &&
		    (fw->iconDepth == 1 || fw->iconDepth == Pdepth ||
			Pdefault || IS_PIXMAP_OURS(fw))) {
			set_icon_pixmap_background(fw);
			XClearArea(dpy, FW_W_ICON_PIXMAP(fw), 0, 0, 0, 0,
			    False);
			cleared = True;
		}
	}
	...
	if (fw->iconPixmap != None) {
		if (fw->iconDepth == 1 || fw->iconDepth == Pdepth || Pdefault ||
			...
			if (draw_icon) {
				if (!cleared &&
					...
					(cs >= 0 &&
						...
						100))) {
					XClearArea(dpy, FW_W_ICON_PIXMAP(fw),
					    clip.x, clip.y, clip.width,
					    clip.height, False);
				}
				PGraphicsRenderPixmaps(dpy,
				    FW_W_ICON_PIXMAP(fw), fw->iconPixmap,
				    fw->icon_maskPixmap, fw->icon_alphaPixmap,
				    fw->iconDepth, &fra, FW_W_ICON_PIXMAP(fw),
				    Scr.TitleGC, Scr.MonoGC, Scr.AlphaGC,
				    clip.x - r.x, clip.y - r.y, clip.width,
				    clip.height, clip.x, clip.y, clip.width,
				    clip.height, False);
			}
		} else {
			...
			if (Scr.bo.do_install_root_cmap)
				InstallRootColormap();
			XCopyArea(dpy, fw->iconPixmap, FW_W_ICON_PIXMAP(fw),
...
void
DrawIconWindow(FvwmWindow *fw, Bool draw_title, Bool draw_pixmap,
    Bool focus_change, Bool reset_bg, XEvent *pev)
{
	...
	if (IS_ICON_SUPPRESSED(fw) ||
	    (pev && fw->Desk != fw->m->virtual_scr.CurrentDesk)) {
		return;
	}
	...
	if (draw_title && FW_W_ICON_TITLE(fw) != None) {
		if (pev && pev->xexpose.window != FW_W_ICON_TITLE(fw)) {
			XEvent e;
			if (FCheckTypedWindowEvent(dpy, FW_W_ICON_TITLE(fw),
				Expose, &e)) {
				flush_accumulate_expose(FW_W_ICON_TITLE(fw),
				    &e);
				DrawIconTitleWindow(fw, &e, BackColor, Shadow,
				    Relief, cs, title_cs);
			}
		} else {
			if (!pev) {
				FCheckWeedTypedWindowEvents(dpy,
				    FW_W_ICON_TITLE(fw), Expose, NULL);
			}
			DrawIconTitleWindow(fw, pev, BackColor, Shadow, Relief,
			    cs, title_cs);
		}
	}
	...
	if (draw_pixmap) {
		...
		if (bg_cs >= 0 &&
		    (fw->iconDepth != 1 || fw->icon_background_padding > 0 ||
			fw->icon_maskPixmap != None ||
			fw->icon_alphaPixmap != None)) {
			if (Pdepth < 2 && Scr.Hilite == fw) {
				Relief = Scr.ScratchGC2;
				...
				Globalgcm	     = GCForeground;
				XChangeGC(dpy, Scr.ScratchGC1, Globalgcm,
				    &Globalgcv);
				Relief = Scr.ScratchGC1;
			}
	...
	if (focus_change && draw_pixmap) {
		...
		if (Pdepth < 2) {
			...
			color_change = (draw_colors.fore !=
					   co_draw_colors.back) ||
			    (draw_colors.fore != co_draw_colors.back);
		}
		if (!relief_change && (fw->iconPixmap != None) &&
		    !IS_ICON_SHAPED(fw) &&
		    (Pdefault || fw->iconDepth == Pdepth ||
			fw->iconDepth == 1 || IS_PIXMAP_OURS(fw))) {
			relief_change = (draw_colors.hilight !=
					    co_draw_colors.hilight) ||
			    (draw_colors.shadow != co_draw_colors.shadow);
		}
		if (cs >= 0 && co_cs >= 0) {
			alpha_change = (Colorset[cs].icon_alpha_percent !=
			    Colorset[co_cs].icon_alpha_percent);
			tint_change  = (Colorset[cs].icon_tint_percent !=
			...
			alpha_change = (Colorset[co_cs].icon_alpha_percent <
			    100);
	...
	if (draw_pixmap && FW_W_ICON_PIXMAP(fw) != None) {
		if (pev && pev->xexpose.window != FW_W_ICON_PIXMAP(fw)) {
			XEvent e;
			if (FCheckTypedWindowEvent(dpy, FW_W_ICON_PIXMAP(fw),
				Expose, &e)) {
				flush_accumulate_expose(FW_W_ICON_PIXMAP(fw),
				    &e);
				DrawIconPixmapWindow(fw, reset_bg, &e, Shadow,
				    Relief, cs);
			}
		} else {
			if (!pev) {
				FCheckWeedTypedWindowEvents(dpy,
				    FW_W_ICON_PIXMAP(fw), Expose, NULL);
			}
			DrawIconPixmapWindow(fw, reset_bg, pev, Shadow, Relief,
			    cs);
		}
	}
 ...
 */
void
ChangeIconPixmap(FvwmWindow *fw)
{
	rectangle g;
 ...
 */
void
AutoPlaceIcon(FvwmWindow *t, initial_window_options_t *win_opts,
    Bool do_move_immediately)
{
  ...
  if (FW_W_ICON_PIXMAP(t) == None && FW_W_ICON_TITLE(t) == None)
  {
	...
	/* Not a good idea for StickyIcons. Neither for icons of windows that
	...
	if (IS_ICON_STICKY_ACROSS_PAGES(t) || IS_STICKY_ACROSS_PAGES(t)) {
		...
		frame_setup_window(t, new_x, new_y, t->g.frame.width,
		    t->g.frame.height, False);
	} else if (IsRectangleOnThisPage(t->m, &(t->g.frame), t->Desk)) {
		base_x = 0;
		base_y = 0;
	} else {
		base_x = ((t->g.frame.x + t->m->virtual_scr.Vx +
			      (t->g.frame.width >> 1)) /
			     monitor_get_all_widths()) *
		    monitor_get_all_widths();
		base_y = ((t->g.frame.y + t->m->virtual_scr.Vy +
			      (t->g.frame.height >> 1)) /
...
#define HRZ_FILL icon_boxes_ptr->IconFlags &ICONFILLHRZ
		...
		while (do_all_iconboxes(t, &icon_boxes_ptr)) {
			...
			FScreenGetScrRect(fscr, FSCREEN_CURRENT, &ref.x, &ref.y,
			...
			for (i = 1; i <= 2; i++) {
				...
				if (dim[i].icon_dimension >=
					...
					dim[i].nom_dimension = dim[i].end_at -
					    dim[i].start_at - 1;
				}
				if (dim[i].step < 0) {
					/* if moving backwards */
			...
			while (
				...
				if (dim[1].start_at <
				    dim[1].screen_offset + dim[1].base) {
				...
				while (
				    (dim[2].step < 0 /* filling reversed */
						...
						dim[2].end_at)) /* check front
								   edge */
				    && (!loc_ok) &&
				    (!loc_ok_wrong_screen2)) { /* nothing found
								  yet */
					dim[2].real_start =
					    dim[2].start_at; /* init */
					if (dim[2].start_at +
					...
					if (FScreenIsRectangleOnScreen(fscr,
						FSCREEN_XYPOS, &ref)) {
						loc_ok = True;
					} else {
...
GetIconWindow(FvwmWindow *fw)
{
	 ...
	 * routine */
	if (XGetGeometry(dpy, fw->wmhints->icon_window, &JunkRoot, &JunkX,
		&JunkY, (unsigned int *)&w, (unsigned int *)&h,
		(unsigned int *)&bw, (unsigned int *)&JunkDepth) == 0) {
		fvwm_debug(__func__,
...
GetIconBitmap(FvwmWindow *fw)
{
	 ...
	 */
	if (!XGetGeometry(dpy, fw->wmhints->icon_pixmap, &JunkRoot, &JunkX,
		&JunkY, (unsigned int *)&width, (unsigned int *)&height,
...
DeIconify(FvwmWindow *fw)
{
	...
	for (t = Scr.FvwmRoot.next; t != NULL; t = t->next) {
		if (t == fw || IS_IN_TRANSIENT_SUBTREE(t)) {
			...
			if (Scr.Hilite == t) {
				border_draw_decorations(t, PART_ALL, False,
			...
			if (t->Desk == t->m->virtual_scr.CurrentDesk) {
				...
				if (IsRectangleOnThisPage(t->m, &r, t->Desk) &&
				    !IsRectangleOnThisPage(t->m, &(t->g.frame),
					t->Desk)) {
					/* Make sure we keep it on screen when
					 * de-iconifying. */
					t->g.frame.x -=
					    truncate_to_multiple(t->g.frame.x,
						monitor_get_all_widths());
					t->g.frame.y -=
					    truncate_to_multiple(t->g.frame.y,
						monitor_get_all_heights());
					XMoveWindow(dpy, FW_W_FRAME(t),
			...
			if (HAS_NO_ICON_TITLE(t) &&
			    FW_W_ICON_PIXMAP(t) == None) {
				memset(&fw->icon_g, 0, sizeof(fw->icon_g));
			}
			...
			if (t == fw) {
				BroadcastPacket(M_DEICONIFY, 11, (long)FW_W(t),
				    (long)FW_W_FRAME(t), (unsigned long)t,
				    (long)icon_rect.x, (long)icon_rect.y,
				    (long)icon_rect.width,
				    (long)icon_rect.height, (long)t->g.frame.x,
				    (long)t->g.frame.y, (long)t->g.frame.width,
				    (long)t->g.frame.height);
			} else {
				BroadcastPacket(M_DEICONIFY, 7, (long)FW_W(t),
				    (long)FW_W_FRAME(t), (unsigned long)t,
				    (long)icon_rect.x, (long)icon_rect.y,
				    (long)icon_rect.width,
			...
			tmp	   = Scr.Hilite;
			Scr.Hilite = t;
			border_draw_decorations(t, PART_ALL,
			    (sf == t) ? True : False, True, CLEAR_ALL, NULL,
			    NULL);
			Scr.Hilite = tmp;
		}
	}
 ...
 */
void
Iconify(FvwmWindow *fw, initial_window_options_t *win_opts)
{
	FvwmWindow	 *t;
	FvwmWindow	 *sf;
	XWindowAttributes winattrs = { 0 };
	...
	if (sf && IS_IN_TRANSIENT_SUBTREE(sf)) {
		 ...
		 * it again. */
		mark_transient_subtree(fw, MARK_ALL_LAYERS, MARK_ALL, False,
		    True);
	}
	...
	for (t = Scr.FvwmRoot.next; t != NULL; t = t->next) {
		if (t == fw || IS_IN_TRANSIENT_SUBTREE(t)) {
			...
			if (t == fw && !IS_ICONIFIED_BY_PARENT(fw)) {
				...
				get_icon_geometry(t, &g);
				BroadcastPacket(M_ICONIFY, 7, (long)FW_W(t),
				    (long)FW_W_FRAME(t), (unsigned long)t,
				    (long)-32768, (long)-32768, (long)g.width,
				    (long)g.height);
	...
	if (FW_W_ICON_TITLE(fw) == None || HAS_ICON_CHANGED(fw)) {
		if (IS_ICON_MOVED(fw) || win_opts->flags.use_initial_icon_xy) {
			...
			CreateIconWindow(fw, win_opts->default_icon_x,
	...
	BroadcastPacket(M_ICONIFY, 11, (long)FW_W(fw), (long)FW_W_FRAME(fw),
	    ...
	    (long)fw->g.frame.x, (long)fw->g.frame.y, (long)fw->g.frame.width,
	    (long)fw->g.frame.height);
	BroadcastConfig(M_CONFIGURE_WINDOW, fw);
 ...
 */
void
SetMapStateProp(const FvwmWindow *fw, int state)
{
...
CMD_Iconify(F_CMD_ARGS)
{
	...
	if (IS_ICONIFIED(fw)) {
		...
		if (toggle == 1) {
			...
			fev_get_evpos_or_query(dpy, Scr.Root, NULL,
			    &win_opts.default_icon_x, &win_opts.default_icon_y);
			Iconify(fw, &win_opts);
			EWMH_SetWMState(fw, False);
		}
	}
