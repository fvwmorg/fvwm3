...
static void
init_style(FvwmWindow *old_t, FvwmWindow *t, window_style *pstyle,
    short *pbuttons)
{
...
static void
apply_window_updates(FvwmWindow *t, update_win *flags, window_style *pstyle,
    FvwmWindow *focus_w)
{
	FvwmWindow	       old_t;
	short		       buttons;
	...
	if (flags->do_redecorate || flags->do_update_title_dir) {
		...
		if (flags->do_redecorate) {
			...
			gravity_get_naked_geometry(old_t.hints.win_gravity,
			    &old_t, &naked_g, &t->g.normal);
			...
			gravity_add_decoration(old_t.hints.win_gravity, t,
			    &t->g.normal, &naked_g);
		}
		if (flags->do_update_title_dir) {
			...
			gravity_resize(t->hints.win_gravity, &t->g.normal, dw,
			    dh);
			gravity_constrain_size(t->hints.win_gravity, t,
			    &t->g.normal, 0);
		}
		...
		if (IS_MAXIMIZED(t)) {
			if (flags->do_redecorate) {
				...
				gravity_get_naked_geometry(NorthWestGravity,
				    &old_t, &naked_g, &t->g.max);
				gravity_translate_to_northwest_geometry_no_bw(
				    NorthWestGravity, &old_t, &naked_g,
				    &naked_g);
				gravity_add_decoration(NorthWestGravity, t,
				    &t->g.max, &naked_g);
			...
			if (flags->do_update_title_dir) {
				frame_g = t->g.max;
				gravity_resize(t->hints.win_gravity, &t->g.max,
				    dw, dh);
				gravity_constrain_size(t->hints.win_gravity, t,
				    &t->g.max, CS_UPDATE_MAX_DEFECT);
			}
			new_g = &t->g.max;
		} else {
			new_g = &t->g.normal;
	...
	if (flags->do_setup_frame) {
		...
		frame_force_setup_window(t, frame_g.x, frame_g.y, frame_g.width,
		    frame_g.height, True);
	...
	if (flags->do_update_cr_motion_method) {
		...
		case WS_CR_MOTION_METHOD_STATIC_GRAV:
			SET_CR_MOTION_METHOD(t,
			    SCR_MOTION_METHOD(&pstyle->flags));
...
Bool
update_fvwm_monitor(FvwmWindow *fw)
{
	rectangle	g;
	struct monitor *mnew;
