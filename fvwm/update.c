    ...
    FvwmWindow *t, update_win *flags, window_style *pstyle, FvwmWindow *focus_w)
{
	...
	exc =
	    exc_create_context(&ecc, ECC_TYPE | ECC_FW | ECC_W | ECC_WCONTEXT);
	...
	if (flags->do_redecorate || flags->do_update_title_dir) {
		...
		if (flags->do_redecorate) {
			...
			gravity_add_decoration(
			    old_t.hints.win_gravity, t, &t->g.normal, &naked_g);
		}
		...
		if (IS_MAXIMIZED(t)) {
			if (flags->do_redecorate) {
				...
				gravity_translate_to_northwest_geometry_no_bw(
				    ...
				    &naked_g);
				gravity_add_decoration(
				    NorthWestGravity, t, &t->g.max, &naked_g);
			...
			if (flags->do_update_title_dir) {
				frame_g = t->g.max;
				gravity_resize(
				    t->hints.win_gravity, &t->g.max, dw, dh);
