 ...
 */
static char *
get_display_name(char *display_name, int screen_num)
{
	char *msg;
	char *new_dn;
	char *cp;
...
Done(int restart, char *command)
{
	...
	if (restart) {
		...
		RestartInSession(
		    restart_state_filename, command == NULL, do_preserve_state);
...
InstallSignals(void)
{
	...
	fvwmSetSignalMask(sigmask(SIGUSR1) | sigmask(SIGUSR2) | sigmask(SIGINT)
...
CreateGCs(void)
{
	...
	Scr.ScratchAlphaPixmap =
	    XCreatePixmap(dpy, Scr.Root, 1, 1, FRenderGetAlphaDepth());
...
setVersionInfo(void)
{
	...
	if (support_len > 0) {
		...
		xasprintf(
		    &Fvwm_SupportInfo, "with support for: %s", support_str);
	} else {
...
#define RC_DEFAULTS_COMPLETE ((char *)-1)
	...
	const char *defaults[][3] = {{"XORValue 0", "", ""},
	     ...
	     */
	    {"DefaultFont", "", ""}, {"DefaultColors black grey", "", ""},
	    {DEFAULT_MENU_STYLE, "", ""},
	    ...
	    {"+ \"&1. XTerm\" Exec xterm", "", ""},
	    {"+ \"&2. ", _("Issue fvwm commands"), "\" Module FvwmConsole"},
	    {"+ \"&D. ", _("Desktop Menu"), "\" Popup FvwmMenu"},
	    {"+ \"&R. ", _("Restart fvwm"), "\" Restart"},
	    {"+ \"&X. ", _("Exit fvwm"), "\" Quit"},
...
main(int argc, char **argv)
{
	...
	if (access(fvwm_userdir, W_OK) != 0) {
		fvwm_debug(
		    __func__, "No write permissions in `%s/'.\n", fvwm_userdir);
	}
	...
	if (strncmp(display_string, "DISPLAY=:", 9) == 0) {
		...
		xasprintf(
		    &rdisplay_string, "HOSTDISPLAY=%s", XDisplayString(dpy));
		flib_putenv("HOSTDISPLAY", rdisplay_string);
		free(rdisplay_string);
	}
	free(display_string);
	...
	if (num_config_commands > 0) {
		...
		xasprintf(
		    &cfg_loc[++nl], "%s/default-config/config", FVWM_DATADIR);
	...
	Scr.SizeWindow = XCreateWindow(dpy, Scr.Root, 0, 0, 1, 1, 0, Pdepth,
