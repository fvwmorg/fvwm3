...
typedef enum { FVWM_RUNNING = 0, FVWM_DONE, FVWM_RESTART } fvwm_run_state_t;
...
static const char *init_function_names[4] = { "InitFunction", "RestartFunction",
	"ExitFunction", "Nop" };
...
SaveDesktopState(void)
{
	...
	for (t = Scr.FvwmRoot.next; t != NULL; t = t->next) {
		...
		XChangeProperty(dpy, FW_W(t), _XA_WM_DESKTOP, _XA_WM_DESKTOP,
...
InternUsefulAtoms(void)
{
	...
	_XA_MIT_PRIORITY_COLORS = XInternAtom(dpy, "_MIT_PRIORITY_COLORS",
	    False);
	_XA_WM_CHANGE_STATE	= XInternAtom(dpy, "WM_CHANGE_STATE", False);
	_XA_WM_STATE		= XInternAtom(dpy, "WM_STATE", False);
	_XA_WM_COLORMAP_WINDOWS = XInternAtom(dpy, "WM_COLORMAP_WINDOWS",
	    False);
 ...
 */
static int
parse_command_args(const char *command, char **argv, int max_argc,
...
#define add_arg_str(str)                        \
	...
	*error_msg = "";
	...
	for (argc = 0; argc < max_argc - 1; argc++) {
		...
		while ((s_quote || !isspace(the_char)) && the_char != '\0' &&
		    can_add_arg_char) {
			if (the_char == '"') {
				if (s_quote) {
					s_quote = 0;
 ...
 */
static char *
get_display_name(char *disp_name, int screen_num)
{
	char *msg;
	char *new_dn;
	char *cp;
...
Done(int restart, char *command)
{
	...
	if (restart) {
		...
		if (command) {
			...
			if (strncmp(command, "--dont-preserve-state", 21) ==
			    0) {
		...
		RestartInSession(restart_state_filename, command == NULL,
		    do_preserve_state);
		...
		if (command) {
			...
			const char *error_msg;
			int	    n = parse_command_args(command, my_argv,
				    MAX_ARG_SIZE, &error_msg);
...
InstallSignals(void)
{
	...
	fvwmSetSignalMask(sigmask(SIGUSR1) | sigmask(SIGUSR2) |
 ...
 ************************************************************************/
static void
LoadDefaultLeftButton(DecorFace *df, int i)
{
 ...
 ************************************************************************/
static void
LoadDefaultRightButton(DecorFace *df, int i)
{
	struct vector_coords *v = &df->u.vector;
	int		      j = 0;
...
CreateGCs(void)
{
	...
	Scr.ScratchAlphaPixmap = XCreatePixmap(dpy, Scr.Root, 1, 1,
...
InitVariables(void)
{
	...
	*/
	Scr.bo.is_raise_hack_needed = (strcmp(ServerVendor(dpy),
					   "Hummingbird Communications Ltd.") ==
					  0) ||
	    (strcmp(ServerVendor(dpy), "Network Computing Devices Inc.") ==
		0) ||
	...
	Scr.flags.is_pointer_on_this_screen = !!FQueryPointer(dpy, Scr.Root,
	    &JunkRoot, &JunkChild, &JunkX, &JunkY, &JunkX, &JunkY, &JunkMask);
...
setVersionInfo(void)
{
	...
	snprintf(version_str, sizeof(version_str), "fvwm3 %s (%s)", VERSION,
	    VERSIONINFO);
	...
	if (support_len > 0) {
		...
		xasprintf(&Fvwm_SupportInfo, "with support for: %s",
		    support_str);
	} else {
...
#define RC_DEFAULTS_COMPLETE ((char *)-1)
 ...
 /* The below is historical -- before we had a default
	 ...
	 */
		...
		{ "+ I StartFunction",				       "",			   ""		      },
		{ "AddToMenu MenuFvwmRoot \"",			       _("Builtin Menu"),	  "\" Title"	     },
		{ "+ \"&1. XTerm\" Exec xterm",			"",			    ""	       },
		{ "+ \"&2. FvwmConsole\" Module FvwmConsole",	      "",			  ""		     },
		{ "+ \"&3. ",					 _("Load Default Config"),
		    "\" LoadDefaultConfig"									  },
		{ "+ \"&4. ",					 _("Restart"),		    " Fvwm\" Restart"},
		{ "+ \"&5. ",					 _("Quit"),		 " Fvwm\" Quit"	},
...
static int
CatchRedirectError(Display *disp, XErrorEvent *event)
{
...
static int
FvwmErrorHandler(Display *disp, XErrorEvent *event)
{
	char errtext[512];
	...
	fvwm_debug(__func__,
	    "Request %d, Error %d, Text %s, "
	    "EventType: %d",
	    event->request_code, event->error_code, errtext, last_event_type);
...
#endif
static void
reopen_fd(int fd, char *mode, FILE *of)
{
	struct stat sbuf;
	FILE	   *f;
	int	    rc;
...
main(int argc, char **argv)
{
	...
	for (i = 1; i < argc; i++) {
		if (strcmp(argv[i], "-debug_stack_ring") == 0 ||
			...
			if (num_config_commands < MAX_CFG_CMDS) {
				config_commands[num_config_commands] = (char *)
				    malloc(6 + strlen(argv[i]));
			...
			if (num_config_commands < MAX_CFG_CMDS) {
				config_commands[num_config_commands] = fxstrdup(
				    argv[i]);
				num_config_commands++;
			} else {
				fvwm_debug(__func__,
...

	{
		...
		if (visualClass != -1) {
			...
			vinfo	       = XGetVisualInfo(dpy,
				     VisualScreenMask | VisualClassMask, &template,
				     &total);
			if (!total) {
				fvwm_debug(__func__,
				    "Cannot find visual class %d", visualClass);
			}
		} else if (visualId != -1) {
			template.visualid = visualId;
			vinfo		  = XGetVisualInfo(dpy,
					VisualScreenMask | VisualIDMask, &template, &total);
			if (!total) {
				fvwm_debug(__func__,
				    "VisualId 0x%x is not valid ", visualId);
			}
		}
		...
		if (Pdepth > 0) {
			if (Pvisual->class == DirectColor) {
				Pcmap = XCreateColormap(dpy, Scr.Root, Pvisual,
				    AllocAll);
			} else {
				Pcmap = XCreateColormap(dpy, Scr.Root, Pvisual,
				    AllocNone);
	...
	Scr.ColorLimit	  = 0;
	PUseDynamicColors = 0;
	Scr.ColorLimit	  = PictureInitColors(PICTURE_CALLED_BY_FVWM, True,
	       &colorLimitop, True, True);
	...
	Scr.gray_bitmap = XCreateBitmapFromData(dpy, Scr.Root, g_bits, g_width,
	    g_height);
	...
	if (num_config_commands > 0) {
		...
		for (j = 0; j < num_config_commands; j++) {
			...
			execute_function(NULL, exc, config_commands[j], NULL,
			    0);
		...
		xasprintf(&cfg_loc[++nl], "%s/default-config/config",
		    FVWM_DATADIR);
		...
		if (tries == upper) {
			fvwm_debug(__func__,
			    ...
			    " tried: \n\t%s/%s\n\t%s/%s\n\t%s/%s\n\t"
			    "%s/%s\n\t%s/%s\n\t%s/system%s\n\t%s/system%s",
			    fvwm_userdir, FVWM_CONFIG, FVWM_DATADIR,
			    FVWM_CONFIG, fvwm_userdir, FVWM2RC, home_dir,
			    FVWM2RC, FVWM_DATADIR, FVWM2RC, FVWM_DATADIR,
			    FVWM2RC, FVWM_CONFDIR, FVWM2RC);
		}
		for (nl = 0; nl < upper; nl++)
			free(cfg_loc[nl]);
	}
	...
	if (Pdepth < 2) {
		Scr.gray_pixmap	       = XCreatePixmapFromBitmapData(dpy,
			   Scr.NoFocusWin, g_bits, g_width, g_height,
			   PictureBlackPixel(), PictureWhitePixel(), Pdepth);
		Scr.light_gray_pixmap  = XCreatePixmapFromBitmapData(dpy,
		     Scr.NoFocusWin, l_g_bits, l_g_width, l_g_height,
		     PictureBlackPixel(), PictureWhitePixel(), Pdepth);
		Scr.sticky_gray_pixmap = XCreatePixmapFromBitmapData(dpy,
		    Scr.NoFocusWin, s_g_bits, s_g_width, s_g_height,
		    PictureBlackPixel(), PictureWhitePixel(), Pdepth);
	}
	...
	Scr.SizeWindow.win = XCreateWindow(dpy, Scr.Root, 0, 0, 1, 1, 0, Pdepth,
